...
lupopedia.headers:
  ...
  last_modified_utc: "20260415014600"
  channel_id: 42
  actor_id: 102
  actor_name: "cursor"
...
lupopedia.edges:
  ...
  - to: lupo-includes/classes/DatabaseFactory.php
    ...
    confidence: 0.7
    source: "code-scan"
  - to: lupo-includes/classes/ActorService.php
    type: USES
    weight: 0.9
    confidence: 0.7
    source: "code-scan"
lupopedia.footer:
  ...
  generated: true
  last_verified: "20260415014600"
  last_verified_by: cursor
  orchestrator: wolfie
  next_action:
  - Maintain header format compliance
...
// Temporal pulse: keep bin/temporal_anchor.json aligned with real UTC when staff use admin (PRD 00 §3.5a)
if ($isUserLoggedIn && is_file(LUPOPEDIA_PATH . '/includes/functions/time.php')) {
    require_once LUPOPEDIA_PATH . '/includes/functions/time.php';
...
if (file_exists(LUPOPEDIA_PATH . '/includes/functions/admin_diagnostics.php')) {
    require_once LUPOPEDIA_PATH . '/includes/functions/admin_diagnostics.php';
...
if ($isAdmin && isset($UNTRUSTED['get']['section']) && is_string($UNTRUSTED['get']['section'])) {
    ...
    if ($section === 'users') {
        ...
        if (!$db) {
            ...
            require_once LUPOPEDIA_PATH . '/includes/classes/AdminUsersHandler.php';
        ...
        if (!$db) {
            ...
            require_once LUPOPEDIA_PATH . '/includes/classes/AdminCsvExportHandler.php';
        ...
        if (!$db) {
            ...
            require_once LUPOPEDIA_PATH . '/includes/classes/AdminDataHubHandler.php';
        ...
        require_once LUPOPEDIA_PATH . '/includes/classes/AdminSemanticWidgetHandler.php';
        $admin_main_content = AdminSemanticWidgetHandler::render($db, $prefix, $base);
    } elseif ($section === 'artifacts') {
        $admin_page_title = function_exists('lupo_t') ? lupo_t('admin.page.artifacts', 'Content') : 'Content';
        $admin_active_key = 'Content';
        if (!$db) {
            $admin_main_content = '<p class="admin-empty">' . htmlspecialchars(lupo_t('admin.db_unavailable', 'Database not available.')) . '</p>';
        } else {
            require_once LUPOPEDIA_PATH . '/includes/classes/AdminArtifactsHandler.php';
            $admin_main_content = AdminArtifactsHandler::render($db, $prefix, $base);
        }
    } elseif ($section === 'channels' && $db) {
        $admin_page_title = 'Channels';
        $admin_active_key = 'Channels';
        require_once LUPOPEDIA_PATH . '/includes/classes/AdminChannelsHandler.php';
        $admin_main_content = AdminChannelsHandler::render($db, $prefix, $base);
    } elseif ($section === 'channel_view' && $db) {
        $admin_page_title = 'Channel View';
        $admin_active_key = 'Channels';
        require_once LUPOPEDIA_PATH . '/includes/modules/channels/ChannelsController.php';
        ...
        require_once LUPOPEDIA_PATH . '/includes/classes/AdminAgentsHandler.php';
        ...
        require_once LUPOPEDIA_PATH . '/includes/classes/AdminActorsHandler.php';
        ...
        require_once LUPOPEDIA_PATH . '/includes/classes/AdminTasksHandler.php';
        ...
        require_once LUPOPEDIA_PATH . '/includes/classes/AdminRegistryHandler.php';
        ...
        require_once LUPOPEDIA_PATH . '/includes/classes/AdminActorStatusHandler.php';
        ...
        require_once LUPOPEDIA_PATH . '/includes/classes/AdminDepartmentsHandler.php';
        ...
        require_once LUPOPEDIA_PATH . '/includes/classes/AdminLeadsHandler.php';
        ...
        require_once LUPOPEDIA_PATH . '/includes/classes/AdminSettingsHandler.php';
        ...
        require_once LUPOPEDIA_PATH . '/includes/classes/AdminChannel66QaHandler.php';
        ...
        require_once LUPOPEDIA_PATH . '/includes/classes/AdminChannelChatHandler.php';
        ...
        require_once LUPOPEDIA_PATH . '/includes/classes/AdminTrustLadderHandler.php';
        ...
        if ($section === 'channels' || $section === 'agents' || $section === 'actors' || $section === 'departments' || $section === 'leads' || $section === 'registry' || $section === 'channel-chat') {
            ...
            ob_start();
            include LUPOPEDIA_PATH . '/includes/themes/default/layouts/admin_sections/info.php';
            $admin_main_content = ob_get_clean();
        } else {
            $admin_section_title = $admin_page_title;
