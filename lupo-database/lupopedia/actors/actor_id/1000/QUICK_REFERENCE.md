...
- **Integration:** Install wizard, TOON generation/validation scripts, registry seeding. Use with `lupo-database/lupopedia/mysql/` for installer SQL; `scripts/verify_db_against_toons.py` for schema checks.
...
- **Install fails (missing SQL):** Ensure `lupo-database/lupopedia/mysql/` exists and contains install/seed/import/migrations; check LUPO_DATABASE_DIR / LUPO_MYSQL_DIR.
