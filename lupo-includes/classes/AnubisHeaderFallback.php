...
lupopedia.headers:
  ...
  web_path: "http://www.lupopedia.com/lupopedia/lupo-includes/classes/AnubisHeaderFallback.php"
  last_modified_utc: "20260406010447"
...
class AnubisHeaderFallback
{
    ...
    private $review_queue_dir = 'lupo-anubis/review_queue/';

    private $quarantine_dir = 'lupo-anubis/quarantine/';

    private $backup_dir = 'lupo-anubis/backups/';
    ...
    {
        // Load agent config from lupo-agents/anubis/
    ...
    private function ensureTimestampClass()
    {
        ...
        if (defined('LUPOPEDIA_PATH')) {
            $p = LUPOPEDIA_PATH . DIRECTORY_SEPARATOR . 'lupo-includes' . DIRECTORY_SEPARATOR . 'classes' . DIRECTORY_SEPARATOR . 'TimestampYmdhis.php';
            if (is_file($p)) {
                require_once $p;
            }
        }
    ...
    private function logAction($action, $file_path, $metadata = array())
    {
        ...
        $log_file = 'lupo-anubis/audit.log';
