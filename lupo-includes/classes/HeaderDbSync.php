<?php
/**
 * HeaderDbSync — PHP mirror of lupo-scripts/lib/header_db_sync.py + import_content helpers.
...
class HeaderDbSync
{
    ...
    {
        $path = rtrim($repoRoot, '/\\') . '/lupo-database/lupopedia/json/lupo_contents.json';
        if (!is_file($path)) {
    ...
    public static function validateFile($filePath, $repoRoot, $checkDb)
    {
        ...
        if ($cid === null || $cid === '' || (is_string($cid) && trim($cid) === '')) {
            $warnings[] = $filePath . ': No content_id - file not linked to lupo_contents. Import: php lupo-scripts/import_content.php (optional --write-back to persist content_id in file)';
        } else {
            $s = trim((string) $cid);
            if ($s !== '' && !ctype_digit($s)) {
    ...
    public static function appendCheckDbWarnings(&$warnings, $filePath, $headerData, $repoRoot)
    {
        ...
        try {
            ...
            if (count($outbound) > 0 && $edgeCount === 0) {
                $warnings[] = $filePath . ': File has outbound_edges but DB has 0 lupo_edges (lupopedia_header). Run python lupo-scripts/import_content.py or php lupo-scripts/import_content.php';
            }
            if ($fileHist !== null) {
                ...
                if ($emptyDb) {
                    $warnings[] = $filePath . ': File has lupopedia.history but revision_history empty in DB. Run python lupo-scripts/import_content.py or php lupo-scripts/import_content.php';
                }
            }
        } catch (Exception $e) {
