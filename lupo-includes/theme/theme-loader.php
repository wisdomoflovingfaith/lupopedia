<?php
/**
 * FILE: lupo-includes/theme/theme-loader.php
 * TYPE: php
 *
 ...
 * @return string Absolute path (e.g. .../lupo-includes/themes/default/layouts/main_layout.php)
 */
function lupo_theme_path($relative_path) {
    ...
    return $base . DIRECTORY_SEPARATOR . 'lupo-includes' . DIRECTORY_SEPARATOR . 'themes' . DIRECTORY_SEPARATOR . $slug . DIRECTORY_SEPARATOR . $rel;
}
 ...
 * 3. lupo-includes/ui/layouts/<file>
 *
...
function lupo_resolve_layout($layout_filename) {
    ...
    $themes_base = $base . $ds . 'lupo-includes' . $ds . 'themes';
    $ui_layouts = $base . $ds . 'lupo-includes' . $ds . 'ui' . $ds . 'layouts' . $ds . $layout_filename;
 ...
 * 3. lupo-includes/ui/components/<file>
 *
...
function lupo_resolve_component($component_filename) {
    ...
    $ds = DIRECTORY_SEPARATOR;
    $themes_base = $base . $ds . 'lupo-includes' . $ds . 'themes';
    $ui_components = $base . $ds . 'lupo-includes' . $ds . 'ui' . $ds . 'components' . $ds . $component_filename;
    $active = lupo_get_active_theme_slug();
    $candidates = [
 ...
 * LUPO_UI_PATH to the theme root so sub-includes (e.g. topbar → collections_dropdown) resolve,
