...
class ValidationTool
{
    ...
    private function validateDatabaseSchema()
    {
        try {
            ...
            $requiredTables = ['lupo_dialog_channels', 'lupo_dialog_messages'];
            ...
            if (!$this->validateTableStructure('lupo_dialog_messages', $this->getMessagesTableStructure())) {
                return false;
            }
    ...
    {
        try {
            $sql = "SELECT * FROM lupo_dialog_channels ORDER BY created_ymdhis DESC";
            $stmt = $this->db->prepare($sql);
            $stmt->execute();
    ...
    {
        try {
            $sql = "SELECT COUNT(*) FROM lupo_dialog_messages";
            $stmt = $this->db->prepare($sql);
            $stmt->execute();
            return (int) $stmt->fetchColumn();
        } catch (Exception $e) {
    ...
    {
        try {
            $sql = "SELECT COUNT(*) FROM lupo_dialog_messages WHERE channel_id = :channel_id";
            $stmt = $this->db->prepare($sql);
    ...
    {
        try {
            $sql = "SELECT m.* FROM lupo_dialog_messages m 
