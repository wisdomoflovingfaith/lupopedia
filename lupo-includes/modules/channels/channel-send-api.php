...
if ($channel_id <= 0 || $dialog_thread_id <= 0) {
    header('Content-Type: application/json');
    echo json_encode(['error' => 'channel_id and dialog_thread_id required']);
    exit;
}
if ($message_text === '') {
    header('Content-Type: application/json');
...
// Insert message (legacy: INSERT livehelp_messages message, channel, timeof, saidfrom, saidto)
...
// Legacy: on send, clear ALL typing (writediv) for the channel — single global clear
