...
lupopedia.headers:
  ...
  file_path_from_root: "lupo-includes/modules/actors/actors-controller.php"
  web_path: "http://www.lupopedia.com/lupopedia/lupo-includes/modules/actors/actors-controller.php"
  last_modified_utc: "20260406014900"
...
{
    if (defined('LUPO_SMTP_HOST') && LUPO_SMTP_HOST !== '') {
        if (!is_file($app_root . '/lupo-includes/PHPMailer/PHPMailer.php')) {
            return false;
        }
...
function actors_handle_my_profile()
{
    ...
    if ($auth_user_id !== null && (int) $auth_user_id > 0) {
        $asmPath = $app_root . '/lupo-includes/classes/AuthSessionManager.php';
        if (is_file($asmPath)) {
