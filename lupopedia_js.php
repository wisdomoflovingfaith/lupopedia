 ...
 * Real track endpoint: {publicPath}/lupo-ajax.php?action=track (JSON POST + CSRF), not api/track.php.
 *
 * @package Lupopedia
 */
...
require_once $lupoRoot . DIRECTORY_SEPARATOR . 'lupo-includes' . DIRECTORY_SEPARATOR . 'classes' . DIRECTORY_SEPARATOR . 'LupopediaConfigResolver.php';
...
$ajaxUrl = ($origin !== '' && $pub !== '') ? ($origin . $pub . '/lupo-ajax.php') : '';
...
if ($configOk && defined('LUPOPEDIA_PUBLIC_PATH')) {
    ...
    $ajaxUrl = ($origin !== '') ? ($origin . $pub . '/lupo-ajax.php') : '';
...
$layersPath = $fsRoot . DIRECTORY_SEPARATOR . 'lupo-includes' . DIRECTORY_SEPARATOR . 'js' . DIRECTORY_SEPARATOR . 'lupo-layers.js';
$monitorPath = $fsRoot . DIRECTORY_SEPARATOR . 'lupo-includes' . DIRECTORY_SEPARATOR . 'js' . DIRECTORY_SEPARATOR . 'lupo-monitor.js';
...
$emit = function ($absPath) use ($fsRoot) {
    ...
    $jsRoot = @realpath($fsRoot . DIRECTORY_SEPARATOR . 'lupo-includes' . DIRECTORY_SEPARATOR . 'js');
    if ($jsRoot === false || strpos(str_replace('\\', '/', $real), str_replace('\\', '/', $jsRoot) . '/') !== 0) {
        echo "\n/* Refused path outside lupo-includes/js */\n";
        return;
    }
    echo "\n/* --- " . basename($real) . " --- */\n";
    readfile($real);
