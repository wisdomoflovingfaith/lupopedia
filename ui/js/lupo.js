// ui/js/lupo.js - Modern The Eye widget
...
class LupoTheEye {
    ...
    renderGoldIndicator(contexts) {
        const goldElement = document.getElementById('gold-indicator');
        if (goldElement) {
    ...
    createGraph(edges) {
        ...
        let svg = d3.select('#context-graph');
        if (svg.empty()) {
            svg = d3.select('body').append('svg')
                .attr('id', 'context-graph')
                .attr('width', 800)
                .attr('height', 600);
        }
    ...
    renderTableView(edges) {
        ...
        const table = document.getElementById('context-table') || this.createTable();
