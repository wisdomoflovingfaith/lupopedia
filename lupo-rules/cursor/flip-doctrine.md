...
lupopedia.headers:
  ...
  file_path_from_root: "lupo-rules/cursor/flip-doctrine.md"
  web_path: "http://www.lupopedia.com/rules/cursor/flip-doctrine"
...
---
# file: Rule — LUPOPEDIA HEADERS (replaces FLIP/FLARE) — session: L-LUPO-WOLFIE — delegation: wolfie:root — web_path: http://www.lupopedia.com/rules/cursor/flip-doctrine
...
- **Storage:** Headers are stored in the **`lupo_metadata`** table (with table prefix). Rows are structured as root → blocks → properties → edges/mappings/actions. Lookup by `entity_type` + `entity_id` and/or `channel_id`.
