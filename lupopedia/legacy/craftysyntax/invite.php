...
for($i=0;$i<count($pair);$i++){
 ...
 print $lang['txt71'] . "<a href=javascript:window.close()>" . $lang['txt40'] . "</a>";
   ...
   if(!(empty($UNTRUSTED['socialpanel']))){  
     $sessiondata = "tab=,glrho&glrho=default&openPop=default";
     $query = "UPDATE livehelp_users set sessiondata='$sessiondata' WHERE user_id=".intval($saidto);
     $mydatabase->query($query);       
  }
...
<br>
<?php if(!(empty($UNTRUSTED['insite']))){ 
	print "<input type=hidden name=askquestions value=Y><input type=hidden name=socialpanel value=Y>";	
	} else { ?>
