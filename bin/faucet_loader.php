 ...
 * 1. Per-actor (override): lupopedia/channels/channels/<channel_id>/actors/<actor_id>/faucets.json
 * 2. Channel-wide (override): lupopedia/channels/channels/<channel_id>/faucets.json
...
class FaucetLoader {
    ...
    /**
     * Resolve base path for lupopedia file-based data (database/lupopedia or equivalent)
     */
    ...
    private function loadToonSchema() {
        ...
        if (!file_exists($toon_file)) {
            $legacy = dirname(__DIR__) . DIRECTORY_SEPARATOR . 'docs' . DIRECTORY_SEPARATOR . 'toons' . DIRECTORY_SEPARATOR . 'lupo_agent_faucets.toon.json';
    ...
    public function loadFaucet($channel_id, $actor_id) {
        ...
        $per_actor_file = $this->base_path . $sep . 'channels' . $sep . 'channels' . $sep . $ch . $sep . 'actors' . $sep . $ac . $sep . 'faucets.json';
        ...
        $channel_wide_file = $this->base_path . $sep . 'channels' . $sep . 'channels' . $sep . $ch . $sep . 'faucets.json';
