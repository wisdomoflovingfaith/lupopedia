...
class SessionManager
{
        ...
        try {
            ...
            $params = array(
                ...
                'session_data' => json_encode(array(
                    ...
                    'session_prefix' => 'L-' . $sessionData['actor_id']
                )),
    ...
    public function migrateExistingSessions()
    {
        $sql = "UPDATE lupo_sessions 
                 SET session_id = CONCAT('L-', actor_id, '-', session_id),
                     metadata = JSON_SET(
                         JSON_SET(metadata, '$.session_prefix', CONCAT('L-', actor_id)),
                         '$.migrated', '20260301'
                     )
                 WHERE session_id NOT LIKE 'L-%' 
                   AND is_deleted = 0";
        ...
        if ($migrated > 0) {
            $logSql = "INSERT INTO lupo_channel_logs 
                ...
                VALUES (0, 1002, 1, 'Migrated $migrated sessions to L- prefix format', :created_ymdhis)";
    ...
    private function upsertSession($params)
    {
        ...
        if ($exists) {
            ...
            $this->db->update(
                ...
                array('session_id' => $params['session_id'])
            );
...
if (php_sapi_name() === 'cli') {
    ...
    switch ($command) {
        ...
        case 'migrate':
            echo "🔄 Migrating existing sessions to L- prefix format...\n";
        ...
        default:
            ...
            echo "  php session_manager.php migrate   - Migrate existing sessions to L- prefix\n";
