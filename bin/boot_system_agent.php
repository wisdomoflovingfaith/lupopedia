 ...
 * 
 * Boots the system agent (actor_id 0) and initializes Channel 0
 * Uses the modern channel boot lifecycle system
 * 
...
require_once __DIR__ . '/channel_boot_lifecycle.php';

class SystemAgentBoot {
    private $bootLifecycle;
    private $errors = [];
    private $warnings = [];
    
    public function __construct() {
        $this->bootLifecycle = new ChannelBootLifecycle();
    }
    ...
    public function bootSystemAgent() {
        ...
        try {
            ...
            $lifecycleId = $this->bootLifecycle->startLifecycle(
                0, // System actor
                $sessionId,
                'system_agent_boot',
                [0] // Channel 0 only
            ...
            if (!$lifecycleId) {
                throw new Exception("Failed to start boot lifecycle");
            }
    ...
    /**
     * Validate system state before boot
     */
    private function validateSystemState($lifecycleId) {
        ...
        $this->bootLifecycle->updateLifecycle($lifecycleId, [
    ...
    private function initializeChannel0($lifecycleId) {
        ...
        if (!$result) {
            ...
            $db->execute($insertSql, ['created_ymdhis' => gmdate('YmdHis')]);
        ...
        $stateData = json_encode([
            ...
            'last_boot' => gmdate('Y-m-d H:i:s UTC'),
            'system_agent' => 'active'
        ]);
    ...
    private function verifyFederationNode0($lifecycleId) {
        ...
        $sql = "SELECT node_id FROM lupo_federation_nodes WHERE node_id = 0";
        $result = $db->fetch($sql);
        
        if (!$result) {
            // Create federation node 0
            $insertSql = "INSERT INTO lupo_federation_nodes 
                (node_id, node_name, node_type, base_url, is_active, created_ymdhis) 
                VALUES (0, 'lupopedia.com', 'primary', 'http://www.lupopedia.com', 1, :created_ymdhis)";
        ...
        $contentResult = $db->fetch($contentSql);
    ...
    private function completeBoot($lifecycleId) {
        echo "🏁 Completing boot process...\n";
        
        // Update final lifecycle status
        $this->bootLifecycle->completeLifecycle($lifecycleId);
        
        // Log system boot event
        $db = DatabaseFactory::getConnection();
        $logSql = "INSERT INTO lupo_channel_logs 
            (channel_id, actor_id, log_type_id, log_text, created_ymdhis) 
            VALUES (0, 0, 1, 'System agent boot completed successfully', :created_ymdhis)";
        
        $db->execute($logSql, ['created_ymdhis' => gmdate('YmdHis')]);
        
        echo "✅ Boot lifecycle completed\n";
        echo "✅ System boot event logged\n";
    ...
    private function handleBootError($exception, $lifecycleId = null) {
        ...
        if ($lifecycleId) {
            $this->bootLifecycle->failLifecycle($lifecycleId, $exception->getMessage());
        }
        ...
        if ($db) {
            $logSql = "INSERT INTO lupo_channel_logs 
                (channel_id, actor_id, log_type_id, log_text, created_ymdhis) 
                VALUES (0, 0, 3, 'System agent boot failed: " . $exception->getMessage() . "', :created_ymdhis)";
    ...
    /**
     * Get any warnings from boot process
     */
    ...
    /**
     * Get any errors from boot process
     */
