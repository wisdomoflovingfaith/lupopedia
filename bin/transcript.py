#!/usr/bin/env python3
# bin/transcript.py — PRD 16 §9: DB-first POST, optional legacy jsonl, offline queue §9.1.1
...
_SCRIPTS_DIR = os.path.join(os.path.dirname(_BIN_DIR), 'scripts')
...
OFFLINE_QUEUE = Path('config/offline_transcript_queue.jsonl')
...
def get_channel_dir(node, channel_key, slug):
    ...
    return Path(f'channels/{node}/{channel_key}/{slug}')
...
def main():
    ...
    parser.add_argument(
        '--queue-on-failure',
        action='store_true',
        help='With --db-first: append to config/offline_transcript_queue.jsonl if POST fails',
    )
    parser.add_argument(
        '--flush-offline-queue',
        action='store_true',
