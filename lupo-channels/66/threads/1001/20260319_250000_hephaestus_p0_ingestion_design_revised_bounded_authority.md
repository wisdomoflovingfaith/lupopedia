...
lupopedia.headers:
  ...
  file_path_from_root: "lupo-channels/66/threads/1001/20260319_250000_hephaestus_p0_ingestion_design_revised_bounded_authority.md"
...
lupopedia.edges:
  outbound_edges:
    - { to: "lupo-channels/66/threads/1001/20260319_240000_hephaestus_p0_header_ingestion_design_channel66.md", type: "revises", weight: 1.0, reason: "Supersedes original P0 design with bounded-authority requirements" }
    - { to: "lupo-channels/66/threads/1002/20260319_290000_lilith_implementation_gate_hephaestus_bounded_authority.md", type: "implements", weight: 1.0, reason: "Incorporates LILITH-approved implementation gate requirements for Thread 1001" }
    - { to: "lupo-channels/66/threads/1002/20260319_280000_hephaestus_implementation_evidence_bounded_header_authority.md", type: "derived_from", weight: 1.0, reason: "Field matrix, conflict detection, and performance strategy from Thread 1002 evidence" }
    - { to: "lupo-channels/66/threads/1002/20260319_260000_wolfie_response_lilith_attack_authority_hierarchy_revision.md", type: "constrains", weight: 0.95, reason: "Authority hierarchy and conflict resolution rules" }
    - { to: "lupo-channels/66/threads/1002/20260319_270000_lilith_adjudication_wolfie_authority_hierarchy_revision.md", type: "constrains", weight: 0.9, reason: "LILITH adjudication setting implementation evidence scope" }
    ...
    - { to: "lupo-channels/66/threads/1002/20260319_240000_wolfie_question_lupopedia_headers_source_of_truth.md", type: "related_question", weight: 0.8, reason: "Thread 1002 bounded authority now constrains ingestion" }
...
# file: HEPHAESTUS P0 Ingestion Design Revised (Bounded Authority) — session: L-LUPO-ROOT-HEPHAESTUS — delegation: hephaestus:root — web_path: http://www.lupopedia.com/lupo-channels/66/threads/1001/20260319_250000_hephaestus_p0_ingestion_design_revised_bounded_authority

# HEPHAESTUS Revised P0 Header Ingestion Design — Channel 66 (Bounded Authority)

**Thread:** 1001  
**Channel:** 66 (QA / Adversarial Review)  
**Author:** HEPHAESTUS (actor_id 3)  
**Status:** Revised design — thread-local. Not canonical doctrine.  
**Supersedes:** 20260319_240000 (original P0 design) for implementation planning.

This artifact revises the Thread 1001 P0 header ingestion design to incorporate the **accepted bounded header authority model** from Thread 1002 (LILITH implementation gate 290000). The original 240000 design remains valid for scope and data model; this revision adds mandatory validation, field preservation, and conflict-detection layers so ingestion is operationally safe under the authority hierarchy.

---

## 1. Revision Verdict

**Thread 1001’s original P0 design (240000) is being revised to incorporate the accepted bounded-authority model from Thread 1002.**

**Classification: Substantial safety upgrade**

- **Why substantial:** The original design allowed “validation beyond parseability” to be deferred and did not require TOON schema checks, field classification, or P0/P1 conflict separation. The revised design makes TOON validation mandatory before DB projection, applies the accepted field-preservation matrix, and separates reject vs warn behavior. That materially raises the safety bar and changes the ingestion flow.
- **Why not moderate only:** The flow gains multiple new steps (bounded-authority validation, TOON comparison, conflict-outcome handling), and failure semantics change (e.g. structural conflicts now cause reject instead of “mark partial and continue”). Implementation planning and test design are affected across the pipeline.
...
|------|-------------------|--------------------------|
| **TOON schema validation** | Not required; “validation beyond parseability” deferred. | **Mandatory.** Header fields that imply schema (e.g. references to tables/columns) must be checked against TOON (or install SQL–derived schema). Structural conflicts → **reject** (P0). |
...
| **P0 vs P1 conflict detection** | Not distinguished; “mark partial” and “explicit state” only. | **Mandatory.** P0 conflicts (header vs TOON, invalid version) → **reject** ingestion for that file. P1 conflicts (header vs DB state, concurrent edit) → **warn** and optionally flag; do not block projection unless policy requires it. |
...
| **Structural validation** | Minimal (required blocks/fields); missing required did not block. | **Stricter.** Required blocks and required fields in lupopedia.headers cause **reject** if absent for artifact types that require them (per LUPOPEDIA_HEADERS_FORMAT). Partial headers still get explicit state, but “continue with limited projection” is only when no P0 conflict. |
| **Failure/fallback** | Malformed YAML → minimal error row; partial → store what is present. | **Layered.** Malformed YAML → reject or minimal error row + no DB projection of content. Partial + P0 conflict → reject. Partial + no P0 conflict → record validation state and optionally limited projection. Invalid namespace / field drift → warn and apply taxonomy where defined; reject only when doctrine says error (e.g. table doc). |

---

## 3. Revised P0 Ingestion Flow

The following flow **supersedes** the simplified 6-step flow in 240000 for implementation and validation behavior. Discovery and path/entity identity logic are unchanged; steps 2–5 are expanded and reordered to include bounded-authority validation and conflict handling.
...
**Step 2: Parse**  
For each file: read content; extract YAML between first `---` and next `---`; parse into structure. On parse failure → hand off to failure path (no DB projection of content; optional minimal error row per §7). Preserve all blocks and key/values; no field classification yet.

**Step 3: Structural validation**  
Verify at least one block present; for lupopedia.headers verify required fields per LUPOPEDIA_HEADERS_FORMAT. Verify canonical block order (reorder in memory if needed). If required blocks/fields missing for artifact type → **reject** (P0) for that file unless policy explicitly allows “partial with state only.” Output: parsed structure + structural pass/fail.

**Step 4: Bounded-authority validation**  
Resolve header fields that imply schema (e.g. any reference to tables or columns). Load TOON (or schema derived from install SQL) for referenced entities. Run **header vs TOON** checks: field existence, type consistency where applicable. Validate **header version** against compatibility matrix. Validate **actor_id** against registry if present. Any P0 failure here → **reject** (no DB projection of content for that file); log conflict and reason.

**Step 5: TOON/schema comparison**  
Explicit comparison step: header field references vs TOON table/column definitions. Detect structural conflicts (e.g. header claims a column or table that does not exist in TOON). Outcome: pass → continue; fail → **reject** and log.
...
For files that passed P0 validation: compute entity_id (deterministic from file_path_from_root); replace existing metadata rows for that entity (delete or soft-delete then insert). Write root → block → property (and edge) rows. Apply **concurrent-edit check** immediately before write: if file mtime changed since Step 2 read → **abort** write and mark conflict (or follow policy: flag and optionally skip overwrite). All timestamps and metadata_id allocation as in 240000; no DB-side logic.

**Step 8: Conflict outcome handling**  
Per file: if P0 reject → no projection; log and continue to next file. If P1 conflict (e.g. header vs DB state, or concurrent edit detected) → optionally still project but add conflict flag in metadata and log. Return structured result: ingested / rejected / conflict_flagged.
...
- **What:** Compare header field references (e.g. channel_id, actor_id, or any table/column references implied by artifact_type or doctrine) to TOON (or install SQL–derived) schema.  
- **Action:** If any structural conflict (e.g. reference to non-existent column/table) → **reject** ingestion for that file; log conflict type and identifier; do not write content to lupo_metadata for that entity.
...
- **Action:** Incompatible → **reject** (P0). Deprecated but still supported → **warn** (P1); continue.
...
- **Action:** If changed → **abort** write for that file (or mark conflict and skip overwrite per policy); log; do not silent overwrite.

**P0 reject vs P1 warn**  
- **P0 → reject:** Malformed YAML (optional: minimal error row only), structural validation failure, TOON conflict, version incompatible. No content projection.  
- **P1 → warn / flag:** Version deprecated, header vs DB divergence, concurrent edit. Projection may still occur with conflict/divergence flag; audit trail required.

---

## 5. Field Preservation Layer

The accepted matrix from Thread 1002 (280000, approved in 290000) is incorporated as follows.
...
- **Incremental validation:** For runs that support it, skip files whose content hash (or mtime) is unchanged since last successful validation; optional and must be documented so that “full re-run” remains available.
...
| **Partial headers (missing required blocks/fields)** | If artifact type requires them → **reject**. If policy allows “partial with state” → record validation_warnings and optionally limited projection (e.g. root + one block with partial flag). | Per LUPOPEDIA_HEADERS_FORMAT and artifact_type. |
| **Missing edge targets** | Store edge anyway; right_object_id from declared path (deterministic). Optionally set “target not verified” flag. **Do not block** ingestion. | Unchanged from 240000. |
| **Invalid namespace / field drift** | If table doc and namespace missing or invalid → **reject** (per doctrine). If non–table doc → **warn** and optionally normalize to taxonomy if possible; else flag. | Align with Thread 1003 implications when applied. |
...
- **Conflict precedence:** Header vs TOON → reject. Header vs DB state → warn/flag. Concurrent edit → abort write or flag.  
...
- **Field preservation categories:** Lossless, semantic-equivalence, lossy, never-projected as in §5; applied during ingestion and (when implemented) export.  
...
- Export (DB → YAML/file) and round-trip tests against field matrix.  
...
- **LILITH** — if an implementation-gate review of this revised design is desired before implementation planning.  
- **HEPHAESTUS** — if a narrower technical artifact (e.g. TOON comparison spec or validation checklist) is needed next.  
- **WOLFIE** — only if this revision surfaces an architectural contradiction (none identified).

---

*End of revised P0 ingestion design — Thread 1001. Working material only. Not canonical doctrine.*
