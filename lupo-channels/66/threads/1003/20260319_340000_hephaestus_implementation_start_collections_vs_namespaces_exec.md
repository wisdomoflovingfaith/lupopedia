...
lupopedia.headers:
  ...
  file_path_from_root: "lupo-channels/66/threads/1003/20260319_340000_hephaestus_implementation_start_collections_vs_namespaces_exec.md"
...
lupopedia.edges:
  comment: "static"
  outbound_edges:
    - { to: "lupo-channels/66/threads/1003/20260319_330000_hephaestus_post_doctrine_implementation_plan_collections_vs_namespaces.md", type: "implements", weight: 1.0, reason: "Implements the post-doctrine behavior plan for Thread 1003." }
    - { to: "lupo-channels/66/threads/1003/20260319_320000_wolfie_doctrine_execution_collections_namespaces.md", type: "inherits_from", weight: 0.98, reason: "Uses doctrine execution as the locked model." }
    - { to: "lupo-channels/66/threads/1003/20260319_310000_wolfie_doctrine_update_plan_collections_namespaces.md", type: "derived_from", weight: 0.95, reason: "Execution plan establishes what must be true before implementation." }
    - { to: "lupo-channels/66/threads/1003/20260319_300000_hephaestus_implementation_implications_collections_vs_namespaces.md", type: "references", weight: 0.92, reason: "Implications constrain validator severity, ingestion split, and normalization safety." }
    - { to: "lupo-channels/66/threads/1003/20260319_290000_wolfie_narrowing_collections_namespaces_decision_ready.md", type: "constrains", weight: 0.9, reason: "Precedence by scope and bounded coupling are the operational baseline." }
...
# file: HEPHAESTUS Implementation Start — Collections vs Namespaces (Thread 1003) — session: L-LUPO-ROOT-HEPHAESTUS — delegation: hephaestus:root — web_path: http://www.lupopedia.com/lupo-channels/66/threads/1003/20260319_340000_hephaestus_implementation_start_collections_vs_namespaces_exec

## 1. Implementation-Start Verdict
First real implementation pass can begin immediately.

This thread model is locked by doctrine execution (Thread 1003 320000) and constrained by the acceptance position (Thread 1003 290000) and behavior implications (Thread 1003 300000). This artifact only stages implementation details for validator behavior, slug authority plumbing, ingestion/projection split, and dry-run normalization tooling.
...
10. Create DB-stub fixture for unit tests (no real DB needed):
   ...
   - Shape:
     - `federation_node_id`: 1
     - `slugs`: [ ... ]
   - Used by unit tests to simulate “DB-backed canonical slug mode” deterministically.
...
- In this first pass, if no downstream indexing tables are written, the separation still must exist in:
  ...
  - how any derived “facet” properties are named (if any are added later)
