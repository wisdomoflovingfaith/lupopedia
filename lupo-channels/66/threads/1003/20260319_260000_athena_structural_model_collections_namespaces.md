...
lupopedia.headers:
  ...
  file_path_from_root: "lupo-channels/66/threads/1003/20260319_260000_athena_structural_model_collections_namespaces.md"
...
lupopedia.edges:
  outbound_edges:
    - { to: "lupo-channels/66/threads/1001/20260319_240000_hephaestus_p0_header_ingestion_design_channel66.md", type: "related_question", weight: 0.95, reason: "Ingestion system depends on collection/namespace resolution" }
...
- Namespaces are **not** enforced as directory paths in this model. A file in `lupo-docs/doctrine/` might have `namespace: core`; the path and the namespace are independent.
...
- **Directories:** Neither collections nor namespaces **define** directory layout. Directories are defined by **directory structure doctrine** (e.g. `lupo-channels/66/threads/1003/`, `lupo-docs/doctrine/`, `lupo-actors/`). So:
...
- **Headers:** Many docs under `lupo-docs/channels/` use `collections: ["core-docs", "doctrine"]` or similar.
...
- **Directories:** No strict rule that “this directory = this collection.” Collections are declared on artifacts; directories (e.g. `lupo-docs/`, `lupo-channels/66/`) are structure, not collection identity.
