...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/prd/45_A-i_TEMPLATE_FIRST_STAGED_UI_WORKFLOW.md
  web_path: https://www.lupopedia.com/lupopedia/docs/prd/45_A-i_TEMPLATE_FIRST_STAGED_UI_WORKFLOW.md
  status: active
  when_updated: '20260513033046'
  trust_tier: canonical
  questions_toon: null
  memory_toon: memory/development/canonical/1026/04/45_template_first_staged_ui_workflow.toon
  atoms_toon: null
  ...
  prd_cluster: 45_A-i_00_A-i_FORBIDDEN_AND_WHY_45_A_TEMPLATE_FIRST_STAGED_UI_WORKFLOW
  title: 'PRD 45: Template-First, Gate-Gated UI Workflow (Staged Operator and Admin Implementation)'
  summary: 'Template-first staged UI: workflow gates G0-G4, prohibition on direct PRD-to-ship for scoped surfaces, templates then includes/lang and lupo_t, then entrypoints; cross-refs PRD 13 16 29 31 00; severity for bypass.'
---
...
This PRD defines the **Template-First Development Workflow** (also called **Staged UI Implementation**) for **non-public-facing** surfaces: **admin tools**, **dashboards**, **operator pages**, and **internal templates** shipped under paths such as **`templates/admin/`**.
...
- **[PRD 29 ??? Project structure](29_project_structure.md)** ??? **`templates/`** and repository layout authority.
...
- Build the template under **`templates/admin/`** (or another **appropriate subdirectory** agreed in the owning PRD or implementation mirror).
...
- **Do not** introduce **`lupo_t()`** calls or new **`includes/lang/`** keys in this phase.
...
- Add new keys to **`includes/lang/en.php`** first; mirror other supported locales when required by **[PRD 00](00_root_constitutional_system_requirements.md)** **RULE 93.UI_STRINGS_LOCALE** and project locale policy.
...
| **G1** | PRD | **`templates/{scope}/`** | **Staged HOW**: structure, control flow, data wiring (**PRD 29** tree) |
| **G2** | Template draft | **Human or orchestrator review** | Reject unsafe patterns, wrong service boundaries, missing edge cases |
| **G3** | Reviewed template | **`includes/lang/{locale}.php`** + **`lupo_t()`** | **Strings only** in catalogs; logic stays in PHP partials and classes |
| **G4** | Localized partial | **Ship entrypoints** | **DEPLOYED wiring**: includes from **`admin.php`** handlers, **`channels/index.php`**, **`includes/themes/`**, **`login.php`**, and similar served paths ??? **not** a second hidden copy of the same business rules |
...
**G3 may open when:** English UX is accepted; new keys land in **`en.php`** first, then other allowed locales.
...
1. **FORBIDDEN:** Implementing **only** from PRD paragraphs pasted into a **ship entrypoint** (for example a large new **`admin.php`** region) **without** a reviewable **`templates/`** artifact that can be exercised in isolation.
...
**Remediation:** revert or quarantine the ship change; restore **PRD to template to locale to entrypoint** ordering; record systemic bypass in **`docs/audits/`** when discovery is cross-cutting.
...
- **`templates/`** partials **MAY** contain structure, includes, and calls into **sanctioned** application code paths (**PDO_DB**, services). They **MUST NOT** introduce **`lupo_t()`** until **Phase 3** unless the surface is **public-facing** per **??4**.
- **`includes/lang/*.php`** catalogs **MUST** expose **string data** (arrays returned to **`lupo_t()`** consumers). They **MUST NOT** perform authorization decisions, schema migrations, remote I/O, or other **business logic**.
