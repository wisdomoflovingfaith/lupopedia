...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/prd/02_A-i_CHANNELS_DB_DESIGN.md
  web_path: https://www.lupopedia.com/lupopedia/docs/prd/02_A-i_CHANNELS_DB_DESIGN.md
  status: active
...
### Channel Key Filesystem Binding (Cross-Doctrine)


Channel keys SHALL map directly to filesystem namespaces under the content invariant path:

```
content/federation_node/{federation_node}/{channel_key}/
```

Rules:

1. `channel_key` MUST be filesystem-safe (lowercase, underscore-separated).
2. Channel keys MUST NOT overlap across federation nodes.
3. Channel keys SHALL be treated as namespace boundaries for content routing.

This rule ensures alignment between:

- Database channel routing
- Filesystem storage
- Agent content resolution

Violation SHALL result in routing ambiguity and is prohibited.
---
...
1. The literal directory segment `federation_node/` is mandatory. It identifies the following path segment as the federation node ID. Agents MUST NOT omit this segment.
        - "fbddef"
        - "cfaaef"
        - "aedcbd"
        - "bbffff"
