...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/prd/19_A-i_GARBAGE_COLLECTION_SYSTEM.md
  web_path: https://www.lupopedia.com/lupopedia/docs/prd/19_A-i_GARBAGE_COLLECTION_SYSTEM.md
  status: active
  when_updated: '20260513033046'
  trust_tier: canonical
  questions_toon: null
  memory_toon: memory/development/canonical/1026/04/19_garbage_collection_system.toon
  atoms_toon: null
  transcript_jsonl: 0/development/prd_files/garbage-collection-system
  artifact_type: prd
  ...
  prd_cluster: 00_A-i_19_A-i
  title: 'PRD: Garbage Collection System (The GC) - Unified Table Architecture'
  summary: Defines the Garbage Collection system for background data aggregation, pruning, and maintenance tasks including paths, referrers, visits, sessions, and memory nodes.
---
...
|-------|---------|-------------|-----------|
| `lupo_paths_raw` | URL-pair transition stream (`from_url` -> `to_url`) | Per event | Purge after rollup (config; default aligns with visits) |
| `lupo_paths_daily` / `lupo_paths_monthly` | URL-pair rollups | Per day / per month (`date_ymd` / `date_ym`) | Same family as `lupo_paths` rollups (config) |
| `lupo_paths` | Content-id navigation aggregates | Per enter/exit + Y/M/D | 90 days (config) |
| `lupo_referers_raw` | Per-hit referrer stream | Per event | Purge after rollup (config) |
| `lupo_referers_daily` | Referrer domain rollup by actor + day | `visit_ymd` | 365 days (config) |
| `lupo_referers` | Referrer detail (content + URL + day) | Per row | 365 days (config) |
| `lupo_visits` | Raw per-hit visits | Per event | 30 days after processed (config) |
| `lupo_visits_daily` | Daily visit rollup | `actor_id` + `visit_ymd` | 365 days (config) |
| `lupo_analytics_campaign_vars` | Campaign tracking | Per campaign per day | Hard-delete prune (no `is_deleted` column) |
| `lupo_sessions` | Session tracking | Per session | After `expires_ymdhis` (config) |
| `lupo_memory_nodes` | Unified memory (incl. expiry) | Per node | Configurable via `expires_ymdhis` + GC |
...
**GC Operations:**
- **Primary path (4.0.x install):** ingest **`lupo_referers_raw`** into **`lupo_referers_daily`** and **`lupo_referers`** (batched), then set **`is_aggregated`** on consumed raw rows; prune old raw after rollup.
- Update existing **`lupo_referers`** rows (increment **`visits`**) when the natural key already exists.
- Prune detail and daily rollup rows older than retention (config).
...
**Canonical DDL:** `install_new_lupopedia.sql` (per-hit events: `path_url`, `session_id`, optional enter/exit content ids, **`is_processed`**, soft delete, no `referer` column on this table in 4.0.x -- referrers use **`lupo_referers_raw`**).

**GC Operations:**
- **Path pairs:** consecutive unprocessed rows in the same **`session_id`** -> increment **`lupo_paths`** (content-id aggregate); mark participating rows **`is_processed = 1`** where implemented.
- **Daily rollup:** group remaining **`is_processed = 0`** rows by **`actor_id`** + calendar day (from **`FLOOR(created_ymdhis / 1000000)`**) -> **`lupo_visits_daily`** with **`ON DUPLICATE KEY`** on **`(actor_id, visit_ymd)`**, then mark those visits processed.
- Prune raw visits older than retention once **`is_processed = 1`**.

### 4. `lupo_visits_daily` -- Daily Visit Aggregates

**Canonical DDL:** `visits_daily_id`, **`actor_id`**, **`visit_ymd`** (YYYYMMDD int), **`total_visits`**, **`unique_sessions`**, **`avg_duration_seconds`**, **`bounce_count`**, timestamps, soft delete. **Unique** (**`actor_id`**, **`visit_ymd`**).

**GC Operations:**
- Upsert counts from **`lupo_visits`** as above.
- Prune rows with **`visit_ymd`** older than retention (packed YYYYMMDD comparison).
...
### 7. `lupo_memory_nodes` -- Unified memory graph

**Canonical DDL:** **`install_new_lupopedia.sql`** / TOON **`lupo_memory_nodes`** (**PRD 38**). Optional retention via **`expires_ymdhis`**.

**GC Operations:**
- Soft-delete rows with **`expires_ymdhis > 0`** and **`expires_ymdhis < cutoff`** (packed UTC).

### 8. Raw navigation streams -- `lupo_paths_raw`, `lupo_referers_raw`

**Purpose:** High-volume append-only captures with **`is_aggregated`**. GC rolls:

- **`lupo_paths_raw`** -> **`lupo_paths_daily`** (**`date_ymd`**) and **`lupo_paths_monthly`** (**`date_ym` = YYYYMM**), keyed by **`from_url` / `to_url`** (and optional content ids), then marks raw rows aggregated.
- **`lupo_referers_raw`** -> **`lupo_referers_daily`** (domain + actor + day) and **`lupo_referers`** (detail), then marks consumed raw ids **`is_aggregated = 1`**.

**Prune:** soft-delete old aggregated raw rows after retention; soft-delete stale rollup rows in **`paths_daily`**, **`paths_monthly`**, **`referers_daily`** per config.

**Normative implementation:** `includes/classes/GarbageCollector.php`.

---

## GC Functions

**Shipped implementation:** `includes/classes/GarbageCollector.php` (prefix-aware, `PDO_DB::query` + `rowCount`, raw->rollup passes in **????8** tables above). The SQL and PHP fragments below are **conceptual**; some still show older patterns (e.g. `ON DUPLICATE KEY` on **`lupo_paths`** where the install has **no** natural unique key--the class uses **SELECT/UPDATE/INSERT** instead).
...
SET is_deleted = 1, deleted_ymdhis = ?
WHERE expires_ymdhis IS NOT NULL AND expires_ymdhis <> 0 AND expires_ymdhis < ? 
  AND is_deleted = 0
LIMIT 10000;
```

### 8. Memory node pruning (`pruneExpiredMemoryNodes`)

**Purpose:** Remove expired unified memory rows.

**Source:** `lupo_memory_nodes`

**Logic:**
```sql
UPDATE lupo_memory_nodes 
SET is_deleted = 1, deleted_ymdhis = ?
WHERE expires_ymdhis > 0 
  AND expires_ymdhis < ?
  AND is_deleted = 0
LIMIT 10000;
```
...
OPTIMIZE TABLE lupo_paths_raw;
OPTIMIZE TABLE lupo_paths_daily;
OPTIMIZE TABLE lupo_paths_monthly;
OPTIMIZE TABLE lupo_referers;
OPTIMIZE TABLE lupo_referers_raw;
OPTIMIZE TABLE lupo_referers_daily;
OPTIMIZE TABLE lupo_visits;
OPTIMIZE TABLE lupo_visits_daily;
OPTIMIZE TABLE lupo_sessions;
OPTIMIZE TABLE lupo_memory_nodes;
```
...
See **`docs/doctrine/CHRONOLOGICAL_TRUST_LADDER.md`** Appendix B for Honolulu lessons summary and rationale.
...
class GarbageCollector {
    ...
    /**
     * Prune expired unified memory nodes (replaces lupo_actor_memory).
     */
    private function pruneExpiredMemoryNodes() {
        if ($this->deleted >= $this->max_per_run) return;
        
        $retention = isset($this->config['gc_memory_retention_days']) ? $this->config['gc_memory_retention_days'] : 30;
        $cutoff = /* use TimestampYmdhis / cutoffPackedDaysAgo($retention) in shipped code */;
        
        $this->prune('lupo_memory_nodes', 'expires_ymdhis > 0 AND expires_ymdhis < ?', array($cutoff));
    }
    ...
    private function optimizeTables() {
        ...
        $tables = array('lupo_paths', 'lupo_paths_raw', 'lupo_paths_daily', 'lupo_paths_monthly',
                   'lupo_referers', 'lupo_referers_raw', 'lupo_referers_daily',
                   'lupo_visits', 'lupo_visits_daily', 'lupo_sessions', 'lupo_memory_nodes');
...
The GC prunes expired memory nodes per PRD 38 retention policies.
```
