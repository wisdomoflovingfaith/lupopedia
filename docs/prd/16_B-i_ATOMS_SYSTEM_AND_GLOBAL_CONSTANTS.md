...
lupopedia.headers:
  ...
  prd_cluster: 00_A-i_16_B-i_16_C-i
...
**Companion reference:** Field-level pointer semantics for **`atoms_toon`** remain in **`docs/doctrine/lupopedia-headers/atoms_toon_schema.md`** (reference, not a PRD).
...
- **`16_lupopedia_headers.md`** defines the **header envelope**: the **22** YAML keys under **`lupopedia.headers`**, order, presence rules, and validation policy (**Header freeze rule (4.1.5)** applies).
...
**See also:** **[PRD 40](40_versioning_doctrine.md)** ??? product version bands, header **`header_format_version`** vs ship version, and upgrade policy. Values under **`constants.versioning`** in the global atoms bundle **MUST** stay aligned with **PRD 40** and **`config/global_atoms.yaml`**.
...
- Global bundle example: **`memory/atoms/lupopedia_global_constants.atom.toon`** (authoritative path referenced by multiple PRD 16 family headers today).
...
- **`memory/atoms/`** for cross-cutting global constants and similar **immutable** bundles.
- Future domain-specific atoms MAY live under **`memory/{channel_key}/...`** when header **`trust_tier`** and routing require it; path MUST be stable and documented before use.

**Linkage via `atoms_toon`**

- **`atoms_toon: null`** ??? no atom sidecar for this artifact (default).
- **`atoms_toon: "memory/atoms/lupopedia_global_constants.atom.toon"`** ??? this file participates in header-field and global-constant truth carried in that bundle.
...
|-------------|-------------------------|--------|
| **`memory/atoms/lupopedia_global_constants.atom.toon`** | **`config/global_atoms.yaml`** + **`scripts/update_atom.py`** | Keep YAML, JSON, and `.atom.toon` in lockstep. |
...
```json
{
  ...
  "constants": {
    "versioning": {
      "current_lupopedia_version": "4.1.5"
    },
    "header_fields": {
      "count": 22,
      "order": [
...
lupopedia.headers:
  ...
  path_from_lupopedia_root: "docs/prd/16_B_ATOMS_SYSTEM_AND_GLOBAL_CONSTANTS.md"
  web_path: "https://www.lupopedia.com/lupopedia/docs/prd/16_B_ATOMS_SYSTEM_AND_GLOBAL_CONSTANTS.md"
  status: "active"
