...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/prd/32_A-i_ACTOR_AUTHORITY_AGENT_ROLES.md
  web_path: https://www.lupopedia.com/lupopedia/docs/prd/32_A-i_ACTOR_AUTHORITY_AGENT_ROLES.md
  status: active
  when_updated: '20260513033046'
  trust_tier: canonical
  questions_toon: null
  memory_toon: memory/development/canonical/1026/04/32_actor_authority_agent_roles.toon
  atoms_toon: null
  transcript_jsonl: 0/development/prd_files/actor-authority-agent-roles
  artifact_type: prd
  ...
  prd_cluster: 00_A-i_32_A-i
  title: 'PRD 32: Actor Authority and Agent Roles'
  summary: Actor hierarchy, approval authority matrix, red team roles, escalation procedures, and agent interaction protocols with clear separation between governance authority and department-scoped operational act-as.
---
# PRD 32: Actor Authority and Agent Roles

Delegation: cursor:root (see repository commit conventions).
...
| **THOTH** | 26 | Knowledge & Records | High | Documentation, knowledge management |
...
| **HEPHAESTUS** | 14 | Implementer | Medium | Code implementation, builds |
...
### 2.3.1 THOTH ??? Canonical Actor Role Clarification

[CORRECTED 20260414] THOTH actor_id is **26**. The previous table entry of 9 was stale and has been corrected. Resolution basis: PRD 16 (when_updated: 20260412163625) explicitly documents actor_id 26 and is more recent than this PRD (when_updated: 20260410130550). Implementation (`DialogMvpService::THOTH_ACTOR_ID = 26`), `channels/index.php`, and all doctrine files consistently use 26. The HEPHAESTUS entry in Section 2.2 was also corrected from 16 to 14 (confirmed by PRD 07 Application Layer table).

**THOTH's actual runtime function (from system design, not just governance):**

THOTH's role is NOT passive documentation ("Knowledge & Records"). THOTH is an active real-time constitutional validator:

1. **Reads every message** in the active channel stream ??? it is the only agent with full read access to dialog history.
2. **Validates against canonical truth** ??? compares any suggestion or output against canonical memory nodes (1026... trust tier, PRD 43).
3. **Catches predictive-text mistakes** ??? AI systems are confidently wrong in repeatable, systematic ways. THOTH's specific mandate is to intercept these before they enter the codebase. Examples: `tinyint(1)` ??? must be `tinyint`; `INT(11)` ??? must be `INT`; `AUTO_INCREMENT` ??? must use `IdGenerator`.
4. **Raises [ALERT]** ??? posts directly into the dialog stream, not through a side-channel, to halt the unconstitutional operation where it occurs.
5. **Does NOT build** ??? THOTH never generates code, never approves PRDs, never implements. Pure monitoring and correction.

**Contradiction detection:** When a staging record or agent proposal contradicts a canonical record, THOTH raises an alert. The staging record is not "newer" ??? it is unverified until promoted. See PRD 43 conflict resolution rule.

### 2.3.2 VISH ??? Context and Organization Monitor (Concept)

VISH (Vishwakarma) is a planned monitoring agent focused on context organization. Unlike THOTH, which checks correctness, VISH checks where things belong.

**Problem it solves:** Human operators and builder agents naturally drift across contexts in a single thread ??? code implementation slides into prompt writing slides into blog content. Without a monitor, all of this lands in one thread and becomes unsearchable mixed context.

**VISH's function:**
- Reads the same stream as THOTH
- Tracks the semantic context of messages over time (not content, but purpose: is this implementation? documentation? blog writing?)
- Detects context drift and suggests reclassification into the appropriate collection/tab
- Example: "This conversation has shifted from implementation to prompt refinement. Collection: development, Tab: blog/prompt refinement"

**Status:** Concept documented, not yet implemented. Actor row not yet created. See PRD 73 for the collections/tabs architecture that VISH would operate against.
