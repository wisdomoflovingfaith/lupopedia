...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/prd/06_A-i_CONTENT_MANAGEMENT.md
  web_path: https://www.lupopedia.com/lupopedia/docs/prd/06_A-i_CONTENT_MANAGEMENT.md
  status: active
  when_updated: '20260513033046'
  trust_tier: canonical
  questions_toon: null
  memory_toon: memory/development/canonical/1026/04/06_content_management.toon
  atoms_toon: null
  ...
  prd_cluster: 06_A-i_00_A-i_FORBIDDEN_AND_WHY_06_A_CONTENT_MANAGEMENT
...
### 3.x Content Filesystem Invariant (Constitutional)

All file-backed content SHALL reside within the canonical filesystem path structure:
...
This rule is a **hard invariant** and SHALL NOT be violated under any circumstance.

---


#### 3.x.1 Path Structure Definition

- `content/`  
  Root directory for all file-backed content.
- `federation_node/`  
  Literal directory segment. Required. Identifies the following segment as the federation node ID.
- `{federation_node}`  
  Integer namespace representing an isolated content universe.
- `{channel_key}`  
  Semantic namespace within a federation node.
- `{YYYY}/{MM}`  
  Timestamp anchoring for deterministic storage and chronological ordering.

---


#### 3.x.2 Hard Requirements

1. Content MUST NOT be placed outside the invariant path.
2. The literal `federation_node/` segment MUST be present.
3. `{YYYY}` MUST be a 4-digit year.
4. `{MM}` MUST be a 2-digit month (01???12).
5. Directory skipping (e.g., omitting month) is forbidden.
6. Files SHALL be written only into their timestamp-resolved directory.

Violation of any of the above SHALL be treated as a **Level 4 structural failure**.

---

#### 3.x.3 Determinism Guarantees

This structure guarantees:

- Single canonical location per content artifact
- No path ambiguity
- No duplicate resolution paths
- Deterministic lookup from metadata (`file_path_from_root`)

---

#### 3.x.4 Teaching Source
...
The teaching artifact provides explanatory context. This PRD defines enforcement.
...
- `content/actor/10000/reference/minimal_tables.md`

### Canonical Rules

1. **File-backed content** is stored as a file (`.md` or `.txt`) under `content/` in the canonical structure above.
...
11. **file_path_from_root:** Repo-relative path from project root; no leading slash. Example: `content/federation_node/0/captains_log/entry_001.md`.
12. **Timestamps:** Packed UTC BIGINT `YYYYMMDDHHIISS`. Never Unix epoch, never ISO strings.
13. **Prefix:** All file-backed content must live under `content/` at the project root. No ad hoc or legacy locations.
...
A content row in `lupo_contents` without a corresponding file header (for file-backed content) is a constitutional violation. A file on disk without a header is **not** a violation; it is simply unprocessed. The violation clock starts when ANUBIS has had the opportunity to process the file and did not produce a result.
