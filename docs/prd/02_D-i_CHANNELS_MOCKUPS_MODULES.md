...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/prd/02_D-i_CHANNELS_MOCKUPS_MODULES.md
  web_path: https://www.lupopedia.com/lupopedia/docs/prd/02_D-i_CHANNELS_MOCKUPS_MODULES.md
  status: active
  ...
  prd_cluster: 00_A-i_02_D-i
  title: PRD 02 -- Channels UI Mockups and Modules
  summary: UI mockups APIs transport template-first Crafty visitors lupo_sessions source fall-forward dialog session endpoints projection parity. + Thread Visualizer constitutional requirement with Crafty Syntax legacy precedent.
---
# PRD 02 -- Channels UI Mockups and Modules


Thread Visualizer UI Requirement: Channels must support multiple isolated threads displayed in the same column using distinct colors. Actors see only messages belonging to their own color/thread. Legacy Crafty Syntax precedent (2011) demonstrated human operators successfully managing parallel threads on one screen. AI actors must receive the same capability with stricter isolation rules.
...
## UI Implementation Doctrine (Constitutional Rule)

**Captain Wolfie’s Design Philosophy (Mandatory for All Agents)**

Lupopedia uses a **hand-crafted, multi-layer liquid design** approach. All UI elements are built using:

- **9+ div layers** with custom background images created in GIMP
- **No centering**
- **No stretching**
- **No flexbox**
- **No CSS Grid**
- **No CSS frameworks** (Bootstrap, Tailwind, Bulma, etc.)
- **No modern CSS tricks** for positioning or scaling

All visual assets are custom-made by Captain Wolfie (agent_id 1) in GIMP and are used as-is with precise pixel positioning via `lupopedia-layers.js`.

**lupopedia-layers.js (Captain Wolfie’s Custom System)**

Captain Wolfie (agent_id 1) has written a custom JavaScript library called `lupopedia-layers.js` that handles:

- Layering multiple images by `z-index`
- Moving images around the screen
- Mouse tracking and interaction
- Complex liquid layouts without relying on framework CSS

All agents and tools **MUST** use `lupopedia-layers.js` for any dynamic layering, movement, or mouse-based interactions instead of modern CSS techniques.

**Rules for All Agents (Including VS Code, Gemini, Claude, Cursor, etc.):**

1. **NEVER** suggest or use flexbox, grid, centering classes, or any CSS framework.
2. **NEVER** use `margin: auto`, `display: flex`, `justify-content`, `align-items`, or similar modern CSS properties for layout.
3. **ALWAYS** use Captain Wolfie’s 9-layer div system with custom GIMP background images.
4. **ALWAYS** use `lupopedia-layers.js` for any dynamic movement, layering, or mouse interaction.
5. **MUST** keep the visual style consistent with the dark fantasy theme shown in the first draft interface (wolf header, parchment elements, hand-crafted look).
6. Any UI code that violates these rules is **FORBIDDEN** and must be rejected.

**Purpose of This Rule:**
This ensures Lupopedia maintains a unique, hand-crafted aesthetic that feels solid and intentional rather than generic modern web design. It also prevents visual inconsistency across different agents and tools.

**Where to Add This Section:**
Insert this new section as **Section 5** in `02_D_CHANNELS_MOCKUPS_MODULES.md`, right after the existing “Thread Visualizer UI Requirement” note.
...
*Sidebar baseline (additive, mockup reference: `templates/channels/index.php`):* Channel picker, **Visitors** (Browsing plus Wants chat), **Users**, **Agents**, **Recent Files**, **Recent Tasks**; collapsible summaries share the **Right sidebar chrome** rules under **Section E**.
...
**Layout (template mockup, additive):** The **`templates/channels/index.php`** operator sidecar stacks **Channel** selector, **Visitors** (Browsing Visitors collapsible, Wants chat strip, **Users** collapsible), **Agents** collapsible, **Recent Files** collapsible, **Recent Tasks** collapsible (**Recent Files** and **Recent Tasks** default **collapsed** on load). Collapsible **summary** headers use one **light bar** style (**`#e0e0e0`** background, **`#222222`** text) across Browsing Visitors, Users, Agents, Files, and Tasks; disclosure pattern MUST match across sections.
...
**Split UI note (additive):** When the operator console **splits** the visitor sidebar into **Browsing Visitors**, **Wants Chat**, and **bottom visitor tabs** (template-first baseline in **`templates/channels/index.php`**), per-subsection rules are normative under **Right sidebar chrome** (**E.a** through **E.c**) below. The single **`N`** triple in this paragraph remains the **data** summary; **UI** disposition of **`N = 1`** between **Invited** (Browsing) versus **Accept Chat** (Wants) MUST be documented in implementation notes (message direction, **`visitor_status`**, or equivalent).
...
**Endpoint family** (exact paths and PHP entrypoints under **`api/`** or **`channels/`** per implementation; names are stable contracts):
...
**Right sidebar chrome (mockup baseline, additive, normative for template `templates/channels/index.php`):** Operator sidecar sections below align with **projection** and **visitor_status** doctrine; production **`channels/index.php`** SHOULD converge unless a written exception states otherwise.
