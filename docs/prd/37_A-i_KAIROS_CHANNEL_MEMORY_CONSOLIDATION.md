...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/prd/37_A-i_KAIROS_CHANNEL_MEMORY_CONSOLIDATION.md
  web_path: https://www.lupopedia.com/lupopedia/docs/prd/37_A-i_KAIROS_CHANNEL_MEMORY_CONSOLIDATION.md
  status: active
  when_updated: '20260513033046'
  trust_tier: canonical
  questions_toon: null
  memory_toon: memory/development/canonical/1026/04/37_kairos_channel_memory_consolidation.toon
  atoms_toon: null
  transcript_jsonl: 0/development/prd_files/kairos-channel-memory-consolidation
  artifact_type: prd
  ...
  prd_cluster: 37_A-i_00_A-i_FORBIDDEN_AND_WHY_37_A_KAIROS_CHANNEL_MEMORY_CONSOLIDATION
  title: 'PRD 37: KAIROS channel memory consolidation'
  summary: null
---
...
**Namespace purpose.** This PRD defines **product behavior**, **truth rules**, and **data contracts** for KAIROS in the **web chat** context. Runtime orchestration is **PHP-first** (`KairosConsolidationService`, `POST api/kairos/tick`); LLM is optional assist only where doctrine allows????????never the sole authority for stored memory.

**KAIROS identity (tooling).** Configuration lives under **`agents/kairos/`**; registry **`kairos`** maps to **`actor_id` 115** for **edge attribution** (`lupo_edges.actor_id`) and service defaults. KAIROS does **not** post chat bubbles as part of this PRD; it **writes `lupo_memory_nodes`**, typed links in **`lupo_memory_edges`**, and (where used) **`lupo_edges`** for cross-object references (**PRD 38**).

### 1.1 Correct mental model

| Concept | Definition |
|--------|------------|
| **Observation** | A **`lupo_memory_nodes`** row with **`memory_type` = `kairos_observation`**, capturing an **atomic note** (often derived from dialog or manual seed), with **`context_json`** (and optional **`.kairos`**) carrying **confidence**, **topic_key**, **department_id**, and **provenance** (e.g. source **`dialog_message_id`**, **`channel_id`**, **`from_actor_id`**). |
| **Consolidated memory** | A **`lupo_memory_nodes`** row with **`memory_type` = `kairos_memory`**, produced by merging **multiple** observations that **normalize** to the same text; linked via **`kairos_consolidates_from`** primarily on **`lupo_memory_edges`**; **`lupo_edges`** MAY hold additional cross-object **`kairos_*`** edges per **section 2.0** and install/code. |
...
### 2.0 Edge table roles (normative)

- **Primary (memory-native):** Typed edges **between memory nodes** (e.g. **`kairos_consolidates_from`**, **`consolidated_into`**, **`merged_into`**) MUST be stored in **`lupo_memory_edges`** (`from_memory_node_id`, `to_memory_node_id`, **`edge_type`** per TOON). This is the canonical table for memory-graph lineage in 4.0.x.
- **`lupo_edges`:** Used for the **cross-object semantic graph** where **`left_object_type` / `right_object_type`** include **`memory_node`** and endpoints may span object kinds (or for product-wide edge weighting/analytics). Some KAIROS paths (e.g. **`kairos_contradicts`**) may write here per **section 6** and PHP constants. The tables are **not** interchangeable: prefer **`lupo_memory_edges`** for memory-node-to-memory-node consolidation unless install/code explicitly records the relationship on **`lupo_edges`**.
...
- **KAIROS-specific edges today:** **`kairos_consolidates_from`**, **`kairos_contradicts`**. **Future:** edges from **`memory_node`** to **`dialog_message`** (or **`metadata`**) as **evidence_of**.
...
**`lupo_memory_nodes`** (TOON under `database/lupopedia/toon/`):

- **`memory_node_id`**, **`owner_actor_id`**, **`memory_type`**, **`memory_key`**, **`memory_value`**, **`context_json`**, **`content_hash`**, **`expires_ymdhis`**, UTC BIGINT timestamps, soft delete.

**`lupo_memory_edges`**:

- Typed links between **`memory_node_id`** values (**`edge_type`**, **`from_memory_node_id`**, **`to_memory_node_id`**, etc.).

**`lupo_edges`** (`database/lupopedia/toon/lupo_edges.toon.json`):

- **`edge_type`**: at minimum **`kairos_consolidates_from`**, **`kairos_contradicts`** (existing PHP constants) where product code records cross-object edges.
- **`left_object_type` / `right_object_type`**: use **`memory_node`** (or install-aligned **`object_type`** strings) for memory-to-memory links; future **`dialog_message`** endpoints as needed.
...
1. **API:** **`POST api/kairos/tick`** requires **authenticated** session; uses **current user????????s `actor_id`** for consolidation scope today????????document **narrowing** vs **channel admin** path when channel-wide jobs are added.
...
- **SHALL** treat **`THREAD_INDEX.md`** and header **`when_updated`** / **`lupopedia.edges`** as **stronger** ordering signals than directory order when ingesting **file-backed** provenance into **`lupo_memory_nodes`** (where applicable).
...
**Tooling:** **`python scripts/scaffold_implementation.py add-status`** creates a dated **`status/`** file, optional **`references`** / **`supersedes`** edge to the **prior** status artifact, and appends **`status/THREAD_INDEX.md`**.
...
- **Implementation:** `app/Services/Kairos/KairosConsolidationService.php`, `includes/modules/api/kairos-api.php`, `ui/js/chat-display.js`  
- **Agents:** `agents/kairos/agent.json`, **PRD 07**  
- **Chat:** **PRD 18**  
