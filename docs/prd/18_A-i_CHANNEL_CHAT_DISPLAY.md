...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/prd/18_A-i_CHANNEL_CHAT_DISPLAY.md
  web_path: https://www.lupopedia.com/lupopedia/docs/prd/18_A-i_CHANNEL_CHAT_DISPLAY.md
  status: active
  ...
  atoms_toon: null
  transcript_jsonl: 0/development/prd_files/channel-chat-display
  artifact_type: prd
  ...
  prd_cluster: 00_A-i_18_A-i
  title: 'PRD: Channel Chat Display'
  summary: null
---
...
**Canonical API (do not duplicate):** Message list and post for channel chat are implemented in `includes/modules/api/channels-api.php`, routed as **`{LUPOPEDIA_PUBLIC_PATH}/api/channels/{channel_id}/messages`**. Extend this endpoint for transport variants (`format=buffer`, `format=image`); do not add a parallel `api/chat/messages.php` unless it is a documented thin wrapper. Pretty channel pages: **`/channels/{id}/`** and **`/channels/{id}/thread/{thread_id}/`** (see `.htaccess` and `lupo_route_slug` in `module-loader.php`).
...
**Canonical column (DDL):** **`lupo_dialog_messages.to_actor_id`** ???????? see **`database/lupopedia/json/lupo_dialog_messages.json`**. Some directives use the prose synonym **???????said-to actor???????** / **`said_to_actor_id`**; **semantics are the same** (routing addressee only).
...
When **`to_actor_id`** matches a **service persona** the product wires for auto-assist (e.g. **LILITH `actor_id` 2**; **THOTH**, **MAAT**, others ???????? **resolve all ids from `database/lupopedia/actors/registry.json`**), **ROSE** (or the PHP router behind it) **MAY**:
...
```
ui/js/chat-display.js          # Modern UI controller (product-specific; ES2015+ allowed if not loaded on legacy path)
ui/js/chat-display-legacy.js   # ES3-safe XHR/ActiveX/image fingerprint helpers (legacy transport tiers)
includes/modules/api/channels-api.php
+-- GET  api/channels/{id}/messages?since=YYYYMMDDHHIISS&format=json|buffer|image
+-- POST api/channels/{id}/messages  (JSON body: body, message_type, routing_type, thread_id, ???????)
+-- format=image: requires whatplace=hundreds|tens|ones; HTTP 302 to /ui/images/digitN.gif (fingerprint = latest row created_ymdhis % 1000, 0 if no rows)
```
...
**Related legacy transport (reference):** `livehelp_js.php` (XHR/ActiveX detection pattern), root `image.php` (digit GIF protocol for visitor session), `ui/images/digit0.gif`????????`digit9.gif` (static tiles used after `format=image` redirect).

**Operator / livehelp polling (separate):** `api/channel/messages` ???????? `channel-messages-api.php` remains the older channel poll surface; new chat display work should prefer **`api/channels/.../messages`** unless integrating that legacy path.

---

## Collections Dual-Mode UI Behavior

### Display Mode

* When invoked in `content/index.php`, Collections are used to display grouped content in a book-style interface
* Collections determine which content is shown, not how it is executed

### Context-Transfer Mode

* When invoked in `channels/index.php`, Collections are used as selectable context
* The user selects an actor to receive the collection/content
* The selected content is passed as context to that actor
* Collections themselves do NOT define routing logic

### Boundary Rule

* PRD 02 defines Collections as structure
* PRD 18 defines how Collections behave in UI
* Actor execution and interpretation occur outside this PRD
...
`GET {LUPOPEDIA_PUBLIC_PATH}/api/channels/{channelId}/messages?since={14_digit_ymdhis}&format=json`

Response shape (JSON): `success`, `channel_id`, `messages[]` with `message_id`, `actor_id`, `actor_name`, `body`, **`created_at`** (BIGINT YmdHis), etc.

**Legacy / ES3 transport (implemented stub):** Use **`ui/js/chat-display-legacy.js`** for XMLHttpRequest + ActiveX + optional **image fingerprint** (`format=image` + `whatplace=???????`; server responds with **302** to `ui/images/digitN.gif` so `img.src` can be parsed). For hidden iframe reads, **`format=buffer`** returns the same JSON as **text/plain**. Do **not** use optional chaining, `const`, arrow functions, or `class` in that file.
...
function pollChannelMessages(publicPath, channelId, since, onMessages) {
    ...
    var url = base + "/api/channels/" + channelId + "/messages?since=" + encodeURIComponent(since || "") + "&format=json";
...
Future **rate limiting** and **CSRF** for browser POSTs should be added in or behind this same endpoint (or middleware invoked from it), not a duplicate `api/chat/messages.php`, unless that file is explicitly documented as a wrapper.
...
When browser POST is added for **`channels-api`**, use session-backed CSRF and send token via header or JSON field. Build POST URL as **`{LUPOPEDIA_PUBLIC_PATH}/api/channels/{channelId}/messages`** with **`Content-Type: application/json`** and body **`{"body":"..."}`** (plus routing fields as needed).
