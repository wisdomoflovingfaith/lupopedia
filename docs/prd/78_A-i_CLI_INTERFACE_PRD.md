...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/prd/78_A-i_CLI_INTERFACE_PRD.md
  web_path: https://www.lupopedia.com/lupopedia/docs/prd/78_A-i_CLI_INTERFACE_PRD.md
  status: active
  when_updated: '20260513033046'
  trust_tier: canonical
  questions_toon: null
  memory_toon: memory/development/canonical/1026/04/78_cli_interface_prd.toon
  atoms_toon: null
  transcript_jsonl: 0/development/prd_files/cli-interface-prd
  artifact_type: prd
  ...
  prd_cluster: 00_A-i_78_A-i
  title: 'PRD 78: CLI Interface (Expanded)'
  summary: null
---
...
**Examples:**
```bash
php bin/lupo.php memory add --actor 1 --type observation --key "login_errors" --value '{"error":"Invalid password"}'
php bin/lupo.php memory list --actor 1 --type observation
php bin/lupo.php memory get --memory-id 12345
php bin/lupo.php memory export --actor 1 --format json
php bin/lupo.php edges add --from 12345 --to 67890 --type "supports"
```
...
```bash
php bin/lupo.php use --actor 1
php bin/lupo.php switch --actor 2
```
...
Export written: memory/2026/04/20260407_130000_actor_1_observation_kairos_observation_login_errors.json
```

### 5.2 List Memory Nodes

```bash
php bin/lupo.php memory list \
    --actor 1 \
    --type observation \
    --status supported \
    --limit 20
...
```bash
php bin/lupo.php memory get --memory-id 1234567890123456
```
...
```bash
php bin/lupo.php memory update \
...
```bash
php bin/lupo.php memory delete --memory-id 1234567890123456
```

**Output:**
```
Memory 1234567890123456 soft-deleted (is_deleted=1)
Export file removed: memory/2026/04/20260407_130000_actor_1_observation_....json
```

### 5.6 Export Memory to Filesystem

```bash
php bin/lupo.php memory export --actor 1 --output-dir ./memory-backup
```
...
```bash
php bin/lupo.php memory import --input-dir ./memory-backup --actor 1
```
...
php bin/lupo.php memory archive --memory-id 202604081200001234

# Archive all memory for an actor older than N days
php bin/lupo.php memory archive --actor 116 --older-than 90

# Preview without actually archiving
php bin/lupo.php memory archive --actor 116 --older-than 90 --dry-run
```
...
**Output (example):** `Archived 202604081200001234 ???????? 102604081200001234; edge archived_to created; export memory/1026/04/...`

### 5.9 Restore from Archive (reverse of ????5.8)

```bash
php bin/lupo.php memory restore --memory-id 102604081200001234
```
...
```bash
php bin/lupo.php edges list --memory-id 1234567890123456
```

### 6.3 Delete Edge

```bash
php bin/lupo.php edges delete --edge-id 9876543210987654
```
...
```bash
php bin/lupo.php memory related --memory-id 1234567890123456 --max-depth 2
```
...
```bash
php bin/lupo.php memory contradictions --actor 1 --topic "login_errors"
```
...
```bash
php bin/lupo.php memory search --query "login error" --actor 1 --limit 20
```
...
Memory Export Path: memory/2026/04/20260407_120000_actor_1_root_actor_root_context.json
```
...
```php
// includes/classes/cli/MemoryCommands.php
...
php bin/lupo.php memory add --actor 1 --type observation --key "chat:user_feedback" --value '{"user":"Eric","feedback":"Likes the eye animation"}'

# Query memory before answering
php bin/lupo.php memory list --actor 1 --type observation --limit 10

# Get full context for an actor
php bin/lupo.php memory get --memory-id $(php bin/lupo.php memory list --actor 1 --type root --format id-only)

# Find contradictions to resolve
php bin/lupo.php memory contradictions --actor 1 --topic "database_schema"
```
...
```bash
alias mem='php /path/to/bin/lupo.php memory'
alias edge='php /path/to/bin/lupo.php edges'
alias who='php /path/to/bin/lupo.php whoami'
```
...
| CLI memory writes auto-export to filesystem | `memory/` updated after each command |
