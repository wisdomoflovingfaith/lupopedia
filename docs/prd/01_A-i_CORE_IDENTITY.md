...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/prd/01_A-i_CORE_IDENTITY.md
  web_path: https://www.lupopedia.com/lupopedia/docs/prd/01_A-i_CORE_IDENTITY.md
  status: active
  when_updated: '20260513033046'
  trust_tier: canonical
  questions_toon: null
  memory_toon: memory/development/canonical/1026/04/01_core_identity.toon
  atoms_toon: null
  transcript_jsonl: 0/development/prd_files/core-identity
  artifact_type: prd
  ...
  prd_cluster: 01_A-i_00_A-i_FORBIDDEN_AND_WHY_01_A_CORE_IDENTITY
  title: 'PRD: Core Identity Database Tables'
  summary: null
---
...
- Memory graph (via `lupo_memory_nodes` / `lupo_memory_edges`; see PRD 38)
...
| **Storage** | Filesystem (`agents/{agent_key}/`) | Database + workspace (`actors/`) |
...
# memory.json in actors/ is DEPRECATED ???????? do not create new memory.json files.
```
...
9. Learned behavior stored as root memory node at `memory/YYYY/MM/{memory_slug}.json`; registered in `lupo_memory_nodes`; all memory relationships expressed via `lupo_edges` (4.0.96+). `memory.json` is deprecated.
...
|-------|------|-------------------|
| 1-2025 | System Actor | `actors/{actor_id}/` |
| 2026+ | Runtime Actor | `actors/YYYY/MM/{actor_id}/` |
...
| `lupo_memory_nodes` | Learned / consolidated memory (incl. KAIROS types) | `memory_node_id` |
...
| `lupo_memory_nodes` | Unified memory storage (per owner; KAIROS + general) | `memory_node_id` | `owner_actor_id` / graph edges |
...
### `lupo_memory_nodes` (and `lupo_memory_edges`)

**Purpose:** Canonical **unified memory graph** for runtime learning (including **KAIROS** observation / consolidation **`memory_type`** values), distinct from static **`agents/`** files. Schema matches **`install_new_lupopedia.sql`**; see **PRD 38** and **PRD 37** for semantics.

**Normative columns (summary):** `memory_node_id`, `owner_actor_id`, `memory_type`, `memory_key`, `memory_value`, `context_json`, `content_hash`, `expires_ymdhis`, `created_ymdhis`, `updated_ymdhis`, soft-delete flags. **Typed relationships** between nodes use **`lupo_memory_edges`** (`from_memory_node_id`, `to_memory_node_id`, `edge_type`, etc.).

**Retention:** Policy-driven via **`expires_ymdhis`** and GC (**`GarbageCollector`**, PRD 19). Cleanup MUST use soft delete on lineage tables unless a documented scratch-table exemption applies.
...
| training_metadata | JSON | YES | NULL | Additional metadata (`source`, `confidence`, `reviewed_by_actor_id`, optional `result_memory_node_id` / skill / tool refs) per `docs/doctrine/TRAINING_METADATA_SCHEMA.md` and product conventions |
...
- **Workspace path resolution rule:**
  - if `actor_id < 2026` (core actors): `actors/<actor_id>/`
  - otherwise (runtime actors): `actors/YYYY/MM/<actor_id>/` where YYYY/MM are extracted from the timestamp ID
...
UPDATE {{prefix}}actors
SET is_deleted = 1, deleted_ymdhis = :now, updated_ymdhis = :now
WHERE actor_id = :source_id;

-- 6. Add provenance edge for merge lineage
INSERT INTO {{prefix}}edges
  (edge_id, left_object_type, left_object_id, right_object_type, right_object_id,
   ...
   is_deleted, deleted_ymdhis, created_ymdhis, updated_ymdhis)
VALUES
  (:edge_id, 'actor', :source_id, 'actor', :target_id,
   'merged_into', 'identity', 'Actor merge provenance edge', :actor_id,
   0, 0, :now, :now);
```
...
|----------------|----------------|---------|
| < 2026 | `actors/{actor_id}/` | `actors/2/` |
| ??????? 2026, year YYYY | `actors/YYYY/MM/{actor_id}/` | `actors/2026/03/202603311735001234/` |
...
if ($actor_id < 2026) {
    ...
    $path = "actors/{$year}/{$month}/{$actor_id}/";
}
```
...
| metadata_json | JSON | YES | NULL | Additional relationship metadata. **Schema**: See `docs/doctrine/METADATA_JSON_SCHEMA.md` |
| created_ymdhis | BIGINT | NO | (application) | UTC timestamp YYYYMMDDHHIISS |
| updated_ymdhis | BIGINT | NO | (application) | UTC timestamp YYYYMMDDHHIISS |
| is_deleted | TINYINT | NO | 0 | Soft delete flag |
| deleted_ymdhis | BIGINT | YES | NULL | UTC timestamp when deleted |
...
**Deployment note:** Behind a **trusted** reverse proxy on a private network, ???????first public IP??????? heuristics may fail; operators may need a **trusted-proxy allowlist** and to accept RFC1918 forwarded addresses when validated. **IPv6 policy:** use the `/64` prefix (first 64 bits) plus session cookie name for fingerprinting; full IPv6 addresses are too specific due to privacy-address rotation.
...
Any value retrieved from `$_GET`, `$_POST`, or `$_COOKIE` MUST be treated as untrusted and validated before use (`$UNTRUSTED` discipline; constitutional section 17.8).
...
| memory_slug | VARCHAR(128) | NO |  | Filesystem-safe slug for mapping to memory/YYYY/MM/{memory_slug} |
| memory_value | TEXT | YES | NULL | Memory content/value |
| context_json | JSON | YES | NULL | Additional context (confidence, provenance, etc.) |
| trust_tier | VARCHAR(32) | NO | 'staging' | Trust ladder tier (`seed` \| `staging` \| `canonical` \| `archive`); planned install extension |
| channel_key | VARCHAR(64) | NO |  | Channel affinity for filesystem/memory routing; planned install extension |
| created_ymdhis | BIGINT | NO | (application) | UTC timestamp YYYYMMDDHHIISS |
| updated_ymdhis | BIGINT | NO | (application) | UTC timestamp YYYYMMDDHHIISS |
| is_deleted | TINYINT | NO | 0 | Soft delete flag |
| deleted_ymdhis | BIGINT | YES | NULL | UTC timestamp when deleted |
...
- `memory_slug` is a unique, filesystem-safe identifier for mapping memory nodes to disk. Memory JSON files are stored in `memory/YYYY/MM/{memory_slug}` where YYYY/MM is derived from `created_ymdhis`.
- `memory_slug` generation rule: `{owner_type}/{memory_type}/{created_ymdhis}-{random_suffix}` (example: `actor/observation/20260410120000-a3f9`) to preserve uniqueness and filesystem safety.
...
- **Cleanup (soft delete, constitutional default):** Periodic job marks expired rows:
  ...
  ```
  Use packed UTC BIGINT for `:now` and `:cutoff` (`gmdate('YmdHis')` / doctrine helpers).
