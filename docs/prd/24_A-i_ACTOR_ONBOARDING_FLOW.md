...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/prd/24_A-i_ACTOR_ONBOARDING_FLOW.md
  web_path: https://www.lupopedia.com/lupopedia/docs/prd/24_A-i_ACTOR_ONBOARDING_FLOW.md
  status: active
  when_updated: '20260513033046'
  trust_tier: canonical
  questions_toon: null
  memory_toon: memory/development/canonical/1026/04/24_actor_onboarding_flow.toon
  atoms_toon: null
  transcript_jsonl: 0/development/prd_files/actor-onboarding-flow
  artifact_type: prd
  ...
  prd_cluster: 00_A-i_24_A-i
  title: 'PRD 24: Actor Onboarding Flow'
  summary: null
---
...
- **Workspace path:**
  - If actor_id < 2026: `actors/{actor_id}/`
  - If actor_id >= 2026: `actors/YYYY/MM/{actor_id}/`
...
1. Create workspace directory: `actors/YYYY/MM/{actor_id}/`
2. Copy template from source agent: `agents/{agent_key}/`
3. Create `agent_link.json` referencing source agent
4. Create `context.json` with department context
5. Register root memory node in `lupo_memory_nodes`; file stored at `memory/YYYY/MM/{memory_slug}.json` (4.0.96+; `memory.json` is DEPRECATED)
