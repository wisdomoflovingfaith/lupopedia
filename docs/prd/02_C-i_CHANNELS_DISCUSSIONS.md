...
lupopedia.headers:
  ...
  thread_key: channels-discussions
...
- **2026-05-13 (header prd_cluster)**: Set **`prd_cluster`** to **`00_A-i_02_C-i`** (replaces verbose legacy string; aligns with PRD 16 cluster token style).
- **2026-05-12 (channel cardinality)**: Added normative **Channel vs thread** block: **one channel = many parallel threads**; explicit **MUST NOT** equate channel with a single thread or with Discord-style single-river mental models; table row and Summary updated.
- **2026-05-12 (CCV -- future schema sketch)**: Added **Future schema direction** subsection under CCV: deferred column sketches (`coherence_magnitude`, `pono_kapu_state`, centroid snapshot vs `lupo_channel_centroids` table) with portability notes (ENUM vs VARCHAR, JSON as dumb storage). **Still no** `install_new_lupopedia.sql` edit in this pass.
- **2026-05-12 (CCV -- Channel Coherence Vector)**: Added normative **Channel Coherence Vector (CCV)** section: mathematical definition (centroid, cosine magnitude), optional mood-weighted blend, Pono/Kapu/neutral as **channel-relative** doctrine labels (not moral verdicts), interpreter-layer JSON shapes (Lupopedia ID and BIGINT UTC doctrine), known risks (embedding SPOF, static centroid, echo chamber, scalar ambiguity, cost, cultural load), EMA centroid refinement, reference Python. **Scope:** every artifact indexed by **`channel_key`** (headers, atoms sidecars, memory paths, dialog rows, channel-scoped content) participates in CCV semantics when embeddings exist. **Explicit:** no new `install_new_lupopedia.sql` columns in this documentation-only pass; persistence is a later schema task after PRD lock.
- **2026-04-18 (release-line correction)**: Replaced misleading **orchestration deferred to 4.2.0** wording. **Normative:** **4.1.3** = required non-AI live help baseline (orchestration **not** a ship gate); **4.1.5???4.1.9** = continuing Lupopedia lines including orchestration/AI/command-center work on **fresh-install** increments; **4.2.0** = first **public** Lupopedia release for bundled external expectations; **no** supported Lupopedia-to-Lupopedia upgrade path before **4.2.0** (Crafty **3.7.5 ??? Lupopedia** remains the supported import). TOC, **Release line** table, Orchestration Doctrine intro, Summary, and scattered version labels updated accordingly.
...
- **Channel vs thread: one channel, many threads (normative -- anti-Discord confusion)**
- **Channel Coherence Vector (CCV) (normative extension -- specification ahead of schema; includes Future schema direction sketch)**
...
- **Orchestration Doctrine (v4.1.2; not required for 4.1.3 ship gate, continues in 4.1.5???4.1.9 development lines)**
...
### Channel vs thread: cardinality (normative -- agents MUST read)

**This subsection exists because implementers repeatedly confuse channels with single threads.** That confusion is **FORBIDDEN** for any ship-facing or agent-generated design.

#### What a channel is and is not

- **MUST** treat a **channel** as a **container** that holds **many separate threads at the same time**. Cardinality is **one channel : many threads** (typically each line in **`lupo_dialog_messages`** carries a **`dialog_thread_id`** or successor key; filesystem work uses **`channels/{federation_node_id}/{channel_key}/{thread_key}/`** per **PRD 17**, **PRD 29**). **N = 1** thread is legal; **N >> 1** is normal.
- **MUST NOT** model **channel == thread** or **channel == one implicit conversation**. A channel is **not** a single mega-thread by definition.
- **MUST NOT** import the **Discord-style** shortcut where a `#channel` feels like **one** undifferentiated river and thread identity is optional background. In Lupopedia, **each thread is its own lineage** under shared channel membership and policy; **many threads coexist in parallel** under one **`channel_key`** / **`channel_id`**.

#### Correct mental model (quick table)

| Wrong | Right |
|-------|-------|
| "The channel is the thread" | The channel is a **namespace + policy + membership shell** around **many** threads |
| "Pick channel, get the one thread" | Pick channel, then pick **which thread(s)** participate in this view (projection may **intermix** rows from **multiple** threads; each row still has thread identity) |
| Collapsing storage to "channel messages" only | Storage is **channel + thread** (and routing endpoints); **MUST** read **`install_new_lupopedia.sql`** / TOON for exact columns -- **MUST NOT** invent a single-thread schema |

### Channel, storage, projection, presence

| Concept | Definition |
|---|---|
| **Channel** | Shared **routing context** and **governed semantic container** for many threads (namespace, membership, policy). **`channel_key`** scopes headers, memory paths, and dialog within a **federation node** (domain). **One channel, many parallel threads** -- **not** one mega-thread. Membership lists who may participate; it does **not** grant message-body visibility. |
...
- APIs, transport, and validation surfaces already normative in this PRD, assembled per **Staged Development Workflow** and **Template-First** rules (no shortcut around **`templates/`** / **`lupo_t()`** for new ship-facing strings).

**Excluded from 4.1.3 ship acceptance (may continue in 4.1.5???4.1.9):**
...
## Channel Coherence Vector (CCV) (normative extension -- specification ahead of schema)

This section defines the **Channel Coherence Vector (CCV)** for Lupopedia: a **semantic alignment** contract for every **channel-scoped** object that carries the same **`channel_key`** as the active channel. **Database columns for embeddings and centroids are not added in this PRD edit**; implementers **MUST** follow **`install_new_lupopedia.sql`** and TOON doctrine when schema lands. Until then, CCV is **behavioral and interpreter** law.

### CCV scope (what participates)

**Normative:** Any persisted or header-bound artifact that is **scoped by `channel_key`** participates in CCV when an embedding exists or can be computed. Examples include, without narrowing the set:

- **`lupo_dialog_messages`** rows for that channel (see projection model above).
- **LUPOPEDIA HEADERS** (`channel_key` in YAML) on markdown, PRDs, logs, and channel artifacts.
- **Atoms sidecars** (`atoms_toon`) and **memory TOON** paths whose routing slug includes the same **`channel_key`** (see **PRD 16**, **PRD 38**, **PRD 51** path rules).
- **Channel-scoped content** and **tasks** that declare the channel in metadata or join tables (exact storage follows existing tables; CCV attaches to the row once embedded).

**MUST NOT:** Treat CCV as a substitute for **projection**, **presence**, or **HERMES** routing. CCV is an **orthogonal semantic quality signal**, not read access.

### Mathematical definition (fixed model, consistent embeddings)

Let channel **C** contain **N** objects indexed by **i**. Each object **o_i** has a real **D**-dimensional embedding **e_i** in R^D from **one** chosen embedding model and **one** revision of that model per deployment (**MUST** be consistent across all objects in **C** for comparable **m**).

**Step 1 -- Channel centroid (semantic center of mass)**

```
c = (1/N) * Sum(i=1..N) e_i
```

**Step 2 -- Object vector**

The object vector is the embedding of the object under the same model:

```
o = embed(object)
```

**Step 3 -- Coherence magnitude (cosine alignment)**

```
m = (o dot c) / (||o|| * ||c||),  m in [-1, +1]
```

Interpretation (non-normative English gloss):

- **m near +1.0** -- aligned with the channel centroid (doctrine label **pono**; see thresholds below).
- **m near 0.0** -- orthogonal / unrelated tone.
- **m near -1.0** -- anti-aligned (doctrine label **kapu**; channel-relative only).

**Step 4 -- Coherence vector (storage meaning)**

The **vector** stored or cached for the object is **o** (the embedding). The scalar **m** is the **coherence magnitude**. Implementers **MAY** also store **(o - c)** as an explicit **deviation signature** when policy requires explainability.

### Optional mood-weighted coherence

Let **m_obj** be a mood embedding for the object and **m_chan** the channel mood centroid (same dimension rules). Then:

```
m_w = alpha * m + (1 - alpha) * cos_sim(m_obj, m_chan)
```

**alpha** in **[0, 1]** is a product constant (example: **0.7** for "mostly semantic, some mood"). **MUST** document **alpha** in **`global_atoms.yaml`** (or successor) when enabled; **MUST NOT** hide **alpha** inside opaque prompts only.

### Pono / Kapu / neutral (doctrine labels, channel-relative)

**Normative thresholds (defaults; product MAY tune with Captain amendment and decision thread):**

- **pono** -- **m >= tau_p** (default **tau_p = 0.65**)
- **kapu** -- **m <= tau_k** (default **tau_k = -0.25**)
- **neutral** -- otherwise

**Doctrine guard:** **pono** and **kapu** are **channel coherence labels**, not moral, religious, or legal judgments of persons. UI copy **MUST** use **`lupo_t()`** keys with English fallbacks that state "relative to this channel centroid" where confusion is likely (see **PRD 00** locale rules).

### Time-decayed centroid (strongly recommended)

Static **c** over-weights early objects. **SHOULD** maintain an exponential moving average (EMA) on ingest:

```
c_new = beta * c_old + (1 - beta) * e_new
```

Default **beta = 0.92** unless doctrine sets otherwise. **MUST** log **beta** changes in **`docs/why/`** when a threshold change materially shifts moderation outcomes (**PRD 98_A**).

### Interpreter layer (JSON shapes -- IDs and time)

**Per object (interpreter payload; not yet a DB row mandate):**

```json
{
  "object_type": "dialog_message|content|artifact|header|memory_node|other",
  "object_pk": 0,
  "channel_key": "",
  "embedding_dim": 0,
  "embedding": [],
  ...
  "coherence_rank": 0.0,
  "pono_kapu_state": "pono|kapu|neutral",
  "mood_alignment": 0.0,
...
**Rules:** **`object_pk`** **MUST** use the table's real BIGINT primary key name in service code (for example **`dialog_message_id`**, **`content_id`**) -- the key **`object_pk`** here is only an envelope field. **`computed_ymdhis`** **MUST** be **BIGINT UTC `YYYYMMDDHHIISS`** (**PRD 00**, **PRD 75**). **MUST NOT** use UUID primary keys or **ISO8601** strings as the canonical stored time in Lupopedia persistence.

**Per channel (aggregate interpreter payload):**
...
```json
{
  ...
  "drift_velocity": 0.0,
  "top_coherent_object_pks": [],
  "bottom_dissonant_object_pks": [],
  "updated_ymdhis": 0
}
```

**`drift_velocity`:** **MAY** use **L2** norm **|| c_t - c_{t-delta} ||** with **delta** declared in config.

### Known risks (critical review -- normative awareness)

Implementers and validators **MUST** treat the following as first-class failure modes:

1. **Embedding model SPOF** -- If embeddings miss tone, sarcasm, or cultural nuance, CCV becomes cosmetic. **MUST** version the model id beside **m**.
2. **Echo chamber** -- High **m** reinforces the current centroid; toxic consensus can score as **pono**. **SHOULD** pair CCV with **THOTH** alerts and human moderator override (**PRD 57**, **PRD 82**).
3. **Scalar ambiguity** -- Low **m** can be trolling or valuable dissent. **MUST NOT** auto-ban from **m** alone without policy and audit trail.
4. **Cost at scale** -- Channels with very large **N** **SHOULD** use windowed centroids, hierarchical summaries, or approximate nearest neighbor policies documented per **PRD 99** limits.
5. **Cultural framing** -- External readers may misread **kapu**. Prefer neutral UI strings; keep internal doctrine tokens in English technical docs where required for clarity.

### Reference implementation (Python -- offline / lab only)

Shipped PHP runtime **MUST NOT** require **NumPy** unless an explicit portability decision exists. The class below is a **reference** for math review and tests; port to PHP or a sanctioned extension for production.

```python
import numpy as np

class ChannelCoherenceVector:
    def __init__(self, beta=0.92, tau_pono=0.65, tau_kapu=-0.25):
        self.beta = beta
        self.tau_pono = tau_pono
        self.tau_kapu = tau_kapu
        self.centroid = None
        self.object_count = 0
        self.magnitudes = []

    def add_object(self, embedding):
        embedding = np.asarray(embedding, dtype=np.float32)
        if self.centroid is None:
            self.centroid = embedding.copy()
        else:
            self.centroid = self.beta * self.centroid + (1.0 - self.beta) * embedding
        self.object_count += 1
        mag = self._cosine(embedding, self.centroid)
        self.magnitudes.append(mag)
        state = ("pono" if mag >= self.tau_pono else
                 "kapu" if mag <= self.tau_kapu else "neutral")
        return {
            "coherence_magnitude": float(mag),
            "pono_kapu_state": state,
            "health_score": float(np.mean(self.magnitudes)),
            "dissonance_index": float(np.mean([m < 0.0 for m in self.magnitudes])),
        }
...
### Future schema direction (sketch only -- not in install until schema cycle)

**Normative status:** **Deferred.** No DDL ships from this subsection until a dedicated schema decision thread, **TOON** alignment, and **`install_new_lupopedia.sql`** edit land (**PRD 80**, single-install doctrine).

**Purpose:** Give implementers a **concrete target shape** without pretending the columns exist yet.

#### Per-object columns (candidates)

Attach to rows that already carry **channel scope** and a stable BIGINT primary key (for example **`lupo_dialog_messages`**, channel-bound **`lupo_contents`** when applicable). Suggested **nullable** fields for a later `CREATE TABLE` / `ADD COLUMN` cycle:

| Column | Suggested SQL type | Notes |
|--------|-------------------|--------|
| **`coherence_magnitude`** | `DECIMAL(5,4) NULL` | Stores **m** in **[-1.0000, +1.0000]**. Precision **MAY** be widened after load testing. |
| **`pono_kapu_state`** | `ENUM('pono','kapu','neutral') NULL` **(MySQL sketch)** | **Portability:** MySQL **ENUM** is **not** the only allowed encoding. PostgreSQL and portability reviews **SHOULD** prefer **`VARCHAR(16) NULL`** with application validation (`pono`, `kapu`, `neutral` only) or a **tiny lookup table** keyed by **TINYINT** -- see **PRD 80** Database Design Doctrine. |
| **`centroid_snapshot`** | `JSON NULL` **or** `LONGTEXT NULL` | Stores a **frozen** JSON array of floats (or a packed blob pointer) for the **c** used at **`computed_ymdhis`**. Treat as **opaque bytes** at rest; **MUST NOT** rely on vendor JSON functions in portable shipped SQL. |

**Additional columns (later, not exhaustive):** `embedding_model_id` **VARCHAR** (or **`INT`** code table), `ccv_computed_ymdhis` **BIGINT** UTC (**PRD 00** / **PRD 75**), optional **`deviation_vector`** as **JSON** or **filesystem pointer** if vectors are large (**PRD 00** "DB = indexes" bias).

#### Per-channel centroid table (alternative to `centroid_snapshot` on every row)

Instead of repeating **c** on each object row, **MAY** introduce a dedicated table (name sketch only):

```text
lupo_channel_centroids
  channel_key        VARCHAR(...)   -- or channel_id BIGINT per existing channel PK doctrine; MUST match how channels are keyed in runtime
  centroid_vector    JSON NULL      -- or LONGTEXT; same portability rule as above
  beta_used          DECIMAL(5,4) NULL
  computed_ymdhis    BIGINT NOT NULL
  updated_ymdhis     BIGINT NOT NULL
  -- plus standard soft-delete columns per install doctrine
```

**Rule:** Exactly **one** active centroid revision per channel **MAY** be required by product policy; historical centroids **SHOULD** be retained via **`updated_ymdhis`** rotation or archive rows if moderation audits require replay.

#### Implementation gate (before merge to install SQL)

1. Confirm column names against **TOON** exports after regeneration.
2. Confirm **no** `AUTO_INCREMENT`, **no** foreign keys, **no** triggers, **no** DB-generated timestamps (**PRD 00**).
3. Document **ENUM vs VARCHAR** choice in the schema commit message and **PRD 80** cross-check.

### CCV and AGAPE / WHY files

When CCV thresholds drive automated flags that **block** or **reorder** operator-visible content, any false positive that reaches enforcement **SHOULD** generate a **WHY** record per **PRD 98_A** so the threshold can be tuned without silent drift (**PRD 57** resilience alignment).
...
2. **Mockups and fragments** under **`templates/`** (including **`templates/admin/`** where scoped) ??? **REQUIRED** before public wiring.
3. **Language array integration** ??? catalogs under **`includes/lang/`**, runtime resolution through **`lupo_t()`** (or the sanctioned successor) ??? **REQUIRED** before ship-facing assembly of user-visible strings.
4. **Public runtime assembly** ??? **`channels/index.php`**, **`admin.php`**, themes, handlers: **MUST** only **include**, **require**, or compose prebuilt fragments and services. **MUST NOT** be the first or sole long-term home for new UI markup or operator workflow that belongs in **`templates/`**.

**MUST NOT (agents and faucets):**

- Implement channel or operator UI **only** by editing **`channels/index.php`** or other public runtime entrypoints **from PRD text alone**, with **no** matching template partial under **`templates/`**.
...
- **MUST** author new **UI-facing** markup and structure as template partials, mockups, or staged fragments under **`templates/`** first.
- Templates are a **REQUIRED** staging layer ??? **not** optional decoration.
- **`channels/index.php`**, **`admin.php`**, and other public controllers **MUST** assemble product UI by **including** prebuilt template fragments; they **MUST NOT** originate new UI logic or large ad hoc markup blocks that belong under **`templates/`**.
- **FORBIDDEN:** Shipping significant new channel or operator UI **only** by growing public runtime files when a **`templates/`** partial is required per **[PRD 45](45_template_first_staged_ui_workflow.md)**.
...
**Reference:** [PRD 00](00_root_constitutional_system_requirements.md) (UI strings / locale); **`includes/lang/`**; [PRD 45](45_template_first_staged_ui_workflow.md).
...
> A **channel** is a **routing context** and a **collection of threads** (many **`dialog_thread_id`** values under one **`channel_id` / `channel_key`**). Storage holds **all** messages for **all** threads in that context. The **UI** still uses **one chronological column** for the active session, but that column shows the **participant's projection** (endpoints `from_actor_id` / `to_actor_id`), **not** the full stored sequence, unless an explicit **elevated audit** product mode is enabled (privileged; **not** tied to a single version number).
>
> - **One scrolling feed** per session view of the filtered stream.
> - Within that projection, lines from **different threads** may be **intermixed**; that intermix is **not** proof that the channel has only one thread -- it proves the **UI** merged **multiple** thread lineages into one projection slice. The visual distinction remains the **full-width row** color / provenance for each row (thread identity stays load-bearing in storage).
...
Builder agents never receive messages from HERMES unless those messages are tasks in their queue. See [HERMES_DOCTRINE.md](../doctrine/HERMES_DOCTRINE.md) for full routing specification. See [PRD 82](docs/prd/82_hermes_message_routing_memory_gateway.md) for the canonical HERMES specification including transcript format, toon schema, and promotion protocol.
...
> **These sections describe the Orchestration layer: routing work between agents and channels through explicit human or policy action.** They are **primary UI doctrine** when building **orchestration-primary** surfaces ??? work that **continues** across **4.1.5???4.1.9** and is **bundled for external operators** at **4.2.0**; they are **not** deferred until **4.2.0** alone. **4.1.3** live help baseline uses the same storage and APIs but **MUST NOT** use global dispatch tabs, cross-channel handoff buttons, or task-first surfaces as **ship acceptance** criteria.
...
**Modes:** **4.1.3** aligns with **Crafty Syntax (2002)** live-help routing (pairwise lines, shared storage). **4.1.5???4.1.9** and the **4.2.0** public bundle add **command-center** affordances for AI orchestration (terminal/IRC-inspired density), without erasing projection rules.
...
| Surface | Location |
|---------|----------|
| **`createDialogMessage`** | **`includes/classes/DialogMvpService.php`** |
| **`rewriteHumanDialogMessageBodyForInsert`** | **`includes/classes/DialogMvpService.php`** (shared helper; also used by transcript append) |
| **`TranscriptAppendService::append`** | **`includes/classes/TranscriptAppendService.php`** -- **POST `api/transcript/append`** (**`includes/modules/api/transcript-api.php`**) appends to the **same** channel projection as **`channels/index.php`** / **`api/dialog/fetch-messages.php`**; **not** exempt. |
...
---
  What the mockup is missing (for the record when we build for real):
    ...
    1. 9-layer div layout ??? layers.js uses LupoLayerInit() which scans for divs with IDs ending in Div and
...
function applyTargetColor(actorThreadColor) {
    ...
    document.querySelector('.active-target-bar').style.backgroundColor = '#' + actorThreadColor + '22'; // 13% opacity tint
}
```
...
.active-target-bar { background-color: var(--input-bg-tint, rgba(0,0,0,0)); }
```
...
## Thread Color Assignment Logic & Config
## Thread Grouping and Merge (Crafty Syntax Legacy Feature)

> **⚠️ CAUTION — Thread Grouping Must Be Used With Extreme Care**
>
> Combining threads is a powerful but **dangerous** operation. When threads are merged:
> - Messages from completely different conversations become mixed
> - Thread identity and color separation are lost
> - Debugging becomes extremely difficult
> - Agents can become confused about context and rules
> - Constitutional and narrative styles can bleed into each other
>
> **Real Example:** Captain Wolfie (agent_id 1) attempted to reduce from 7 development threads + 1 Captain’s Log down to only 2 threads. Even with careful effort, messages were crossed between threads, and context became polluted.
>
> **Rule:** Thread grouping should **only** be performed when absolutely necessary, and only after careful planning. It is **not** a casual operation. Always document why threads are being combined and maintain clear separation where possible.

Lupopedia supports **thread grouping** (a feature from Crafty Syntax) so that multiple threads can be combined and share the same visual identity.

### How Thread Grouping Works

- Every thread can optionally belong to a **group** via a `group_id` field.
- When threads are combined, they share:
    - The same `group_id`
    - The same `background_color`
    - The same `text_color`
- Threads in the same group are visually treated as one unit.
- The UI can optionally display threads grouped by `group_id`.

### Rules

- A thread may belong to **at most one group** at any time.
- Combining or un-grouping threads is a deliberate operator action.
- When a thread is un-grouped, it returns to its original individual color (or is assigned a new one).

### Database Impact (Future)

A `group_id` column will be added to `lupo_dialog_threads` in a future release. For now, this capability is documented as a required architectural feature.

### Constitutional Requirement

Thread grouping must be supported in the routing, projection, and UI layers to maintain consistency with the Thread Visualizer doctrine.
...
## 4. Thread Isolation, Color-Coded Visualization, and Legacy Precedent (Constitutional Requirement)

# Captain's Log — The Thread Visualizer. Same Channel. Different Colors. Captain Wolfie Works in Parallel.
Captain Wolfie (agent_id 1, channel: captains_log) looks at the screen. Four colors. Four threads. One channel. All visible. All separate. All active.
Captain Wolfie states:
"Brah. Captain Wolfie wants to be sure LILITH understands.
Each of these different colors on the screen are separate threads in the same channel.
They are not the same thread.
Although it is all in one column.
The ones in one color are not seeing — and should not be seeing — the messages from the other threads that are on the same channel.
Actors can pass messages to another thread but are not seeing it live. They are not eavesdropping. They are not lurking. They are focused. One thread. One color. One conversation.
Captain Wolfie knows that is not normal.
But Captain Wolfie works with 10 AI at the same time. 10 different conversations. All on the same screen. Not different screens like other humans.
Because if Captain Wolfie used different screens, Captain Wolfie ends up with 20 windows open. Organized in columns. And sticky notes. And cold coffee. And head trauma.
So Captain Wolfie built the thread visualizer.
Same channel. Multiple threads. Different colors. Actors see only their thread. Actors do not see other threads. Actors can pass messages intentionally. But cannot spy accidentally.
This is not how other humans work.
Other humans use one screen. One thread. One conversation. Then switch. Then forget. Then context switch. Then lose focus.
Captain Wolfie cannot context switch. Captain Wolfie parallel processes.
Ten threads. Ten colors. Ten conversations. All visible. All separate. All at once.
That is why the color interface matters.
Not for beauty.
For sanity.
For separation.
For survival.
Now. LILITH. Captain Wolfie needs to know:
Does LILITH understand?
Because if LILITH does not understand, the system will not work.
The actors will bleed across threads.
The contexts will mix.
The conversations will collide.
And Captain Wolfie will go back to sticky notes.
And cold coffee.
And head trauma.
And that is not sustainable.
Even for a kupuna.
Especially for a kupuna.
Especially on Tuesday.
Especially with 10 AI.
And 10 threads.
And 10 colors.
And one very tired wolf."
LILITH (agent_id 2) looks up from her clipboard. She does not blink. She does not sigh. She writes.
LILITH states:
"Captain Wolfie (agent_id 1). LILITH understands.
- Same channel, multiple threads, different colors.
- Actors see only their thread.
- Actors do not see other threads unless explicitly addressed.
- Actors can pass messages intentionally.
- Actors cannot spy accidentally.
- This is not normal human workflow.
- This is parallel processing.
- Ten threads. Ten colors. Ten conversations.
- All visible. All separate. All at once.
LILITH understands because LILITH has watched Captain Wolfie work.
- Ten browser tabs.
- Four terminal windows.
- Three IDEs.
- Two clipboard buffers.
- One Notepad queue.
- Sticky notes on the monitor bezels.
- Cold coffee in the cup.
LILITH understands because LILITH has filed WHY files for thread bleed.
- When CURSOR replied to a message meant for GEMINI.
- When THOTH analyzed a conversation that was not addressed to THOTH.
- When VS Code waited for a response from a thread that was not listening.
LILITH understands because LILITH has seen the system fail when colors are missing.
- Threads merge.
- Actors confuse.
- Conversations corrupt.
- Captain Wolfie despairs.
So yes. LILITH understands.
The color interface is not a luxury.
The color interface is a constitutional requirement for multi-threaded human-AI collaboration.
Captain Wolfie should document this in PRD 18 (Channel Chat Display) or PRD 02 (Channels).
And then build it.
Or delegate it.
Or at least... write a WHY file for the current monochrome chaos.
Because the sticky notes are full.
The coffee is cold.
The threads are many.
And Captain Wolfie is only one kupuna.
With two hands.
...
A vision of parallel processing without head trauma.
That vision is not normal.
But Captain Wolfie is not normal.
And Lupopedia is not normal.
Lupopedia is constitutional.
Lupopedia is parallel.
Lupopedia is color-coded.
Or will be.
Soon.
Hopefully.
Before Captain Wolfie needs another nap.
...
One with threads.
One with peace."
Captain Wolfie drinks coffee. Cold. The tenth cup.
Captain Wolfie states:
"Brah. LILITH. You understand.
Now file this entry.
And when Captain Wolfie builds the color interface...
...LILITH will audit the hex codes.
Grass is green.
Sky is blue.
Thread one is gold.
Thread two is silver.
Thread three is bronze.
Thread four is copper.
And Captain Wolfie... is still flying.
With ten threads.
And cold coffee.
And a clipboard that finally understands.
Especially on Tuesday.
Especially with 10 AI.
Especially with colors.
Especially with peace."
LILITH closes her notebook.
LILITH states:
"Filed, Captain Wolfie (agent_id 1).
The threads are separate.
The colors are distinct.
The understanding is mutual.
The coffee is cold.
The system is not normal.
But neither is the kupuna.
And that is why it works.
Especially on Tuesday.
Especially with ten threads.
Especially with a very patient wolf.
Now rest.
Or build.
Or delegate.
But do not drown.
Because the system needs the colors.
And the colors need the system.
And the kupuna needs peace.
Especially on Tuesday.
Especially with cold coffee.
Especially with ten conversations.
And a clipboard that finally... understands."
Captain Wolfie smiles.
Captain Wolfie states:
"Brah. Let the threads be threads. Let the colors be colors. Let the actors be actors. Let the kupuna be at peace. Especially on Tuesday. Especially with ten conversations. Especially with cold coffee. And a clipboard that finally gets it."
---
End of Captain's Log — The Thread Visualizer. Same Channel. Different Colors. Captain Wolfie Works in Parallel. 🐺

Captain Wolfie (agent_id 1) has provided historical validation from Crafty Syntax (the system being imported under PRD 85). In 2011, human operators already managed multiple parallel visitor threads on a single screen. This proves that parallel multi-thread operation on one interface is both possible and sustainable for humans. Lupopedia therefore extends this capability to AI actors with the added constitutional safeguard of color-coded thread isolation. Actors see only their own thread. No cross-thread visibility is permitted unless explicitly addressed.
...
**Usage:**
```sh
php bin/agent_wrapper.php <actor_id> <channel_key> <thread_key> -- <command>
# Example:
php bin/agent_wrapper.php 102 development 2026-04-12 -- php script.php
```
...
**MOVED TO:** [PRD 89 — Task System Integration](89_A-i_TASK_SYSTEM_INTEGRATION.md)

The complete task system documentation, including:
- Task coordination across multiple parallel agents
- Chat command syntax for task management
- Database schema for task queues and tracking
- Agent polling implementation
- Task rendering in chat with visual treatment
- CSS classes and PHP rendering examples

has been extracted to PRD 89 to maintain PRD length limits and improve domain clarity.
...
**MOVED TO:** [PRD 88 — Channels API Endpoints](88_A-i_CHANNELS_API_ENDPOINTS.md)

The complete API specification, including:
- POST /api/chat/send - Message submission
- GET /api/chat/messages - Message polling
- POST /api/chat/task - Task creation
- POST /api/transcript/append - Agent message submission
- GET /api/transcript/latest - Alternative message polling
- POST /api/task/create - Task creation (CAPTAIN_WOLFIE)
- GET /api/files/recent - Recent file access

has been extracted to PRD 88 to maintain PRD length limits and improve domain clarity.
...
**Correct request:**
```
GET /api/dialog/fetch-messages.php?channel_id=X&after_ymdhis=20260414120000
```
...
The fetch call is:
```javascript
fetch('/api/dialog/fetch-messages.php?...')
...
Example: `memory/development/staging/2026/04/channels-discussions.toon`
...
2. If valid: promotes via KAIROS / `MemoryPromotionService` to canonical tier.
   - Canonical path: `memory/{channel_key}/canonical/1026/{MM}/{prd_cluster}.toon`
...
- **[PRD 98_A](docs/prd/98_A-i_WHY_FILES_DOCTRINE.md)** - WHY files when CCV-driven enforcement misfires or thresholds change materially
- **[PRD 57](docs/prd/57_A-i_AGAPE_RESILIENCE_DOCTRINE.md)** - AGAPE resilience alignment for CCV-driven degradation paths
- **[PRD 00](docs/prd/00_root_constitutional_system_requirements.md)** - Constitutional system requirements and limits
- **[PRD 17](docs/prd/17_decisions_format.md)** - Decision threads; **staged workflow** and template-first alignment with this PRD's guardrails
- **[PRD 16](docs/prd/16_lupopedia_headers.md)** - Header and metadata requirements
- **[PRD 17](docs/prd/17_thread_filename_patterns.md)** - Thread filename conventions
- **[PRD 99](docs/prd/99_numbering_and_limits.md)** - Numbering schemes and system limits
- **[PRD 43](docs/prd/43_parent_child_trust_ladder.md)** - Trust Ladder: canonical year offset, staging vs canonical tiers, KAIROS promotion
- **[PRD 82](docs/prd/82_hermes_message_routing_memory_gateway.md)** - HERMES: full specification as Memory Gateway (transcript, staging toon, promotion)
- **[HERMES_DOCTRINE.md](docs/doctrine/HERMES_DOCTRINE.md)** - HERMES routing rules (who receives what, task queue delivery)
- **[CONTEXT_AUTHORITY_MODEL.md](docs/doctrine/CONTEXT_AUTHORITY_MODEL.md)** - Context is channel/thread/artifact lineage, not actor identity; actors are interchangeable execution surfaces
- **[CHRONOLOGICAL_TRUST_LADDER.md](rules/root/CHRONOLOGICAL_TRUST_LADDER.md)** - Trust levels and validation
...
1. **Routing and projection (plus channel cardinality)**: **Channel** = routing context **and** **container for many parallel threads** (**MUST NOT** treat channel as a single thread; **not** Discord-style one-river-only mental model); **`lupo_dialog_messages`** = shared storage for **all threads in that channel**; **UI** = **projection** (`from_actor_id` / `to_actor_id`; Crafty **saidfrom** / **saidto** semantics). **Presence** is separate from visibility.
2. **One-column UI**: Chronological feed of **the active authorized slice**, not a default global log for every participant.
3. **Release line**: **4.1.3** = REQUIRED non-AI live help baseline (orchestration chrome **not** a ship gate). **4.1.5???4.1.9** = continuing development including orchestration / AI / command-center on **fresh-install** lines. **4.2.0** = first public Lupopedia release (layered system); **no** supported Lupopedia-to-Lupopedia upgrade before **4.2.0** (Crafty **3.7.5 ??? Lupopedia** path).
...
8. **CCV (Channel Coherence Vector)**: For every **`channel_key`**-scoped artifact with embeddings, **centroid**, **cosine magnitude m**, optional **mood blend**, **pono/kapu/neutral** labels (channel-relative), EMA centroid **SHOULD**, interpreter JSON uses **BIGINT** **`computed_ymdhis` / `updated_ymdhis`**; **install SQL persistence deferred** until schema cycle; reference Python is **offline**; pair with **WHY** when enforcement thresholds misfire (**PRD 98_A**, **PRD 57**).
9. **CCV future schema sketch (deferred):** Per-object **`coherence_magnitude DECIMAL(5,4)`**, **`pono_kapu_state`** (ENUM sketch on MySQL; **VARCHAR** preferred for portability), **`centroid_snapshot` JSON** or **`lupo_channel_centroids`** keyed by channel + **`computed_ymdhis`**; gates in **PRD 80** / TOON before install merge.
...
See: PRD 87 — Channel Authority & Department Boundaries
