...
lupopedia.headers:
  ...
  atoms_toon: null
  transcript_jsonl: 0/development/07-agents-faucets
  artifact_type: prd
...
1. **Developer-Friendly**: Human-readable directory names (`agents/wolfie/` vs `agents/1/`)
...
5. **Agent Key Support**: Natural support for human-readable agent identifiers
6. **Reserved numeric IDs vs. filesystem keys:** Numeric `agent_id` values **1????????2025** are reserved for core system agents (resolve from `registry.json` and seed). **Filesystem** discovery uses `agents/{agent_key}/` ???????? there are no empty numeric placeholder folders; an agent exists only when that directory exists. See `00_root_constitutional_system_requirements.md` ????5.5.
...
The `AgentDiscovery` PHP class (`includes/classes/AgentDiscovery.php`) provides dynamic agent discovery and management:
...
- **Search**: Search by name, role, or agent_key
...
// Search agents by name, role, or agent_key
...
### Optional soul-layer and runtime files (not required)

The following files MAY exist at `agents/{agent_key}/` root. None are mandatory. This PRD lists names only; meaning and constraints are in **PRD 6x** (Agent Soul Model). No runtime behavior is specified here.
...
agents/hephaestus/
agents/hermes/api/
agents/hermes/assets/
```

**See also:** The `_TEMPLATE/` agent directory for canonical file templates and folder structure.

## state.jsonl (optional runtime log)

When present under `agents/{agent_key}/`, `state.jsonl` is:

- append-only JSONL
- tracks tasks and interrupts
- references memory_cluster paths
- not part of agent identity
- optional per agent

## Agent memory path rule (canonical)

Agents MUST NOT store memory inside `agents/{agent_key}/` as the durable memory layer.

All memory MUST resolve through:
...
**DOCTRINE:** File-based agent definitions in agents/{agent_key}/ are authoritative. The lupo_agents table is runtime-only and reflects the filesystem. The database must never modify agent files.
...
| THOTH | thoth | 26 | knowledge_management, record_keeping, information_architecture |

> [CORRECTED 20260414] THOTH ID was 9 in this table. Corrected to 26 per PRD 16 timestamp hierarchy (PRD 16 when_updated: 20260412163625 > this file's prior when_updated: 20260411085204). Implementation and all doctrine files consistently use 26.
...
**Actor hub vs agent template (COUNTERMEASURE):** Agent configuration stays under **`agents/countermeasure/`** (`agent_key`). The **actor** hub on disk for **`actor_id` 111** is **`actors/111/`** (registry `dir`); see [PRD 00 ????5.6](00_root_constitutional_system_requirements.md#56-actor-id-semantics) and [PRD 15](15_actors.md).
...
| Survivability (pack `agape`) | agape | 705 | agentic_guidance, environment_awareness, fallback_ladders, doctrine_gap_surfacing |
...
- All governance agent definitions have been moved to their correct namespaces (74_governance_rules.md, 71_truth_knowledge.md).
...
AgentDiscovery::searchAgents($query)        // Search by name, role, or agent_key
...
**Definition**: An actor is a runtime instance that combines three components:

1. **auth_user_id** - The human operator (e.g., Wolfie = 10000)
2. **agent_id** - The agent blueprint being used (e.g., LILITH = 2)
3. **llm_provider** - The LLM service for the session (e.g., DeepSeek)

**Parallel Instances**:
- A single user may spawn multiple actor instances simultaneously
- Each instance represents an independent execution context
- Multiple IDE sessions can create distinct actors with the same agent

**Lifecycle Behavior**:
- Actors are ephemeral runtime instances
- Actors expire after approximately 24 hours of inactivity
- Expired actors are no longer valid execution contexts
- New actors are created for each session start

**Identity Rule**:
- Actors are NOT aliases for agents
- Actors are NOT persistent identities
- Actors are time-bound execution instances only
- Actor identity is fixed for the session duration

**Example**:
When Wolfie (auth_user_id 10000) uses LILITH (agent_id 2) via DeepSeek, this creates ONE actor instance with a unique actor_id that expires after 24 hours of inactivity.

### Operational States (Human-Controlled)

**Active**
* Actor/tool is usable and producing value
* Normal operational state for all functioning agents

**ICU**
* Actor/tool is failing or unstable
* Under observation or potential recovery
* Not yet suspended, but monitored closely

**Graveyard**
* Temporary suspension (30-90 days)
* Used for:
  - Token bleed (excessive consumption)
  - Repeated non-learning failures
  - Execution instability
* Actors in Graveyard:
  - Are not executed
  - Do not consume tokens
  - Retain all prior context
* Reactivation is manual

**Retired**
* Permanent removal from use
* No further execution expected
* Final state for deprecated or failed agents

**Control Rules**
* All states are human-controlled
* No automatic transitions between states
* No timers enforced by system
* No orchestration logic
* No database schema requirements

**Clarification**
* Graveyard ??? deletion
* Graveyard = pause usage
* History and context remain intact
* These are operational labels, not system-enforced states

### Collections as Actor Context

* Actors may receive collections as contextual input
* Collections provide grouped content context, not instructions
* Actors interpret the content based on existing PRD and channel context
* Collections do NOT define routing or execution behavior
* Collections do not alter actor identity, role, or execution rules
...
2. **File Modification**: Direct editing of agent files in `agents/{agent_key}/`
...
- **Agent Key Support**: Natural human-readable agent identifiers
...
**IMPORTANT**: Agent definitions are now filesystem-based in `agents/{agent_key}/` directories, not database-driven. The `AgentDiscovery` class provides the primary interface for agent management.
...
1. **Identity & Metadata** (`agent.json`):
   - Agent ID, name, role, layer, and agent_key
...
### ROSE Capability ??? Cultural Mirroring

ROSE supports cultural mirroring as a core capability.

Definition:

* adaptive response behavior based on user communication style

Behavior:

* detects tone, dialect, and structure
* adjusts output style accordingly
...
Cultural mirroring is a presentation-layer behavior only.
...
1. **Agent Definition**: File-based in `agents/{agent_key}/` (immutable source of truth)
...
**Rules**:
- **File ???????? DB is authoritative**: Agent files in `agents/` define the agent
...
| **Storage** | `agents/{agent_key}/` | Hub: **`actors/{actor_id}/`** (registry `dir`; see [PRD 15](15_actors.md)) ???????? dated subpaths are optional product layout, not a second identity |
...
#### Actor Workspace
```
actors/{actor_id}/   # canonical hub per registry / PRD 15
+-- agent_link.json # References source agent
+-- # memory: root memory node at memory/YYYY/MM/{memory_slug}.json (4.0.96+; memory.json DEPRECATED)
...
**Note:** Additional dated or nested folders under **`actors/`** are **organizational** only; they must not imply a different **`actor_id`** or private per-user ownership of the actor row.
...
- Agent definitions live in `agents/{agent_key}/` (immutable source of truth)
...
- `bin/temporal_anchor.json` (structured temporal data)
...
- File versions in `agents/{agent_key}/versions/`
...
4. **Memory & Context** (system_prompt.txt + runtime):
   ...
   - Runtime memory stored in `lupo_memory_nodes` / `lupo_memory_edges` (PRD 38; KAIROS uses `memory_type` keys per PRD 37)
   - Context snapshots in `lupo_agent_context_snapshots`

### Agent ???????? Actor Relationship

1. **Agent Definition**: File-based in `agents/` (immutable source of truth)
...
### File Location
`includes/classes/AgentDiscovery.php`
...
- Agent key distribution statistics
...
+-- # memory template: see memory/YYYY/MM/{memory_slug}.json (4.0.96+; memory.json DEPRECATED)
...
## ANUBIS: Filesystem Content Intake Role

ANUBIS (actor_id: 19) is the sole agent authorized to process filesystem-path content into the database. This role is distinct from ANUBIS's general custody and integrity functions.

### Trigger

ANUBIS activates when a file appears on disk without a Lupopedia header. This is the normal and expected output of any filesystem agent (Claude Code, Crafty, external tools). It is not an error condition.

### Responsibilities (Path 2 intake)

1. **Detect** orphan files via filesystem watcher or scheduled scan
2. **Analyze** file content to infer correct parent (channel, thread, collection)
3. **Generate** complete Lupopedia header (`lupopedia.headers` block, all 22+ fields)
4. **Write** header to the file in place
5. **Create** `lupo_contents` database row
6. **Build** memory graph edges (`lupo_memory_edges`)
7. **Escalate** to PRD 19 (Garbage Collection) if parent cannot be resolved

### Authority Boundary

- Only ANUBIS may write headers on behalf of filesystem-path files (PRD 32)
- No other agent may create `lupo_contents` rows for unheadered files
- ANUBIS does not process files that already have a valid header

### Full Doctrine

See **PRD 16 ??15 (Two Paths to Content)** for the authoritative specification of both intake paths, the comparison table, and agent behavior rules.
