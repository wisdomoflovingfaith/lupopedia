...
lupopedia.headers:
  ...
  file_path_from_root: "docs/implementations/33_softaculous_certification_4_1_0_gate/SOFTACULOUS_PACKAGE_BUILD.md"
  web_path: "http://www.lupopedia.com/lupopedia/docs/implementations/33_softaculous_certification_4_1_0_gate/SOFTACULOUS_PACKAGE_BUILD.md"
  questions_toon: null
...
---
# file: Softaculous package build — web_path: [http://www.lupopedia.com/lupopedia/docs/implementations/33_softaculous_certification_4_1_0_gate/SOFTACULOUS_PACKAGE_BUILD.md](http://www.lupopedia.com/lupopedia/docs/implementations/33_softaculous_certification_4_1_0_gate/SOFTACULOUS_PACKAGE_BUILD.md)
...
- **`database/.htaccess`** — deny direct web access to SQL and seeds.
...
Ship **`lupopedia-config-sample.php`** in the package root. It mirrors production config shape: **`define()`** for database and salts (WordPress-classic style), then **`require_once`** **`includes/bootstrap.php`**.
...
- `includes/`, `app/`, `ui/`, `emoji/`, `config/`, `install/` (contains **`InstallWizardHtaccessWriter.php`**), `database/lupopedia/mysql/install/`, `database/lupopedia/mysql/import/`
- **No** `.htaccess` anywhere (FTP-safe)

**After** install config succeeds on **Apache** with a writable docroot: **`.htaccess`** and **`database/.htaccess`** appear.
...
| `scripts/` | Dev scripts (the **builder** lives here; run it **before** packaging — it is **not** inside the zip). |
| `craftysyntax-reference/` | Reference tree — not runtime. |
| `archive/` | Entire archive dir excluded (includes **`legacy/wordpress-reference/`** study tree; GPL) — not runtime; keeps zip small. |
| `legacy/` | Legacy reference — not runtime (adjust if a release explicitly ships legacy). |
| `research/` | Research sandboxes. |
| `tests/` | Tests. |
| `database/lupopedia/toon/`, `database/lupopedia/json/` | Generated schema mirrors — **install SQL is authoritative**. |
| `tools/`, `node_modules/` | Tooling / npm caches. |

### Critical correction (runtime)

**`database/lupopedia/content/`** (especially **`app/`**) **MUST ship** — `LUPO_APP_DIR` points here (`lupopedia-config.php`, installer). **Do not** exclude this tree in the Softaculous package.
...
- `logs/`
- `uploads/`
- `tmp/` (aligned with **`start_over.php`**)

**Bootstrap self-heal:** **`includes/bootstrap.php`** also **`@mkdir(..., 0755, true)`** those four directories under **`LUPOPEDIA_ABSPATH`** when missing, so **auto-installer** installs that never run the wizard still get writable folders on first request.
...
**Path:** `scripts/build_softaculous_package.sh`

**Requirements:** `rsync`, `find`, `zip`, `tar` (e.g. **Git Bash** on Windows, or Linux/macOS).

```bash
# From repo root (after chmod +x on Unix):
./scripts/build_softaculous_package.sh 4.1.0

# Custom output directory:
DIST_DIR=/path/to/out ./scripts/build_softaculous_package.sh 4.1.0
```
...
- [ ] After install (Apache): `find . -name '.htaccess' -type f` → docroot + `database/.htaccess`  
...
| `docs/`, `scripts/`, `tests/`, `research/`, `archive/` | No | Non-runtime |
| `database/lupopedia/content/app/` | **Yes** | **`LUPO_APP_DIR`** runtime |
| `install/InstallWizardHtaccessWriter.php` | **Yes** | Post-install `.htaccess` + wizard dirs |
