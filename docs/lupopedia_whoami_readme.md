...
lupopedia.edges:
  outbound_edges:
    ...
    - { to: "database/lupopedia/actors/registry.json", type: "references", weight: 0.9 }
    - { to: "docs/database/lupopedia/tables/sessions.md", type: "references", weight: 0.9 }
    - { to: "database/session.md", type: "references", weight: 0.8 }
    - { to: "includes/classes/DialogHeaderValidator.php", type: "references", weight: 0.8 }
...
- **Registry:** `database/lupopedia/actors/registry.json` is the source of truth, keyed by `actor_name`.
...
- **`php bin/lupo.php whoami`** — Human-readable context (actor, channel, federation node, workspace, session, system version, context source).
- **`php bin/lupo.php whoami --verbose`** — Same context as JSON.
- **`php bin/lupo.php context`** — Alias for `whoami --verbose` (JSON only).
...
Context is resolved in this order (see **ContextResolver::resolve()** in `includes/classes/ContextResolver.php`):
...
2. **session.md (fallback)** — If the database is unavailable, the CLI reads `database/session.md`. Expected frontmatter or key-value lines: `actor_name`, `channel_id`, `federation_node_id`, `session_id` (optional: `actor_id`). This allows CLI use when the database is offline.
3. **System defaults** — If neither exists: `actor_name: system`, `actor_id: 0`, `channel_id: 0`, `federation_node_id: 0`.

Workspace is always computed as `/actors/{actor_name}/` (or the project’s `LUPO_ACTORS_DIR` + actor_name).
...
```bash
ln -s /actors/cursor /actors/1003
```
...
- **Human-readable context:** `php bin/lupo.php whoami`  
...
- **JSON context:** `php bin/lupo.php context` or `php bin/lupo.php whoami --verbose`  
  Outputs the same data as a single flat JSON object (no nested objects). Suitable for scripting and tooling.

### Help

- **General help:** `php bin/lupo.php help` — Lists all commands and includes Basic Usage for whoami and context.
- **whoami help:** `php bin/lupo.php help whoami` — Explains the three identity layers (Effective Actor, Human Identity, Active Agent), session mode rules, and example outputs (hybrid, human_direct, autonomous_agent, system).
- **context help:** `php bin/lupo.php help context` — Describes the flat JSON format, ContextResolver resolution order (session.md first → DB enrichment → registry → defaults), and a sample JSON payload.
...
**Session file first (v4.0.61+):** The resolver reads `database/session.md` as the first-class source when the file exists and has usable keys (e.g. actor_name, channel_id, session_id). It then enriches from the database (lupo_sessions) and actor registry. **Context source** is reported as: `session.md`, `session.md + registry`, `lupo_sessions`, or `default`. When the database is unavailable, context is resolved from session.md and registry/defaults; the CLI does not exit on DB failure for whoami/context.
