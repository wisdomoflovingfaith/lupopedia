...
# FLARE Header (aliases: Wolfie, FLIP, FLP, FLPH, CROP) — see http://www.lupopedia.com/status/FLARE_HEADER_REFINEMENT_4.0.57
---
flare.headers:
  flare.version: "1.0"
  flare.schema: "report"
...
- **After:** `# FLARE Header (aliases: Wolfie, FLIP, FLP, FLPH, CROP) — see http://www.lupopedia.com/<web_path>`
...
- **grep (new format):** Four docs/status files now use the new pattern `# FLARE Header (aliases: Wolfie, FLIP, FLP, FLPH, CROP) — see http`.
...
- **Lilith (actor 2):** Requested for meta-review of this refinement report and of flame-aligned header usage (Safety Rule compliance for headers). See `flare.footer.delegation` above.
