...
- **Common fields:** `flare.headers` with `flare.version`, `flare.schema`, `file_path_from_root`, `file_hash`, `system_version`, `channel_id`, `actor_id`, `delegation_chain`, `artifact_type`, `purpose`, `mood_rgb`, `traits`, `tags`, `lupo_agent`; optional `flare.edges` (outbound_edges), `flare.footer` (last_verified, last_verified_by).
