...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/actor_registration_checklist.md
  web_path: https://www.lupopedia.com/lupopedia/docs/actor_registration_checklist.md
  status: ''
...
This checklist is the **canonical** process for registering a new IDE agent or web terminal agent as a Lupopedia actor. It is derived from the actual data model (TOON files, install SQL, seed files) and from the repository's **actor registry** and **database** fallback behavior. Do not participate as an unnamed or unregistered agent.

**Source of truth for schema:** `database/lupopedia/mysql/install/install_new_lupopedia.sql` (table `lupo_actors`) and TOON files in `database/lupopedia/toon/`. **Actor registry (lupo_actors):** `database/lupopedia/actors/registry.json`. **lupo_agents id map:** `database/lupopedia/actors/actor_id/registry.json` (`agents` object). For resolution and propagation context, see [docs/doctrine/AGENT_REGISTRY.md](doctrine/AGENT_REGISTRY.md).
...
with the matching `slug` (and `database/lupopedia/actors/actor_id/registry.json` only for **lupo_agents** numeric `agent_id` by slug).
...
| **C — Agent exists but not fully integrated** | No new actor. Complete: rules propagation target (see [Extending rules propagation](#extending-rules-propagation)), validation test (e.g. `tests/unit/<agent>_rules_enforcement.php`), and any agent-specific config or docs. |
...
1. **Review canonical root rules.**  
   Read [rules/root/README.md](../rules/root/README.md) and the root rule files in `rules/root/`. All agents must follow these rules. Pay special attention to:
   - [ide-agent-identity-actor-pairing-doctrine.md](../rules/root/ide-agent-identity-actor-pairing-doctrine.md) (ACT001) — agent identity, paired orchestrator, no anonymous operation.
   - [reserved-id-doctrine.md](../rules/root/reserved-id-doctrine.md) (DB006) — `actor_id` is **not** AUTO_INCREMENT; you must supply an explicit ID and follow the reserved-ID rules.
   - [database-offline-fallback-import-doctrine.md](../rules/root/database-offline-fallback-import-doctrine.md) (DB008) — when the live DB is unavailable, persistence can use `database` (e.g. registry file and CSV) with rehydration-safe structure.
...
4. **Declare default project context.**  
   New actors must operate within a project. When persisting the actor (registry and/or `lupo_actors`), declare **default_project_id** and **default_channel_id** (e.g. in metadata or config) so the agent immediately operates in a known project and channel. IDE agents infer project from workspace when not explicitly set; external actors must supply project_id and channel_id in every request. See [docs/projects/PROJECTS.md](projects/PROJECTS.md) and [docs/projects/PROJECTS_API.md](projects/PROJECTS_API.md).
...
The table `lupo_actors` (see `database/lupopedia/toon/lupo_actors.toon.json` and install SQL) has:
...
Seed examples: `database/lupopedia/mysql/seed/seed_actors_agents_4.0.45.sql`.
...
- **Path:** `database/lupopedia/actors/registry.json`
...
- **lupo_agents numeric ids (optional):** If you add a matching `agents/<slug>/` pack, add `"<slug>": <agent_id>` to the `agents` map in `database/lupopedia/actors/actor_id/registry.json` using an unused **agent_id**.
...
```bash
php scripts/propagate_agent_rules.php --target=<your-agent>
```
...
1. **Registry:** Your `actor_id` and slug appear in `database/lupopedia/actors/registry.json` and do not duplicate an existing entry (and `lupo_agents` map updated in `actor_id/registry.json` if you added a pack).
...
4. **Root rules:** You have read and will follow the root rules in `rules/root/`.
...
|------|------|--------|------|
| 1 | Add registry entry | `database/lupopedia/actors/registry.json` (+ optional `agents` map in `actor_id/registry.json`) | Always (required) |
| 2 | Persist in DB | `lupo_actors` via seed or documented INSERT | When DB available (canonical) |
| 3 | Fallback | Same registry; optionally `database/lupopedia/csv/lupo_actors.csv` | When DB unavailable |
...
- **Script:** `scripts/propagate_agent_rules.php`. Add the target to `$validTargets`, define the output directory (e.g. `.cascade/`), implement a `write_<target>_outputs($dir, $rules)` function patterned on `write_cascade_outputs` or `write_windsurf_outputs`, and invoke it when `$target === 'all' || $target === '<target>'`.
...
- **Validation:** Add `tests/unit/<target>_rules_enforcement.php` following `cascade_rules_enforcement.php` to verify artifacts exist, JSON is valid, rules match canonical root, and headers are present.
...
- [rules/root/](../rules/root/) — Canonical root rules (all agents must follow).
- [ide-agent-identity-actor-pairing-doctrine.md](../rules/root/ide-agent-identity-actor-pairing-doctrine.md) (ACT001).
- [reserved-id-doctrine.md](../rules/root/reserved-id-doctrine.md) (DB006).
- [database-offline-fallback-import-doctrine.md](../rules/root/database-offline-fallback-import-doctrine.md) (DB008).
...
- `database/lupopedia/toon/lupo_actors.toon.json` — Column/type reference (derived from install SQL).
- `database/lupopedia/mysql/seed/seed_actors_agents_4.0.45.sql` — Example actor/seed pattern.
...
| DB008 | database-offline-fallback-import-doctrine.md | Offline fallback persistence in database |
