...
lupopedia.headers:
  ...
  artifact_type: "guide"
  artifact_kind: "documentation"
  purpose: "Documentation for emotional-agent-range.md"
  mood_vector: "4169E1"
  traits: ["flare", "indexed", "v4.0.50"]
  tags: ["docs", "channels", "doctrine", "emotional-agent-rangemd"]
  lupo_agent: "windsurf"
...
- ✅ **Emotional agents must not generate `mood_vector` (reserved for DIALOG).**
...
- ❌ Generate `mood_vector` (DIALOG's exclusive domain)
...
1. **DIALOG processes message** → Creates summary with `mood_vector` and `weight`
...
- **DIALOG:** Assigns `mood_vector` (emotional color vector)
...
- **Counting-in-Light Doctrine:** `docs/appendix/COUNTING_IN_LIGHT.md` - Emotional coordinate system (X, Y, Z axes)
- **DIALOG Agent Guide:** `agents/3/COUNTING_IN_LIGHT.md` - DIALOG mood assignment guide
- **HUMOR Agent Guide:** `agents/14/REFLECTIVE_HUMOR_GEOMETRY.md` - HUMOR humor vector assignment guide
