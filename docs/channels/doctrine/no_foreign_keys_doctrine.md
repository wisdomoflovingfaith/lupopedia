...
- **PHP Class:** `includes/classes/anubis.php`
- **Database Tables:**
...
When ANUBIS performs a hard delete, it must follow the **registry_open** doctrine so deleted IDs can be safely reused by the allocator (findpuka). ANUBIS must NOT add an ID to `{prefix}registry_open` if the row has an active redirect in anubis_redirects or is an unresolved orphan; only fully resolved, redirect-free IDs may be returned to the free list. ANUBIS must never modify REGISTRY (the reserved-ID ledger); it interacts only with registry_open. See [ANIBUS_DOCTRINE.md](ANIBUS_DOCTRINE.md) section 15 for full rules and prefix doctrine.
