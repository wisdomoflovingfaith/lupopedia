...
dialog:
  speaker: CURSOR
  target: @everyone
  message: "Created HERMES Routing Doctrine with channel-aware and mood-aware routing. Documents canonical routing behavior using 2-actor mood vector model and compare-notes protocol."
  mood: "00FF88"
tags:
...
**Format**: `(X, Y, Z)` with each axis ∈ {-1, 0, 1}
...
$moodZ = $moodService->getLatestMood($actorB);
...
**Mood values**: X, Y, Z ∈ {-1, 0, 1} (discrete only)

### 6.3 Emotional Geometry

**MANDATORY**: Always use the 2-actor mood vector model
...
class HERMES {
    ...
    public function route(int $channelId, int $currentActorId): array {
        ...
        foreach ($coherenceMap as $actorA => $pairs) {
            foreach ($pairs as $actorZ => $coherence) {
...
- `doctrine/EMOTIONAL_GEOMETRY_DOCTRINE.md` - 2-Actor mood vector model
...
✅ **Mood-Aware**: Uses 2-actor mood vector model  
