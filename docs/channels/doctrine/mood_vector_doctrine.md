...
lupopedia.headers:
  ...
  file_path_from_root: "docs/channels/doctrine/MOOD_VECTOR_DOCTRINE.md"
  file_hash: "37fdb28090f5305f1eabf39b4547180d6ee1ddbf3884f8af78aa6a033864ff7f"
...
lupopedia.headers:
  ...
  artifact_type: "guide"
  artifact_kind: "documentation"
  purpose: "Documentation for MOOD_VECTOR_DOCTRINE.md"
  mood_vector: "4169E1"
  traits: ["flare", "indexed", "v4.0.50"]
  tags: ["docs", "channels", "doctrine", "mood_vector_doctrinemd"]
  lupo_agent: "windsurf"
...
dialog:
  speaker: CURSOR
  target: @everyone
  message: "Updated MOOD_VECTOR Doctrine: Added comprehensive Counting-in-Light emotional geometry system documentation, three axes explanation, mood calculation doctrine, and usage across DIALOG, HERMES, CADUCEUS, and operator UI."
  mood: "00FF00"
tags:
...
in_this_file_we_have:
  - Important Clarification: mood_vector Terminology
  - Doctrine Boundaries
...
**Legacy Naming Note:** The term "mood_vector" is a historical convention and does **NOT** refer to mood vector channels or visual colors. It represents an emotional polarity tensor with three abstract axes: strife (X: +1 to -1), harmony (Y: +1 to -1), and memory depth (Z: +1 to -1).
...
This section documents the Countingâ€‘inâ€‘Light emotional polarity tensor system used throughout Lupopedia. The system encodes emotional tone as a 6â€‘character hex value (mood_vector) representing three abstract emotional axes: strife, harmony, and memory depth.

Countingâ€‘inâ€‘Light is used by:
- DIALOG (expressive rendering),
- dialog_messages.mood_vector (storage),
...
- **X** â€” strife, conflict, tension  
- **Y** â€” harmony, cooperation, warmth  
- **Z** â€” memory depth, reflection, introspection  
...
**NOTE: "mood_vector" is an emotional polarity tensor (strife/harmony/memory axes), encoded as hex. NOT actual mood vectors.**

The emotional polarity tensor is stored in:

dialog_messages.mood_vector  
VARCHAR(7) NULL  
Format: XXYYZZ (no leading #)
...
- Pattern: `^[0-9A-Fa-f]{6}$`
- Storage: `dialog_messages.mood_vector CHAR(6) DEFAULT '666666'`
- Naming: Always `mood_vector` (lowercase, underscore) in DB, YAML, and code.

**Critical:** X, Y, and Z are abstract emotional axes, not literal color meanings. User interfaces **MUST** treat `mood_vector` as coordinates, not as culturally fixed color semantics. The hex encoding is coincidental and carries no color semantics.
...
CADUCEUS uses mood_vector to compute routing currents:
...
- High X may increase assertive/critical currents.
- High Y may increase expressive/cooperative currents.
- High Z may increase reflective/deepâ€‘reasoning currents.
...
- DialogManager **MUST** validate `mood_vector` against the hex pattern.
- If `mood_vector` is null, empty, or invalid:
...
## Agent Behavior
- Agents **MUST** only write valid hex `mood_vector` values.
- Agents **SHOULD** use neutral (`'666666'`) when unsure.
- Agents **MUST NOT** systematically misuse `mood_vector` (e.g., always max red) for routing manipulation.
- Repeated misuse **MAY** be flagged by THEMIS or Agent 0 as a governance violation.

## Routing Behavior
- CADUCEUS computes currents from `mood_vector` as defined in [COUNTING_IN_LIGHT.md](../appendix/appendix/COUNTING_IN_LIGHT.md).
- If the sum of currents is zero (e.g., `'000000'`), HERMES **MUST** fall back to default routing.
- Routing **MUST** remain deterministic and safe even with invalid or missing `mood_vector`.
...
- [INLINE_DIALOG_SPECIFICATION.md](../dialogs/agents/INLINE_DIALOG_SPECIFICATION.md) â€” Inline dialog format with mood_vector field
