...
lupopedia.headers:
  ...
  artifact_type: "guide"
  artifact_kind: "documentation"
  purpose: "Documentation for THREAD_MOOD_RFC.md"
  mood_vector: "4169E1"
  traits: ["flare", "indexed", "v4.0.50"]
  tags: ["docs", "channels", "architecture", "protocols", "thread_mood_rfcmd"]
  lupo_agent: "windsurf"
...
## **Status of This Memo**
This document defines the **Thread Mood Aggregation Standard** for Lupopedia 3.0.1. It specifies how individual dialog message moods (`mood_vector`) are aggregated into a single **threadâ€‘level mood**, enabling emotional continuity, analytics, and routing context across multiâ€‘agent conversations.
...
## **1. Introduction**
Lupopedia's dialog system stores emotional metadata for each message using the **Countingâ€‘inâ€‘Light** coordinate system (`mood_vector`). While messageâ€‘level mood captures the emotional geometry of individual interactions, many system behaviors require a **threadâ€‘level mood**, including:
...
- **Message Mood** â€” A 6â€‘digit hex RGB value stored in `dialog_messages.mood_vector`.  
- **Neutral Mood** â€” `666666`, representing balanced midâ€‘range values.  
- **Zero Mood** â€” `000000`, representing a void/noâ€‘signal state.  
- **Aggregation** â€” The process of averaging axis components across messages.  
...
Thread mood is computed by averaging the axis components of all valid message moods.
...
4. Convert each hex mood to decimal axis components.  
5. Sum X, Y, and Z across all messages.  
...
```php
$moods = $db->query(
    "SELECT mood_vector FROM dialog_messages WHERE conversation_id = ?"
)->fetchAll();
