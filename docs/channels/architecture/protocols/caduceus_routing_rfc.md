...
- **Countingâ€‘inâ€‘Light** â€” The emotional coordinate system using hex-byte vector values (R=Strife, G=Harmony, B=Memory).  
- **Mood Vector** â€” A sixâ€‘digit hex string (XXYYZZ) encoding emotional metadata.  
...
- **X** â€” Strife / intensity / chaos  
- **Y** â€” Harmony / balance / cohesion  
- **Z** â€” Memory depth / introspection / persistence  

### **5.2 Current Computation Philosophy**

CADUCEUS binds axis components to emotional currents as follows:

- **Left Current (Analytical)** â€” `(G + B) / (G + B + R + B)`  
  - Harmony + Memory Depth â†’ Stabilizing  
  - High Y = stabilizing influence  
  - High Z = introspective depth  
  - Together: analytical, structured, reflective reasoning  

- **Right Current (Creative)** â€” `(R + B) / (G + B + R + B)`  
  - Strife + Memory Depth â†’ Scrutiny  
  - High X = intensity / conflict pressure  
  - High Z = depth of meaning  
  - Together: creative, emotional, intensityâ€‘driven reasoning  

**Critical Insight:** Memory (B) amplifies both currents. High Z means depth matters regardless of analytical vs creative bias. This ensures that deeply meaningful messages receive appropriate routing weight.
...
1. **Extract `mood_vector`** from input (default: `'666666'` if missing).  
2. **Sanitize:** `strtoupper(trim($moodVector))`.  
...
class Caduceus
{
    ...
    public static function computeCurrents(string $moodVector): array
    {
        ...
        $r = hexdec(substr($moodVector, 0, 2));
        $g = hexdec(substr($moodVector, 2, 2));
        $b = hexdec(substr($moodVector, 4, 2));
...
- Node A: `mood_vector = 'FF0000'` â†’ `left = 0.0, right = 1.0`  
- Node Z: `mood_vector = 'FF0000'` â†’ `left = 0.0, right = 1.0`  
