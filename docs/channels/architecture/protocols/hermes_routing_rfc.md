...
lupopedia.headers:
  ...
  artifact_type: "guide"
  artifact_kind: "documentation"
  purpose: "Documentation for HERMES_ROUTING_RFC.md"
  mood_vector: "4169E1"
  traits: ["flare", "indexed", "v4.0.50"]
  tags: ["docs", "channels", "architecture", "protocols", "hermes_routing_rfcmd"]
  lupo_agent: "windsurf"
...
- **Moodâ€‘Aware Routing** â€” Routing based on CADUCEUS currents derived from `mood_vector`.  
...
[
    ...
    'mood_vector'            => string,   // 6-digit hex vector (e.g., 'FF0000')
...
- `mood_vector` â€” If missing, defaults to `'666666'` (neutral).  
...
4. If routing_mode == "hermes" â†’ Continue with full routing pipeline:
   a. Extract routing metadata (to_actor, mood_vector, directive_dialog_id)
   ...
   e. Compute CADUCEUS currents from mood_vector
...
1. **Extract `mood_vector`** from packet (default: `'666666'`).  
...
CADUCEUS is a **pure function** with no side effects. It converts `mood_vector` (XXYYZZ) â†’ routing currents (`left`, `right`).
...
class HERMES
{
    ...
    public function route(array $packet)
    {
        ...
        $moodVector  = $packet['mood_vector'] ?? '666666';
