...
**File:** `includes/classes/hermes.php`

**Purpose:**
- Central message router for Lupopedia
- Reads `to_actor`, `mood_vector`, `directive_dialog_id` from message packets
...
**File:** `includes/classes/caduceus.php`
...
**Doctrine:**
- X = strife / intensity / chaos
- Y = harmony / balance / cohesion
- Z = memory depth / introspection / persistence
...
**File:** `includes/classes/iris.php`
...
**File:** `includes/classes/dialog-manager.php`
...
**File:** `includes/classes/thoth.php`
...
**File:** `includes/classes/wolfmind.php`
...
- `mood_vector` — Emotional coordinate (XXYYZZ, char(6))
...
- `classification_json` — **REQUIRED** JSON field containing agent classification and routing identity metadata
  ...
  - **MUST** mirror `agents/[AGENT_NAME]/classification.json` file
...
2. **Mood-Based Routing:**
   ...
   - Extract `mood_vector` from message packet
   - Call `Caduceus::computeCurrents($mood_vector)`
...
### **mood_vector Interpretation:**

- **X** = strife / intensity / chaos / conflict
- **Y** = harmony / balance / cohesion / stability
- **Z** = memory depth / introspection / persistence / reflection

### **Currents Formula:**

```php
// Extract axis components from mood_vector (XXYYZZ)
$r = hexdec(substr($mood_vector, 0, 2)); // strife
$g = hexdec(substr($mood_vector, 2, 2)); // harmony
$b = hexdec(substr($mood_vector, 4, 2)); // memory depth
...
1. **Insert Incoming Message**
   ...
   - Store `actor_id`, `content`, `mood_vector`, `thread_id`
...
**File:** `includes/classes/timestamp_ymdhis.php`
...
**CARMEN depends on:**
- **`agape`** pack / Survivability lens agent (required; see `SURVIVABILITY_DOCTRINE.md`)
- ERIS (required)
- METIS (required)
...
**File:** `includes/classes/rose.php`
...
**File:** `includes/classes/chronos.php`
...
### **File Locations:**
- All classes must remain in `includes/`
...
- Cursor must enforce CARMEN dependencies (`agape` Survivability lens, ERIS, METIS) in PHP
