...
**Identity:**
- **Class Name:** `HERMES` (PHP class in `includes/classes/hermes.php`)
...
**Identity:**
- **Class Name:** `Caduceus` (PHP class in `includes/classes/caduceus.php`)
...
- Converts Counting-in-Light hex-byte vector values (`mood_vector`) into normalized emotional currents
...
- **Mood Signals:** Emotional context from `mood_vector` (via CADUCEUS)
...
- `route($messagePacket)` â€” Main routing entry point
  ...
  - Extracts `to_actor`, `mood_vector`, `directive_dialog_id`
...
- **Input:** Message packet array:
  ...
  [
      'actor_id'            => int,
      'to_actor'            => int|null,
      'mood_vector'            => string,  // 6-digit hex (XXYYZZ)
...
CADUCEUS converts emotional metadata (`mood_vector`) from the Counting-in-Light system into deterministic emotional currents (`left`, `right`) that HERMES may optionally use as context for routing decisions.

**Core Functionality:**

1. **Input Validation:** Sanitize and validate `mood_vector` hex string
2. **RGB Extraction:** Parse X, Y, Z components (0-255)
...
- `computeCurrents(string $moodVector): array` â€” Main computation function
...
- **Input:** `mood_vector` (string, 6-digit hex, e.g., `"88FF88"`)
...
$r = hexdec(substr($moodVector, 0, 2)); // Strife (0-255)
$g = hexdec(substr($moodVector, 2, 2)); // Harmony (0-255)
$b = hexdec(substr($moodVector, 4, 2)); // Memory (0-255)
...
- **X:** Strife / intensity / chaos â†’ influences `right_current`
- **Y:** Harmony / balance / cohesion â†’ influences `left_current`
- **Z:** Memory depth / introspection / persistence â†’ amplifies both currents
...
- **Memory Amplification:** High Z (memory depth) amplifies both currents, ensuring deeply meaningful messages receive appropriate routing weight
...
**Files:**
- **Implementation:** `includes/classes/hermes.php`
...
2. **CADUCEUS** â€” Called via `Caduceus::computeCurrents($moodVector)`
...
**Files:**
- **Implementation:** `includes/classes/caduceus.php`
...
1. **HERMES** â€” Called exclusively via `Caduceus::computeCurrents($moodVector)`
...
Messages created before v3.0.0 **MAY** lack `mood_vector`. HERMES **MUST** treat missing moods as:
...
1. **During HERMES Routing:**
   ```
   HERMES::route($packet)
     â†’ Extract mood_vector from packet
     â†’ Caduceus::computeCurrents($moodVector) â†’ ['left' => float, 'right' => float]
...
**Problem It Solves:**
CADUCEUS computes channel emotional current by reading and blending the moods of polar agents within a channel. Counting-in-Light provides emotional metadata as hex-byte vector values (0-255), which CADUCEUS converts into normalized emotional currents (0.0-1.0) that represent the channel's blended emotional state. HERMES may optionally use these currents as context when routing messages.

**Why Not Direct RGB Comparison:**
- hex-byte vector values are not normalized (0-255 range)
...
- **Memory Amplification:** High Z (memory depth) amplifies both currents, ensuring deep messages get appropriate routing weight
...
Message Packet
    |
    +- Read routing_mode from channels.metadata_json
    |
    +- routing_mode != "hermes"? --YES--â†’ Bypass routing logic â†’ Direct/Operator/Broadcast mode
    |
    +- routing_mode == "hermes"? --YES--â†’ Full routing pipeline:
                |
                +- Read Channel Intent â†’ Filter Candidate Agents
                |
                +- Filter by Agent Classification â†’ Filter by agent_class
                |
                +- to_actor set? --YES--â†’ Validate â†’ Direct Route
                |                          |
                |                          +-FAIL--â†’ Channel Intent + Classification Filter â†’ Mood-Aware Routing
                |
                +- NO --â†’ Channel Intent + Classification Filter â†’ CADUCEUS::computeCurrents(mood_vector)
                            |
                            +- left > right --â†’ Analytical Pool (filtered) â†’ Agent Selection
                            |                       |
                            |                       +- NO AGENTS --â†’ Opposite Pool â†’ System Agent 0
                            |
                            +- right > left --â†’ Creative Pool (filtered) â†’ Agent Selection
                            |                       |
                            |                       +- NO AGENTS --â†’ Opposite Pool â†’ System Agent 0
                            |
                            +- left == right --â†’ Tiebreaker â†’ Default Pool â†’ System Agent 0 (if empty)
```
...
1. **Sanitize and Validate Input:**
   ```php
   $moodVector = strtoupper(trim($moodVector)); // "  ff0000 " â†’ "FF0000"
   if (!preg_match('/^[0-9A-F]{6}$/', $moodVector)) {
       $moodVector = '666666'; // Invalid â†’ neutral fallback
   }
   
   // Handle void mood
   if ($moodVector === '000000') {
       $moodVector = '666666'; // Void â†’ neutral fallback
   }
   ```
...
$packet = [
    'actor_id' => 1,
    'to_actor' => null,
    'mood_vector' => '00FF00',  // High harmony, low strife
...
1. **Equal Currents (Tiebreaker):**
   - Input: `mood_vector = "0000FF"` (pure memory depth)
...
2. **Invalid Hex (Neutral Fallback):**
   - Input: `mood_vector = "FFGG00"` (invalid hex)
...
3. **Void Mood (Neutral Fallback):**
   - Input: `mood_vector = "000000"` (void state)
   - Expected: CADUCEUS substitutes `"666666"` â†’ `left = 0.5, right = 0.5`
...
6. **Federation Mock (Binding Consistency):**
   - Scenario: Same `mood_vector = "00FF00"` routed on two nodes with different left/right bindings
...
- **[MOOD_VECTOR_DOCTRINE.md](../doctrine/MOOD_VECTOR_DOCTRINE.md)** â€” MOOD_VECTOR Doctrine
