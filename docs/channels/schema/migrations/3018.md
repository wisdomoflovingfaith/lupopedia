...
lupopedia.headers:
  ...
  file_path_from_root: "docs/channels/schema/migrations/3.0.18.md"
  file_hash: "1f3bce29c495c95df2eb7e045b1496c46113253f33a7a034585ef7f5da330754"
...
lupopedia.headers:
  ...
  lupopedia.edges: []
  file_path_from_root: "docs\channels\schema\migrations\3.0.18.md"
  file_hash: "f5ecec585b5ec7bba774c64fe356666695bc2ed1444bf2418ce0df06da48979b"
  file_path_from_root: "docs\channels\schema\migrations\3.0.18.md"
  file_hash: "cdf5c9ad21a63535527f4a7efc98caaf286d36b7239b24f1f0dafe7976463477"
  ...
  artifact_type: "guide"
  artifact_kind: "documentation"
  purpose: "Documentation for 3.0.18.md"
  mood_vector: "4169E1"
  traits: ["flare", "indexed", "v4.0.50"]
  tags: ["docs", "channels", "schema", "migrations", "3018md"]
  lupo_agent: "windsurf"
...
dialog:
  ...
  target: @everyone
  mood_vector: "00FF00"
  message: "Created migration guide for version 3.0.18 - UI Layout Template Integration. Updated main layout to match template structure, moved Collections dropdown to topbar, fixed tab counting and routing. No schema changes or breaking changes."
tags:
  categories: ["documentation", "migration", "3.0.18"]
  collections: ["core-docs"]
  channels: ["dev", "release"]
file:
  title: "LUPOPEDIA 3.0.18 — Migration Guide"
  description: "Migration notes and upgrade instructions for version 3.0.18. Includes UI layout template integration, Collections dropdown component, and routing improvements."
  version: GLOBAL_CURRENT_LUPOPEDIA_VERSION
  status: published
  author: GLOBAL_CURRENT_AUTHORS
---

# LUPOPEDIA 3.0.18 — Migration Guide

**Release Date:** 2026-01-15  
**Previous Version:** 3.0.17  
...
1. **Collections Dropdown Component** (`includes/ui/components/collections_dropdown.php`)
...
2. **Content Controller Collection Tab Handler** (`content_handle_collection_tab()`)
   - New function in `includes/modules/content/content-controller.php`
...
1. **Main Layout Template** (`includes/ui/layouts/main_layout.php`)
...
2. **Topbar Navigation** (`includes/ui/components/topbar.php`)
...
3. **Collection Tabs Loader** (`includes/functions/collection-tabs-loader.php`)
...
4. **Module Router** (`includes/modules/module-loader.php`)
...
1. Backup your UI layout files:
   ...
   - `includes/ui/components/topbar.php`
   - `includes/ui/components/collections_dropdown.php` (if exists)

2. Backup your routing files:
   - `includes/modules/module-loader.php`
   - `includes/modules/content/content-controller.php`
...
1. **New Component** - Add Collections dropdown component:
   - `includes/ui/components/collections_dropdown.php` (new file)

2. **Updated Components** - Update existing files:
   - `includes/ui/layouts/main_layout.php` (major update)
   - `includes/ui/components/topbar.php` (removed Collections link, added dropdown)
   - `includes/modules/module-loader.php` (routing priority update)
   - `includes/modules/content/content-controller.php` (new function)
   - `includes/functions/collection-tabs-loader.php` (child tabs loading)
...
- **Location**: `includes/ui/components/collections_dropdown.php`
- **Features**: 
...
- **Location**: `includes/ui/layouts/main_layout.php`
- **Features**:
...
- **Location**: `includes/modules/content/content-controller.php`
...
- **Location**: `includes/functions/collection-tabs-loader.php`
...
- ✅ **Fully backward compatible** with 3.0.17
...
If you need to rollback to 3.0.17:

1. Restore UI components from backup:
   - `includes/ui/layouts/main_layout.php`
   - `includes/ui/components/topbar.php`
   - Remove `includes/ui/components/collections_dropdown.php` (if added)
