...
lupopedia.headers:
  ...
  file_hash: "cb6d50e52106a7d64b685a4f1d6ca03957f096231801e5ee8c4c8dc74590c306"
...
lupopedia.headers:
  ...
  artifact_type: "guide"
  artifact_kind: "documentation"
  purpose: "Documentation for ACTOR_MOOD_SERVICE.md"
  mood_vector: "4169E1"
  traits: ["flare", "indexed", "v4.0.50"]
  tags: ["docs", "channels", "kernel", "services", "actor_mood_servicemd"]
  lupo_agent: "windsurf"
...
dialog:
  speaker: CURSOR
  target: @everyone
  message: "Created ActorMoodService documentation. Service provides actor mood logging and retrieval using 2-actor mood vector model with discrete values (-1, 0, 1). Includes API reference, usage examples, and coherence calculation guidance."
  mood: "00FF88"
tags:
...
file:
  ...
  description: "Service layer for actor mood logging and retrieval using 2-actor mood vector model"
...
The **ActorMoodService** provides a service layer for logging and retrieving actor moods using the canonical 2-actor mood vector model. It validates discrete mood values and manages persistence in the `actor_moods` table.

### Key Features

âœ… **Discrete Mood Values**: X, Y, Z âˆˆ {-1, 0, 1} (doctrine-compliant)  
...
Logs an actor mood with discrete hex-byte vector values.
...
$moodService->logMood(
    actorId: 2,
    moodX: 0,   // Neutral strife
    moodY: 1,   // High harmony
    moodZ: 1    // Deep memory/reflection
);

// Log mood with specific timestamp
$moodService->logMood(
    actorId: 1,
    moodX: -1,  // Calm (negative strife)
    moodY: 0,   // Neutral harmony
    moodZ: 0,   // Neutral memory
    timestampUtc: 20260120150000
);
```

**RGB Axis Semantics**:
- **X**: Strife/Conflict/Intensity
  - `-1` = Calm, low conflict
  - `0` = Neutral
  - `1` = High strife, tension
  
- **Y**: Harmony/Cohesion/Warmth
  - `-1` = Low harmony, tension
  - `0` = Neutral
  - `1` = High harmony, supportive
  
- **Z**: Memory/Persistence/Reflection
...
```php
// After DIALOG generates mood_vector, log discrete mood
$moodService->logMood(
    actorId: $dialog_actor_id,
    moodX: $discrete_r,  // Convert from RGB hex
    moodY: $discrete_g,
    moodZ: $discrete_b
);
```
...
- [MOOD_VECTOR_DOCTRINE.md](../../doctrine/MOOD_VECTOR_DOCTRINE.md) - Mood Color System
...
âœ… **Discrete Mood Values**: X, Y, Z âˆˆ {-1, 0, 1}  
