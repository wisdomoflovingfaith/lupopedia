---
lupopedia.headers:
  file_path_from_root: "docs/database/lupopedia/tables/deprecated/audit_log.md"
...
- { to: "docs/database/lupopedia/tables/lupo_actors.md", type: "references", weight: 0.8, reason: "Actor identity attribution" }
    - { to: "database/lupopedia/toon/lupo_audit_log.toon.json", type: "schema_reference", weight: 1.0 }
...
**Schema:** See `database/lupopedia/toon/lupo_audit_log.toon.json`. Column names (e.g. entity_type, entity_id, event_type, payload_json, created_ymdhis) must match the TOON.
...
**Migration:** `docs/doctrine/migrations/livehelp_operator_history_migration.md`, `import_from_old_crafty_syntax.sql`. Legacy opid â†’ entity_id, entity_type = 'actor', event_type and payload from legacy action/session. livehelp_operator_history â†’ IMPORTED â†’ DROPPED.
