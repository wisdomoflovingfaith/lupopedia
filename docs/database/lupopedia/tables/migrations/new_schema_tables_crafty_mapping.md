...
The Lupopedia agent template system (agents/ filesystem) has no Crafty equivalent. Agent definitions are seeded from the `agents/` directory during fresh install, not from Crafty data.
...
Seeded from `agents/{slug}/capabilities.json` files during install.
...
Seeded from `agents/{slug}/tools.json` files during install.
...
Seeded from `agents/{slug}/boundaries.json` files during install.
