...
lupopedia.headers:
  ...
  file_path_from_root: docs/database/lupopedia/tables/active/lupo_session_events.md
  namespace: auth
  channel_id: 1
  actor_id: 1003
...
lupopedia.edges:
  ...
  outbound_edges:
  - to: database/lupopedia/toon/lupo_session_events.toon.json
...
|--------|------|----------|---------|-------------|
| session_event_id | bigint | No | â€” | Primary key. |
| session_id | varchar(255) | No | â€” | Session reference (logical â†’ lupo_sessions.session_id). |
| actor_id | bigint | Yes | â€” | Actor at event time. |
| tab_id | varchar(255) | Yes | â€” | Tab identifier. |
| world_id | bigint | Yes | â€” | World context id. |
| world_key | varchar(255) | Yes | â€” | World key. |
| world_type | varchar(50) | Yes | â€” | World type. |
| event_type | varchar(100) | No | â€” | Event type code. |
| event_data | json | Yes | â€” | Optional event payload. |
| created_ymdhis | bigint | No | 0 | Event timestamp. |

## Relationships

- **Logical references:** session_id â†’ lupo_sessions.session_id; actor_id â†’ lupo_actors.actor_id.
