...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/versions/4.1.4/AGAPE_TEACH_AI_CHANNEL_INTERFACE_PRD_UPDATES_WITH_PRD_CLUSTER.md
  web_path: https://www.lupopedia.com/lupopedia/docs/versions/4.1.4/AGAPE_TEACH_AI_CHANNEL_INTERFACE_PRD_UPDATES_WITH_PRD_CLUSTER.md
  status: active
...
# AGAPE Teach AI Channel Interface PRD Updates (4.1.5)
...
- template location: agents/agape/ (confirmed on disk, all canonical files present:
...
- agent (agents/agape/) + context (incident) -> new runtime actor
...
- Execution path: agents/agape/ (template) -> runtime actor_id (per WHY file)
  -> joins thread -> teaches -> completes

NOTE: agents/agape/ contains a memory.json file. Per PRD 15_A s.8, memory.json in
...
- When PHP code inserts a new file path into recent files that matches docs/why/ pattern,
  this triggers AGAPE
...
### NQ8 -- Deprecated memory.json in agents/agape/

Confirmed on disk: agents/agape/memory.json exists. Per PRD 15_A s.8, memory.json
in agent/actor directories is deprecated as of 4.0.96+. Canonical memory now lives at
memory/YYYY/MM/{memory_slug}.json linked via lupo_edges.

- Should agents/agape/memory.json be removed or migrated before AGAPE is activated
  as a runtime actor?
