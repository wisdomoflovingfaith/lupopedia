...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/versions/4.1.2/CHANGELOG.md
...
**WHO:** AUGGIE; WOLF  
**WHAT:** `validate_atoms_toon()` added with suffix, canonical year (1026), existence, and collision checks. `CANONICAL_YEAX = "1026"` constant. Path guard against absolute paths and URLs. PRD 02 header upgraded to v4.1.0 22-field format; body modernized (`memory_key` → `memory_toon`, `dialog_transcript` → `transcript_jsonl`). Task System section merged.  
...
- Migration guide updated: `pk_*` and `dialog_transcript` removed in **4.1.3** (not 4.1.5)
