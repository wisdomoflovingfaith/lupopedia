...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/versions/4.1.2/status/open_questions.md
...
Option Z: Routing is inferred from the message chain — if a task message includes a `[task]` command with source metadata in the body, provenance is derived by parsing the message log.
...
Option Z: Sticky notes are chat messages with `message_type='note'`. They appear in the feed but are visually distinguished. No separate table.
