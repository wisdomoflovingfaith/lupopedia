...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/versions/4.0.99/status/agent_definition_audit.md
  web_path: https://www.lupopedia.com/lupopedia/docs/versions/4.0.99/status/agent_definition_audit.md
  status: active
  ...
  thread_key: agent-definition-audit-4.0.99
...
The audit task specified `docs/prd/06_content_management.md` (PRD 06) as the agent definition
...
**Seed authority:** `database/lupopedia/mysql/seed/seed_4.1.0.sql` is the install-time
...
| `agents/thoth/system_prompt.txt` | 26 | CONFIRMED |
...
| Agent directory exists | PASS | `agents/thoth/` |
...
| `agents/thoth/9/` subdir | PASS | Version subdirectory exists (agent version folder) |

**Capabilities gap:** `agents/thoth/capabilities.json` lists knowledge management and archival
...
**Recommendation:** Add these 4 capability entries to `agents/thoth/capabilities.json` to
...
| Agent directory exists | PASS | `agents/hermes/` |
...
**Numeric directory:** `agents/15/` does NOT exist. `major_agents_manifest.json` references
numeric directories as `agent_dirs`; these do not exist on disk. The real config is at
`agents/hermes/` (slug-based). See Section 5.
...
| Agent directory exists | PASS | `agents/rose/` |
...
**Minor capabilities gap:** `agents/rose/capabilities.json` is missing:
...
| PRD 07 entry | PASS | Coordination Layer |
| Agent directory exists | PASS | `agents/lilith/` |
...
| Agent directory exists | PASS | `agents/vishwakarma/` |
...
**Remaining gap:** `agents/vishwakarma/system_prompt.txt` still says actor_id=106. This
...
**File:** `database/lupopedia/actors/registry.json`
...
**File:** `database/lupopedia/actors/major_agents_manifest.json`

**Stale references:**
- `agent_dirs` entries reference numeric paths (`agents/26/`, `agents/91/`, `agents/15/`) — NONE EXIST
- Actual config is in slug-based paths (`agents/thoth/`, `agents/vishwakarma/`, `agents/hermes/`)
...
|---|---|---|
| thoth | `agents/26/` | `agents/thoth/` |
| vishwakarma | `agents/91/` | `agents/vishwakarma/` |
| hermes | `agents/15/` | `agents/hermes/` |

`agents/thoth/9/` does exist as a subdirectory (version folder for agent_id 9) — this is
...
- `docs/prd/06_content_management.md` — confirmed: zero agent content
- `docs/prd/08_core_agents_system.md` — canonical agent registry (when_updated: 20260414173500)
- `docs/prd/07_agents_faucets.md` — coordination layer table
- `agents/thoth/agent.json`, `system_prompt.txt`, `capabilities.json`
- `agents/hermes/agent.json`, `system_prompt.txt`, `capabilities.json`
- `agents/rose/agent.json`, `system_prompt.txt`, `capabilities.json`
- `agents/lilith/agent.json`, `system_prompt.txt`, `capabilities.json`
- `agents/vishwakarma/agent.json`, `system_prompt.txt`, `capabilities.json`
