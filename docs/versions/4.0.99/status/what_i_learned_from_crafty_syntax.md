...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/versions/4.0.99/status/what_i_learned_from_crafty_syntax.md
  web_path: https://www.lupopedia.com/lupopedia/docs/versions/4.0.99/status/what_i_learned_from_crafty_syntax.md
  status: active
...
`database/lupopedia/mysql/import/import_from_old_crafty_syntax.sql` (1916 lines)

Doctrine documents read:
- `docs/doctrine/migrations/crafty_syntax_ancestral_intent.md`
- `docs/doctrine/migrations/livehelp_migrations_readme.md`
- `docs/doctrine/migrations/generated/README.md`
...
**Lupopedia application**: `api/dialog/post-message.php` is already a focused
endpoint. The pattern suggests adding a corresponding `api/dialog/fetch-messages.php`
...
`docs/database/lupopedia/tables/migrations/` (see `livehelp_migrations_readme.md`).
...
**Lupopedia lessons**:
- Single-purpose dispatch endpoint: adopt `whattodo`-style routing for `api/dialog/`
...
## 8. DHTML / Layer Model — DynAPI Ancestry and layers.js
...
### 8.2 layers.js as conceptual continuation

`layers.js` is the **direct architectural heir** of this DynAPI lineage. Its
...
// Modern (layers.js) — use bracket notation + standard DOM:
...
|---|---|---|
| Delta-fetch polling (HTMLtimeof) | GET `/api/dialog/fetch-messages.php?channel_id=X&since_ymdhis=Y` returning `[{dialog_message_id, from_actor_id, message_text, message_type, created_ymdhis}]` | HIGH — current live feed requires page reload on every message |
...
## Appendix Z: The livehelp_messages volatility doctrine
...
implications for `layers.js`. The new section explains:
...
3. The `layers.js` / DHTML ancestry has a specific API contract to preserve.
