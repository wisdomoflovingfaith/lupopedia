---
lupopedia.headers:
  header_format_version: "4.1.8"
  path_from_lupopedia_root: docs/versions/status/thread_index.md
  web_path: https://www.lupopedia.com/lupopedia/docs/versions/status/thread_index.md
  status: ''
  when_updated: '20260513033046'
  trust_tier: null
  questions_toon: null
  memory_toon: null
  atoms_toon: null
  transcript_jsonl: null
  artifact_type: documentation
  artifact_kind: thread_index
  channel_key: null
  federation_node_id: 0
  thread_key: ''
  lupopedia.schema: documentation
  prd_cluster: null
  title: ''
  summary: ''
---
# file: THREAD_INDEX — docs/versions/status

| Artifact | Summary |
|----------|---------|
| [STATUS_INSTALL_WIZARD_AND_FRESH_INSTALL_20260408043450.md](STATUS_INSTALL_WIZARD_AND_FRESH_INSTALL_20260408043450.md) | Installer changes (SHOW/`tableExists`, no `information_schema`, PDO opts, prefix SQL, chmod, PRD 00); **fresh install safe** after drop tables + remove `lupopedia-config.php` + `install.php` |
