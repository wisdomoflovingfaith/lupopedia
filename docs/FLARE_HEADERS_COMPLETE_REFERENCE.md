...
**Purpose:** Comprehensive documentation of all available FLARE header and footer fields, including table-specific attributes and TOON references.  
**Version:** 4.0.47  
**Also Known As:** FLARE Complete Reference, Header/Footer Field Guide  
...
|-------|------|-------------|---------|
| `file_path_from_root` | string | Exact path from project root | `"docs/guide/example.md"` |
| `system_version` | string | Current Lupopedia version | `"4.0.47"` |
| `channel_id` | integer | Channel number (0=doctrine, 1=main, 42=development) | `1` |
| `actor_id` | integer | Author's actor ID (from registry) | `1001` |
| `last_modified_utc` | string | Last update date (YYYYMMDD) | `"20260226"` |
| `delegation_chain` | string | Actor delegation chain | `"1001:10000"` |
| `artifact_type` | string | Type of artifact | `"guide"` |

### **📋 Standard Optional Fields**

| Field | Type | Description | Valid Values | Example |
|-------|------|-------------|--------------|---------|
| `purpose` | string | Brief description of file purpose | Any text | `"Implementation guide for X"` |
| `mood_rgb` | string | Color mood indicator | Hex color code | `"4B0082"` |
...
When documenting database tables, include these additional fields that reference TOON files:

| Field | Type | Description | Format | Example |
|-------|------|-------------|--------|---------|
| `lupo_<table_name>.<column>` | mixed | Table-specific metadata from TOON | Matches TOON schema | See examples below |
| `table_primary_key` | string | Primary key column name | Column name | `"dialog_message_id"` |
| `table_engine` | string | Database engine | `"InnoDB"`, `"MyISAM"` | `"InnoDB"` |
| `table_charset` | string | Character set | `"utf8mb4"`, `"utf8"` | `"utf8mb4"` |
| `table_collation` | string | Collation | `"utf8mb4_unicode_ci"` | `"utf8mb4_unicode_ci"` |
| `table_indexes` | array | Index definitions | From TOON | `["PRIMARY", "idx_thread_id"]` |
| `table_foreign_keys` | array | Foreign key references | From TOON | `["actor_id", "channel_id"]` |
...
flare.headers:
  # ... required fields ...
  artifact_kind: "table"
  lupo_dialog_messages.dialog_message_id: "BIGINT primary key containing YYYYMMDDHHMMSS UTC timestamp"
  ...
  lupo_dialog_messages.from_actor_id: "BIGINT references lupo_actors.actor_id"
  lupo_dialog_messages.to_actor_id: "BIGINT references lupo_actors.actor_id"
  lupo_dialog_messages.message_text: "TEXT message content"
  lupo_dialog_messages.created_ymdhis: "BIGINT YYYYMMDDHHIISS UTC timestamp"
  table_primary_key: "dialog_message_id"
  table_indexes: ["PRIMARY", "idx_thread_id", "idx_channel_id", "idx_from_actor"]
  table_foreign_keys: ["dialog_thread_id", "channel_id", "from_actor_id", "to_actor_id"]
```
