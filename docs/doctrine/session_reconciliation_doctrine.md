...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/doctrine/session_reconciliation_doctrine.md
  web_path: https://www.lupopedia.com/lupopedia/docs/doctrine/session_reconciliation_doctrine.md
  status: ''
...
**Session state is DB-backed (Model A).** Browser stores only `session_id`; identity and CSRF come from `lupo_sessions`. No signed payloads, no JWT for web sessions. See `docs/doctrine/SESSION_MODEL.md`.

This doctrine defines how **session truth** is determined when both the database (`lupo_sessions`) and file-based session artifacts (`database/sessions/*.md`) exist, and how reconciliation and corrections are handled.
...
| **`database/sessions/*.md`** (files) | Portable, human-editable session context for IDE agents and tooling. | Offline/fallback, agent bootstrap, version-controlled context. |
...
- **When the database is not available:** Session **files** under `database/sessions/` MAY be used for fallback (see architecture doc: fallback to MD/CSV). No “winner” is defined for conflict between two session files; tooling should report conflicts, not auto-resolve.
...
For deterministic continuity, IDE session files under `database/sessions/` SHOULD include (in the `lupopedia.session` block or equivalent):
...
- `docs/doctrine/LUPOPEDIA_HEADERS/LUPOPEDIA_HEADERS_FORMAT.md` — session block format.
- `database/sessions/README.md` — session file layout and naming.
