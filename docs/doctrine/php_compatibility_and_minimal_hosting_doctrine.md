...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/doctrine/php_compatibility_and_minimal_hosting_doctrine.md
  web_path: https://www.lupopedia.com/lupopedia/docs/doctrine/php_compatibility_and_minimal_hosting_doctrine.md
  status: ''
...
lupopedia.edges: []
  ...
  artifact_type: "guide"
  artifact_kind: "documentation"
  purpose: "Documentation for PHP_COMPATIBILITY_AND_MINIMAL_HOSTING_DOCTRINE.md"
  mood_vector: "4169E1"
  traits: ["flare", "indexed", "v4.0.50"]
  tags: ["docs", "doctrine", "php_compatibility_and_minimal_hosting_doctrinemd"]
  lupo_agent: "windsurf"
...
- **PDO only.** All database access must use PDO. No mysqli, no mysql_*, no other drivers. **Installer exception (constitutional carve-out):** **`docs/doctrine/DATABASE_DOCTRINE.md`** — **Runtime database access (PDO_DB) and installer exception** — **`install.php`** / wizard **may** use **`mysqli`** only for the narrow listed installer purposes; **runtime** uses **`PDO_DB`** only and **named placeholders**.
