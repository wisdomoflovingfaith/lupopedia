...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/doctrine/migration_doctrine.md
  web_path: https://www.lupopedia.com/lupopedia/docs/doctrine/migration_doctrine.md
  status: ''
...
2. **A new migration SQL file** — (From 4.1.0 onward.) A one-time patch for development, stored in `database/lupopedia/mysql/migrations/`. Used to apply the same change to an existing (live) database. **Before 4.1.0:** the migrations directory is not used (see `database/lupopedia/mysql/migrations/README.md`).

**Pre-4.1.0 (v4.0.74 consolidation):** There is no Lupopedia → Lupopedia upgrade path. Migration replay is not used. All schema for a fresh 4.0.x install is in install_new_lupopedia.sql and future_features_lupopedia.sql only. Do not add new migration files for 4.0.x; update the canonical install (or future_features) SQL only. See `database/lupopedia/mysql/migrations/README.md`.
...
| Canonical schema | `database/lupopedia/mysql/install/install_new_lupopedia.sql` must match TOONs. |
