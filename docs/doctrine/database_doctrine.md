...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/doctrine/database_doctrine.md
  web_path: https://www.lupopedia.com/lupopedia/docs/doctrine/database_doctrine.md
  status: ''
...
6. The installer **MUST NOT** introduce **`mysqli`** usage into **runtime** code (bootstrap, **`includes/`**, **`app/`**, themes, APIs, or any code path that serves normal requests after install).
...
**Cross-reference:** Timestamp *value* rules for stored clocks are unchanged for runtime code; see **`docs/doctrine/TIMESTAMP_DOCTRINE.md`** §4.5.
...
Agents and code must **not**: use AUTO_INCREMENT for these tables, use `lastInsertId()` for them, or rely on casual numeric allocation. The installer, importer, and seed logic must preserve reserved IDs exactly. See `rules/root/reserved-id-doctrine.md` and `docs/doctrine/REGISTRY_DOCTRINE.md`.
