...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/doctrine/COUNTING_IN_LIGHT_DOCTRINE.md
  web_path: https://www.lupopedia.com/lupopedia/docs/doctrine/COUNTING_IN_LIGHT_DOCTRINE.md
  status: active
  when_updated: '20260513033046'
  trust_tier: seed
  questions_toon: null
  memory_toon: memory/constitutional/seed/counting-in-light-doctrine.toon
  atoms_toon: null
  ...
  thread_key: counting-in-light-doctrine
...
---
# file: Counting in Light Doctrine -- delegation: cursor:root -- web_path: https://www.lupopedia.com/lupopedia/docs/doctrine/COUNTING_IN_LIGHT_DOCTRINE.md
...
|-------|------------------|-----------|
| **CARMEN** | **706** | **`agents/carmen/`** |
| **ROSE** | **3** | **`agents/rose/`** |

**All other agents** (**AGAPE**, **LILITH**, **ARA**, **IDE facets**, coordination personas, and any agent not listed above) **MUST** use the **neutral token** **`666666`** on **own** envelopes whenever a six-hex **`mood_vector`** field is required. **Canonical emission table:** **`docs/doctrine/AGAPE_DEFECT_TAXONOMY.md`** (constitutional section).
...
The token is **exactly six** hexadecimal digits **`[0-9A-F]{6}`** (uppercase preferred), **no** `#` prefix. The canonical notation is **`XXYYZZ`**, not `RRGGBB`, because the token is not RGB color data. Each axis is stored as an exact two-hex-digit byte rather than a float so agents, databases, logs, and replay tools preserve identical values across runtimes.

Bytes map **left to right** as printed:

| Byte pair (1-indexed) | Axis | Range | Meaning |
|------------------------|------|-------|---------|
| **XX** / bytes 1-2 (chars 1-2) | **Frequency** | `00`-`FF` | How often the pattern occurs (`00` = rare, `FF` = saturation) |
| **YY** / bytes 3-4 (chars 3-4) | **Severity** | `00`-`FF` | Impact of the pattern (`00` = trivial, `FF` = critical) |
| **ZZ** / bytes 5-6 (chars 5-6) | **Urgency** | `00`-`FF` | How soon action is needed (`00` = whenever, `FF` = immediate) |

Synonym for integrators: **XX = Frequency**, **YY = Severity**, **ZZ = Urgency**.

---

## Encoding example

Frequency = **255** (`FF`), Severity = **99** (`63`), Urgency = **71** (`47`) yields **`FF6347`** in `XXYYZZ` notation.
...
**AGAPE** (`lupo_agents` **705**, slug **`agape`**) uses Counting in Light in **`AGAPE_PATTERN_REPORT`** and related telemetry. **AGAPE** MUST flag **sentimental drift** and **game-like misuse** of this system as **high-frequency defect classes** (see **`agents/agape/system_prompt.md`**). On **AGAPE-authored** envelopes, **`mood_vector`** **MUST** remain **`666666`**; full-axis tokens belong **only** in **`pattern_table`** / evidence rows. **Emission policy** (emotional agents only vs neutral **`666666`**) is normative in **`docs/doctrine/AGAPE_DEFECT_TAXONOMY.md`**.

---

## Relationship to the Survivability Doctrine

Cross-reference: **`docs/doctrine/SURVIVABILITY_DOCTRINE.md`** (**PRD 00** section **14.6**).
...
- **Pillar 2 -- Learning Transfer:** When **`light_state`** is **`flare`**, treat as a **Learning Transfer trigger** per **Survivability** section 7. When an agent **repeatedly fixates** on hex or buckets (non-terminating narration, "optimization" of token for social effect), treat as **failed transfer** and open a remediation item.
...
| AGAPE system prompt | **`agents/agape/system_prompt.md`** |
| AGAPE defect taxonomy | **`docs/doctrine/AGAPE_DEFECT_TAXONOMY.md`** |
| ROSE synthetic dialog | **PRD 36**, **`agents/rose/system_prompt.md`** |
