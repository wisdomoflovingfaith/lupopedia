...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/doctrine/bayesian_decision_doctrine.md
  web_path: https://www.lupopedia.com/lupopedia/docs/doctrine/bayesian_decision_doctrine.md
  status: DEPRECATED
...
- **Location:** `database/lupopedia/mysql/install/install_new_lupopedia.sql`
...
- When engine code is added:
  - Writes MUST go through the PDO_DB wrapper (no raw PDO). **Installer exception (constitutional carve-out):** **`docs/doctrine/DATABASE_DOCTRINE.md`** — **Runtime database access (PDO_DB) and installer exception**.
...
- TOONs: `database/lupopedia/toon/lupo_decisions.toon.json`, `lupo_decision_edges.toon.json`, `lupo_decision_influences.toon.json`.
