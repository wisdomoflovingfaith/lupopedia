...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/doctrine/health_check_doctrine.md
  web_path: https://www.lupopedia.com/lupopedia/docs/doctrine/health_check_doctrine.md
  status: ''
...
**Command:** `php bin/lupo.php doctor` [options]

**Behavior:** When the DOCTOR actor scripts are present (`agents/1009/doctor.php`), the CLI runs that script. Otherwise it runs the built-in **`lupo_doctor_health_check()`** function.
...
- **Context kernel issues:** If any are found, each is listed with `*` and a follow-up line suggests: `Run: php bin/lupo.php doctor-context [--repair]`
...
| Item | Default / source |
|------|-------------------|
| **LUPO_DATABASE_DIR** | `database` (from config or bootstrap) |
...
When `agents/1009/doctor.php` exists, `lupo doctor` is handled by the DOCTOR actor script instead of `lupo_doctor_health_check()`. The actor can use `DoctorService` and may produce different output; the **checks** above describe the built-in fallback.
...
- agents/1009/ — DOCTOR actor (1009) when used for `lupo doctor`
