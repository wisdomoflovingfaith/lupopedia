...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/doctrine/actor_primary_key_doctrine.md
  web_path: https://www.lupopedia.com/lupopedia/docs/doctrine/actor_primary_key_doctrine.md
  status: active
...
Legacy registry at `database/lupopedia/actors/actor_id/registry.json` remains for backward compatibility; new code should prefer the name-keyed registry.

The `dir` field in **`registry.json`** is the **actor hub** path relative to the repo root. It MUST match [PRD 00 §5.6](../prd/00_root_constitutional_system_requirements.md#56-actor-id-semantics) and **`IDENTITY_LAYERS_DOCTRINE.md` §3.5**: **`actors/{actor_id}/`** for **`actor_id` &lt; 2026**, and **`actors/YYYY/MM/{actor_id}/`** for typical runtime allocations **`actor_id` ≥ 2026**. Slug-only hubs such as **`actors/countermeasure/`** are invalid for registry-backed actors when **`dir`** is numeric.

## 3.1 Directory structure (actor_id–keyed hub)

- **Canonical hub paths:** Decimal **`actor_id`** (and date sharding when required). Examples: `actors/0/`, `actors/111/`, `actors/102/`.
...
- **Resolving path in code:** Use `ActorService::getActorDir($actor_name)` to get the registry **`dir`** (e.g. `actors/111`). Use `Resolver::actorPathByDir($base_path, $actor['dir'])` where applicable for safe file access.
...
- `getActorDir($actor_name)` — returns registry `dir` (e.g. `actors/0`, `actors/111`) or fallback `LUPO_ACTORS_DIR . '/' . $actor_name`
...
- `actor_id` (int)
- `actor_name` (string)
- `dir` (string) — relative path from registry (e.g. `actors/0`) for filesystem access
...
2. **Existing DB:** Run `database/lupopedia/mysql/migrations/20260306_actor_primary_key_migration.sql` to add `actor_name`, backfill from `actor_id` mapping, then switch primary key and update referencing tables.
3. **Deployment:** After migration, ensure `database/lupopedia/actors/registry.json` exists and is loaded by ActorService / ActorLookup.
4. **Filesystem (name-based dirs):** Run `php database/lupopedia/mysql/migrations/20260306_actor_directory_migration.php` (use `--dry-run` first). This renames numeric dirs to name-based and creates symlinks for backward compatibility. See section 3.1.

## 7. References

- Install schema: `database/lupopedia/mysql/install/install_new_lupopedia.sql`
- Seed: `database/lupopedia/mysql/seed/seed_actors_agents_4.0.45.sql`
- Migration (DB): `database/lupopedia/mysql/migrations/20260306_actor_primary_key_migration.sql`
- Migration (dirs): `database/lupopedia/mysql/migrations/20260306_actor_directory_migration.php`
