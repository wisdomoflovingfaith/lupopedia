...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/doctrine/PRD_GAPS.md
  web_path: https://www.lupopedia.com/lupopedia/docs/doctrine/PRD_GAPS.md
  status: active
  when_updated: '20260513033046'
  trust_tier: canonical
  questions_toon: null
  memory_toon: memory/development/canonical/1026/04/prd_gaps.toon
  atoms_toon: null
...
---
**Location:** `docs/doctrine/PRD_GAPS.md`  
**Linked from:** `docs/prd/PRD_INDEX.md` (via cross-reference)

# PRD number gaps and reserves (00-99)

## Purpose

PRD filenames use a **two-digit numeric prefix** (`NN_*.md`) as a **group identifier** (see **`docs/prd/PRD_INDEX.md`**). **Gaps** (no file yet) are not automatically errors: some ranges are **reserved**, some are **deferred**, and some await drafting.

## Numbers with no primary PRD file today

The following **NN** values have **no** `NN_*.md` in `docs/prd/` at the time of this document (inventory aligns with the generator scan and manual review):
...
| **Known relocated / superseded** | **81** | Superseded. Original material (`81_agent_orchestration_chat.md`) moved to `docs/prd/archive/`. Do not reuse 81 for new topics. |
...
2. Add **`NN_topic.md`** under **`docs/prd/`** with a complete **`lupopedia.headers`** block (**PRD 16** freeze: **`header_format_version: "4.1.3"`**).

3. Run **`python scripts/generate_prd_index.py`** (do **not** hand-edit **`PRD_INDEX.md`**).
...
2. **Move to archive** — `mv docs/prd/NN_*.md docs/prd/archive/`
...
4. **Add redirect note** — At the top of the archived file (after header):
   ...
   > This PRD has been superseded by PRD MM. See `docs/prd/MM_*.md`.

5. **Regenerate index** — `python scripts/generate_prd_index.py`
...
- **`docs/prd/PRD_INDEX.md`** — Auto-generated index (cross-link from index body to this file).
