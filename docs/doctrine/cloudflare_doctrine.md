...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/doctrine/cloudflare_doctrine.md
  web_path: https://www.lupopedia.com/lupopedia/docs/doctrine/cloudflare_doctrine.md
  status: ''
...
- **Handler:** `includes/classes/CloudflareRequestHandler.php` runs from `bootstrap.php` after security headers. It normalizes `REMOTE_ADDR` from `CF-Connecting-IP` (with optional proxy IP validation) and defines `LUPO_CLIENT_IP`, `LUPO_CLIENT_COUNTRY`, `LUPO_CF_RAY`, `LUPO_CF_THREAT_SCORE`, `LUPO_REQUEST_VIA_CLOUDFLARE`.
...
// define('LUPO_CLOUDFLARE_IPS_FILE', ABSPATH . 'database/cloudflare-ips-v4.txt');
...
2. Save as e.g. `database/cloudflare-ips-v4.txt` (one CIDR per line).
...
- `agents.php?actor_id=0&what=federation_node_id=0` → path like `database/.../federation_node_id_0/actor_id/0/` or `actors/0/`.
...
1. Ensure `CloudflareRequestHandler.php` is under `includes/classes/` and bootstrap is unchanged (it already requires and runs `CloudflareRequestHandler::process()`).
2. If using IP validation, create `database/cloudflare-ips-v4.txt` and set `LUPO_CLOUDFLARE_IPS_FILE`.
