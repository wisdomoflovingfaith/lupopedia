...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/doctrine/configuration_doctrine.md
  web_path: https://www.lupopedia.com/lupopedia/docs/doctrine/configuration_doctrine.md
  status: active
...
**Canonical implementation:** `includes/classes/LupopediaConfigResolver.php` — used by `index.php`, `admin.php`, `install.php`, `channel.php`, `ajax.php`, `livehelp_js.php`, `image.php`, `login.php`, and related entrypoints **before** loading `lupopedia-config.php`.

`includes/bootstrap.php` does **not** search; it runs only **after** the config file has been loaded and defined `LUPOPEDIA_CONFIG_LOADED`.
...
4. `dirname(LUPOPEDIA_PATH)/lupopedia-config.php` only if that parent does **not** contain `includes/bootstrap.php` (not another Lupopedia tree; mirrors WordPress skipping parent `wp-config` when the parent is another install).
...
The `includes/bootstrap.php` file MUST:
