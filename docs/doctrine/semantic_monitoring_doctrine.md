...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/doctrine/semantic_monitoring_doctrine.md
  web_path: https://www.lupopedia.com/lupopedia/docs/doctrine/semantic_monitoring_doctrine.md
  status: active
  ...
  thread_key: semantic-monitoring-doctrine
...
**Canonical UI specification:** **`docs/prd/28_semantic_monitoring_widget.md`** — floating chrome, collections behavior, edge presentation, motion/DynAPI constraints, etc.

**Current implementation anchor (repo):**

- Dynamic JS output: **`includes/modules/nav/semantic-navbar-js.php`**
- Integration entry: **`includes/modules/nav/semantic_navbar.php`**
- **Routing** (front controller): **`includes/modules/module-loader.php`** — slugs such as **`nav/semantic-navbar`**, **`nav/semantic-navbar-js`**, **`nav/semantic_navbar`**
- JSON backing for navbar types: **`includes/modules/api/semantic-navbar-api.php`** — invoked for slugs matching **`edges|contexts|hashtags|folders|qa|references|namespaces|next|previous`/`{slug}`** (see **`module-loader.php`**)
...
- **Root generator:** **`livehelp_js.php`** (bootstraps config, emits JS; **`PDO_DB`** / **`lupo_`** tables only — **installer exception** for DB driver rules: **`docs/doctrine/DATABASE_DOCTRINE.md`** — **Runtime database access (PDO_DB) and installer exception**)
- **Slug-routed module:** **`includes/modules/crafty_syntax/livehelp-js.php`** when the request is normalized to **`livehelp_js`** / **`livehelp-js`** (**`module-loader.php`**)
- **Legacy image / state channel:** root **`image.php`** and **`includes/modules/crafty_syntax/visitor-image.php`** (for **`what=`** patterns routed as **`image`**)
...
**Softaculous / installer:** the distribution must ship **root** **`livehelp_js.php`**, **`image.php`**, **`index.php`**, **`includes/`** (including **`modules/nav/`**, **`modules/crafty_syntax/`**, **`modules/api/`**), and **rewrite rules** so slugs and **`api/`**-style paths reach **`index.php`** — see **`InstallWizardHtaccessWriter`** and **PRD 33**.
...
**`docs/doctrine/MOBILE_SEPARATION_DOCTRINE.md`** — same semantic obligations, **different** DOM/JS where interaction diverges. **WOLFIE**-owned desktop chrome vs IDE-assisted **mobile web** per **`WOLFIE_WORKFLOW_DOCTRINE.md`**.
...
| Topic | Canonical doc |
|-------|----------------|
| Eye widget UI | **`docs/prd/28_semantic_monitoring_widget.md`** |
| Analytics / visits / edges | **`docs/prd/11_analytics_tracking.md`** |
| Chat display / actors | **`docs/prd/18_channel_chat_display.md`** |
| JS foundation | **`docs/prd/04_lupopedia_js_foundation.md`** |
| 4.1.0 / embed checklist | **`docs/prd/33_softaculous_certification_4_1_0_gate.md`** |
| Mobile vs desktop | **`docs/doctrine/MOBILE_SEPARATION_DOCTRINE.md`** |
| Build / consumer order | **`docs/doctrine/WOLFIE_WORKFLOW_DOCTRINE.md`** |
