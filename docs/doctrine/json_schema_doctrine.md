...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/doctrine/json_schema_doctrine.md
  web_path: https://www.lupopedia.com/lupopedia/docs/doctrine/json_schema_doctrine.md
  status: draft
  ...
  thread_key: json-schema-doctrine
...
**Location:** `docs/headers/{file_stem}.json`
...
**Generator:** `python scripts/generate_json_headers.py`

---

### `toon_v1` — Memory Node File (`.toon`)

**Location:** `memory/{YYYY}/{MM}/M-{slug}-{date}.toon`
...
**Generator:** `python scripts/generate_json_headers.py` or `python scripts/migrate_transcript_to_memory.py`
...
3. Migration scripts live in `scripts/` and handle version upgrades
...
```bash
python scripts/validate_json_schema.py --schema header_json_v1 --dir docs/headers/
python scripts/validate_json_schema.py --schema toon_v1 --dir memory/
```

*(Validator script: TODO — create `scripts/validate_json_schema.py`)*
