...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/doctrine/session_model.md
  web_path: https://www.lupopedia.com/lupopedia/docs/doctrine/session_model.md
  status: active
...
- `session_identity_hash` — Composite base identity hash: SHA256(ClassC_IP + user_id_or_unknown + Filtered_UA + SALT). MUST NOT include `actor_id` or `auth_user_id`; those are post-login fields and the base hash must be stable for anonymous/pre-login visitors.
...
- `docs/database/lupopedia/tables/active/lupo_sessions.md` — Table documentation.
- `app/auth/Session.php` — canonical `App\Auth\Session` implementation (`includes/bootstrap.php` loads this before `LUPO_APP_DIR/auth/`).
