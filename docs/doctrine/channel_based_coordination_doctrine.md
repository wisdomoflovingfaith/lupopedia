...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/doctrine/channel_based_coordination_doctrine.md
  web_path: https://www.lupopedia.com/lupopedia/docs/doctrine/channel_based_coordination_doctrine.md
  status: ''
...
- `channels/` is communication-only scope (discussion, coordination, reasoning, planning).
- `channels/` is not canonical doctrine/documentation authority, not final context storage, and not schema/edge authority.
- Canonical documentation lives in `docs/`; structural authority lives in database and canonical schema sources.
...
**Channel 42** (`channels/42/`) is the designated primary coordination channel for all multi-agent work in Lupopedia.
...
- ❌ Using `docs/status/` for coordination
