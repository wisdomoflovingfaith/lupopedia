...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/doctrine/learned_from_wordpress.md
  web_path: https://www.lupopedia.com/lupopedia/docs/doctrine/learned_from_wordpress.md
  status: active
  ...
  thread_key: learned-from-wordpress
...
This document records **specific, actionable patterns** observed in **WordPress** source under **`archive/legacy/wordpress-reference/`** (educational reference only; **GPL** — **do not** paste or ship WordPress code into Lupopedia).

**Checkout note:** **`archive/`** is **`.gitignore`d**; a fresh **git clone** may not include the WordPress tree. Place a local study copy under **`archive/legacy/wordpress-reference/`** when verifying line numbers.

**Why this file exists**

- IDE agents should **not** re-scan the entire **`archive/legacy/wordpress-reference/`** tree for routine work.
- This is the **single Lupopedia doctrine** surface for WordPress-derived resilience patterns.
- When you discover a **new** applicable pattern, **append** it here with a path + line reference into **`archive/legacy/wordpress-reference/`** (WordPress version in this checkout: **6.9.4** unless the tree is upgraded).
...
|----------------|--------|
| `archive/legacy/wordpress-reference/wp-admin/includes/class-wp-debug-data.php` | `SERVER_SOFTWARE`, extensions, `.htaccess` marker strip, writability |
| `archive/legacy/wordpress-reference/wp-includes/PHPMailer/SMTP.php` | `stream_socket_client` vs `fsockopen`, `set_error_handler` |
| `archive/legacy/wordpress-reference/wp-includes/version.php` | Required PHP version and extension list |
| `archive/legacy/wordpress-reference/wp-includes/load.php` | `is_ssl()`, server var fixes |
| `archive/legacy/wordpress-reference/wp-includes/functions.php` | `wp_normalize_path()`, `wp_raise_memory_limit()` |
| `archive/legacy/wordpress-reference/wp-includes/class-wpdb.php` | `prepare()` / placeholder discipline |
...
**File:** `archive/legacy/wordpress-reference/wp-admin/includes/class-wp-debug-data.php`  
...
**Shipped:** **`install/InstallWizardHtaccessWriter.php`** — **`isApacheHtaccessEnvironment()`** (Apache / LiteSpeed vs Nginx / IIS / Caddy; empty **`SERVER_SOFTWARE`** still attempts **`.htaccess`** write for dev/odd stacks).
...
**File:** `archive/legacy/wordpress-reference/wp-includes/PHPMailer/SMTP.php`  
...
**Shipped:** **`InstallWizardHtaccessWriter::insertWithMarkers()`** — writes **`# BEGIN LUPOPEDIA` / `# END LUPOPEDIA`** (root) and **`# BEGIN LUPOPEDIA_DB` / `# END LUPOPEDIA_DB`** (**`database/.htaccess`**).  
...
**File:** `archive/legacy/wordpress-reference/wp-includes/version.php`  
...
**File:** `archive/legacy/wordpress-reference/wp-includes/load.php`  
...
**File:** `archive/legacy/wordpress-reference/wp-includes/functions.php`  
...
**File:** `archive/legacy/wordpress-reference/wp-includes/class-wpdb.php`  
...
- **Installer exception (constitutional carve-out):** **`install.php`** / wizard **may** use **`mysqli`** only as in **`docs/doctrine/DATABASE_DOCTRINE.md`** — **Runtime database access (PDO_DB) and installer exception**; runtime **must not** use **`mysqli`**.
...
1. Read this file first when the task mentions WordPress-style hosting, fallbacks, or **`archive/legacy/wordpress-reference/`**.
2. Open **`archive/legacy/wordpress-reference/`** only to **verify** line numbers or study a **new** pattern to add here.
3. **Do not** paste WordPress sources into Lupopedia shipping paths.

**Humans**

1. Keep entries **short**: pointer, pattern, Lupopedia mapping.
2. When upgrading **`archive/legacy/wordpress-reference/`**, re-spot-check cited line numbers.

---

## References (Lupopedia)

- **Constitutional:** `docs/prd/00_root_constitutional_system_requirements.md` — **§15**
- **Study / gate:** `docs/prd/33_softaculous_certification_4_1_0_gate.md` — **Section 14**
- **LILITH answers:** `docs/implementations/33_softaculous_certification_4_1_0_gate/answers/20260404_061932_ANSWER_wordpress_distribution_patterns_lilith.md`
