...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/doctrine/table_count_doctrine.md
  web_path: https://www.lupopedia.com/lupopedia/docs/doctrine/table_count_doctrine.md
  status: active
...
- **Method:** Count of `CREATE TABLE` statements in that file, or run `python scripts/generate_toon_files.py` and count TOON files in `database/lupopedia/toon/`.
- **Verified 2026-03-16 (4.0.77, post-install TOON regeneration):** **161** tables. After drop → load Crafty 3.7.5 baseline → run install → `python scripts/generate_toon_files.py`, TOON count is 161. Install file is the schema authority. Table ceiling remains advisory (e.g. 222); current count is within capacity.
...
- In-repo TOON set: `database/lupopedia/toon/*.toon.json` (from `generate_toon_from_sql.py`).
- Live-DB TOON output: `database/lupopedia/json/*.json` and `database/lupopedia/toon/*.toon` (from `generate_toon_files.py`).
