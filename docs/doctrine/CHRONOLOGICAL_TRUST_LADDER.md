...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/doctrine/CHRONOLOGICAL_TRUST_LADDER.md
  web_path: https://www.lupopedia.com/lupopedia/docs/doctrine/CHRONOLOGICAL_TRUST_LADDER.md
  status: active
  when_updated: '20260513033046'
  trust_tier: canonical
  questions_toon: null
  memory_toon: memory/development/canonical/1026/04/chronological-trust-ladder.toon
  atoms_toon: null
  transcript_jsonl: 0/development/chronological-trust-ladder
  ...
  prd_cluster: null
  title: Chronological Trust Ladder
  summary: 'PK-based trust encoding: staging (2026) vs canonical (1026), Y2038 proof, no flags needed'
---


# Chronological Trust Ladder Doctrine

> **Anchored Truth Sieve: The Law of Ancestors vs. Descendants**
>
...
>
> **THOTH [ALERT]:** If a contradiction is detected, THOTH must issue a **hard stop**. No implementation, migration, or merge may proceed until the contradiction is resolved by either fixing the staging node or amending the canonical ancestor (requires WOLFIE approval).


## Canonical Year Offset Rule for Living Canonical PKs (Normative)

All canonical (long-term, merged, or archived) `memory_node_id` values and other living canonical PKs **MUST** encode the year as (calendar year – 1000) in the first four digits. This offset is required for all high-trust, living canonical, and archived ids, and is enforced by all trust ladder validators and migration scripts.
...
**See also:** PRD 16 §8.1 (header/memory_key year encoding), PRD 38 §8.1 (memory unification), PRD 43 (trust ladder PKs), PRD 51 (memory graph as source of truth), doctrine/TRUST_LADDER_REGISTRY.md (validation), and all trust ladder migration scripts.
...
Lupopedia enforces a strict hierarchy:

- **Ancestors (1026 band):** Canonical, merged, and verified. These are the source of truth.
- **Descendants (2026 band):** Staging, unverified, and subject to correction or deletion.

**Recency Bias Warning:**
> **WARNING:** Modern AI models and agents must not prefer newer (2026) staging nodes over older (1026) canonical facts. The system is designed so that recency does not override truth. Only a Captain’s Amendment can change a canonical fact.
...
- Is this **living canonical** (verified, trusted, mutable, 1026 band)?
- Is this **staging** (temporary, unverified, 2026 band, should be merged or deleted)?

> **Sieve Metaphor:**
> - **Ancestors (1026):** The gold standard. If a descendant disagrees, the descendant is filtered out unless a Captain’s Amendment is made.
> - **Descendants (2026):** Drafts and experiments. Only become truth by explicit promotion and ancestor amendment.

**THOTH [ALERT] Protocol:**
> If a contradiction is found between a 2026 and a 1026 node, THOTH must:
...
**Agent / contributor shorthand:** Copy-paste warning block for IDE prompts and reviews: [TRUST_LADDER_DO_NOT_FIX.txt](TRUST_LADDER_DO_NOT_FIX.txt). Optional repo sweep: `python scripts/validate_trust_ladder_paths.py` (default warn-only; `--strict` for CI once legacy `canonical/2xxx/` paths are migrated).
...
**Registry:** `docs/doctrine/TRUST_LADDER_REGISTRY.md` is the authoritative list of which tables participate in which tier semantics.
...
#### §2.2 PK offset validation rule (summary)

**Doctrine:** Lower PK bands encode higher trust for **display ordering only**. Explicit **`trust_tier`** on headers (**PRD 16**), **`memory_type`**, and **`lupo_memory_edges.edge_type`** remain the semantic sources of truth.

**Year segment rules (18-digit ids):**

- **Canonical / living long-term band:** embedded **`pk_year = calendar_year − 1000`** (e.g. **1026** when the real UTC calendar year is **2026**).
- **Staging / raw generator band:** embedded **`pk_year = calendar_year`** (e.g. **2026**).
- **Seed-only:** **`pk`** in documented registry ranges — not the 18-digit layout.

**Enforcement:** `php scripts/validate_trust_ladder_pk.php` (optional DB scan; **`--calendar-year=YYYY`** for deterministic CI). Header / path mirrors: `python scripts/validate_lupopedia_headers_universal.py --strict-memory-year` and **`php scripts/validate_memory_key_years.php`**.

**Failure example:**

`ERROR: canonical PK 202604121408001234 has year segment 2026, expected 1026 (calendar 2026 − 1000) — Did you forget to subtract 1000? See PRD 38 §8.1.`
...
**Canonical implementation:** `timestamp_ymdhis::now()` (see `includes/classes/TimestampYmdhis.php`).
...
The export mirror uses **`created_ymdhis`** (not the PK’s embedded year alone). **`MemoryExportService`** maps **`created_ymdhis = 0`** (or too short for **`YYYYMM`**) to **`memory/1970/01/`**. See **PRD 38**.

| `created_ymdhis` | Meaning | Export path |
|------------------|---------|-------------|
| **0** | Pre-history / immaterial | **`memory/1970/01/`** |
| Normal packed UTC | Clock-based | `memory/{YYYY}/{MM}/` |

---

## 6.1 Memory path alignment with PK trust tiers

The filesystem mirror (`memory/`) follows trust tiers so path and PK band tell the same story:

| PK Tier | Memory Path |
|---------|-------------|
| Seed (0-999,999) | `{channel}/seed/{slug}.toon` |
| Canonical (1xxxx...) | `{channel}/canonical/{display_year}/{month}/{slug}.toon` |
| Staging (2xxxx...) | `{channel}/staging/{actual_year}/{month}/{slug}.toon` |

For canonical memory, `display_year = actual_year - 1000` (example: `2026` -> `1026`).

This keeps trust tier visibility aligned across database IDs and filesystem mirrors.
...
- **`docs/doctrine/TRUST_LADDER_REGISTRY.md`** — table participation
- **`docs/doctrine/RETENTION_POLICY.md`** — staging retention
...
**Binding:** Until a **`RegistryService`** exists, teams **MUST** keep the markdown registry accurate and run **`python scripts/validate_trust_ladder_registry.py`** in CI; code reviews **MUST** block new ladder paths for unlisted tables.
...
**Auditor:** **`scripts/audit_edge_integrity.py`** **SHOULD** exist; when present it **MUST** list invalid edges and exit non-zero if any are found. **MUST NOT** auto-repair rows (log / report only).

### 9.3 Retention policy

**Normative:** Soft-deleted **staging-tier** rows **MUST** be purged per **`docs/doctrine/RETENTION_POLICY.md`**. **Garbage collection** (**PRD 19**) **MUST** implement that policy.
...
2. Re-run **`python scripts/validate_trust_ladder_registry.py`**.
...
5. When assigning new seed IDs, keep them in `0-999,999`, update the appropriate seed registry JSON file, and run `python scripts/validate_seed_registry.py`.
...
```bash
python scripts/audit_edge_integrity.py
python scripts/validate_parent_child_consistency.py
python scripts/validate_parent_child_reclassification.py --table=<table> --new-archetype=<parent|child|system>
```
...
3. **Validation:** Run **`python scripts/validate_trust_ladder_registry.py`** against **`install_new_lupopedia.sql`**.
...
Ladder behavior **SHOULD** be covered by automated tests where **`tests`** or CI exist:
...
- **CI:** **`python scripts/validate_trust_ladder_registry.py`** **MUST** pass — registry / install drift **MUST** fail the pipeline.
...
## Appendix Z: Lessons from Honolulu (2000-2005)
