...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/doctrine/schema_canonical_sources.md
  web_path: https://www.lupopedia.com/lupopedia/docs/doctrine/schema_canonical_sources.md
  status: ''
...
1. **`database/lupopedia/mysql/install/install_new_lupopedia.sql`** — Highest authority. Defines the full schema for 4.0.x. All required tables and columns live here.
2. **TOON files** (e.g. `database/lupopedia/toon/*.toon.json`) — Generated from or aligned with install SQL. They must match install SQL; if they do not, regenerate or update TOONs to match install SQL (do not change install SQL to match stale TOONs unless the TOON reflects an intended fix).
3. **Doctrine documents** (e.g. `docs/doctrine/BAYESIAN_DECISION_DOCTRINE.md`) — Describe the schema and usage rules. If doctrine does not match install SQL, update doctrine to describe reality.
4. **Planning copies** (e.g. `docs/database/lupopedia/tables/planning/`) — Lowest priority. May be historical or stale. Either update them to match install SQL or label them clearly as non-canonical snapshots.
