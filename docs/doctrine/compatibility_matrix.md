...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/doctrine/compatibility_matrix.md
  web_path: https://www.lupopedia.com/lupopedia/docs/doctrine/compatibility_matrix.md
  status: ''
...
**Status:** Canonical summary; syntax detail: **`rules/root/php-7-4-compatibility.md`**  
**Applies to:** Lupopedia 4.0.x — core PHP code (`includes/`, entrypoints, themes)  
...
Do **not** use these in **`includes/`**, **`admin.php`**, **`index.php`**, and theme layouts unless the file is explicitly modern-only:
...
**Installer exception (constitutional carve-out):** **`docs/doctrine/DATABASE_DOCTRINE.md`** — **Runtime database access (PDO_DB) and installer exception**. **Runtime** uses **`PDO_DB`** only and **named placeholders**; **`install.php`** / wizard **may** use **`mysqli`** only for the narrow installer purposes listed there.
