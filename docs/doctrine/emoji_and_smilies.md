...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/doctrine/emoji_and_smilies.md
  web_path: https://www.lupopedia.com/lupopedia/docs/doctrine/emoji_and_smilies.md
  status: active
  when_updated: '20260513033046'
  trust_tier: canonical
  questions_toon: null
  memory_toon: memory/development/canonical/1026/04/emoji-and-smilies.toon
  atoms_toon: null
  ...
  summary: 'Canonical Lupopedia filesystem emoji: ::img|folder|file:: tokens, emoji/ storage, render-time replacement only; Crafty DB smilies deprecated; validation, encoding safety, chat storage rules.'
---
...
| Aspect | Rule |
|--------|------|
| **Storage** | **Filesystem** under **`emoji/`** (subfolders + image files). |
...
Channels are dialog-based; operators and visitors insert emoji by **token** or UI selector that emits the token. The runtime resolves tokens against **`emoji/`** when building **view** output.
...
- **`foldername`**: subdirectory under **`emoji/`** (see **Validation Rules**).
...
Resolves at render time to an image sourced from **`emoji/classic/smile.png`** (subject to base path and extension rules below).

## Emoji selector popup

- UI may offer a picker that lists images discovered under **`emoji/`** (by folder).
...
| **Base path** | Resolved file **must** stay under **`emoji/`**; reject absolute paths, drive letters, or `..` segments. |
...
- **Enforce base path** **`emoji/`** relative to install layout; do not honor arbitrary filesystem roots from token content.
...
- **Do NOT** allow **arbitrary file paths** inside tokens — only validated **`foldername`** + **`filename`** under **`emoji/`** with allowed extensions.
...
- Operators can add/remove sets by changing files under **`emoji/`**.
