...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/doctrine/lupopedia-headers/lupopedia_headers_format.md
...
**Normative:** **[PRD 16](../../prd/16_lupopedia_headers.md) — Header freeze rule (4.1.3)** is binding. The declared **`header_format_version`** string for **new and edited** normative envelopes is **`"4.1.3"`** until the Crafty Syntax human live-help baseline is complete. **No** normative doctrine, templates, or agent examples should treat **`4.1.5`**, **`4.2.0`**, or generic **`4.1.x`** placeholders as the active contract during the freeze. **Header-format evolution resumes in 4.1.5+** after that baseline (not abandoned; paused).
...
**Key order (mandatory):** All **22** fields MUST appear in the **exact order** below (lines 3–24 under `lupopedia.headers:` in Markdown — dense grid, **no blank lines** between fields). Validators emit **`HDR_KEY_ORDER`** on mismatch. Full list: **[PRD 16](../../prd/16_C-i_LUPOPEDIA_HEADERS.md)** section 4.2.

**Normative version:** **`header_format_version: "4.1.9"`** for new and migrated files.

**YAML `null` vs empty string:** Use **`null`** for nullable pointers (`questions_toon`, `memory_toon`, `atoms_toon`, `edges_toon`, `source_timestamp`). Use **`''`** for `thread_key`, `title`, `status`, `summary`, `prd_cluster` when empty. **`channel_index`** is always a non-empty string.

**Forbidden in v4.1.9 headers (HDR_REMOVED_FIELD):** `content_id`, `content_parent_id`, `default_collection_id`, `content_slug`, `pk_*`, `prd_id`, `prd_slug`, `parent_prd`.

| # | Field | Type | Notes |
|---|---|---|---|
| 1 | `header_format_version` | string | **`"4.1.9"`** |
| 2 | `path_from_lupopedia_root` | string | Repo-relative path (POSIX `/`). Agent anchor. |
| 3 | `web_path` | string | `https://www.lupopedia.com/lupopedia/` + path |
| 4 | `status` | string | Lifecycle; `''` allowed |
| 5 | `when_updated` | string | UTC `YYYYMMDDHHIISS` (Lupopedia mutation time) |
| 6 | `trust_tier` | string | `canonical` \| `development` (legacy tiers may warn) |
| 7 | `questions_toon` | null or string | Ends in `.questions.toon` when set |
| 8 | `memory_toon` | null or string | Ends in `.toon`; context graph sidecar |
| 9 | `atoms_toon` | null or string | Ends in `.atoms.toon` when set |
| 10 | `transcript_jsonl` | string | DB slug `{federation_node_id}/{channel_key}/{routing_slug}` |
| 11 | `artifact_type` | string | Closed enum per PRD 16 |
| 12 | `artifact_kind` | string | Cross-validates with `artifact_type` |
| 13 | `channel_key` | string | Semantic channel slug |
| 14 | `federation_node_id` | integer | `0` repo, `1` local, `2+` external/research |
| 15 | `thread_key` | string | Thread scope; `''` when unbound (`thread_id` legacy alias) |
| 16 | `lupopedia.schema` | string | Must equal `artifact_type` |
| 17 | `prd_cluster` | string | Governing PRD token chain |
| 18 | `title` | string | Short title for head-of-file discovery |
| 19 | `summary` | string | Single-line description |
| 20 | `edges_toon` | null or string | Ends in `.edges.toon`; **required** when `channel_index != lupopedia` |
| 21 | `channel_index` | string | `lupopedia` \| `patreon` \| `website` \| `facebook` \| `blog` \| `external` \| `imported` |
| 22 | `source_timestamp` | null or ISO 8601 | Origin time in source system; **required** when external |

## Removed from YAML header (v4.1.9)

- **`content_id`**, **`content_parent_id`**, **`default_collection_id`**, **`content_slug`** and all **`pk_*` / `prd_*` identity keys** -- use `path_from_lupopedia_root`, `memory_toon`, and import tooling instead.
- **`lupopedia.edges`**, **`lupopedia.footer`**, tags, purpose, author blocks -- belong in **`memory_toon`** / **`edges_toon`** sidecars or file footer, not in the 22-key envelope.

## New minimal example (4.1.9 dense envelope)

**25-line** Markdown envelope: lines 3–24 are the 22 keys; line 25 closes `---`; body starts line 26.
...
lupopedia.headers:
  ...
  title: LUPOPEDIA HEADERS Format Specification (v4.1.9)
  summary: "22-field envelope; edges_toon, channel_index, source_timestamp"
  edges_toon: null
