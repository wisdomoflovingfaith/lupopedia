...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/doctrine/lupopedia-headers/readme.md
  web_path: https://www.lupopedia.com/lupopedia/docs/doctrine/lupopedia-headers/readme.md
  status: ''
  ...
  thread_key: headers-readme-index
  lupopedia.schema: doctrine
  prd_cluster: null
  title: Lupo docs doctrine lupopediaheaders readme
  summary: ''
---
# file: Lupo docs doctrine lupopediaheaders readme — delegation: cursor:root — web_path: http://www.lupopedia.com/lupopedia/docs/doctrine/LUPOPEDIA_HEADERS/README.md

# LUPOPEDIA HEADERS

## Single source of truth (binding doctrine)
**Important:**
The `lupopedia.edges` block must never appear inside the `lupopedia.headers` YAML block at the top of a file.
- The header is for metadata only (see PRD 16 and header format docs).
- `lupopedia.edges` is always a separate block at the very bottom of the file, after the main content.
- This trailing block is for machine-readable graph edges, import instructions, or cross-references, and is scanned by the import pipeline and graph engine.
- Do not include `lupopedia.edges` in the header or any YAML at the top of the file.
- See AGENTS.md and PRD 02 for rationale and examples.

**One file only:** **[`rules/root/LUPOPEDIA_HEADERS_DOCTRINE.md`](../../../rules/root/LUPOPEDIA_HEADERS_DOCTRINE.md)** — required header keys, `lupopedia.schema` / `artifact_type` / `artifact_kind` taxonomies, federation rules, validation expectations, **database-first mapping** (`lupo_contents`, `lupo_metadata`, `lupo_edges`, `revision_history`), and **`database/lupopedia/json/*.json`** / install SQL authority.

**Canonical specification:** **[`docs/prd/16_lupopedia_headers.md`](../../../prd/16_lupopedia_headers.md)** — defines header field requirements, author/verifier distinction, and artifact type/kind taxonomy.
...
- Root-relative paths like `http://www.lupopedia.com/docs/...` are invalid for this project.
...
```yaml
lupopedia.headers:
  file_path_from_root: "docs/doctrine/LUPOPEDIA_HEADERS/README.md"
  web_path: "http://www.lupopedia.com/lupopedia/docs/doctrine/LUPOPEDIA_HEADERS/README.md"
```
...
**Grounding rule (binding).** THOTH’s **conclusions** about schema, tables, columns, and edge types **must** be **evidence-backed** from repository files: at minimum **`database/lupopedia/json/*.json`** and **`docs/database/lupopedia/tables/active/*.md`**, plus install SQL / root rules as listed below. **General AI parametric knowledge** (model weights, uncited recall) is **not** authoritative and **must not** be treated as proof of current schema. An LLM may **rephrase** or **summarize** only **after** those sources are loaded into the working context (IDE buffer, PHP-fed prompt, or human paste).

**Cross-references:** **`docs/prd/00_root_constitutional_system_requirements.md`** — **§5.9** (THOTH), **§5.10** (service agents, PHP-first). Full architecture: **`docs/doctrine/SERVICE_AGENT_ARCHITECTURE.md`** — **§6**.
...
|-------------|----------|---------|
| **Table documentation** | `docs/database/lupopedia/tables/active/*.md` | Current table structure and usage |
| **TOON exports** | `database/lupopedia/toon/*.toon` (and `.toon.json`) | Canonical column/type reference from live DB |
| **JSON table exports** | `database/lupopedia/json/*.json` | Individual table structure snapshots |
| **Root rules** | `rules/root/*.md` | Constitutional constraints and invariants |
| **Edge model doctrine** | `docs/doctrine/EDGE_MODEL_DOCTRINE.md` | Canonical edge model and type registry |
| **Version documentation** | `docs/versions/<version>/` | Version-specific context and decisions |
...
2. **THOTH performs semantic truth check:**
  ...
  - Verify rule references against `rules/root/`
3. **Outcome:**
...
Files under `docs/database/lupopedia/tables/active/*.md` are a mapping surface and must include a grounded `lupopedia.edges` block.
...
- `docs/doctrine/SERVICE_AGENT_ARCHITECTURE.md` (THOTH grounding; service agents)
- `docs/doctrine/RELEASE_ARTIFACT_APPROVAL_GOVERNANCE_DOCTRINE.md`
- `rules/root/`
