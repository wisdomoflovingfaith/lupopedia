...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/doctrine/actor_requirements.md
  web_path: https://www.lupopedia.com/lupopedia/docs/doctrine/actor_requirements.md
  status: active
  ...
  summary: ''
---
# file: ACTOR_REQUIREMENTS — delegation: cursor:root
...
2. **Agent workspace** — Optional `requirements.json` (or `requirements` inside `agent.json`) in the agent’s directory under `agents/` or `actors/`. If present, `AgentService::getRequirements()` uses it first.
...
LUPO’s doctrine is seeded in **`database/lupopedia/mysql/seed/seed_registry_comprehensive_4.0.45.sql`**: the agent registry row for entity_index_id 106 includes a `requirements.database` block in `metadata_json`. No separate requirements table or column is used.

## Schema doctrine audit (LUPO)

The **canonical** schema audit tool checks **TOON files** in `database/lupopedia/toon/` (generated from the database) for doctrine violations. It does **not** use `information_schema` or a live DB connection for structure.
...
- **PHP (canonical):** `php scripts/audit_schema_doctrine.php` — reads TOONs from `database/lupopedia/toon/` (fallback: `json/`), writes `artifacts/reports/schema_doctrine_audit.json`. Use this as the source of truth.
...
- `includes/classes/AgentService.php` — `getRequirements($actor_id)`.
- `includes/classes/ActorRequirementsValidator.php` — `validateSqlAgainstLupo($sql)`.
