...
lupopedia.headers:
  ...
  path_from_lupopedia_root: docs/doctrine/ACTOR_HANDOFF_TOON_PROTOCOL.md
  web_path: https://www.lupopedia.com/lupopedia/docs/doctrine/ACTOR_HANDOFF_TOON_PROTOCOL.md
  status: active
  when_updated: '20260513033046'
  trust_tier: canonical
  questions_toon: null
  memory_toon: memory/development/canonical/1026/04/actor-handoff-toon-protocol.toon
  atoms_toon: null
  ...
  summary: Mandatory disambiguation for Lupopedia .toon memory sidecars; naming under memory/handoffs/; write-before-work checkpoint; PRD-16-aligned template; edges including disambiguates cartoon; ties to PRD 50 coordination.
---
...
**Directory (normative):** `memory/handoffs/`  
...
`memory/handoffs/development_102_handoff.toon`

## 4. Timestamps and authority

- **Header `when_updated`:** Use real UTC from `python bin/tick.py` / `python bin/echo_anchor_utc.py` only — no guessed or local-offset timestamps ([TIMESTAMP_DOCTRINE.md](TIMESTAMP_DOCTRINE.md) / PRD 00).
...
lupopedia.headers:
  ...
  file_path_from_root: "memory/handoffs/{channel_key}_{actor_identifier}_handoff.toon"
  web_path: "https://www.lupopedia.com/lupopedia/memory/handoffs/{channel_key}_{actor_identifier}_handoff.toon"
  status: "draft"
...
**UTC anchor:** paste from `python bin/echo_anchor_utc.py`
...
```json
{
  "edges": {
    "outbound": [
      ...
      {
        "type": "relates_to",
        "to": "docs/prd/50_agent_coordination_protocol.md",
...
Field names (`to` vs `target`) **SHOULD** match whatever the active graph importer expects; align with [`generate_memory_from_header.py`](../../scripts/generate_memory_from_header.py) output when using auto-generated sidecars.
...
[`generate_memory_from_header.py`](../../scripts/generate_memory_from_header.py) emits JSON-shaped `.toon` files for header-linked artifacts. Handoff files may be **authored** in Markdown for human clarity; runners **MAY** normalize to the JSON sidecar shape in a follow-up pass. Do not conflate that mechanical JSON with **Token-Oriented Object Notation** used at LLM boundaries (see captain's log on TOON disambiguation).
