...
/**
 * @wolfie.headers {
 *   file_path_from_root: "route.php",
 *   system_version: "4.0.39",
 *   channel_id: 42,
 *   mood_vector: "4169E1",
 *   purpose: "Slug front controller (pages, APIs). Default landing is index.php Color Registry mock.",
 ...
 *
 * Config search uses LupopediaConfigResolver (subdirectory OS: install dir first, then DOCUMENT_ROOT leftovers).
 *
 * @var string
 */
...
if (!defined('LUPOPEDIA_CONFIG_PATH') || !LupopediaConfigResolver::isSafeLocalConfigPath(LUPOPEDIA_CONFIG_PATH)) {
    ...
    $installUrl = rtrim($dir, '/') . '/install.php';
    if ($installUrl === '/install.php') { $installUrl = '/install.php'; }
    header('Location: ' . $installUrl);
    exit;
}
 ...
 * Priority: $_GET['resolved_uri'] (T5 rewrite) > $_GET['route'] (PRD 16 API) > $_GET['slug'] > PATH_INFO > REQUEST_URI
 */
$slug = '';
...
// Method 0b: explicit route= (e.g. index.php?route=api/transcript/append per PRD 16 §9.1)
elseif (isset($_GET['route']) && $_GET['route'] !== '') {
    $slug = is_string($_GET['route']) ? trim($_GET['route'], '/') : '';
}
