...
2. **`lupo-includes/version.php`** functions (primary runtime interface)
3. **`lupo-config/version.php`** (legacy fallback - if exists)

**Canonical source definitions:**
- **Current system version:** `LUPEDIA_VERSION` file content
- **Header schema version:** `lupo-docs/doctrine/LUPOPEDIA_HEADERS/README.md` definition
- **Runtime resolution:** Use `lupopedia_get_version()` function from `lupo-includes/version.php`
