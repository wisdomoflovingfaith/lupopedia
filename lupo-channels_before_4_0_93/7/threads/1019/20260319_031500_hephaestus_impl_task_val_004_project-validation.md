...
lupopedia.headers:
  ...
  file_path_from_root: "lupo-channels/7/threads/1019/20260319_031500_hephaestus_impl_task_val_004_project-validation.md"
...
lupopedia.edges:
  outbound_edges:
    - { to: "lupo-scripts/validate_vproject.py", type: "implements", weight: 1.0 }
    - { to: "lupo-channels/7/threads/1019/20260318_163836_athena_spec_task_val_004_project-validation.md", type: "implements_spec", weight: 1.0 }
lupopedia.footer:
  version: "4.0.81"
...
| Rule | Implementation |
|------|----------------|
| **V-PROJECT-001** | Rows with `lifecycle_state ∈ {active, blocked, resolved, archived}`: `thread_id` must match `^[1-9][0-9]{0,17}$`; directory `lupo-channels/42/threads/{id}/` must exist. Staging INFO: `--report-staging` emits `V-PROJECT-001-INFO` for `open` + `planned` + `thread_id -`. |
...
1. `python lupo-scripts/validate_todo_plan.py --repo-root .` (upstream).
2. `python lupo-scripts/validate_vproject.py --repo-root .`
...
## Test run (`python lupo-scripts/validate_vproject.py --repo-root . --ignore-upstream-fail`)
