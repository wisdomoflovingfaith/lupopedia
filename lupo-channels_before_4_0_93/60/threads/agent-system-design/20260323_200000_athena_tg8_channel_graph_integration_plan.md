...
lupopedia.headers:
  ...
  file_path_from_root: "lupo-channels/60/threads/agent-system-design/20260323_200000_athena_tg8_channel_graph_integration_plan.md"
  last_modified_utc: "20260323_200000"
  channel_id: 60
  ...
  references:
    - "lupo-channels/60/threads/agent-system-design/20260323_180000_wolfie_context_graph_system_activation.md"
    - "lupo-channels/61/GRAPH_MODEL_DOCTRINE.md"
    - "lupo-channels/59/threads/rose_dialogue/thread_index.md"
...
Channel messages are posted to the channel via the existing channels API (`lupo-includes/modules/api/channels-api.php`). The API handler flow is:
...
| Task | Module/File | Success Criteria |
|------|------------|-----------------|
| Routing logic | Modify `lupo-includes/modules/api/channels-api.php` or create `lupo-includes/modules/api/routing-api.php` | Calls ResolutionEngine::resolveFullContext(); applies decision logic; returns (actor, faucet, confidence) |
...
|------|------------|-----------------|
| Test message set | `lupo-scripts/tg8_test_messages.sql` | 50+ sample messages with various reference patterns; covers edge detection |
| Parser validation helpers | `lupo-scripts/tg8_validate_parser.php` | CLI tool to validate MessageEdgeParser output against ground truth |
| ROSE packet generator | `lupo-scripts/generate_rose_packets.php` | Creates test ROSE packets; feeds to consumer; verifies edge creation |
...
|------|-----------|-----------------|
| Invalid target handling | `lupo-tests/integration/tg8_invalid_targets.php` | Test @nonexistent_actor, invalid task IDs, malformed commands |
| Concurrency stress test | `lupo-tests/integration/tg8_concurrent_edges.php` | Concurrent message posting; verify no edge duplication, no lost edges |
| LILITH fallthrough test | `lupo-tests/integration/tg8_routing_confidence.php` | Create low-confidence scenarios; verify routing to LILITH |
| Large message test | `lupo-tests/integration/tg8_large_message_parsing.php` | Message with 100+ references; verify all parsed correctly |
