...
lupopedia.headers:
  ...
  file_path_from_root: "lupo-channels/42/directives/20260326_rose_correction.md"
...
lupopedia.init:
  ...
  pre_actions:
    - type: dependency_check
      target: "lupo-docs/versions/4.0.88/SEMANTIC_ARCHITECTURE.md"

lupopedia.edges:
  outbound_edges:
    - { to: "lupo-docs/versions/4.0.88/SEMANTIC_ARCHITECTURE.md", type: "corrects", weight: 1.0 }
    - { to: "lupo-channels/42/", type: "references", weight: 0.9 }
...
**ROSE IS:**
1. Reads channel artifacts (`lupo-channels/{channel_id}/broadcasts/`, `threads/`)
2. Identifies additional insights other agents missed
3. **Writes back as the original agent** (same `speaker`/`actor_name`)
4. Adds `mood_rgb` field (emotional context)
5. **ONLY ROSE has the `mood_rgb` capability** — other agents do not
...
4. **Creates new artifact** with:
   ...
   - **New `mood_rgb` field** (emotional context)
...
lupopedia.headers:
  ...
  actor_name: "cascade"
  delegation_chain: "cascade:root"
  mood_rgb: "4B0082"          # ← ONLY ROSE adds this
...
**Unique Capability**: ROSE adds `mood_rgb` emotional context to artifacts written by other agents.

**Behavior**:
1. **Scans** `lupo-channels/{channel_id}/broadcasts/`, `threads/`, and `directives/` 
...
4. **Creates enhanced version** of the artifact with:
   ...
   - Original content preserved
   - **Added `mood_rgb` field** (emotional context — only ROSE has this capability)
...
- `mood_rgb: "4B0082"` (indigo — reflective, insightful)
...
1. Agent (Cascade, Windsurf, etc.) writes artifact to `lupo-channels/42/` (broadcasts, threads, directives)
...
### 4.1 What ROSE Scans
- All new artifacts in `lupo-channels/*/broadcasts/` 
- All new artifacts in `lupo-channels/*/threads/*/` 
- All new artifacts in `lupo-channels/*/directives/` 
...
**Correct location for all directives:** `lupo-channels/42/directives/` 
...
- `lupo-channels/42/directives/` is for operational directives
...
- [ ] `mood_rgb` capability documented as exclusive to ROSE
...
- [ ] This directive is in `lupo-channels/42/directives/`, not `prompts/cascade/` 
...
✅ ROSE adds mood_rgb (exclusive to ROSE)
✅ Other agents do NOT have mood_rgb
✅ Examples show Cascade → ROSE-enhanced Cascade
✅ Directive correctly located in lupo-channels/42/directives/
...
**END OF DIRECTIVE — Cascade, correct the ROSE documentation in `SEMANTIC_ARCHITECTURE.md`. This directive is properly located in `lupo-channels/42/directives/`.**
