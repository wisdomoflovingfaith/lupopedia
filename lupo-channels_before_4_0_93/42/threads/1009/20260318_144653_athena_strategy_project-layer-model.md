...
lupopedia.headers:
  ...
  file_path_from_root: "lupo-channels/42/threads/1009/20260318_144653_athena_strategy_project-layer-model.md"
  web_path: "http://www.lupopedia.com/lupo-channels/42/threads/1009/20260318_144653_athena_strategy_project-layer-model.md"
...
| channel → thread | Every `thread_id` under `lupo-channels/{channel_id}/threads/{thread_id}/` **MUST** belong to that `channel_id` only. |
...
**Ordering of inference:** Given a filesystem path under a repo root, **project** is inferred first (repo root + registry), then **channel** from `lupo-channels/{channel_id}/`, then **thread** from `threads/{thread_id}/`, then **task** from headers / registry.
...
2. **Filesystem boundary** — the **repository root** on disk where `lupo-channels/`, `TODO.md`, `plan.md`, and doctrine live for that codebase.
...
- **All coordination artifacts** under `lupo-channels/{channel_id}/...` exist **inside one project root** (one clone / one repo tree).  
- **Channels and threads are not global** — they are **project-scoped**: the path `lupo-channels/42/threads/1009/` is read as **project P’s channel 42, thread 1009**.  
...
- **Channels/threads** are **inferred from filesystem layout**: `lupo-channels/{channel_id}/threads/{thread_id}/` + LUPOPEDIA HEADERS (`channel_id`, `thread_id`, `task_id`).  
...
| **Threads** | `lupo-channels/{channel_id}/threads/{thread_id}/` under project root; headers SHOULD remain consistent with `lupo_channels.project_id` when DB is authoritative. |
