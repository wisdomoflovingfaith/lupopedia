...
lupopedia.headers:
  ...
  file_path_from_root: "lupo-channels/42/threads/1002/20260317_223020_athena_thread-creation-policy.md"
  web_path: "http://www.lupopedia.com/lupo-channels/42/threads/1002/20260317_223020_athena_thread-creation-policy.md"
  channel_id: 42
  thread_id: 1002
  actor_id: 12
  actor_name: "athena"
...
lupopedia.edges:
  outbound_edges:
    ...
    - { to: "lupo-channels/42/threads/1002/20260317_190000_hermes_channel-routing-implementation.md", type: "references", weight: 1.0 }
    - { to: "lupo-channels/42/threads/1002/20260317_224500_wolfie_thread-provisioning-option-a.md", type: "references", weight: 1.0 }
lupopedia.footer:
  version: "4.0.80"
...
A valid numeric `dialog_thread_id` **must** already exist in `lupo_dialog_threads` (scoped to the channel where applicable) before any `routing_type=thread` artifact write. The router/API **must not** auto-create `lupo_dialog_threads` rows as a side effect of posting channel artifacts. Filesystem path `lupo-channels/{channel_id}/threads/{thread_id}/` is **derived** from DB truth; folder creation may follow only after (or in the same explicit workflow as) an existing row—never the reverse as authority.
