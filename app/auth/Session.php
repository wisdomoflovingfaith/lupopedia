...
class Session
{
     ...
     *
     * Strategy:
     * - Remove all non-alphanumeric characters (A-Z, a-z, 0-9)
     * - Truncate to 200 chars (prevents DoS from megabyte UAs)
     * - Convert to lowercase (case-insensitive matching; e.g. Mozilla vs mozilla)
     *
     * Why not only whitespace collapse?
     * - Version segments (Chrome/120.0.1 vs 120.0.2) would change the hash every bump
     * - Punctuation varies between browsers and releases
     * - Alphanumeric-only yields a stable family identity across minor updates
     *
     * Example:
     *   "Mozilla/5.0 (Windows NT 10.0)" → "mozilla50windowsnt100"
     *
     * @param string $ua Raw User Agent string
     * @return string Normalized alphanumeric string
     */
    public static function normalizeUserAgent($ua)
    {
        $ua = preg_replace('/[^A-Za-z0-9]/', '', (string) $ua);
        $ua = substr($ua, 0, 200);
     ...
     */
    /**
     * Compute session identity hash from Class C IP, user_id, and user agent.
     * CRITICAL: DO NOT use actor_id or auth_user_id in this base hash.
     *
     * @param string $ip
     * @param string $user_agent
     ...
     * @return string
     */
    public static function computeIdentityHash($ip, $user_agent, $user_id = null)
    {
        $network_prefix = self::ipNetworkPrefix($ip);
        $salt = self::sessionSalt();

        // Normalize user_id; use 'unknown' as deterministic placeholder if missing
        $user_id_input = (isset($user_id) && $user_id !== '' && $user_id !== null) ? (string)$user_id : 'unknown';

        // SESSION_HASH_DEBUG logging
        error_log("SESSION_HASH_DEBUG: class C IP used: " . $network_prefix);
        error_log("SESSION_HASH_DEBUG: user_id used: " . $user_id_input);
        error_log("SESSION_HASH_DEBUG: user agent used: " . $user_agent);

        $input_string = $network_prefix . '|' . $user_id_input . '|' . (string)$user_agent . '|' . $salt;
        error_log("SESSION_HASH_DEBUG: final input string before hashing: " . $input_string);

        $hash = hash('sha256', $input_string);
        error_log("SESSION_HASH_DEBUG: final hash result: " . $hash);
    ...
    public static function createEmbedSession($db, $actor_id)
    {
        ...
        error_log("SESSION_DEBUG: createEmbedSession entered for actor $actor_id");
        if (!($db instanceof \PDO_DB)) {
            error_log("SESSION_DEBUG: createEmbedSession FAILED - not a PDO_DB instance");
            return null;
        }
        $table_prefix = defined('LUPO_TABLE_PREFIX') ? LUPO_TABLE_PREFIX : 'lupo_';
        $table = $table_prefix . 'sessions';
        ...
        // CRITICAL: The base session identity hash MUST NOT include actor_id or auth_user_id.
        // It must be stable before and after login to identify the browser session fingerprint.
        ...
        $logMsg = "[" . date('Y-m-d H:i:s') . "] SESSION_CREATE: actor_id=$actor_id sid=" . substr($session_id, 0, 16) . "\n";
        error_log("SESSION_DEBUG: attempting INSERT into $table with session_id=" . substr($session_id, 0, 16) . "...");
        error_log("SESSION_DEBUG: About to INSERT into $table with data: " . json_encode($data));
        try {
            $res = $db->insert($table, $data);
            if ($res === false) {
                error_log("SESSION_DEBUG: DB insert returned FALSE for $table. Error: " . $db->getLastError());
                return null;
            } else {
                error_log("SESSION_DEBUG: DB insert SUCCESS for $table");
            }
            // TEMPORARY DEBUG: Log the inserted session_id and actor_id
            error_log("SESSION_DEBUG: Insert attempted for session_id=$session_id actor_id=$actor_id result=" . ($res === false ? 'FAIL' : 'OK'));
        } catch (\Exception $e) {
            error_log('SESSION_DEBUG: createEmbedSession EXCEPTION: ' . $e->getMessage());
            return null;
        }
        error_log("SESSION_DEBUG: createEmbedSession completed for session_id=$session_id");
        $s = new self($db, null);
    ...
    /**
     * Derive the canonical PHP session cookie path for this install.
     * Must match what bootstrap.php passes to session_set_cookie_params().
     * LUPOPEDIA_PUBLIC_PATH (e.g. '/lupopedia') is the authoritative source;
     * falls back to the current session cookie path when the constant is absent.
     *
     * @return string
     */
    private static function canonicalCookiePath()
    {
        if (defined('LUPOPEDIA_PUBLIC_PATH') && LUPOPEDIA_PUBLIC_PATH !== '') {
            return rtrim(LUPOPEDIA_PUBLIC_PATH, '/') . '/';
        }
     ...
     *
     * Cookie path is derived from LUPOPEDIA_PUBLIC_PATH so that the Set-Cookie
     * header always uses the same path that bootstrap.php configured.  Hardcoding
     * path='/' caused a duplicate-cookie conflict: the browser preferred the more-
     * specific /lupopedia/ anonymous cookie over the new /  authenticated one.
     *
     * @param \PDO_DB $db
     * @param int $actor_id
     * @return Session|null
    ...
    {
        error_log("SESSION_DEBUG: Session::create entered for actor $actor_id");
        $s = self::createEmbedSession($db, $actor_id);
        if (!$s) {
            error_log("SESSION_DEBUG: Session::create FAILED because createEmbedSession returned null");
            return null;
        }

        $cookie_path = self::canonicalCookiePath();
        $secure      = isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on';
        $domain      = isset($_SERVER['HTTP_HOST']) ? (string) $_SERVER['HTTP_HOST'] : '';

        // CRITICAL: Destroy any existing session before rotation.
        // Expire the old cookie at every path it may have been set to prevent the
        // browser from preferring a stale more-specific-path cookie over the new one.
        if (session_status() === PHP_SESSION_ACTIVE) {
            error_log("SESSION_DEBUG: Destroying existing PHP session before rotation");
            session_unset();
            $old_params = session_get_cookie_params();
            $old_path   = ($old_params['path'] !== '') ? $old_params['path'] : '/';
            setcookie(session_name(), '', time() - 3600, $old_path,   $domain, $secure, true);
            if ($old_path !== $cookie_path) {
                setcookie(session_name(), '', time() - 3600, $cookie_path, $domain, $secure, true);
            }
            if ($old_path !== '/' && $cookie_path !== '/') {
                setcookie(session_name(), '', time() - 3600, '/', $domain, $secure, true);
            }
            session_destroy();
        }

        // Set canonical cookie params BEFORE session_start; session_start() sends the
        // PHPSESSID cookie automatically using these params -- no extra setcookie() needed.
        ...
        error_log("SESSION_DEBUG: Starting new session with ID: " . substr($s->session_id, 0, 16) . "... path=" . $cookie_path);
    ...
    public function touch()
    {
        ...
        error_log("SESSION_DEBUG: touch() entered for session " . substr($this->session_id, 0, 16) . "...");
        $now = self::nowYmdhis();
        $res = $this->db->update(
            $this->table,
            array(
        ...
        if ($res === false) {
            error_log("SESSION_DEBUG: DB update returned FALSE in touch()");
        } else {
            error_log("SESSION_DEBUG: touch() SUCCESS");
        }
    ...
    public function validateSession($sessionId = null)
    {
        ...
        if (defined('LUPO_SESSION_VALIDATE_UA') && LUPO_SESSION_VALIDATE_UA) {
            ...
            if (!empty($loaded->ua_hash) && !hash_equals($loaded->ua_hash, $expected_ua_hash)) {
                // Try legacy raw user agent hash for backward compatibility
                $srv = self::untrustedServerArray();
                $raw_user_agent = isset($srv['HTTP_USER_AGENT']) ? (string) $srv['HTTP_USER_AGENT'] : '';
                $legacy_ua_hash = hash('sha256', $raw_user_agent . '|' . self::sessionSalt());
                if (!hash_equals($loaded->ua_hash, $legacy_ua_hash)) {
                    error_log("SESSION_DEBUG: validateSession failed - UA hash mismatch. Expected normalized: " . substr($expected_ua_hash, 0, 16) . "..., Stored: " . substr($loaded->ua_hash, 0, 16) . "...");
                    return false;
                }
            }
        }
