...
class AuthService
{
    ...
    public function getCurrentUser()
    {
        ...
        // Path 1: actor_auth_users mapping table (handles all actor types including system hybrids)
        $user = $this->db->fetchRow(
            "SELECT a.actor_id, aau.auth_user_id, au.username, au.display_name, au.email, au.is_active as user_is_active
             FROM {$a} a
             INNER JOIN {$prefix}actor_auth_users aau ON a.actor_id = aau.actor_id
             INNER JOIN {$au} au ON aau.auth_user_id = au.auth_user_id
             WHERE a.actor_id = :actor_id
               AND aau.status = 'active' AND (aau.is_deleted = 0 OR aau.is_deleted IS NULL)
               AND (a.is_deleted = 0 OR a.is_deleted IS NULL)
               AND (au.is_deleted = 0 OR au.is_deleted IS NULL)
             ORDER BY aau.is_primary DESC LIMIT 1",
            array('actor_id' => $actorId)
        );

        // Path 2: direct actor_source link (legacy fallback for user/lupo_auth_users actors)
        if (!$user) {
            $user = $this->db->fetchRow(
