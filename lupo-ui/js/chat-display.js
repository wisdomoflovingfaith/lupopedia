 ...
 *
 * API: GET/POST {LUPOPEDIA_PUBLIC_PATH}/api/lupo-channels/{id}/messages
 * See lupo-includes/modules/api/channels-api.php (format=json|buffer|image).
 */
...
ChatDisplay.prototype.messagesUrl = function (format) {
    ...
    var u = base + '/api/lupo-channels/' + this.channelId + '/messages?since=' + encodeURIComponent(this.lastSinceStr || '');
    if (this.threadId > 0) {
...
ChatDisplay.prototype.sendMessage = function (text) {
    ...
    var url = base + '/api/lupo-channels/' + this.channelId + '/messages';
