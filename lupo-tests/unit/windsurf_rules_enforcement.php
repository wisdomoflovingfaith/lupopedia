...
function test_windsurf_artifacts_exist()
{
    global $tests, $windsurfDir;
    
    $jsonFile = $windsurfDir . DIRECTORY_SEPARATOR . 'lupopedia_rules.json';
    $readmeFile = $windsurfDir . DIRECTORY_SEPARATOR . 'README.md';
    $rulesDir = $windsurfDir . DIRECTORY_SEPARATOR . 'rules';
    ...
    if (!$jsonPath || !file_exists($jsonPath)) {
        $tests['errors'][] = "Missing .windsurf/lupopedia_rules.json at path: " . $jsonFile;
        return false;
    }
    
    if (!$readmePath || !file_exists($readmePath)) {
        $tests['errors'][] = "Missing .windsurf/README.md at path: " . $readmeFile;
        return false;
    }
    
    if (!$rulesPath || !is_dir($rulesPath)) {
        $tests['errors'][] = "Missing .windsurf/rules/ directory at path: " . $rulesDir;
        return false;
    }
    
    test_result(true, "Windsurf artifacts exist and are accessible");
    return true;
}

function test_windsurf_json_parsable()
{
    global $tests, $windsurfDir;
    ...
    if (!$jsonPath || !file_exists($jsonPath)) {
        $tests['errors'][] = "Cannot access .windsurf/lupopedia_rules.json at path: " . $jsonFile;
        return false;
    }
    ...
    if ($json === null) {
        $tests['errors'][] = "Cannot parse .windsurf/lupopedia_rules.json";
        return false;
    }
    
    if (!isset($json['rules']) || !is_array($json['rules'])) {
        $tests['errors'][] = "Invalid structure in .windsurf/lupopedia_rules.json";
        return false;
    }
    
    test_result(true, "Windsurf JSON is parsable and valid");
    return true;
}
...
function test_no_duplicate_rule_ids()
{
    ...
    foreach ($json['rules'] as $rule) {
        if (!isset($rule['id'])) {
            $tests['errors'][] = "Rule missing 'id' field in .windsurf/lupopedia_rules.json";
            return false;
        }
        
        $ruleId = $rule['id'];
        if (isset($ruleIds[$ruleId])) {
            $tests['errors'][] = "Duplicate rule ID '$ruleId' found in .windsurf/lupopedia_rules.json";
            return false;
        }
        $ruleIds[$ruleId] = true;
    }
    
    test_result(true, "No duplicate rule IDs found in Windsurf rules");
    return true;
}

function test_rules_correspond_to_canonical()
{
    global $tests, $windsurfDir, $rootDir;
    
    $jsonFile = $windsurfDir . DIRECTORY_SEPARATOR . 'lupopedia_rules.json';
    $content = file_get_contents($jsonFile);
    $windsurfRules = json_decode($content, true);
    
    // Load canonical root rules
    ...
    foreach ($windsurfRules['rules'] as $rule) {
    ...
    if (!empty($missingCanonical)) {
        $tests['errors'][] = "Windsurf rules not found in canonical root: " . implode(', ', $missingCanonical);
        return false;
    }
    
    test_result(true, "All Windsurf rules correspond to canonical root rules");
    return true;
}
...
function test_rule_files_have_headers()
{
    ...
    test_result(true, "All Windsurf rule files have proper LUPOPEDIA HEADERS");
    return true;
}
...
function test_no_cross_target_contamination()
{
    ...
    // Check that Windsurf artifacts don't contain references to other IDE directories
    $rulesDir = $windsurfDir . DIRECTORY_SEPARATOR . 'rules';
    $files = glob($rulesDir . DIRECTORY_SEPARATOR . '*.md');
    ...
    foreach ($files as $file) {
        ...
        // Check for references to other IDE directories
    ...
    test_result(true, "No cross-target contamination in Windsurf artifacts");
    return true;
}

// Run all tests
echo "Windsurf Rules Enforcement Test\n";
echo "==============================\n\n";

test_windsurf_artifacts_exist();
test_windsurf_json_parsable();
