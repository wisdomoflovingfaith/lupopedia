...
$channels_dir = defined('LUPO_CHANNELS_DIR') ? LUPO_CHANNELS_DIR : 'lupo-channels';

$classes = array('Resolver', 'FlareParser', 'GuardEvaluator', 'DriftDetector', 'SyncService', 'HookExecutor', 'Renderer', 'ActorLookup');
foreach ($classes as $cls) {
    $f = dirname(__FILE__) . DIRECTORY_SEPARATOR . 'lupo-includes' . DIRECTORY_SEPARATOR . 'classes' . DIRECTORY_SEPARATOR . $cls . '.php';
    if (is_file($f)) {
        require_once $f;
    }
}
