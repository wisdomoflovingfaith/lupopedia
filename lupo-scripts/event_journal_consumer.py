...
def load_journal_entry(event_id):
    ...
    journal_file = f'lupo-database/journal/{event_id}.json'
...
def write_divergence_artifact(journal_entry, divergence_type, details=None):
    ...
    Location: lupo-database/divergences/{timestamp}_{divergence_type}.json
    """
    os.makedirs('lupo-database/divergences', exist_ok=True)
    
    timestamp = get_current_ymdhis()
    filename = f'lupo-database/divergences/{timestamp}_{divergence_type}.json'
...
def consumer_status():
    ...
    journal_dir = 'lupo-database/journal'
    divergence_dir = 'lupo-database/divergences'
