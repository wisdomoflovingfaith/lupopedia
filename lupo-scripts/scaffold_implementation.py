...
def get_implementations_base_path():
    script_dir = get_script_dir()
    return os.path.normpath(os.path.join(script_dir, "..", "lupo-docs", "implementations"))
...
def read_header_utc_from_file(path, max_bytes=8000):
    """Return 14-digit when_updated or last_modified_utc from first YAML block, or None."""
    try:
...
def run_add_status(argv):
    ...
    rel_prev = (
        "lupo-docs/implementations/{0}/status/{1}".format(impl_name, previous_base)
        if previous_base
        else None
    )

    file_path_from_root = "lupo-docs/implementations/{0}/status/{1}".format(impl_name, fname)
...
Dated status artifact created by **`lupo-scripts/scaffold_implementation.py add-status`**.
