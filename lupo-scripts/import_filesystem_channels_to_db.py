...
def import_channels(
    ...
    for ch in channels:
        ...
        created = parse_ymdhis(hdrs.get("last_modified_utc") or hdrs.get("created_ymdhis")) or now
...
INSERT IGNORE INTO {table_prefix}dialog_threads
  (dialog_thread_id, title, federation_node_id, channel_id,
...
def import_metadata(
    ...
    for art in artifacts:
        hdrs = art.headers
        actor_id = int(hdrs.get("actor_id") or SYSTEM_ACTOR_ID_DEFAULT)
        created = parse_ymdhis(hdrs.get("last_modified_utc")) or now
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    # DB connection
    db_group = p.add_argument_group("Database connection (not needed with --dry-run)")
    db_group.add_argument("--host", default=os.environ.get("MYSQL_HOST", "localhost"))
    db_group.add_argument("--port", default=os.environ.get("MYSQL_PORT", "3306"))
    db_group.add_argument("--user", default=os.environ.get("MYSQL_USER", ""))
    db_group.add_argument("--password", default=os.environ.get("MYSQL_PASSWORD", ""))
    db_group.add_argument("--database", default=os.environ.get("MYSQL_DATABASE", ""))
    db_group.add_argument("--table-prefix", default=os.environ.get("LUPO_TABLE_PREFIX", "lupo_"))
    return p
...
def main() -> int:
    ...
    if not args.dry_run and args.verbose:
        print(f"DB connection: {args.user}@{args.host}:{args.port}/{args.database}")
