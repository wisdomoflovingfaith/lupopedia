#!/usr/bin/env python3
"""
Repair malformed LUPOPEDIA headers in lupo-docs/doctrine/**/*.md so that
...
UTC timestamps: read from lupo-bin/temporal_anchor.json (same source as tick.py anchor).

Usage:
  python lupo-scripts/fix_doctrine_headers.py --dry-run
  python lupo-scripts/fix_doctrine_headers.py --apply
"""
...
ANCHOR = ROOT / "lupo-bin" / "temporal_anchor.json"
...
def load_anchor_utc():
    ...
    try:
        ...
        pass
    print(
        "ERROR: Run: python lupo-bin/tick.py  (then retry)",
        file=sys.stderr,
    )
    sys.exit(1)
...
def build_footer_block(utc, include_edges=True):
    ...
    fb.append('    - "Run: python lupo-scripts/apply_doctrine_prd_lineage.py --apply"')
    return "\n".join(fb)
