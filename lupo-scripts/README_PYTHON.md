...
lupopedia.headers:
  ...
  lupopedia.edges: []
  file_path_from_root: "lupo-scripts\README_PYTHON.md"
  file_hash: "14d10014240145f15f639bea3ccf90d3dd280523b5a8691453085e9eb2945779"
  file_path_from_root: "lupo-scripts\README_PYTHON.md"
  file_hash: "88382714fd9a7e1c36545337b7413b8fa8f2fbb194b81572c5e84b6a18de87be"
  ...
  artifact_type: "guide"
  artifact_kind: "documentation"
  purpose: "Python Migration Scripts"
  mood_rgb: "4169E1"
  traits: ["flare", "indexed", "v4.0.50"]
  tags: ["scripts", "readme_pythonmd"]
  lupo_agent: "windsurf"
...
```bash
# Dry run (no changes)
python lupo-scripts/migrate_filesystem_to_db.py --dry-run

# Migrate only agents
python lupo-scripts/migrate_filesystem_to_db.py --type=agents

# Migrate only channels
python lupo-scripts/migrate_filesystem_to_db.py --type=channels

# Migrate everything
python lupo-scripts/migrate_filesystem_to_db.py

# Migrate everything (explicit)
python lupo-scripts/migrate_filesystem_to_db.py --type=all
```
...
✓ Database connected
✓ Created upload directory: lupo-agents/
✓ Created upload directory: lupo-channels/
...
python lupo-scripts/cleanup_old_directories.py

# Force cleanup (no confirmation)
python lupo-scripts/cleanup_old_directories.py --force

# Cleanup only agents
python lupo-scripts/cleanup_old_directories.py --type=agents

# Cleanup only channels
python lupo-scripts/cleanup_old_directories.py --type=channels
```
...
```bash
ls -R lupo-uploads/agents/
ls -R lupo-uploads/channels/
```
...
### Permission Denied: lupo-uploads/

**Solution:**
```bash
chmod 755 lupo-uploads/
chown www-data:www-data lupo-uploads/  # Linux/Mac
```
...
python lupo-scripts/migrate_filesystem_to_db.py --type=agents --dry-run
...
- Ensure adequate disk space in lupo-uploads/ directory
...
**Documentation:** See `lupo-docs/doctrine/FILESYSTEM_MIGRATION_GUIDE.md`
