...
def _get_db_connection(table_prefix):
    """
    Obtain a database connection using project config.
    Tries lib.db_connection first (canonical), falls back to db_config.
    Returns (connection, table_prefix_str) or raises RuntimeError.
    """
    ...
    try:
        ...
        try:
            ...
            import db_config
            params = db_config.load_db_config() if hasattr(db_config, "load_db_config") else {
                "host": getattr(db_config, "DB_HOST", "localhost"),
                "user": getattr(db_config, "DB_USER", ""),
                "password": getattr(db_config, "DB_PASS", ""),
                "database": getattr(db_config, "DB_NAME", ""),
                "port": int(getattr(db_config, "DB_PORT", 3306)),
            }
    except Exception as exc:
        raise RuntimeError("Cannot load DB config: %s" % exc)
    ...
    conn = pymysql.connect(
        ...
        database=params.get("database", params.get("db", params.get("name", ""))),
