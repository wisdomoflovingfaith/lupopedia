...
#   file_path_from_root: "lupo-scripts/watcher_auto_draft.py"
...
Default behavior:
- Watches lupo-channels/{channel_id}/threads/{thread_id}/ for new/changed .md
- When an eligible file is detected (help_response + passes ATER001), emit a draft status
  artifact into lupo-channels/{channel_id}/threads/{out_thread_id}/.

Run (PowerShell):
  python lupo-scripts/watcher_auto_draft.py --repo-root . --channel 42 --watch-threads 1001,1002 --out-thread 1001
"""
...
def _help_response_ater001_errors(path: Path) -> list[str]:
    """
    Mirror lupo-scripts/validate_channel_artifacts.py help_response rules:
    - body >= 200 chars
...
def _emit_draft_status(
    *,
    ...
    now = _utc_now()
    ts = _ymdhis(now)
    out_dir = repo_root / "lupo-channels" / str(channel_id) / "threads" / str(out_thread_id)
    _safe_mkdir(out_dir)
...
def _iter_candidate_files(repo_root: Path, channel_id: int, thread_ids: list[str]) -> list[Path]:
    out: list[Path] = []
    base = repo_root / "lupo-channels" / str(channel_id) / "threads"
...
def main() -> int:
    ...
    ap.add_argument("--state-file", default="lupo-scripts/state/watcher_auto_draft_state.json")
