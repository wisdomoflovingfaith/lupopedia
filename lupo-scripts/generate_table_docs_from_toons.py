#!/usr/bin/env python3
# lupopedia.headers:
#   when_updated: "20260327235000"
#   file_path_from_root: "lupo-scripts/generate_table_docs_from_toons.py"
#   last_modified_utc: "20260327235000"
#   channel_id: 42
#   actor_id: 23
...
- Source of truth: schema from lupo-database/lupopedia/json/*.json.
- No DB interactions.
...
TOON_JSON_DIR = PROJECT_ROOT / "lupo-database" / "lupopedia" / "json"
ACTIVE_DIR = PROJECT_ROOT / "lupo-docs" / "database" / "lupopedia" / "tables" / "active"
CORRUPTION_MANIFEST = PROJECT_ROOT / "lupo-docs" / "reports" / "CORRUPTION_MANIFEST.md"
PHASE2_MANIFEST_OUT = PROJECT_ROOT / "lupo-docs" / "reports" / "PHASE2_REGENERATION_MANIFEST_20260327.md"
PHASE2_COMPLETION_OUT = PROJECT_ROOT / "lupo-docs" / "reports" / "Phase2_Completion_Report_20260327.md"
...
def build_synthetic_frontmatter(table_name, rel_file):
    ...
    return "\n".join([
        ...
        "  outbound_edges:",
        "  - to: \"lupo-database/lupopedia/json/%s.json\"" % table_name,
        "    type: \"references\"",
        "    weight: 1.0",
...
def normalize_recovered_frontmatter(frontmatter, table_name, rel_file):
    ...
    if re.search(r"^\s*last_modified_utc\s*:", fm, flags=re.M):
        fm = re.sub(r"^\s*last_modified_utc\s*:.*$", '  last_modified_utc: "%s"' % PHASE_TS, fm, flags=re.M)
    else:
        fm = fm.replace("lupopedia.headers:\n", "lupopedia.headers:\n  last_modified_utc: \"%s\"\n" % PHASE_TS)
    ...
    if re.search(r"^\s*namespace\s*:", fm, flags=re.M):
        fm = re.sub(r"^\s*namespace\s*:.*$", '  namespace: "%s"' % ns, fm, flags=re.M)
    else:
        fm = fm.replace("lupopedia.headers:\n", "lupopedia.headers:\n  namespace: \"%s\"\n" % ns)
    ...
    if "lupopedia.edges:" not in fm:
        ...
        fm += "  outbound_edges:\n"
        fm += "  - to: \"lupo-database/lupopedia/json/%s.json\"\n" % table_name
        fm += "    type: \"references\"\n"
        fm += "    weight: 1.0\n"
...
def build_body(table_name, toon):
    ...
    lines.append("- Source of truth: `lupo-database/lupopedia/json/` TOON exports")
...
def run_header_validator(file_paths):
    results = []
    validator = PROJECT_ROOT / "lupo-scripts" / "validate_lupopedia_headers.php"
    for p in file_paths:
...
def write_phase2_reports(manifest_rows, stats, header_results, schema_results, variance_note):
    ...
    out.append('  file_path_from_root: "lupo-docs/reports/PHASE2_REGENERATION_MANIFEST_20260327.md"')
    out.append('  last_modified_utc: "20260327234500"')
    ...
    rep.append('  file_path_from_root: "lupo-docs/reports/Phase2_Completion_Report_20260327.md"')
    rep.append('  last_modified_utc: "20260327234500"')
