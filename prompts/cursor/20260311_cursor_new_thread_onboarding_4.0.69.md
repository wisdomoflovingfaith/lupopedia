...
lupopedia.headers:
  ...
  file_path_from_root: "prompts/cursor/20260311_cursor_new_thread_onboarding_4.0.69.md"
...
| 3 | **.cursor/rules/** | Root rules applied in Cursor (synced from `lupo-rules/root/*.md`). Run `php scripts/sync_root_rules_to_cursor.php` after editing root rules. |
...
- **Sync to Cursor:** After editing any file in `lupo-rules/root/`, run `php scripts/sync_root_rules_to_cursor.php` so `.cursor/rules/*.mdc` stay in sync.
