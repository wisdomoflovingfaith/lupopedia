...
lupopedia.headers:
  ...
  file_path_from_root: "prompts/cursor/20260310_cursor_thread_onboarding_4.0.69.md"
...
| 3 | **.cursor/rules/** | Root rules applied in Cursor (synced from `lupo-rules/root/*.md`). All 16 .mdc files; run `php scripts/sync_root_rules_to_cursor.php` after editing root rules. |
...
**Schema source of truth:** `lupo-database/lupopedia/mysql/install/install_new_lupopedia.sql`. TOONs in `lupo-database/lupopedia/toon/` (or project TOON path); regenerate with `python scripts/generate_toon_files.py`.
...
| **Seeds** | `lupo-database/lupopedia/mysql/seed/` (registry, actors/agents, rules, skills, root rules, fallback rule, etc.) |
...
- **Sync root → Cursor:** `php scripts/sync_root_rules_to_cursor.php`
