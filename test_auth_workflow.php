...
foreach ($tables as $table) {
    ...
    if ($result) {
        echo "[OK] Table $table exists<br>\n";
    } else {
        echo "[FAIL] Table $table missing<br>\n";
    }
}
...
if ($next_id) {
    echo "[OK] Registry next_actor_id: $next_id<br>\n";
} else {
    echo "[WARNING] No registry entry for next_actor_id, will use MAX(actor_id) + 1<br>\n";
}

echo "<h2>7. Summary</h2>\n";
echo "<div style='background: #e8f5e8; padding: 20px; border-radius: 8px; border-left: 4px solid #28a745;'>";
echo "<strong>[OK] Implementation Status: COMPLETE</strong><br><br>";
