...
export async function findActorFaucet(actorId: number, channelId: number = 42): Promise<any | null> {
    ...
    const possiblePaths = [
        path.join(root, 'database', 'lupopedia', 'channels', 'channels', String(channelId), 'actors', String(actorId), 'faucets.json'),
        path.join(root, 'database', 'lupopedia', 'actors', 'faucets', 'by_actor.json') // Manifest
    ];
