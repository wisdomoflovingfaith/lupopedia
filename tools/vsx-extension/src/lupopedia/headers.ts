/**
 * LUPOPEDIA Header parser — reads YAML front-matter blocks from file text.
 *
 * A LUPOPEDIA Header (also known as LUPOPEDIA Header / CROP Header) is a YAML
 ...
 *   ---
 *
 * @module lupopedia/lupopedia
 */

export interface LupopediaHeader {
    ...
    // LUPOPEDIA HEADER Footer Fields
    ...
    /** Any extra keys found in the LUPOPEDIA HEADER block that weren't mapped */
 ...
 */
const CANONICAL_MAP: Record<string, keyof LupopediaHeader | string> = {
    // File Identity & Path
    'file_path_from_root': 'file_path_from_root',
    'x-file-path': 'file_path_from_root',
    'x-lupopedia-file-path': 'file_path_from_root',
    'lupopedia-file-path': 'file_path_from_root',

    // System & Versioning
    'file.last_modified_system_version': 'file_last_modified_system_version',
    'x-version': 'file_last_modified_system_version',
    'x-lupopedia-version': 'file_last_modified_system_version',
    'lupopedia-version': 'file_last_modified_system_version',

    // Temporal
    'file.last_modified_utc': 'file_last_modified_utc',
    'x-lupopedia-timestamp': 'file_last_modified_utc',

    // Routing & Identity
    'channel_id': 'channel_id',
    'x-channel': 'channel_id',
    'lupopedia-channel': 'channel_id',
    'federated_node_id': 'federated_node_id',
    'x-federated-node-id': 'federated_node_id',
    'x-lupopedia-federated-node-id': 'federated_node_id',
    'thread_id': 'thread_id',
    'x-thread': 'thread_id',
    'x-lupopedia-thread': 'thread_id',
    'actor_id': 'actor_id',
    'x-actor-from': 'actor_id',
    'x-lupo_actor-id': 'actor_id',
    'x-actor-identity': 'lupo_actor_identity',
    'actor_identity': 'lupo_actor_identity',
    'from': 'from',
    'x-from': 'from',
    'delegation_chain': 'delegation_chain',
    'x-delegation-chain': 'delegation_chain',
    'x-actor-to': 'lupo_actor_to',

    // Status & Doctrine
    'status': 'status',
    'x-status': 'status',
    'x-doctrine': 'lupo_doctrine',

    // Survivor Protocol
    'x-survivor-protocol': 'lupo_survivor_protocol',
    'x-forwarded-for': 'lupo_forwarded_for',
    'x-forward-chain': 'lupo_forward_chain',
    'x-origin-status': 'lupo_origin_status',
    'x-ban-reason': 'lupo_ban_reason',
    'x-ban-timestamp': 'lupo_ban_timestamp',
    'x-relay-validated-by': 'lupo_relay_validated_by',
    'x-collapse-ratio': 'lupo_collapse_ratio',
    'x-system-state': 'lupo_system_state',

    // Registry & Task
    'x-task': 'lupo_task',
    'x-registry-mode': 'lupo_registry_mode',
    'x-registry-source': 'lupo_registry_source',
    'x-toon-path': 'lupo_toon_path',
    'x-csv-path': 'lupo_csv_path',

    // Mood/Emotional
    'mood_vector': 'mood_vector',
    'x-mood-vector': 'mood_vector',

    // Verbose/Offline
    'x-timestamp': 'lupo_timestamp',
    'x-utc-timestamp': 'lupo_utc_timestamp',
    'x-location': 'lupo_location',
    'tags': 'tags',

    // Registry
    'x-registry-id': 'registry_id',
    'x-entity-type': 'entity_type',
    'x-entity-index-id': 'entity_index_id',
    'x-federation-node-id': 'federation_node_id',
    'x-registry-metadata': 'registry_metadata',

    // Content
    'x-content-id': 'content_id',
    'content_id': 'content_id',
    'x-content-parent-id': 'content_parent_id',
    'x-triage-status': 'triage_status',
    'triage_status': 'triage_status',
    'x-visibility': 'visibility',
    'visibility': 'visibility',
    'x-view-count': 'view_count',
    'x-share-count': 'share_count',
    'x-version-number': 'version_number',
    'x-seo-keywords': 'seo_keywords',

    // Collections
    'x-collection-id': 'collection_id',
    'x-collection-name': 'collection_name',
    'x-collection-slug': 'collection_slug',

    // Channels
    'x-channel-key': 'channel_key',
    'x-channel-type': 'channel_type',
    'x-channel-name': 'channel_name',

    // System
    'x-is-kernel': 'is_kernel',
    'x-is-active': 'is_active',
    'x-is-deleted': 'is_deleted'
};
...
function isValidDatabaseMapping(key: string): boolean {
    ...
    const afterPrefix = lowerKey.slice(8); // Remove 'x-'
...
/**
 * Parse a LUPOPEDIA Header/footer from raw file text.
 */
export function parseLupopediaHeader(text: string): HeaderParseResult {
    const errors: string[] = [];
    ...
    try {
        ...
        if (rawHeader === null) {
            return {
                ...
                header: null,
                raw: '',
                errors: ['No LUPOPEDIA Header block found (expected --- ... --- at the top).'],
            };
        }
        ...
        const mapped: Partial<LupopediaHeader> = { extras: {}, database_mapping: {} };
        ...
        for (const [key, value] of Object.entries(kv)) {
            ...
            if (!canonicalKey) {
                const cleanKey = lowerKey.replace(/^(x-|x-lupopedia-|lupopedia-|flp-)/, 'x-');
                canonicalKey = CANONICAL_MAP[cleanKey] as string;
            }

            if (canonicalKey) {
                const k = canonicalKey as keyof LupopediaHeader;
        ...
        // Validation
        if (!header.file_path_from_root) errors.push("Missing required LUPOPEDIA HEADER field: file_path_from_root");
        if (!header.file_last_modified_system_version) errors.push("Missing required LUPOPEDIA HEADER field: file_last_modified_system_version");
        if (!header.file_last_modified_utc) errors.push("Missing required LUPOPEDIA HEADER field: file_last_modified_utc");
        ...
        return {
            ...
            header: null,
            raw: '',
            errors: [`LUPOPEDIA HEADER Parser Panic: ${err instanceof Error ? err.message : String(err)}`],
        };
    }
}

/**
 * Format a LupopediaHeader back into a YAML front-matter string.
 * Uses X-Lupo-* canonical keys for maximum doctrine adherence.
 */
export function formatLupopediaHeader(header: LupopediaHeader): string {
    const lines = [
        '---',
    ...
    if (header.mood_vector) lines.push(`X-Lupo-Mood-Vector: ${header.mood_vector}`);
    ...
    for (const [k, v] of Object.entries(header.extras)) {
        if (!k.startsWith('X-Lupo-') && !k.startsWith('lupopedia.')) {
            lines.push(`${k}: ${v}`);
        }
    }
    lines.push('---');
