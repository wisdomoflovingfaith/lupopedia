// src/lupopedia/headers/logic/RepairService.ts
...
export class RepairService {
    ...
    public async normalizeMetadata(document: vscode.TextDocument): Promise<boolean> {
        ...
        if (data.lupopedia && data.lupopedia.headers) {
            if (data.lupopedia.headers.x_lupo_forwarded && !data.lupopedia.headers.delegation_chain) {
                data.lupopedia.headers.delegation_chain = data.lupopedia.headers.x_lupo_forwarded;
                delete data.lupopedia.headers.x_lupo_forwarded;
            }
    ...
    public async repairDelegationChain(document: vscode.TextDocument, actorId: number): Promise<boolean> {
        ...
        let chain = '';
        if (data.lupopedia && data.lupopedia.headers) {
            chain = data.lupopedia.headers.delegation_chain || '';
            const repaired = DelegationEngine.autoRepair(chain, actorId);
            data.lupopedia.headers.delegation_chain = repaired;
            data.lupopedia.headers.actor_id = actorId;
        } else if (data.identity) {
