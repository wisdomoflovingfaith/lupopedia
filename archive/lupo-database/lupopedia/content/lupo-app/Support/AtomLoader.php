...
class AtomLoader
{
    ...
    /** @var array|null */
    private static $globalConstantsAtomCache = null;
    ...
    {
        $atom_version = $this->getGlobalConstantsVersion();
    ...
    /**
     * Load version from global constants atom single source.
     *
     * @return string|null
     */
    private function getGlobalConstantsVersion()
    {
        if (self::$globalConstantsAtomCache === null) {
            self::$globalConstantsAtomCache = false;
            $base = dirname($this->configDir);
            $atom_path = $base . '/lupo-memory/atoms/lupopedia_global_constants.atom.toon';
            if (is_file($atom_path)) {
                $raw = @file_get_contents($atom_path);
                if ($raw !== false) {
                    $decoded = @json_decode($raw, true);
                    if (is_array($decoded)) {
                        self::$globalConstantsAtomCache = $decoded;
                    }
                }
            }
        }

        if (is_array(self::$globalConstantsAtomCache)) {
            $v = self::$globalConstantsAtomCache['constants']['versioning']['current_lupopedia_version'] ?? null;
            if (is_string($v) && $v !== '') {
                return $v;
            }
        }
