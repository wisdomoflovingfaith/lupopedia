...
Review scope: All tables in `lupo-database/lupopedia/mysql/install/install_new_lupopedia.sql`
Constitutional reference: `lupo-rules/root/LUPOPEDIA_CONSTITUTIONAL_ROOT_RULES.md`
...
The system_prompt for an agent should be a **filesystem path reference**, not an inline text blob. Agent system prompts live in `lupo-agents/{slug}/system_prompt.txt`. The database should store the path, not the content. This avoids sync drift between file and database.
