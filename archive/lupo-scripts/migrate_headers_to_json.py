#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# DRAFT — lupo-scripts/migrate_headers_to_json.py
# Scan .md files with lupopedia.headers YAML blocks,
# extract edges/tags/footer, and write companion JSON files
# to lupo-docs/headers/{filename_without_ext}.json
#
# Usage:
#   python lupo-scripts/migrate_headers_to_json.py --dir lupo-docs/prd
#   python lupo-scripts/migrate_headers_to_json.py --dir lupo-docs/prd --dry-run
#   python lupo-scripts/migrate_headers_to_json.py --file lupo-docs/prd/00_root_constitutional_system_requirements.md
#   python lupo-scripts/migrate_headers_to_json.py --dir lupo-docs/prd --force
...
HEADERS_DIR = PROJECT_ROOT / "lupo-docs" / "headers"
MEMORY_BASE = "lupo-memory/2026/04"
...
def build_header_json(md_path: Path, parsed: dict, ts: str) -> dict:
    ...
    year = date_part[:4]
    month = date_part[4:6]
    memory_ref = f"lupo-memory/{year}/{month}/M-{toon_slug}-{date_part}.toon"
...
def build_toon(md_path: Path, companion: dict, ts: str) -> tuple:
    ...
    toon_path = PROJECT_ROOT / "lupo-memory" / year / month / f"{toon_id}.toon"
