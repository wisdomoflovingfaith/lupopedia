...
#   file_path_from_root: "lupo-scripts/consolidate_lupo_changelog_pending.py"
#   web_path: "https://www.lupopedia.com/lupopedia/lupo-scripts/consolidate_lupo_changelog_pending.py"
#   status: "active"
#   when_updated: "20260417212800"
#   trust_tier: "canonical"
#   questions_toon: null
#   memory_toon: "lupo-memory/development/canonical/1026/04/consolidate-lupo-changelog-pending.toon"
#   atoms_toon: null
#   transcript_jsonl: "0/development/consolidate-lupo-changelog-pending"
...
#   content_slug: "consolidate-lupo-changelog-pending"
#   default_collection_id: null
#   lupopedia.schema: documentation
#   title: "Consolidate lupo-changelog-pending JSON into version changelog.md"
#   summary: "Reads CHANGELOG_BUFFER_ARCHITECTURE JSON files from lupo-changelog-pending, appends Entry blocks to a target changelog, archives merged files."
# ---------------------------------------------------------------------
"""
consolidate_lupo_changelog_pending.py

Merges mandatory JSON buffer entries (see lupo-docs/doctrine/CHANGELOG_BUFFER_ARCHITECTURE.md)
from lupo-changelog-pending/ into a version changelog Markdown file (default:
lupo-docs/versions/4.1.3/changelog.md).

Dedup: each merged fragment embeds <!-- lupo-changelog-merged: FILENAME -->; re-runs skip
...
Usage:
  python lupo-scripts/consolidate_lupo_changelog_pending.py
  python lupo-scripts/consolidate_lupo_changelog_pending.py --commit
  python lupo-scripts/consolidate_lupo_changelog_pending.py --target lupo-docs/versions/4.1.3/changelog.md --commit
"""
...
MERGED_COMMENT_RE = re.compile(r"<!--\s*lupo-changelog-merged:\s*([^>]+?)\s*-->")
...
def read_anchor_utc(repo):
    anchor = os.path.join(repo, "lupo-bin", "temporal_anchor.json")
...
def changelog_stub(target_rel, when_updated):
    """Initial body for lupo-docs/versions/4.1.3/changelog.md when created by this tool."""
    return """---
lupopedia.headers:
  header_format_version: "4.1.3"
...
def already_merged(target_text, filename):
    marker = "<!-- lupo-changelog-merged: %s -->" % filename
...
def format_entry(row):
    lines = []
    lines.append("## Entry")
    lines.append("<!-- lupo-changelog-merged: %s -->" % row["filename"])
...
def ensure_archive_directory(archive_abs, repo_root):
    """
    CHANGELOG_BUFFER_ARCHITECTURE expects a directory at lupo-changelog-archive/.
...
def main():
    parser = argparse.ArgumentParser(
        description="Merge lupo-changelog-pending/*.json into a version changelog.md"
    )
    parser.add_argument(
        "--target",
        default="lupo-docs/versions/4.1.3/changelog.md",
        help="Repo-relative path to changelog Markdown (default: 4.1.3)",
    )
    parser.add_argument(
        "--pending-dir",
        default="lupo-changelog-pending",
        help="Repo-relative pending JSON directory",
    )
    parser.add_argument(
        "--archive-dir",
        default="lupo-changelog-archive",
    ...
    if not new_utc:
        print("[ERROR] Run python lupo-bin/tick.py first (temporal_anchor.json missing or invalid).")
        sys.exit(1)
