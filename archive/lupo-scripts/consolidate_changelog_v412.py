...
def main():
    repo = os.getcwd()
    pending_dir = os.path.join(repo, "lupo-changelog-pending")
    archive_dir = os.path.join(repo, "lupo-changelog-archive")
    target_changelog = os.path.join(repo, "lupo-docs/versions/4.1.2/CHANGELOG.md")
    target_oq = os.path.join(repo, "lupo-docs/versions/4.1.2/status/open_questions.md")
    ...
    with open(target_changelog, 'a', encoding='utf-8') as f:
        for entry in merged_entries:
            ...
            for fn in entry['filenames']:
                f.write(f"<!-- lupo-changelog-merged: {fn} -->\n")
