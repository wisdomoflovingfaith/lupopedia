...
#   file_path_from_root: "lupo-scripts/export_channel_snapshots.py"
#   web_path: "https://www.lupopedia.com/lupopedia/lupo-scripts/export_channel_snapshots.py"
#   status: "deprecated"
#   when_updated: "20260412011607"
#   trust_tier: "canonical"
#   questions_toon: null
#   memory_toon: "lupo-memory/development/canonical/1026/04/export-channel-snapshots-deprecated.toon"
#   atoms_toon: null
...
1. **Memory graph** — query **`lupo_memory_nodes`** / **`lupo_memory_edges`** (or IDE/PHP
   tools that read them). CLI helper example: **`lupo-scripts/export_memory_nodes_116.py`**
   (facet-scoped export; not a full graph dump).
2. **PHP DB export** — **`php lupo-bin/export.php`** (**`MemoryExportService`**) for
   canonical node JSON mirrors from **`lupo_memory_nodes`**.
3. **Direct SQL** — parameterized queries against **`lupo_channels`**, **`lupo_dialog_*`**,
   **`lupo_contents`**, etc., via **PDO_DB** (runtime) or **`lupo-scripts/db_config`** +
   **pymysql** (tooling), with **`LUPO_TABLE_PREFIX`**.

**Archived implementation (unsupported):**
``lupo-scripts/legacy/export_channel_snapshots_archived_20260324.py`` — run only if you
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "Use DB, memory graph, or php lupo-bin/export.php. See module docstring."
        )
    )
    parser.add_argument(
        "--legacy-archive-path",
    ...
    if args.legacy_archive_path:
        print("lupo-scripts/legacy/export_channel_snapshots_archived_20260324.py")
        return 0
    ...
    sys.stderr.write(
        ...
        "  - php lupo-bin/export.php --node-id N or --full\n"
        "  - Parameterized DB access via PDO_DB / db_config + pymysql\n\n"
        "Archived copy (unsupported): "
        "lupo-scripts/legacy/export_channel_snapshots_archived_20260324.py\n"
        "  python lupo-scripts/export_channel_snapshots.py --legacy-archive-path\n\n"
