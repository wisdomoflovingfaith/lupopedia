...
#   file_path_from_root: "lupo-scripts/generate_prd_index.py"
#   web_path: "https://www.lupopedia.com/lupopedia/lupo-scripts/generate_prd_index.py"
#   status: "complete"
#   when_updated: "20260422160000"   # ← will be updated on next run
#   trust_tier: "canonical"
#   questions_toon: null
#   memory_toon: "lupo-memory/development/canonical/1026/04/generate-prd-index.toon"
#   atoms_toon: null
...
#   title: "Regenerate PRD_INDEX.md from lupo-docs/prd"
...
def scan_prds() -> list[dict]:
    ...
    for md_file in sorted(PRD_DIR.glob("*.md")):
        ...
        # Extract PRD number from filename
        m = re.match(r"^(\d{2})_", md_file.name)
        if not m:
            m = re.match(r"^PRD_(\d{2})_", md_file.name)
    ...
    return sorted(prds, key=lambda p: (p["prefix"], p["artifact_kind"] != "specification", p["filename"].lower()))
...
"""
    ...
    body = [
        "# PRD Index",
        f"*Auto-generated by `lupo-scripts/generate_prd_index.py` -- {ts}*",
    ...
    for prd in prds:
        num = f"{prd['prefix']:02d}"
        kind = prd["artifact_kind"]
        title = prd["title"].replace("|", "\\|")
        body.append(f"| {num} | `{prd['filename']}` | {kind} | {title} |")
