...
#   file_path_from_root: "lupo-scripts/migrate_legacy_docs.py"
#   questions_toon: null
#   channel_id: 42
#   actor_id: 102
#   actor_name: "cursor"
...
"""
Phase 2: Migrate legacy docs in lupo-docs/ to add canonical Synthesized Documentation headers.

Scans Markdown files under lupo-docs/ (or given path), adds a YAML header block if missing.
...
Usage:
  python scripts/migrate_legacy_docs.py [--dry-run] [--path lupo-docs/]
"""
...
def main():
    parser = argparse.ArgumentParser(description="Add canonical headers to legacy Markdown in lupo-docs/")
    parser.add_argument("--dry-run", action="store_true", help="Do not write files")
    parser.add_argument("--path", default=None, help="Root path to scan (default: lupo-docs/ under project root)")
    args = parser.parse_args()
    root = args.path
    if not root:
        root = os.path.join(SCRIPT_DIR, "..", "lupo-docs")
