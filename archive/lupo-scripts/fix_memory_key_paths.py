...
#   file_path_from_root: "lupo-scripts/fix_memory_key_paths.py"
#   web_path: "https://www.lupopedia.com/lupopedia/lupo-scripts/fix_memory_key_paths.py"
#   status: "in_progress"
#   when_updated: "20260416185134"
#   trust_tier: "staging"
#   questions_toon: null
#   memory_toon: "lupo-memory/development/staging/2026/04/fix-memory-key-paths.toon"
#   atoms_toon: null
...
Correct formula (PRD 16 §5.2 + §8.1):
  lupo-memory/{channel_key}/{trust_tier}/{year_display}/{MM}/{filename}.toon
...
Usage:
  python lupo-scripts/fix_memory_key_paths.py --dry-run
  python lupo-scripts/fix_memory_key_paths.py --dry-run --dir lupo-docs/prd
  python lupo-scripts/fix_memory_key_paths.py            (live — writes files)
  python lupo-scripts/fix_memory_key_paths.py lupo-docs/prd/16_lupopedia_headers.md
"""
...
_MK_LINE_RE = re.compile(r'^(\s{2}(?:memory_toon|memory_key):\s+)"?(lupo-memory/[^"\s]+\.toon)"?(.*)$')
...
def compute_correct_memory_key(fields: dict, existing_mk: str) -> tuple[str | None, str | None]:
    ...
    # Expected: ['lupo-memory', seg1, seg2, year, MM, filename]
    if len(parts) != 6 or parts[0] != 'lupo-memory' or not parts[5].endswith('.toon'):
    ...
    correct = f'lupo-memory/{channel_key}/{trust_tier}/{display_year}/{month_mm}/{filename}'
    return correct, None
...
def main():
    ...
    ap.add_argument('--dir', default='lupo-docs/prd',
                    help='Directory to scan for *.md files (default: lupo-docs/prd)')
