...
DEFAULT_KEYWORD_TARGETS = {
    "dynapi": "lupo-docs/doctrine/DYNAPI_DOCTRINE.md",
    "stoned wolfie": "lupo-docs/doctrine/ADVERSARIAL_TEST_IDENTITY_DOCTRINE.md",
    "two-ui": "lupo-docs/doctrine/MOBILE_SEPARATION_DOCTRINE.md",
    "two ui": "lupo-docs/doctrine/MOBILE_SEPARATION_DOCTRINE.md",
    "mobile separation": "lupo-docs/doctrine/MOBILE_SEPARATION_DOCTRINE.md",
    "utc timestamp": "lupo-docs/doctrine/TIMESTAMP_DOCTRINE.md",
    "tick.py": "lupo-docs/doctrine/TICK_PY_DOCTRINE.md",
    "foreign keys": "lupo-docs/doctrine/DATABASE_DOCTRINE.md",
    "no foreign key": "lupo-docs/doctrine/DATABASE_DOCTRINE.md",
    "soft delete": "lupo-docs/doctrine/DATABASE_DOCTRINE.md",
    "silent harvest": "lupo-docs/doctrine/SILENT_HARVEST_DOCTRINE.md",
    "reverse engineering": "lupo-docs/doctrine/REVERSE_ENGINEERING_DOCTRINE.md",
    "self hacking": "lupo-docs/doctrine/SELF_HACKING_DOCTRINE.md",
...
PRD_PATH_RE = re.compile(
    r"lupo-docs/prd/(\d{1,3})_[^\s\])\"']+\.md", re.IGNORECASE
)
INLINE_PRD_PATH = re.compile(
    r"(lupo-docs/prd/\d{1,3}_[^\s\]\)\"\'>]+\.md)", re.IGNORECASE
)
CODE_PATH_RE = re.compile(
    r"\b((?:lupo-includes|lupo-scripts|app|lupo-agents|lupo-ui|lupo-bin)/[a-zA-Z0-9_./-]+\.(?:php|js|css|py))\b"
)
DIR_PATH_RE = re.compile(
    r"\b((?:lupo-includes|lupo-scripts|craftysyntax-reference)/[a-zA-Z0-9_./-]+/?)\b"
)
...
def resolve_prd_files(prd_num):
    ...
    prd_dir = os.path.join(ROOT, "lupo-docs", "prd")
    ...
    try:
        for name in os.listdir(prd_dir):
            if name.startswith(prefix) and name.endswith(".md"):
                found.append(_posix(os.path.join("lupo-docs", "prd", name)))
    except OSError:
        pass
...
def find_prd_refs(content):
    ...
    for m in INLINE_PRD_PATH.finditer(content):
        rel = m.group(1).strip()
        if _exists_rel(rel):
            out.append((rel, 0.97, "Inline lupo-docs/prd path"))
    return out
...
def main():
    ...
    ap.add_argument(
        "--headers",
        action="store_true",
        help="Match ## headings against other docs (slower; indexes lupo-docs/)",
    )
    ap.add_argument(
        "--keyword-file",
        metavar="YAML",
    ...
    header_index = None
    if args.headers:
        print("Building header index for lupo-docs/ ...", file=sys.stderr)
        header_index = build_header_index(["lupo-docs"])
