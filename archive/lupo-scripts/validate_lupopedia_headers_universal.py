...
#   file_path_from_root: "lupo-scripts/validate_lupopedia_headers_universal.py"
#   web_path: "https://www.lupopedia.com/lupopedia/lupo-scripts/validate_lupopedia_headers_universal.py"
#   status: "active"
#   when_updated: "20260422232349"
#   trust_tier: "canonical"
#   questions_toon: null
#   memory_toon: "lupo-memory/development/canonical/1026/04/validate-lupopedia-headers-universal.toon"
#   atoms_toon: null
...
# ---------------------------------------------------------------------
"""
PRD 16 v4.1.4 — §4.2 / §4.3 universal validator for Markdown and comment-embedded LUPOPEDIA HEADERS (Python, PHP, JS).

Enforced in-tree today (non-exhaustive): v4.1.4 header_format_version family;
required **22-key** dense grid (PRD 16 v4.1.4); trust_tier; thread_id rules; transcript_jsonl (≥3 segments,
...
LEGACY_FIELD_ALIASES = {
    "prd_id": "content_id",
    "prd_slug": "REMOVED_content_slug",      # REMOVED in v4.1.4
    "parent_prd": "content_parent_id",
    "pk_id": "content_id", 
    "pk_slug": "REMOVED_content_slug",       # REMOVED in v4.1.4
...
def _try_extract_v3_md_inner_yaml_block(content: str):
    """
    If content matches PRD 16 v4.1.4 Markdown envelope (25 lines, closing --- on line 25),
    return the inner YAML string (lines 2–24: lupopedia.headers: + 22 keys) for safe_load.
    ...
    if lines[0].strip() != "---" or lines[1].strip() != "lupopedia.headers:":
        return None
    # Dense v4.1.4: lines 3–24 are keys (indices 2–23), line 25 = ---
    dense_ok = lines[24].strip() == "---"
    if dense_ok:
        for i in range(2, 24):
...
def validate_markdown_mechanical_key_line_order(lines, file_path, legacy_20):
    """Mechanical key order on physical lines (canonical names; prd_* and pk_* accepted on legacy rows).

    v4.1.1: pk_* fields (pk_id, pk_slug, parent_pk_id) are accepted at any position with
    a HDR_PK_LEGACY_ALIAS warning, to support gradual migration from v4.1.0 to v4.1.1.
    """
    # Legacy 20-field support removed - always use current 22-field model
    expected_canon = list(V4_HEADER_KEYS_ORDERED)
    idxs = list(range(2, 24))
    for pos, i in enumerate(idxs):
        ...
        if ck == expected_canon[pos]:
            continue
        # v4.1.1: accept pk_* legacy keys at any position; emit migration warning
        if rk in LEGACY_FIELD_ALIASES:
            canonical_name = LEGACY_FIELD_ALIASES[rk]
            print(
                "[WARN] %s: line %d: legacy %r field found. Migrate to %s. (HDR_PK_LEGACY_ALIAS)"
                % (file_path, i + 1, rk, canonical_name)
            )
            continue
        print(
            "[ERROR] %s: line %d: key %r (canonical %r), expected %r (HDR_KEY_ORDER)"
            % (file_path, i + 1, rk, ck, expected_canon[pos])
        )
        return False
    return True
...
def validate_markdown_header_line_count(
    ...
    """
    PRD 16 v4.0.99 (transitional) §4.3 — Markdown: lines 1-25 are the header envelope; content starts line 26.
      line 1:  ---
      line 2:  lupopedia.headers:
      lines 3-24: 22 key/value lines (dense v4.0.99) OR legacy 20 keys + blank 23-24
      line 25: ---
      line 26+: body

    Legacy (v4.0.0–98): blank lines 23-24 before closing --- — WARN (HDR_LEGACY_ENVELOPE),
    unless ``reject_legacy_envelope`` (then ERROR). If ``header_format_version`` ≥ 4.0.99,
    legacy blanks 23–24 are ERROR (HDR_HEADER_INTERNAL_BLANK).
    """
    if not content.startswith("---\n"):
        return True
    ...
    if len(lines) < 25:
        print(
            "[ERROR] %s: header envelope must include at least 25 lines (HDR_LINE_COUNT), got %s"
            % (file_path, len(lines))
        )
    ...
    if lines[24].strip() != "---":
        print(
            "[ERROR] %s: line 25 must be closing --- (HDR_MISSING_CLOSE)"
            % (file_path,)
        )
        _print_md_envelope_missing_v3_keys_hint(lines, file_path)
        return False
    ...
    legacy_20 = (not lines[22].strip() and not lines[23].strip())
    ...
    if legacy_20:
        ...
        if _hf_tuple_requires_dense_v4099(hf_t):
            print(
                "[ERROR] %s: header_format_version >= 4.0.99 requires dense 22-key lines 3-24 "
                "(no blank lines 23-24) (HDR_HEADER_INTERNAL_BLANK)"
                % (file_path,)
            )
            _print_md_envelope_missing_v3_keys_hint(lines, file_path)
            return False
        ...
        for i in range(2, 24):
            if not lines[i].strip():
                print(
                    "[ERROR] %s: blank line inside key block lines 3-24 (HDR_HEADER_INTERNAL_BLANK)"
                    % (file_path,)
                )
                _print_md_envelope_missing_v3_keys_hint(lines, file_path)
                return False
    ...
    if len(lines) < 26:
        if development_mode:
            ...
            return True
        print(
            "[ERROR] %s: no body after header — file must extend past line 25 (HDR_EMPTY_BODY)"
            % (file_path,)
        )
        return False
    if not lines[25].strip():
        if development_mode:
            print(
                "[WARN]  %s: line 26 empty or whitespace-only (HDR_EMPTY_BODY) — allowed under --development"
                % (file_path,)
            )
            return True
        print(
            "[ERROR] %s: line 26 must start body content (no blank-only line after closing ---) (HDR_EMPTY_BODY)"
            % (file_path,)
        )
        return False
...
def _python_body_offset_after_header(lines, has_shebang):
    """Byte offset in normalized \\n content where code body starts (first line after 25-line header)."""
    # Universal: 25 header lines; with shebang, line 1 is shebang then lines 2-26 are the same layout as MD lines 1-25.
    if has_shebang:
        n_before_body = 26
    else:
        n_before_body = 25
...
def validate_python_mechanical_key_line_order(header_slice, file_path, py_legacy_20):
    ...
    for pos, i in enumerate(inner_idx):
        ...
        if rk in LEGACY_FIELD_ALIASES:
            canonical_name = LEGACY_FIELD_ALIASES[rk]
            print(
                "[WARN] %s: python header key %r found. Migrate to %s. (HDR_PK_LEGACY_ALIAS)"
                % (file_path, rk, canonical_name)
            )
            continue
        print(
            "[ERROR] %s: python header key %r (canonical %r), expected %r (HDR_KEY_ORDER)"
            % (file_path, rk, ck, expected_canon[pos])
        )
        return False
    return True
...
def _validate_hash_comment_25line_slice(
    ...
    """
    Shared 25-line ``#`` comment grid (open sep, ``lupopedia.headers:`` + keys, close sep).
    ...
    if not PY_HEADER_SEP_LINE_RE.match(header_slice[0]) or not PY_HEADER_SEP_LINE_RE.match(
        ...
        print(
            "[ERROR] %s: %s header must start (line 1 of block) and end (line 25) with # --- separator lines (HDR_PYTHON_HEADER)"
            % (file_path, label)
        )
        _print_py_envelope_missing_v3_keys_hint(header_slice, file_path)
        return False, None

    py_legacy_20 = not header_slice[22].strip() and not header_slice[23].strip()
    ...
    if py_legacy_20:
        ...
        if _hf_tuple_requires_dense_v4099(hf_t):
            print(
                "[ERROR] %s: header_format_version >= 4.0.99 requires dense 22-key %s header "
        ...
        if reject_legacy_envelope:
            print(
                "[ERROR] %s: legacy %s header envelope rejected (HDR_LEGACY_ENVELOPE)"
                % (file_path, label)
            )
            return False, None
        if not suppress_legacy_envelope_warn:
            print(
                "[WARN]  %s: legacy %s header (blank lines before closing # ---); "
                "re-add headers with add_lupopedia_header_to_file.py for dense v4.0.99 (HDR_LEGACY_ENVELOPE)"
                % (file_path, label)
            )
    ...
    if not validate_python_mechanical_key_line_order(header_slice, file_path, py_legacy_20):
        return False, None

    want = 21 if py_legacy_20 else 23
    if len(inner) != want:
        print(
...
def validate_python_header_envelope(
    ...
    """
    Enforce contiguous python comment header: optional shebang, then 25-line # block
    (open sep, lupopedia + 22 keys dense, close sep) or legacy (20 keys + two blank lines).
    ...
    if has_shebang:
        ...
        if len(lines) < 26:
            print(
                "[ERROR] %s: Python header with shebang needs >= 26 lines before code (HDR_PYTHON_LINE_COUNT), got %s"
                % (file_path, len(lines))
            )
            if len(lines) >= 2:
                _print_py_envelope_missing_v3_keys_hint(lines[1:26] if len(lines) > 25 else lines[1:], file_path)
            return False, None, None
        header_slice = lines[1:26]
    else:
        if len(lines) < 25:
            print(
                "[ERROR] %s: Python header without shebang needs >= 25 lines before code (HDR_PYTHON_LINE_COUNT), got %s"
                % (file_path, len(lines))
            )
            if len(lines) >= 2:
                _print_py_envelope_missing_v3_keys_hint(lines[1:25] if len(lines) > 24 else lines[1:], file_path)
            return False, None, None
...
def validate_star_mechanical_key_line_order(header_slice, file_path, star_legacy_20, err_tag):
    ...
    for pos, i in enumerate(inner_idx):
        ...
        if rk in LEGACY_FIELD_ALIASES:
            canonical_name = LEGACY_FIELD_ALIASES[rk]
            print(
                "[WARN] %s: star header key %r found. Migrate to %s. (HDR_PK_LEGACY_ALIAS)"
                % (file_path, rk, canonical_name)
            )
            continue
        print(
            "[ERROR] %s: star header key %r (canonical %r), expected %r (HDR_KEY_ORDER)"
            % (file_path, rk, ck, expected_canon[pos])
        )
        return False
    return True
...
def validate_removed_fields(hdr, file_path, strict_mode=False):
    """Validate that removed slug fields are not present in current headers.
    
    For PRD 16 v4.1.4 headers: ALWAYS fail on removed slug fields (no warnings).
    For legacy headers: warn about removed fields in non-strict mode, fail in strict mode.
    """
    removed_fields = ['content_slug', 'pk_slug', 'prd_slug']
    legacy_slug_mappings = {
        'pk_slug': 'REMOVED_content_slug',
        'prd_slug': 'REMOVED_content_slug'
    }
    
    # Check if this is a current v4.1.4 header
    header_version = str(hdr.get("header_format_version", ""))
    is_current_header = header_version.startswith("4.1.4")
    
    for field in removed_fields:
        if field in hdr:
            if is_current_header or strict_mode:
                # ALWAYS fail for current headers or strict mode
                print(f"[ERROR] {file_path}: removed field '{field}' is not allowed in PRD 16 v4.1.4 (HDR_REMOVED_FIELD_CONTENT_SLUG)")
                if field in legacy_slug_mappings:
                    print(f"[ERROR] {file_path}: legacy removed slug field '{field}' maps to '{legacy_slug_mappings[field]}' and is forbidden in current header contract")
                if is_current_header:
                    print(f"[ERROR] {file_path}: v4.1.4 headers cannot contain removed fields - no exceptions allowed")
                return False
            else:
                # Only warn for legacy headers in non-strict mode
                print(f"[WARN] {file_path}: removed field '{field}' found - should be removed for v4.1.4 compliance (HDR_REMOVED_FIELD_CONTENT_SLUG)")
                if field in legacy_slug_mappings:
                    print(f"[WARN] {file_path}: legacy slug field '{field}' maps to removed content_slug")
...
def _is_valid_header_format_version(version_value):
    """
    Version policy (PRD 16 §15): accept any 4.1.x patch string or 4.2.0 upgrade gate.
    Legacy '3' is accepted for backward compatibility only.

    In-family migration rules (e.g. pk_* vs patch level) are enforced separately —
    see check_version_compliance() / validate_legacy_pk_alias_vs_claimed_version() (PRD 16 §11 / §15.4).

    Rejects 4.0.x and all other families (HDR_VERSION_FAMILY / deprecated family).
    4.2.0 is the first Lupopedia → Lupopedia upgradable version (future gate).
    """
    if isinstance(version_value, int):
        return version_value == 3
    if isinstance(version_value, str):
        s = version_value.strip()
        if s == "3":
            return True
        # Any 4.1.x patch is valid (PRD 16 §15: all changes are patches before 4.2.0)
        if s.startswith("4.1."):
            patch = s[4:]
            return bool(re.match(r"^\d+$", patch))
        # 4.2.0 is the upgrade gate (first upgradable version)
        if s == "4.2.0":
            return True
        return False
    return False
...
def _resolve_canonical_field_order(hdr, file_path):
    ...
    try:
        ...
        if len(order_norm) != 22 or frozenset(order_norm) != V3_HEADER_KEYS:
            print(
...
def validate_field_ordering(hdr, file_path):
    """PRD 16 §4.2 — exact key set and order for v3 header family (HDR_KEY_ORDER / HDR_EXTRA_KEY).

    Note: v3 refers to header family naming, not version 3 - this validates v4.1.4 canonical order.
    v4.1.1: if atoms_toon is set and resolves to a valid atom, load expected order from
    atom; else fallback to V3_HEADER_KEYS_ORDERED (v4.1.4 canonical).
    """
    if not isinstance(hdr, dict):
        return True
    ...
    expected = _resolve_canonical_field_order(hdr, file_path)
    ...
    if actual_set != expected_set:
        ...
        if extra:
            print(
                "[ERROR] %s: extra keys under lupopedia.headers (HDR_EXTRA_KEY): %s"
                % (file_path, ", ".join(extra))
            )
        if missing:
            print(
                "[ERROR] %s: missing required v3 keys (HDR_MISSING_KEY): %s"
                % (file_path, ", ".join(missing))
            )
        return False

    print("[ERROR] %s: keys out of §4.2 canonical order (HDR_KEY_ORDER)" % (file_path,))
...
def validate_ascii_safe_names(hdr, file_path):
    ...
    if file_path_val:
        ...
        if not ASCII_FILENAME_PATTERN.match(filename.lower()):
            errors.append(f"filename '{filename}' contains non-ASCII or invalid characters")
...
def validate_content_id(hdr, file_path, header_format_version=None):
    ...
    if "content_id" not in hdr:
        print(f"[ERROR] {file_path}: content_id field is missing (HDR_CONTENT_ID_MISSING)")
        return False
...
def _prd_parent_resolves_on_disk(repo_root, parent_str):
    """
    True if parent_str references a normative PRD under lupo-docs/prd/ either by
    filename prefix ({n}_*.md) or by matching lupopedia.headers.content_id in a
    root-level PRD .md (after import_content write-back).
    """
    prd_pattern = os.path.join(repo_root, "lupo-docs", "prd", parent_str + "_*.md")
    if glob.glob(prd_pattern):
        return True
    prd_dir = os.path.join(repo_root, "lupo-docs", "prd")
...
def validate_memory_key_path_shape(hdr, file_path, strict_memory_year=False):
    """
    PRD 16 §5.2 / §10.1 — memory_toon must start with lupo-memory/{channel_key}/{trust_tier}/…
    ...
    parts = mk.split("/")
    if len(parts) < 4 or parts[0] != "lupo-memory":
        print(
            "[ERROR] %s: memory_toon must start with "
            "lupo-memory/{channel_key}/{trust_tier}/… (HDR_MEMORY_KEY)"
            % (file_path,)
        )
        return False
    if parts[1] != ck:
...
def validate_required_header_fields(hdr, file_path):
    ...
    if not _is_valid_header_format_version(version):
        print(
            "[ERROR] %s: header_format_version must be 4.1.x (any patch) or '3' (legacy); "
            "got %r — see PRD 16 §15 version policy (HDR_VERSION_FAMILY)"
            % (file_path, version)
        )
        return False
    ...
    for key in REQUIRED_HEADER_KEYS:
        if key not in hdr:
            if key == "content_id":
                print("[ERROR] %s: content_id field is missing (HDR_CONTENT_ID_MISSING)" % (file_path,))
            else:
        ...
        if key == "transcript_jsonl":
            ...
            continue
        if key == "content_id":
            # NULL is valid and means "not imported yet".
            continue
        if key == "content_parent_id":
            # NULL is valid; when set must be integer.
            if val is None:
                continue
            s = str(val).strip()
            if not s:
                print(
                    "[ERROR] %s: content_parent_id cannot be empty string; use null or integer "
                    "(HDR_CONTENT_PARENT_ID_INVALID)"
                    % (file_path,)
                )
                return False
            if not s.isdigit():
                print(
                    "[ERROR] %s: content_parent_id must be null or integer "
                    "(HDR_CONTENT_PARENT_ID_INVALID), got %r"
                    % (file_path, val)
                )
                return False
            continue
        if key == "default_collection_id":
            # NULL is valid; optional collection reference (v4.1.1).
            if val is None:
                continue
            s = str(val).strip()
            if not s:
                print(
                    "[ERROR] %s: default_collection_id cannot be empty string; use null or integer "
                    "(HDR_DEFAULT_COLLECTION_INVALID)"
                    % (file_path,)
                )
                return False
            if not s.isdigit():
                print(
                    "[ERROR] %s: default_collection_id must be null or integer "
                    "(HDR_DEFAULT_COLLECTION_INVALID), got %r"
                    % (file_path, val)
                )
                return False
            continue
...
def validate_thread_id_format(thread_id, file_path, artifact_type=None):
    """PRD 16 §4.2.1 — thread_id rules by artifact_type."""
    at = str(artifact_type or "").strip()
    if at == "prd":
        if thread_id is not None and str(thread_id).strip() != "":
            print(
                "[ERROR] %s: artifact_type prd requires thread_id empty '' (§4.2.1), got %r"
                % (file_path, thread_id)
            )
            return False
        return True
    if at == "discussion":
        if thread_id is None or str(thread_id).strip() == "":
            print("[ERROR] %s: discussion requires non-empty thread_id" % (file_path,))
            return False
        tid = str(thread_id).strip()
        if not THREAD_ID_PATTERN.match(tid):
            print(
                "[ERROR] %s: thread_id must match ^[a-z0-9][a-z0-9-]*$ (got %r)"
                % (file_path, tid)
            )
            return False
        return True
    ...
    if not THREAD_ID_PATTERN.match(tid):
        print(
            "[ERROR] %s: thread_id must be '' or ^[a-z0-9][a-z0-9-]*$ (got %r)"
            % (file_path, tid)
        )
        return False
    return True
...
def validate_federation_node_id(file_path, fed_node):
    """Core repo paths under lupo-docs/ and lupo-rules/ must not claim external federation_node_id >= 2."""
    if file_path.startswith("lupo-docs/") or file_path.startswith("lupo-rules/"):
        if fed_node >= 2:
            print(
                "[ERROR] %s: federation_node_id must be 0 or 1 for lupo-docs/ and lupo-rules/ paths, got %s"
                % (file_path, fed_node)
            )
            return False
...
def validate_memory_key_toon_file_exists(
    ...
    if os.path.isfile(toon_abs):
        return True
    rel_mk = mk.replace("\\", "/")
    hint = "python lupo-scripts/generate_memory_from_header.py %s" % (
        file_path.replace("\\", "/"),
    )
    msg_tail = (
...
def validate_memory_key_json_master_pair(
    ...
    if os.path.isfile(json_abs):
        return True
    hint = (
        "python lupo-scripts/json_to_toon.py --json %r --toon %r"
...
def _validate_lupopedia_headers_payload(
    ...
    if not validate_v3_scalar_header_values(hdr, file_path):
        return False, None
    ...
    if not validate_thread_id_format(
        hdr.get("thread_id"), file_path, hdr.get("artifact_type")
    ):
        return False, None
...
def validate_php_file(
    ...
    if _PHP_LEGACY_INLINE_YAML_RE.search((content or "")[:8000]):
        print(
            ...
            "and ``python lupo-scripts/add_lupopedia_header_to_file.py``. (HDR_PHP_LEGACY_INLINE_V3)"
            % (file_path,)
        )
        return False, None
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Examples:\n"
            "  python lupo-scripts/validate_lupopedia_headers_universal.py lupo-docs/prd/16_lupopedia_headers.md\n"
            "  python lupo-scripts/validate_lupopedia_headers_universal.py lupo-scripts/add_lupopedia_header_to_file.py\n"
            "  python lupo-scripts/validate_lupopedia_headers_universal.py file.md --check-links --quiet\n"
            "  python lupo-scripts/validate_lupopedia_headers_universal.py lupo-docs/prd/16_lupopedia_headers.md --strict-memory-year\n"
            "  python lupo-scripts/validate_lupopedia_headers_universal.py path/to/file.md --strict-memory-files\n"
        ),
    )
    parser.add_argument(
        "file_path",
    ...
    parser.add_argument(
        "--version",
        action="version",
        version="validate_lupopedia_headers_universal.py (PRD 16 / 4.1.4 envelope)",
    )
    ...
    if not args.quiet:
        print(
            "validate_lupopedia_headers_universal.py - PRD 16 / header_format_version 4.1.x patch family"
        )
