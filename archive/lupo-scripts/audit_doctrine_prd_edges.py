...
def scan_file(path):
    ...
    content_prd_refs = sorted(
        set(
            int(m.group(1))
            for m in re.finditer(r"(?:lupo-docs/)?prd/(\d+)[^\s\"')]*\.md", head)
        )
    )
...
def load_apply_module():
    ...
    spec = importlib.util.spec_from_file_location(
        ...
        ROOT / "lupo-scripts" / "apply_doctrine_prd_lineage.py",
    )
...
def load_fix_classify():
    spec = importlib.util.spec_from_file_location(
        "fix_doctrine_headers", ROOT / "lupo-scripts" / "fix_doctrine_headers.py"
    )
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod
