#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# DRAFT — lupo-scripts/generate_json_headers.py
# Companion to generate_prd_shorthands.py.
# Reads PRD .md files and/or .toon memory files, generates:
#   1. JSON header companions -> lupo-docs/headers/{stem}.json
#   2. Memory .toon files -> lupo-memory/YYYY/MM/M-{slug}-{date}.toon
#
...
#
# Usage:
#   python lupo-scripts/generate_json_headers.py --all
#   python lupo-scripts/generate_json_headers.py --prd 0
#   python lupo-scripts/generate_json_headers.py --toon lupo-memory/2026/04/M-example.toon
#   python lupo-scripts/generate_json_headers.py --all --force
#   python lupo-scripts/generate_json_headers.py --check
...
def build_header_companion(md_path: Path, data: dict, ts: str) -> dict:
    ...
    date_part = ts[:8]
    year, month = date_part[:4], date_part[4:6]
    memory_ref = f"lupo-memory/{year}/{month}/M-{toon_slug(stem)}-{date_part}.toon"
