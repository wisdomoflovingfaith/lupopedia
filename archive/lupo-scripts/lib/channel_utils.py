...
#   file_path_from_root: "lupo-scripts/lib/channel_utils.py"
#   web_path: "https://www.lupopedia.com/lupopedia/lupo-scripts/lib/channel_utils.py"
#   status: "complete"
#   when_updated: "20260417113505"   # ← update this timestamp when saving
#   trust_tier: "canonical"
#   questions_toon: null
#   memory_toon: "lupo-memory/development/canonical/1026/04/channel-utils.toon"
#   atoms_toon: null
...
def derive_channel_from_lupo_memory_path(path_like: str) -> Optional[str]:
    ...
    Derive channel_key from Type A path:
      lupo-memory/{channel_key}/{trust_tier}/...
    ...
    if not path:
        return None
    parts = path.split("/")
    if len(parts) < 3 or parts[0] != "lupo-memory":
        return None
...
def resolve_channel_key_for_artifact(
    ...
    Resolve channel_key using strict doctrine order:
      ...
      2. Derive from memory_toon or file_path_from_root (lupo-memory/{channel_key}/...)
      3. Unresolved (None)
