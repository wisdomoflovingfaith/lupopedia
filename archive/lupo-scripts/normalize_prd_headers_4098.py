...
def build_header(path, prd_id, slug, title, timestamp, parsed):
    ...
    if trust_tier == "seed":
        memory_toon_default = f"lupo-memory/development/seed/1026/04/{slug}.toon"
    else:
        memory_toon_default = f"lupo-memory/development/canonical/1026/04/{slug}.toon"
...
def write_sidecar(path, prd_id, slug, timestamp, body):
    ...
    sidecar_path = f"lupo-memory/headers/prd/{year}/{month}/{stem}.metadata.json"
