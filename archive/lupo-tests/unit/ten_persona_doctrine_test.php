<?php
/**
 * Ten Persona Doctrine Validation Test
 * 
 * Validates that MULTI_AGENT_COORDINATION_DOCTRINE.md has been updated
 * to use the 10 Primary Coordination Personas instead of the old 4-persona model
 */

echo "=== Ten Persona Doctrine Validation Test ===\n";
...
// Test 2: All 10 personas listed in purpose section
$total++;
...
if ($foundPersonas === 10) {
    $passed++;
    $tests[] = "PASS: All 10 personas found in doctrine";
} else {
    $tests[] = "FAIL: Only {$foundPersonas}/10 personas found";
}

// Test 3: Persona table has 10 entries
$total++;
$tableLines = explode("\n", $doctrine);
$personaTableRows = 0;
$inPersonaTable = false;
foreach ($tableLines as $line) {
    if (strpos($line, '| Persona | Role | Responsibility |') !== false) {
        $inPersonaTable = true;
        continue;
    }
    if ($inPersonaTable && strpos($line, '|---') !== false) {
        continue;
    }
    if ($inPersonaTable && strpos($line, '| **') !== false) {
        $personaTableRows++;
    }
    if ($inPersonaTable && trim($line) === '' && $personaTableRows > 0) {
        break;
    }
}
if ($personaTableRows === 10) {
    $passed++;
    $tests[] = "PASS: Persona table has 10 entries";
} else {
    $tests[] = "FAIL: Persona table has {$personaTableRows} entries (expected 10)";
}

// Test 4: LEXA is included as Security Enforcement
$total++;
...
// Test 5: Role boundaries table updated for 10 personas
$total++;
$roleBoundaryRows = 0;
$inRoleBoundaries = false;
foreach ($tableLines as $line) {
    if (strpos($line, '| Persona | CAN | CANNOT |') !== false) {
        $inRoleBoundaries = true;
        continue;
    }
    if ($inRoleBoundaries && strpos($line, '|---') !== false) {
        continue;
    }
    if ($inRoleBoundaries && strpos($line, '| WOLFIE |') !== false) {
        $roleBoundaryRows++;
    }
    if ($inRoleBoundaries && trim($line) === '' && $roleBoundaryRows > 0) {
        break;
    }
}
if ($roleBoundaryRows === 10) {
    $passed++;
    $tests[] = "PASS: Role boundaries table has 10 entries";
} else {
    $tests[] = "FAIL: Role boundaries table has {$roleBoundaryRows} entries (expected 10)";
}

// Test 6: New artifact types defined
$total++;
$expectedArtifacts = [
    'LEXA_ENFORCEMENT_*',
    'HEIMDALL_SECURITY_*',
...
// Test 7: Execution flow updated for 10-persona model
$total++;
...
// Test 8: No old 4-persona references remain
$total++;
...
// Test 9: Primary Coordination Personas section updated
$total++;
if (strpos($doctrine, '### Primary Coordination Personas') !== false &&
...
// Test 10: Agent actor IDs are correct
$total++;
$expectedIds = [
    'WOLFIE (actor_id 1)',
    'LEXA (actor_id 24)',
...
if ($foundIds >= 8) {
    $passed++;
    $tests[] = "PASS: Actor IDs correct ({$foundIds}/10)";
} else {
    $tests[] = "FAIL: Too few correct actor IDs ({$foundIds}/10)";
}
