...
python lupo-scripts/validate_lupopedia_headers_universal.py --reject-legacy-fields $(git diff --cached --name-only --diff-filter=ACMR | grep -E '\.(md|py|php|js)$' | head -20)
HEADER_EXIT_CODE=$?
...
if [ $HEADER_EXIT_CODE -ne 0 ] || [ $IMPL_EXIT_CODE -ne 0 ]; then
    ...
    echo "Run 'python lupo-scripts/validate_lupopedia_headers_universal.py --reject-legacy-fields' for manual validation"
    exit 1
fi
