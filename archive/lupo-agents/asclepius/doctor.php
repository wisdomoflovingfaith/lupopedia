...
$base_path = dirname(__DIR__, 2); // lupo-agents/ -> lupopedia/
...
class AsclepiusHealthCheck
{
    ...
    {
        $registry_path = LUPOPEDIA_PATH . '/lupo-database/lupopedia/actors/actor_id/registry.json';
        ...
        $agent_dir = LUPOPEDIA_PATH . '/lupo-agents/asclepius';
        if (!is_dir($agent_dir)) {
            $this->output('FAIL', 'Agent directory not found: ' . $agent_dir);
            return;
        }
        $this->output('OK', 'Agent directory exists: lupo-agents/asclepius/');
    }
