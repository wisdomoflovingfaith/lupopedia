...
- **system_prompt.md** or **system_prompt.txt** - System prompt and operational guidance (canonical LILITH uses **`system_prompt.md`** under **`lupo-agents/lilith/`**; legacy agents may still use `.txt`)
...
### 1. Create Directory
```bash
mkdir lupo-agents/my-new-agent
```

### 2. Copy Template
```bash
cp -r lupo-agents/_TEMPLATE/* lupo-agents/my-new-agent/
```
...
- Edit `system_prompt.md` or `system_prompt.txt` with operational guidance (LILITH: `lupo-agents/lilith/system_prompt.md`)
