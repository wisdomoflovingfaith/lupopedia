...
#   file_path_from_root: "lupo-templates/admin/api_dashboard.php"
#   web_path: "https://www.lupopedia.com/lupopedia/lupo-templates/admin/api_dashboard.php"
#   status: "in_progress"
#   when_updated: "20260418031245"
#   trust_tier: "canonical"
#   questions_toon: null
#   memory_toon: "lupo-memory/development/canonical/1026/04/api-dashboard-php.toon"
#   atoms_toon: null
 ...
 * and LupoLocale/lupo-i18n if the caller uses lupo_t(). This file uses plain English only.
...
if (!$has_provider_rows) {
    echo '<p class="lupo-api-dash-notice">' . htmlspecialchars($base_case_text, ENT_QUOTES, 'UTF-8') . '</p>';
} elseif (!$service_ok) {
    echo '<p class="lupo-api-dash-notice">' . htmlspecialchars($service_unavailable_text, ENT_QUOTES, 'UTF-8') . '</p>';
}

echo '<p class="lupo-api-dash-summary">';
...
echo '<table class="lupo-table"><thead><tr>';
...
foreach ($raw_providers as $pkey => $raw_cfg) {
    ...
    $status_class = 'lupo-status-green';
    $status_text = 'OK';
    if (!$has_key) {
        $status_class = 'lupo-status-muted';
        $status_text = $offline_status;
    } else {
        if ($remaining < 2.0) {
            $status_class = 'lupo-status-red';
            $status_text = 'Critical';
        } elseif ($remaining < 5.0) {
            $status_class = 'lupo-status-yellow';
            $status_text = 'Low';
        }
    }
