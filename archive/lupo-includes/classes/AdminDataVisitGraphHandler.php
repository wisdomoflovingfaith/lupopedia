...
class AdminDataVisitGraphHandler
{
    ...
    public static function renderDocument($db, $prefix, $base, $pathUrl, $year, $month, $actorId)
    {
        ...
        foreach ($series as $pt) {
            ...
            $rowsHtml .= '<td class="lupo-visit-graph-cell" valign="bottom">'
                . '<div class="lupo-visit-graph-num">' . (int) $cnt . '</div>'
                . '<div class="lupo-visit-graph-bar" style="height:' . $h . 'px"></div>'
                . '</td>';
        }

        $daysHtml = '';
        foreach ($series as $pt) {
            $daysHtml .= '<td class="lupo-visit-graph-day">' . sprintf('%02d', (int) $pt['day']) . '</td>';
        }
        ...
        return '<!DOCTYPE html><html lang="en"><head><meta charset="utf-8" />'
            ...
            . '.lupo-visit-graph-url{font-size:0.9rem;word-break:break-all;margin-bottom:16px;color:#4e342e;}'
            . 'table.lupo-visit-graph{border-collapse:collapse;background:#fff;border:1px solid #bcaaa4;}'
            . 'td.lupo-visit-graph-cell{text-align:center;padding:4px 3px 0;vertical-align:bottom;}'
            . '.lupo-visit-graph-num{font-size:0.75rem;margin-bottom:2px;}'
            . '.lupo-visit-graph-bar{width:12px;margin:0 auto;background:#6d4c41;border-radius:2px 2px 0 0;min-height:0;}'
            . 'td.lupo-visit-graph-day{text-align:center;font-size:0.75rem;padding:4px 3px;border-top:1px solid #d7ccc8;}'
            . '.lupo-visit-graph-caption{margin-top:12px;font-size:0.85rem;color:#5d4037;}'
            . 'a{color:#4e342e;}</style></head><body>'
            . '<h1>' . htmlspecialchars($title, ENT_QUOTES, 'UTF-8') . '</h1>'
            . '<div class="lupo-visit-graph-url" title="' . $fullEsc . '">' . $escUrl . '</div>'
            . '<p class="lupo-visit-graph-caption">'
            . htmlspecialchars(sprintf(lupo_t('admin.data.graph_month_caption', '%04d-%02d UTC — lupo_visits'), $year, $month), ENT_QUOTES, 'UTF-8')
            . '</p>'
            . '<table class="lupo-visit-graph" role="presentation"><tr>' . $rowsHtml . '</tr><tr>' . $daysHtml . '</tr></table>'
