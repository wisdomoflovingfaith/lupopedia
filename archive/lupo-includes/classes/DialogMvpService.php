...
#   file_path_from_root: "lupo-includes/classes/DialogMvpService.php"
#   web_path: "https://www.lupopedia.com/lupopedia/lupo-includes/classes/DialogMvpService.php"
#   status: "active"
#   when_updated: "20260418163036"
#   trust_tier: "canonical"
#   questions_toon: null
#   memory_toon: "lupo-memory/development/canonical/1026/04/18_channel_chat_display.toon"
#   atoms_toon: null
 ...
 *  - Schema source: lupo-database/lupopedia/json/*.json — reference ONLY.
 *  - Nulls:         Pass NULL for nullable columns; never 0 as a null substitute.
 *  - mood_vector:      lupo-docs/doctrine/COUNTING_IN_LIGHT_DOCTRINE.md — neutral 666666 for all actors
 ...
 *  - Survivability: lupo-docs/doctrine/SURVIVABILITY_DOCTRINE.md — Pillar 1 fallbacks (nowYmdHis, generateId,
 *                   json_encode) emit logDefect breadcrumbs; never hide silent DB failures where logged.
 */
class DialogMvpService
{
    // Actor IDs with elevated read access (registry: lupo-database/lupopedia/actors/registry.json)
    ...
    private static function generateId()
    {
        if (!class_exists('IdGenerator', false) && defined('LUPOPEDIA_PATH')) {
            $path = LUPOPEDIA_PATH . '/lupo-includes/classes/IdGenerator.php';
            if (file_exists($path)) {
                require_once $path;
            }
        }
    ...
    // Primary sink: lupo-changelog-pending/*.json. Fallback: error_log(JSON) per Survivability Pillar 1.
    // ---------------------------------------------------------------

    /**
     * Defect breadcrumb: Pillar 1 (survivability / fallback) and Pillar 2 (recurrence / learning transfer).
     * Sink: lupo-changelog-pending/*.json; fallback error_log per SURVIVABILITY_DOCTRINE.md.
     ...
     * @return bool true if JSON file written under lupo-changelog-pending/
     */
    public static function logDefect($pattern_id, $context = array())
    {
        ...
        $dir = LUPOPEDIA_PATH . DIRECTORY_SEPARATOR . 'lupo-changelog-pending';
        if (!is_dir($dir)) {
            @mkdir($dir, 0755, true);
        }
     ...
     * - mood_vector: lupo-docs/doctrine/COUNTING_IN_LIGHT_DOCTRINE.md — use neutral 666666 for all actors
    ...
    public static function createDialogThread($db, $channel_id, $title, $created_by_actor_id)
    {
        ...
        $ins = $db->insert($t_threads, array(
            ...
            // PRD 82 transcript path: lupo-memory/transcripts/{federation_node_id}/... — keep same node id on dialog_threads rows, transcript_jsonl path triples, and Hermes routing context (default local = 0).
            'federation_node_id'     => 0,
            'channel_id'             => (int) $channel_id,
            'project_slug'           => '',
            'task_name'              => '',
