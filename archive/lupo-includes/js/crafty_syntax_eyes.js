...
function init() {
    ...
    var semMode = "";
    try {
        var elCtr = document.querySelector(".resources-middle-center[data-lupo-edge-focus]");
        if (elCtr && elCtr.getAttribute) {
            semMode = elCtr.getAttribute("data-lupo-edge-focus") || "";
        }
        if (!semMode) {
            var elAt = document.querySelector(".resources-middle-center[data-lupo-artifact-type]");
            if (elAt && elAt.getAttribute) {
                var ax = elAt.getAttribute("data-lupo-artifact-type") || "";
                if (ax === "help_guide") {
                    semMode = "tutorial";
   ...
   document.tempeyes.src=blank.src;
	...
	// DynLayer global from lupo-layers.js (LupoLayer; DynLayer-compatible move/slide API)
