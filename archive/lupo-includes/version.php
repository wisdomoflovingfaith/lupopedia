 ...
 * @version 4.1.4
 * 
 * @note VERSION DOCTRINE: Runtime version resolves from
 *       lupo-memory/atoms/lupopedia_global_constants.atom.toon
 *       (constants.versioning.current_lupopedia_version), with
 *       config/global_atoms.yaml as legacy mirror fallback.
 *       Constants are defined from the resolved atom value, ensuring a single
 *       source of truth. See
...
if (function_exists('get_lupopedia_version')) {
    ...
    // Bootstrap fallback: read global constants atom directly first.
    $atom_path = dirname(__DIR__) . '/lupo-memory/atoms/lupopedia_global_constants.atom.toon';
    if (file_exists($atom_path)) {
        $atom_raw = @file_get_contents($atom_path);
        if ($atom_raw !== false) {
            $atom_data = @json_decode($atom_raw, true);
            if (is_array($atom_data)) {
                $version_from_atom = isset($atom_data['constants']['versioning']['current_lupopedia_version'])
                    ? $atom_data['constants']['versioning']['current_lupopedia_version']
                    : null;
            }
        }
    }
    if ($version_from_atom === null) {
        // Legacy mirror fallback via global atoms YAML.
...
// Fallback to neutral sentinel if atom loader fails.
$current_version = $version_from_atom !== null ? $version_from_atom : '0.0.0';
 ...
 * 
 * Loaded from global constants atom:
 * constants.versioning.current_lupopedia_version
 * (with GLOBAL_CURRENT_LUPOPEDIA_VERSION legacy mirror fallback)
 * 
 * @var string
 */
 ...
 * 
 * Loaded from global constants atom (same as LUPOPEDIA_VERSION)
 * 
 * @var string
 */
...
if (!defined('LUPOPEDIA_VERSION_DATE')) {
    define('LUPOPEDIA_VERSION_DATE', 20260423050000);
}
...
function lupopedia_get_version()
{
    ...
    return defined('LUPOPEDIA_VERSION') ? LUPOPEDIA_VERSION : '0.0.0';
}
...
{
    return function_exists('lupopedia_get_version') ? lupopedia_get_version() : (defined('LUPOPEDIA_VERSION') ? LUPOPEDIA_VERSION : '0.0.0');
}
