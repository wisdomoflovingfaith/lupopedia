<?php
/**
 * Try2 collections chrome: .lupo-dropdown / .dropdown-panel / .floating-submenu.
...
$lupo_try2_ctl = defined('LUPOPEDIA_PATH') ? (LUPOPEDIA_PATH . '/lupo-includes/functions/collection-tabs-loader.php') : '';
...
<div class="saved-collections-container debug-try2-inner">
    <div class="lupo-dropdown lupo-master-collections-wrap" id="lupo-try2-master" data-qa-type="collections">
        <button type="button"
                class="dropdown-button btn-blue"
                onclick="lupoDbgNavToggle(this, event)"
                aria-expanded="false"
                aria-haspopup="true"
                aria-controls="lupo-try2-master-panel"
        ...
        <div class="dropdown-panel" id="lupo-try2-master-panel" role="menu"<?php if (!empty($lupo_master_nav_groups)) { ?> data-lupo-master-nav-server="1" data-lupo-hydrated="1"<?php } ?>>
            <?php
    ...
    if (!empty($collectionsData) && is_array($collectionsData)) {
        foreach ($collectionsData as $dbg_type => $dbg_collection_type_data) {
            ...
            ?>
            <div class="lupo-dropdown" data-qa-type="<?php echo htmlspecialchars($dbg_type, ENT_QUOTES, 'UTF-8'); ?>">
    ...
    <div id="collection-tabs-container">
        ...
        if ($collection_id !== null && !empty($tabs_data) && is_array($tabs_data)) {
            foreach ($tabs_data as $main_tab => $sub_tabs) {
                ...
                ?>
                <div class="lupo-dropdown" data-qa-type="<?php echo htmlspecialchars($qa_type, ENT_QUOTES, 'UTF-8'); ?>">
                    <button type="button" class="dropdown-button btn-green"
                            onclick="lupoDbgNavToggle(this, event)"
                            aria-expanded="false"
                            aria-haspopup="true"
