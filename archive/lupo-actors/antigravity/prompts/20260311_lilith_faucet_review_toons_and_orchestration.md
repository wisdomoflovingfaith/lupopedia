...
lupopedia.headers:
  ...
  file_path_from_root: "lupo-prompts/antigravity/20260311_lilith_faucet_review_toons_and_orchestration.md"
  ...
  purpose: "Directive for Antigravity IDE to operate as Lilith (actor_id 2) faucet: review orchestration doc and all TOONs, produce Lilith suggestions on lupo-database/channels/semantic organisation."
...
lupopedia.edges:
  outbound_edges:
    - { to: "lupo-docs/status/ORCHESTRATION_ACTORS_AND_SUPPORTING_ACTORS_REVIEW_4.0.69.md", type: "references", weight: 1.0 }
...
- Read **`lupo-docs/status/ORCHESTRATION_ACTORS_AND_SUPPORTING_ACTORS_REVIEW_4.0.69.md`** in full.
...
- TOON files are named `{table_name}.toon` (e.g. `lupo_actors.toon`, `lupo_channels.toon`, `lupo_edges.toon`). There are 160+ such files; the exact count is whatever `lupo-scripts/generate_toon_files.py` last wrote.
...
- You may summarise by table *category* (e.g. actor-related, channel-related, dialog, edges, semantic, lupo-rules/tasks, auth/sessions, federation) rather than line-by-line for every TOON.

### 2.3 Canonical architecture

- Skim **`lupo-docs/status/cursor_actors_channels_semantic_architecture_4.0.69.md`** so your suggestions align with installation path, fallback, actor/faucet, channel, edges, and full table list there.
...
- **Path:** `lupo-docs/status/lilith_suggestions_on_database_channels_semantic_organisation.md`
...
| Item | Value |
|------|--------|
| Orchestration doc | `lupo-docs/status/ORCHESTRATION_ACTORS_AND_SUPPORTING_ACTORS_REVIEW_4.0.69.md` |
...
| Output file | `lupo-docs/status/lilith_suggestions_on_database_channels_semantic_organisation.md` |
