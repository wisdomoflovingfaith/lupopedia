...
def _default_db_from_config():
    """Defaults from lupopedia-config.php via lupo-scripts/lib.db_connection (no installer bypass)."""
    root = Path(__file__).resolve().parent.parent
    scripts = str(root / "lupo-scripts")
...
def main():
    ...
    ap.add_argument("--host", default=d["host"], help="MySQL host")
    ap.add_argument("--user", default=d["user"], help="MySQL user")
    ap.add_argument("--password", default=d["password"], help="MySQL password")
    ap.add_argument("--database", default=d["database"], help="MySQL database")
    ap.add_argument("--port", type=int, default=d.get("port", 3306), help="MySQL port")
    args = ap.parse_args()
