/**
 * LUPOPEDIA Header Editor Webview Panel
 *
 * Shows parsed LUPOPEDIA Header fields in an editable form.
 * User can edit values and click "Validate" to re-check locally,
 * or "Fetch from Server" to call POST /semantic/lupopedia-header.
 *
 * @module webviews/headerEditor
 */

import * as vscode from 'vscode';
import { LupopediaHeader, formatLupopediaHeader } from '../lupopedia/headers';
import { getLupopediaHeaderFromServer } from '../lupopedia/semantic';
import { ActorIdentity } from '../lupopedia/identity';

export class HeaderEditorPanel {
    public static currentPanel: HeaderEditorPanel | undefined;
    private static readonly _viewType = 'lupopediaHeader';
    ...
    public static createOrShow(
        ...
        header: LupopediaHeader,
        baseUrl: string,
        identity: ActorIdentity | null
    ): void {
        const column = vscode.ViewColumn.Beside;
        if (HeaderEditorPanel.currentPanel) {
            HeaderEditorPanel.currentPanel._panel.reveal(column);
            HeaderEditorPanel.currentPanel._update(header);
            return;
        }
        const panel = vscode.window.createWebviewPanel(
            HeaderEditorPanel._viewType,
            'Lupopedia: LUPOPEDIA Header Editor',
            column,
            { enableScripts: true }
        );
        HeaderEditorPanel.currentPanel = new HeaderEditorPanel(panel, extensionUri, header, baseUrl, identity);
    }
    ...
    private constructor(
        ...
        this._panel.webview.onDidReceiveMessage(
            async (msg: { command: string; header?: LupopediaHeader; filePath?: string }) => {
                if (msg.command === 'fetchFromServer' && msg.filePath) {
                    try {
                        const result = await getLupopediaHeaderFromServer(this._baseUrl, {
                            file_path: msg.filePath,
                        });
                        vscode.window.showInformationMessage(
                            `Lupopedia: Server LUPOPEDIA Header fetched. Channel: ${result.channel_id ?? 'unresolved'}`
                        );
                    } catch (err) {
                        vscode.window.showErrorMessage(`LUPOPEDIA HEADER fetch failed: ${String(err)}`);
                    }
                }
                if (msg.command === 'copyToClipboard' && msg.header) {
                    const text = formatLupopediaHeader(msg.header);
                    await vscode.env.clipboard.writeText(text);
                    vscode.window.showInformationMessage('Lupopedia: LUPOPEDIA Header copied to clipboard.');
                }
            },
            null,
            this._disposables
    ...
    private _update(header: LupopediaHeader): void {
        this._panel.webview.html = this._getHtml(header);
    }

    private _getHtml(header: LupopediaHeader): string {
        const nonce = getNonce();
        const formatted = esc(formatLupopediaHeader(header));
...
<head>
  ...
  <title>Lupopedia LUPOPEDIA Header Editor</title>
  <style nonce="${nonce}">
    :root {
...
<body>
  ...
  <script nonce="${nonce}">
    ...
    document.getElementById('btn-validate').addEventListener('click', () => {
      ...
      if (errors.length === 0) {
        status.className = 'ok';
        status.textContent = '✓ LUPOPEDIA Header is valid.';
    ...
    private _dispose(): void {
        HeaderEditorPanel.currentPanel = undefined;
        this._panel.dispose();
