...
All agents MUST read `lupo-config/global_atoms.json` before any task.
...
```python
import json
with open('lupo-config/global_atoms.json') as f:
    atoms = json.load(f)
...
If the edit changes **Python code logic** in any meaningful way, set `when_updated` to the current UTC timestamp from `lupo-config/global_atoms.json`.
...
Then read `current_utc` from `lupo-config/global_atoms.json`.
