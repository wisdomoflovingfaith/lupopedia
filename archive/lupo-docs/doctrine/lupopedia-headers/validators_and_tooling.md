...
lupopedia.headers:
  ...
  file_path_from_root: "lupo-docs/doctrine/lupopedia-headers/validators_and_tooling.md"
  web_path: "https://www.lupopedia.com/lupopedia/lupo-docs/doctrine/lupopedia-headers/validators_and_tooling.md"
  status: "active"
  when_updated: "20260418192619"
  trust_tier: "canonical"
  questions_toon: null
  memory_toon: "lupo-memory/development/canonical/1026/04/validators-and-tooling.toon"
  atoms_toon: null
  ...
  thread_id: "headers-validators-v3"
  content_id: null
...
**Freeze:** **[PRD 16](../../prd/16_lupopedia_headers.md) — Header freeze rule (4.1.3)**. Validator mechanics for legacy **`4.0.*`**, **`4.1.0`–`4.1.2`**, and integer **`3`** remain as implemented; **normative new edits** declare **`"4.1.3"`**. Header-format evolution resumes **4.1.4+** after the Crafty Syntax human live-help baseline is complete.
...
- **`lupo-scripts/validate_lupopedia_headers_universal.py`** — shipped implementation (source of truth for checks and `HDR_*` messages).
...
**Generators:** `lupo-scripts/add_lupopedia_header_to_file.py` emits both shapes. Batch coverage: `lupo-scripts/add_lupopedia_headers_everywhere.py` reuses the same builders.
...
That strictness applies to **authoring validation** (`validate_lupopedia_headers_universal.py`). **Repair/dedupe** tools may use a more forgiving peel: `lupo-scripts/lib/lupopedia_markdown_header_peel.py` implements a **25-line fast path** plus a **line-based fallback** to find a closing `---` when merging duplicates—see that module’s docstring. Peeling does not relax the universal validator for normal commits.
...
|-----------|---------------|---------------------|
| Two YAML headers at top | `HDR_MULTIPLE_HEADERS` | `python lupo-scripts/fix_double_headers.py` (`--dry-run` first), then re-validate |
| Envelope / blank lines / close `---` | `HDR_LINE_COUNT`, `HDR_MISSING_BLANK_LINE`, `HDR_MISSING_CLOSE` | Fix missing keys/order first; then `python lupo-scripts/normalize_lupopedia_md_header_25.py` where applicable |
| Box-drawing / mojibake in header region | `HDR_UNICODE_BOX` | `python lupo-scripts/fix_unicode_box_drawing_ascii.py` (`--dry-run` / `--check` first); default `--path` `lupo-docs/prd/**/*.md` |
...
| Batch sweep (PRDs or all `lupo-docs` md) | — | `python lupo-scripts/batch_validate_prd_headers.py` (optional `--all-md`, `--verbose`, `--timeout`) |
...
- **New file / missing header:** `python lupo-scripts/add_lupopedia_header_to_file.py <path.py|path.md> [--create] [--title "..."]` — emits the **4.0.x** envelope. See **[TEMPLATES_NEW_FILE.md](TEMPLATES_NEW_FILE.md)**.
- **Git hook (sample):** `lupo-scripts/git-hooks/pre-commit-lupopedia-headers.sample` — substring guard; for strict checks, call the universal validator from CI or a wrapper.
- Generators must emit **`channel_key`**, **`trust_tier`**, and tier-shaped **`memory_key`**; map legacy **`channel_id`** via **`lupo-channels/registry.json`**.
...
- `python lupo-scripts/json_to_toon.py --json "lupo-memory/constitutional/seed/prd-00-constitutional.json" --toon "lupo-memory/constitutional/seed/prd-00-constitutional.toon"`
- `python lupo-scripts/validate_memory_json_toon_pair.py --base "lupo-memory/constitutional/seed/prd-00-constitutional"`
