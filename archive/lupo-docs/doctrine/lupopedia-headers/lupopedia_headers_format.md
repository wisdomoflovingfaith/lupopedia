...
lupopedia.headers:
  ...
  file_path_from_root: "lupo-docs/doctrine/lupopedia-headers/lupopedia_headers_format.md"
  web_path: "https://www.lupopedia.com/lupopedia/lupo-docs/doctrine/lupopedia-headers/lupopedia_headers_format.md"
  status: "active"
  when_updated: "20260418192619"
  trust_tier: "canonical"
  questions_toon: null
  memory_toon: "lupo-memory/development/canonical/1026/04/lupopedia-headers-format.toon"
  atoms_toon: null
...
2. **TOON derived** (`<basename>.toon`) — generated from JSON via **`python lupo-scripts/json_to_toon.py`**.
...
Deep pairing checks: **`python lupo-scripts/validate_memory_json_toon_pair.py`** (see [VALIDATORS_AND_TOOLING.md](VALIDATORS_AND_TOOLING.md)).

### Trust-tier path pattern

**`memory_toon`** follows trust-tiered pathing (after the `lupo-memory/` prefix, the next segment is always **`{channel_key}`**, then **`{trust_tier}`**):

- **`seed`:** `lupo-memory/{channel_key}/seed/{slug}.toon`  
  Example: `lupo-memory/constitutional/seed/prd-00-constitutional.toon`
...
- **`staging`:** `lupo-memory/{channel_key}/staging/{actual_year}/{month}/{slug}.toon` (real calendar year in the path segment)
- **`archive`:** `lupo-memory/{channel_key}/archive/{actual_year}/{month}/{slug}.toon` — **same calendar-year shape as `staging`** (not the 1000-offset); the **`archive`** segment marks deprecated / superseded lineage.

**Compact legacy paths** (no `{year}/{month}` after `trust_tier`) may still appear in the repo (e.g. `lupo-memory/{channel_key}/seed/{slug}.toon` only); validators accept them when segments match header **`channel_key`** and **`trust_tier`** (**PRD 16** §8.1, **VALIDATORS_AND_TOOLING**).
...
#   web_path: "https://www.lupopedia.com/lupopedia/lupo-scripts/example.py"
#   questions_toon: null
#   federation_node_id: 0
#   channel_key: "development"
#   trust_tier: "canonical"
#   memory_key: "lupo-memory/development/canonical/1026/04/example.toon"
...
Use **`python lupo-scripts/add_lupopedia_header_to_file.py`**, **`fix_double_headers.py`**, **`normalize_lupopedia_md_header_25.py`**, then **`validate_lupopedia_headers_universal.py`** (see **VALIDATORS_AND_TOOLING**).
...
| Survey all PRD metadata | `grep -h "pk_id:\|title:\|status:" lupo-docs/prd/*.md` | ~3 lines/file |
| Find all orphans (no DB row) | `grep -rl "content_id: null" lupo-docs/` | O(n headers) |
| Find staging files | `grep -rl 'trust_tier: "staging"' lupo-docs/` | O(n headers) |
