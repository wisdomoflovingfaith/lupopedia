...
lupopedia.headers:
  ...
  file_path_from_root: "lupo-docs/doctrine/VALIDATION_PATTERNS.md"
  web_path: "https://www.lupopedia.com/lupopedia/lupo-docs/doctrine/VALIDATION_PATTERNS.md"
  status: "active"
  when_updated: "20260412131327"
  trust_tier: "canonical"
  questions_toon: null
  memory_toon: "lupo-memory/development/canonical/1026/04/validation-patterns.toon"
  atoms_toon: null
...
- **Probe runtime guard (examinee output):** `python lupo-scripts/probe_runtime_guard.py` — extracts first fenced code block; otherwise prints **`ERROR: PROBE_BOUNDARY_VIOLATION`** and exit code **2**. See [`AI_ACTOR_PROBE_HARNESS_AND_GUARDS.md`](AI_ACTOR_PROBE_HARNESS_AND_GUARDS.md).
- **LUPOPEDIA HEADERS:** `python lupo-scripts/validate_lupopedia_headers_universal.py <path>` — see [PRD 16](../prd/16_lupopedia_headers.md), [`LUPOPEDIA_HEADERS/VALIDATORS_AND_TOOLING.md`](LUPOPEDIA_HEADERS/VALIDATORS_AND_TOOLING.md).
- **Pseudocode discipline (optional):** `python lupo-scripts/validate_pseudocode_discipline.py` — [PRD 17](../prd/17_decisions_format.md).
- **Directory tree / versioning prep:** `python lupo-scripts/generate_directory_tree.py` — authoritative list in `DIRECTORY_TREE.md`; required before certain audits per project rules (see required-tables doctrine in **`.cursor/rules`**).
...
- **Validators MUST run after runtime guard extraction** on probe-classified examinee output (canonical script: [`lupo-scripts/probe_runtime_guard.py`](../../lupo-scripts/probe_runtime_guard.py); normative PRD: [PRD 53](../prd/53_runtime_guard.md)).
...
|------|-----|-------------|----------------|
| `VALIDATION_PATTERNS.md` | `lupo-docs/prd/61_doctrine_consolidation_shorthand_compiler.md` | `doctrine_rule` | `invariant_checklist_validation_index` |
