...
lupopedia.headers:
  ...
  web_path: "https://www.lupopedia.com/lupopedia/lupo-docs/doctrine/CHRONOLOGICAL_TRUST_LADDER.md"
  status: "active"
  when_updated: "20260414220000"
  trust_tier: "canonical"
  questions_toon: null
  memory_toon: "lupo-memory/development/canonical/1026/04/chronological-trust-ladder.toon"
  atoms_toon: null
...
**Agent / contributor shorthand:** Copy-paste warning block for IDE prompts and reviews: [TRUST_LADDER_DO_NOT_FIX.txt](TRUST_LADDER_DO_NOT_FIX.txt). Optional repo sweep: `python lupo-scripts/validate_trust_ladder_paths.py` (default warn-only; `--strict` for CI once legacy `canonical/2xxx/` paths are migrated).
...
**Enforcement:** `php lupo-scripts/validate_trust_ladder_pk.php` (optional DB scan; **`--calendar-year=YYYY`** for deterministic CI). Header / path mirrors: `python lupo-scripts/validate_lupopedia_headers_universal.py --strict-memory-year` and **`php lupo-scripts/validate_memory_key_years.php`**.
...
The filesystem mirror (`lupo-memory/`) follows trust tiers so path and PK band tell the same story:
