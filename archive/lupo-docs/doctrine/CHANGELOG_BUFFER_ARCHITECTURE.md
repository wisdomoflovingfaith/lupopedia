...
lupopedia.headers:
  ...
  file_path_from_root: "lupo-docs/doctrine/CHANGELOG_BUFFER_ARCHITECTURE.md"
  web_path: "https://www.lupopedia.com/lupopedia/lupo-docs/doctrine/CHANGELOG_BUFFER_ARCHITECTURE.md"
  status: "active"
  when_updated: "20260419010000"
  trust_tier: "canonical"
  questions_toon: null
  memory_toon: "lupo-memory/development/canonical/1026/04/changelog-buffer-architecture-md.toon"
  atoms_toon: null
  ...
  summary: "Defines mandatory JSON buffer entries in lupo-changelog-pending and lupo-changelog-archive, with consolidation rules and merge logic."
---
...
## 2. Directories
- Pending: lupo-changelog-pending/
  - Contains unmerged JSON entries.
- Archive: lupo-changelog-archive/
  - Contains processed JSON entries (moved here after consolidation).
- Version Changelogs: lupo-docs/versions/{version}/CHANGELOG.md
  - The target for consolidated entries.

## 3. JSON Schema (Required)
All files in lupo-changelog-pending/ must be valid JSON with these fields:
...
Example:
{
  ...
  "files_changed": ["lupo-content/federation_node/0/captains_log/30_file.md"],
  "open_questions": [],
  "handoff_to": null,
  "related_toons": ["lupo-memory/captains_log/canonical/1026/04/30.toon"]
}

## 4. Consolidation and Merge Rules
The consolidator (e.g., `lupo-scripts/consolidate_changelog_v412.py` or `lupo-scripts/consolidate_lupo_changelog_pending.py`) follows these rules:
...
3. **Idempotency**: The consolidator checks the target `CHANGELOG.md` for a hidden merge marker `<!-- lupo-changelog-merged: {filename} -->` to avoid duplicate processing.
...
5. **Archive on Success**: Processed buffer files are moved to `lupo-changelog-archive/` only after successful write to the target `CHANGELOG.md`.
...
- Agents MUST write one buffer entry per logical task/session to `lupo-changelog-pending/`.
