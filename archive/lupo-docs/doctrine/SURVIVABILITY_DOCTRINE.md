...
lupopedia.headers:
  ...
  file_path_from_root: "lupo-docs/doctrine/SURVIVABILITY_DOCTRINE.md"
  web_path: "https://www.lupopedia.com/lupopedia/lupo-docs/doctrine/SURVIVABILITY_DOCTRINE.md"
  status: "active"
  when_updated: "20260418133945"
  trust_tier: "seed"
  questions_toon: null
  memory_toon: "lupo-memory/constitutional/seed/survivability-doctrine.toon"
  atoms_toon: null
  ...
  content_slug: "survivability-doctrine"
...
---
# file: Survivability Doctrine — delegation: cursor:root — web_path: https://www.lupopedia.com/lupopedia/lupo-docs/doctrine/SURVIVABILITY_DOCTRINE.md

# Survivability Doctrine (short: **Survivability**)

## Binding anchor

**Constitutional law:** **`lupo-docs/prd/00_root_constitutional_system_requirements.md`** — **§14.6** (Survivability Doctrine).
...
It is expressed through **`function_exists()`**, **`extension_loaded()`**, manual config paths when writes fail, and PDO-safe failure surfaces. **Installer database carve-out:** **`lupo-docs/doctrine/DATABASE_DOCTRINE.md`** — runtime **`PDO_DB`** only; installer **`mysqli`** only where documented.
...
For **multi-axis qualitative telemetry** (pattern **frequency**, **severity**, **urgency**) used in agent reports such as **`AGAPE_PATTERN_REPORT`**, implementations **SHOULD** follow **`lupo-docs/doctrine/COUNTING_IN_LIGHT_DOCTRINE.md`**:
...
**Validators** MUST return error code **`AGAPE_SENTIMENTAL_PHRASE`** with severity **`error`** (hard fail) when these appear as acceptance/pass-fail criteria. **Note:** the tag is a **legacy stable identifier**; it denotes **Survivability Doctrine** violations (sentimental framing of technical quality), not a reference to love or religion. Implement in **`lupo-scripts/`** (dedicated scanner or extension of existing validators); preserve this **exact** code string when adding enforcement.
...
## 6. Relationship to `lupo-agents/agape/` (agent pack slug **agape**, id **705**)

The **`lupo-agents/agape/`** directory is the **historic pack slug** for the coordination lens whose normative meaning is **this doctrine**: environment-aware resilience, adaptive fallback pathing, and evidence-driven improvement. The on-disk folder name is **not** an invitation to sentimental language in reviews or validators.
...
5. **Knowledge gap closure**
   ...
   - The agent **MUST** document what it **learned** in a **durable** repo location appropriate to the workstream (`lupo-memory/`, `lupo-docs/implementations/.../`, channel thread under **PRD 02**, etc.).
...
| Memory graph / remediation | **PRD 38**, **`lupo-docs/doctrine/AI_ACTOR_KNOWLEDGE_UPDATE_PROTOCOL.md`** |
| Counting in Light / `mood_vector` | **`lupo-docs/doctrine/COUNTING_IN_LIGHT_DOCTRINE.md`** |
