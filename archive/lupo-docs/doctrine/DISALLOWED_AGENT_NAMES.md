...
lupopedia.headers:
  ...
  file_path_from_root: "lupo-docs/doctrine/DISALLOWED_AGENT_NAMES.md"
  web_path: "https://www.lupopedia.com/lupopedia/lupo-docs/doctrine/DISALLOWED_AGENT_NAMES.md"
  status: "active"
  when_updated: "20260418160751"
  trust_tier: "canonical"
  questions_toon: null
  memory_toon: "lupo-memory/development/canonical/1026/04/disallowed-agent-names.toon"
  atoms_toon: null
...
**New** agent packs under `lupo-agents/{slug}/` and the `agent_key` / `slug` fields in new `agent.json` files **must not** collide with system authority strings, recursion-prone tokens, generic placeholders, or already-reserved Lupopedia and IDE identities.

Canonical packs that already own those slugs (for example `lupo-agents/wolfie/`, `lupo-agents/cursor/`) are not violations; the rule blocks **additional** registrations that would impersonate them.

**Enforcement surface:** `python lupo-scripts/validate_agent_name.py` (see section 5). This document is the normative human-readable list; the script embeds the same exact-name set and must be updated in lockstep when this file changes.
...
4. **Uniqueness:** At most one top-level pack per slug; `agent_key` values must be unique across all `lupo-agents/*/agent.json` files discovered by the scanner (see script `--scan-root`).
5. **Pre-merge validation:** New agent packs **MUST** pass **`python lupo-scripts/validate_agent_name.py --scan-root`** before merge or registration (exit **0**; fix or document waiver per change control before shipping).

## 5. Tooling

```bash
# Validate a proposed NEW slug (reserved + structural rules; exit 0 = ok, 1 = invalid)
python lupo-scripts/validate_agent_name.py my-new-agent

# Validate agent_key (or slug) inside an existing agent.json (same rules as new slug)
python lupo-scripts/validate_agent_name.py --file lupo-agents/my-new-agent/agent.json
...
python lupo-scripts/validate_agent_name.py --scan-root
```

## 6. Relationship to actors and agents

Reserved **actor** identities live in `lupo-database/lupopedia/actors/actor_id/registry.json` and related tables. This doctrine governs **agent pack slugs** under `lupo-agents/` so new packs do not impersonate personas, facets, or product names. See also `lupo-docs/ACTOR_REGISTRATION_CHECKLIST.md` and PRD 07 / PRD 50 for coordination identity rules.

## 7. Change control

Any addition or removal from section 2 requires: (1) edit this file, (2) edit `lupo-scripts/validate_agent_name.py` constants to match, (3) run `python lupo-scripts/validate_agent_name.py --scan-root`, (4) changelog buffer entry.

## 8. Pillar 2 integration (learning transfer)

**Canonical defect ID (AGAPE taxonomy):** **`P1-NAMESPACE-COLLISION-001`** — same literal as **`lupo-docs/doctrine/AGAPE_DEFECT_TAXONOMY.md`** (agent pack / namespace collision row).

When a violation is detected (for example a **new** agent pack or `agent_key` using a disallowed name):

1. **Detection:** `python lupo-scripts/validate_agent_name.py` exits **non-zero** for that candidate.
...
3. **Logging:** The violation **MUST** be logged under `lupo-changelog-pending/` as JSON with **`pattern_id`** set exactly to **`P1-NAMESPACE-COLLISION-001`**, plus **`files_changed`**, **`summary`**, and **`agent_id`** (facet slug) per **`lupo-docs/doctrine/CHANGELOG_BUFFER_ARCHITECTURE.md`**.
...
| Phase | Action | Result |
|-------|--------|--------|
| Install-time | `python lupo-scripts/validate_agent_name.py --scan-root` | **Hard fail** — block installation / wizard advance until violations are cleared or explicitly waived by documented operator-only procedure (if any exists for that installer). |
