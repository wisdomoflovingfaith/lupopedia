...
lupopedia.headers:
  ...
  file_path_from_root: "lupo-docs/doctrine/gaps/MEMORY_CHANNEL_ENFORCEMENT.md"
  web_path: "https://www.lupopedia.com/lupopedia/lupo-docs/doctrine/gaps/MEMORY_CHANNEL_ENFORCEMENT.md"
  status: "active"
  when_updated: "20260417000000"
  trust_tier: "canonical"
  questions_toon: null
  memory_toon: "lupo-memory/development/canonical/1026/04/memory-channel-enforcement-gaps.toon"
...
PRD 38 mentions `allowed_cross_channel_memory` in `lupo-channels/registry.json` as the mechanism
...
Define the minimal shape in `lupo-channels/registry.json`:
...
3. A validator rule that reads `lupo-channels/registry.json` when `--verify-channels` is passed
...
The check:
1. Parses the first path segment of `memory_toon` after `lupo-memory/`
...
`MemoryExportService` exports to `lupo-memory/{YYYY}/{MM}/{slug}.json` — no channel segment.
...
reads live DB structure via `SHOW TABLES` / `DESCRIBE`, writes to `lupo-database/lupopedia/`.
Does not read `lupo-memory/*.toon` files and does not write to `lupo_memory_nodes`. "Rerun
...
2. **Path segment** — if header is absent or unparseable, extract `parts[1]` from the
   `memory_toon` value relative to `lupo-memory/` (e.g. `lupo-memory/development/...` → `development`)
...
|------|----------|------------|
| `lupo-scripts/import_memory_edges_from_sidecar.py` | auto-create-source-node INSERT (~line 319) | Add `channel_key` derivation from `memory_toon` path + INSERT column |
| `lupo-scripts/lib/db_memory_writer.py` | `create_memory_node` INSERT (~line 148) | Add `channel_key` to INSERT column list; caller must populate `row["channel_key"]` |
| `lupo-scripts/lib/db_memory_writer.py` | `create_memory_edges` INSERT (~line 228) | Add `channel_key` to INSERT column list; derive from source node or caller context |
...
- `lupo-channels/registry.json` — channel registry (schema TBD)
