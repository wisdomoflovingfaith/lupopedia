...
lupopedia.headers:
  ...
  file_path_from_root: lupo-docs/prd/17_A_DECISIONS_FORMAT.md
  web_path: "https://www.lupopedia.com/lupopedia/lupo-docs/prd/17_A_DECISIONS_FORMAT.md"
  status: active
  when_updated: "20260422232349"
  trust_tier: canonical
  questions_toon: null
  memory_toon: lupo-memory/development/canonical/1026/04/17_decisions_format.toon
  atoms_toon: null
...
2. **`lupo-templates/`** ??? Mockups, partials, admin fragments, and HTML/PHP **fragments** live in the template tree **before** they are the only copy of the UI.
3. **Language arrays / localization** ??? Keys and English strings are added to locale catalogs (`lupo-includes/lang/`, **`lupo_t()`** when wired); other locales mirror keys per locale doctrine.
...
- Public paths (**`channels/index.php`**, **`admin.php`**, themes, public handlers) are **assembly** layers: they integrate artifacts that already exist under **`lupo-templates/`** (and documented PRD or thread decisions), not ad hoc markup dumps.
- Skipping documentation or skipping the **`lupo-templates/`** bridge is a **process violation**, not a shortcut.
...
under **`lupo-templates/`** (including **`lupo-templates/admin/`** and other subtrees as policy defines), **then** be wired into authenticated or public entrypoints.
...
**Anti-pattern:** Large blocks of new UI markup exist **only** in **`channels/index.php`** or **`admin.php`** with no corresponding **`lupo-templates/`** partial.
...
**References:** [PRD 45](45_template_first_staged_ui_workflow.md); **`lupo-includes/lang/`**; **`lupo_t()`** / **`LupoLocale`** in AGENTS.md and PRD 00.
...
| 2 | **`lupo-templates/`** partials, mockups, fragments | Controlled iteration and review before runtime assembly |
...
This section is the **single source of truth** for thread artifact filenames under **`decisions/{channel_key}/`** (canonical **global** tree at repository root), and for **parallel** contexts (**`lupo-channels/...`**, **`lupo-docs/implementations/...`**, **`lupo-agents/...`**) that use the **same** `TYPE` / optional `STATUS` rules.
...
- **No** **`lupo-docs/versions/{version}/`** prefix for **new** global threads ??? **version is metadata**, not a parent directory (see [Core doctrine: decisions are derived, not primary](#core-doctrine-decisions-are-derived-not-primary)).

### Parallel contexts (same filename rules; different root)

| Root | Notes |
|------|--------|
| `lupo-channels/{federation_node_id}/{channel_key}/{thread_key}/` | Operational coordination per [PRD 02](02_channels_discussions.md) / [PRD 29](29_project_structure.md). Prefer **`questions/`** for both questions and answers for **new** files. |
| `lupo-docs/implementations/{prd_file_stem}/` | [PRD 31](31_implementation_folder_guidelines.md) mirrors ??? same **`THREAD_INDEX.md`** and edge rules. |
| `lupo-agents/{agent_key}/` | Agent-local policy when used. |

### Legacy `lupo-docs/versions/<version>/` (read / migrate only)

Version-scoped **`decisions/`**, **`questions/`**, **`answers/`**, **`comments/`** trees under **`lupo-docs/versions/`** are **legacy**. **Do not** add new thread files there; migrate into **`decisions/{channel_key}/`** (or an implementation mirror / channel tree) and record **`target_version`** in the artifact body or **`lupopedia.footer`**.

### Monolithic `decisions.md` (forbidden for new work)

The single-file **`decisions.md`** under **`lupo-docs/versions/<version>/`** is **deprecated** for new entries. Parse into per-thread files when migrating.
...
lupopedia.headers:
  ...
  trust_tier: "canonical"
  questions_toon: null
  memory_toon: "lupo-memory/development/canonical/1026/04/header_format_decision.toon"
  atoms_toon: null
...
- **Goal:** Bridge **decisions to design**: sketch signatures, document **Option A vs Option B**, trade-offs, open questions (`TODO:` / `QUESTION:`), and task-level drivers **before** ship-ready production code exists. **Ship-facing UI** still obeys **[Staged Development Workflow (Required)](#staged-development-workflow-required)** ( **`lupo-templates/`** , then **`lupo_t()`** / catalogs, then public assembly); pseudocode does **not** replace that sequence.
...
Parallel contexts use the **same** inner **`decisions/`** + **`pseudocode/`** shape under their own root (for example **`lupo-channels/0/development/my-thread/decisions/`**).
...
lupopedia.headers:
  ...
  web_path: "https://www.lupopedia.com/lupopedia/lupo-channels/0/example/decisions/pseudocode/KairosConsolidationService.pseudo.php"
...
Describe the agreed **build path** using **[Staged Development Workflow (Required)](#staged-development-workflow-required)**. For **UI-facing** work, cite **`lupo-templates/`** partials and **[Language Array Integration Rule](#language-array-integration-rule)** before pointing to public runtime edits. Placeholder: [How to implement].
...
10. **Staged UI delivery** (process audits) - For **new UI-facing** merges, reviewers **should** verify existence of **`lupo-templates/`** partials and **`lupo_t()`** / catalog keys per **[Staged Development Workflow (Required)](#staged-development-workflow-required)** before approving large **public-runtime-only** diffs.
...
**Parallel examples:** **`lupo-docs/implementations/{prd_file_stem}/`** mirrors and **`lupo-channels/{federation_node_id}/{channel_key}/{thread_key}/`** trees illustrate the **same** filename, **`THREAD_INDEX.md`**, and **`lupopedia.edges`** rules under different roots.

**Legacy:** **`lupo-docs/versions/*/decisions/`** may still contain historical threads ??? **read-only** reference for migration; do **not** extend.
