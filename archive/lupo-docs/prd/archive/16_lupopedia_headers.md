...
lupopedia.headers:
  ...
  web_path: "https://www.lupopedia.com/lupopedia/lupo-docs/prd/16_lupopedia_headers.md"
  status: "active"
  when_updated: "20260415162356"
  trust_tier: "canonical"
  questions_toon: null
  memory_toon: "lupo-memory/headers/canonical/1026/04/16_lupopedia_headers.toon"
  ...
  content_slug: "lupopedia-headers"
...
- Migration procedures and concrete examples are defined in companion docs:
  - `lupo-docs/prd/16_lupopedia_headers_migration.md`
  - `lupo-docs/prd/16_lupopedia_headers_examples.md`
...
- Channel-scoped transcript artifacts live under `lupo-channels/` when generated/exported.
...
- 4.1.4: remove `dialog_transcript` alias support
...
- Migration guide: `lupo-docs/prd/16_lupopedia_headers_migration.md`
...
- **Normative ceilings** for header schema evolution (e.g. **22** keys in **4.1.1**, four trust tiers) are documented with other system limits in **`lupo-docs/prd/99_limits_for_everything_and_why.md`** (**PRD 99**).
...
Migration: Validators MUST accept `dialog_transcript` as a legacy alias for `transcript_jsonl` during migration and emit **`HDR_DIALOG_TRANSCRIPT_RENAMED`** (WARN) when the old name is encountered. New artifacts MUST use `transcript_jsonl`. Simultaneously, the canonical field order has been restructured — see §4.2 and the v4.1.0 changelog entry. Migration script: `lupo-scripts/rename_dialog_transcript_to_transcript_jsonl.py`.
...
| **ANUBIS** | System orphan processor. **actor_id/agent_id are sourced from** `lupo-memory/atoms/lupopedia_global_constants.atom.toon` (`constants.actors.anubis`, `constants.agents.anubis`). ANUBIS creates `lupo_contents` rows, resolves/creates transcript thread bindings, and writes completed headers for filesystem-created files. |
...
*For the authoritative list, see `lupo-memory/atoms/lupopedia_global_constants.atom.toon`.*
...
```
File on disk:   lupo-docs/prd/16_lupopedia_headers.md
                      |
                      ▼  (header field)
                content_id: 12345
                      |
...
**Header field count:** See `lupo-memory/atoms/lupopedia_global_constants.atom.toon` -> `constants.header_fields.count`
...
1. `header_format_version` - string in the **`4.1.x`** family (example: **`"4.1.1"`**). Validators MUST accept any `4.1.*` patch; also accept `4.0.*` and legacy `3` for older artifacts during migration. **For new artifacts,** SHOULD equal **`GLOBAL_CURRENT_LUPOPEDIA_VERSION`** from **`lupo-config/global_atoms.yaml`**.
...
9. **`atoms_toon`** - YAML **`null`** or string ending in **`.atoms.toon`** (general atoms sidecars) or **`.atom.toon`** (global constants atom); nullable pointer to immutable atom truth files for this artifact. Atom files contain truths that must not silently drift — canonical definitions, version invariants, structural rules, and global constants. **`null`** when no atom file exists (always valid). Empty string **`''`** is **forbidden** — use **`null`** instead (validator: **`HDR_ATOMS_TOON_SUFFIX`**). File existence is NOT enforced by the validator. **THOTH** (future) will use this pointer for corpus-wide integrity validation. **Replaces deprecated `module` field** (legacy subsystem label); validators accept `module` as alias during migration (emit **`HDR_MODULE_DEPRECATED`** WARN). Schema: `lupo-docs/doctrine/lupopedia-headers/atoms_toon_schema.md`.
...
Two on-disk layouts are normative; they use **different second-segment semantics** after `lupo-memory/...` - do not conflate **`artifact_type`** with **`trust_tier`**.

| Layout | Purpose | Path pattern | Segment after fixed prefix |
|--------|---------|--------------|----------------------------|
| **`header_metadata` JSON** | Sidecar **`header_metadata`** file (taxonomy index) | `lupo-memory/headers/{artifact_type}/{YYYY}/{MM}/{stem}.metadata.json` | **`artifact_type`** (`prd`, `doctrine`, ...) - **not** `trust_tier` |
| **`memory_toon` `.toon`** | Compacted memory export / graph mirror | `lupo-memory/{channel_key}/{trust_tier}/{memory_year}/{MM}/{slug}.toon` | **`trust_tier`** - **not** `artifact_type`; **`memory_year`** from **§8.1** (`YYYY` or `YYYY - 1000`) |

**Disambiguation:** The directory name **`headers`** in the metadata path is a **fixed repository convention** for the metadata index tree. It is **not** required to equal **`channel_key`**. When **`channel_key`** happens to be `headers`, a `.toon` may still live under `lupo-memory/headers/{trust_tier}/...`; the **third** path segment is then **`trust_tier`**, while the metadata JSON for the same document uses **`lupo-memory/headers/{artifact_type}/...`** where the third segment is **`artifact_type`**. Those segments are **never interchangeable**.

```text
Header declares  ->  memory_toon  ->  .../{trust_tier}/{memory_year}/{MM}/{slug}.toon
Tooling derives   ->  header_metadata  ->  lupo-memory/headers/{artifact_type}/{YYYY}/{MM}/{stem}.metadata.json
```
...
1. Let `stem` = basename of `file_path_from_root` **without extension** (e.g. `16_lupopedia_headers` from `lupo-docs/prd/16_lupopedia_headers.md`).
2. Let `YYYY` / `MM` = UTC calendar year and month from `when_updated`, each zero-padded (`2026`, `04`).
3. **Path:** `lupo-memory/headers/{artifact_type}/{YYYY}/{MM}/{stem}.metadata.json`
 - Example: `artifact_type: prd`, `stem` as above -> `lupo-memory/headers/prd/2026/04/16_lupopedia_headers.metadata.json`
 - **This PRD file** (`file_path_from_root: lupo-docs/prd/16_lupopedia_headers.md`), with `when_updated` in April 2026 and `artifact_type: prd`, derives to: **`lupo-memory/headers/prd/2026/04/16_lupopedia_headers.metadata.json`** (YYYY/MM from `when_updated`).

**`memory_toon`** is **independent**: it MUST follow **`lupo-memory/{channel_key}/{trust_tier}/{memory_year}/{MM}/{slug}.toon`** (or equivalent slug) per **§8.1** and **PRD 38**. Importers MUST treat **`header_metadata`** as authoritative for `purpose`, `tags`, `edges`, `footer`, `init`, and dual `transcript_jsonl` alignment.
...
**Offset value:** See `lupo-memory/atoms/lupopedia_global_constants.atom.toon` -> `constants.year_offset.canonical_offset`

**Example:**
- Calendar year: 2026
- Display year: 2026 - 1000 = 1026
- Path: `lupo-memory/{channel_key}/canonical/1026/{mm}/{slug}.toon`
...
- **Canonical:** `python lupo-bin/transcript.py` (or browser/IDE client) -> **HTTP POST** -> PHP endpoint -> **`lupo_dialog_messages`** (or dedicated transcript table if split - same logical row).
- **Authentication:** session cookie **or** `X-Lupo-Api-Token` (or `X-API-Token`) header for automation; PHP validates **one** policy.
- **Channel transcript artifact:** Channel-scoped transcript files are written under `lupo-channels/{federation_node_id}/{channel_key}/{prd_cluster}/transcript.jsonl` for filesystem visibility and replay.
...
| Item | Rule |
|------|------|
| **Queue file path** | `lupo-config/offline_transcript_queue.jsonl` (repo root-relative; one JSON object per line). |
...
| **Writer** | Only `lupo-bin/transcript.py` (or a single named script) may append; no scatter of queue files. |
| **Reconciliation** | Operator or agent runs `python lupo-bin/transcript.py --flush-offline-queue` (or equivalent) which POSTs each line to **§9.1** endpoint in order; on success, **truncate or rotate** the queue file. |
...
- **Channel storage location:** transcript artifacts for channel work live under `lupo-channels/{federation_node_id}/{channel_key}/{prd_cluster}/transcript.jsonl`.
...
**Important:** `transcript_jsonl` is a slug, not a literal path string. File placement under `lupo-channels/` is derived from that slug using the canonical `{federation_node_id}/{channel_key}/{prd_cluster}` mapping, not by treating header text as a raw filesystem path.
...
```bash
python lupo-scripts/compaction/generate_memory_toon.py --transcript-slug "0/headers/lupopedia-headers" --file-path "lupo-docs/prd/16_lupopedia_headers.md"
python lupo-scripts/json_to_toon.py --json "lupo-memory/headers/prd/2026/04/16_lupopedia_headers.json" --toon "lupo-memory/headers/canonical/1026/04/16_lupopedia_headers.toon"
python lupo-scripts/validate_memory_json_toon_pair.py --path "lupo-memory/headers/"
```
...
1. Create `lupo-docs/versions/4.0.98/` with same subtree pattern as `4.0.97/`.
...
7. **Tooling:** consolidate `lupo-scripts/lib/header_spec_v3_1.py` (new) + `validate_lupopedia_headers_universal.py` + `lib/header_validation.py` into **one** ruleset matching **§12** (implementation task, not a separate spec).
...
9. **Duplicate consecutive YAML blocks (`HDR_MULTIPLE_HEADERS`):** Run `python lupo-scripts/fix_double_headers.py` (use `--dry-run` first; optional `--backup`, `--verbose`) on affected Markdown. The tool merges multiple leading `lupopedia.headers` blocks using the newest `when_updated` as the tie-break, then rebuilds the **§4.3** dense envelope.
10. **Missing `content_id` / inner key line count:** Run `python lupo-scripts/normalize_lupopedia_md_header_25.py` (`--dry-run` or `--check` first; optional `--backup`, `--verbose`, `--recursive`) so the inner block matches **§4.3** rule 8: **22** contiguous key lines (**3–24**) with **no** internal blanks, then closing delimiter on line **25**.
11. **Box-drawing / mojibake before header repair:** Run `python lupo-scripts/fix_unicode_box_drawing_ascii.py` (`--dry-run` / `--check` first) so **`HDR_UNICODE_BOX`** does not mask envelope parsing (see [VALIDATORS_AND_TOOLING.md](../doctrine/LUPOPEDIA_HEADERS/VALIDATORS_AND_TOOLING.md)).
...
8. **Manual (LILITH):** browser exercise - create transcript, view graph, confirm expected append under `lupo-channels/` for the active channel/thread.
9. **Gate (transcript.jsonl):** From a clean baseline, run the test window, then search **only**  
   `lupo-channels/**/*.jsonl`  
   (recursive under `lupo-channels/`, files ending in `.jsonl`). The expected channel/thread transcript file MUST be created or appended during the window, and writes MUST stay within the matching slug-derived directory.
10. **Gate (offline transcript queue):** During normal web-only E2E, **`lupo-config/offline_transcript_queue.jsonl`** MUST show **zero new lines** (fallback queue must remain idle unless DB/PHP path is unavailable).
...
# or all Markdown under lupo-docs:
python lupo-scripts/batch_validate_prd_headers.py --all-md
# single file:
...
Some trees still contain **`lupo-channels/{federation_node_id}/{channel_key}/{slug}/transcript.jsonl`**. That layout is **not** a normative write target after freeze (**§15** gate). **`transcript_jsonl`** remains a **DB slug** (**§4.2** field **10**); do not treat those paths as implied by the slug.
...
| Surface | Role |
|---------|------|
| **`python lupo-bin/transcript.py`** | **Client** - builds JSON and **POST**s to **§9.1** when online; **§9.1.1** queue when offline |
...
| `tool` | CLI / one-off script (e.g. `lupo-scripts/*.py`) | `content_parent_id`, `status` |
...
- **`lupo-scripts/validate_lupopedia_headers_universal.py`** - enforcement of **§4** / **§12** (including **22**-key **v4.1.1** envelope; legacy alias handling during transition)
- **`lupo-scripts/import_content.py`** - content import flow (independent from fixed header field set)
...
```json
{
  ...
  "source_file": "lupo-docs/prd/16_lupopedia_headers.md",
  ...
  "edge_to_memory_toon": "lupo-memory/headers/canonical/1026/04/16_lupopedia_headers.toon"
}
```
...
**Pattern:**
`lupo-memory/{channel_key}/questions/{YYYY}/{MM}/{slug}.questions.toon`

**Example for this PRD:**
```
memory_toon: "lupo-memory/headers/canonical/1026/04/16_lupopedia_headers.toon"
questions_toon: "lupo-memory/headers/questions/2026/04/16_lupopedia_headers.questions.toon"
```
...
- **Tooling**: Run `python lupo-scripts/normalize_lupopedia_md_header_25.py` on each file at migration time — the script will replace `last_modified_utc: "timestamp"` with `questions_toon: null`.
- **Batch migration command**: `python lupo-scripts/rename_last_modified_to_questions_toon.py` — renames the field in all Python script own-headers in one pass.
...
- [ ] `LEGACY_KEYS_V4` entry `"last_modified_utc": "questions_toon"` removed from `lupo-scripts/lib/header_spec_v3_1.py`
- [ ] `HDR_LAST_MODIFIED_RENAMED` removed from validator error-code taxonomy (§19.3)
- [ ] `lupo-scripts/remove_phase3_legacy_support.py` run to completion and output reviewed

**Trigger**: WOLFIE or Actor 116 confirms all items, then runs `python lupo-scripts/remove_phase3_legacy_support.py`.
**Closes**: OQ-38 in `lupo-docs/versions/4.0.99/status/open_questions.md`.
...
head -25 lupo-docs/prd/16_lupopedia_headers.md
```
...
grep -h "artifact_type:\|content_id:\|memory_toon:" lupo-docs/prd/*.md | head -3
```
...
import subprocess
for path in sorted(Path("lupo-docs/prd").glob("*.md")):
...
grep -rl "content_id: null" lupo-docs/ lupo-scripts/ lupo-includes/
```
...
```bash
grep -rl 'trust_tier: "staging"' lupo-docs/prd/
```

And to find files at the old header format version needing migration:

```bash
grep -rl 'header_format_version: "4.0.98"' lupo-docs/
```
...
|---|---|---|
| "What PRDs exist?" | Read all 66 files | `rg -n "content_id:|content_slug:|title:" lupo-docs/prd/*.md` — ~200 lines |
...
Developer workstation:
  ...
  /var/www/lupopedia/
        +-- index.php
        +-- lupo-docs/prd/16_lupopedia_headers.md   ← HEADER IS IN THE ZIP
        +-- lupo-memory/.../16_lupopedia_headers.toon
        +-- lupo-memory/headers/prd/.../16_lupopedia_headers.metadata.json
        +-- lupo-includes/...
```
...
```text
 ...
 4 |   file_path_from_root: "lupo-docs/example.md"
 5 |   web_path: "https://www.lupopedia.com/lupopedia/lupo-docs/example.md"
 6 |   status: "active"
 7 |   when_updated: "20260410120000"
 8 |   trust_tier: "canonical"
 9 |   questions_toon: null
10 |   memory_toon: "lupo-memory/development/canonical/1026/04/example.toon"
11 |   atoms_toon: null
...
# lupopedia.headers:
#   header_format_version: "4.1.1"
#   file_path_from_root: "lupo-includes/example.php"
#   web_path: "https://www.lupopedia.com/lupopedia/lupo-includes/example.php"
#   status: "active"
#   when_updated: "20260410120000"
#   trust_tier: "canonical"
#   questions_toon: null
#   memory_toon: "lupo-memory/development/canonical/1026/04/example-php.toon"
#   atoms_toon: null
