...
lupopedia.headers:
  ...
  file_path_from_root: lupo-docs/prd/19_A_GARBAGE_COLLECTION_SYSTEM.md
  web_path: "https://www.lupopedia.com/lupopedia/lupo-docs/prd/19_A_GARBAGE_COLLECTION_SYSTEM.md"
  status: active
  when_updated: "20260422232349"
  trust_tier: canonical
  questions_toon: null
  memory_toon: lupo-memory/development/canonical/1026/04/19_garbage_collection_system.toon
  atoms_toon: null
...
**Normative implementation:** `lupo-includes/classes/GarbageCollector.php`.

---

## GC Functions

**Shipped implementation:** `lupo-includes/classes/GarbageCollector.php` (prefix-aware, `PDO_DB::query` + `rowCount`, raw->rollup passes in **????8** tables above). The SQL and PHP fragments below are **conceptual**; some still show older patterns (e.g. `ON DUPLICATE KEY` on **`lupo_paths`** where the install has **no** natural unique key--the class uses **SELECT/UPDATE/INSERT** instead).
