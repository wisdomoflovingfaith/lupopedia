...
lupopedia.headers:
  ...
  file_path_from_root: lupo-docs/prd/16_C_LUPOPEDIA_HEADERS.md
  web_path: "https://www.lupopedia.com/lupopedia/lupo-docs/prd/16_C_LUPOPEDIA_HEADERS.md"
  status: active
...
The canonical 22-field order (authoritative from `lupo-memory/atoms/lupopedia_global_constants.atom.toon`):
...
# - doctrine      # Constitutional rules (lupo-rules/root/*.md)
...
For each `artifact_type`, `artifact_kind` MUST be one of the values in the corresponding set. This table is normative and MUST match `ARTIFACT_TYPE_ALLOWED_KINDS` in `lupo-scripts/lib/header_spec_v3_1.py` (single source for validators).
...
Each selector token expands to all files matching `lupo-docs/prd/NN_X*`
...
**Example:** `lupo-memory/headers/questions/2026/04/lupopedia_headers.questions.toon`
...
- **channel/path consistency** (`HDR_CHANNEL_PATH_MISMATCH`): when `memory_toon` is non-null,
  the first path segment after `lupo-memory/` MUST equal the declared `channel_key`. A mismatch
...
def check_channel_path_consistency(channel_key, memory_toon):
    ...
    # memory_toon form: lupo-memory/{channel_key}/{trust_tier}/...
    parts = memory_toon.strip().split('/')
    if len(parts) < 2 or parts[0] != 'lupo-memory':
...
This rule enforces the two-type memory path doctrine defined in [PRD 38](38_memory_unification.md) **Channel Scope for Memory**: Type A `.toon` artifacts MUST have a path whose first segment after `lupo-memory/` matches `channel_key`.
...
Actor and agent IDs are authoritative from `lupo-memory/atoms/lupopedia_global_constants.atom.toon`:
...
See:
`lupo-docs/doctrine/runtime/ANUBIS_QUEUE_EXECUTION_PROPOSAL.md`
...
- **Sidecar Synchronization:** Using `--generate-sidecars` to update `lupo-memory/` artifacts based on the current header state.
