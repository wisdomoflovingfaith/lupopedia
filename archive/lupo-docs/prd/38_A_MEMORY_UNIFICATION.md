...
lupopedia.headers:
  ...
  file_path_from_root: lupo-docs/prd/38_A_MEMORY_UNIFICATION.md
  web_path: "https://www.lupopedia.com/lupopedia/lupo-docs/prd/38_A_MEMORY_UNIFICATION.md"
  status: active
  when_updated: "20260421223000"
  trust_tier: canonical
  questions_toon: null
  memory_toon: lupo-memory/development/canonical/1026/04/38_memory_unification.toon
  atoms_toon: null
  ...
  prd_cluster: 00_A_FORBIDDEN_AND_WHY_38_A_MEMORY_UNIFICATION
...
- **`lupo-memory/`** exports, repo docs, and **`lupo-scripts/*.py`** under a served directory MUST be treated as **potentially world-readable** unless ops **explicitly** blocks those paths or extensions.
...
| `owner_actor_id` | `actor_id` | Execution provenance ??? records which actor wrote the row. Does NOT define context scope or ownership. Context scope is defined by `channel_key` + `thread_id` + artifact lineage. See `lupo-docs/doctrine/CONTEXT_AUTHORITY_MODEL.md`. |
...
| Tier | PK Band | Path in `lupo-memory/` | Mutability | Use Case |
|------|---------|------------------------|------------|----------|
...
**Status:** IMPLEMENTED (path doctrine); DB enforcement deferred ??? see `lupo-docs/doctrine/gaps/MEMORY_CHANNEL_ENFORCEMENT.md`
...
Two distinct artifact types coexist under `lupo-memory/`. They have different paths, formats,
and authority levels. They MUST NOT be confused or mixed.

**Type A ??? Channel-scoped memory (primary authored memory)**

```text
lupo-memory/{channel_key}/{trust_tier}/{YYYY}/{MM}/{slug}.toon
```
...
Examples:
```text
lupo-memory/development/canonical/1026/04/38_memory_unification.toon
lupo-memory/headers/staging/2026/04/some-draft.toon
```

**Type B ??? System / global export mirror (non-channel)**

```text
lupo-memory/{YYYY}/{MM}/{slug}.json
```
...
Examples:
```text
lupo-memory/1970/01/19700101_000000_actor_1_root_actor_root_context.json
lupo-memory/2026/04/20260401_120000_actor_2_root_actor_root_context.json
```
...
`lupo-channels/registry.json` maps `channel_key` to numeric ID and metadata.
...
Cross-channel memory access MUST be declared explicitly in `lupo-channels/registry.json` under
...
The `allowed_cross_channel_memory` field shape in `lupo-channels/registry.json` is not yet
formally specified. See `lupo-docs/doctrine/gaps/MEMORY_CHANNEL_ENFORCEMENT.md` Gap 3.
...
`lupo-docs/doctrine/gaps/MEMORY_CHANNEL_ENFORCEMENT.md` Gap 1.
...
**Note:** The **`MemoryExportService`** mirror under **`lupo-memory/YYYY/MM/{slug}.json`** may remain JSON for IDE readability; where the pipeline emits or pairs a **`.toon`**, that **`.toon`** **MUST** conform to the ordering spec???including any **`edges`** slice materialized from **`lupo_memory_edges`** (**??5.2**). **PRD 16** ??5.2.2 (JSON master ??? TOON derived) still applies.
...
artifacts, which follow `lupo-memory/{channel_key}/{trust_tier}/{YYYY}/{MM}/{slug}.toon` as
...
The `lupo-memory/` directory contains two coexisting layout patterns corresponding to the two
...
under `lupo-memory/1970/01/` (pre-history). Slug stem still encodes owner, type, and key.
...
4. Query helpers **MUST** abstract band logic ??? use **`TrustLadder`** (**`lupo-includes/classes/TrustLadderQueryHelper.php`**) and explicit edge traversal rather than ad hoc raw SQL on ladder tables alone.
...
### 11.1 Migration Script: `lupo-scripts/migrate_memory_to_unified_graph.php`
...
**Canonical procedure:** [`lupo-docs/doctrine/AI_ACTOR_COMPETENCY_TEST_PATTERN.md`](../doctrine/AI_ACTOR_COMPETENCY_TEST_PATTERN.md) ??? when **two or more actors** participate, follow **no self-grading**, **`<TEST_COMPLETE>`** termination by the examiner, **anti-parrot**, **fixed roles**, and **external containment** (violation codes in that file). **When a probe fails:** record the missing rule as **`lupo_memory_nodes`** and bind with **`lupo_memory_edges`** (node-to-node) per [`lupo-docs/doctrine/AI_ACTOR_KNOWLEDGE_UPDATE_PROTOCOL.md`](../doctrine/AI_ACTOR_KNOWLEDGE_UPDATE_PROTOCOL.md); [PRD 50 section 1.3](50_agent_coordination_protocol.md). **Constitutional anchor:** [PRD 00](00_root_constitutional_system_requirements.md) (AI actor verification protocols). **Coordination context:** [PRD 50 sections 1.2???1.4](50_agent_coordination_protocol.md) and shared state / transcripts ??? probes complement, they do not replace, pending tasks and memory edges.

---

## 18. Collection to memory graph binding (payload v1.0.0)

**Canonical payload:** [`lupo-docs/doctrine/collection_payload_format_v1_0_0.md`](../doctrine/collection_payload_format_v1_0_0.md). **Constitutional:** [PRD 00 section 22](00_root_constitutional_system_requirements.md). **Operational sequence (orchestrator + actor):** [PRD 50 section 1.4](50_agent_coordination_protocol.md) subsections **1.4.1???1.4.7**.
