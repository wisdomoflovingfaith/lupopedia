...
lupopedia.headers:
  ...
  file_path_from_root: lupo-docs/prd/18_A_CHANNEL_CHAT_DISPLAY.md
  web_path: "https://www.lupopedia.com/lupopedia/lupo-docs/prd/18_A_CHANNEL_CHAT_DISPLAY.md"
  status: active
...
**Canonical column (DDL):** **`lupo_dialog_messages.to_actor_id`** ???????? see **`lupo-database/lupopedia/json/lupo_dialog_messages.json`**. Some directives use the prose synonym **???????said-to actor???????** / **`said_to_actor_id`**; **semantics are the same** (routing addressee only).
