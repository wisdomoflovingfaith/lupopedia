...
lupopedia.headers:
  ...
  file_path_from_root: lupo-docs/prd/12_B_TOKEN_GOVERNANCE_SECTION.md
  web_path: "https://www.lupopedia.com/lupopedia/lupo-docs/prd/12_B_TOKEN_GOVERNANCE_SECTION.md"
  status: active
  when_updated: "20260422232349"
  trust_tier: canonical
  questions_toon: null
  memory_toon: lupo-memory/prd/canonical/1026/04/token-governance-section.toon
  ...
  thread_id: null
  content_id: null
  content_parent_id: 12
  default_collection_id: null
  lupopedia.schema: prd
  prd_cluster: 00_A_12_B
...
```
lupo-data/fallback/token_usage/
????????? 2026/
???   ????????? 04/
???   ???   ????????? 20_usage.jsonl
???   ???   ????????? 20_budgets.json
...
function logTokenUsageFallback($usage_data) {
    $date = date('Y/m/d');
    $filename = "lupo-data/fallback/token_usage/{$date}_usage.jsonl";
...
function syncFallbackToDatabase() {
    $fallback_files = glob('lupo-data/fallback/token_usage/*_usage.jsonl');
