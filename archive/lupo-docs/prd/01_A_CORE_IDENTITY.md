...
lupopedia.headers:
  ...
  web_path: "https://www.lupopedia.com/lupopedia/lupo-docs/prd/01_core_identity.md"
  status: active
  when_updated: "20260421223000"
  trust_tier: canonical
  questions_toon: null
  memory_toon: lupo-memory/development/canonical/1026/04/01_core_identity.toon
  atoms_toon: null
...
**Purpose:** Canonical **unified memory graph** for runtime learning (including **KAIROS** observation / consolidation **`memory_type`** values), distinct from static **`lupo-agents/`** files. Schema matches **`install_new_lupopedia.sql`**; see **PRD 38** and **PRD 37** for semantics.
...
| training_metadata | JSON | YES | NULL | Additional metadata (`source`, `confidence`, `reviewed_by_actor_id`, optional `result_memory_node_id` / skill / tool refs) per `lupo-docs/doctrine/TRAINING_METADATA_SCHEMA.md` and product conventions |
