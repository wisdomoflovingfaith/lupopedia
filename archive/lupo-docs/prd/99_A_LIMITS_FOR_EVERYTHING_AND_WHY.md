...
lupopedia.headers:
  ...
  file_path_from_root: lupo-docs/prd/99_A_LIMITS_FOR_EVERYTHING_AND_WHY.md
  web_path: "https://www.lupopedia.com/lupopedia/lupo-docs/prd/99_A_LIMITS_FOR_EVERYTHING_AND_WHY.md"
  status: active
  when_updated: "20260422225147"
  trust_tier: canonical
  questions_toon: null
  memory_toon: lupo-memory/development/canonical/1026/04/99_limits_for_everything_and_why.toon
...
**`lupo-memory/atoms/lupopedia_global_constants.atom.toon`** -> `constants.constitutional_limits`
...
| **Forbidden** | **PRD 100** and any three-digit PRD file prefix in `lupo-docs/prd/` |
...
**Enforcement:** `python lupo-scripts/validate_prd_number.py` (repository scan). **`python lupo-scripts/generate_prd_index.py`** should surface **collisions** when two files resolve to the same numeric id.
...
- **Explicitly named scratch** tables (prefix `tmp_`, suffix `_tmp`, or documented maintenance tables in **`lupo-database/migrations_legacy/`** only when those tables are **not** part of **`install_new_lupopedia.sql`**).
...
Compare against **199** after exclusions (see **`lupo-scripts/validate_table_count.php`**).
...
| Property | Value |
|----------|-------|
| **Maximum** | **999** **seed / registry-reserved** actor ids in the **`1-999`** band (see **PRD 41**, **`lupo-database/lupopedia/actors/registry.json`**, **Reserved ID doctrine**) |
...
**Enforcement:** `php lupo-scripts/validate_actor_id.php` (CLI checks + callable helper for insert paths).
...
**Enforcement:** `python lupo-scripts/validate_lupopedia_headers_universal.py` (**`HDR_TRUST_TIER_INVALID`**).
...
**Enforcement:** `python lupo-scripts/validate_lupopedia_headers_universal.py`.
...
1. **Audit all limits** in one pass (`php lupo-scripts/check_limit_utilization.php`).
...
|-------|-------------|------------------|
| PRDs 00-99 | `lupo-scripts/validate_prd_number.py` | CI / pre-commit **fail** on illegal prefix or >99 |
| Tables <=199 | `lupo-scripts/validate_table_count.php` | Exit **non-zero**; blocks ???quiet??? schema creep when wired into CI |
| Seed actors <=999 | `lupo-scripts/validate_actor_id.php` | Exit **non-zero** on duplicate **`actor_id`** or malformed **`actors`** list; **facet** rows **>999** in **`registry.json`** are expected and not errors |
...
Reports land under **`lupo-docs/reports/`** (created on demand). **Weekly** scheduling: see **`lupo-scripts/cron_limit_utilization.sample`** for a host cron entry (operators copy to their environment; the repo does not install cron automatically).
...
- **`lupo-docs/doctrine/TRUST_LADDER_REGISTRY.md`**, **`CHRONOLOGICAL_TRUST_LADDER.md`**.
