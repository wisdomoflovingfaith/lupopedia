...
lupopedia.headers:
  ...
  file_path_from_root: "lupo-docs/audits/DOCTRINE_PRD_LINKAGE_AUDIT.md"
  web_path: "https://www.lupopedia.com/lupopedia/lupo-docs/audits/DOCTRINE_PRD_LINKAGE_AUDIT.md"
  status: "active"
  when_updated: "20260418215458"
  trust_tier: "canonical"
  questions_toon: null
  memory_toon: "lupo-memory/development/canonical/1026/04/doctrine-prd-linkage-audit.toon"
  atoms_toon: null
  ...
  content_slug: "doctrine-prd-linkage-audit"
...
Every hand-authored Markdown file under **`lupo-docs/doctrine/`** (and nested doctrine trees) **SHOULD** appear as an inbound reference from **at least one** PRD (`lupo-docs/prd/*.md`): either an explicit Markdown link, a normative "see doctrine X" pointer, or inclusion in a PRD-maintained doctrine index (**PRD 26**, **PRD 31**).
...
**Orphans** (no inbound PRD pointer found) must be **linked**, **merged**, **archived under `lupo-docs/archive/`** with justification, or **deleted** only when superseded and recorded per **[CHANGELOG_BUFFER_ARCHITECTURE.md](../doctrine/CHANGELOG_BUFFER_ARCHITECTURE.md)** (buffer entries under **`lupo-changelog-pending/`** with justification; reviewer approval where policy requires).
...
1. Enumerate doctrine Markdown: `lupo-docs/doctrine/**/*.md` (exclude generated vendor trees if any).
2. Apply **Exclusions** below; do not flag exempt paths as orphans.
3. For each remaining basename, search PRD corpus: `rg -l "FILENAME|stem" lupo-docs/prd` (manual review for renamed stems).
...
- **`lupo-docs/doctrine/templates/**/*.md`** — template examples, not normative (path reserved; may be absent).
- **`lupo-docs/doctrine/archive/**/*.md`** — historical, read-only (path reserved; may be absent).
- **`lupo-docs/doctrine/EXEMPTIONS.md`** — when present, machine-readable / human registry of additional exemptions (self-describing; not required until exemptions are centralized).
...
rg -l "TOON_ORDERING_SPEC.md" lupo-docs/prd/
```
...
A script **`lupo-scripts/audit_doctrine_orphans.py`** SHOULD:

1. Scan **`lupo-docs/doctrine/**/*.md`** minus **Exclusions**.
...
| **Next step** | **Cadence:** weekly audit (Mondays). **Owner:** LILITH (Actor 2) or WOLFIE-delegated reviewer. **Automation:** add CI job invoking **`lupo-scripts/audit_doctrine_orphans.py`** once implemented; until then run the manual method before **PRD 4.2.0** release packaging. Open remediation rows for true orphans. |

## Examples already linked from PRDs

- **`TOON_ORDERING_SPEC.md`** — cited from **PRD_INDEX** cross-cutting block and **PRD 16 / 38 / 51** (see index and those PRDs). Verify with **`rg -l "TOON_ORDERING_SPEC.md" lupo-docs/prd/`** as above.
