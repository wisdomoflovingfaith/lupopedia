...
- **Preferred stored value:** path **relative to the Lupopedia project root** (directory containing `lupo-scripts/`), **forward slashes**, no leading slash — e.g. `lupo-docs/prd/16_lupopedia_headers.md`. This is independent of the **public URL** subfolder name (`/lupopedia/`, `/wiki/`, etc.).
- **Runtime:** `ANUBIS_QueueProcessor` in `lupo-includes/classes/ANUBIS/QueueProcessor.php` resolves disk access with `LUPOPEDIA_PATH` + stored path; `addToQueue()` normalizes absolute paths under the project root to repo-relative form.
