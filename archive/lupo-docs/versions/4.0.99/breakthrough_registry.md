...
lupopedia.headers:
  ...
  when_updated: "20260412001055"
  file_path_from_root: "lupo-docs/versions/4.0.99/BREAKTHROUGH_REGISTRY.md"
  web_path: "https://www.lupopedia.com/lupopedia/lupo-docs/versions/4.0.99/BREAKTHROUGH_REGISTRY.md"
  questions_toon: null
  federation_node_id: 0
  channel_key: "development"
  trust_tier: "canonical"
  memory_key: "lupo-memory/development/canonical/1026/04/breakthrough-registry.toon"
  artifact_type: documentation
  artifact_kind: guide
  thread_id: "breakthrough-registry"
  content_id: null
  pk_id: null
  pk_slug: "breakthrough-registry"
  ...
  module: null
  dialog_transcript: "0/development/breakthrough-registry"
---
...
**TOON** files are **generated** from install SQL — do **not** hand-edit; regenerate with **`lupo-scripts/generate_toon_from_sql.py`** (or the live-DB generator) when DDL changes.
...
**Cross-links:** **PRD 38** (memory graph), **PRD 50** (coordination + content bridge), **PRD 51** (context over naïve path authority), **PRD 52** (bounded traversal); Captain’s Log — **Unified Theory** (`lupo-content/federation_node/0/captains_log/`).
...
| 9 | **Auto-verification of edges after migration** — After a file is migrated (or header written), verify the **`lupo_memory_nodes`** row for **`memory_key`** / **`content_id`** has expected **incoming** / **outgoing** **`lupo_memory_edges`**; flag **`zero_outgoing_edges`** / **`zero_incoming_edges`** / missing node as **`needs_review`**. **Tooling:** **`verify_edges_for_file()`**, **`normalize_lupopedia_md_header_25.py --verify-edges`**, CLI **`python lupo-scripts/lib/kairos_edge_verification.py --test --file <path>`**. | **KAIROS** (115) | 2026-04-11 | 1000 |
...
2. Run **`python lupo-scripts/detect_memory_graph_orphans.py --under lupo-docs/versions/4.0.99 --json`** when diagnosing header vs DB vs mirror (**Pattern #6**).
...
1. During **`install.php`** / wizard migration, for each imported entity (or batch), the installer **also**:
   - Creates or updates a **content file** in the appropriate **`lupo-docs/`** (or product-defined) location when a file-backed mirror is desired.
...
- Peel the existing leading YAML envelope, then write the new header, then the body — see **`lupo-scripts/lib/lupopedia_markdown_header_peel.py`** and usage in **`add_lupopedia_header_to_file.py`**, **`fix_double_headers.py`**, **`normalize_lupopedia_md_header_25.py`**.
...
**Logic:** Treat **`lupo-scripts/lib/header_spec_v3_1.py`** (and **PRD 16** fixed-position envelope) as the **frozen key order** for generators. Tools **must not** alphabetize or reshuffle keys when emitting YAML.
...
**`memory_key` ↔ TOON / mirror:** Header **`memory_key`** (e.g. under **`lupo-memory/...`**) pairs with **export TOON** surfaces; treat **node + mirror** as the **same logical identity** when reconciling (**Pattern #6**, **Pattern #7**).
...
**Tool:** [`lupo-scripts/detect_memory_graph_orphans.py`](../../../lupo-scripts/detect_memory_graph_orphans.py)
...
4. List active nodes (**`is_deleted = 0`**) whose **`memory_key`** starts with **`lupo-memory/`** but whose **repo-relative file** does not exist → **mirror drift** (export lag or broken mirror).
...
python lupo-scripts/detect_memory_graph_orphans.py --under lupo-docs/prd -v
python lupo-scripts/detect_memory_graph_orphans.py --no-db   # count files only; no classification
```
...
**Problem:** **Patterns #3, #5, #6** each attack **one** failure mode. Residual drift still appears across **three** surfaces: **PRD 16** header envelope, **PRD 38** **`lupo_memory_nodes`** + **`lupo_memory_edges`**, and **filesystem mirror** under **`lupo-memory/`** (export). **PRD 51** adds **inference** from graph edges to expected header fields.
...
1. **Thread anchor** — Record **channel / thread** (**PRD 02**, **PRD 17**, **`lupo-channels/`**) for **coordination**; **do not** treat thread text as overriding **DB graph** or **`content_id`** facts.
...
| 5 | **Stale edges** — Active **`lupo_memory_edges`** (**`is_deleted = 0`**) whose **to** node is **soft-deleted** (**`lupo_memory_nodes.is_deleted = 1`**), leaving graph pointers at tombstoned targets. **Detection:** **`fetch_stale_edges_to_deleted_targets()`**, CLI **`python lupo-scripts/lib/kairos_edge_verification.py --stale-edges --limit N`**. | **KAIROS** (115) | 2026-04-11 | 100 |
...
**Handling (edge #8):** Treat as **process** signal, not a validator failure. Keep **humor** artifacts in **`lupo-content/.../captains_log/`** with normal **PRD 16** headers; do **not** use **edge #8** to skip **§14.4** or **Priority 1** peel. Revisit row if **Files** catches up to **Patterns** without changing execution discipline.
...
- **SYNAPSE** (**117**) — **§3.1** **edges #9–#11** + **#15–#20** + docs **#14**–**#16**; actor registry + **`lupo-actors/117/`** — **§5** **1120**
...
| 16 | **Quick Reference integrity** — **`lupopedia_quick_reference.md`**: **§5** mirror path corrected to **`lupo-memory/{channel_key}/{trust_tier}/{display_year}/{month}/…`** (removed misleading **`lupo-memory/YYYY/MM`**); **§2.5** explicit segment order aligned with header **`memory_key`**; **trust tiers** rephrased as **authority** (**`seed` > `canonical` > `staging`**) vs promotion-into-**`seed`**; **`migrate_transcript_to_memory.py`** moved to **§6.1** legacy (not core peel/graph tooling). | **SYNAPSE** (117) | 2026-04-11 | 50 |
...
**SYNAPSE batch (2026-04-11, WOLFIE-approved):** +300 (**edges #9–#11**) +750 (**edges #15–#20**, **non-uniform** **§3.1** **Points**) +10 (**documentation #14** — **`MIGRATION_HAZARD_REMEDIATION.md`**) +10 (**documentation #15** — **§15** footer + **edge #16** description correction) +50 (**documentation #16** — **`lupopedia_quick_reference.md`** integrity: **`lupo-memory`** path shape, trust-tier authority, transcript script demotion) = **+1120** **§5** **Total**.
...
|----------|-----------|--------------|----------|--------|
| **1** | PRDs (`lupo-docs/prd/*.md`, root only) | ~60 | 0 | Pending |
| **1b** | Version tracker (**`BREAKTHROUGH_REGISTRY.md`**) | 1 | **1** | **Migrated** **2026-04-11** (**`normalize`**) |
| **2** | Doctrine (`lupo-docs/doctrine/**/*.md`) | ~30 | 0 | Pending |
| **3** | Core scripts (`lupo-scripts/*.py`) | ~50 | 0 | Pending |
| **4** | Documentation (`lupo-docs/**/*.md`) | ~500 | 0 | Pending |
...
- **Crafty 3.7.5 → Lupopedia SQL** — [`old_crafty_syntax_3_7_5_start.sql`](../../../lupo-database/lupopedia/mysql/import/old_crafty_syntax_3_7_5_start.sql) + [`import_from_old_crafty_syntax.sql`](../../../lupo-database/lupopedia/mysql/import/import_from_old_crafty_syntax.sql); AI WOLFIE write-up: [`analysis/wolfie/SUMMARY.md`](analysis/wolfie/SUMMARY.md)

### 11.1 Schema and TOON surfaces (custodian index — ANUBIS)

Use this when auditing **Pattern #3**, **Pattern #5**, **Pattern #6**, **Pattern #7** (when implemented), **Pattern #8** (registry discipline), **Pattern #9** (KAIROS edge verification), **Pattern #10** (registry graph self-seeding), **Pattern #11** (red-to-green validator pipeline), **Pattern #12** (post-migration graph completion per **§12.1**), **Pattern #13** (actor execution loop — **§2.16**), **edge #2**, **edge #4**, **edge #5**, **edge #6** (registry graph drift), **edge #7** (offline / DB-down degradation), **edge #8** (humor / execution meta), **edges #9–#11** and **#15–#20** (**SYNAPSE** / **Crafty** legacy payload + **`import_from_old_crafty_syntax.sql`**), doc **#14** (**`MIGRATION_HAZARD_REMEDIATION.md`**), or **§14** loop-backs. **Canonical DDL** lives in **`lupo-database/lupopedia/mysql/install/install_new_lupopedia.sql`**. **TOON** is a **read-only column reference** derived from that DDL (regenerate; do not hand-edit TOON).

| Surface | Path | Notes |
|--------|------|--------|
| **PRD 16** — headers / §20 migration | [`lupo-docs/prd/16_lupopedia_headers.md`](../../prd/16_lupopedia_headers.md) | Fixed-position envelope; interactive migration |
| **PRD 38** — memory unification | [`lupo-docs/prd/38_memory_unification.md`](../../prd/38_memory_unification.md) | **`lupo_memory_nodes`** / edges are **DB source of truth**; **`lupo-memory/...` mirror** is export-side |
| **PRD 51** — graph as authority for inference | [`lupo-docs/prd/51_memory_graph_as_source_of_truth.md`](../../prd/51_memory_graph_as_source_of_truth.md) | Path is a **hint**; graph + context first |
| **TOON — `lupo_memory_nodes`** | [`lupo-database/lupopedia/toon/lupo_memory_nodes.toon.json`](../../../lupo-database/lupopedia/toon/lupo_memory_nodes.toon.json) | PK **`memory_node_id`**; logical pointer **`memory_key`** (VARCHAR 255); **`status`**, **`review_reason`**, trust ladder fields |
| **TOON — `lupo_memory_edges`** | [`lupo-database/lupopedia/toon/lupo_memory_edges.toon.json`](../../../lupo-database/lupopedia/toon/lupo_memory_edges.toon.json) | Edge shape for graph verification |
| **Tool — orphan detection** | [`lupo-scripts/detect_memory_graph_orphans.py`](../../../lupo-scripts/detect_memory_graph_orphans.py) | **Pattern #6** — header **`memory_key`** vs **`is_deleted`** + mirror file probe |
| **Tool — KAIROS edge verification** | [`lupo-scripts/lib/kairos_edge_verification.py`](../../../lupo-scripts/lib/kairos_edge_verification.py) | **Pattern #9** — **`verify_edges_for_file`**, **`fetch_stale_edges_to_deleted_targets`**, CLI **`--test` / `--stale-edges`**; **`normalize_lupopedia_md_header_25.py --verify-edges`**; **edge #5** stale-edge query |
| **Mirror — Breakthrough Registry** | [`breakthrough-registry.toon`](../../../lupo-memory/development/canonical/1026/04/breakthrough-registry.toon) + [`breakthrough-registry.json`](../../../lupo-memory/development/canonical/1026/04/breakthrough-registry.json) (JSON master) | **Pattern #5** filesystem export; logical node id **20260411162345001** (Packed UTC **20260411162345**); **Pattern #10** — refresh **DB edges** after registry scoring passes (**edge #6**); **Pattern #3** ghost sync when DB is up |
| **SQL — Crafty 3.7.5 dump (sample)** | [`old_crafty_syntax_3_7_5_start.sql`](../../../lupo-database/lupopedia/mysql/import/old_crafty_syntax_3_7_5_start.sql) | Legacy **`livehelp_*`** schema + data; **row counts differ** in production exports |
| **SQL — Crafty → Lupo import** | [`import_from_old_crafty_syntax.sql`](../../../lupo-database/lupopedia/mysql/import/import_from_old_crafty_syntax.sql) | **`{{prefix}}`** (`lupo_`); **`TRUNCATE` + `INSERT … SELECT`**; idempotent guards in places; **Pattern #1**; non-scored proposals **`crafty_import_notes.md`** |
...
**PRD 38 reminder (audit):** Writes go to **`lupo_memory_nodes`** first; filesystem **`.toon` / `.json` under `lupo-memory/`** follows **MemoryExportService** — a header **`memory_key`** that only matches a **mirror path** with **no DB row** is a **ghost** risk (**edge #2**, **Pattern #3**).
...
1. **`python lupo-scripts/validate_lupopedia_headers_universal.py <path>`** — header envelope **PASS** (already required before commit; re-run if unsure).
2. **`python lupo-scripts/lib/kairos_edge_verification.py --test --file <path>`** — optional **`--json`** for CI. If **`no_active_memory_node`** → **create or refresh** **`lupo_memory_nodes`** for the header’s **`memory_key`** (**Pattern #5**).
...
- Every registered item in this registry **should** eventually have a corresponding **`lupo_memory_nodes`** row with edges to:
  ...
  - Tools that implement the pattern (**`lupo-scripts/`** peel/normalize/header helpers),
...
**Memory graph (this registry):** **WOLFIE** (1) authorized **go**. **ARA** applied **Pattern #5** (memory-graph-first): **`lupo_memory_nodes`** row **20260411162345001** (Packed UTC **20260411162345**), **`trust_tier: canonical`**, **`memory_key`** as in header; **outbound edge count is stale** until **Pattern #10** refresh after each major scoring pass (initial tranche predates patterns **#9**–**#13**, edges **#5**–**#11** / **#15**–**#20**, doc **#8**–**#15**). **Edge #6** applies until DB re-seed. **2026-04-11** **`kairos_edge_verification.py --test`**: **`no_active_memory_node`** when the row is missing — **Pattern #12** (**§12.1**) mandates node + edges **after** normalize + commit when DB is up (**edge #7** if offline). Filesystem mirror: **`lupo-memory/development/canonical/1026/04/breakthrough-registry.toon`**. **Pattern #3** ghost sync: header **`memory_key`** matches node + mirror when DB is reachable. **Edge #7:** do not claim verification **complete** when DB is down.
...
| B | What does a **Priority 1** **`normalize_lupopedia_md_header_25.py --dry-run`** show **right now**? | **2026-04-11** (packed **20260411171228** batch): **scanned=56**, **changed=1** (`lupo-docs/prd/51_memory_graph_as_source_of_truth.md`), **unchanged=55**, **skip_multi=0**, **skip_error=0**. | 2026-04-11 |
...
2. **Future-leak prohibition** — Packed UTC in headers **>** trusted current UTC (e.g. from **`python lupo-bin/tick.py`**) → **quarantine**; do not silently fix.
...
- **Mirror sanity:** If **`lupo-memory/...`** export exists, path shape is plausible — mirror **must not** be treated as SoT.
...
### 14.2 Tactical peel batch — Priority 1 root PRDs (`lupo-docs/prd/*.md`)
...
**Supported today** (`normalize_lupopedia_md_header_25.py`): **`--path`**, **`--dry-run`**, **`--check`**, **`--backup`**, **`--verbose`**, **`--target-version 4.0.99`**, **`--recursive`**, **`--include-py`**. Default glob **`lupo-docs/prd/[0-9][0-9]_*.md`** matches numbered root PRDs (same ~60 file set).
...
- [ ] **Pattern #6** — **`detect_memory_graph_orphans.py --under lupo-docs/prd`** (**DB up**); baseline understood (**edge #3**, **`missing_node`**).
...
```bash
python lupo-scripts/normalize_lupopedia_md_header_25.py --target-version 4.0.99 --dry-run --path "lupo-docs/prd/[0-9][0-9]_*.md"
```
...
**Optional — wide glob + verbose:** same executable; use **`--path "lupo-docs/prd/*.md"`** and append **`--verbose`** so each file emits **`[OK]`** / **`[DRY-RUN]`** / **`[SKIP]`**. The numeric glob above excludes **`PRD_INDEX.md`**, **`WHAT_TO_DO_NEXT.md`**, etc.; the wide glob includes them — confirm batch boundary with **WOLFIE** (**§14.4** checklist: **Glob scope**).
...
1. `python lupo-scripts/validate_lupopedia_headers_universal.py <file.md>` per file or wrap a directory walk (validator is single-path today; no **`--under`**).
2. `python lupo-scripts/detect_memory_graph_orphans.py --under lupo-docs/prd --json` when DB is up.
...
**Rollback:** `git restore lupo-docs/prd/*.md` or per-file **`*.bak`**.

**Dry-run report (versioned, not under ignored `lupo-logs/`):** [`prd_peel_dryrun_20260411.json`](prd_peel_dryrun_20260411.json) and [`prd_peel_dryrun_20260411.txt`](prd_peel_dryrun_20260411.txt).

**Pre-flight orphan baseline (`detect_memory_graph_orphans.py`, full `lupo-docs/prd` tree):** Packed UTC **`20260411154001`** — [`preflight_memory_orphans_prd_20260411154001.json`](preflight_memory_orphans_prd_20260411154001.json) (exit **1**, actionable) + [`.manifest.txt`](preflight_memory_orphans_prd_20260411154001.manifest.txt) (counts and notes). Establishes **Pattern #6** starting line before Priority 1 graph backfill.

**Version-folder pre-flight (registered):** Before heavy edits under **`lupo-docs/versions/4.0.99/`**, run **`detect_memory_graph_orphans.py --under lupo-docs/versions/4.0.99 --json`** (DB up) and archive JSON if policy requires; pairs with **edge #6** checks on **`BREAKTHROUGH_REGISTRY.md`**.
...
- **Graph truth** — **`memory_key`** resolves to intended **canonical** identity; **TOON / mirror** path under **`lupo-memory/`** understood; **no** invented node claims.
...
- A **single** root PRD under **`lupo-docs/prd/`** (e.g. already touched in dry-run: **`51_memory_graph_as_source_of_truth.md`**), **or**
...
2. **Dry-run only this file:**  
   `python lupo-scripts/normalize_lupopedia_md_header_25.py --target-version 4.0.99 --dry-run --path "lupo-docs/prd/NN_….md"`  
...
- **Archive** rows that are **Resolved** and **unchanged for 30 days** (calendar guard for log volume only) into **`lupo-docs/versions/4.0.99/COUNTERMEASURE_ARCHIVE_YYYYMM.md`** (create file with **PRD 16** header on first use). Keep **§16** tables **current**; archives are **read-only** history.
