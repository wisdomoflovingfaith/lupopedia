...
lupopedia.headers:
   ...
   file_path_from_root: "lupo-docs/versions/4.0.99/changelog.md"
   web_path: "https://www.lupopedia.com/lupopedia/lupo-docs/versions/4.0.99/changelog.md"
   questions_toon: null
   federation_node_id: 0
   channel_key: "development"
   trust_tier: "canonical"
   memory_key: "lupo-memory/development/canonical/1026/04/version-4-0-99-changelog.toon"
   artifact_type: changelog
   artifact_kind: version_specific
   thread_id: "version-4-0-99-changelog"
   title: "Changelog 4.0.99"
   status: "active"
   summary: "Complete session log for 4.0.99 header standardization and constitutional limits"
   dialog_transcript: "0/development/4_0_99_changelog"
---
...
**WHEN:** Header/footer batch UTC **`20260412001055`** (`python lupo-bin/tick.py`).
...
1. **KAIROS (`verify_edges_for_file`)** — **`lupo-scripts/lib/kairos_edge_verification.py`**: After Markdown migration, resolves **`lupo_memory_nodes`** by optional **`memory_node_id`**, else header **`content_id`**, else **`memory_key`**; counts outgoing edges by **`edge_type`** (distinct **`outgoing_edge_types`**), counts incoming; classifies **`node_status`**: **`complete`**, **`isolated`** (zero outgoing), **`incomplete`** (optional **`expected_edge_types`** not all present, case-insensitive), **`missing`**, **`deleted_only`** (soft-deleted row). CLI **`--test`** gains **`--expected-edge-types`** (comma-separated). Exit **1** only for **`missing`** / **`deleted_only`**; **`isolated`** / **`incomplete`** exit **0** with **stderr** warnings on **`--test`**.
2. **Pattern #6 integration** — **`lupo-scripts/detect_memory_graph_orphans.py`**: After header-vs-DB **`ok`**, calls **`verify_edges_for_file`**; attaches **`kairos`** dict per file row; **`missing`** / **`deleted_only`** from KAIROS set **`actionable`** (exit **1**); **`isolated`** / **`incomplete`** log **`[WARN]`** to **stderr** only; human output lists **`[kairos_*]`** lines for KAIROS-hard failures. Docstring and notes updated (three tracks; exit semantics).
3. **Normalize post-verify** — **`lupo-scripts/normalize_lupopedia_md_header_25.py`**: **`_kairos_verify_after_normalize`** branches on **`node_status`** so **`missing`**/**`deleted_only`** vs **`isolated`**/**`incomplete`** vs clean paths have appropriate **`[KAIROS]`** messaging.
...
- `lupo-scripts/lib/kairos_edge_verification.py`
- `lupo-scripts/detect_memory_graph_orphans.py`
- `lupo-scripts/normalize_lupopedia_md_header_25.py`
- `AGENTS.md`
- `lupo-docs/versions/4.0.99/CHANGELOG.md` (this entry)
- `lupo-docs/versions/4.0.99/status/session_report_20260412_eod_kairos_pattern6_agents_cursor102.md` (session / EOD report)
...
**WHEN:** Packed UTC **`20260412023225`** — wall clock **2026-04-12 02:32:25** UTC (`python lupo-bin/tick.py` for this documentation batch).
...
- **Validation:** `python lupo-scripts/validate_lupopedia_headers_universal.py` on touched Python modules and **`AGENTS.md`** (**PASS** without **`--strict-memory-pair`** for **`AGENTS.md`** — sidecar **`.toon`** at **`memory_key`** may still be created in a follow-up for strict pairing).
...
3. **Filesystem + memory alignment:** Renamed canonical **JSON/TOON** pairs under **`lupo-memory/development/canonical/2026/04/`** (e.g. **`02-data-model` → `70-data-model`**), renamed **`lupo-memory/headers/prd/2026/04/*.metadata.json`**, **`lupo-memory/install/seed/41-install-seed-doctrine.toon` → `79-install-seed-doctrine.toon`**, updated **`1026/04`** sidecars for database doctrine; bulk **`lupo-docs/prd/*.md`** path replacements across the tree where safe.
4. **Index + tooling:** Regenerated **`lupo-docs/prd/PRD_INDEX.md`** via **`generate_prd_index.py`**, restored **PRD 16**-dense front matter, fixed **HDR_EMPTY_BODY** (no blank-only line immediately after closing `---`); patched generator to emit a single newline after the header fence; updated **`README.md`**, **`07_agents_faucets.md`**, **`lupo-docs/prd/decisions/pseudocode/THREAD_INDEX.md`**, constitution **`.pseudo.md`** filenames (→ **`70_*` … `79_*`**) + YAML paths; **`migrate_top_prds_v3.py`**, **`generate_phase2_prd_memory_json.py`** filename lists.

**WHERE:** `lupo-docs/prd/` (renamed **11** PRD files + **PRD_INDEX**), `lupo-memory/development/canonical/2026/04/`, `lupo-memory/headers/prd/2026/04/`, `lupo-memory/install/seed/`, `lupo-memory/development/canonical/1026/04/`, `lupo-docs/prd/decisions/pseudocode/`, `README.md`, `lupo-scripts/generate_prd_index.py`, `lupo-scripts/migrate_top_prds_v3.py`, `lupo-scripts/generate_phase2_prd_memory_json.py`, `lupo-docs/versions/4.0.99/CHANGELOG.md` (this entry), `lupo-docs/versions/4.0.99/status/session_report_20260412_eod_prd_renumber_cursor102.md`.
...
**WHEN:** Packed UTC **`20260412134809`** — wall clock **2026-04-12 13:48:09** UTC (`python lupo-bin/tick.py` from repo root for this documentation batch).
...
- **Memory rows:** Python one-shot patched **`metadata[].prd_id`** / **`source_path`** in JSON and synced the **TOON** metadata line; bulk **`str.replace`** walk for **`lupo-docs/prd/<old>.md` → `<new>.md`** (scoped extensions).
...
**WHERE:**
- **PRD Updates:** `lupo-docs/prd/72_tags_metadata.md`, `lupo-docs/prd/73_collections_navigation.md`, `lupo-docs/prd/43_parent_child_trust_ladder.md`, `lupo-docs/prd/38_memory_unification.md`, `lupo-docs/prd/51_memory_graph_as_source_of_truth.md`
- **New Services:** `app/Services/MemoryExportService.php`, `app/Services/CollectionMemoryService.php`
- **Documentation:** `lupo-docs/versions/4.0.99/CHANGELOG.md`
...
**Files Created:**
- `lupo-scripts/generate_memory_from_header.py` - Memory sidecar generator (.json + .toon)
- `lupo-scripts/validate_table_count.php` - Database table count validator (<=199)
- `lupo-scripts/validate_actor_id.php` - Seeded actor validator (1-999)
- `lupo-scripts/validate_trust_ladder_pk.php` - PK year offset validator
- `lupo-scripts/backfill_canonical_offsets.php` - Staging→canonical PK migration
- `lupo-scripts/check_limit_utilization.php` - Constitutional limits dashboard
- `lupo-includes/classes/TrustLadderQueryHelper.php` - PK band abstraction trait
- `lupo-docs/prd/99_limits_for_everything_and_why.md` - Constitutional limits PRD

**Files Updated (v4.0.99 header standardization):**
- `lupo-scripts/validate_lupopedia_headers_universal.py` - Added `--strict-memory-files`
- `lupo-scripts/add_lupopedia_header_to_file.py` - Added `--skip-memory-sidecar`, `--force-memory`
- `lupo-scripts/add_lupopedia_headers_everywhere.py` - Batch header adder

**Documentation Updated:**
- `lupo-docs/prd/16_lupopedia_headers.md` - v4.0.99 dense envelope, PK fields, orphan doctrine
- `lupo-docs/prd/99_limits_for_everything_and_why.md` - Complete constitutional limits
- `lupo-docs/doctrine/TRUST_LADDER_REGISTRY.md` - PK offset validation section
- `lupo-docs/doctrine/CHRONOLOGICAL_TRUST_LADDER.md` - §2.2 PK offset rule
- `lupo-docs/prd/PRD_INDEX.md` - Regenerated (65 PRDs indexed)
...
- READ LOG: added DialogMvpService::updateReadLog() — tracks last_read_message_id per (actor, channel, thread).
...
- **Database Table Report**: Created `lupo-docs/versions/4.0.99/status/gemni_database_table_report.md`.
- **Install SQL Alignment**: Updated `lupo-database/lupopedia/mysql/install/install_new_lupopedia.sql` to match live schema:
...
- **Task Management System**:
  ...
  - API Endpoints: Created `create.php`, `update.php`, `list.php`, `next.php` in `lupo-api/v1/tasks/`.
...
### WHERE
- `lupo-docs/versions/4.0.99/status/gemni_database_table_report.md`
- `lupo-database/lupopedia/mysql/install/install_new_lupopedia.sql`
- `lupo-includes/classes/TaskService.php`
- `lupo-api/v1/tasks/`
- `channels/index.php`
...
### WHERE
- `lupo-docs/versions/4.0.99/changelog.md`
- `lupo-docs/versions/4.0.99/archive/changelog_archive.md`
- `lupo-docs/versions/4.0.99/status/todo.md`
- `lupo-docs/versions/4.0.99/status/plan.md`
...
- `lupo-scripts/lib/header_spec_v3_1.py` — `V4_HEADER_KEYS_ORDERED` updated; `LEGACY_KEYS_V4` mapping added
- `lupo-scripts/validate_lupopedia_headers_universal.py` — added `validate_questions_toon()`, `warn_legacy_last_modified_utc()`; removed timestamp equality check; added `HDR_QUESTIONS_TOON_SUFFIX`, `HDR_LAST_MODIFIED_RENAMED` error/warn codes; added `questions_toon` to null-allowlist so `null` is always valid
- `lupo-scripts/add_lupopedia_header_to_file.py` — template updated to `questions_toon: null`
- `lupo-scripts/fix_double_headers.py` — tie-break logic updated to use `when_updated` only
- `lupo-scripts/lib/lupopedia_markdown_header_peel.py` — legacy regex note added
- `lupo-scripts/rename_last_modified_to_questions_toon.py` — NEW migration script (batch rename for `.py` own-headers)

*Doctrine and spec files updated:*
- `lupo-docs/prd/16_lupopedia_headers.md` — §4.2 field 6 definition; §3 new definitions (`questions_toon`, Q&A TOON); §14.9 tie-break description; all 11 example YAML occurrences; new §19 Q&A System (19.1–19.5: purpose, planned `.questions.toon` JSON structure, path convention, validator rules, 3-phase migration plan); new changelog row
- `lupo-docs/doctrine/lupopedia-headers/lupopedia_headers_format.md` — field 6 table row; all 6 example occurrences
- `lupo-docs/doctrine/lupopedia-headers/validators_and_tooling.md` — timestamps bullet; `questions_toon` bullet; tooling table row
...
- All 66 `lupo-docs/prd/*.md` header field 6 migrated from `last_modified_utc: "TIMESTAMP"` → `questions_toon: null` (within lines 1–25 only; body references preserved)
- `for_auggie.md` — added complete 25-line v4.0.99 header (was an orphan)
- `lupo-docs/versions/4.0.99/changelog.md` — own-header field 6 migrated
- `lupo-docs/versions/4.0.99/status/open_questions.md` — own-header field 6 migrated
...
- `lupo-scripts/fix_double_headers.py`
- `lupo-scripts/lib/lupopedia_markdown_header_peel.py`
- `lupo-scripts/rename_last_modified_to_questions_toon.py` (NEW)
- All 66 `lupo-docs/prd/*.md` (header field 6 only)
- `for_auggie.md`, `for_gemini.md`
- `lupo-docs/versions/4.0.99/changelog.md`, `status/open_questions.md`
...
- `lupo-scripts/lib/header_spec_v3_1.py` — `V4_HEADER_KEYS_ORDERED` field 6; `LEGACY_KEYS_V4` mapping; `QUESTIONS_TOON_SUFFIX` constant
- `lupo-scripts/lib/header_validation.py` — backward-compat check for `last_modified_utc` (`HDR_LAST_MODIFIED_RENAMED`); `questions_toon` null/path/collision validation
- `lupo-scripts/lib/header_db_sync.py` — migration logic: delete `last_modified_utc`, populate `questions_toon: null` on encounter
- `lupo-scripts/lib/lupopedia_markdown_header_peel.py` — `_LAST_MOD_RE` kept for legacy tie-break; docstrings updated
- `lupo-scripts/import_content.py` — `_parse_last_modified_utc` renamed to `_parse_file_last_modified_utc(headers, when_updated_fallback)`; `when_updated` used as primary fallback for `file_last_modified_utc` DB column; stale `last_modified_utc` block removed
- `lupo-scripts/import_filesystem_channels_to_db.py` — `when_updated` as primary fallback at all 8 read sites; Phase 3 removal comments
- `lupo-scripts/import_context.py` — backward-compat comment added to timestamp validation loop
- `lupo-scripts/validate_lupopedia_headers.py` — `REQUIRED_HEADER_FIELDS_V4_0_99` updated
- `lupo-scripts/new_validate_lupopedia_headers_universal.py` — `required_keys` list updated; `questions_toon` case added to value validator; `last_modified_utc` kept as backward-compat check
- `lupo-scripts/validate_pseudocode_discipline.py` — required field check accepts `questions_toon:` OR legacy `last_modified_utc:`
- `lupo-scripts/regenerate_headers_for_stale_files.py` — migrates `last_modified_utc` → `questions_toon: null` on encounter
- `lupo-scripts/update_stale_headers.py` — pattern `last_modified_utc: "TIMESTAMP"` → `questions_toon: null`
...
- `lupo-tests/unit/test_header_validators.py` — obsolete ordering violation test removed; legacy test preserved for backward compat

**New doctrine file:**
- `lupo-docs/doctrine/lupopedia-headers/questions_toon_schema.md` — complete JSON Schema draft-07 for `.questions.toon` files; path convention; validation rules table; implementation status checklist
...
### WHERE
- `lupo-scripts/lib/` (4 library files)
- `lupo-scripts/import_*.py` (3 import scripts)
- `lupo-scripts/validate_*.py` (3 validator scripts)
- `lupo-scripts/generate_*.py`, `lupo-scripts/fix_*.py`, `lupo-scripts/normalize_*.py`, `lupo-scripts/scaffold_*.py`, `lupo-scripts/structural_*.py`, `lupo-scripts/full_system_sync_*.py` (10 generator/repair scripts)
- `lupo-tests/unit/test_header_validators.py`
- `lupo-docs/doctrine/lupopedia-headers/questions_toon_schema.md` (NEW)
...
**WHERE:**
- `lupopedia/lupo-scripts/lib/header_spec_v3_1.py`
- `lupopedia/lupo-scripts/validate_lupopedia_headers_universal.py`
...
**HOW:**
- Read PRD 16 (`lupo-docs/prd/16_lupopedia_headers.md`) and `lupo-memory/atoms/lupopedia_global_constants.atom.toon` to confirm the 22-field canonical v4.1.1 order
...
- Verified: `lupo-docs/prd/16_lupopedia_headers.md` → `[PASS]`; `AGENTS.md` (pk_* file) → `[PASS]` with `HDR_PK_LEGACY_ALIAS` warnings
