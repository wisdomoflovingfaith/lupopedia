...
lupopedia.headers:
  ...
  file_path_from_root: "lupo-docs/versions/4.0.99/status/lupopedia_header_review.md"
  web_path: "https://www.lupopedia.com/lupopedia/lupo-docs/versions/4.0.99/status/lupopedia_header_review.md"
  questions_toon: null
  federation_node_id: 0
  channel_key: "development"
  trust_tier: "staging"
  memory_key: "lupo-memory/development/staging/2026/04/lupopedia-header-review.toon"
...
**Date:** 2026-04-15 UTC
**Scope:** PRD 16 v4.0.99 (`lupo-docs/prd/16_lupopedia_headers.md`, 1532 lines)
...
`lupo-scripts/validate_lupopedia_headers_universal.py` **EXISTS and works** (2797 lines). It handles `.md`, `.py`, `.php`, and `.js` correctly. It was validated by running it on 10 PRD files and on itself.
...
| `lupo-bin/transcript.py` | ✅ EXISTS | POSTs to PHP API endpoint |
...
- `lupo-includes/classes/TranscriptAppendService.php` (10,557 bytes, 2026-04-10)
- `lupo-includes/modules/api/transcript-api.php` (5,685 bytes, 2026-04-10)
- `lupo-bin/transcript.py` POSTs to `{base_url}/index.php?route=api/transcript/append` ✅

**However:** Two JSONL files in `lupo-channels/` still show write activity dated **2026-04-09**, which is post-freeze per §9:
- `lupo-channels/0/development/general/transcript.jsonl`
- `lupo-channels/0/development/prd_files/44_prd_discussion/transcript.jsonl`

### 1.4 Sidecar JSON Files

`lupo-memory/headers/prd/2026/04/` contains sidecar JSON files. The system is partially deployed for recently authored PRDs. No automated sidecar-creation for older PRDs confirmed. **Naming quirk:** Sidecars are named `16_lupopedia_headersmetadata.json` (no dot before `metadata`) instead of `16_lupopedia_headers.metadata.json`. Consistent across files but deviates from the spec's stated path formula.

### 1.5 Memory (.toon) Files

`.toon` files exist in `lupo-memory/development/canonical/1026/04/` — confirming that **year-offset encoding (calendar − 1000 = 1026) is correct** for newly generated canonical files. The `header_spec_v3_1.py` library correctly declares `memory_key: "lupo-memory/development/canonical/1026/04/header-spec-v3-1.toon"`. New tooling honors §8.1.
...
`lupo-docs/doctrine/lupopedia-headers/` **EXISTS** (lowercase). Contains 11 documents: `lupopedia_headers_format.md`, `templates_new_file.md`, `validators_and_tooling.md`, `verification_guide.md`, `versioning_model.md`, `lupopedia_headers_doctrine.md`, `lupopedia_headers_migration.md`, `memory_file_schema.md`, `taxonomy_reference.md`, `optional_blocks.md`, `readme.md`. `lupo-docs/doctrine/TOON_ORDERING_SPEC.md` ✅ also exists.
...
**This is the #1 operational issue in the repo.** All 66 PRD files in `lupo-docs/prd/` fail validation.

**Root cause:** When `trust_tier` was changed from `staging` → `canonical` across PRDs, the `memory_key` paths were not updated. The path segment for `trust_tier` in `memory_key` still says `staging`. Additionally, PRD 16 itself has `channel_key: headers` but `memory_key: lupo-memory/development/...` — the channel_key segment is wrong too.
...
- `lupo-docs/doctrine/LUPOPEDIA_HEADERS/TEMPLATES_NEW_FILE.md`
...
`lupo-config/offline_transcript_queue.jsonl` does **not** exist. **This is good** — the PHP endpoint has been reliably reachable and the offline fallback was never triggered. No action needed.
...
| `lupo-bin/transcript.py` | §9 | ✅ EXISTS |
| `lupo-bin/tick.py` | cursor rule | ✅ EXISTS |
| `lupo-bin/echo_anchor_utc.py` | cursor rule | ✅ EXISTS |
...
- Action: Update `memory_key` to `lupo-memory/{channel_key}/{trust_tier}/...` for all 66 PRDs
- Fix PRD 16 specifically: `channel_key: headers` requires `lupo-memory/headers/canonical/1026/04/16-lupopedia-headers.toon`
...
- Current: `channel_key: headers`, `memory_key: lupo-memory/development/staging/...`
- Required: `memory_key: lupo-memory/headers/canonical/1026/04/16-lupopedia-headers.toon`
...
- Derived path formula: `lupo-memory/headers/{artifact_type}/{YYYY}/{MM}/{stem}.metadata.json`
...
**P1-C: Stop writing JSONL in lupo-channels/**
...
lupopedia.headers:
  ...
  channel_key: "development"
  trust_tier: "staging"
  memory_key: "lupo-memory/development/staging/YYYY/MM/file-slug.toon"
  artifact_type: documentation
  artifact_kind: guide
  thread_id: ''
  content_id: null
...
| Stop JSONL writes in lupo-channels/ | 2–4 hours |
