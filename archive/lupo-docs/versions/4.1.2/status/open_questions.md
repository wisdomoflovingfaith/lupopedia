...
lupopedia.headers:
  ...
  file_path_from_root: "lupo-docs/versions/4.1.2/status/open_questions.md"
  web_path: "https://www.lupopedia.com/lupopedia/lupo-docs/versions/4.1.2/status/open_questions.md"
  status: "active"
  when_updated: "20260417114334"
  trust_tier: "canonical"
  questions_toon: null
  memory_toon: "lupo-memory/development/canonical/1026/04/version-4-1-2-open-questions.toon"
  atoms_toon: null
...
Rolled over from `lupo-docs/versions/4.1.0/open_questions.md` on `20260415180000`.  
...
> **Reference:** See `lupo-docs/versions/4.1.0/open_questions.md` for full text of OQ-01 through OQ-40.
...
- OQ-11: `lupo-tasks-list` element missing from HTML
...
- OQ-18: `lupo-layers.js` dependency — undocumented
...
**WHEN:** 20260415180000
**WHO:** AUGGIE
**AREA:** lupo-scripts/add_lupopedia_header_to_file.py, lupo-scripts/generate_memory_from_header.py, lupo-scripts/lib/lupopedia_markdown_header_peel.py
**STATUS:** open
...
**WHEN:** 20260415200000
**WHO:** AUGGIE
**AREA:** lupo-memory/development/canonical/1026/04/
**STATUS:** open
...
|---|---|---|
| `lupo-docs/versions/4.1.2/README.md` | `version-4-1-2-readme.toon` | ❌ |
| `lupo-docs/versions/4.1.2/TODO.md` | `version-4-1-2-todo.toon` | ❌ |
| `lupo-docs/versions/4.1.2/status/open_questions.md` | `version-4-1-2-open-questions.toon` | ❌ |
| `lupo-scripts/migrate_pk_to_content_fields.py` | `migrate-pk-to-content-fields.toon` | ❌ |
...
**WHEN:** 20260415200000
**WHO:** AUGGIE
**AREA:** lupo-docs/versions/4.1.2/ (all four version docs)
**STATUS:** open
...
By contrast, `lupo-scripts/migrate_pk_to_content_fields.py` has `content_parent_id: "16"` and will correctly produce an edge to PRD 16 when its sidecar is generated.
...
- Generated sidecars for all four missing targets:
  - `lupo-docs/versions/4.1.2/README.md`
  - `lupo-docs/versions/4.1.2/TODO.md`
  - `lupo-docs/versions/4.1.2/status/open_questions.md`
  - `lupo-scripts/migrate_pk_to_content_fields.py`
...
- Updated `content_parent_id` from `null` to `16` in:
  ...
  - `lupo-docs/versions/4.1.2/TODO.md`
  - `lupo-docs/versions/4.1.2/status/open_questions.md`
  - `lupo-docs/versions/4.1.2/CHANGELOG.md`
- Regenerated sidecars with `--force`.
- Confirmed sidecars now include outbound references to:
  - `lupo-docs/prd/16_lupopedia_headers.md` (`type: references`, `reason: content_parent_id -> PRD file`)
...
**AREA:** `lupo-docs/prd/16_lupopedia_headers.md` §11 vs §15.4; `validate_lupopedia_headers_universal.py`  
**STATUS:** resolved  
...
**WHEN:** 20260415213745
**WHO:** LILITH (audit) / Cursor (actor_id 102) execution
**AREA:** PRD 16 §4.2 field 11; `lupo-docs/versions/4.1.2/` headers
**STATUS:** resolved
...
- **Observed evidence:** `lupo-scripts/generate_toon_files.py` only runs `DESCRIBE`/`SHOW INDEX` and writes JSON/TOON schema files; no INSERT/UPDATE logic exists.
...
**AREA:** lupo-docs/prd/02_channels_discussions.md; proposed `lupo_operator_scratchpad` table
**STATUS:** open
...
2. A flat file per session (e.g., `lupo-content/scratchpad/WOLFIE/YYYYMMDD.md`) — simpler, no migration cost
...
**WHEN:** 20260415224500
**WHO:** AUGGIE (gap review — auggie_sticky_note_channel_review.md)
**AREA:** lupo-docs/prd/02_channels_discussions.md; channel model
**STATUS:** open
...
**WHEN:** 20260416120000
**WHO:** AUGGIE (PRD 02 update + Q&A session)
**AREA:** `lupo-memory/atoms/lupopedia_global_constants.atom.toon`; `lupo_actors`; blog
**STATUS:** resolved
...
**AREA:** `lupo-channels/0/translation/concepts/`; `lupo-docs/doctrine/system/TRANSLATION_MODEL.md`
**STATUS:** open
...
**WHY THIS MATTERS:**
Impact **HIGH**. Helen report cycle documented roughly five hours to acceptance; primary drivers were explanation rewriting, terminology confusion, and missing reusable communication layer until late in the cycle. See `lupo-docs/versions/4.1.2/status/weekly_report_lessons_learned_20260416.md`.

**OPTIONS:**
1. Codify checklist in `lupo-docs/versions/4.1.2/status/` (link from AGENTS.md or channel README)
...
**AREA:** `lupo-scripts/import_memory_edges_from_sidecar.py` ~L319; `lupo-scripts/lib/db_memory_writer.py` ~L148
...
**WHEN:** 20260417110000
**WHO:** Claude Code (actor_id 116) — memory channel separation pass
**AREA:** `lupo-scripts/generate_toon_files.py`; `lupo_memory_nodes`
**STATUS:** implementation_complete_pending_verification

**QUESTION:**
`generate_toon_files.py` was confirmed as a schema documentation exporter (reads `DESCRIBE`/`SHOW INDEX`, writes to `lupo-database/lupopedia/`). It does NOT write to `lupo_memory_nodes`. "Rerun successfully" after the live DB migration confirms schema docs regenerated — not that memory node rows were populated with `channel_key`. Is there any other script that populates existing `lupo_memory_nodes` rows with `channel_key` retroactively, or does backfill require a new migration script?
...
**QUESTION:**
PRD 38 §6.1 defines MemoryExportService and its Type B path (`lupo-memory/{YYYY}/{MM}/{slug}.json`). Is this service implemented, stubbed, or only doctrinal? If implemented, which file and class? If only doctrinal, is the path spec binding on any current script that outputs `.json` memory artifacts?
...
3. Accept that `.json` exports in `lupo-memory/` are informally Type B until service exists
...
**WHEN:** 20260417110000
**WHO:** Claude Code (actor_id 116) — memory channel separation pass
**AREA:** `lupo-docs/prd/38_memory_unification.md` header; `trust_tier` field
**STATUS:** open
...
**WHEN:** 20260417110000
**WHO:** Claude Code (actor_id 116) — memory channel separation pass
**AREA:** `lupo-channels/registry.json`; PRD 38 cross-channel doctrine
**STATUS:** open

**QUESTION:**
Gap 3 defines the required JSON shape for `allowed_cross_channel_memory` in `lupo-channels/registry.json`. The shape is specified but no agent loading code reads it and no validator enforces it. Who is the designated enforcer — a validator flag, a runtime agent, THOTH, or an import-time check?
...
**AREA:** `lupo-docs/prd/16_lupopedia_headers.md`; `16_lupopedia_headers_examples.md`; `16_lupopedia_headers_migration.md`
**STATUS:** open
...
**AREA:** `lupo-scripts/validate_lupopedia_headers_universal.py`; PRD 16 §10
**STATUS:** open
...
**AREA:** `lupo-scripts/lib/db_memory_writer.py`; `lupo-scripts/import_memory_edges_from_sidecar.py`
**STATUS:** implementation_complete_pending_verification
...
**AREA:** `lupo-docs/versions/4.1.2/status/open_questions.md` header
**STATUS:** resolved
...
**WHY THIS MATTERS:**
If reconstruction falls back to path parsing without strict validation, edge cases where a file resides in a default directory (e.g. `lupo-docs/`) but declares a specific channel might be incorrectly mapped or overwrite correct metadata, causing loss of channel boundaries.
...
**AREA:** `lupo-scripts/lib/channel_utils.py`, `lupo-scripts/lib/db_memory_writer.py`, `lupo-scripts/import_memory_edges_from_sidecar.py`
**STATUS:** implementation_complete_pending_verification
...
**WHEN:** 20260418144640
**WHO:** Cursor
**AREA:** lupo-agents/rose/properties.json
**STATUS:** open

**QUESTION:**
`lupo-agents/rose/properties.json` still contains empathetic marketing strings that contradict the new "technical orchestration only" doctrine. Should these be aligned or deprecated if the runtime loads them?
