...
lupopedia.headers:
  ...
  file_path_from_root: "lupo-docs/versions/4.1.3/PLAN_DATABASE_MIGRATION.md"
  web_path: "https://www.lupopedia.com/lupopedia/lupo-docs/versions/4.1.3/PLAN_DATABASE_MIGRATION.md"
  status: "active"
  when_updated: "20260419215000"
  trust_tier: "canonical"
  questions_toon: null
  memory_toon: "lupo-memory/database/canonical/1026/04/database-migration-plan.toon"
  atoms_toon: null
  transcript_jsonl: "0/database/plan-database-migration"
  ...
  content_slug: "plan-database-migration"
  default_collection_id: null
  lupopedia.schema: documentation
  title: "PLAN -- Database migration plan (Crafty 3.7.5 to Lupopedia)"
  summary: "Full mirror of TODO_DATABASE_MIGRATION.md; edit both files together when the database lane plan changes."
---
# DATABASE MIGRATION PLAN -- CRAFTY SYNTAX -> LUPOPEDIA

**Sync rule:** Keep this file substantively aligned with **`lupo-docs/versions/4.1.3/TODO_DATABASE_MIGRATION.md`** except for this header block and the **PLAN** title line. Note: PLAN uses "MIGRATED (DEV IMPORT -- NOT PRODUCTION SAFE)" where TODO uses "MAPPED (DEV IMPORT)"; the intent is identical.

**Scope:** `lupo-database/lupopedia/mysql/import/import_from_old_crafty_syntax.sql` (dev import) vs `lupo-database/lupopedia/mysql/install/install_new_lupopedia.sql` (canonical DDL) vs `lupo-database/lupopedia/mysql/import/old_crafty_syntax_3_7_5_start.sql` (legacy Crafty 3.7.5 schema).
...
|-----------------------------|--------|----------------|
| `livehelp_autoinvite` | **MIGRATED (DEV IMPORT -- NOT PRODUCTION SAFE)** | `{{prefix}}crafty_syntax_auto_invite` |
...
| `livehelp_departments` | **MIGRATED (DEV IMPORT -- NOT PRODUCTION SAFE)** | `{{prefix}}departments`, `{{prefix}}department_metadata` |
| `livehelp_emailque` | **NOT MIGRATED** | Comment: target out of scope |
| `livehelp_emails` | **MIGRATED (DEV IMPORT -- NOT PRODUCTION SAFE)** | `{{prefix}}crm_lead_messages` |
...
| `livehelp_layerinvites` | **MIGRATED (DEV IMPORT -- NOT PRODUCTION SAFE)** | `{{prefix}}crafty_syntax_layer_invites` |
| `livehelp_leads` | **MIGRATED (DEV IMPORT -- NOT PRODUCTION SAFE)** | `{{prefix}}crm_leads` |
| `livehelp_leavemessage` | **MIGRATED (DEV IMPORT -- NOT PRODUCTION SAFE)** | `{{prefix}}crafty_syntax_leave_message` |
...
| `livehelp_operator_departments` | **MIGRATED (DEV IMPORT -- NOT PRODUCTION SAFE)** | `{{prefix}}actor_departments` (with post-import UPDATE wiring) |
| `livehelp_operator_history` | **MIGRATED (DEV IMPORT -- NOT PRODUCTION SAFE)** | `{{prefix}}audit_log` |
| `livehelp_paths_firsts` / `livehelp_paths_monthly` | **MIGRATED (DEV IMPORT -- NOT PRODUCTION SAFE)** | `{{prefix}}paths` (script contains duplicate comment blocks; verify single coherent mapping in SQL review) |
| `livehelp_qa` | **MIGRATED (DEV IMPORT -- NOT PRODUCTION SAFE)** | `{{prefix}}truth_questions` / `{{prefix}}truth_answers` (split across two target tables) |
| `livehelp_questions` | **MIGRATED (DEV IMPORT -- NOT PRODUCTION SAFE)** | `{{prefix}}crafty_syntax_chat_questions` |
| `livehelp_quick` | **MIGRATED (DEV IMPORT -- NOT PRODUCTION SAFE)** | `{{prefix}}actor_reply_templates` |
| `livehelp_referers_daily` / `livehelp_referers_monthly` | **MIGRATED (DEV IMPORT -- NOT PRODUCTION SAFE)** | `{{prefix}}referers` (per script comments) |
| `livehelp_sessions` | **DROPPED** | Per script comment |
| `livehelp_smilies` | **DROPPED** | Replaced by token system per table comment |
| `livehelp_transcripts` | **MIGRATED (DEV IMPORT -- NOT PRODUCTION SAFE)** | `{{prefix}}dialog_threads` + `{{prefix}}dialog_messages` (split across two target tables) |
| `livehelp_users` | **MIGRATED (DEV IMPORT -- NOT PRODUCTION SAFE)** | `{{prefix}}auth_users` + `{{prefix}}actors` (split; auth_user_id linkage repaired by cascade 20260419203000) |
| `livehelp_visits_daily` / `livehelp_visits_monthly` / `livehelp_visit_track` | **MIGRATED (DEV IMPORT -- NOT PRODUCTION SAFE)** | `{{prefix}}visits` |
| `livehelp_websites` | **MIGRATED (DEV IMPORT -- NOT PRODUCTION SAFE)** | `{{prefix}}federation_nodes` |
...
6. **Vendor SQL elimination (phased):** Phase A: catalog every `JSON_OBJECT`, `ROW_NUMBER`, `ON DUPLICATE KEY`, `REGEXP`. Phase B: move transforms to **Python importer** per table group. Phase C: leave a **minimal MySQL-only** dev script only if explicitly approved.
...
9. **TOON and table docs:** Regenerate or hand-update TOON exports from install after DDL changes; update **`lupo-docs/database/lupopedia/tables/`** companions per **PRD 13** implementer obligation.
