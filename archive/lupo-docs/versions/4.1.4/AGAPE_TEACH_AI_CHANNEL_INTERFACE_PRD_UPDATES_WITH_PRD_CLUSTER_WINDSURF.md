...
lupopedia.headers:
  ...
  file_path_from_root: "lupo-docs/versions/4.1.4/AGAPE_TEACH_AI_CHANNEL_INTERFACE_PRD_UPDATES_WITH_PRD_CLUSTER_WINDSURF.md"
  web_path: "https://www.lupopedia.com/lupopedia/lupo-docs/versions/4.1.4/AGAPE_TEACH_AI_CHANNEL_INTERFACE_PRD_UPDATES_WITH_PRD_CLUSTER_WINDSURF.md"
  status: "active"
...
1. **Actor Identity**
   ...
   **PROPOSED (UNVERIFIED — NOT IN PRD):**
   STATUS: UNCONFIRMED
   - AGAPE is an AGENT (template in lupo-agents/agape/)
   ...
   - Pattern: lupo-agents/agape/ (template) → runtime actor_id (per WHY file) → joins thread → teaches → completes
...
2. **Event System**
   ...
   **PROPOSED (UNVERIFIED — NOT IN PRD):**
   ...
   - When PHP code inserts a new file path into recent files that matches `lupo-docs/why/` pattern, this triggers AGAPE
