...
lupopedia.headers:
  ...
  file_path_from_root: "lupo-docs/versions/4.1.4/TODO.md"
  web_path: "https://www.lupopedia.com/lupopedia/lupo-docs/versions/4.1.4/TODO.md"
  status: "active"
  when_updated: "20260421124200"
  trust_tier: "canonical"
  questions_toon: null
  memory_toon: "lupo-memory/development/canonical/1026/04/version-4-1-4-todo.toon"
  atoms_toon: null
  ...
  content_slug: "version-4-1-4-todo"
  default_collection_id: null
  lupopedia.schema: "version-doc"
  title: "Lupopedia 4.1.4 TODO -- Crafty Syntax Features & Human-only Live Help"
  summary: "4.1.4 focuses on completing Crafty Syntax features, human-only live help, and preparing for May 1 deadline. Multi-AI orchestration deferred to maintain focus on delivery."
---
lupopedia.edges:
  outbound_edges: []
...
. . . . ############# . . . . .--------------------------------------------------------------.
<!-- /ASCII_ART_BLOCK -->
# TODO -- Lupopedia 4.1.4 (Target: Stable before May 1, 2026)

**Scope of 4.1.4:** Complete Crafty Syntax 3.7.5 live-help features with human-only support.  
...
### 4.1.4+ Development Items (Deferred)
...
- [ ] Freeze 4.1.4 once foundation is solid
...
- **2026-04-18 ~03:57 UTC** -- EOD handoff: channels **dark channel-sidecar** (right rail); **`?api_dashboard=1`** uses **`lupo-templates/admin/api_dashboard.php`** partial + **`admin_layout.php`**; **PRD 45** template-first staged UI workflow + **PRD_INDEX** / **PRD 02** cross-links; captain log **LUPOPEDIA headers** (`20260418_apple2e_2_supercomputer.md`); changelog buffer consolidated into [`lupo-docs/versions/4.1.3/changelog.md`](lupo-docs/versions/4.1.3/changelog.md). **Next:** unify task surfaces (HERMES `lupopedia_dialog_pending_tasks` vs `TaskService` / `lupopedia_tasks`); optional **PRD 45** implementation mirror once **`lupo-docs/implementations/_template`** exists; resume channels / prompt library follow-through.
- **2026-04-12 13:48 UTC** -- PRD **core 01-49** vs **secondary 70-99** renumber; **Database Design Doctrine** -> **PRD 80**; **`PRD_INDEX`** + memory + pseudocode. Changelog: [`lupo-docs/versions/4.0.99/CHANGELOG.md`](lupo-docs/versions/4.0.99/CHANGELOG.md) (**`[2026-04-12 13:48 UTC]`**). Status: [`lupo-docs/versions/4.0.99/status/session_report_20260412_eod_prd_renumber_cursor102.md`](lupo-docs/versions/4.0.99/status/session_report_20260412_eod_prd_renumber_cursor102.md). Version backlog: [`lupo-docs/versions/4.0.99/TODO.md`](lupo-docs/versions/4.0.99/TODO.md) / [`PLAN.md`](lupo-docs/versions/4.0.99/PLAN.md).
- **2026-04-12 02:32 UTC** -- Version changelog: [`lupo-docs/versions/4.0.99/CHANGELOG.md`](lupo-docs/versions/4.0.99/CHANGELOG.md) (KAIROS **`node_status`** + Pattern **#6** hook + **`AGENTS.md`** v4.0.99 header). Status: [`lupo-docs/versions/4.0.99/status/session_report_20260412_eod_kairos_pattern6_agents_cursor102.md`](lupo-docs/versions/4.0.99/status/session_report_20260412_eod_kairos_pattern6_agents_cursor102.md).

### 2026-04-16 HERMES Task Queue Follow-up

- [x] `lupo-bin/agent_poll_tasks.php` reviewed and set to `status: "active"` in header.
- [x] Added `lupo-api/v1/tasks/complete.php` for agent completion updates on `lupo_dialog_pending_tasks` (`completed` / `failed`).
- [ ] Unify UI + API task surfaces: `channels/index.php` currently creates/reads `lupo_dialog_pending_tasks` (HERMES path) while `/lupo-api/v1/tasks/list.php` is backed by `lupo_tasks` (`TaskService` path).
...
| Date | Change |
|------|--------|
| 20260421124200 | Created 4.1.4 TODO with unfinished tasks carried forward from 4.1.3 |
