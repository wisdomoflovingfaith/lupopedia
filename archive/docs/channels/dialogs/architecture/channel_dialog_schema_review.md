...
**Version:** 3.0.16  
**Source:** Toon files from `database/lupopedia/toon/`
...
- `mood_vector`: CHAR(6) NOT NULL DEFAULT '666666' (Mood color: 6-char hex)
...
- `mood_vector` uses 6-char hex format (no # prefix)
- Soft delete pattern follows WOLFIE doctrine
...
- `mood_vector` and `weight` duplicated from parent message (consider normalization)
...
- **Consider normalization:** Remove `mood_vector` and `weight` if they're always synced with parent message
