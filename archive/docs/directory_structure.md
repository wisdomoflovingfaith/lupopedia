...
Lupopedia follows a strict directory naming convention using the **``** prefix for core system folders. This ensures namespacing, avoids collisions with user content, and supports the **Filesystem-First "Root Truth"** doctrine.
...
| Directory | Purpose |
|-----------|---------|
| **`actors/`** | Actor workspaces and resource hubs (name-based). |
| **`agents/`** | AI agent configuration files and system prompts. |
| **`app/`** | Core application logic (Controllers, Services, Http, Middleware). |
| **`bin/`** | System binaries, CLI utilities (e.g., `lupo.php`), and boot scripts. |
| **`channels/`** | Communication entities, task threads, and coordination metadata. |
| **`config/`** | Modular configuration, atoms (SSOT), and global settings. |
| **`content/`** | Themes, uploads, plugins, and public assets. |
| **`database/`** | Database schema, SQL seeds, migrations, and TOON references. |
| **`includes/`** | Core PHP functions, bootstrap, and vendor-like helper classes. |
| **`logs/`** | Centralized system and error logs. |
| **`routes/`** | Routing definitions for the modular loader. |
| **`sessions/`** | Runtime session storage (PHP and file-based). |
| **`tests/`** | Unit, regression, and integration test suites. |
| **`tools/`** | Specialized development tools and VSX extension source. |
...
Under **`actors/{actor_name}/`**, the structure is further subdivided into standard resource folders:
...
3.  **`includes/bootstrap.php`** is loaded, initializing the `mydatabase` connection and loading core Services (e.g., `ActorService`).
