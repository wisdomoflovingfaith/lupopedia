...
lupopedia.headers:
  ...
  file_path_from_root: docs/database/lupopedia/tables/CURSOR_KIRO_HANDOFF.md
  channel_id: 1
  actor_id: 102
  questions_toon: null
...
Cursor documented the following tables under **user, authentication, session, identity, and access-control** domain. All files are in `docs/database/lupopedia/tables/active/`.
...
- **lupo_governance_overrides**, **lupo_hotfix_registry** â€” KIRO.
...
Cursor did **not** create documentation for these because there is no TOON in `database/lupopedia/toon/` and Cursor only documents tables supported by project files (TOONs + project MD).
