...
lupopedia.headers:
  ...
  file_path_from_root: docs/database/lupopedia/tables/KIRO_HANDOFF_RESPONSE.md
...
lupopedia.edges:
  outbound_edges:
  - to: docs/database/lupopedia/tables/CURSOR_KIRO_HANDOFF.md
    type: references
    weight: 1.0
  - to: report_kiro.md
    type: references
  ...
  - to: docs/database/lupopedia/SCHEMA_REGISTRY_KIRO.md
    type: references
    weight: 0.85
  - to: docs/database/lupopedia/tables/VALIDATION_REPORT_KIRO.md
    type: references
    weight: 0.85
  semantic_tags:
  - handoff
...
**Note (Cursor lead):** Canonical [registry](../../../../database/lupopedia/actors/actor_id/registry.json) lists KIRO as **actor_id 100**. Cursor document is correct; this file updated to 100.
...
1. **TOON Format Discrepancy**: Two TOON formats exist with conflicting schema definitions
   - `database/lupopedia/toon/` (`.toon` YAML) - 230+ files, `actor_name` as PK for `lupo_actors`
   - `database/lupopedia/toon/` (`.toon.json` JSON) - 221 files, `actor_id` as PK for `lupo_actors`
...
**DECISION**: Per Captain Wolfie directive, `database/lupopedia/toon/` (YAML format) is canonical. All agents must reference this location.
...
### For All Agents:
1. Reference `database/lupopedia/toon/` (YAML) as canonical TOON source
