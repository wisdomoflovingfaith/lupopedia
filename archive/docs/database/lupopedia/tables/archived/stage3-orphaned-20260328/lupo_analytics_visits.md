...
lupopedia.headers:
  ...
  file_path_from_root: docs/database/lupopedia/tables/active/lupo_analytics_visits.md
  ...
  artifact_kind: table
  namespace: analytics
  purpose: Documentation for lupo_analytics_visits table - analytics visit and session
    activity for traffic and usage tracking
  tags:
  - database
  - table
  - analytics
...
lupopedia.edges:
  comment: Snapshot of edges for lupo_analytics_visits table doc at 4.0.79 (grounded
    by repo search; non-exhaustive).
  meta: php_hits=2 python_hits=2
  outbound_edges:
  - to: database.table.lupo_analytics_visits
...
|--------|------|----------|---------|-------------|
| analytics_visit_id | bigint | No | â€” | Primary key. |
| visit_type | varchar(20) | No | 'realtime' | Visit bucket (e.g. realtime, pageview). |
| session_id | varchar(100) | No | â€” | Session identity. |
| actor_id | bigint | No | 0 | Actor reference. |
| content_id | bigint | Yes | â€” | Content reference. |
| federation_node_id | bigint | No | â€” | Federation scope. |
| url_path | varchar(500) | No | '' | URL path. |
| referer_url | varchar(500) | Yes | â€” | Referrer URL. |
| referer_domain | varchar(255) | Yes | â€” | Referrer domain. |
| referer_path | varchar(500) | Yes | â€” | Referrer path. |
| came_from | varchar(500) | Yes | â€” | Campaign/source tag. |
| department_id | bigint | No | 1 | Department scope. |
| period_type | varchar(64) | Yes | â€” | Period label (e.g. daily, monthly). |
| period_date | bigint | Yes | â€” | Period date bucket. |
| date_ymd | bigint | Yes | â€” | YYYYMMDD bucket. |
| date_ym | bigint | Yes | â€” | YYYYMM bucket. |
| first_seen_ymdhis | bigint | No | â€” | First seen timestamp (BIGINT UTC). |
| last_seen_ymdhis | bigint | No | â€” | Last seen timestamp (BIGINT UTC). |
...
| user_agent | varchar(255) | Yes | â€” | User agent. |
| ip_address | varchar(45) | Yes | â€” | IP address. |
| created_ymdhis | bigint | No | 0 | Created timestamp (BIGINT UTC). |
| updated_ymdhis | bigint | No | â€” | Updated timestamp (BIGINT UTC). |
...
- **Logical references (no DB FKs):** actor_id â†’ lupo_actors.actor_id; content_id â†’ lupo_contents.content_id; department_id â†’ department/channel scope; federation_node_id â†’ federation node; session_id is application-defined session identity. All integrity in application code.
