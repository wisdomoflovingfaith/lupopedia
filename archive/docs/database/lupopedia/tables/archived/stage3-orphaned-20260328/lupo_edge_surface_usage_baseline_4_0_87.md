...
lupopedia.headers:
  ...
  file_path_from_root: docs/database/lupopedia/tables/active/lupo_edge_surface_usage_baseline_4_0_87.md
...
- Read: includes/classes/EmergentRoleDiscovery.php (multiple SELECT statements by actor and edge_type, plus domain/time window filters).
- Read: database/lupopedia/content/app/Services/ActorService.php (actor-switch authorization via edge_type = 'supports').
...
- Read: database/lupopedia/content/app/Services/BayesianDecisionService.php
...
- Read: includes/classes/EdgeQueryService.php (canonical read API; reads by object, type, channel, counts, existence).
...
- Read/Write (legacy service path): database/lupopedia/content/app/Services/EdgeService.php
  - INSERT row
  - soft-delete UPDATE
...
- Read: includes/classes/EdgeQueryService.php (active edge type registry reads from edge_types).
- Read: check_edge_state.php count check.
- Write: hephaestus_execute_migrations.php seed inserts.
- Write: database/lupopedia/mysql/seed/seed_traits_edge_types_action_auth_4.0.69.sql canonical seed/upsert.
...
- Note: includes/classes/SearchIndexer.php references a non-prefixed table name search_index (legacy/parallel surface).

### lupo_memory_rollups

- No active runtime read/write callsite found in PHP service/controller code during this pass.
- schema-config list presence only (includes/schema-config.php).

## 2. Callsite Matrix

| Table | File Path | Read/Write | Query Shape | Edge Type Assumptions |
|---|---|---|---|---|
| lupo_actor_edges | includes/classes/EmergentRoleDiscovery.php | Read | SELECT grouped by edge_type; SELECT by actor/time windows | dynamic edge_type semantics in properties |
| lupo_actor_edges | database/lupopedia/content/app/Services/ActorService.php | Read | JOIN actors to actor_edges by source/target + is_deleted | hardcoded supports |
...
| lupo_decision_edges | database/lupopedia/content/app/Services/BayesianDecisionService.php | Read | SELECT outgoing/incoming by source_decision_id/target_decision_id | decision graph edge types (table-driven) |
| lupo_edges | includes/classes/EdgeQueryService.php | Read | SELECT by object side, by edge_type, by channel_id, grouped counts, existence check | registry-driven edge_type slugs |
...
| lupo_edges | database/lupopedia/content/app/Services/EdgeService.php | Write + Read | INSERT full row; UPDATE soft-delete; SELECT by left/right/channel | caller-supplied edge_type |
| lupo_edge_types | includes/classes/EdgeQueryService.php | Read | SELECT active edge types and lookup by slug | slug registry |
| lupo_edge_types | database/lupopedia/mysql/seed/seed_traits_edge_types_action_auth_4.0.69.sql | Write | INSERT/UPSERT 12 canonical channel/thread slugs | seeded 12 slugs |
| lupo_edge_type_definitions | database/lupopedia/mysql/seed/seed_traits_edge_types_action_auth_4.0.69.sql | Write | INSERT/UPSERT allowed left/right object type constraints | per-edge allowed object types |
