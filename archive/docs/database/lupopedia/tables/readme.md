...
This directory contains comprehensive documentation for all Lupopedia database tables. Each table has corresponding TOON schema files in `database/lupopedia/toon/` that serve as the authoritative source of truth for table structure.
...
### 3. TOON Schema Files
Located in `database/lupopedia/toon/` - these are the authoritative schema definitions:
