...
lupopedia.headers:
  ...
  purpose: "Documentation for the actors directory: actor-specific resources hub; dual-identity and paired_actor_id"
  mood_vector: "4169E1"
  traits: ["canonical", "actors", "directory_structure", "dual_identity", "v4.0.61"]
  tags: ["actors", "actors", "directory", "documentation", "actor_resources"]

lupopedia.edges:
  outbound_edges:
    - { to: "AGENTS.md", type: "references", weight: 0.9 }
    - { to: "lupopedia-config.php", type: "references", weight: 0.8 }
    - { to: "database/lupopedia/actors/actor_id/registry.json", type: "references", weight: 0.8 }
    ...
    - { to: "database/lupopedia/actors/registry.json", type: "references", weight: 0.8 }
...
- **Default value:** `actors` (relative to project root)
...
- **Constant for channels:** `LUPO_CHANNELS_DIR` (default `'channels'`). Channel artifacts live under `channels/{node_id}/{channel_id}/` (e.g. `metadata.md` with FLARE headers). Actor workspaces are name-based under `actors/{actor_name}/` (e.g. `actors/system/`); numeric paths (e.g. `0/`) may be symlinks to them.
...
Each installed actor has a subdirectory under `actors/` given by registry **`dir`** (often `actors/{actor_id}/`, e.g. `system/`, `1/`, `antigravity/`). Inside each actor directory, the following subdirectories are used:
...
| **skills/**    | Agent skills: reusable modular capabilities, specialized knowledge, or tool definitions (e.g. `actors/1/skills/web_search`). |
| **www/**       | Web-accessible content rendered at `/agent/<actor_name>/`. Priority: `readme.md` > `index.htm` > `index.php`. |
| **logs/**      | Actor-specific logs (optional, e.g. `actors/system/logs/`). |
...
- **system** (Actor 0) — **System actor.** Core platform operations, security, low-level management. Directory `actors/system/` (numeric `0/` may symlink here).
- **wolfie** (Actor 1) — **WOLFIE.** Governing agent; coordination and orchestration. Directory `actors/1/`.
- **anubis** (Actor 19) — **Anubis.** Recovery: orphan adoption, quarantine, recovery. Directory `actors/anubis/`.
- **antigravity** (Actor 42) — **Antigravity.** Canonical actor_id 42 (IDE extensions, VSX). Directory `actors/antigravity/`. Resolve `?actor=antigravity` or `?actor_name=antigravity` via registry.
...
To add a new actor and give it a resource directory under `actors`:
...
5. **Optional: ensure directory exists at runtime**  
   If your code assumes the directory exists, ensure your setup or config loader creates `actors/{name}/` and the standard subdirs. Use `ActorService::getActorDir($actor_name)` to resolve the correct path.

### Example: Creating `actors/2/` for a Hypothetical Actor

```bash
# From project root
mkdir -p actors/my-agent/apps actors/my-agent/tools actors/my-agent/docs actors/my-agent/db-changes actors/my-agent/api actors/my-agent/needs actors/my-agent/prompts actors/my-agent/skills actors/my-agent/www actors/my-agent/logs
```
...
- **`agents/`** — AI agent configuration (e.g. `agent.json`, `system_prompt.txt`) per agent. This is separate from `actors/`, which holds **resources** (apps, tools, docs, db-changes, api, needs) per actor.
- **`database/lupopedia/actors/`** — Registry and identity data (e.g. `actor_id/registry.json`, per-actor config under `actor_id/{id}/`). This is the source of truth for actor identity; `actors/` is the source of truth for actor-specific file-based resources.

## Initialization

If the `actors` directory or an actor's subdirectory does not exist, it can be created by:

- **Manual creation** — Create `actors` and the desired `actors/{id}/` and subdirs by hand or via the “Adding a New Actor” steps above.
- **Setup script or config loader** — If the project provides a setup or install path that initializes directories, it may create `actors` (and optionally `actors/0/`, `actors/1/`, and their six subdirs) when not present. The path is read from `LUPO_ACTORS_DIR` in `lupopedia-config.php`.
...
The FLARE frontmatter parser (`includes/classes/FlareParser.php`) supports:
...
- **Error handling:** On parse failure, log to `actors/0/logs/parser_errors.log` and return empty headers. Consuming code must use `isset()` or `array_key_exists()` for keys (e.g. `last_modified_utc`).
...
- **Policy:** Auto-resolve if filesystem UTC ≥ DB UTC (last write wins for body). If both changed and DB newer, flag conflict and log to queue (e.g. `actors/42/logs/conflicts.log`) with path and details for Anubis or manual review. No automatic three-way merge.
