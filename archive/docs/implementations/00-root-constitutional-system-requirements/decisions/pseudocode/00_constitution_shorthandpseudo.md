...
lupopedia.headers:
  ...
  file_path_from_root: "docs/implementations/00_root_constitutional_system_requirements/decisions/pseudocode/00_constitution_shorthand.pseudo.md"
  web_path: "http://www.lupopedia.com/lupopedia/docs/implementations/00_root_constitutional_system_requirements/decisions/pseudocode/00_constitution_shorthand.pseudo.md"
  questions_toon: null
...
| Rule | Forbidden | Required |
|------|-----------|----------|
| Core paths | PHP 8-only syntax where 7.4 band applies | **PHP 7.4+** compatible patterns in core (`includes/`, entrypoints, themes) |
| Dependencies | Composer / npm as **runtime** for shipped app | In-tree **`includes/`**; study off-tree, ship native code |
...
(Condensed for Purpose‑1 quickload; see PRD 00 §16 — `layers.js`, no `eval` / `new Function(string)` / string timers; vanilla JS; `lupo_t()`; in-tree assets.)
