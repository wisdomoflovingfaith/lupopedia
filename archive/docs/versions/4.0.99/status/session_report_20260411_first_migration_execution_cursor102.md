...
lupopedia.headers:
  ...
  file_path_from_root: "docs/versions/4.0.99/status/session_report_20260411_first_migration_execution_cursor102.md"
  web_path: "https://www.lupopedia.com/lupopedia/docs/versions/4.0.99/status/session_report_20260411_first_migration_execution_cursor102.md"
  questions_toon: null
  federation_node_id: 0
  channel_key: "development"
  trust_tier: "canonical"
  memory_key: "memory/development/canonical/1026/04/session-report-first-migration-execution-20260411.toon"
...
**Window:** 2026-04-11 UTC; anchor **`20260411180532`** (**`python bin/tick.py`**) for this artifact’s header refresh (**ARA** memory consolidation pass).
...
**Canonical changelog:** **`docs/versions/4.0.99/CHANGELOG.md`** — section **`[2026-04-11 17:55 UTC] — First executed header migration: BREAKTHROUGH_REGISTRY + Captain's Log + edge #8`**.
...
| **Captain's log** | **`20260411_SHALL_WE_PLAY_A_GAME.md`** under **`content/federation_node/0/captains_log/`** with **PRD 16** envelope; validator **PASS**. |
...
| **`tick.py` side effects** | **`tick.py`** may touch **`config/session.json`** and related anchor files. | Accidental commit of session state pollutes history. | Stage **only** intentional paths; revert **`session.json`** if tick was only for UTC. |
...
3. **Captain's log as migration artifact** — A **content** file under **`content/.../captains_log/`** with a valid **PRD 16** block is **both** human narrative and **machine-checkable** surface.
...
python scripts/validate_lupopedia_headers_universal.py docs/versions/4.0.99/BREAKTHROUGH_REGISTRY.md
python scripts/lib/kairos_edge_verification.py --test --file docs/versions/4.0.99/BREAKTHROUGH_REGISTRY.md
python scripts/lib/kairos_edge_verification.py --test --json --file docs/versions/4.0.99/BREAKTHROUGH_REGISTRY.md

python scripts/validate_lupopedia_headers_universal.py docs/versions/4.0.99/status/session_report_20260411_first_migration_execution_cursor102.md
python scripts/lib/kairos_edge_verification.py --test --file docs/versions/4.0.99/status/session_report_20260411_first_migration_execution_cursor102.md
```

**Pattern #10 / #12** (after **`lupo_memory_nodes` + edges** exist — **approved DB workflow** only; then orphans optional):

```bash
python scripts/detect_memory_graph_orphans.py --under docs/versions/4.0.99 --json
```

**Post-write normalize** (when you **rewrite** headers and want **KAIROS** hooked to the normalizer — **not** dry-run):

```bash
python scripts/normalize_lupopedia_md_header_25.py --target-version 4.0.99 --path "path/to/file.md" --verify-edges
```
