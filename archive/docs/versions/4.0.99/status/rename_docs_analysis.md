...
## 1. Script Purpose
The script's primary goal is to **normalize filenames** within specific directories (`docs` and `memory`) to follow a strict lowercase, alphanumeric, snake_case convention. It aims to ensure filesystem compatibility across different operating systems (case-sensitivity issues) and environments.

## 2. Main Execution Flow
1.  **Scope Definition:** Targets only `docs/` and `memory/` directories.
...
-   When a file is renamed, every file in the project (not just `docs`) might contain a reference to it.
