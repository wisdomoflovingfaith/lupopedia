...
lupopedia.headers:
  ...
  file_path_from_root: "docs/versions/4.0.99/pilot_14_5_quick_reference_20260412_000121.md"
  web_path: "https://www.lupopedia.com/lupopedia/docs/versions/4.0.99/pilot_14_5_quick_reference_20260412_000121.md"
  questions_toon: null
  federation_node_id: 0
  channel_key: "development"
  trust_tier: "canonical"
  memory_key: "memory/development/canonical/1026/04/pilot-14-5-quick-reference-20260412-000121.toon"
...
- **`memory_key`:** `memory/development/canonical/1026/04/lupopedia-quick-reference.toon`
...
- **SYNAPSE integrity fixes present:** §5 uses `{channel_key}/{trust_tier}/{display_year}/{month}`; no `memory/YYYY/MM` as browse path; `migrate_transcript_to_memory.py` in §6.1 legacy; trust tiers described as authority, not promotion into `seed`.
...
```text
python scripts/normalize_lupopedia_md_header_25.py --target-version 4.0.99 --dry-run --verbose --path lupopedia_quick_reference.md
python scripts/normalize_lupopedia_md_header_25.py --target-version 4.0.99 --backup --path lupopedia_quick_reference.md
```
...
**Post-write header UTC:** `when_updated` / `last_modified_utc` set to **`20260412000121`** (`python bin/tick.py` for this batch).
...
```text
python scripts/validate_lupopedia_headers_universal.py lupopedia_quick_reference.md
```
...
1. **Created** one active **`lupo_memory_nodes`** row with:
   - **`memory_key`:** `memory/development/canonical/1026/04/lupopedia-quick-reference.toon` (matches header)
...
2. **Outbound `references` edges (intended first safe set):**  
   `FILE:docs/prd/16_lupopedia_headers.md`, `…/28_semantic_monitoring_widget.md`, `…/38_memory_unification.md`, `…/50_agent_coordination_protocol.md`, `…/51_memory_graph_as_source_of_truth.md`, `…/52_memory_graph_focus_manifest.md`, `FILE:docs/versions/4.0.99/BREAKTHROUGH_REGISTRY.md`
...
```text
python scripts/lib/kairos_edge_verification.py --test --file lupopedia_quick_reference.md --json
```
...
```text
python scripts/detect_memory_graph_orphans.py --under . --json
```

**Row for this file:** `"db_status": "**ok**"` (node present).

**`.toon` on disk (header path):** Still **no** committed hand-authored **`.toon`** at the literal **`memory/development/canonical/1026/04/`** header path in this tree (logical **`memory_key`** unchanged).

**Mirror production:** Export is **not** hand-authored. **Canonical:** PHP **`MemoryExportService`** via **`bin/export.php`**. *Earlier batch:* export not run. **Superseded by Step 4c** — **`export.php --node-id`** executed; see **Step 4c** for the **`.json`** path.
...
- **Workflow:** One-off Python using **`scripts/lib/db_connection.get_connection()`** + **`db_config.get_table_prefix()`** (same credential chain as **`DBMemoryWriter`** / KAIROS) — **`UPDATE`** only, **`WHERE memory_node_id=%s AND is_deleted=0`**, verified **`memory_key`** matches header **`memory/development/canonical/1026/04/lupopedia-quick-reference.toon`** before write.
...
- **`DBMemoryWriter.create_memory_edges`** — **not invoked** (would still insert **`0`** rows; **`FILE:docs/prd/…`** resolution requires an **existing** target **`memory_key`** equal to that path string, and no such rows exist).
...
- **Ran:** `php bin/export.php --node-id 202604120023190000` → **`[OK]`**.
- **Produced file (this environment):**  
  **`memory/2026/04/20260412_002319_actor_1_documentation_memory_development_canonical_1026_04_lupopedia-quick-reference.toon.json`**
- **`MemoryExportService`** writes **`.json`** under **`memory/{yyyy}/{mm}/`** derived from **`created_ymdhis`** + slug — **not** necessarily the literal **`memory/{channel_key}/{trust_tier}/1026/04/*.toon`** path in the Markdown header. **DB row + this JSON** are consistent; **header `.toon` path** is still a **logical** key / sidecar convention — **do not claim** a hand-placed **`.toon`** file exists at the header path unless a **separate** TOON/sidecar pipeline creates it.
...
python scripts/normalize_lupopedia_md_header_25.py --target-version 4.0.99 --backup --path lupopedia_quick_reference.md
python scripts/validate_lupopedia_headers_universal.py lupopedia_quick_reference.md
python scripts/lib/kairos_edge_verification.py --test --file lupopedia_quick_reference.md
python scripts/detect_memory_graph_orphans.py --under . --json
...
php bin/export.php --node-id 202604120023190000
python scripts/lib/kairos_edge_verification.py --test --file lupopedia_quick_reference.md --json
python scripts/detect_memory_graph_orphans.py --under . --json
```
