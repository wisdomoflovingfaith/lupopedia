...
lupopedia.headers:
  ...
  file_path_from_root: "docs/decisions/pseudocode/00_NON_NEGOTIABLES_IMPORTANT_OVERWRITES_SHORTHAND.pseudo.md"
  web_path: "http://www.lupopedia.com/lupopedia/docs/decisions/pseudocode/00_NON_NEGOTIABLES_IMPORTANT_OVERWRITES_SHORTHAND.pseudo.md"
  questions_toon: null
...
**Two-file model (not a rename):** **`00_NON_NEGOTIABLES_IMPORTANT_OVERWRITES.pseudo.md`** still exists as its **own** digest. **This** file = **one-screen table**; **dodo** = **long-form** anti-patterns and SQL examples. Shared **`00_`** prefix only sorts both to the top of `pseudocode/`. Extension is **`.pseudo.md`** on purpose (**PRD 17**); there is **no** parallel plain **`.md`** copy to avoid drift.

**Deeper examples and “why”:** [00_NON_NEGOTIABLES_IMPORTANT_OVERWRITES.pseudo.md](./00_NON_NEGOTIABLES_IMPORTANT_OVERWRITES.pseudo.md). **Full shorthand table:** [00_constitution_shorthand.pseudo.md](../../implementations/00_root_constitutional_system_requirements/decisions/pseudocode/00_constitution_shorthand.pseudo.md).
...
| **Shipped UI** | npm, React/Vue for app JS | **Vanilla** JS; **`layers.js`**; **no** npm-as-runtime (**§16**) |
...
| **JS “flexibility”** | `eval`, `new Function`, string timers | **§16** — **`layers.js`**; forbidden patterns explicit in constitution |
