...
lupopedia.headers:
  ...
  file_path_from_root: "docs/decisions/pseudocode/00_NON_NEGOTIABLES_IMPORTANT_OVERWRITES.pseudo.md"
  web_path: "http://www.lupopedia.com/lupopedia/docs/decisions/pseudocode/00_NON_NEGOTIABLES_IMPORTANT_OVERWRITES.pseudo.md"
  questions_toon: null
...
(Removed PHP example per Purpose‑1 constitutional rules; see PRD 00 §3.5, §19, and `includes/classes/TimestampYmdhis.php`.) Use **explicit `INSERT` columns** in real code (§1). **`SELECT *`** in reads is **fine**.
...
(Removed shell example per Purpose‑1 constitutional rules; see PRD 00 §4, §16.) Native PHP; in-tree **`includes/`**; **`timestamp_ymdhis`** / **`gmdate('YmdHis')`** for clocks.
...
Webpack, Vite, SPA stacks for **`includes`**.
...
Vanilla JS; **`layers.js`**; **`lupo_t()`** for copy.
