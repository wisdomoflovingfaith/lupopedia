...
lupopedia.headers:
  ...
  file_path_from_root: "lupo-channels/42/threads/1036/20260321_150000_athena_canonical_actor_architecture_and_repair_plan.md"
  web_path: "http://www.lupopedia.com/lupo-channels/42/threads/1036/architecture"
...
lupopedia.edges:
  outbound_edges:
    - { to: "lupo-channels/42/threads/1035/20260321_140000_wolfie_governance_directive_doctrine_authority_validation_and_refactor_safety.md", type: "governance_aligned", weight: 0.95, reason: "Governance rules apply to how this architecture is approved and implemented" }
    - { to: "AGENTS.md", type: "refines", weight: 0.9, reason: "Will update AGENTS.md to reflect includes/actors/ canonical location" }
    - { to: "lupo-rules/root/", type: "references", weight: 0.85, reason: "Shared base rules location referenced in design" }
    - { to: "lupo-skills/", type: "references", weight: 0.85, reason: "Shared skills library to be created per design" }
    - { to: "lupo-database/lupopedia/actors/actor_id/registry.json", type: "supersedes", weight: 0.8, reason: "Canonical actor identity will move to includes/actors/manifest.json" }
...
ACTOR = {
  ...
  Skills (skills.md) {
    shared_skills: Array<Reference>,     // Pointers to lupo-skills/shared/
    actor_specific_skills: Array<String> // Skills unique to this actor
  },
  Rules (rules.md) {
    shared_rules: Array<Reference>,      // Pointers to lupo-rules/root/
...
Layer 1: Shared Base (READ-ONLY)
  +-- All root rules (lupo-rules/root/*.md)
  +-- All shared skills (lupo-skills/shared/)
...
## Shared Rules (inherited from lupo-rules/root/)
...
includes/actors/
+-- shared/
|   +-- rules/                       (empty — base rules in lupo-rules/root/)
|   +-- skills/                      (symlink or reference to lupo-skills/shared/)
+-- wolfie/
|   +-- manifest.json
|   +-- personality.md
|   +-- soul.md
...
|-----------|------|-----------------------------------|
| `lupo-agents/` | AI runtime metadata (model, provider, prompt) | Separate from actor definition; referenced in actor manifest under `paired_agents` |
| `lupo-rules/root/` | Shared base rules | Referenced in actor `rules.md` (never copied) |
| `lupo-skills/shared/` | Shared base skills | Referenced in actor `skills.md` (never copied) |
| `lupo-channels/` | Coordination artifacts (threads, tasks) | Actors referenced in channel artifacts via `actor_id`, `actor_name` |
...
| `lupo-docs/actors/` | Actor documentation | Generated from canonical sources |
...
| `lupo-channels/42/actors/{slug}/README.md` | **Derived** | **NEVER** | System | Generated from canonical |
| `lupo-database/lupopedia/actors/actor_id/registry.json` | **Derived** | **NEVER** (after migration) | System | `propagate_actor_identity.php` |
...
---------------------------------------------------------------------
lupo-rules/root/*.md                        .cursor/rules/*.mdc
...
3. **Derived IDE files** are generated by `lupo-scripts/propagate_agent_rules.php` which:
   - Reads root rules from `lupo-rules/root/`
...
4. **Generated files must declare provenance** in their footers:
   ...
   lupopedia.footer:
     ...
     source_canonical: ["lupo-rules/root/*.md", "includes/actors/wolfie/rules.md"]
...
- Actor README files in channels (`lupo-channels/42/actors/{slug}/README.md`)

**Propagation order:**
1. Edit `includes/actors/{slug}/manifest.json`
2. Run `lupo-scripts/propagate_actor_identity.php`
...
| **Shared rules storage** | Only in `lupo-rules/root/` | Already correct — no change needed |
| **Actor-specific rules storage** | In IDE directories (derived, hand-edited) | Move to `includes/actors/{slug}/rules.md` (canonical) |
| **Actor skills definition** | Undefined, scattered, or implied | Share library in `lupo-skills/shared/`; refs in `includes/actors/{slug}/skills.md` |
...
- [ ] Create `lupo-skills/shared/` directory (empty for now)
- [ ] Create `lupo-scripts/propagate_actor_identity.php` (new script)
- [ ] Extend `lupo-scripts/propagate_agent_rules.php` to:
...
- [ ] Create `includes/actors/{slug}/manifest.json` from:
  - Existing `lupo-database/lupopedia/actors/actor_id/registry.json` (actor_id, slug, name)
...
- [ ] Create `rules.md`:
  ...
  - Format: references to `lupo-rules/root/`, overlays as new sections
- [ ] Create `skills.md`:
  - List shared skills this actor uses
  - Add actor-specific skills (if any exist currently)
  - Format: references to `lupo-skills/shared/`, additions as new sections
...
2. Create `lupo-skills/shared/` directory (empty; ready for skills)
...
5. Create shared skill library structure (THOTH designs content; HEPHAESTUS creates files)
   - `lupo-skills/shared/header_validation.md`
   - `lupo-skills/shared/doctrine_review.md`
   - `lupo-skills/shared/schema_authority.md`
   - `lupo-skills/shared/thread_navigation.md`
...
1. Create `lupo-docs/doctrine/ACTOR_ARCHITECTURE_DOCTRINE.md`
   - Canonical reference for this design
   - Authority hierarchy for actor definitions
   - Governance rules
2. Create `lupo-docs/doctrine/SHARED_SKILLS_DOCTRINE.md`
...
- [ ] `lupo-skills/shared/` directory created
...
- [ ] rules.md correctly references `lupo-rules/root/` files (not copying content)
- [ ] skills.md correctly references `lupo-skills/shared/` files (not copying content)
