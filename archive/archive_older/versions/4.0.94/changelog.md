...
lupopedia.headers:
  ...
  file_path_from_root: "docs/versions/4.0.94/CHANGELOG.md"
  web_path: "http://www.lupopedia.com/lupopedia/docs/versions/4.0.94/CHANGELOG.md"
  questions_toon: null
...
---
# file: docs/versions/4.0.94/CHANGELOG.md — delegation: cursor:root — web_path: http://www.lupopedia.com/lupopedia/docs/versions/4.0.94/CHANGELOG.md
...
**WHERE:** `database/lupopedia/mysql/install/` + `mysql/schema_review/`.
...
**HOW:** Receipt **`decisions/20260407_015813_DECISION_cursor_install_schema_merge_receipt.md`**; paired Q/A **`20260407_015814`** → **`20260407_015815`**; **`PLAN.md`** / **`TODO.md`** / **`edges.md`** updated in the same UTC batch (**`20260407015813`** via **`python bin/tick.py`**).
...
- `agents/chronos/` fully activated from reserved-slot stub.
...
- **Handoff:** Ongoing doctrine batches, **`FOR_CLAUDE_CODE_2026_04_06.md`**, and **`docs/versions/4.0.95/CHANGELOG.md`** are the **active-line** homes; root **`CHANGELOG.md`** stays index-only per **Where release notes live (4.0.85+)**.
...
- **`ToonSchemaCache`**: When `LUPOPEDIA_DEBUG` is true, `__construct` logs deprecation in favor of canonical JSON under `database/lupopedia/json/` (removal targeted 4.1.0).
- **`main_layout.php`**: Bulk layout CSS moved to `includes/css/main-layout.css`; decorative border tile `background` rules stay in a small inline `<style>` block because URLs require `LUPOPEDIA_PUBLIC_PATH`.
- **`main_layout.php`**: Nav/collections JavaScript moved to `includes/js/main-layout.js` and `main-layout-collections.js`; inline script sets `window.LUPO_MAIN_LAYOUT` (public path + collection context) for API `fetch` targets.
- **`rules/root/`**: Front matter migrated from flat `actor_id` / `actor_name` to structured `author` block (`type`, `id`, `name`) where those keys appeared at header indent; body examples unchanged.
...
- **PHP Tiered Compatibility (Option 4)** – Updated section 4 with tiered approach:
  ...
  - Legacy: PHP 5.6+ allowed with installer flags (`LUPOPEDIA_LEGACY_INSTALL`, `install-legacy-php.flag`)
  - 32-bit override: `LUPOPEDIA_ALLOW_32BIT=1` or `install-allow-32bit.flag` (not recommended)
...
- **ToonSchemaCache** – Deprecated, now reads from JSON schema files:
  - Reads from `database/lupopedia/json/<table_name>.json` (canonical)
  - Removed YAML dependency (`yaml_parse()`)
  - `@deprecated` annotation with migration path

- **DYNAPI_DOCTRINE.md** – Deprecated:
  - `status: deprecated`
  - `deprecated_by: docs/doctrine/LUPO_LAYERS_DOCTRINE.md`
  - Heritage content retained for context

- **LUPO_LAYERS_DOCTRINE.md** – Created as active doctrine:
  - `status: active`
  - Documents `layers.js` (LupoLayer) as canonical UI layer controller
...
- **ToonSchemaCache** – Deprecated, use JSON schema files under `database/lupopedia/json/`
...
- **Channel 66 production extended integration test** – `tests/integration/channel66_production_extended_test.php`: LUPOPEDIA HEADER, `DatabaseFactory::getConnection()`, ingester fixture paths under `channels/66/threads/1001/`, merged monitoring validation, removed stub/shell paths; **Channel66ProductionIngester::discoverChannelFiles** – `thread_id === null` processes all threads (branch fix).
...
- **`docs/versions/4.0.95/`** — planning scaffold created; open and deferred tasks moved to **`4.0.95/TODO.md`** (P3-005+, P4-*, D-001–D-005, Phase 7 test matrix continuation).
...
Frozen baseline: see **`docs/versions/4.0.93/README.md`** and **`docs/versions/4.0.93/CHANGELOG.md`** (prior release notes).
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); **`docs/prd/00_root_constitutional_system_requirements.md`** — **§17.9** clarified: **ROSE** exception for **sandboxed** multi-persona **dialog** per **PRD 36**; **write surface** = **`lupo_dialog_messages`** (+ **PRD 36**-scoped dialog metadata), **no** content / **lupo_metadata** (non-dialog) / config / actors / channels / auth updates from ROSE pipeline; compliance test split **IDE vs ROSE**; **`00_NON_NEGOTIABLES_IMPORTANT_OVERWRITES_SHORTHAND.pseudo.md`** + **`00_constitution_shorthand.pseudo.md`**; UTC **`20260405224232`** (`python bin/tick.py` this batch).
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); **`docs/prd/00_root_constitutional_system_requirements.md`** — **§17.9** (impersonation, instruction override, secrets, automation boundaries, service vs dialogue personas; optional filters note); **edge** to **`ADVERSARIAL_TEST_IDENTITY_DOCTRINE.md`**; **`agents/_shared/ide_facet_base_system_prompt.txt`** (prompt-injection block); **`00_NON_NEGOTIABLES_IMPORTANT_OVERWRITES_SHORTHAND.pseudo.md`** + **`00_constitution_shorthand.pseudo.md`**; **§10.2** row **§17.7–§17.9**; UTC **`20260405223937`** (`python bin/tick.py` this batch).
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); **`docs/prd/00_root_constitutional_system_requirements.md`** — new **§17.7** (PHP execution hygiene, JS cross-ref **§16**, **`unserialize`** ban on untrusted data, **`lupo_sessions` / `App\Auth\Session`** vs **`$_SESSION`**, **PRD 33 §5.1** decode/re-encode uploads); **`00_NON_NEGOTIABLES_IMPORTANT_OVERWRITES_SHORTHAND.pseudo.md`** + **`00_constitution_shorthand.pseudo.md`** table rows; UTC **`20260405222700`** (`python bin/tick.py` this batch).
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); **`docs/prd/17_decisions_format.md`** (new subsection: zero-guessing, Purpose 1 vs Purpose 2 scope, anchors, option forks); **`scripts/validate_pseudocode_discipline.py`** (optional warnings for Purpose 2 files under `decisions/pseudocode/`); **`AGENTS.md`** (IDE summary + edge); UTC **`20260405222024`** (`python bin/tick.py` this batch).
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); new **`docs/decisions/pseudocode/00_NON_NEGOTIABLES_IMPORTANT_OVERWRITES_SHORTHAND.pseudo.md`** (Purpose 1 router + overrides table); **`THREAD_INDEX.md`** (both trees); **`00_constitution_shorthand`**, **`lupopedia_quickstart`**, **`00_NON_NEGOTIABLES_IMPORTANT_OVERWRITES.pseudo.md`** (expanded digest), **PRD 00** edges/intro; UTC **`20260405220110`** (`python bin/tick.py` this batch).
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); **`docs/prd/00_root_constitutional_system_requirements.md`** (**§17.3** — explicit **`INSERT`** columns; positional **`INSERT`** hazard; **`SELECT *`** reads not forbidden); **`docs/decisions/pseudocode/00_NON_NEGOTIABLES_IMPORTANT_OVERWRITES.pseudo.md`** (reorder: **§1 INSERT**; drop **`SELECT *`** as dodo item; summary note); **`00_constitution_shorthand.pseudo.md`** (database **INSERT**/**SELECT** rows; security blurb); **`THREAD_INDEX.md`**; UTC **`20260405215402`** (`python bin/tick.py` this batch).
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); **`docs/decisions/pseudocode/00_NON_NEGOTIABLES_IMPORTANT_OVERWRITES.pseudo.md`** — reorder (**`INSERT`** without column list = **critical**; **`SELECT *`** = wasteful/low); expanded §2; summary danger column; golden rule; **`00_constitution_shorthand.pseudo.md`** security row (**INSERT** vs **SELECT**); UTC **`20260405214944`** (`python bin/tick.py` this batch).
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); **`docs/prd/00_root_constitutional_system_requirements.md`** — new **§3.5.4** (Y2038 compliance, forbidden/required patterns, industry context); **§19** checklist item + normative link to dodo digest; **`docs/decisions/pseudocode/00_NON_NEGOTIABLES_IMPORTANT_OVERWRITES.pseudo.md`** (new) + **`THREAD_INDEX.md`**; **`00_constitution_shorthand.pseudo.md`** (timestamp-first table + edge); **`lupopedia_quickstart.pseudo.md`** (link row); UTC **`20260405214736`** (`python bin/tick.py` this batch).
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); **`docs/prd/00_root_constitutional_system_requirements.md`** — **§3.5.1** explicit “`BIGINT` ≠ epoch”; **§3.5.2** storage UTC vs display timezone; **§3.5.3** **`timestamp_ymdhis`** canonical utility (with **`gmdate('YmdHis')`** allowed as equivalent “now”); new **§19** binding IDE/LLM checklist; **`README.md`** (timestamp subsection under mandatory reading); **`00_constitution_shorthand.pseudo.md`** pointer; UTC **`20260405213749`** (`python bin/tick.py` this batch).
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); **`includes/classes/TimestampYmdhis.php`** (`addSeconds` / `diffInSeconds` / `fromHuman` / `convert_iso8601_to_bigint` — **`DateTime` UTC** instead of **`gmmktime`/`strtotime`** bridge; **`diffInSeconds`** sign preserved via **`$db->diff($da)`**); **`docs/prd/00_root_constitutional_system_requirements.md`** (**§3.5** — explicit **Y2038**: packed storage ≠ Unix epoch; **64-bit PHP** required); **`00_constitution_shorthand.pseudo.md`** digest row; UTC **`20260405212338`** (`python bin/tick.py` this batch).
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); **`docs/prd/00_root_constitutional_system_requirements.md`** (expanded **§3.5** — storage semantics, forbidden SQL patterns, PHP/`timestamp_ymdhis` pattern, bound parameters); **§3.6** bullet cross-ref; **`docs/implementations/00_root_constitutional_system_requirements/decisions/pseudocode/00_constitution_shorthand.pseudo.md`** (digest row + one-liner); UTC **`20260405212034`** (`python bin/tick.py` this batch).
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); **`docs/implementations/00_root_constitutional_system_requirements/decisions/pseudocode/`** — new **`*_constitution.pseudo.md`** for **PRD 05, 15, 16, 26, 31, 28, 33** plus **`lupopedia_quickstart.pseudo.md`**; refreshed **`THREAD_INDEX.md`**; **`README.md`** (00 mirror); **`docs/prd/17_decisions_format.md`** (shipped bundle table + edge to quickstart); **`implementations/README.md`** cross-cutting row; UTC **`20260405211127`** (`python bin/tick.py` this batch).
- **WHAT:** Digest-only files (Purpose 1); canonical text remains each **`docs/prd/*.md`**. Injected: department act-as, **`to_actor_id`** routing, Tier 1/2 split, **`prd_file_stem`**, API dual routing, PRD 33 gate / `.htaccess` optional. **Priority 4** (PRD 36/37) listed as optional in quickstart only.
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); **`docs/prd/17_decisions_format.md`** (Pseudocode Directory: Purpose 1 constitution shorthand vs Purpose 2 design notes; naming table; **`THREAD_INDEX`** example; “why two purposes”); new **`docs/implementations/00_root_constitutional_system_requirements/`** (**`README.md`**, **`decisions/THREAD_INDEX.md`**, **`decisions/pseudocode/00_constitution_shorthand.pseudo.md`**, **`decisions/pseudocode/THREAD_INDEX.md`**); **`docs/implementations/README.md`** (cross-cutting + index rows); UTC **`20260405210708`** (`python bin/tick.py` this batch).
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); merged **`docs/implementations/25_departments_systems/`** into **`docs/implementations/25_departments_system/`** (canonical PRD **`25_departments_system.md`**); updated cross-links (**PRD 25**, **30**, root **README**, **IMPLEMENTATION_QUESTIONS_GUIDE**, **create_implementation_question.py**, templates, **validation_report.json**, **26** edges/spec); **PRD 31** + **implementations/README.md** — non-negotiable **character-for-character** match to PRD basename; **PRD 30** example path → existing question file; removed duplicate **PRD 25** outbound edge; UTC **`20260405205804`** (`python bin/tick.py` this batch).
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); **`docs/prd/00_root_constitutional_system_requirements.md`** (**§2** subdirectory routing with/without `mod_rewrite`; **§9.5** `.htaccess` optional; new **§18** `RULE 93.NO_SEARCH_INDEX_ASSUMPTION`; **§17.5** cross-ref when `.htaccess` absent); **`docs/prd/28_semantic_monitoring_widget.md`** (API Endpoints — clean URL vs query-parameter fallbacks); **`docs/prd/33_softaculous_certification_4_1_0_gate.md`** (**§5.1** table row; **§14.1** / **§14.2** / **§14.4 Q2** aligned with fallbacks; new **§14.6** clarification; footer `next_action`); UTC **`20260405205506`** (`python bin/tick.py` this batch).
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); **`scripts/lib/header_validation.py`** (`inject_legacy_actor_from_author`, called from **`validate_header`**); **`validate_lupopedia_headers_universal.py`** (`--check-links`, **`when_updated` ≥ `last_verified`**, inject before author checks); **`lib/header_db_sync.py`** (**`ext.`** metadata writes, legacy **`block.`** read-back, **`append_history`** on **`sync_header_artifact_to_db`**, transaction note in docstring); **`import_content.py`** (`--append-history`); **`bin/tick.py`** (`--copy` via optional **`pyperclip`**); **`prd16_headers_lifecycle.pseudo.md`** (sections 1.4, 2.1, 3.2, 4.2, 5.2, diagram, cheat sheet); UTC **`20260405204851`** (`python bin/tick.py` this batch).
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); **`docs/prd/16_lupopedia_headers.md`** (pseudocode row in applicability table; removed optional exception; validator scope + handoff note); **`docs/prd/17_decisions_format.md`** (rules, validation, example `.pseudo.php`, main validation §9); **`LUPOPEDIA_HEADERS_FORMAT.md`** (Overview); **`AGENTS.md`**; **`sync_header_artifact_to_db.pseudo.php`** (embedded YAML header block); **`prd16_headers_lifecycle.pseudo.md`** + **`pseudocode/THREAD_INDEX.md`**; UTC **`20260405204205`** (`python bin/tick.py` this batch).
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); **`docs/implementations/16_lupopedia_headers/decisions/pseudocode/prd16_headers_lifecycle.pseudo.md`** (full front matter + edges + footer); **`decisions/pseudocode/THREAD_INDEX.md`** (index note); UTC **`20260405203933`** (`python bin/tick.py` this batch).
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); **`docs/prd/16_lupopedia_headers.md`** (new **Header applicability and scope**: required extensions vs optional vs not applicable; PRD 17 pseudocode exception; overview + constitutional bullet fixes); **`docs/prd/17_decisions_format.md`** (pseudocode “no headers” row tied explicitly to PRD 16); **`docs/doctrine/LUPOPEDIA_HEADERS/LUPOPEDIA_HEADERS_FORMAT.md`** (Overview alignment); **`rules/root/LUPOPEDIA_HEADERS_DOCTRINE.md`** (enforcement rationale wording); **`AGENTS.md`** (LUPOPEDIA HEADERS bullet scoped to PRD 16); **`implementations/16_lupopedia_headers/.../prd16_headers_lifecycle.pseudo.md`** (§0 scope); UTC **`20260405203735`** (`python bin/tick.py` this batch).
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); **`docs/implementations/16_lupopedia_headers/decisions/`** — `THREAD_INDEX.md`; **`decisions/pseudocode/`** — `THREAD_INDEX.md`, `prd16_headers_lifecycle.pseudo.md`, `sync_header_artifact_to_db.pseudo.php`; UTC **`20260405202839`** (`python bin/tick.py` this batch).
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); **`docs/prd/17_decisions_format.md`** (new Pseudocode Directory section, canonical folder tree + version `decisions/` tree, THREAD_INDEX rule, validation item 9, PRD 31 cross-ref edge, doc version **1.1**); UTC **`20260405202458`** (`python bin/tick.py` this batch).
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); **`docs/prd/30_channel_usage_patterns.md`** (added help_documentation channel); **`docs/prd/16_lupopedia_headers.md`** (updated file_path_from_root docs); **`content/0/help_documentation/`** (new structure with 5 guides, 8 questions, 8 answers, 34 edges); **`database/lupopedia/mysql/seed/seed_online_help_and_content.sql`** (updated with channel_key paths); **`scripts/build_consolidated_seed_4_1_0.py`** (added help seed); **`install/seed_lupopedia_4_1_0.sql`** (generated consolidated seed); version folder **`docs/versions/4.0.94/`** — `decisions/20260405172914_DECISION_APPROVED_help_content_organization_channel_key_structure.md`, `questions/20260405172914_QUESTION_what_is_correct_help_content_structure.md`, `answers/20260405172914_ANSWER_channel_key_based_organization.md`, `comments/20260405172914_COMMENT_cursor_session_end_help_content_organization.md`, `CHANGELOG` (this entry), `PLAN`, `TODO`, `edges`, `THREAD_INDEX` files; UTC **`20260405172914`** (`python bin/tick.py` this batch).
- **WHAT:** Replaced numeric `content/0/0/` structure with semantic `content/0/help_documentation/` using **channel_key** instead of **channel_id**. **PRD 30:** Added help_documentation channel definition with purpose, content types, and organization guidelines. **PRD 16:** Updated file_path_from_root documentation to specify `content/{federation_node_id}/{channel_key}/{content_id}_{slug}.md` format. **Content created:** 5 help guides (getting started, actors, channels, content, edges), 8 common questions, 8 answers, 34 relationship edges. **Database integration:** Updated seed SQL with correct file_path_from_root values, added channel_key field to headers. **Installation:** Updated build script, generated consolidated seed (27,607 bytes), integrated with installer workflow. **Removed:** Old `content/0/0/` structure.
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); **`includes/classes/SemanticNavbarEmbedContext.php`**; **`includes/modules/api/semantic-navbar-api.php`**; **`includes/classes/AdminSemanticWidgetHandler.php`**; **`includes/lang/en.php`** (semantic widget strings); **`admin.php`** (semantic-widget section blurb); **`docs/prd/21_semantic_navbar.md`**; version folder **`docs/versions/4.0.94/`** — `decisions/20260405104405_DECISION_APPROVED_semantic_navbar_embed_admin_prd21_cursor_thread.md`, `comments/20260405104405_COMMENT_cursor_session_end_semantic_navbar_crafty_handoff.md`, `CHANGELOG` (this entry), `PLAN` Phase **M**, `TODO`, `edges`, `WHAT_TO_WORK_ON_NEXT_SESSION`, `THREAD_INDEX` files; UTC **`20260405104405`** (`python bin/tick.py` this batch).
...
- **Co-commit note:** The same **`git push`** may include **additional** workspace paths (e.g. **login** / **i18n**, **import SQL**, **data hub** handlers, **PRD 00** / **11**, **AGENTS**, images, **`logs`**) not itemized in this **CHANGELOG** bullet — verify with **`git log -1 --stat`** / **GitHub** diff. **Do not** attribute those paths to this narrative without per-file evidence.
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); **`logout.php`**; **`includes/themes/default/layouts/admin_layout.php`**; **`includes/css/admin-intro-scroll.css`**; **`includes/js/admin-intro-scroll.js`** (comment); version folder **`docs/versions/4.0.94/`** — `decisions/20260405001004_DECISION_APPROVED_admin_nav_logout_intro_cursor_thread.md`, `comments/20260405001004_COMMENT_cursor_session_end_admin_nav_logout_handoff.md`, `CHANGELOG`, `PLAN` Phase **L**, `TODO`, `edges`, `WHAT_TO_WORK_ON_NEXT_SESSION`, `THREAD_INDEX` files; UTC **`20260405001004`** (`python bin/tick.py` this batch).
- **WHAT:** **`sessionStorage` key `lupo_admin_scroll_intro_v1`** survived logout in the **same tab**, so the admin scroll intro skipped on second login — **fix:** logout returns minimal HTML that **`removeItem`** then redirects to **`login.php`**. **Left nav:** **90×60** logo link (defaults **`images/logoface.png`** → **`index.php`**; overridable **`$admin_nav_logo_*`**). **Right nav:** removed **`ACTOR:`** prefix; display name **15** chars + **`..`** when longer (**`mb_*`** when present); full name in **`title`**. CSS: **`max-width: 12em`** on actor text.
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); **`docs/versions/4.0.94/`** — `decisions/THREAD_INDEX.md`, `comments/THREAD_INDEX.md`, `edges.md`, `PLAN.md` Phase **K**, `TODO.md` (completed + open follow-ups), `WHAT_TO_WORK_ON_NEXT_SESSION.md`; UTC **`20260404175352`** (`python bin/tick.py` this batch).
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); **`docs/prd/18_channel_chat_display.md`** (*Multi-actor routing*), **`36_rose_multi_persona_synthetic_dialog.md`** §1.3 switchboard, **`37_kairos_channel_memory_consolidation.md`** §10.6 chat context, **`31_implementation_folder_guidelines.md`** cross-reference table, **`05_auth_user_actor_agent_transformation.md`** channel communication model; UTC **`20260404174956`** (`python bin/tick.py` this batch).
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); **`docs/prd/37_kairos_channel_memory_consolidation.md`** new **§10** (Temporal discipline / anti-backwards reads), renumber open questions / references; **`docs/doctrine/AGAPE_DOCTRINE.md`** §1.3 Temporal awareness; **`scripts/scaffold_implementation.py`** **`add-status`** subcommand (backward-compatible legacy CLI); **`docs/prd/31_implementation_folder_guidelines.md`** usage block; UTC **`20260404173921`** (`python bin/tick.py` this batch).
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); `docs/prd/00_root_constitutional_system_requirements.md` **§14.6** + edge, new **`docs/doctrine/AGAPE_DOCTRINE.md`**, `rules/root/lilith-noninterference-doctrine.md`, `.cursor/rules/lilith-noninterference-doctrine.mdc`, `docs/doctrine/LUPOPEDIA_HEADERS/VALIDATORS_AND_TOOLING.md`, `docs/prd/07_agents_faucets.md`, `docs/prd/36_rose_multi_persona_synthetic_dialog.md`, `agents/agape/*`, `agents/rose/system_prompt.txt` (+ v1.0.0), scrub **`SEMANTIC_SECURITY_CHECKLIST_4_0_30.md`**, **`WHAT_LUPOPEDIA_IS.md`**, **`VERSION_PLANS_3.0.82_3.0.88.md`**; UTC **`20260404172442`** (`python bin/tick.py` this batch).
...
# [2026-04-04] WordPress study tree path — archive/legacy/wordpress-reference; .gitignore note; packager exclude archive/ (Cursor)

- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); `README.md`, `AGENTS.md`, `docs/prd/00_root_constitutional_system_requirements.md` §15 table + prose, `docs/prd/33_softaculous_certification_4_1_0_gate.md`, `docs/doctrine/LEARNED_FROM_WORDPRESS.md`, `docs/implementations/33_softaculous_certification_4_1_0_gate/SOFTACULOUS_PACKAGE_BUILD.md`, Q/A/status threads under implementation 33, `scripts/build_softaculous_package.sh`, `.gitignore` comment; UTC **`20260404165054`** (`python bin/tick.py` this batch).
- **WHAT:** Canonical path **`archive/legacy/wordpress-reference/`** (replacing repo-root **`wordpress-reference/`**); documented **`.gitignore`** on **`archive/`**; **`rsync`** **`--exclude='archive/'`** (retains exclude of legacy root **`wordpress-reference/`**).
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); `scripts/scaffold_implementation.py`, `docs/prd/31_implementation_folder_guidelines.md` (scaffold behavior paragraph), `docs/implementations/README.md` (scaffold note); UTC **`20260404164842`** (`python bin/tick.py` for PRD 31 headers).
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); `docs/prd/00_root_constitutional_system_requirements.md` **§5.8**, `docs/prd/31_implementation_folder_guidelines.md`, `docs/implementations/README.md`, `IMPLEMENTATION_QUESTIONS_GUIDE.md`, `docs/prd/29_project_structure.md`, `AGENTS.md`, `SERVICE_AGENT_ARCHITECTURE.md` §7, `scripts/scaffold_implementation.py`, `implementations/service_agents/README.md`, `implementations/36_rose_multi_persona_synthetic_dialog/README.md`; UTC **`20260404163615`** (`python bin/tick.py` this batch).
- **WHAT:** Documented **`prd_file_stem`** rule (implementation folder name = canonical PRD **`docs/prd/{stem}.md`** basename without **`.md`**); forbidden examples; scaffold **`--title`** alignment; cross-links among **PRD 31**, **§5.8**, index, **AGENTS** mandatory literacy + behavior **5**, **PRD 29** table cleanup.
- **WHY:** Prevent misnamed **`docs/implementations/`** directories and make **decisions/questions/answers** location obvious for IDE agents.
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); renamed **`docs/implementations/prd_36_rose/`** → **`docs/implementations/36_rose_multi_persona_synthetic_dialog/`**; edges and prose in PRD 00, PRD 36, `SERVICE_AGENT_ARCHITECTURE.md`, `implementations/service_agents/README.md`; UTC **`20260404163220`** (`python bin/tick.py` this batch).
...
# [2026-04-04] ROSE synthetic choir — constitution §5.10.3, PRD 36, SERVICE_AGENT_ARCHITECTURE, `agents/rose`, implementation `36_rose_multi_persona_synthetic_dialog/` (Cursor)

- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); `docs/prd/00_root_constitutional_system_requirements.md` **§5.10** table + **§5.10.3**, `docs/prd/36_rose_multi_persona_synthetic_dialog.md`, `docs/doctrine/SERVICE_AGENT_ARCHITECTURE.md`, `docs/implementations/service_agents/README.md`, `docs/implementations/36_rose_multi_persona_synthetic_dialog/` (README, `status/`, `decisions/20260404_162844_DECISION_rose_batch_every_10_messages.md`), `agents/rose/agent.json` + `versions/v1.0.0/agent.json`; UTC **`20260404162844`** (`python bin/tick.py` first batch); folder path corrected **`20260404163220`** (see entry above).
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); `docs/prd/00_root_constitutional_system_requirements.md` §5.1–§5.10, `docs/doctrine/SERVICE_AGENT_ARCHITECTURE.md`, `docs/doctrine/LUPOPEDIA_HEADERS/README.md` (THOTH grounding), `docs/implementations/service_agents/`, `docs/implementations/33_softaculous_certification_4_1_0_gate/SOFTACULOUS_PACKAGE_BUILD.md`, repo root `lupopedia-config-sample.php` + `README.txt`, `includes/bootstrap.php`, `install.php`, `index.php`, `scripts/build_softaculous_package.sh`; version-folder receipt `decisions/20260404_161001_DECISION_APPROVED_service_agent_architecture_and_softaculous_auto_installer_docs.md`, `comments/20260404_161001_COMMENT_cursor_service_agents_softaculous_version_doc_sync.md`; UTC **`20260404161001`** (`python bin/tick.py` this batch).
- **WHAT (thread-verified only):**
  - **Constitution PRD 00 — Section 5:** Expanded **§5.1–5.2** (agents as blueprint, actors as hybrid instances); **§5.7** KAIROS memory consolidation (invocation, `lupo_actor_memory`, `lupo_edges`); **§5.8** implementation mirroring under `docs/implementations/<prd_slug>/`; **§5.9** THOTH stale-artifact audit epoch; **§5.10** service agents — **explicit roster** **IRIS**, **ANUBIS**, **ROSE**, **THOTH**, **KAIROS**, table vs **`RuntimeActorLoopService`** / **`runtime_actors.yaml`** conversational path; YAML edges to `SERVICE_AGENT_ARCHITECTURE.md`, `implementations/service_agents/README.md`, KAIROS PHP, `iris.php`, PRD 36/37.
  ...
  - **LUPOPEDIA_HEADERS README:** **THOTH** — binding **grounding rule** (evidence from `database/lupopedia/json/*.json` + `docs/database/lupopedia/tables/active/`; not unchecked model parametric knowledge); cross-refs to constitution §5.9/5.10 and service agent doctrine.
  - **Implementation mirror:** `docs/implementations/service_agents/` — README, `status/`, `decisions/` (incl. `20260404_160645_DECISION_php_first_service_agents.md`), Q/A/C THREAD_INDEX stubs.
  - **Softaculous / auto-installer:** **`SOFTACULOUS_PACKAGE_BUILD.md`** — §1b silent install contract, §1c `lupopedia-config-sample.php`, bootstrap self-heal note, summary table (`lupopedia-config-sample`, exclude live `lupopedia-config.php`, `license.txt` / `README.md` / `README.txt`), §10 subdirectory JS; **`lupopedia-config-sample.php`** (`[[softdb*]]`, `define()`-style); **`README.txt`**; **`bootstrap.php`** — `mkdir` for `cache`, `logs`, `uploads`, `tmp`; **`install.php`** — `LUPO_INSTALLING`; **`index.php`** — comment that config present ⇒ no redirect to wizard; **`build_softaculous_package.sh`** — exclude `lupopedia-config.php` / backup from zip.
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); `docs/prd/00_root_constitutional_system_requirements.md` §15, `docs/doctrine/LEARNED_FROM_WORDPRESS.md`, `SEMANTIC_MONITORING_DOCTRINE.md`, `CHAT_UI_JAVASCRIPT_SHARED_STATE_DOCTRINE.md`, `docs/prd/33_*` / `28_*` / `18_*` edges, `install/InstallWizardHtaccessWriter.php`, `scripts/build_softaculous_package.sh`, `docs/implementations/33_softaculous_certification_4_1_0_gate/`; UTC **`20260404074421`** (`python bin/tick.py` this batch).
- **WHAT (thread-verified only):**
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**), LILITH audit (`actor_id` **2**); `docs/versions/4.0.94/decisions/`, `questions/`, `answers/`; UTC **`20260403222041`** (`python bin/tick.py` this batch).
- **WHAT (thread-verified only):**
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); **`ActorService.php`**, **`AuthSessionManager.php`**, **`docs/prd/15_actors.md`**, **`docs/prd/25_departments_system.md`** (`lupo_actor_departments` columns), **`docs/prd/07_agents_faucets.md`**, **`docs/prd/32_actor_authority_agent_roles.md`**, **`AGENTS.md`**, **`docs/versions/4.0.94/CHANGELOG.md`**; UTC **`20260403211538`** (`python bin/tick.py`).
...
# [2026-04-03] `actors/` — COUNTERMEASURE hub at `111/` + registry/doctrine alignment (Cursor)

- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); **`actors/111/`** (moved from **`countermeasure/`**), **`database/lupopedia/actors/registry.json`**, **`docs/doctrine/IDENTITY_LAYERS_DOCTRINE.md`**, **`docs/doctrine/ACTOR_PRIMARY_KEY_DOCTRINE.md`**, **`actors/README.md`**, **`ActorService.php`** (docblock); UTC **`20260403210921`** (`python bin/tick.py`).
- **WHAT:** Registry **`dir`** for COUNTERMEASURE → **`actors/111`** (PRD 00 §5.6: reserved **`actor_id` &lt; 2026** use **`actors/{actor_id}/`**). Docs clarify runtime **`actor_id` ≥ 2026** → **`actors/YYYY/MM/{actor_id}/`**; slug-only **`actors/countermeasure/`** removed as incorrect actor hub. **`agents/countermeasure/`** unchanged (agent_key namespace).
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); **`scripts/generate_toon_files.py`**, **`scripts/generate_toon_from_sql.py`**, **`docs/versions/4.0.94/CHANGELOG.md`**; UTC **`20260403193256`** (`python bin/tick.py`).
- **WHAT:** **`clear_toon_files`** now deletes **all regular files** in **`database/lupopedia/json/`** and **`.../toon/`** before writing live-DB exports (primary workflow: schema mirrors the database, no orphans). Fixes **`FileNotFoundError`** on **`unlink`**: **`toon/*.toon.json`** matched both **`*.json`** and **`*.toon.json`**, so the same path was removed twice. Docstrings: **DB-first** vs **`generate_toon_from_sql.py`** (offline install-SQL + targeted **`lupo_*`** prune only).
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); **`scripts/generate_toon_from_sql.py`**, **`scripts/generate_toon_files.py`**; UTC **`20260403192553`** (`python bin/tick.py`).
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); **`install_new_lupopedia.sql`**, **`ActorService.php`**, **`AuthSessionManager.php`**, **`AuthService.php`**, **`generate_toon_from_sql.py`**, **`database_audit_fresh.py`**, **`database/lupopedia/toon/`**, **`docs/prd/05_auth_user_actor_agent_transformation.md`**, **`docs/database/.../lupo_actor_auth_users.md`**, **`docs/versions/4.0.94/CHANGELOG.md`**; batch UTC **`20260403192018`** (real UTC via `python bin/tick.py`).
- **WHAT (verified):**
  ...
  - **Tooling** — **`generate_toon_from_sql.py`** replaces **`{{prefix}}`** with **`lupo_`** before parsing so TOON generation is non-zero; **166** TOONs written under **`database/lupopedia/toon/`**.
  - **Repo hygiene** — Removed stale **`database/lupopedia/json/`** exports for dropped tables; **`database_audit_fresh.py`** priority table list updated; PRD **05** + **`lupo_actor_auth_users`** table doc aligned with pairing + concurrent sessions.
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); **`scripts/find_edges.py`**; version doc touch UTC **`20260403143059`** (real UTC via `python bin/tick.py`).
- **WHAT (verified):**
  - **New script** — **`scripts/find_edges.py`**: scans markdown for links, PRD references, optional keyword hints, code/tree paths; optional **`--headers`** (`##` match across `docs/`); prints suggested **`outbound_edges`** with weight + reason; **does not write** by default.
...
- **Artifacts:** `scripts/find_edges.py`, `docs/versions/4.0.94/CHANGELOG.md` (this entry), `docs/versions/4.0.94/WHAT_TO_WORK_ON_NEXT_SESSION.md`.

# [2026-04-03] Doctrine audit tooling, version ghosts, mobile / workflow doctrines (Cursor + LILITH thread)

- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**), LILITH audit framing (`actor_id` **2**), orchestrator WOLFIE (`actor_id` **1**); **`docs/doctrine/`**, **`docs/prd/`**, **`AGENTS.md`**, **`docs/LESSONS_LEARNED_FROM_THE_WILD_WEST.md`**, **`docs/versions/4.0.94/`**; documentation batch UTC **`20260403140552`** (real UTC via `python bin/tick.py`).
- **WHAT (thread-verified only):**
  - **`python scripts/audit_doctrine_prd_edges.py`** — **189** files under `docs/doctrine/` with **`lupopedia.edges`** including PRD lineage (**0** missing).
  - **`docs/implementations/29_project_structure/status/version_ghosts_report.json`** — **34** files with **critical** ghost findings (scanner: **`scripts/find_version_ghosts.py`**).
  - **Repository scripts (existence only — no verified batch counts in this thread):** `audit_doctrine_prd_edges.py`, `find_version_ghosts.py`, `fix_doctrine_headers.py`, `apply_doctrine_prd_lineage.py`, `convert_wolfie_to_lupo.py` under **`scripts/`**.
  - **Documentation written or materially updated in this thread:** **`MOBILE_SEPARATION_DOCTRINE.md`** (Two-UI, admin desktop-first exception, Eye / PRD 28 split); **`WOLFIE_WORKFLOW_DOCTRINE.md`** (consumer mobile-first / admin desktop-first); **`docs/prd/35_mobile_native_app_separation.md`** (draft); **`docs/prd/33_softaculous_certification_4_1_0_gate.md`** (§7.4 mobile checklist + edges, where edited in-repo); **`AGENTS.md`** (mobile, Two-UI, workflow, Eye, hand-coding policy); **`docs/LESSONS_LEARNED_FROM_THE_WILD_WEST.md`** (hand-coding doctrine, UI-framework note).
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); **`docs/doctrine/DYNAPI_DOCTRINE.md`**; file header **`last_modified_utc` / `when_updated` → `20260403031423`** (real UTC via `python bin/tick.py`); **`lupopedia.footer.last_verified` → `20260402`** (LILITH audit date, 8-digit form per universal validator).
- **WHAT (this thread only — verified):**
  - **New doctrine** — **`DYNAPI_DOCTRINE.md`**: DynAPI (Dan Steinman / Bob Clary) as **approved in-tree** DHTML helper; **IDE** must-not / must-do; compact API reference; **WOLFIE Eyes** traceability via **`docs/prd/28_semantic_monitoring_widget.md`**, **`includes/js/crafty_syntax_eyes.js`**, canonical **`includes/js/dynlayer.js`**; **`lupopedia.edges`** include **PRD 33** (Softaculous / gate — §8 in-tree libraries), PRD 28 path, and code paths aligned to files that exist in-repo.
  - **Tooling:** `python scripts/validate_lupopedia_headers_universal.py docs/doctrine/DYNAPI_DOCTRINE.md` — exit **0** (optional INFO: no `content_id`).
- **WHY:** Single canonical doc so IDE agents stop “replace DynAPI” churn; matches **PRD 33** approved-library posture and **DynAPI** usage already loaded from **`includes/ui/ui-loader.php`**.
...
- **Artifacts:** `docs/doctrine/DYNAPI_DOCTRINE.md` only.

# [2026-04-03] PRD 31 — LILITH final audit merged + 4.0.94 version sync (Cursor thread)

- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); **`docs/prd/31_implementation_folder_guidelines.md`**, **`docs/versions/4.0.94/`**; PRD 31 doc stamps UTC **`20260403024822`**; version-folder sync UTC **`20260403025155`** (thread filenames **`20260403_025155`** … **`20260403_025158`**).
- **WHAT (this thread only — verified):**
  ...
  - **Tooling:** `python scripts/validate_lupopedia_headers_universal.py docs/prd/31_implementation_folder_guidelines.md` — exit **0** (existing WARN: **`lupopedia.schema: prd`** not in small schema list).
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` **102**); **`docs/prd/33_softaculous_certification_4_1_0_gate.md`**, **`docs/implementations/33_softaculous_certification_4_1_0_gate/`**, **`docs/versions/4.0.94/`**; version-folder update stamp **UTC `20260403022543`** (thread filenames **`20260403_022543`** … **`20260403_022546`**).
- **WHAT (this thread only — no PRD 16/26/30/31 validator claims unless edited in-repo with evidence):**
  ...
  - **Implementation workspace** — **`docs/implementations/33_softaculous_certification_4_1_0_gate/`** (README, changelog, todo, authors, edges, `status/` including LILITH audit import, typed thread folders) — *already present from prior work; this pass records it in the version graph.*
...
- **WHAT (thread-verified only):**
  - **`agents/`** — thin facet packs: `kiro/`, `windsurf/`, `warp/`, `cascade/`, `vscode-ide/`, `trae/` (`agent.json`, `capabilities.json`, `properties.json`, `system_prompt.txt`), each `extends_shared` → `_shared/ide_facet_base_system_prompt.txt`; propagation metadata (`rules_propagation_target` **vscode** for `vscode-ide`, **pending** for `warp` / `trae`).
  - **`actors/`** — hub **`README.md`** for **`100`**, **`101`**, **`104`**, **`105`**, **`106`**, **`107`** (links to registry, shared prompt, propagation or pending note).
  - **`scripts/propagate_agent_rules.php`** — valid target **`vscode`**; **`write_vscode_outputs()`** → **`.vscode/lupopedia/rules/`**, `lupopedia_rules.json`, `README.md` (does not overwrite root `.vscode/settings.json`).
  - **`database/lupopedia/actors/registry.json`** / **`actor_id/registry.json`** — facet entries including **`vscode-ide`** (`actor_id` **106**, `agent_id` **113`) and **`trae`** (`actor_id` **107**, `agent_id` **114**); **`agents`** map entries for IDE slugs (aligned in thread).
  ...
  - **`docs/doctrine/AGENT_REGISTRY.md`** — actor rows **106/107**, propagation matrix **`vscode`**, **`lilith`**/**`lexa`** in valid-targets text; removed stale **zencoder** propagation row; capability matrix dedupe.
  - **`agents/_shared/README.md`** — facet pack table expanded.
  - **`scripts/validate_actor_identity.py`** — **`IDE_FAUCETS`** set: **`vscode-ide`**, **`trae`**, **`antigravity-ide`**; **`zencoder`** removed.
...
- **WHO / WHERE / WHEN:** Cursor (`actor_id` 102), `docs/versions/4.0.94/`; version-folder header sync UTC **`20260402225416`** via `python bin/tick.py` (thread artifacts filenames **`20260402_225223`** / **`20260402_225226`**).
...
- **`docs/doctrine/IDENTITY_LAYERS_DOCTRINE.md`:** New **§3** canonical actor/agent/facet/directory rules; facet **`actor_id`** model; **no** hardcoded `auth_user`; **registry.json** authority; **IdGenerator** / PRD 01 for generated ids; renumbered sections **4–8**; timestamps **`20260404220000`**; edge to `comments/20260404_220000_COMMENT_…`.
...
- **`decisions/`:** APPROVED decision [20260404_200000_DECISION_APPROVED_documentation_coordination_channel_semantic_mood_vector.md](decisions/20260404_200000_DECISION_APPROVED_documentation_coordination_channel_semantic_mood_vector.md) — WHO/WHAT/WHERE/WHEN/WHY/HOW for PRD 17 alignment, channel layout, Mood Vector thread, root README → 4.0.94, archive `42/` link fixes.
...
- **`docs/prd/32_actor_authority_agent_roles.md`:** PRD defining actor hierarchy, approval authority matrix, and red team agent roles.
- **`docs/ACTOR_AUTHORITY_QUICK_REFERENCE.md`:** Decision trees and approval chains for fast reference.
...
- **`docs/prd/30_channel_usage_patterns.md`:** PRD defining clear boundaries between channels (coordination) and docs (documentation).
- **`docs/prd/31_implementation_folder_guidelines.md`:** PRD for implementation folder scaffolding, question lifecycle, and decision logging.
- **`docs/CHANNEL_VS_DOCS_QUICK_REFERENCE.md`:** Decision tree and usage patterns for quick reference.
- **`docs/IMPLEMENTATION_FRAMEWORK_SUMMARY.md`:** Complete framework overview and implementation summary.
- **`scripts/scaffold_implementation.py`:** Automated implementation folder creation with UTF-8 encoding.
- **`scripts/validate_framework_compliance.py`:** Framework compliance validation tool.
...
# [2026-04-04] Channel `semantic` + thread `mood_vector_system`

- **`channels/0/semantic/mood_vector_system/`** — `THREAD_MANIFEST.md`, `README.md`, `decisions|questions|answers|comments/` + two APPROVED decisions (evidence sources / color definitions).
- **`channels/channel_index.md`:** Row for **semantic** (Semantic & Knowledge Systems).
- **`docs/doctrine/MOOD_VECTOR_DOCTRINE.md`:** Summary doctrine; canonical thread + decisions; archive evidence edges live in the evidence decision file.

# [2026-04-04] Numeric channel path scan — fix broken `channels/42/` links

- Active tree has **no** `channels/42/` (numeric exemplars live under **`channels_before_4_0_93/42/`**).
- **`rules/root/CHANNEL_ARTIFACT_ROUTING_DOCTRINE.md`:** §1 clarification; §6–8 relative links retargeted to `channels_before_4_0_93/42/...` (**archive**).
- **`docs/implementations/25_.../THREAD_INDEX.md`**, **`26_.../DECISION_INDEX.md`:** “Related threads” links point at archive `42/threads/`.

# [2026-04-04] Phase A — `.cursorrules` §30 + channel path wording

- **`.cursorrules`:** New **§30 Channel filesystem paths** — active `channels/{federation_node_id}/{channel_key}/{thread_key}/`, legacy numeric `.../{channel_id}/threads/{thread_id}/`, archive `channels_before_4_0_93/`, REST vs filesystem note.
- **`rules/root/CHANNEL_ARTIFACT_ROUTING_DOCTRINE.md`:** §1.1 human-readable documentation tree (parallel to numeric API-mirrored tree).
- **`rules/root/README.md`**, root **`README.md`**, **`AGENTS.md`:** Channel literacy and coordination bullets updated.
- **`docs/prd/02_channels_discussions.md`**, **`17_decisions_format.md`**, **`21_thread_graduation_doctrine.md`**, **`DOCUMENTATION_ARCHITECTURE.md`**, **`LUPOPEDIA_HEADERS/LUPOPEDIA_HEADERS_FORMAT.md`:** Dual path semantics.
- **`docs/implementations/25_.../THREAD_INDEX.md`**, **`26_.../DECISION_INDEX.md`:** Legacy vs active links.

# [2026-04-04] PRD 17 thread filenames, PRD 02/29 alignment, org thread + schema

- **`docs/prd/17_decisions_format.md`:** Authoritative **Thread filename pattern** (per-folder `TYPE`/`STATUS`, `HHIISS`, optional `YYYYMMDDHHIISS` prefix); validator and diagram updates.
- **`docs/prd/02_channels_discussions.md`**, **`docs/prd/29_project_structure.md`:** Cross-links to PRD 17; PRD 29 edge to PRD 17.
- **`docs/versions/4.0.93/README.md`:** Points to PRD 17 for full naming rules; decision example uses `DECISION_APPROVED_…`.
- **`channels/0/organization/prd_29_project_organization/`:** Cherry-pick review comment and thread indexes (PRD 29 coordination).
- **Schema / tooling:** `install_new_lupopedia.sql`, `add_thread_key_to_dialog_threads.sql`, JSON registry files, `generate_toon_files.py`, **`docs/doctrine/JSON_SCHEMA_REFERENCE_DOCTRINE.md`**.

# [2026-04-03] 4.0.93 TODO freeze cleanup + PRD 29 channel strategy

- **`docs/versions/4.0.93/TODO.md`:** Removed all open checkboxes; **Open Work → 4.0.94 Only** pointer; historical `[x]` completions retained.
- **`docs/versions/4.0.94/TODO.md`:** Merged deduplicated backlog from former 4.0.93 open items (installer, Softaculous, Glass, migration, tooling, etc.).
- **`docs/prd/29_project_structure.md`:** Channel filesystem strategy table (old archive vs new tree); coordination path `channels/0/organization/prd_29_project_organization/`.
- **`channels/channel_index.md`:** Added **organization** channel.
- **`channels/0/organization/prd_29_project_organization/`:** New thread scaffold (`README.md`, `decisions|questions|answers|comments/THREAD_INDEX.md`).

# [2026-04-02] Bump GLOBAL_CURRENT_LUPOPEDIA_VERSION to 4.0.94

- `config/global_atoms.yaml` and `includes/version.php` now report **4.0.94** for the working tree (after tag `v4.0.93`).

# [2026-04-02] Scaffold 4.0.94 version directory

- Added working version folder `docs/versions/4.0.94/` with `PLAN.md`, `TODO.md`, `CHANGELOG.md`, `edges.md`, `decisions/`, `questions/`, `answers/`, `comments/`, and `prd/`.
- PRD 30 working copy: `prd/30_prd_development_guide.md` (moved from `docs/prd/`).
