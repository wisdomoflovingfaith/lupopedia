...
lupopedia.headers:
  ...
  file_path_from_root: "docs/versions/4.0.94/PLAN.md"
  web_path: "http://www.lupopedia.com/lupopedia/docs/versions/4.0.94/PLAN.md"
  questions_toon: null
...
---
# file: docs/versions/4.0.94/PLAN.md — delegation: cursor:root — web_path: http://www.lupopedia.com/lupopedia/docs/versions/4.0.94/PLAN.md
...
- [ ] Rewrite PRD 30 as writing guide (not metadata spec) — see **`docs/versions/4.0.95/TODO.md`**
...
- [x] Add locale strings to `includes/lang/en.php`
...
**Epoch:** `2026-04-06 17:30 UTC` (header UTC **`20260406173021`** from **`python bin/tick.py`**).
...
Normative gate and packaging narrative: **`docs/prd/33_softaculous_certification_4_1_0_gate.md`** (4.1.0 line); this phase executes **4.0.94 tarball / installer smoke** on Linux shared-host-class environment.
...
- [ ] **Pre-release:** Clean DB + remove `lupopedia-config.php` + full **`install.php`** run from current tree + smoke — see **`docs/versions/4.0.94/TODO.md`** (T-VERIFY-001–004)
- [ ] Run full regression test suite (`sh scripts/run_tests.sh .`) — **TODO.md** T-VERIFY-005
...
- [x] CHRONOS kernel agent activated (`agents/chronos/` — agent.json, identity.json, tools.json, capabilities.json, system_prompt.txt)
...
- [x] Option B migration: moved `actors/wolfie/` → `actors/1/`, `actors/lilith/` → `actors/2/`; `registry.json` `dir` values and repo path references updated; legacy named folders removed
...
**Epoch:** `2026-04-07 02:00 UTC` (Cursor thread; header UTC **`20260407015813`** from **`python bin/tick.py`**).

### Completed tasks

- [x] Merge **`schema_corrected_core.sql`** + **`schema_corrected_missing.sql`** into **`database/lupopedia/mysql/install/install_new_lupopedia.sql`**
...
4. Track deferred product/docs tasks in **`docs/versions/4.0.95/TODO.md`**
