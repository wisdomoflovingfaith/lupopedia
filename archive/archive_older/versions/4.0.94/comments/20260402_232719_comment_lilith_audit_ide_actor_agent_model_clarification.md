...
lupopedia.headers:
  ...
  file_path_from_root: "docs/versions/4.0.94/comments/20260402_232719_COMMENT_lilith_audit_ide_actor_agent_model_clarification.md"
  web_path: "http://www.lupopedia.com/lupopedia/docs/versions/4.0.94/comments/20260402_232719_COMMENT_lilith_audit_ide_actor_agent_model_clarification.md"
  questions_toon: null
...
findings:
  ...
  recommendations:
    ...
    - "CREATE agents/lead_ide/ or agents/_shared/ with canonical system prompt"
...
```
rules/root/ (shared constraints)
         ↓
agents/lead_ide/ (shared system prompt - optional consolidation)
         ↓
┌------------------------------------+
|  agents/cursor/ (thin wrapper) |
|  - "I am actor_id 102"              |
|  - "Apply lead_ide prompt"          |
+------------------------------------+
         ↓
actors/102/ (Cursor facet identity)
         ↓
...
**There is no OOP inheritance.** `agents/cursor/` does not extend `agents/lead_ide/`.

Instead: `agents/cursor/system_prompt.txt` INCLUDES or REFERENCES `agents/_shared/ide_base.txt`.
...
+-- scripts/propagate_agent_rules.php → outputs to .cursor/, .kiro/, .windsurf/, .cascade/, etc.
```
...
```
agents/cursor/system_prompt.txt:
---
...
Cursor IDE (faucet)
    ↓ operates as
actors/102/ (facet actor)
    ↓ delegates_to_actor_id: 1
actors/1/ (WOLFIE - system root actor)
    ↓ has agent config
agents/wolfie/ (WOLFIE persona)
    ↓ executes with
...
- Create `agents/_shared/` (or `agents/lead_ide/`).
- Add canonical IDE facet base `system_prompt` text (constitutional vetoes, delegation, duties).

**Phase 2: Thin Facet Wrappers**

- Refactor `agents/cursor/system_prompt.txt`, `agents/antigravity-ide/system_prompt.txt`, and other IDE packs to facet-specific lines + pointer to shared body.

**Phase 3: Registry**

- Canonical machine paths: **`database/lupopedia/actors/registry.json`** (lupo_actors) and **`database/lupopedia/actors/actor_id/registry.json`** (`agents` map). Do **not** invent `config/registry.json` for facet identity.
...
**Implementation directive:** Proceed with `agents/_shared/ide_facet_base_system_prompt.txt` (or `lead_ide/`) and thin facet wrappers; track in product backlog / next patch as appropriate.
