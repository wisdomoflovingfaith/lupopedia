...
lupopedia.headers:
  ...
  when_updated: null
  file_path_from_root: "docs/versions/4.0.85/actor_agent_sync_model_docs.md"
...
| Subsystem | Authority |
|-----------|-----------|
| Actor and agent definitions | Filesystem (`agents/`, `database/lupopedia/actors/actor_id/registry.json`) |
...
### 2c. Actor workspace (`actors/{id_or_slug}/`)
...
**Script:** `scripts/import_filesystem_actors_agents_to_db.py`
...
```bash
python scripts/import_filesystem_actors_agents_to_db.py \
  --repo-root . --verbose \
...
**Script:** `scripts/export_db_actors_agents_to_filesystem.py`
...
```bash
python scripts/export_db_actors_agents_to_filesystem.py \
...
python scripts/import_filesystem_channels_to_db.py ...
```
...
| `actors/` vs `agents/` | Ambiguous | Clarified role separation |
