...
lupopedia.headers:
  ...
  file_path_from_root: "docs/versions/4.0.95/CHANGELOG.md"
  web_path: "http://www.lupopedia.com/lupopedia/docs/versions/4.0.95/CHANGELOG.md"
  questions_toon: null
...
---
# file: docs/versions/4.0.95/CHANGELOG.md — delegation: cursor:root
...
- **Status:** **4.0.95** documentation line marked **finalized**; open backlog migrated to **`docs/versions/4.0.96/TODO.md`** (UTC `20260407172944`).
- **PRD / schema alignment:** Memory model and filesystem mapping for **`lupo_memory_nodes`** / **`memory/`** are documented in this changelog (entry below) and cross-linked PRDs **01** and **29**; install/schema merge work remains tracked under **4.0.96** where not yet landed in canonical install SQL.
- **Successor line:** **`docs/versions/4.0.96/`** — **`CHANGELOG.md`**, **`TODO.md`**, **`SUMMARY.md`**, **`MIGRATION_NOTES.md`**, **`PRD_PATCHES/`**, **`SCHEMA_DIFFS/`**, **`NOTES/`**.
...
- **lupo_memory_nodes** table updated: added `memory_slug` column (filesystem-safe, required) for mapping memory nodes to disk as `memory/YYYY/MM/{memory_slug}` (YYYY/MM from `created_ymdhis`).
- **Filesystem:** New `memory/` directory documented in [PRD 29](../../../prd/29_project_structure.md) and created in the repo. Each memory node can be exported as a JSON file in `memory/YYYY/MM/{memory_slug}`.
- **README:** `memory/README.md` explains the folder structure, slug rules, and backup/restore purpose.
...
- **Project Structure:** [PRD 29](../../../prd/29_project_structure.md) updated to include `memory/` in directory tables and important sub-folders, with cross-references to the core identity PRD.
...
**See also:** [PRD 01](../../../prd/01_core_identity.md), [PRD 29](../../../prd/29_project_structure.md), `memory/README.md`.
...
**Carry forward (migrated to 4.0.96):** The three items previously listed here as not completed were moved to **`docs/versions/4.0.96/TODO.md`** (section **Carried Over from 4.0.95**) on UTC `20260407172944`.
...
- **`config/global_atoms.yaml`** — `version`, **`GLOBAL_CURRENT_LUPOPEDIA_VERSION`**: **4.0.95**
- **`includes/version.php`**, **`version.txt`**, **`docs/doctrine/VERSIONING_DOCTRINE.md`** (canonical current version §1), root **`README.md`** / **`CHANGELOG.md`** pointers — aligned to **4.0.95**
- **`rules/root/php-7-4-compatibility.md`** — rule stamp **4.0.95**
...
- **`includes/.htaccess`** — Denies direct HTTP access to **`*.php`** under **`includes/`** (replaces the old Rewrite-based block).
- **`install/InstallWizardHtaccessWriter::getRootHtaccessBody()`** — Aligned with the new no-rewrite root policy for installs that regenerate **`.htaccess`**.

### URL helpers (`includes/functions/auth-helpers.php`)
...
- **`database/.../auth/AuthService.php`** — **`requireLogin()`** / password-change redirects use helpers or equivalent **`index.php`** query URLs.
...
- **`database/.../views/auth/login.php`** — Google/GitHub buttons use slug URLs.
- **`database/.../Controllers/OAuthController.php`** — **`getRedirectUri()`** returns slug form.
- **`config/oauth.example.php`** — Comments show **`index.php?slug=…`** callback examples.
- Post-OAuth redirect to **`admin.php`** (was **`/admin`**, which no longer resolves without rewrite).

### Channel admin samples

- **`database/lupopedia/channels/channel_id/1/admin/settings.php`** — System links use **`index.php?slug=…`** and **`login.php`**; docs entry uses **`resolved_uri=docs`**.
...
- **`docs/reference/architect_background.md`** — New factual timeline reference (education, HPC internships, professional work, Crafty Syntax, 2014 hiatus and Sales Syntax fork, return to Lupopedia).
