...
lupopedia.headers:
  ...
  when_updated: null
  file_path_from_root: "docs/versions/4.0.86/TODO.md"
  web_path: null
  questions_toon: null
...
- [x] Consolidate `config.php` and `lupopedia-config.php` into root.
...
- [ ] Initialize documentation and headers for all 22+ agents in `actors/`.
- [ ] Implement `scripts/validate_headers.py` with cross-language parity.
- [ ] Add `mood_label` and `mood_vector` to `lupo_agents` schema in `install_new_lupopedia.sql`.
- [ ] Finalize `ACTOR_RESOLUTION_CONTRACT.md` for v4.0.86.

## Header Validation Follow-up

### Python Validation Parity
- [x] Shared Python header validation module added (`scripts/lib/header_validation.py`)
...
- [ ] `includes/classes/MetadataExtractor.php` (header-bearing metadata read path)
- [ ] `includes/classes/UrlResolver.php` fallback parser mode
...
- [ ] Update docs/actors.md
- [ ] Update actors/README.md
- [ ] Update docs/ACTOR_IDENTITIES.md
- [ ] Align Channel 58 thread docs with `docs/doctrine/IDENTITY_MODEL.md`

### Identity Doctrine Clarity (LOCK)
- [ ] Confirm canonical definitions are explicit for:
  - auth_user (`auth_user_id` in `lupo_auth_users`)
  - actor (`actor_id`, `actor_slug` in `lupo_actors` and `actors/<actor_slug>/`)
  - agent (`agent_id`, `agent_slug` in `lupo_agents` and preferred `agents/<agent_slug>/`)
...
- [ ] Create JSON definitions for all 22 agents in agents/
...
- [ ] Create folders for all 22 agents in actors/<slug>/
