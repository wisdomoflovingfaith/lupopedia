...
lupopedia.headers:
  ...
  when_updated: null
  file_path_from_root: "docs/versions/4.0.86/PLAN.md"
  web_path: null
  questions_toon: null
...
- **Focus**: Populating `actors/` with compliant headers for 22+ agents.
...
1. **Complete Actor System (Channel 58)**
   ...
   - Cross-link canonical role boundaries with `docs/doctrine/ROSE_DOCTRINE.md`
   - Lock identity layers with `docs/doctrine/IDENTITY_MODEL.md`
   - Keep actor definitions canonical in `lupo_actors` (actor_id) and `actors/<actor_slug>/`
...
4. **Complete Identity Model Lock (System-Wide)**
   - Canonical doctrine source: `docs/doctrine/IDENTITY_MODEL.md`
   ...
   - Ensure where definitions live is explicit:
     ...
     - actors: `lupo_actors` + `actors/<actor_slug>/`
     - agents: `lupo_agents` + preferred `agents/<agent_slug>/` (numeric aliases allowed for compatibility)
