...
lupopedia.headers:
  ...
  when_updated: null
  file_path_from_root: "docs/versions/4.0.79/PLAN.md"
...
- **1.1 Auth (remaining)**  
  - **lupo_auth_providers**, **lupo_auth_audit_log**, **lupo_banned_actors**, **lupo_bans_log** — Update to 4.0.79 LUPOPEDIA_HEADERS with Table Overview, "Where This Table Is Used," column docs from [install_new_lupopedia.sql](../../../database/lupopedia/mysql/install/install_new_lupopedia.sql) where present, indexes, relationships, doctrine notes. Zencoder pattern; namespace auth.
...
- **1.5 Pattern and truth**  
  - Schema truth: [install_new_lupopedia.sql](../../../database/lupopedia/mysql/install/install_new_lupopedia.sql) → TOON → table markdown. Pattern: [TABLE_DOCUMENTATION_4_0_77_STOP_LINE.md](../../status/TABLE_DOCUMENTATION_4_0_77_STOP_LINE.md).
...
- **2.1a Header doctrine + table-doc edges exception (docs + active table docs)**  
  - Update LUPOPEDIA HEADERS doctrine/spec/templates/examples so ordinary docs teach only stable, human-authored header blocks, with active table docs as the explicit exception where verbose edges are required. Populate grounded verbose table-doc edges in `docs/database/lupopedia/tables/active/*.md` when PHP/Python references exist. Track in: [header_doctrine_and_table_edges_update_4_0_79.md](../../status/header_doctrine_and_table_edges_update_4_0_79.md).
