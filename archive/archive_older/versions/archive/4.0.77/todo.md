...
lupopedia.headers:
  ...
  when_updated: null
  file_path_from_root: "docs/versions/4.0.77/TODO.md"
...
4. **Export headers (file → YAML)**
   - [x] Implemented: `php bin/lupo.php headers export <path> [--output=path] [--json]` and `scripts/export_lupopedia_headers.php`. Extracts front-matter YAML from a Markdown file. Export from **DB** (`lupo_metadata`) remains **deferred**.

5. **Import headers (YAML → file)**
   - [x] Implemented: `php bin/lupo.php headers import <target.md> [source.yaml]` and `scripts/import_lupopedia_headers.php`. Replaces header block in target file; body preserved; replacement validated. Import into **DB** (`lupo_metadata`) remains **deferred**.

6. **Validator and round-trip**
   - [x] Validator: `php bin/lupo.php headers validate <path>` and `scripts/validate_lupopedia_headers.php`. Fixtures: `tests/fixtures/headers/`.
   - [x] Round-trip steps documented in `tests/fixtures/headers/README.md`. Export → import → validate verifiable; content-equivalence intended.
...
12. **Plan and report alignment**
    ...
    - [x] Coordination artifacts: `database/coordination/4.0.77/` with README (protocol), header-validator.status, upgrade-validation.status, bayesian-foundation-alignment.status, truth-alignment.status.
...
*This TODO list should be updated as 4.0.77 work progresses. When significant items are completed, update CHANGELOG.md and docs/version.md summaries accordingly.* 
