...
lupopedia.headers:
  ...
  file_path_from_root: "docs/versions/4.0.89/CHANGELOG.md"
  web_path: "http://www.lupopedia.com/lupopedia/docs/versions/4.0.89/CHANGELOG.md"
  questions_toon: null
...
| Area | Description |
|------|-------------|
| **Header doctrine** | Single source of truth in **`rules/root/LUPOPEDIA_HEADERS_DOCTRINE.md`**; taxonomy, validation rules, DB mapping |
...
| **Database authority** | **`install_new_lupopedia.sql`** + **`database/lupopedia/json/*.json`** as canonical schema; no FK/triggers/auto-timestamps in DB |
...
| **PHP agent filesystem safety** | Allowed dirs: **`rules/`**, **`docs/`**, **`channels/`**, **`content/`**; extensions: **`.md`**, **`.txt`**, **`.yaml`**, **`.yml`**, **`.json`**, **`.csv`**, **`.xml`** — **no** **`.html` / `.htm` / `.js` / `.php`**. **`AgentFileWriter`** + deploy doc — **`TODO.md` H9**, **`README.md` #12** |
| **`*` literacy** | Root **`ORGANIZATION.md` §4** + **`docs/ORGANIZATION.md`**; release verify **`TODO.md` H4.4** |

### Agent toolchains

- **IDE agents** (Cursor, Windsurf, Kiro): **full repository write access** (developer tools; **not** limited by **`AgentFileWriter`**); Python toolchain in **`scripts/`**
- **PHP agents** (DeepSeek, OpenAI, Grok via API): **invoke** PHP scripts in **`scripts/`** on shared hosting; **author** markdown/docs only in **safe** trees — see **`TODO.md` H9**
...
- Schema: **`install_new_lupopedia.sql`** only; **`database/lupopedia/json/*.json`** mirrors for tooling
...
**Release scope refocus (2026-03-28):** Tagging **4.0.89** is **conditional on LUPOPEDIA HEADERS readiness** — Python + PHP validation (and operator/web surfacing where required), **import/regenerate** round-trips, **`*` organization** and **IDE rule packs** aligned with **`rules/root/`**, **header-related database** parity, and **documented release-gate tests**. **Agent model:** **`README.md`** (**#5**, **#6**, **#11**, **#12**), **`TODO.md` H4.4**, **H8**, **H9**, **`docs/ORGANIZATION.md` §2.1–§2.2**; dependency-ordered plan: **`PLAN.md`**.

**Running log (2026-03-29):** **`lupopedia.history`** and **`lupo_contents.revision_history`** are the **paired audit trail**. Binding rules live in **`rules/root/LUPOPEDIA_HEADERS_DOCTRINE.md`** (*Dual running log*). **`TODO.md` H7** + **`README.md` criterion 10** require a **recorded** round-trip before tag.

**Product backlog moved to 4.0.90:** Context model database work, Crafty Syntax **implementation**, and documentation-clarity tasks **5.1–5.4** are **not** 4.0.89 exit criteria; they live under **`docs/versions/4.0.90/`** (`README.md`, `TODO.md`, `PLAN.md`, `SCOPE_CARRYOVER_FROM_4_0_89.md`).
...
|---|------------------------|--------|------------|----------------------------------|
| 1 | `rules/root/LUPOPEDIA_HEADERS_DOCTRINE.md` | OK (historical row may still show legacy `content_id` until re-import) | OK (script emitted footer/YAML ordering / timestamp notices) | **OK** |
| 2 | `docs/doctrine/LUPOPEDIA_HEADERS/README.md` | OK | OK | **OK** |
| 3 | `docs/doctrine/LUPOPEDIA_HEADERS/VALIDATORS_AND_TOOLING.md` | OK | OK | **OK** |
...
**Prerequisite fix (same session):** `scripts/lib/header_validation.py` — `file_path_from_root` now allows **hyphens** in path segments (e.g. `rules/`); `thread_id` accepts **numeric or slug** `^[a-z0-9][a-z0-9-]*$` to match binding doctrine and **`HeaderDbSync.php`**. Without this, import failed on canonical paths.
...
| 6 | `*` literacy | Process | **H4.4** — team sign-off |
...
- **Single binding doctrine:** `rules/root/LUPOPEDIA_HEADERS_DOCTRINE.md` only — field matrix, validation, **`lupo_contents` JSON column mapping** (which columns header import sets vs leaves for runtime).
- **Stable alias:** `docs/doctrine/LUPOPEDIA_HEADERS/LUPOPEDIA_HEADERS_DOCTRINE.md` — pointer + **`outbound_edges`** to all import/validate scripts and companion docs (no duplicate binding text).
- **Cross-links:** Repo `README.md`, `rules/root/README.md`, `docs/doctrine/LUPOPEDIA_HEADERS/README.md`, `VALIDATORS_AND_TOOLING.md`, `docs/doctrine/INDEX.md`; root doctrine YAML edges expanded to the same tooling set.
- **4.0.89 narrative:** `docs/versions/4.0.89/HEADER_DB_FIRST_AND_DOCTRINE_CONSOLIDATION_4.0.89.md` — integration note preserving prior actor artifacts (THOTH README, WOLFIE changelog claims, clarity review).
...
**§6 follow-up (later 2026-03-28, universal validator):** **`validate_lupopedia_headers_universal.py`** was brought in line with the same doctrine-heavy checks: optional **`--check-db`** (same drift idea when **`content_id`** is set), **required `lupopedia.headers` keys** (including **`thread_id`** pattern `^[a-z0-9][a-z0-9-]*$` and paired UTC **`when_updated` / `last_modified_utc`**), single **parsed-YAML** path for **`lupopedia.history`**, and **`outbound_edges[].to`** existence checks resolved from the **repository root** (walk up until `scripts` + `includes`). PyYAML imported at module top; log tags stay ASCII-safe on Windows.

### 7. PLAN.md — execution retrospective and 4.0.90 planning guidance (LILITH, 2026-03-28)

**Adds narrative** to the version plan (does not remove ATHENA phase structure): tables of **major deliverables** underplayed in the original plan (header consolidation, binding doctrines, registry removal, `IdGenerator`, WSL doc, manifesto), **planned vs approximate actual** phase order, verdict (**plan not followed sequentially; acceptable as history**), and **recommendations** for **`docs/versions/4.0.90/PLAN.md`** as a **living document** updated during development, with **dependency-based** phasing per **`TASK_PLANNING_DOCTRINE.md`**. Full text: **`docs/versions/4.0.89/PLAN.md`** § *Retrospective — execution vs plan*.

### 8. Version scope split — 4.0.89 headers release vs 4.0.90 backlog (2026-03-28)

**4.0.89** `README.md` / `PLAN.md` / `TODO.md` refocused: **release tag** when **LUPOPEDIA HEADERS** validation (Python + PHP + operator surface), **import/regenerate**, **`*` + IDE rules**, **header-related DB parity**, and **documented import/read/regenerate tests** are satisfied. **4.0.90** folder opened (`README`, `TODO`, `PLAN`, `CHANGELOG`, `SCOPE_CARRYOVER_FROM_4_0_89.md`) for **context model**, **Crafty Syntax execution**, and **doc-clarity** tasks **5.1–5.4** previously under 4.0.89. Retrospective appendix for pre-refocus execution lives in **`docs/versions/4.0.90/PLAN.md`** Appendix.
...
- **IdGenerator Class**: Created `includes/classes/IdGenerator.php` with 4-digit random suffix
...
- **Doctrine layout**: Binding text only under `rules/root/`; `docs/.../LUPOPEDIA_HEADERS_DOCTRINE.md` is alias + tooling edge hub
...
- **IdGenerator Class**: Created includes/classes/IdGenerator.php with 4-digit sequence
- **Migration Script**: Created tmp/fix_registry_auto_increment.sql for existing installations
...
### Database Schema
- `database/lupopedia/mysql/install/install_new_lupopedia.sql` - Removed registry tables

### PHP Classes
- `includes/classes/IdGenerator.php` - NEW timestamp-based ID generator
- `includes/classes/AdminRegistryHandler.php` - Updated with deprecation warnings

### Rules & Documentation
- `rules/root/DATABASE_NEUTRAL_SQL_DOCTRINE.md` - NEW comprehensive doctrine
- `rules/root/DATABASE_DOCTRINE.md` - NEW canonical database doctrine by WOLFIE
- `rules/root/INDEPENDENT_CODERS_MANIFESTO.md` - NEW eternal philosophy by WOLFIE
- `rules/root/README.md` - Updated with binding philosophy section
- `rules/root/WINDOWS_WSL_COMMAND_PATTERNS.md` - Updated with WSL enforcement
- `.windsurf/rules/lupopedia-rules.md` - NEW IDE-specific WSL rules

### Migration & Utilities
- `tmp/fix_registry_auto_increment.sql` - Registry removal migration script

### Documentation
- `docs/versions/4.0.89/CHANGELOG.md` - This file
- `docs/versions/4.0.89/HEADER_DB_FIRST_AND_DOCTRINE_CONSOLIDATION_4.0.89.md` - Integration note (DB-first headers + doctrine SST)
- `docs/versions/4.0.89/README.md` - §4 focus area + §5 PLAN retrospective pointer + Related Documentation link
- `docs/versions/4.0.89/PLAN.md` - LILITH retrospective (actual vs planned deliverables); `outbound_edges` to major artifacts + `TASK_PLANNING_DOCTRINE.md`; footer `next_action` for 4.0.90 living plan
- `rules/root/LUPOPEDIA_HEADERS_DOCTRINE.md` - Expanded edges; DB mapping section (binding); §6 follow-up: `lupo_edges` YAML↔SQL column table; regeneration wording (`build_yaml_data_from_db`)
- `docs/doctrine/LUPOPEDIA_HEADERS/LUPOPEDIA_HEADERS_DOCTRINE.md` - Alias + tooling edges
- `docs/doctrine/LUPOPEDIA_HEADERS/README.md`, `VALIDATORS_AND_TOOLING.md`, `docs/doctrine/INDEX.md` - SST / alias clarity; `VALIDATORS_AND_TOOLING.md` — `validate_lupopedia_headers.py` + `--check-db`
- Root `README.md`, `rules/root/README.md` - Pointers to binding doctrine

### Python (header pipeline)
- `scripts/import_content.py`, `scripts/generate_headers_from_db.py` (module docstring), `scripts/ensure_imported.py`
- `scripts/lib/header_db_sync.py`, `scripts/lib/header_validation.py`, `scripts/validate_lupopedia_headers.py` (`argparse`, `--check-db`), `scripts/validate_lupopedia_headers_universal.py` (parity: `--check-db`, required fields, `thread_id`, repo-root edges, history on parsed YAML)
