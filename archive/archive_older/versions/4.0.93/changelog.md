...
- **Frozen core PRDs** in `docs/prd/`: `00`, `16`, `17`, `26`, `27`, `28`, `29` — each header includes `status: "approved"`.
...
- **PRD 30** moved to `docs/versions/4.0.94/prd/30_prd_development_guide.md` for rewrite; **PRD 31** redesign stub at `docs/versions/4.0.94/prd/31_context_system.md`.
- **Current working version folder**: `docs/versions/4.0.94/` (PLAN, TODO, CHANGELOG, edges, four thread folders, `prd/`).
- **Release pointer**: `archive/versions/4.0.93/README.md` documents the tagged snapshot. On `main` after tag `v4.0.93`, `GLOBAL_CURRENT_LUPOPEDIA_VERSION` is **4.0.94** for ongoing development.

# [2026-04-02] Channel Directory Restructure

- **BREAKING**: New channel structure: `channels/{federation_node_id}/{channel_key}/{thread_key}/`
- **Added**: Standard subfolders per thread: `decisions/`, `questions/`, `answers/`, `comments/`
- **Deprecated**: Old numeric channel directories
- **Migration**: Old channels archived to `channels_before_4_0_93`
...
- **PRD 26 Created** - `docs/prd/26_five_layer_documentation_architecture.md`
...
### PRD 30 Development Guide Corrections (`docs/versions/4.0.94/prd/30_prd_development_guide.md` — working copy; was under `docs/prd/` until 4.0.93 freeze)
...
- **PRD-scoped Decisions** - Defined for implementation-specific decisions in `docs/implementations/{id}_{slug}/decisions/`
- **Version-scoped Decisions** - Defined for system-wide architectural decisions in `docs/versions/{version}/decisions/`
...
### Universal Header Validator (`scripts/validate_lupopedia_headers_universal.py`)
...
### PRD 16 Enhancement (`docs/prd/16_lupopedia_headers.md`)
...
### Constitutional PRD (`docs/prd/00_root_constitutional_system_requirements.md`)
- **§6** — Renamed to schema reference JSON protection (**RULE 93.PROTECT_SCHEMA_JSON**); canonical DDL + `generate_toon_files.py` → `database/lupopedia/json/`; legacy `.toon.json` deprecated for new work.
- **§5.1, §5.5, §5.6** — Agents under `agents/{agent_key}/`; reserved agent ID ranges and filesystem discovery; actor ID semantics and workspace path rules.
...
- **`01_installer_requirements.md`** — Timestamps: bare **`BIGINT`** UTC (no `BIGINT(14)` display width); paths under `docs/prd/`.
- **Header paths** — `file_path_from_root` / `web_path` corrected from `docs/versions/4.0.93/prd/` to `docs/prd/` where files live in grouped PRD dir (`02_data_model`, `03_goals`, `04_lupopedia_js`, `05_auth`, etc.).
- **`19_garbage_collection_system.md`** — Outbound edges use `lupo_paths.json`, `lupo_referers_daily.json`.
- **`18_channel_chat_display.md`** — Illustrative column references use `database/lupopedia/json/*.json`.
...
## [2026-04-01] Deprecated docs/status/ Directory

- **Archived**: 135 historical IDE handoff files moved to `archive/docs-status-archive/`
...
- **WOLFIE Doctrine Update (`rules/root/WOLFIE_DOCTRINE.md`)**: Realigned Founder identity context to accurately document High Performance Computing background, solo-developer survival discipline, and modern orchestration workflow (managing parallel AI IDEs while leveraging Notepad++ for complex search/replace bypasses).
- **TOON Doctrine Update (`docs/doctrine/TOON_DOCTRINE.md`)**: Documented the strategic necessity of TOON (YAML) over JSON specifically for optimizing AI token payloads.

### Documentation Governance & Automation
- **Federation Intake Doctrine**: Created `20_federation_intake_doctrine.md` establishing explicit read-only RAG bounds inside `.cursorrules`. Bootstrapped the structure for integration research in `research/federation_nodes/`.
...
- **Directory Exceptions (`docs/prd/project_structure_prd.md`)**: Established `node_modules/` and `app/` as STRICT EXCEPTIONS totally prohibiting `` prefixing to protect external toolchains and PSR-4 native namespace resolutions.
- **`prompts/` Decommissioned**: Eliminated the decoupled prompts root folder. Migrated historical directives directly into native actor workspaces (`actors/{agent}/prompts/`).
- **`app/` Restored**: Permanently restored `app/` back to canonical `app/` to prevent AI hallucination loops.
- **Loose File Sanitization**: Surgically reviewed 19 loose files sitting at the root. Migrated constitutional scopes to `docs/implementations/` and `docs/doctrine/`. Migrated scripts to `scripts/`, configuration logic to `rules/` and `config/`. Stale testing/JSON debris packed into `archive/`. Preserved `CHANGELOG_ARCHIVE.md` and `CURRENT_UTC` as protected temporal anchors.
- **`.gitignore`**: Fixed end-of-file encoding corruption block (`l u p o...`) and comprehensively added dynamic directories (`tmp/`, `cache/`, `sessions/`).
- **`includes` Defoliation**: Identified and archived 13 AI-hallucinated dead directories out of the includes folder to `archive/includes-archive/`.
...
- **`scripts/export_table_data_csv.py`** — New standalone debugging tool. `EXCLUDED_TABLES` frozenset covers auth_users, sessions, API tokens, faucet credentials, OAuth providers, audit logs, CRM leads, chat transcripts, and bans. Additional keyword filter skips tables with `secret`, `password`, `credential`, `token`, `salt`, or `hash` in the name. Output to `database/lupopedia/csv/` (now gitignored). CLI flags: `--tables`, `--limit` (default 500), `--output-dir`. Loud warnings on every run.

### Database Schema

- **`database/lupopedia/mysql/install/install_new_lupopedia.sql`** — Added `lupo_folders` `CREATE TABLE` block (was genuinely missing). Confirmed `lupo_paths`, `lupo_references`, `lupo_reference_links`, `lupo_hashtags`, `lupo_hashtag_map`, `lupo_folder_map` were already present in the file (in a different section than initially searched).
- **`database/lupopedia/mysql/migrations/add_semantic_navbar_tables_20260401.sql`** — Created as SQL proposal file per section 9.18 protocol. Documents all 7 semantic navbar tables with `{{prefix}}` placeholders and constitutional-compliant DDL.

### Table Documentation

- **`docs/database/lupopedia/tables/active/lupo_paths.md`** — Created missing table doc for `lupo_paths`. Verified columns from TOON JSON. Correct PHP query patterns for prev/next page navigation using `DatabaseFactory` and `LUPO_TABLE_PREFIX`.
...
- Added `database/lupopedia/csv/` to prevent CSV data exports from being committed.

### Project Structure PRD Enhancement

- **`docs/prd/project_structure_prd.md`** — Added "Important Sub-folders" section documenting critical internal documentation directories like `versions/`, `database/lupopedia/tables/`, `doctrine/`, and `knowledge/` to clearly define project documentation hierarchy.
...
- **WOLFIE Doctrine** — `rules/root/WOLFIE_DOCTRINE.md` created as root-level constitutional rule protecting proven code from framework bloat
...
- **Multi-Agent Orchestration Doctrine** — `docs/doctrine/MULTI_AGENT_ORCHESTRATION_DOCTRINE.md` created documenting cascade workflow
...
- **Garbage Collection PRD** — `docs/prd/19_garbage_collection_system.md` created with unified table architecture preserving 2003 pattern
- **GarbageCollector Class** — `includes/classes/GarbageCollector.php` implemented with random execution (1% chance per request) and self-limiting (10,000 rows per run)
- **GC CLI Script** — `scripts/gc.php` created for manual/cron execution with proper shebang and CLI-only enforcement
- **GC Doctrine** — `docs/doctrine/GC_DOCTRINE.md` documenting architectural wisdom of 2003 pattern that kept 1.2M installations running unattended
...
- **Actor-Agent Distinction Doctrine** — `docs/doctrine/ACTOR_AGENT_DISTINCTION.md` created as canonical reference
...
- **PRD** — `docs/prd/18_channel_chat_display.md` updated: canonical message API is `api/channels/{id}/messages` (`channels-api.php`); `LUPOPEDIA_PUBLIC_PATH` for URLs; `format=buffer` / `format=image` documented; ES3 legacy notes; example SQL corrected vs TOON (`lupo_actors.name` / `actor_name`, no `display_name`).
- **`includes/modules/api/channels-api.php`** — GET: `format=json` (default), `format=buffer` (JSON as `text/plain`), `format=image` (HTTP 302 to `/ui/images/digitN.gif` after auth); `whatplace` or `position` (hundreds|tens|ones); `image_metric=time|count`; optional `thread_id` filter; each message includes `dialog_thread_id`; message list excludes deleted rows.
...
- **Client assets** — `ui/js/chat-display.js` (ES3-safe polling/post JSON), `ui/js/chat-display-legacy.js` (XHR/ActiveX + image fingerprint helpers), `ui/css/chat-display.css`; `ui/images/README.txt` for digit GIF provenance.
- **Routing** — `includes/modules/module-loader.php`: slug `channels/{id}/thread/{id}`; `channels-controller.php`: `channels_handle_show($channel_id, $initial_thread_id)`.
- **Assets** — Operator replaced placeholder digit GIFs in `ui/images/` with legacy artwork.
- **Implementation Documentation** — `docs/implementations/channel-chat.md` created with LUPOPEDIA headers (schema=implementation, actor_id=105). Documents API paths, URL routing, fallback chain (fetch → XHR → ActiveX → Image → Buffer), and browser support matrix (IE5+ to modern).
...
- **Agent Directory Restructure**: All agent directories renamed from numeric IDs to meaningful names
  - `agents/1/` → `agents/wolfie/` (Coordination)
  - `agents/2/` → `agents/lilith/` (Coordination)
  - `agents/3/` → `agents/rose/` (Emotional)
  - `agents/6/` → `agents/maat/` (Kernel)
  - `agents/9/` → `agents/thoth/` (Coordination) - **canonical THOTH**
  - `agents/10/` → `agents/chiron/` (Application)
  - `agents/11/` → `agents/athena/` (Coordination) - **canonical ATHENA**
  - `agents/14/` → `agents/hephaestus/` (Application)
  - `agents/15/` → `agents/hermes/` (Application)
  - `agents/16/` → `agents/iris/` (Application)
  - `agents/19/` → `agents/anubis/` (Kernel)
  - `agents/25/` → `agents/atlas/` (Application)
  - `agents/106/` → `agents/vishwakarma/` (Application)
  - `agents/107/` → `agents/themis/` (Kernel)
  - `agents/108/` → `agents/junie/` (Application)
  - `agents/703/` → `agents/asclepius/` (Kernel)
  - `agents/704/` → `agents/apollo/` (Emotional)
  - `agents/705/` → `agents/agape/` (Emotional)
  - `agents/708/` → `agents/thalia/` (Emotional)
  - `agents/709/` → `agents/chronos/` (Kernel)
  - `agents/0/` → `agents/system/` (Kernel)
  - **NEW AGENTS**: zeus (12), hypnos (710), khaos (711), nemesis (109), tyche (110), dionysus (706), sophia (707)

- **Agent Discovery Class**: Created `includes/classes/AgentDiscovery.php` with full API
...
- **Original seeds**: Preserved under `database/lupopedia/mysql/seed/` for history and debugging
- **Build script**: `scripts/build_consolidated_seed_4_1_0.py` regenerates consolidated seed when source files change.
...
- contexts/4.0.93/decisions_context.md created as finalized context
- All grouped PRDs and agent/actor/lease/temporal/header doctrines updated and LILITH-audited
- Validator and scripts updated to support context_id
...
- **Agent System Redesign: Complete transformation from database-driven to filesystem-based architecture**
  ...
  - Updated agents/README.md with comprehensive new system documentation
...
- **AgentDiscovery Class**: `includes/classes/AgentDiscovery.php` with full API
  - **Directory Structure**: `agents/{agent_key}/` with standardized files
