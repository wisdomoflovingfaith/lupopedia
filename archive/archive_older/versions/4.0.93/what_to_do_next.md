...
3. Never manually edit JSON files in database/lupopedia/json/
...
- The root constitutional PRD (00_root_constitutional_system_requirements.md) is now in docs/prd/ and should be treated as the single anchor for all system-level requirements.
- Remove or archive the duplicate in docs/versions/4.0.93/prd/ if not needed for historical reference.
- Ensure all PRDs in docs/versions/4.0.93/prd/ and docs/prd/ reference the canonical root PRD in their outbound_edges.
...
- The plan and goals in docs/versions/4.0.93/prd/README.md and 03_goals_and_success_criteria.md should explicitly reference the root constitutional PRD and doctrine.
...
### 7. Next Steps
- Audit all PRDs in docs/versions/4.0.93/prd/ for compliance and reference to the root constitutional PRD.
...
- **Canonical Agent Model**: Ensure all agent directories (e.g., agents/2/) follow the canonical template and versioning as per PRD_AGENT_DEFINITION_MODEL.md.
...
Directory prefixes remain fixed ()
...
- Per-file seeds under `database/lupopedia/mysql/seed/` are **retained** (history, debugging, multi-actor merge base).
...
Consolidated seed file created: install/seed_lupopedia_4_1_0.sql (sections concatenated in dependency-safe order: registry → actors → seed_4.1.0 → remainder; see scripts/build_consolidated_seed_4_1_0.py)
...
**Observed**
- PLAN/TODO already claimed “dynamic table prefix” and SQL migration; the gap was **centralized `{{prefix}}` substitution in `runSqlFile`** and a **single** runtime seed artifact—now documented in `docs/versions/4.0.93/CHANGELOG.md`, `PLAN.md`, `TODO.md`, `prd/01_installer_requirements.md`.
...
- **Other threads:** Versioned docs in `docs/versions/4.0.93/` were updated incrementally; re-read before editing to avoid undoing parallel work.

**Suggestions**
- After any edit under `database/lupopedia/mysql/seed/`, run `python scripts/build_consolidated_seed_4_1_0.py` and commit both the script and the generated SQL if changed.
...
- Changelog: `/docs/versions/4.0.93/CHANGELOG.md`
- Plan alignment: `/docs/versions/4.0.93/PLAN.md` (Completed + SQL/Installer Migration)
- Session handoff: `/docs/versions/4.0.93/what_to_do_next_session.md`
- Installer PRD: `/docs/versions/4.0.93/prd/01_installer_requirements.md` §3.1
...
- Crafty import: **`/database/lupopedia/mysql/import/import_from_old_crafty_syntax.sql`**.
...
- The build script `scripts/build_consolidated_seed_4_1_0.py` handles prefix migration automatically
- Original seed files preserved for historical and debugging purposes
...
- Legacy non-runtime seed files removed from `database/lupopedia/mysql/seed/` where they were no longer used by installer execution path.
...
- Actor workspace resolution/provisioning now supports sharded path `actors/YYYY/MM/actor_id` with legacy flat path fallback for compatibility.
