...
lupopedia.headers:
  ...
  file_path_from_root: "docs/versions/4.0.93/decisions.md"
  web_path: "http://www.lupopedia.com/lupopedia/docs/versions/4.0.93/decisions.md"
  questions_toon: null
...
| D-49  | Decision  | includes Defoliation                      | ANTIGRAVITY| Completed   | 2026-04-01  |           |
...
| D-54  | Directive | IDE: constitutional anchor edge on every docs/prd/*.md (except 00_root) | CURSOR | Completed | 2026-04-01 | D-05 |
...
| D-26  | Decision  | Channel chat: canonical api/channels + PRD 18 + standalone channel-chat UI | CURSOR | Completed | 2026-03-31 | D-05 |
...
### Decision
Update `decisions.md`, `PLAN.md`, `TODO.md`, `CHANGELOG.md` under `docs/versions/4.0.93/` for this thread only (no speculative items).
...
### Decision
- Created `rules/root/WOLFIE_DOCTRINE.md` as root-level constitutional rule
...
### Context
Applying the `` prefix to ecosystem-standard names breaks external toolchains. `node_modules` breaking shatters JS semantic builds; `app` shatters PHP PSR-4 `App\` namespace resolution, causing chronic AI hallucination.

### Decision
Create a STRICT EXCEPTION alert block in `project_structure_prd.md` formally prohibiting the `` prefixing of `node_modules` and `app/`. `app/` permanently renamed back to `app/`.
...
- Created `database/lupopedia/mysql/migrations/add_semantic_navbar_tables_20260401.sql` as the proposal file.
...
- Created `docs/database/lupopedia/tables/active/lupo_paths.md` as the missing table doc.
...
### Context
User requested to document the important sub-directories within `docs/` in the `project_structure_prd.md` before proceeding with other PRD reviews.

### Decision
- Added an "Important Sub-folders" section to `docs/prd/project_structure_prd.md`.
...
### Consequences
- Clarifies the structure of the `docs/` directory.
...
### Context
A read-only review of all files in `docs/prd/` found internal inconsistencies: constitutional §6 vs §9.9 TOON path wording, `agents/<agent_id>/` vs `{agent_key}`, two competing agent file layouts (`PRD_AGENT_DEFINITION_MODEL.md` vs `01_core_identity.md`), ambiguous “no reserved slots” in `07_agents_faucets.md`, actor ID wording vs registry, overlapping `08_actors.md` / `15_actors.md`, and `05_auth_user_actor_agent_transformation.md` lease wording vs `lupo_actor_auth_users`. Remediation was applied in-repo (see CHANGELOG 2026-04-01 PRD consistency pass).

### Decision
- Align `00_root_constitutional_system_requirements.md` with schema reference JSON under `database/lupopedia/json/`, rule **93.PROTECT_SCHEMA_JSON**, sections **5.5–5.6** (reserved agent IDs, actor ID semantics), updated **§6**, **§9.9**, **§9.16**, **§9.18**; deprecate hand-maintained `.toon.json` trees for new work.
...
- **`01_installer_requirements.md`**: timestamps as bare `BIGINT` (no display width); paths under `docs/prd/`.
- Correct **`file_path_from_root` / `web_path`** where they pointed at `docs/versions/4.0.93/prd/` for files living in `docs/prd/`.
...
### Context
User requested `docs/versions/4.0.93/` `decisions.md`, `PLAN.md`, `TODO.md`, and `CHANGELOG.md` be updated to record the PRD alignment work and constitutional-edge sweep from this thread.
