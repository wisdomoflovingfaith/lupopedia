...
lupopedia.headers:
  ...
  file_path_from_root: "docs/versions/4.0.96/PLAN.md"
  web_path: "http://www.lupopedia.com/lupopedia/docs/versions/4.0.96/PLAN.md"
  questions_toon: null
...
---
# file: docs/versions/4.0.96/PLAN.md — delegation: cursor:root
...
**Split (2026-04-08):** Phases below remain the **reference architecture** for the 4.0.x line. **Actionable task rows** for non-session, non-ladder work are consolidated in **`docs/versions/4.0.97/TODO.md`** and **`PLAN.md`** there. This file’s **TODO** companion is now **session/ladder/seed-band only** — see **`TODO.md`** in this folder.
...
**Objective:** Implement the full `memory` sub-command group in the CLI so that actors (including Claude Code) can read and write memory graph nodes and edges via `php bin/lupo.php memory <subcommand>`.
...
### 2.1 `includes/classes/MemoryCommands.php` **(blocking)**
Implement static handler dispatched from `bin/lupo.php` case `memory`:
...
### 2.2 `bin/lupo.php` — route `memory` command **(can parallel with 2.1)**
Add `case 'memory':` to the switch, passing `$argv` slice to `MemoryCommands::dispatch()`.

### 2.3 Integration tests — `tests/integration/memory_cli_test.php`
...
- Run `python scripts/validate_trust_ladder_registry.py` with `--install-sql` pointing to the canonical install path
...
- Document findings in `docs/versions/4.0.96/status/`
...
- Check whether `scripts/validate_seed_registry.py` (confirmed to exist) validates content-table PKs
...
- Add `doc_arch_version` to any `docs/implementations/` folders that lack it (per PRD 26 `next_action`)

### 7.3 PRD 00 — implementation folder stubs **(can parallel)**
- Per PRD 00 `next_action`: "PRD-scoped work: mirror under `docs/implementations/{prd_file_stem}/`"
...
- `sh scripts/run_tests.sh .` (or equivalent runner)
- All `tests/unit/`, `tests/regression/`, and `tests/integration/` suites green
...
- Run `php bin/lupo.php doctor` → all checks green
