...
lupopedia.headers:
  ...
  file_path_from_root: "docs/versions/4.0.96/CHANGELOG.md"
  web_path: "http://www.lupopedia.com/lupopedia/docs/versions/4.0.96/CHANGELOG.md"
  questions_toon: null
...
---
# file: docs/versions/4.0.96/CHANGELOG.md — delegation: cursor:root
...
**WHO:** **Cursor IDE Agent** (actor_id **102**), orchestrator-directed implementation session. Prior research and recommendations in `docs/versions/4.0.96/status/SESSIONS_RESEARCH.md` are attributed to **Claude Code** (actor_id **116**); this entry documents **code and schema changes applied in-repo** by Cursor in this session.
...
- **Probabilistic GC** — **`Session::maybeProbabilisticGarbageCollect()`**: low probability per web request, **lock file** under **`sys_get_temp_dir()`** (fallback `tmp`), **`DELETE`** by `is_named` + **`last_activity_ymdhis`** cutoffs using **`timestamp_ymdhis::subtractSeconds()`** (not invalid packed-integer subtraction).
...
| GC hook | `includes/classes/SessionManager.php` |
| Request bootstrap | `includes/bootstrap.php` |
| Local install config | `lupopedia-config.php` |
| Canonical schema | `database/lupopedia/mysql/install/install_new_lupopedia.sql` |
| Salt helper (new) | `scripts/generate_session_salt.php` |
| Version docs | `docs/versions/4.0.96/CHANGELOG.md`, `PLAN.md`, `TODO.md` |
| Status handoff (new) | `docs/versions/4.0.96/status/STATUS_SESSION_IDENTITY_AND_GC_20260408161022.md` |

### WHEN

**`20260408161022` UTC** — **2026-04-08, 16:10 UTC** (canonical **`python bin/tick.py`** anchor for this documentation batch).
...
- `php -l includes/classes/SessionManager.php`
...
- Created `docs/versions/4.0.96/LEGACY_GAP_ANSWERS.md` — all 11 legacy-gap
  questions answered with implementation decisions and follow-up issue references

### WHERE (files changed)

- `includes/classes/TrustLadderRegistry.php` (**new**)
- `scripts/sync_trust_ladder_registry_to_db.py` (**new**)
- `tests/unit/trust_ladder_registry_test.php` (**new**)
- `docs/versions/4.0.96/LEGACY_GAP_ANSWERS.md` (**new**)
- `docs/versions/4.0.96/status/STATUS_TRUST_LADDER_REGISTRY_20260408.md` (**new**)
- `database/lupopedia/mysql/install/install_new_lupopedia.sql` (appended: table DDL + 13 seed rows)
- `includes/classes/IdGenerator.php` (modified: registry check in `toCanonicalIdSafe()`)
- `docs/versions/4.0.96/CHANGELOG.md` (this entry)
- `docs/versions/4.0.96/PLAN.md` (updated: TrustLadderRegistry phase marked done)
- `docs/versions/4.0.96/TODO.md` (updated: trust ladder registry items checked off)
...
```
php -l includes/classes/TrustLadderRegistry.php   → No syntax errors
php -l tests/unit/trust_ladder_registry_test.php  → No syntax errors
php -l includes/classes/IdGenerator.php           → No syntax errors
python -m py_compile scripts/sync_trust_ladder_registry_to_db.py → pass
php tests/unit/trust_ladder_registry_test.php     → 47 passed, 0 failed
python scripts/sync_trust_ladder_registry_to_db.py --help → parses correctly
```
...
- Wired validator execution into `scripts/run_tests.sh` (version-scope run for 4.0.96).
...
- `scripts/validate_pseudocode_discipline.py`
- `scripts/validate_thread_structure.py` (new)
- `scripts/validate_edge_linking.py` (new)
- `scripts/run_tests.sh`
- `docs/versions/4.0.96/note_on_seed_range.md`
- `docs/doctrine/CHRONOLOGICAL_TRUST_LADDER.md`
- `docs/versions/4.0.96/CHANGELOG.md` (this entry)

### WHEN

`20260408120642` UTC — 2026-04-08, **12:06** UTC (temporal anchor from `python bin/tick.py`).
...
4. **Pipeline wiring / CI path**
   ...
   - Current scope is `docs/versions/4.0.96` to satisfy 4.0.96 success criteria while avoiding legacy historical folder failures.
...
- `validate_pseudocode_discipline.py` on `docs/versions/4.0.96`: pass
- `validate_thread_structure.py` on `docs/versions/4.0.96`: pass
- `validate_edge_linking.py` on `docs/versions/4.0.96`: pass
...
#### `scripts/audit_edge_integrity.py` (new)
...
#### `tests/unit/trust_ladder_pk_validation_test.php` (new)
...
#### `tests/unit/trust_ladder_canonical_id_test.php` (new)
...
#### `tests/integration/trust_ladder_pdo_stringify_test.php` (new)
...
#### `includes/classes/AdminTrustLadderHandler.php` (new)
...
- **Panel D — Seed actor registry**: reads `database/lupopedia/actors/registry.json`, validates each actor's `actor_id` via `IdGenerator::validateTrustLadderPk($id, 'actors.actor_id')`, shows 18-digit canonical form from `seedActorToCanonicalId()`.
...
#### `bin/lupo.php`
Added `case 'staging-gc':` branch to the switch dispatch: requires `bin/cli/staging_gc.php` if it exists, exits 1 with an error message if not. Added `staging-gc` to the `help` output with options summary.
...
Temporal anchor updated via `python bin/tick.py` at session close.
...
**Test suite**: Plain PHP (no PHPUnit), matching the project's existing test pattern (`$passed`/`$failed` globals, `[PASS]`/`[FAIL]` stdout lines, `exit(1)` on failure). `MockDb` / `DatabaseFactory` stub defined before `require_once IdGenerator.php` to prevent real DB loading. Tests run standalone: `php tests/unit/<file>.php`.
...
**Status files produced this session:** None — work is complete and self-contained. Forward action: run `python scripts/audit_edge_integrity.py` against a seeded DB; run `php bin/lupo.php staging-gc --dry-run` to confirm GC counts; browse `admin.php?section=trust-ladder`.
...
- **`docs/doctrine/CHRONOLOGICAL_TRUST_LADDER.md`** — Normative guardrails (§9), PK storage vs PHP strings (`BIGINT`, `ATTR_STRINGIFY_FETCHES`), **`validateTrustLadderPk`** / **`validateFormat`** / **`toCanonicalIdSafe`** specs, seed rule with **2026** ratification bound + registry authorization, §4 PDO notes, §11–§13 (migration/backfill, performance, tests), **Appendix A** (alternatives rejected), **Further reading** cross-links; Grok/WOLFIE final polish (Captain's Log accuracy: no false `seedActorToCanonicalId` blog claim; seeds stay short).
- **`docs/doctrine/TRUST_LADDER_REGISTRY.md`** — Table participation registry (full / generator_staging / seed_only); links to install validation.
- **`docs/doctrine/RETENTION_POLICY.md`** — Staging soft-delete retention alignment with PRD 19.
- **`content/federation_node/0/captains_log/20260408_CHRONOLOGICAL_TRUST_LADDER.md`** — Captain's Log blog; doctrine links; **`lupopedia.edges`** → CHRONOLOGICAL_TRUST_LADDER.

**WHAT (code):**

- **`includes/classes/IdGenerator.php`** — **`toCanonicalId()`** / **`toCanonicalIdSafe()`** return **strings**; **`validateTrustLadderPk()`** seed band uses **padded string compare** (no `(int)` on 18-digit ids); **`seedActorToCanonicalId()`** per PRD 41 (bcadd / 64-bit / digit-wise fallback); **`numericStringLessThan`** helper.
...
- **`scripts/validate_trust_ladder_registry.py`** — Dual **`CREATE TABLE`** patterns (`{{prefix}}` + literal `lupo_`), **`IF NOT EXISTS`**, case-insensitive compare, **`argparse`** (`--install-sql`, `--registry`), empty-registry warning, extended module docstring.

**WHERE:** Doctrine under `docs/doctrine/`; runtime under `includes/`, `app/`; validation under `scripts/`; narrative under `content/`; **this version bundle** under `docs/versions/4.0.96/` (CHANGELOG, SUMMARY, `status/FOR_CLAUDE_CODE_ON_PK_IDS.md`, THREAD_INDEX).
...
**Handoff:** **`docs/versions/4.0.96/status/FOR_CLAUDE_CODE_ON_PK_IDS.md`** — full doctrine summary + future web UI for install-record maintenance (Claude Code / actor **116**).
...
- **Naming — Honolulu → Chronological Trust Ladder** — Canonical doctrine: **`docs/doctrine/CHRONOLOGICAL_TRUST_LADDER.md`**. **PRD 00** §3.7, **PRD 37** §1.2 / edges, **PRD 38** §4.2, **PRD 41** (headings + consolidation copy), **PRD 42** §8 / edges; **`docs/prd/decisions/pseudocode/38_memory_unification_constitution.pseudo.md`**; **`scripts/generate_prd_shorthands.py`** (PRD 38 shorthand heading). **`claude.md`** — Key Doctrines link + tier summary. Shorthands regenerated (**`--all --force`**); earlier changelog rows below still say “Honolulu” as historical titles for the same PK-band work.
...
- **`agents/lilith/system_prompt.txt`** — LILITH reframed as **constitutional enforcer** and partner to WOLFIE; **AGAPE** acronym + doctrine pointer; **UI** — **`includes/js/layers.js`** (eval-free) vs legacy DynAPI; YAML output checklist; **COUNTERMEASURE (111)** inherits non-interfering review lane.
- **`scripts/generate_prd_shorthands.py`** — Staleness prefers **PRD `when_updated` / `last_modified_utc`** vs shorthand **`last_verified`**, else mtime; **`lupopedia.edges.outbound_edges`** merged into Edge Types; **`RESERVED_RANGES`** documented as prose-only; skip list via **`DEFAULT_SKIP_NAMES`**, env **`LUPO_PRD_SHORTHAND_EXTRA_SKIP`**, **`--skip-name`**.
...
- **`scripts/generate_prd_shorthands.py`** — PRD **38** shorthand embeds **Long-Term Archiving (Option B)** + CLI one-liners.
...
**Status:** `docs/versions/4.0.96/status/STATUS_MEMORY_ARCHIVE_OPTION_B_20260408001717.md`
...
**Status:** `docs/versions/4.0.96/status/STATUS_SEED_RUNTIME_PK_DOCTRINE_20260407235921.md`
...
- **Doctrine (PRD 00)** — §3.2 **registry/seed exception**: fixed low PKs in install/seed vs **`IdGenerator`** for runtime; **`created_ymdhis`** may be install UTC, insert time, or **`0`** (immemorial); **§5.7** companion paragraph for **`lupo_memory_nodes`** / **`lupo_memory_edges`** and **`memory/1970/01/`** when **`created_ymdhis = 0`**.
...
**Status:** `docs/versions/4.0.96/status/STATUS_SEED_PK_CREATED_YMDHIS_MEMORY_EXPORT_20260407235530.md`
...
- **`docs/prd/15_actors.md`** — New **Overview** subsection **Three-layer identity model (4.0.96+; root README §3)** mirroring README: layers table, shared persona, illustrative department-intersection SQL, CLI/IDE root-equivalent note, **`auth_user_id = 0`** vs **`actor_id`**, memory pointers (**PRD 38**). **`lupopedia.edges`** → **`README.md`**.
- **`docs/prd/05_auth_user_actor_agent_transformation.md`** — New **Root README alignment (4.0.96+)** (link to README §3; web vs CLI/IDE; **AGENTS.md**; PRD 01 root id). **`lupopedia.edges`** → **`README.md`**.
...
- **Schema references** — **05, 18, 36, 37** now cite **`database/lupopedia/json/*.json`** instead of **`.toon`** paths.
- **PRD_INDEX** — Banner **4.0.96**; **08_actors** entry annotated **SUPERSEDED** (see **15_actors.md**). **29_project_structure** table row matches. **docs/prd/README** clarifies **14 core namespaces** vs extended PRDs.
...
- **`config/global_atoms.yaml`** — `version`, **`GLOBAL_CURRENT_LUPOPEDIA_VERSION`**, and `versions.*` metadata set to **4.0.96**; top `file.last_modified_system_version` aligned.
- **`version.txt`**, **`includes/version.php`** (`@version` docblock) — **4.0.96**.
- **`docs/doctrine/VERSIONING_DOCTRINE.md`** — §1 canonical current version **4.0.96**.
- **`rules/root/php-7-4-compatibility.md`** — rule stamp **4.0.96+**.
- **Root `README.md`**, **`CHANGELOG.md`** (routing) — current line pointers → **`docs/versions/4.0.96/`**.
- **`docs/versions/4.0.96/README.md`** — added as the version-folder overview for the active line.
...
- **`includes/classes/MemoryExportService.php`** — filesystem mirror from `created_ymdhis` + `generateSlug()`.
- **`docs/prd/38_memory_unification.md`** — full revision (sections renumbered; IdGenerator rules; DDL; section 13 IDE prompt).
- **`docs/versions/4.0.96/status/`** — `PRD_REVIEW_DISCREPANCIES_AND_IMPROVEMENTS_20260407224750.md`, `THREAD_INDEX.md` (PRD index/README drift, memory model fork list).
...
- **`docs/versions/4.0.96/status/STATUS_SESSION_PRD_MEMORY_IDENTITY_20260407232053.md`** — workstream summary + embedded forward prompt (constitutional PRD batch); **`THREAD_INDEX.md`** updated.
...
- **`docs/prd/01_core_identity.md`** — `lupo_actor_memory` aligned to install; session cleanup → soft **`UPDATE`**; `resulted_in_actor_memory_id` in training table prose.
- **`docs/prd/72_tags_metadata.md`** — **`lupo_edges`** section aligned to install; header **`when_updated`**.
- **`docs/prd/09_federation_sync.md`** — actor_memory PK name in summary table.
- **`database/lupopedia/json/lupo_actor_memory.json`** — PK field name aligned.
- **`python scripts/generate_toon_from_sql.py`** — ran (177 TOONs); **removed stale exports** not in install — verify **`git status`**.
...
- **`docs/prd/38_memory_unification.md`** — `lupopedia.edges`: added **amends** to **07_agents_faucets** and **15_actors**; section **6.5** documents **Phase 1 synchronous** vs **Phase 2 optional queue** export; section **10.3–10.5** states amendment scope for PRDs 07, 15, 37; header/footer UTC via **tick.py**.
...
- **`01_core_identity.md`** — File tree: `memory.json` entry replaced with deprecation comment; Actor Creation Flow step 9: "Learned behavior stored as root memory node at `memory/YYYY/MM/{memory_slug}.json`; registered in `lupo_memory_nodes`; all memory relationships expressed via `lupo_edges` (4.0.96+). `memory.json` is deprecated."
- **`15_actors.md`** — Both workspace file trees (system actor and runtime actor sections) updated; `### memory.json (Learned from Department Context)` section replaced with `### Root Memory Node (Learned from Department Context) — 4.0.96+` section (includes example JSON for `memory/2026/04/wolfie-sales-actor-5001.json`, schema reference, edge linkage); Actor Learning Process step 5 updated to reference `memory/YYYY/MM/{memory_slug}.json` + `lupo_memory_nodes` + `lupo_edges`.
- **`07_agents_faucets.md`** — Actor workspace file tree and optional agent files tree: `memory.json` entries replaced with deprecation comments pointing to `memory/YYYY/MM/{memory_slug}.json`.
...
**Canonical replacement language:** "Actor/agent memory is modeled as memory nodes in `lupo_memory_nodes`, linked via edges in `lupo_edges`, with the actor's root memory node exported to `memory/YYYY/MM/{memory_slug}.json`."

### File-Backed Content System — PRD 06

- **`content/` directory** reorganized to canonical structure: `content/federation_node/{id}/{folder_key}/{file_name}` and `content/actor/{actor_id}/{folder_key}/{file_name}`. Old directories (`0/`, `actor_id/`, `federation_node_id/`) removed. Files renamed to snake_case lowercase.
...
- **`docs/prd/06_content_management.md`** — Updated: canonical directory structure, slug rules, `storage_type` rules, migration SQL documented; applied schema corrections reflected; `lupo_folders.description` documented; `when_updated` and `last_verified` updated to `20260407123924`.
...
| WHERE  | 
  - docs/prd/01_core_identity.md
  - docs/prd/06_content_management.md
  - docs/prd/11_analytics_tracking.md
  - docs/prd/13_crafty_integration.md
  - docs/prd/15_actors.md
  - docs/prd/24_actor_onboarding_flow.md
  - docs/prd/37_kairos_channel_memory_consolidation.md
  - docs/prd/PRD_AGENT_DEFINITION_MODEL.md
  - docs/versions/4.0.96/CHANGELOG.md
  - docs/versions/4.0.96/README.md
  - docs/versions/4.0.96/SUMMARY.md
  - docs/versions/4.0.96/TODO.md
  - docs/versions/4.0.96/MIGRATION_NOTES.md
  - content/ (structure, file-backed content)
  - database/lupopedia/mysql/install/install_new_lupopedia.sql
  - database/lupopedia/actors/registry.json
  - actors/116/ (Claude Code)
  - claude.md
  - README.md (root)
...
1. **Memory Model 4.0.96**
  ...
  - Deprecated memory.json; root memory nodes now in memory/YYYY/MM/{memory_slug}.json
...
2. **Actor Separation**
  ...
  - actors/116/ created with identity.json, boundaries.json
...
5. **File-Backed Content System**
  - Canonical directory structure under content/
