...
1. **`scripts/hermes_scan_threads.py`** (new): scan `threads/1001` and `threads/1002`, list `.md` with parsed `artifact_kind`, `message_type`, `actor_id`; output JSON or Markdown index; exit 0.
...
1. Add **`scripts/hermes_scan_threads.py`** — arguments `--repo-root`, `--channel 42`, `--threads 1001,1002` — print a Markdown table: filename | actor_name | artifact_kind | purpose (first line). No DB required.
