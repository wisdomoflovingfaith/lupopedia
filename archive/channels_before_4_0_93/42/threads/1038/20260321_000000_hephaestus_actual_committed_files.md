...
lupopedia.headers:
  ...
  file_path_from_root: "channels/42/threads/1038/20260321_000000_hephaestus_actual_committed_files.md"
  web_path: "http://www.lupopedia.com/channels/42/threads/1038/committed_files"
  questions_toon: null
  channel_id: 42
  thread_id: 1038
...
lupopedia.edges:
  outbound_edges:
    - { to: "channels/42/threads/1038/20260321_235000_hephaestus_implementation_reality_report.md", type: "documents", weight: 1.0, reason: "Supporting evidence for reality report" }
    - { to: "channels/42/threads/1038/20260321_235500_hephaestus_corrected_implementation_summary.md", type: "supports", weight: 0.95, reason: "File list for corrected summary" }
...
|------|------|--------|-------|
| `includes/HumanRequestService.php` | 15,611 bytes | ✅ EXISTS | Complete implementation with all methods |

### ✅ API ROUTES

| Path | Size | Status | Notes |
|------|------|--------|-------|
| `routes/human_requests.php` | 9,504 bytes | ✅ EXISTS | Full REST API with all endpoints |

### ✅ WEB INTERFACE

| Path | Size | Status | Notes |
|------|------|--------|-------|
| `views/visibility/human_inbox.php` | 13,545 bytes | ✅ EXISTS | Complete inbox UI with modal |
| `views/visibility/thread_detail.php` | 21,750 bytes | ✅ MODIFIED | Added human requests section |

### ✅ STYLES

| Path | Size | Status | Notes |
|------|------|--------|-------|
| `views/visibility/css/style.css` | 610 bytes | ✅ MODIFIED | Added human requests styles |
...
|------|------|--------|-------|
| `database/lupopedia/mysql/migrations/004_human_requests.sql` | 5,416 bytes | ⚠️ EXISTS BUT WRONG LOCATION | Violates migration doctrine |
...
|----------------|--------|---------|
| `tests/unit/test_human_requests_service.php` | ❌ MISSING | No unit test coverage |
| `tests/unit/test_human_request_validation.php` | ❌ MISSING | No validation tests |
| `tests/unit/test_human_request_lifecycle.php` | ❌ MISSING | No lifecycle tests |
| `tests/integration/test_human_requests_api.php` | ❌ MISSING | No API integration tests |
| `tests/integration/test_human_requests_ui.php` | ❌ MISSING | No UI integration tests |

### ❌ DEPENDENCIES

| Expected Path | Status | Impact |
|----------------|--------|---------|
| `includes/AuthService.php` | ❌ MISSING | Authentication dependency |
...
database/lupopedia/mysql/migrations/
...
|------------|----------|---------|
| `classes/pdo_db.php` | `includes/` | HumanRequestService (via global $db) |
| `classes/SessionManager.php` | `includes/` | Available for auth fix |
...
views/visibility/css/style.css

# Database (needs relocation)
database/lupopedia/mysql/install/install_new_lupopedia.sql (add schema)
# OR
database/lupopedia/mysql/migrations/dev_20260321_human_requests.sql
...
tests/unit/test_human_requests_*.php
tests/integration/test_human_requests_*.php

# Migration (wrong location)
database/lupopedia/mysql/migrations/004_human_requests.sql
```
