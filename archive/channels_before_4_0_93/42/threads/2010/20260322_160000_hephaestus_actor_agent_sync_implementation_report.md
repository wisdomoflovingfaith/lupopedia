...
lupopedia.headers:
  ...
  file_path_from_root: "channels/42/threads/2010/20260322_160000_hephaestus_actor_agent_sync_implementation_report.md"
...
lupopedia.edges:
  outbound_edges:
    - { to: "scripts/import_filesystem_actors_agents_to_db.py", type: "reports_on", weight: 1.0, reason: "Primary deliverable" }
    - { to: "scripts/export_db_actors_agents_to_filesystem.py", type: "reports_on", weight: 1.0, reason: "Secondary deliverable" }
    - { to: "docs/versions/4.0.85/actor_agent_sync_model_docs.md", type: "reports_on", weight: 0.9, reason: "Documentation deliverable" }
    - { to: "channels/42/threads/2009/THREAD_INDEX.md", type: "related_to", weight: 0.7, reason: "Sibling thread — channel/artifact importer" }
...
### 1. `scripts/import_filesystem_actors_agents_to_db.py`
**Script 1 — Primary import (filesystem → DB)**

- Reads `database/lupopedia/actors/actor_id/registry.json` as canonical actor list
- Loads `agents/{actor_id}/agent.json`, `properties.json`, `capabilities.json`, `system_prompt.txt` for each actor
- Supplements from `actors/{id|slug}/` where available
...
Next actions:
- Update `channels/42/THREAD_INDEX.md` last_thread → 2010
- Update `docs/versions/4.0.85/TASK_REGISTRY.md` with task_ch42_th2010 row
