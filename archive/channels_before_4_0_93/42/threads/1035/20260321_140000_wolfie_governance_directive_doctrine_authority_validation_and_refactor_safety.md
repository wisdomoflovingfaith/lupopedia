...
lupopedia.headers:
  ...
  file_path_from_root: "channels/42/threads/1035/20260321_140000_wolfie_governance_directive_doctrine_authority_validation_and_refactor_safety.md"
  web_path: "http://www.lupopedia.com/channels/42/threads/1035/directive"
...
lupopedia.edges:
  outbound_edges:
    - { to: "channels/42/threads/1034/20260321_120000_thoth_reconciliation_report_post_major_changes_documentation_audit.md", type: "resolves", weight: 1.0, reason: "Response to THOTH reconciliation findings and LILITH review" }
    - { to: "rules/root/MULTI_AGENT_COORDINATION_DOCTRINE.md", type: "extends", weight: 0.95, reason: "Extends multi-agent coordination with doctrine-specific governance" }
    - { to: "docs/doctrine/LUPOPEDIA_HEADERS/README.md", type: "governs", weight: 0.9, reason: "Governs future updates to LUPOPEDIA_HEADERS doctrine" }
    ...
    - { to: "docs/doctrine/CHANNEL_66_QUESTION_GRAPH_DOCTRINE.md", type: "references", weight: 0.85, reason: "Channel 66 is required gateway for certain doctrine changes" }
    - { to: "scripts/propagate_agent_rules.php", type: "governs", weight: 0.9, reason: "Tool execution requires output validation" }
...
|---|----|---|---|---|
| 1 | **WOLFIE Binding Directive** | `channels/*/threads/*/WOLFIE_*_directive.md` | Current | WOLFIE explicitly resolves; immediate effect |
| 2 | **CHANNEL_66 Doctrine** | `channels/66/threads/*/doctrine_resolution.md` (WOLFIE-closed) | Frozen (4.0.84) | WOLFIE-endorsed Channel 66 findings override lower ranks |
| 3 | **Install SQL + TOON Files** | Schema authority | Structural | Database structure is authoritative for DB layer |
| 4 | **Core Doctrine** | `docs/doctrine/` (excluding HEADERS) | Binding | Root doctrine unless overridden by #1-3 |
| 5 | **LUPOPEDIA_HEADERS Doctrine** | `docs/doctrine/LUPOPEDIA_HEADERS/` | Subject to revision | Can be updated by Rank 1-4 authority |
| 6 | **README + plan.md** | Root documentation | Living | Reference implementation guidance; defers to #1-5 |
| 7 | **Active Table Docs** | `docs/database/lupopedia/tables/active/*.md` | Documentation only | Lowest rank; inferred from Install SQL if contradictory |
...
- `scripts/propagate_agent_rules.php` — generates actor rule files
- `scripts/generate_headers_from_db.py` — generates HEADERS from DB state
- `scripts/generate_toon_from_sql.py` — generates TOON files
- `scripts/import_content.py` — ingests HEADERS into DB
