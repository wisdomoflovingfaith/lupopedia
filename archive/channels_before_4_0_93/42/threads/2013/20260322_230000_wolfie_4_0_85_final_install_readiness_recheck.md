...
lupopedia.headers:
  ...
  file_path_from_root: "channels/42/threads/2013/20260322_230000_wolfie_4_0_85_final_install_readiness_recheck.md"
  questions_toon: null
  channel_id: 42
  thread_id: 2013
  actor_id: 1
...
**File**: `database/lupopedia/mysql/install/install_new_lupopedia.sql`  
...
| `config/global_atoms.yaml` line 22: `GLOBAL_CURRENT_LUPOPEDIA_VERSION` | `"4.0.85"` | ✅ CORRECT |
| `config/global_atoms.yaml` line 17: `version` | `"4.0.85"` | ✅ CORRECT |
| `config/global_atoms.yaml` line 3: `file.last_modified_system_version` | `4.0.85` | ✅ CORRECT |
| `includes/version.php` line 9: `@version` docblock | `4.0.85` (fixed this session — was 4.0.84) | ✅ FIXED |
| `config/global_atoms.yaml` lines 253–255, 404 | `4.0.84` in **comments only** | ✅ NON-BLOCKING (comment/archive section) |
...
- Version read at lines 113–115: tries `config/`, then `config/` for `global_atoms.yaml` — file exists at `config/global_atoms.yaml` ✅
- `require_once` chain: `install_wizard_classes.php` → `install/` modules → `includes/` utilities — no dependency on `lupopedia-config.php`
...
|---|---|---|
| Route handler actor resolution | `routes/human_requests.php` lines 74–96 | ✅ Always session-derived; client `actor_id` ignored |
| Atomic idempotency guard | `includes/HumanRequestService.php` + install SQL | ✅ DB UNIQUE constraint; try/catch race-loss detection |
| Failure state handling | `includes/HumanRequestService.php` `routeToHumanMvp()` | ✅ try/catch wraps createRequest(); forces `failed` terminal status |
| Auth user resolution | `includes/HumanRequestService.php` `resolveAuthUserIdForActor()` | ✅ Queries `lupo_actor_auth_users` where `is_primary=1 AND status='active' AND is_deleted=0` |
...
1. Drop all tables
2. Load `database/lupopedia/mysql/import/old_crafty_syntax_3_7_5_start.sql`
3. Run `install.php`
4. Verify upgrade
...
2. `config/global_atoms.yaml` lines 253–255, 404 — archive comment sections referencing 4.0.84; can be cleaned up in a future documentation pass (non-blocking)
