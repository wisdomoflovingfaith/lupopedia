---
lupopedia.headers:
  file_path_from_root: "channels/42/threads/1051/STALE_DOCUMENTATION_CLEANUP_SESSION_SUMMARY.md"
  ...
  web_path: "http://www.lupopedia.com/channels/42/threads/1051/STALE_DOCUMENTATION_CLEANUP_SESSION_SUMMARY.md"
lupopedia.footer:
...
| Organize archived files | ✅ COMPLETE | 64 files organized by category in archive/ |
...
- **File**: `scripts/update_stale_headers.py`
- **Language**: Python 3
- **Functionality**:
  - Scans `docs/doctrine/` recursively
...
- **Usage**: `python scripts/update_stale_headers.py`
...
1. Execute batch update script:
   ```bash
   python scripts/update_stale_headers.py
   ```
...
### Related Files
- `.gitignore` — Updated with `archive/` exclusion
- `archive/README.md` — Archive overview
- `archive/ORGANIZATION.md` — Detailed navigation guide
- `CHANGELOG_ARCHIVE.md` — Restored with updated headers
- `STALE_DOCUMENTATION_ANALYSIS_REPORT.md` — This analysis
- `scripts/update_stale_headers.py` — Automation tool
...
**Next Action**: Execute `python scripts/update_stale_headers.py` when terminal is available
