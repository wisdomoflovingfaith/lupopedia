...
lupopedia.headers:
  ...
  file_path_from_root: "channels/42/threads/1030/20260321_140000_hephaestus_operational_visibility_web_interface_implementation.md"
  web_path: "http://www.lupopedia.com/channels/42/threads/1030/operational_visibility_web_interface"
...
lupopedia.edges:
  outbound_edges:
    - { to: "channels/42/threads/1031/20260320_182000_wolfie_directive_canonical_schema_implementation_database_backed_visibility.md", type: "depends_on", weight: 1.0, reason: "Canonical schema definition for channels, threads, tasks" }
    - { to: "channels/42/threads/1032/20260321_090000_wolfie_directive_canonical_project_model_schema_authority_and_migration_contract_4_0_84.md", type: "depends_on", weight: 1.0, reason: "Project model, project_id schema, actor_projects" }
    - { to: "database/lupopedia/mysql/install/install_new_lupopedia.sql", type: "queries", weight: 0.9, reason: "Schema source of truth for lupo_channels, lupo_dialog_threads, lupo_tasks" }
    - { to: "includes/modules/", type: "extends", weight: 1.0, reason: "New visibility module for operational UI routes" }
    - { to: "includes/ui/", type: "extends", weight: 1.0, reason: "New visibility UI components and layouts" }
    - { to: "channels/42/threads/1038/", type: "defers_to", weight: 0.8, reason: "Verification workflow, human-actor interaction belongs to Thread 1038" }
...
**Location:** `includes/modules/visibility/VisibilityService.php`
 ...
 */
$visibility_module = LUPOPEDIA_ABSPATH . 'includes/modules/visibility/visibility-controller.php';
...
```php
<?php
// File: includes/modules/visibility/visibility-controller.php
...
require_once LUPOPEDIA_ABSPATH . 'includes/modules/auth/auth-check.php';
...
require_once LUPOPEDIA_ABSPATH . 'includes/modules/visibility/VisibilityService.php';
...
if (count($parts) >= 1 && $parts[0] === 'visibility') {
    ...
    if (count($parts) === 1 || $parts[1] === 'channels') {
        ...
        require LUPOPEDIA_ABSPATH . 'includes/modules/visibility/views/channels-overview.php';
        exit;
    }
    ...
    elseif ($parts[1] === 'channels' && isset($parts[2])) {
        ...
        require LUPOPEDIA_ABSPATH . 'includes/modules/visibility/views/channel-threads-list.php';
        exit;
    }
    ...
    elseif ($parts[1] === 'attention') {
        ...
        require LUPOPEDIA_ABSPATH . 'includes/modules/visibility/views/attention-view.php';
        exit;
    }
    ...
    elseif ($parts[1] === 'threads' && isset($parts[2])) {
        ...
        require LUPOPEDIA_ABSPATH . 'includes/modules/visibility/views/thread-detail.php';
        exit;
    }
}
...
```html
<?php require LUPOPEDIA_ABSPATH . 'includes/header.php'; ?>
...
<?php require LUPOPEDIA_ABSPATH . 'includes/footer.php'; ?>
```
...
**Location to insert** (in `includes/modules/module-loader.php` after AUTH/OAuth blocks, before TRUTH):
...
- [ ] Create directory: `includes/modules/visibility/`
...
1. **Create visibility module directory**
   ```
   mkdir -p includes/modules/visibility/views/partials
   ```
