...
lupopedia.headers:
  ...
  file_path_from_root: "channels/42/threads/1002/20260317_183000_lilith_channel-system-review.md"
  web_path: "http://www.lupopedia.com/channels/42/threads/1002/20260317_183000_lilith_channel-system-review.md"
  questions_toon: null
  channel_id: 42
  thread_id: 1002
  ...
  superseded_path: "channels/42/threads/4.0.80/LILITH_CHANNEL_SYSTEM_REVIEW_4_0_80.md"
---
...
- The channel model is implemented in code + schema, but the transition from `docs/status/` is incomplete and partially broken in governance.
...
### Runtime / Code
- `includes/modules/api/channels-api.php` with GET/POST controllers and membership gating.
- `Lupo_Channel_Message_Router` (broadcast/direct/thread) writes `channels/{channel}/{broadcasts|direct/{actor}|threads/{thread}}/*.md` artifacts.
...
### File structure
- Actual channel artifacts under `channels/42/` exist with subfolders: broadcasts, content, direct, rolls, rules, tasks, threads.
- Thread folders like `4.0.80/` exist and are used by at least one artifact.
- There is a direct conflict: docs state “channel artifacts” but doctrine still references `docs/status/`.

## 3. What Is Only Documented or Implied

- `rules/root/MULTI_AGENT_COORDINATION_DOCTRINE.md` still says COM001 coordinates via `docs/status/`; this is inconsistent with channel path implementation.
- `docs/architecture/HOW_ACTORS_ORCHESTRATE_ON_CHANNELS.md` describes actor-channel flows correctly, but does not enforce channel output path rules.
- `agents` and `ACTOR_REGISTRATION_CHECKLIST.md` imply channel assignment from registry but do not specify step-by-step channel-thread artifact naming.
- `rules/root` has placeholder rule COM001 (status-based) and COM003 (no private channels) which contradict partial channel highway.

## 4. Structural Problems

1. Artifact routing ambiguity
   - Responsibility split: `docs/status/` (old doctrine) vs `channels/42/threads/` (intended new system).
...
5. Broken principle with existing docs
   - `docs/status/` remains used by multiple reports and has no auto-redirection to channel.
...
- Forge a single canonical routing model in doctrine and code: **all coordination artifacts for channel 42 must be written under `channels/42/`** (task, broadcast, direct, thread, rules, content). `docs/status/` becomes archival, not primary.
...
- Write a clear minimal reference doc `rules/root/CHANNEL_ARTIFACT_ROUTING_DOCTRINE.md` and enforce in `propagate_agent_rules.php` targets.

### What not to write to status (future)
- No new implementation reports.
- No task updates.
- No code review artifacts.
- Use `docs/status/` only for archival, compliance snapshots, and translation backups.

## 6. Required Rules for Agents

1. **Channel path selection**
   - If working on channel 42 issues, artifact base must be: `channels/42/`.
...
2. **Thread path selection**
   ...
   - Write under `channels/42/threads/{thread_id}/`.
...
- `rules/root/CHANNEL_BASED_COORDINATION_DOCTRINE.md` (must replace COM001/COM002 from multi-agent doctrine)
- `rules/root/MULTI_AGENT_COORDINATION_DOCTRINE.md` (fix COM001 to channel artifact path not status)
- `docs/ACTOR_REGISTRATION_CHECKLIST.md` (make channel assignment and thread routing explicit), plus sample pages for channel artifacts.
- `docs/architecture/HOW_ACTORS_ORCHESTRATE_ON_CHANNELS.md` (add stable folder rules and `channels` narrative; mark status docs read-only archive)
- `docs/doctrine/IDE_AGENT_CONTINUITY_PROTOCOL.md` (add rule that when transition to channel path, status reports must be redirected content)
- `docs/status/CURSOR_CHANNEL_42_ROLE_AND_MEMBERSHIP_ALIGNMENT_4_0_79.md` update pointer to channel artifacts.
...
- Must break hard from `docs/status/` as primary output and use `channels/42/` as canonical runtime path.
- Require immediate doctrine patch + regression tests covering file destination rules.

---

### Final deliverable summary
1. **Canonical file path:** `channels/42/threads/1002/20260317_183000_lilith_channel-system-review.md`
2. Current model state: ⚠️ partial/inconsistent (structurally broken until you enforce rules)
3. Top 3 structural fixes:
   - unify artifact routing in doctrine to `channels/42/` and deprecate `docs/status/` for active workflows
   - formalize thread naming / path rules and enforce via code and loader
   - close mismatch between `MULTI_AGENT_COORDINATION_DOCTRINE` and implemented API that currently says `docs/status` instead of channel path
4. Doctrine files to update next:
   - `rules/root/CHANNEL_BASED_COORDINATION_DOCTRINE.md`
   - `rules/root/MULTI_AGENT_COORDINATION_DOCTRINE.md`
   - `docs/ACTOR_REGISTRATION_CHECKLIST.md`
