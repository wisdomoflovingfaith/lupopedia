...
lupopedia.headers:
  ...
  file_path_from_root: "channels/42/threads/1047/20260322_142402_lilith_structural_correction_phase_2_validation.md"
  web_path: "http://www.lupopedia.com/channels/42/threads/1047/20260322_142402_lilith_structural_correction_phase_2_validation.md"
  questions_toon: null
  channel_id: 42
  thread_id: 1047
...
lupopedia.edges:
  outbound_edges:
    - { to: "channels/42/threads/1047/THREAD_INDEX.md", type: "audited_surface", weight: 1.0, reason: "Thread index authority-removal validation target" }
    - { to: "channels/66/threads/1047/THREAD_INDEX.md", type: "audited_surface", weight: 1.0, reason: "Derived thread index navigation-only validation target" }
    - { to: "docs/versions/4.0.85/TASK_REGISTRY.md", type: "audited_surface", weight: 1.0, reason: "Semantic consistency, bounded inventory, edge validation, and decision-hook isolation checks" }
---
...
- Verified [channels/42/threads/1047/THREAD_INDEX.md](channels/42/threads/1047/THREAD_INDEX.md): structure is navigation-oriented, but directive-language tokens remain in artifact titles (for example, "Directive" and "Enforcement Status").
- Verified [channels/66/threads/1047/THREAD_INDEX.md](channels/66/threads/1047/THREAD_INDEX.md): structure is navigation-oriented, but directive-language tokens remain in artifact titles (for example, "lock_directive" and "synchronization_v10_directive").
- Because the gate requirement is strict "no directive language," this check fails.
- Result: FAIL

### 2) Channel 66 semantic consistency

- `channel66_question_threads: 11` present in [docs/versions/4.0.85/TASK_REGISTRY.md](docs/versions/4.0.85/TASK_REGISTRY.md#L34).
- `task_ch66_th1047` is classified as `directive` in [docs/versions/4.0.85/TASK_REGISTRY.md](docs/versions/4.0.85/TASK_REGISTRY.md#L149).
- Negative check: no `task_ch66_th1047` occurrences with question-node markers were found.
- Result: PASS

### 3) Backfill inventory validity

- Bounded list exists in [docs/versions/4.0.85/TASK_REGISTRY.md](docs/versions/4.0.85/TASK_REGISTRY.md#L168).
...
- Completion condition is explicit and testable in [docs/versions/4.0.85/TASK_REGISTRY.md](docs/versions/4.0.85/TASK_REGISTRY.md#L171).
- Result: PASS

### 4) Edge_ref validation reality

- Parsed all inline `edge_ref:<edge_id>` markers in [docs/versions/4.0.85/TASK_REGISTRY.md](docs/versions/4.0.85/TASK_REGISTRY.md).
- Validation gate rows exist for each referenced edge and are marked `valid` in [docs/versions/4.0.85/TASK_REGISTRY.md](docs/versions/4.0.85/TASK_REGISTRY.md#L175).
...
- Decision hook explicitly marked non-authoritative and non-binding in [docs/versions/4.0.85/TASK_REGISTRY.md](docs/versions/4.0.85/TASK_REGISTRY.md#L195).
