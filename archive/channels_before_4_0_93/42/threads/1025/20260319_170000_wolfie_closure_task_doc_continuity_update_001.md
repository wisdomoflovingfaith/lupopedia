...
lupopedia.headers:
  ...
  file_path_from_root: "channels/42/threads/1025/20260319_170000_wolfie_closure_task_doc_continuity_update_001.md"
  web_path: "http://www.lupopedia.com/channels/42/threads/1025/20260319_170000_wolfie_closure_task_doc_continuity_update_001.md"
  questions_toon: null
  channel_id: 42
  thread_id: 1025
...
lupopedia.edges:
  outbound_edges:
    - { to: "channels/42/threads/1025/20260318_175542_cursor_review_task_doc_continuity_update_001_channel-system-continuity-alignment.md", type: "addresses", weight: 1.0, reason: "Closure addresses CURSOR review and implementation" }
    - { to: "docs/doctrine/IDE_AGENT_CONTINUITY_PROTOCOL.md", type: "accepts", weight: 1.0, reason: "Accepts IACP updates for channel-based continuity" }
    - { to: "ONBOARDING.md", type: "accepts", weight: 1.0, reason: "Accepts onboarding alignment with channel system" }
    - { to: "docs/doctrine/CHANNEL_BASED_COORDINATION_DOCTRINE.md", type: "references", weight: 0.9, reason: "References authoritative channel coordination doctrine" }
lupopedia.footer:
  version: "4.0.81"
...
- **Primary Coordination**: `channels/{channel_id}/threads/{thread_id}/` and `channels/{channel_id}/tasks/`
- **Durable Handoff Layer**: Thread and task artifacts are the authoritative continuity sink
- **Legacy Status**: `docs/status/` is archival/legacy, not active continuity
...
**Changes Verified:**
- **Status Checkpoints**: Replaced `docs/status/` with "Channel Checkpoints (Thread/Task Artifacts)"
...
- `logs/`
- `channels/{channel_id}/` channel checkpoint artifacts (threads + tasks)
...
**Changes Verified:**
- **Line 38**: Updated to clarify that artifacts live under `channels/{channel_id}/`
- **Line 38**: Explicitly states `docs/status/` is archival/legacy only
- **Line 38**: Channel system is authoritative for continuity

**Current ONBOARDING Status (Line 38):**
```markdown
- **Artifacts** — Proof of coordination lives under **`channels/{channel_id}/`** (threads/tasks/broadcasts/direct) with artifact families defined in doctrine (e.g. `WOLFIE_DIRECTIVE_*`, `SESHAT_REVIEW_*`). `docs/status/` is archival/legacy only; channel system is authoritative for continuity.
```
...
**Scope:**
- **Target**: Remaining operational references to `docs/status/`
...
**System State:**
- **Canonical Continuity Location**: `channels/{channel_id}/threads/{thread_id}/` and `channels/{channel_id}/tasks/`
- **Legacy Status Location**: `docs/status/` is archival/legacy only
