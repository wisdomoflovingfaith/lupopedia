...
lupopedia.headers:
  ...
  file_path_from_root: "channels/42/threads/1019/20260318_163836_athena_spec_task_val_004_project-validation.md"
  web_path: "http://www.lupopedia.com/channels/42/threads/1019/20260318_163836_athena_spec_task_val_004_project-validation.md"
  questions_toon: null
  channel_id: 42
  thread_id: 1019
...
lupopedia.edges:
  outbound_edges:
    ...
    - { to: "scripts/validate_threads.py", type: "integrates_with", weight: 0.9, reason: "V-THREAD" }
lupopedia.footer:
  version: "4.0.81"
...
| `channels/42/threads/{id}/` | Filesystem thread roots; scan `*.md` headers for `task_id`, `thread_id`. |
...
**Statement:** Every registry row whose work is **execution-bound** MUST map to **exactly one** numeric `thread_id`, and the directory **`channels/42/threads/{thread_id}/`** MUST exist on disk.
...
- Exactly one directory MUST exist: `channels/42/threads/{thread_id}/` (non-empty or empty dir both OK; “exists” = path is a directory).
...
**Statement:** For every thread directory under `channels/42/threads/{N}/`:
...
**Legacy threads (1001, 1002, …):** MAY be listed in **allowlist** `scripts/vproject_legacy_threads.txt` (one thread_id per line) → downgrade 002-MIX to WARN for those IDs only.
...
- **Channel:** `42`.
- Any `thread_id` directory under `channels/42/threads/` is **in-project**; there is **no** second channel tree in scope for this validator pass.

**Orphan thread (ERROR or WARN per policy):**

- Directory `channels/42/threads/{N}/` exists.
...
for r in rows:
  if r.lifecycle in {active, blocked, resolved, archived}:
    ...
    assert isdir(f"channels/42/threads/{r.thread_id}/")

# 002
for N in list_thread_dirs("channels/42/threads"):
...
**Suggested CLI:** `python scripts/validate_vproject.py [--strict] [--allowlist path]`  
...
| File | Purpose |
|------|---------|
| `scripts/vproject_legacy_threads.txt` | Thread IDs allowed **mixed** `task_id` headers (historical). |
| `scripts/vproject_orphan_threads.txt` | Thread dirs with no registry row (infra only). |
