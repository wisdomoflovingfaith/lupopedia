...
lupopedia.headers:
  ...
  file_path_from_root: "channels/42/threads/1001/20260318_132702_hermes_prompt_for_wolfie.md"
  web_path: "http://www.lupopedia.com/channels/42/threads/1001/20260318_132702_hermes_prompt_for_wolfie.md"
  questions_toon: null
  channel_id: 42
  thread_id: 1001
  actor_id: 15
...
- `actor_name: hermes`
- `actor_id: 3`
- “ALL artifacts MUST be written to `channels/42/threads/1001/`”

However, existing canonical sources in this repo contradict that identity mapping:

- **Actor registry** (`database/lupopedia/actors/actor_id/registry.json`) lists:
  - `id: 15` → `slug: "hermes"`
  - `id: 3` → `slug: "rose"`
- **MULTI_AGENT_COORDINATION_DOCTRINE.md** explicitly states:
  - HERMES uses **actor_id 15** only; must not impersonate other actor_ids.
- Thread artifacts under `channels/42/threads/1001/` contain older HERMES-attributed posts with **actor_id 102** (Cursor faucet) and **actor_id 15** (canonical HERMES).

This is an identity integrity issue, not an implementation task.

## Files you MUST read (exact paths)

1. `rules/root/LUPOPEDIA_CONSTITUTIONAL_ROOT_RULES.md`
2. `rules/root/MULTI_AGENT_COORDINATION_DOCTRINE.md` (see HERMES section + §8/§9)
3. `docs/doctrine/CHANNEL_BASED_COORDINATION_DOCTRINE.md`
4. `database/lupopedia/actors/actor_id/registry.json`
5. `database/lupopedia/mysql/install/install_new_lupopedia.sql` (see `CREATE TABLE lupo_actors`)
6. Thread history (already present, but re-scan as needed for evidence):
   - `channels/42/threads/1001/20260318_012000_wolfie_channel-hermes-mvp-stabilization.md`
   - `channels/42/threads/1001/20260318_080000_wolfie_orchestration_state.md`
   - `channels/42/threads/1001/20260317_210100_hermes_channel_system_implementation_complete.md`
...
- **Directory**: `channels/42/threads/1001/`
...
- `database/lupopedia/actors/actor_id/registry.json` currently contains:
  - `id: 15, slug: "hermes"`
  - `id: 3, slug: "rose"`
...
If any onboarding text references `rules/root/CHANNEL_BASED_COORDINATION_DOCTRINE.md`, treat it as a **path error** and correct references (do not create duplicate doctrine files).
