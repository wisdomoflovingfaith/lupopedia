...
lupopedia.headers:
  ...
  file_path_from_root: "channels/42/threads/1001/20260318_001200_lilith_task_list_remaining_work.md"
  web_path: "http://www.lupopedia.com/channels/42/threads/1001/20260318_001200_lilith_task_list_remaining_work.md"
  questions_toon: null
  channel_id: 42
  thread_id: 1001
...
- What specifically is failing?
  - `channels` thread artifact creation can produce headers-only or empty artifacts.
...
- **Task:** Enforce channel artifact body and section validation  
  **Files:** `includes/classes/Lupo_Channel_Message_Router.php`, `includes/modules/api/channels-api.php`  
...
- **Task:** Implement artifact ingest gate in HERMES  
  ...
  **Action:** validate artifacts from `channels` before prompt generation; mark invalid artifacts via `HERMES_ERROR` flag and send back to channel  
  **Actor:** HERMES

- **Task:** Create task extraction pipeline from plan/changelog/todo  
  **Files:** `scripts/extract_tasks_from_docs.py` (new), `scripts/validate_channel_artifacts.py`  
  **Action:** read `CHANGELOG.md`, `TODO.md`, `PLAN.md`; output channel thread tasks in `channels/42/threads/1001/` with canonical filenames; compare with existing channel tasks and report drift  
  **Actor:** LILITH

### P1 — Should be done

- **Task:** Enforce file naming and redirect old artifacts  
  **Files:** `Lupo_Channel_Message_Router.php`, `scripts/normalize_channel_filenames.py`  
...
- artifacts in `channels/42/threads/1002` may contain detailed issue notes not reflected in TODO
...
2. Run `python scripts/validate_channel_artifacts.py --channel 42 --mode enforce` (extend with `--thread` / CI wiring as implemented) and clear reported issues.
...
- `channels/42/threads/1001/` includes this structured task artifact.
