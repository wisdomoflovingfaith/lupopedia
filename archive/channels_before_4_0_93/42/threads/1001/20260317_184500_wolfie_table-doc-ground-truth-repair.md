...
lupopedia.headers:
  ...
  file_path_from_root: "channels/42/threads/1001/20260317_184500_wolfie_table-doc-ground-truth-repair.md"
  web_path: "http://www.lupopedia.com/channels/42/threads/1001/20260317_184500_wolfie_table-doc-ground-truth-repair.md"
  questions_toon: null
  channel_id: 42
  thread_id: 1001
...
# file: WOLFIE table-doc ground-truth repair — thread 1001 — web_path: channels/42/threads/1001/20260317_184500_wolfie_table-doc-ground-truth-repair
...
**CRITICAL ROUTING FAILURE**: This artifact was initially written to `docs/status/WOLFIE_TABLE_DOC_GROUND_TRUTH_REPAIR_4_0_80.md` incorrectly, violating channel-based coordination expectations for channel-scoped work.

**Correction Applied**:
- Canonical artifact under numeric thread: `channels/42/threads/1001/20260317_184500_wolfie_table-doc-ground-truth-repair.md`
- Previous location at `docs/status/` will be replaced with redirect stub
...
- **Cross-ref**: `channels/42/threads/1002/20260318_003000_hermes_actor-identity-violation.md` (actor-identity rule).
...
### Critical Violations:
- **Install / TOON oracle not used**: Table docs were not grounded from **`install_new_lupopedia.sql`** (canonical DDL) and, where generated, matching **`docs/toons/<table>.toon.json`** (per **toon-source-of-truth**)
- **Root Rules Ignored**: Rules in `rules/root/` were not followed before documentation changes
...
### What Went Wrong:
1. **Rules Not Followed**: `rules/root/` contains explicit requirements for schema grounding that were ignored
2. **Schema oracle ignored**: **`install_new_lupopedia.sql`** (per-table `CREATE TABLE`) was not used as the primary extract; generated **`docs/toons/<table>.toon.json`** was not used for parity checks
3. **Guessing Occurred**: Without full DDL+TOON pass, column details were invented or approximated

### Specific Rule Violations:
- **TOON Source of Truth Rule**: `rules/root/toon-source-of-truth.md` requires TOON files as schema ground truth
- **Constitutional Root Rules**: `rules/root/LUPOPEDIA_CONSTITUTIONAL_ROOT_RULES.md` requires deterministic, grounded documentation
...
### Repair Rule:
Every affected table documentation will be re-grounded from **`database/lupopedia/mysql/install/install_new_lupopedia.sql`** (authoritative DDL) and cross-checked against **TOON JSON** when present under `docs/toons/` (regenerate via `scripts/generate_toon_from_sql.py` if missing). Any guessed column list will be replaced with oracle-derived schema.
...
5. **Rule Compliance**: Final check against `rules/root/`
...
- **Rule**: `rules/root/toon-source-of-truth.md` — TOON JSON is generated **from** install SQL; both must agree
...
- **Rule**: `rules/root/LUPOPEDIA_CONSTITUTIONAL_ROOT_RULES.md`
...
- **Rule**: `docs/doctrine/CHANNEL_BASED_COORDINATION_DOCTRINE.md` (v4.0.80)
...
1. **Install-first oracle**: Table documentation MUST match **`install_new_lupopedia.sql`** DDL for that table; TOON JSON under `docs/toons/` must agree (regenerate TOONs from install when needed)
2. **Root Rules Must Be Read**: `rules/root/` MUST be read before table-doc changes
...
1. **Channel thread path**: Numeric **`dialog_thread_id`** only — `channels/42/threads/{thread_id}/` (see **CHANNEL_ARTIFACT_ROUTING_DOCTRINE**); do **not** use version strings as thread folders for active work
2. **No Status-Based Coordination**: `docs/status/` is obsolete for active coordination
...
- [ ] Read applicable rules in `rules/root/`
