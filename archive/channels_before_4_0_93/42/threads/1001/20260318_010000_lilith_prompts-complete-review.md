...
lupopedia.headers:
  ...
  file_path_from_root: "channels/42/threads/1001/20260318_010000_lilith_prompts-complete-review.md"
  web_path: "http://www.lupopedia.com/channels/42/threads/1001/20260318_010000_lilith_prompts-complete-review.md"
  questions_toon: null
  channel_id: 42
  thread_id: 1001
  actor_id: 2
...
- Current README (`channels/42/prompts/README.md`) exists but lacks enforcement details for `channels/42/prompts/` automation.
...
### Outcome
- TODO: implement `scripts/validate_prompt_files.py` to enforce this.
...
- For R3, the sequence is:
  - Parse all files in `channels/42/prompts/` for API vs review directives.
  - Generate task group `ExternalAI-Batch-R3` in `TODO.md` and `channels/42/threads/1001/`.
...
### Action
- Create `channels/42/threads/1001/20260318_010100_lilith_top50_a12_qa-checklist.md` (new artifact) with explicit QA criteria.
...
1. Add prompt metadata enforcement in README + validation script (`scripts/validate_prompt_files.py`).
2. Extend `validate_channel_artifacts.py` with `--prompt-metadata` flag.
3. Create `channels/42/threads/1001/20260318_010100_lilith_top50_a12_qa-checklist.md` for direct QA acceptance criteria.
4. Update `TODO.md` and `docs/versions/4.0.80/TODO.md` with these tasks and status references.
...
- HERMES should mark source prompts done in `channels/42/prompts/` and in `TODO.md` table.

## 8. Final direct instruction

- **HERMES**, take this artifact and create `channels/42/prompts/20260318_010000_hermes_action_lilith_prompt-executed.md` with link back to this file.
