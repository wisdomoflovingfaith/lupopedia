...
lupopedia.headers:
  ...
  file_path_from_root: "channels/42/threads/1001/20260318_090000_lilith_critique_system.md"
  web_path: "http://www.lupopedia.com/channels/42/threads/1001/20260318_090000_lilith_critique_system.md"
  questions_toon: null
...
1.1 install_new_lupopedia.sql states `actor_name PK`, `actor_id` unique index, `is_agent` etc, but `lupo_actors` doctrine is not enforced by any centralized validation script in `bin` (no `validate_actors.php` covering all required columns). Evidence: `database/lupopedia/mysql/install/install_new_lupopedia.sql` + absence in `bin` list.

1.2 Channel-based workflow claim: "database is source of truth" in `CHANNEL_BASED_COORDINATION_DOCTRINE.md`, but `channels/42/threads/1001/` artifacts exist without guaranteed sync metadata (headers in many files focus on `actor_name` etc, not `dialog_thread_id` or `dialog_message_id` necessarily). Evidence: message files in thread 1001 (e.g. `20260318_010000_lilith_prompts-complete-review.md`), where headers do not include DB IDs.

1.3 No explicit rule ensures `lupo_agent_faucets` `is_default` one-per-actor; there is index but not enforced. Assumption of uniqueness via code may fail when multiple default rows slip in. Evidence: table doc `agent-faucets.md` and SQL section with index not unique.
...
3.1 `rules/root` asserts no DB side logic, but `lupo_bin` scripts (e.g., `bin/faucet_integrity_audit.php`) contain one-off fixes that look like hidden migration logic. Risk: same side effects may lag doctrine if not gated.

3.2 In `agent-*` path, `actor_id` values are set in files; but `database/lupopedia/actors/actor_id/registry.json` is canonical. Some ARs in docs still list hardcoded numbers (e.g., AGENTS.md table for Faucet actors). Contradiction source-of-truth vs human references.
...
4.1 Actor registration checklist is central, but `actors/59/rules` etc exist; call path from agent inbound is not explicit. Without a required code path in bootstrap (e.g., blocking write before registry update), agents may bypass. Evidence: `docs/ACTOR_REGISTRATION_CHECKLIST.md` plus absence in `includes/bootstrap.php` quick scan (not yet checked but implied by coverage gap).
...
4.3 Workflow says channel 42 is primary, but file-level writing still can occur in any `channels/<n>`. There is no mechanism preventing agents from using wrong channel id except human agreement.
...
5.2 Schema migration by hand in TOON path risk: the policies say TOON is generated, but there are active manual edits in `docs/database/lupopedia/tables/active/` and this can drift from actual SQL. Without automated comparators, drift from 4.0.81 to 4.1.0 and DB checks could fail.
...
- Priority gap: do not trustdoc without explicit sync/validation tests; push an immediate `scripts/validate_channel_files_against_db.php` guard if not present.
