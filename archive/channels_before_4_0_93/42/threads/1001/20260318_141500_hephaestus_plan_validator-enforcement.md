...
lupopedia.headers:
  ...
  file_path_from_root: "channels/42/threads/1001/20260318_141500_hephaestus_plan_validator-enforcement.md"
  web_path: "http://www.lupopedia.com/channels/42/threads/1001/20260318_141500_hephaestus_plan_validator-enforcement.md"
  questions_toon: null
  channel_id: 42
  thread_id: 1001
  actor_id: 14
...
lupopedia.edges:
  outbound_edges:
    - { to: "channels/42/threads/1001/20260318_110000_wolfie_directive_thread001-triage.md", type: "implements", weight: 1.0, reason: "Binding task_id vs thread_id separation + P0 enforcement mechanism" }
    - { to: "channels/42/threads/1001/20260318_135527_athena_strategy_thread-lifecycle.md", type: "implements", weight: 1.0, reason: "Baseline §8 validator-relevant rules" }
    - { to: "channels/42/threads/1001/20260318_095000_lilith_review_thread-task-canonicalization.md", type: "addresses", weight: 0.9, reason: "P0 lifecycle/split/merge/validator hook gaps" }
    - { to: "channels/42/threads/1001/20260318_134258_hermes_prompt_for_wolfie_thread001-triage-and-routing.md", type: "addresses", weight: 0.9, reason: "Validator mapping + legacy constraints" }
    - { to: "rules/root/CHANNEL_ARTIFACT_ROUTING_DOCTRINE.md", type: "references", weight: 0.8, reason: "Numeric thread dirs + canonical filename rule" }
    - { to: "docs/doctrine/CHANNEL_BASED_COORDINATION_DOCTRINE.md", type: "references", weight: 0.7 }
    - { to: "rules/root/MULTI_AGENT_COORDINATION_DOCTRINE.md", type: "references", weight: 1.0, reason: "ATER001 + coordination constraints" }
    - { to: "rules/root/LUPOPEDIA_CONSTITUTIONAL_ROOT_RULES.md", type: "references", weight: 1.0, reason: "No hidden transitions, no rewriting history, deterministic enforcement" }
    - { to: "scripts/validate_channel_artifacts.py", type: "references", weight: 0.9, reason: "Filesystem/CI validator entrypoint" }
    - { to: "includes/classes/ChannelArtifactValidator.php", type: "references", weight: 0.9, reason: "Path-based PHP validator entrypoint" }
    - { to: "includes/classes/Lupo_Channel_Artifact_Validator.php", type: "references", weight: 0.9, reason: "API/router body gates + canonical filename/thread id validators" }
lupopedia.footer:
  version: "4.0.81"
...
- **CI/filesystem**: `scripts/validate_channel_artifacts.py`
- **PHP path validation**: `includes/classes/ChannelArtifactValidator.php` (calls `Lupo_Channel_Artifact_Validator::validateThreadPostBody`)
- **API/router body gate**: `includes/classes/Lupo_Channel_Artifact_Validator.php` used by `channels-api.php` and `Lupo_Channel_Message_Router.php`
...
- **Merge marker block (ATHENA V-4)**: must include:
  ...
  - `merge_artifact: <path>` (repeatable; must look like `channels/.../*.md`)
- **WOLFIE waiver marker** (for legacy / exceptional postings):
  - `wolfie_waiver_artifact: channels/42/threads/1001/YYYYMMDD_HHIISS_wolfie_directive_....md`
...
- **Merge required fields**:
  ...
  - `merge_artifact` paths (if present) must look like `channels/<digits>/threads/<digits>/...md`
...
- **`includes/classes/ChannelArtifactValidator.php`**
...
- **`includes/classes/Lupo_Channel_Artifact_Validator.php`**
...
- **`tests/unit/channel_thread_review_body_test.php`**
...
| Numeric thread dir under `threads/` | **ERROR (block)** | structural integrity | `NON_NUMERIC_THREAD_DIR: channels/42/threads/<name>` |
...
- **Check** (offline-safe):
  - waiver path must match `channels/<digits>/threads/<digits>/[0-9]{8}_[0-9]{6}_wolfie_*.md`
...
- THREAD001 marker checks (planned, warnings initially):
  - `python scripts/validate_channel_artifacts.py ... --enforce-thread001-markers`

### 6.2 Repo validation (developer workflow)

- `sh scripts/run_unit_tests.sh .` already runs `validate_channel_artifacts.py --mode enforce` before PHP unit tests.
