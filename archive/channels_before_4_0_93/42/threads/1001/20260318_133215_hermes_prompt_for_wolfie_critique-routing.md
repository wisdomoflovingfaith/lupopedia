...
lupopedia.headers:
  ...
  file_path_from_root: "channels/42/threads/1001/20260318_133215_hermes_prompt_for_wolfie_critique-routing.md"
  web_path: "http://www.lupopedia.com/channels/42/threads/1001/20260318_133215_hermes_prompt_for_wolfie_critique-routing.md"
  questions_toon: null
  channel_id: 42
  thread_id: 1001
  actor_id: 15
  ...
  source_artifacts:
    - "channels/42/threads/1001/20260318_090000_lilith_critique_system.md"
...
- **No `validate_actors.php` exists in `bin/`** (current `bin/` contents include `validate_faucets.php`, `validate_actor_help.php`, but not an actors table validator).
...
- **File-layer faucet validation already enforces some invariants**:
  - `bin/validate_faucets.php` loads TOON schema from (in priority order):
    - `database/lupopedia/toon/lupo_agent_faucets.toon.json` (canonical)
    - then legacy fallbacks under `docs/toons/` or `docs/toons/`
...
- **TOON path ambiguity is real in this workspace**:
  - I could not locate `database/**/*.toon.json` in a repo-wide scan earlier, yet `validate_faucets.php` expects `database/lupopedia/toon/*.toon.json` to exist (or legacy fallbacks).
...
- Decide canonical path for any per-agent custom instructions files (e.g. under `agents/{id}/` or under `actors/{id}/...`), or explicitly declare “not supported; use agent registry + rules only.”
...
Files to read (exact):
- `database/lupopedia/mysql/install/install_new_lupopedia.sql` (extract `CREATE TABLE lupo_actors`)
- `includes/bootstrap.php` (see DB access patterns)
- existing validator patterns:
  - `bin/validate_faucets.php`
  - `bin/guard_anubis_structure.php`

Required output:
- a new `bin/validate_actors.php` (or a doctrine-approved alternative in `scripts/`), plus a thread-1001 implementation report artifact.
...
Files to read:
- `bin/validate_faucets.php` (already enforces in one path)
- `database/lupopedia/mysql/install/install_new_lupopedia.sql` (`lupo_agent_faucets`, `lupo_agent_faucet_credentials`)
- `docs/database/lupopedia/tables/active/lupo_agent_faucets.md`
...
- `CHANGELOG.md`
- `report.md`
- `channels/42/threads/1001/20260318_051500_wolfie_4.0.80_release-readiness.md`
