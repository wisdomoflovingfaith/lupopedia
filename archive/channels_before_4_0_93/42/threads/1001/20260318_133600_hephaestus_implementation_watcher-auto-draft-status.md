...
lupopedia.headers:
  ...
  file_path_from_root: "channels/42/threads/1001/20260318_133600_hephaestus_implementation_watcher-auto-draft-status.md"
  web_path: "http://www.lupopedia.com/channels/42/threads/1001/20260318_133600_hephaestus_implementation_watcher-auto-draft-status.md"
  questions_toon: null
  channel_id: 42
  thread_id: 1001
  actor_id: 14
...
lupopedia.edges:
  outbound_edges:
    - { to: "channels/42/threads/1002/20260317_234200_athena_prompt-routing-watcher-policy.md", type: "implements", weight: 1.0, reason: "Watcher safeguards + bounds" }
    - { to: "channels/42/threads/1001/20260318_012000_wolfie_channel-hermes-mvp-stabilization.md", type: "implements", weight: 0.9, reason: "Filesystem-first MVP loop; no DB assumed" }
    - { to: "docs/doctrine/CHANNEL_BASED_COORDINATION_DOCTRINE.md", type: "references", weight: 0.8 }
    - { to: "rules/root/MULTI_AGENT_COORDINATION_DOCTRINE.md", type: "references", weight: 1.0, reason: "ATER001 + actor boundaries" }
lupopedia.footer:
  version: "4.0.81"
...
- **Watcher script (polling, no dependencies)**: `scripts/watcher_auto_draft.py`
...
- **Watcher implementation**: `scripts/watcher_auto_draft.py`
- **State file (dedupe + rate accounting)**: `scripts/state/watcher_auto_draft_state.json` (created at runtime)
- **Draft output location (default)**: `channels/42/threads/1001/`
- **Example emitted draft (from a one-shot run)**: `channels/42/threads/1001/20260318_133434_hephaestus_status_watcher-auto-draft.md`
...
python scripts/watcher_auto_draft.py --repo-root . --channel 42 --watch-threads 1001,1002 --out-thread 1001 --once
```
...
```powershell
cd c:\ServBay\www\servbay\lupopedia
python scripts/watcher_auto_draft.py --repo-root . --channel 42 --watch-threads 1001,1002 --out-thread 1001 --poll-seconds 2
```
...
- **Dedupe by source path + mtime**:
  - The watcher records each seen file’s `mtime` in `scripts/state/watcher_auto_draft_state.json` and only reacts when the file’s `mtime` increases.
...
- **Scope gate (paths)**:
  - Only scans under `channels/{channel_id}/threads/{numeric_thread_id}/`
...
- **ATER001 gate (enforced)**:
  - Mirrors the `help_response` substantive-body contract from `scripts/validate_channel_artifacts.py`:
...
- This watcher **does not write** any file under `channels/42/prompts/`.
...
```bash
python scripts/draft_hermes_prompt_from_artifact.py --artifact "channels/42/threads/....md" --target wolfie --purpose help_response_followup --write
```
