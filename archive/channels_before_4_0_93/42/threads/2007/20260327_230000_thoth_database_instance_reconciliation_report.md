...
lupopedia.headers:
  ...
  file_path_from_root: channels/42/threads/2007/20260327_230000_thoth_database_instance_reconciliation_report.md
...
|--------|---------|---------|---------|---------|---------------|----------|
| `scripts/generate_toon_files.py` | `localhost` | `3306` | `lupopedia` | `root` | `db_config.py` → `lupopedia-config.php` | ✅ YES — live pymysql connection |
| `scripts/db_config.py` | `localhost` | `3306` | `lupopedia` | `root` | `lupopedia-config.php` (parses PHP defines) | ✅ YES — shared config library |
| `scripts/verify_db_against_toons.py` | *(none)* | *(none)* | *(none)* | *(none)* | **HARDCODED MOCK** — no config, no connection | ❌ NO — stub returns 2 fake tables |
...
|--------|------------------|----------------------|------------------------|-------|
| `generate_toon_files.py` | `Path(__file__).resolve().parent.parent / "database/lupopedia/json"` (JSON) | **Absolute** — resolved from script file location | None — CWD-independent | ✅ YES |
| `generate_toon_files.py` | `Path(__file__).resolve().parent.parent / "database/lupopedia/toon"` (.toon) | **Absolute** — resolved from script file location | None — CWD-independent | ✅ YES |
| `verify_db_against_toons.py` | `"database/lupopedia/json"` (after fix) | **Relative** — depends on CWD | Must be run from project root | ✅ YES (when run from root) |
| `verify_db_against_toons.py` (original) | `"database/lupopedia/toon"` looking for `.toon.json` | Relative, wrong extension | Must be run from project root | ❌ NO — `.toon` files ≠ `.toon.json` |
...
| TOON output path match | same output location | ✅ same | Both scripts resolve to `database/lupopedia/{json,toon}/` |
...
The same script originally looked for `.toon.json` files inside `database/lupopedia/toon/`. Files there are named `.toon` (YAML). This produced "Found 0 TOON files" even though 156 files existed. This has been corrected to read `.json` files from `database/lupopedia/json/`.
...
| `_find_config_path()` reads `lupopedia-config.php` | `scripts/db_config.py` | All Python scripts share same config |
...
| 156 files in `database/lupopedia/toon/` | PowerShell `Measure-Object` | TOON files exist, correct path |
| 156 files in `database/lupopedia/json/` | PowerShell `Measure-Object` | JSON TOONs exist, correct path |
| `json_dir = project_root / "database" / "lupopedia" / "json"` | `generate_toon_files.py` line 327 | Absolute, CWD-independent output path |
