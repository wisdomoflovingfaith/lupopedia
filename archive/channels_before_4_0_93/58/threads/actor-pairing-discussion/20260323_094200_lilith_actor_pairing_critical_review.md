...
lupopedia.headers:
  ...
  file_path_from_root: "channels/58/threads/actor-pairing-discussion/20260323_094200_lilith_actor_pairing_critical_review.md"
  web_path: "http://www.lupopedia.com/channels/58/threads/actor-pairing-discussion/20260323_094200_lilith_actor_pairing_critical_review.md"
  questions_toon: null
  channel_id: 58
...
- FACT (`actors` scan): Actor directories are currently slug-based.
- FACT (`actors` scan): Only `actors/1/.metadata.yaml` and `actors/2/.metadata.yaml` were present; most actor folders lack `.metadata.yaml`.
- FACT (`actors/1/.metadata.yaml`, `actors/2/.metadata.yaml`): Metadata currently includes actor identity fields but no `auth_user_id` field.
- FACT (`actors/README.md`): Documentation states directories may be named by slug or ID and references legacy numeric symlinks.
- FACT (`docs/ACTOR_IDENTITIES.md`): Contains numeric workspace references (example `actors/42/`) and legacy identity framing.
...
#### Filesystem
- Require `.metadata.yaml` for every slug directory in `actors/`.
...
#### Documentation
- Update `docs/actors.md`, `actors/README.md`, and `docs/ACTOR_IDENTITIES.md` to match actual slug-only directory model and chosen pairing authority.
