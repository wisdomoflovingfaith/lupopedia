...
lupopedia.headers:
  ...
  file_path_from_root: "channels/0/threads/VERSION_4.0.61/tldr.md"
...
- **HelpRenderer class:** `includes/classes/HelpRenderer.php`
...
- **whoami, session, actor_name:** `php bin/lupo.php whoami` → **actor_name** (e.g. `wolfie`), **session mode** (typically `autonomous_agent` or `system`), **session_id**, **channel_id**, **context_source** — who this session is, what actor_name, current session details
- **Directory:** `/actors/1/` (name-based)
...
- **Session-first:** `database/session.md` read before database
