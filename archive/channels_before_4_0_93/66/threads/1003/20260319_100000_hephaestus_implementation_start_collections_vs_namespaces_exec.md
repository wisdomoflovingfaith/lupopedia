...
lupopedia.headers:
  ...
  file_path_from_root: "channels/66/threads/1003/20260319_100000_hephaestus_implementation_start_collections_vs_namespaces_exec.md"
  web_path: "http://www.lupopedia.com/channels/66/threads/1003/20260319_340000_hephaestus_implementation_start_collections_vs_namespaces_exec"
...
lupopedia.edges:
  comment: "static"
  outbound_edges:
    - { to: "channels/66/threads/1003/20260319_090000_hephaestus_post_doctrine_implementation_plan_collections_vs_namespaces.md", type: "implements", weight: 1.0, reason: "Implements the post-doctrine behavior plan for Thread 1003." }
    - { to: "channels/66/threads/1003/20260319_080000_wolfie_doctrine_execution_collections_namespaces.md", type: "inherits_from", weight: 0.98, reason: "Uses doctrine execution as the locked model." }
    - { to: "channels/66/threads/1003/20260319_070000_wolfie_doctrine_update_plan_collections_namespaces.md", type: "derived_from", weight: 0.95, reason: "Execution plan establishes what must be true before implementation." }
    - { to: "channels/66/threads/1003/20260319_060000_hephaestus_implementation_implications_collections_vs_namespaces.md", type: "references", weight: 0.92, reason: "Implications constrain validator severity, ingestion split, and normalization safety." }
    - { to: "channels/66/threads/1003/20260319_050000_wolfie_narrowing_collections_namespaces_decision_ready.md", type: "constrains", weight: 0.9, reason: "Precedence by scope and bounded coupling are the operational baseline." }
    - { to: "docs/doctrine/COLLECTIONS_DOCTRINE.md", type: "constrains", weight: 1.0, reason: "Collections = nav/tabs/URLs/breadcrumbs; not filesystem authority." }
    - { to: "docs/doctrine/LUPOPEDIA_HEADERS/README.md", type: "references", weight: 0.92, reason: "Headers = artifact metadata; collections/namespace relationship and precedence." }
    - { to: "docs/doctrine/LUPOPEDIA_HEADERS/LUPOPEDIA_HEADERS_FORMAT.md", type: "constrains", weight: 1.0, reason: "Namespace is single-value taxonomy field; table-doc requirements; block order rules." }
    - { to: "rules/root/FILE_BOUNDARY_VALIDATION_RULE.md", type: "references", weight: 0.8, reason: "Validator must protect file boundary and disallow path derivation." }
    ...
    - { to: "channels/66/threads/1003", type: "related_question", weight: 1.0, reason: "Current question context for collections vs namespaces implementation." }
    - { to: "channels/66/threads/1001", type: "related_question", weight: 0.95, reason: "Ingestion must inherit split-field semantics (collections membership vs namespace policy) from Thread 1001." }
    - { to: "channels/66/threads/1002", type: "related_question", weight: 0.95, reason: "Validation and bounded-authority behavior constrain error/warn thresholds and safety handling." }
lupopedia.footer:
  version: "4.0.80"
...
# file: HEPHAESTUS Implementation Start â€” Collections vs Namespaces (Thread 1003) â€” session: L-LUPO-ROOT-HEPHAESTUS â€” delegation: hephaestus:root â€” web_path: http://www.lupopedia.com/channels/66/threads/1003/20260319_340000_hephaestus_implementation_start_collections_vs_namespaces_exec
...
1. Update `scripts/validate_lupopedia_headers.php`
...
2. Create `includes/classes/CanonicalCollectionSlugResolver.php`
   - Methods:
...
3. Create `includes/classes/CollectionsNamespacesHeaderValidator.php`
   - Methods:
...
4. Create offline snapshot file:
   - `cache/offline/collections_slug_snapshot_federation_1.json`
   - Contents:
...
5. Create snapshot builder script (dry-run only; used for offline cache creation):
   - `scripts/build_collections_slug_snapshot.php`
...
6. Create `scripts/normalize_collections_namespaces_dry_run.php`
   - Reads header files, runs validator logic, and aggregates mismatch report categories.
   - Outputs:
     - `logs/normalization/YYYYMMDDHHIISS_collections_namespaces_dry_run_report.json`
...
7. Update (or create, if Thread 1001 code was not implemented yet) these ingestion/projection modules to apply split-field semantics:
   ...
   - `includes/classes/HeaderFieldPreservationMatrix.php`
...
8. Optional (only if required for discoverability by your test harness):
   - Update `bin/lupo.php` to add a subcommand:
     - `php bin/lupo.php collections namespace-collections dry-run <path>`
...
9. Create fixture directory:
   - `tests/fixtures/collections_namespaces/`
...
10. Create DB-stub fixture for unit tests (no real DB needed):
   - `tests/fixtures/collections_namespaces/db_stub_active_slugs.json`
   - Shape:
     - `federation_node_id`: 1
     - `slugs`: [ ... ]
...
10. Create unit test script:
   - `tests/unit/collections_namespaces_validator_test.php`
...
11. Create projection unit test script:
   - `tests/unit/channel66_collections_namespaces_projection_test.php`
...
Key definitions:
- Table-doc scope: any artifact path under `docs/database/lupopedia/tables/` (same detection approach as current `validate_lupopedia_headers.php`).
...
- `--root-path=<path>` (required)
  - Defaults (if omitted by tool owner): `channels/66/`
- `--scope=<table_docs|all|channel_only>` (optional)
  - For first pass default: `channel_only` (Channel 66 files only), but the scanner must still correctly enforce table-doc rules when it encounters `docs/database/lupopedia/tables/`.
...
- JSON file written to:
  - `logs/normalization/<timestamp>_collections_namespaces_dry_run_report.json`
- Must include:
...
1. `php tests/unit/collections_namespaces_validator_test.php`
2. `php tests/unit/channel66_collections_namespaces_projection_test.php`
...
2. Canonical slug resolver correctness
   - When DB is unavailable, validator behavior is deterministic using `cache/offline/collections_slug_snapshot_federation_1.json`.
