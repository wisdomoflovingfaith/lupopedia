...
lupopedia.headers:
  ...
  file_path_from_root: channels/66/threads/1001/20260319_030000_hephaestus_implementation_plan_revised_p0_bounded_authority_ingestion.md
  web_path: http://www.lupopedia.com/channels/66/threads/1001/20260319_030000_hephaestus_implementation_plan_revised_p0_bounded_authority_ingestion.md
...
# file: HEPHAESTUS Implementation Plan — Revised P0 Bounded Authority Ingestion — session: L-LUPO-ROOT-HEPHAESTUS — delegation: hephaestus:root — web_path: http://www.lupopedia.com/channels/66/threads/1001/20260319_030000_hephaestus_implementation_plan_revised_p0_bounded_authority_ingestion
...
1. **File discovery / parsing**
   - Deterministic enumeration of `channels/66/**/*.md`
...
2. **Structural validation**
   - Reuse existing validator behavior where safe (`php lupo.php headers validate` / `scripts/validate_lupopedia_headers.php`) for baseline checks
...
4. **TOON/schema comparison**
   - Load TOON schema from in-repo TOON set (`database/lupopedia/toon/*.toon.json`) as structural truth
...
1. **Existing baseline validator**
   - `scripts/validate_lupopedia_headers.php` for structural checks (reuse/extend)
   - `bin/lupo.php` for CLI dispatch location (add a new subcommand for ingestion runner)

2. **New bounded-authority ingestion runner (new)**
   - `scripts/ingest_channel66_headers_bounded_authority.php` (CLI entry) or a `bin` subcommand that calls into it

3. **New ingestion core (new)**
   - `includes/classes/Channel66HeaderIngester.php` (or equivalent)
   - `includes/classes/BoundedHeaderAuthorityValidator.php`
   - `includes/classes/ToonSchemaCache.php`
...
4. **TOON loading helper (new or shared)**
   - A helper to load `database/lupopedia/toon/<table>.toon.json` and extract:
...
5. **Logging / audit output (new)**
   - A small logger that prints run summary to stdout and writes a deterministic log file (or channel-thread-local log under `tools/`), including per-file outcome (`ok/reject/conflict/warn`).

6. **Test fixtures**
   - `tests/fixtures/channel66_ingestion/` (new fixture directory)
   - PHP test runners under `tests/unit/` or a targeted runner script (project uses “plain php scripts executed directly”)
...
- **TOON presence assurance**: confirm in-repo TOON JSON set exists at the expected path (`database/lupopedia/toon/*.toon.json`) for schema checks during ingestion runs.
