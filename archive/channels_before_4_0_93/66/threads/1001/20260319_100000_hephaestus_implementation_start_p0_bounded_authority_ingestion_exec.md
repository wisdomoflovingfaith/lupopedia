...
lupopedia.headers:
  ...
  file_path_from_root: channels/66/threads/1001/20260319_100000_hephaestus_implementation_start_p0_bounded_authority_ingestion_exec.md
  web_path: http://www.lupopedia.com/channels/66/threads/1001/20260319_100000_hephaestus_implementation_start_p0_bounded_authority_ingestion_exec.md
...
# file: HEPHAESTUS Implementation Start — P0 Bounded-Authority Ingestion (First Pass) — session: L-LUPO-ROOT-HEPHAESTUS — delegation: hephaestus:root — web_path: http://www.lupopedia.com/channels/66/threads/1001/20260319_340000_hephaestus_implementation_start_p0_bounded_authority_ingestion_exec
...
3. Structural validation (P0 gate)
   - Reuse baseline checks by calling `php bin/lupo.php headers validate <path>` or by directly calling `validate_lupopedia_headers($path)` when available.
   - Then apply P0-specific required fields enforcement (per `docs/doctrine/LUPOPEDIA_HEADERS/LUPOPEDIA_HEADERS_FORMAT.md` minimum required identity fields):
...
5. TOON/schema validation (structural truth enforcement)
   - Load TOON schemas from canonical directory `database/lupopedia/toon/`.
...
9. Logging / outcome reporting (required)
   ...
   - Logging destination for first pass:
     - Append JSONL to `logs/admin/` (existing directory).
...
1. `scripts/ingest_channel66_headers_bounded_authority.php`
   ...
   - Flags (first pass):
     ...
     - `--toon-dir=<path>` (test hook; default `database/lupopedia/toon/`)
...
Tests:
 - `tests/unit/channel66_bounded_authority_ingestion_p0_test.php` (new)
...
Test harness assumptions:
 ...
 - The ingestion runner supports `--scope-root` to point at a temporary directory containing fixture files under `channels/66/`.
...
7. Missing edge target continues
   - `missing_edge_target_thread1001.md` with `lupopedia.edges.outbound_edges` pointing `to: "channels/66/threads/1001/DOES_NOT_EXIST.md"`.
...
7. Logs:
   - append-only JSONL records are written to `logs/admin/` per run
