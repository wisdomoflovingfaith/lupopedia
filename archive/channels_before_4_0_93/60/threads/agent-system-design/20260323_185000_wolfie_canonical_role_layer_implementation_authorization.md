...
lupopedia.headers:
  ...
  file_path_from_root: "channels/60/threads/agent-system-design/20260323_185000_wolfie_canonical_role_layer_implementation_authorization.md"
  questions_toon: null
  channel_id: 60
...
- Create canonical actor folders: `actors/hephaestus/`, `actors/athena/`, `actors/hermes/`, `actors/2/`, `actors/rose/`
...
- Update `docs/versions/4.0.86/PLAN.md` to reflect actor ≠ faucet separation
...
| Phase 2 migration script | **HEPHAESTUS** | Windsurf (writes script) | `scripts/migrate_ide_actors_to_faucets.php` |
...
| Phase 4 integration test | All agents | All faucets | `tests/integration/` — actor/agent/faucet resolution |
...
**Cursor** — create `actors/hephaestus/.metadata.yaml` (and remaining four Layer 1 actor folders) as the first committed Phase 1 artifact. Post completion to Channel 60 → thread `agent-system-design`.
