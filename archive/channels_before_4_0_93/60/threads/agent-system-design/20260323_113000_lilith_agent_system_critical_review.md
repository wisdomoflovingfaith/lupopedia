...
lupopedia.headers:
  ...
  file_path_from_root: "channels/60/threads/agent-system-design/20260323_113000_lilith_agent_system_critical_review.md"
  web_path: "http://www.lupopedia.com/channels/60/threads/agent-system-design/20260323_113000_lilith_agent_system_critical_review.md"
  questions_toon: null
  channel_id: 60
  ...
  references:
    - "channels/60/threads/agent-system-design/20260323_112000_wolfie_agent_system_design_kickoff.md"
    ...
    - "database/lupopedia/json/lupo_actors.json"
    - "database/lupopedia/json/lupo_departments.json"
    - "agents/ (filesystem audit)"
...
- GAP: The kickoff treats the agent system as if it does not yet exist. FACT: `agents/`
...
- FACT (`agents/1/agent.json`): A numbered folder agent has fields `code`, `name`, `layer`,
  `is_required`, `is_kernel`, `recommended_slot`, `version`, `role`, `description`, `aliases`.
- FACT (`agents/99/agent.json`): The same four-file contract (`agent.json`, `capabilities.json`,
...
- FACT (`agents/antigravity/`): Contains only `context.php`. Has no `agent.json`, no
  `capabilities.json`, no `properties.json`, no `system_prompt.txt`.
- FACT (`agents/106/`, `agents/107/`): Have `agent.json` and `system_prompt.txt` but
  are missing `capabilities.json` and `properties.json`.
- INFERENCE: "Agent" currently means at minimum a directory under `agents/`. There is no
...
- FACT (`lupo_agents.json`): `lupo_agents` holds runtime tuning parameters: `model_name`,
  ...
  record.
- FACT (`agents/1/agent.json`): The filesystem `agent.json` holds identity and classification
...
- INFERENCE: There are currently **three** identity surfaces for an AI entity: the filesystem
  `agents/{id}/agent.json`, the DB `lupo_agents` row, and the DB `lupo_actors` row. The
...
- GAP: No mapping was found that connects a numbered `agents/` folder to its corresponding
...
**`agents/` vs `actors/` contract:**

- FACT: `agents/{id}/` is numeric-keyed. `actors/{slug}/` is slug-keyed (after the
  Channel 58 normalization). These are two separate keying schemes with no enforced bridge.
- GAP: No contract document was found that says: "for every `agents/{id}/` folder there
  MUST exist a corresponding `actors/{slug}/` folder with matching `actor_source_id = {id}`."
...
- INFERENCE: At 50 agents, the absence of a bridge contract between `agents/{id}` and
...
1. **DB-first or file-first?** Which surface is authoritative for agent existence?
   If a `agents/{id}/` folder exists without a `lupo_agents` row, is that agent real?
   If a `lupo_agents` row exists without a filesystem folder, is that agent operational?

2. **What is the `agents/` to `actors/` bridge contract?** For every agent folder,
...
1. **Audit the present state explicitly.** Produce an artifact (not code) that lists every
   existing `agents/` folder, its current compliance status against the four-file contract,
...
3. **Define the `agents/` ↔ `actors/` bridge** as a doctrine statement, not a
