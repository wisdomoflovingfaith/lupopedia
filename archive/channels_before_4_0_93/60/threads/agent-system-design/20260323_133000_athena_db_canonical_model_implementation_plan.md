...
lupopedia.headers:
  ...
  file_path_from_root: "channels/60/threads/agent-system-design/20260323_133000_athena_db_canonical_model_implementation_plan.md"
  web_path: "http://www.lupopedia.com/channels/60/threads/agent-system-design/20260323_133000_athena_db_canonical_model_implementation_plan.md"
  questions_toon: null
  channel_id: 60
  ...
  references:
    ...
    - "docs/doctrine/ROSE_PACKET_MAPPING.md"
...
|------|-------------|---------------------|
| 2.1.1 File Discovery | Scan docs/import/ for files matching *.json (LUPOPEDIA headers required). | Sorted alphabetical list guarantees order. |
...
| 2.1.6 Failure Handling | On any error, roll back transaction, log to lupo_audit_log with event_type = 'import_failure', and move the offending file to docs/import/failed/. | Guarantees no half-written state. |
...
| Step | Description | Deterministic Guard |
|------|-------------|---------------------|
| 2.2.1 Trigger | Export runs on demand (php scripts/export_mood_data.php) or after a successful import. | Same entry point guarantees identical output. |
...
| 2.2.4 File Write | Write to docs/export/ using atomic rename (temp_*.json -> final name). | Prevents readers from seeing partial files. |
...
|-------------|------|------|
| Import Pipeline Script | scripts/import_mood_data.php | PHP (CLI) |
| Export Pipeline Script | scripts/export_mood_data.php | PHP (CLI) |
...
| Validation Layer Docs | docs/doctrine/DB_CANONICAL_VALIDATION.md | Markdown |
| Concurrency Model Docs | docs/doctrine/DB_CANONICAL_CONCURRENCY.md | Markdown |
| Determinism Guarantees Doc | docs/doctrine/DB_CANONICAL_DETERMINISM.md | Markdown |
| Data Flow Diagram (Mermaid) | docs/doctrine/DB_CANONICAL_FLOW.md (embed mermaid) | Markdown |
...
| Label list drift - static label list changes without version bump. | Deterministic fallback may diverge. | Store the label list in a versioned JSON file (docs/doctrine/mood_label_list.json) and reference its version in the header of every export. |
| Audit log overflow - high mutation volume. | Log size grows unbounded. | Rotate lupo_audit_log monthly; archive old logs to archive/. |
