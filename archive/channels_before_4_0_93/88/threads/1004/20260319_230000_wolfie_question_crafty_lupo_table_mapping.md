...
lupopedia.headers:
  ...
  file_path_from_root: "channels/88/threads/1004/20260319_230000_wolfie_question_crafty_lupo_table_mapping.md"
  web_path: "http://www.lupopedia.com/channels/88/threads/1004/20260319_230000_wolfie_question_crafty_lupo_table_mapping"
...
lupopedia.edges:
  outbound_edges:
    - { to: "database/lupopedia/mysql/import/old_crafty_syntax_3_7_5_start.sql", type: "requires_reading", weight: 1.0, reason: "Crafty Syntax 3.7.5 source schema — all livehelp_ table definitions" }
    - { to: "database/lupopedia/mysql/import/import_from_old_crafty_syntax.sql", type: "requires_reading", weight: 1.0, reason: "Canonical import: livehelp_ data → lupo_ tables" }
    - { to: "database/lupopedia/mysql/import/drop_old_crafty_syntax_tables.sql", type: "references", weight: 0.8, reason: "Post-upgrade drop of legacy tables" }
    - { to: "database/lupopedia/mysql/install/install_new_lupopedia.sql", type: "requires_reading", weight: 1.0, reason: "Lupopedia 4.x canonical schema — all lupo_ tables" }
    - { to: "docs/database/lupopedia/tables/MIGRATION_MAPPING_REFERENCE.md", type: "references", weight: 1.0, reason: "Index of legacy → Lupopedia table mappings" }
    - { to: "docs/channels/schema/migrations/analysis/CRAFTY_SYNTAX_TO_LUPOPEDIA_STRUCTURED_MAPPING.md", type: "references", weight: 1.0, reason: "Structured mapping report — table and column mapping" }
    - { to: "docs/channels/schema/migrations/analysis/CRAFTY_SYNTAX_TO_LUPOPEDIA_ANALYSIS.md", type: "references", weight: 0.9, reason: "Crafty → Lupopedia analysis" }
    - { to: "docs/database/lupopedia/tables/PROJECT_REGISTRY_SCHEMA_DESIGN.md", type: "references", weight: 0.7, reason: "Registry/schema design context if relevant to mapping" }
    - { to: "docs/doctrine/MIGRATION_DOCTRINE.md", type: "references", weight: 0.9, reason: "Migration doctrine — two-place rule, no hidden logic" }
    - { to: "docs/doctrine/CRAFTY_SYNTAX_INTEGRATION_PLAN.md", type: "references", weight: 0.8, reason: "Crafty Syntax integration and upgrade path" }
    - { to: "rules/root/single-install-no-4.0-upgrade-doctrine.md", type: "depends_on", weight: 0.9, reason: "Upgrade path: Crafty 3.7.5 → Lupopedia 4.0.x only; no Lupopedia→Lupopedia until 4.1" }
    - { to: "channels/66/threads/1001/20260319_200000_wolfie_audit_channel66_system_phase1_thread_repost.md", type: "related_question", weight: 0.5, reason: "Channel 66 ingestion/indexing — may inform migration pipeline" }
    - { to: "channels/66/threads/1002/20260319_000000_wolfie_question_lupopedia_headers_source_of_truth.md", type: "related_question", weight: 0.5, reason: "Channel 66 headers/metadata source of truth — may inform migration metadata" }
    - { to: "docs/database/lupopedia/tables/migrations/livehelp_sessions_migration.md", type: "references", weight: 0.8, reason: "Per-table migration doc example — livehelp_sessions" }
    - { to: "docs/database/lupopedia/tables/migrations/livehelp_messages_migration.md", type: "references", weight: 0.8, reason: "Per-table migration doc — livehelp_messages" }
    - { to: "docs/database/lupopedia/tables/migrations/livehelp_transcripts.md", type: "references", weight: 0.8, reason: "Per-table migration doc — transcripts → dialog" }
    - { to: "docs/doctrine/migrations/livehelp_migrations_readme.md", type: "references", weight: 0.85, reason: "Relocation notice: livehelp_*.md moved to tables/migrations/; import SQL See: uses old path" }
...
# file: Thread 1004 Question — Crafty → Lupo Mapping — session: L-LUPO-ROOT-WOLFIE — delegation: wolfie:root — web_path: http://www.lupopedia.com/channels/88/threads/1004/20260319_230000_wolfie_question_crafty_lupo_table_mapping
...
- **Where mapping is currently documented:**
  - `docs/database/lupopedia/tables/MIGRATION_MAPPING_REFERENCE.md` — index of legacy → new
  - `docs/channels/schema/migrations/analysis/CRAFTY_SYNTAX_TO_LUPOPEDIA_STRUCTURED_MAPPING.md` — structured table/column mapping
  - `docs/channels/schema/migrations/analysis/CRAFTY_SYNTAX_TO_LUPOPEDIA_ANALYSIS.md` — analysis
  - Per-table migration docs under `docs/database/lupopedia/tables/migrations/` (e.g. livehelp_sessions_migration.md, livehelp_messages_migration.md, livehelp_transcripts.md, livehelp_qa_migration.md, livehelp_departments_migration.md, etc.)
...
- **Canonical location (current):** `docs/database/lupopedia/tables/migrations/`
...
- **Relocation notice:** `docs/doctrine/migrations/livehelp_migrations_readme.md` states that legacy migration docs were **relocated** from `docs/doctrine/migrations/` to `docs/database/lupopedia/tables/migrations/`. Use the latter as the canonical path.
...
|--------------------------|-------------------------------|
| `docs/doctrine/migrations/livehelp_*_migration.md` or `/docs/doctrine/migrations/...` | `docs/database/lupopedia/tables/migrations/livehelp_*_migration.md` |
...
**Action for HEPHAESTUS or maintainers:** Update the `-- See:` lines in `import_from_old_crafty_syntax.sql` to the canonical path `docs/database/lupopedia/tables/migrations/<filename>` so that the import script and the docs tree stay in sync.
...
| Source | Path | Role |
|--------|------|------|
| Crafty Syntax 3.7.5 schema (livehelp_*) | database/lupopedia/mysql/import/old_crafty_syntax_3_7_5_start.sql | Source table definitions and seed data |
| Lupopedia install schema (lupo_*) | database/lupopedia/mysql/install/install_new_lupopedia.sql | Target table definitions |
| Import mapping (data movement) | database/lupopedia/mysql/import/import_from_old_crafty_syntax.sql | Canonical livehelp_ → lupo_ INSERT/SELECT |
| Drop legacy tables | database/lupopedia/mysql/import/drop_old_crafty_syntax_tables.sql | Post-upgrade cleanup |
| Mapping index | docs/database/lupopedia/tables/MIGRATION_MAPPING_REFERENCE.md | Legacy → new table index and notes |
| Structured mapping report | docs/channels/schema/migrations/analysis/CRAFTY_SYNTAX_TO_LUPOPEDIA_STRUCTURED_MAPPING.md | Table and column mapping |
| Crafty → Lupo analysis | docs/channels/schema/migrations/analysis/CRAFTY_SYNTAX_TO_LUPOPEDIA_ANALYSIS.md | Analysis and context |
| Migration doctrine | docs/doctrine/MIGRATION_DOCTRINE.md | Two-place rule; no hidden logic |
| Crafty integration plan | docs/doctrine/CRAFTY_SYNTAX_INTEGRATION_PLAN.md | Upgrade and integration |
| Single-install doctrine | rules/root/ (single-install-no-4.0-upgrade-doctrine) | Crafty 3.7.5 → 4.0.x only |
| Per-table migration docs | docs/database/lupopedia/tables/migrations/livehelp_*.md | Per legacy table migration notes (62+ files); canonical location |
| Relocation readme | docs/doctrine/migrations/livehelp_migrations_readme.md | Documents move from docs/doctrine/migrations/ to tables/migrations/ |
| PROJECT_REGISTRY_SCHEMA_DESIGN | docs/database/lupopedia/tables/PROJECT_REGISTRY_SCHEMA_DESIGN.md | Registry/schema context if relevant |
| TOON files | docs/database/lupopedia/tables/active/*.md, database/lupopedia/json/*.json | lupo_ table column reference (from install SQL) |
...
- **Thread 1004:** Keep all mapping artifacts, evidence, and closure in this thread until promoted (e.g. to docs/doctrine) only when the question is resolved and the system accepts a canonical mapping artifact.
