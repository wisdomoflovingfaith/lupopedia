...
lupopedia.headers:
  ...
  file_path_from_root: "channels/channel_creation_doctrine.md"
  web_path: "http://www.lupopedia.com/channels/channel_creation_doctrine.md"
  questions_toon: null
...
lupopedia.edges:
  outbound_edges:
    - { to: "channels/channel_index.md", type: "governs", weight: 1.0, reason: "Governs channel index structure" }
    - { to: "channels/channel_reservations.md", type: "governs", weight: 1.0, reason: "Governs channel reservations" }
    - { to: "database/lupopedia/tables/lupo_channels.toon.json", type: "references", weight: 1.0, reason: "References channel table structure" }
...
Starting now, all newly created channel directories under `channels/` SHALL use `channel_slug`, not `channel_id`.
...
Existing numeric directories (for example `channels/42/`) are legacy-compatible and remain valid historical paths.
...
3. Create directory as `channels/<channel_slug>/`.
...
- `channels/1_channel_refactor_governance/`
