...
cp lupo-hooks/pre-commit.sh .git/hooks/pre-commit
...
For Windows:
```cmd
copy lupo-hooks\pre-commit.bat .git\hooks\pre-commit.bat
```

## Validation Scripts

The hooks call validation scripts located in `lupo-scripts/`:
...
If hooks fail:
1. Run validation manually: `python lupo-scripts/validate_lupopedia_headers_universal.py`
