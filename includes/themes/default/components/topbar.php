...
lupopedia.headers:
  ...
  when_updated: "20260409222210"
  file_path_from_root: "includes/themes/default/components/topbar.php"
  web_path: "http://www.lupopedia.com/lupopedia/includes/themes/default/components/topbar.php"
  questions_toon: null
  ...
  purpose: "Global top navigation; lupo_t UI strings; DatabaseFactory; Model A actor via lupo_session and App\\Auth\\Session::loadById; ActorService for act-as list. Collections UI lives in main_layout saved-collections-nav (light blue master), not in this topbar."
...
// Theme root for topbar and other theme components (main_layout / theme-loader usually set this first).
if (!defined('LUPO_UI_PATH') && defined('LUPOPEDIA_PATH')) {
    $lupo_tr = rtrim(LUPOPEDIA_PATH, "/\\") . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'themes' . DIRECTORY_SEPARATOR . 'default';
    if (is_dir($lupo_tr)) {
        define('LUPO_UI_PATH', $lupo_tr);
    }
}
...
if (!class_exists('DatabaseFactory', false) && $root !== '') {
    $df = $root . '/includes/classes/DatabaseFactory.php';
    if (is_file($df)) {
        require_once $df;
    }
}
...
if (!isset($userAvatar)) {
    if ($isUserLoggedIn && $currentUserId > 0) {
        ...
        if (file_exists($avatar_path)) {
            $userAvatar = $base . '/uploads/avatars/' . $currentUserId . '_avatar.jpg';
        } else {
            $userAvatar = $base . '/images/logoface.png';
        }
    } else {
        $userAvatar = $base . '/images/logoface.png';
    }
...
$avatarTimestamp = '';
if ($userAvatar !== '' && strpos($userAvatar, $base . '/uploads/avatars/') === 0) {
...
<header class="main-header">
    <div class="nav-logo-container" style="position: absolute; top: 20px; left: 0; z-index: 2000;">
        <a href="<?= htmlspecialchars($base . '/index.php') ?>" class="nav-logo" onclick="scrollToTop()" title="<?= htmlspecialchars(function_exists('lupo_t') ? lupo_t('nav.logo_title', 'Lupopedia home') : 'Lupopedia home') ?>">
            <img src="<?= htmlspecialchars($base . '/images/logoface.png') ?>" alt="<?= htmlspecialchars(function_exists('lupo_t') ? lupo_t('nav.logo_alt', 'Lupopedia') : 'Lupopedia') ?>" width="50" height="50" border="0" style="border-radius: 50%;" />
        </a>
    </div>
    <nav class="main-nav">
