 ...
 * wolfie.header.placement: /includes/themes/default/components/collections_dropdown.php
 ...
 *
 * Canonical UI: main_layout saved-collections-nav (light blue master + green tabs).
 * topbar.php does not include this file (4.0.97+); keep for optional or manual includes only.
 * 
 * @version 3.0.18
...
?>
<div class="saved-collections-dropdown master-collections-wrap" data-qa-type="collections" style="display: inline-block; position: relative;">
...
(function() {
    ...
    // Select a collection: full reload with lupo_collection_id so green tabs match server chrome.
    window.selectCollection = function(collectionId, collectionName) {
        if (typeof window.lupoCollectionContextSwitchHref === 'function') {
            window.location.href = window.lupoCollectionContextSwitchHref(collectionId);
            return;
        }
    ...
    function loadCollectionTabs(collectionId, collectionName) {
        ...
        if (typeof window.loadCollectionTabs === 'function') {
            ...
            url.searchParams.set('lupo_collection_id', String(collectionId));
    ...
    if (collectionsButton) {
        collectionsButton.addEventListener('click', function(e) {
            ...
            // Toggle dropdown
            toggleSavedCollectionsDropdown(this, e);
