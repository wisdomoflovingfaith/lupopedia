...
#   file_path_from_root: "includes/themes/default/layouts/admin_layout.php"
#   web_path: "https://www.lupopedia.com/lupopedia/includes/themes/default/layouts/admin_layout.php"
#   status: "complete"
#   when_updated: "20260418035748"
#   trust_tier: "canonical"
#   questions_toon: null
#   memory_toon: "memory/development/canonical/1026/04/admin-layout-php.toon"
#   atoms_toon: null
#   transcript_jsonl: "0/development/admin-layout-php"
...
#   content_slug: "admin-layout-php"
#   default_collection_id: null
#   lupopedia.schema: implementation
#   title: "Admin layout — scroll chrome and menu shell"
#   summary: "Admin page layout with scroll chrome, intro squares, user dropdown, actor switcher; UNTRUSTED GET for section."
# ---------------------------------------------------------------------
...
if (!defined('LUPO_UI_PATH')) {
    define('LUPO_UI_PATH', LUPOPEDIA_PATH . '/includes/themes/default');
}
...
if (!isset($admin_nav_logo_src) || $admin_nav_logo_src === '') {
    $admin_nav_logo_src = LUPOPEDIA_PUBLIC_PATH . '/images/logoface.png';
}
...
$admin_nav_user_avatar = LUPOPEDIA_PUBLIC_PATH . '/images/logoface.png';
$admin_nav_avatar_ts = '';
if ($admin_nav_auth_user_id > 0) {
    $admin_nav_avatar_disk = LUPOPEDIA_PATH . '/uploads/avatars/' . $admin_nav_auth_user_id . '_avatar.jpg';
    if (file_exists($admin_nav_avatar_disk)) {
        $admin_nav_user_avatar = LUPOPEDIA_PUBLIC_PATH . '/uploads/avatars/' . $admin_nav_auth_user_id . '_avatar.jpg';
...
<head>
    ...
    <link rel="stylesheet" href="<?= LUPOPEDIA_PUBLIC_PATH ?>/includes/css/admin-intro-scroll.css">
    <style>
        ...
        .admin-nav-user.nav-user {
            margin: 0;
        }
        .admin-nav-user .user-profile-btn {
            padding: 2px 8px;
            gap: 6px;
            max-height: 56px;
            font-size: 0.75rem;
        }
        .admin-nav-user .user-avatar {
            width: 26px;
            height: 26px;
            flex-shrink: 0;
        }
        .admin-nav-user .user-name.admin-nav-trigger-label {
            max-width: 10em;
            overflow: hidden;
        ...
        .admin-nav-user .dropdown-arrow {
            flex-shrink: 0;
        }
        .admin-nav-user .user-dropdown-menu {
            z-index: 200010;
        }
        .admin-nav-user .admin-dropdown-actor-line {
            font-size: 0.8rem;
            color: #6b7280;
            margin-top: 6px;
            line-height: 1.3;
...
<body>
<div id="admin-app" class="admin-wrap admin-app" data-admin-intro="<?= $admin_disable_scroll_intro ? '0' : '1' ?>" data-admin-scroll-skin="<?= htmlspecialchars($admin_scroll_skin, ENT_QUOTES, 'UTF-8') ?>">

    <div id="admin-scroll-shell">
        <nav class="admin-nav-row" id="admin-nav-row" role="navigation" aria-label="<?= htmlspecialchars(lupo_t('admin.layout.nav_aria', 'Admin quick nav')) ?>">
            <div class="admin-nav-lead">
                <a class="admin-nav-logo" href="<?= htmlspecialchars($admin_nav_logo_href, ENT_QUOTES, 'UTF-8') ?>" title="<?= htmlspecialchars($admin_nav_logo_alt, ENT_QUOTES, 'UTF-8') ?>">
                    <img src="<?= htmlspecialchars($admin_nav_logo_src, ENT_QUOTES, 'UTF-8') ?>" alt="<?= htmlspecialchars($admin_nav_logo_alt, ENT_QUOTES, 'UTF-8') ?>">
                </a>
            </div>
            <div class="admin-nav-squares">
            <?php foreach ($admin_intro_nav_links as $idx => $navItem): ?>
                ...
                ?>
            <a class="admin-nav-sq<?= $ph ? ' is-placeholder' : '' ?>" href="<?= $nh ?>"<?= $ph ? ' onclick="return false;" aria-hidden="true"' : '' ?>><span><?= $nl ?></span></a>
            <?php endforeach; ?>
            </div>
            <div class="admin-nav-tail">
                <?php if ($isUserLoggedIn): ?>
                <div class="nav-user admin-nav-user" id="lupoAdminNavUserDropdown">
                    <div class="user-dropdown">
                        <button type="button" class="user-profile-btn admin-user-profile-btn" onclick="toggleLupoAdminUserDropdown(event)" aria-haspopup="true" aria-expanded="false" aria-controls="lupoAdminUserDropdownMenu" id="lupoAdminUserDropdownBtn">
                            <div class="user-avatar">
                                <img src="<?= htmlspecialchars($admin_nav_user_avatar . $admin_nav_avatar_ts, ENT_QUOTES, 'UTF-8') ?>"
                                     alt=""
                                     style="width: 100%; height: 100%; border-radius: 50%; object-fit: cover;">
                            </div>
                            <span class="user-name admin-nav-trigger-label" title="<?= htmlspecialchars($admin_nav_actor_label, ENT_QUOTES, 'UTF-8') ?>"><?= htmlspecialchars($admin_nav_actor_display) ?></span>
                            ...
                            <div class="dropdown-header">
                                <div class="user-info">
                                    ...
                                    <div class="user-details">
                                        ...
                                        <?php endif; ?>
                                        <div class="admin-dropdown-actor-line"><?= htmlspecialchars(lupo_t('admin.layout.acting_as', 'Acting as:')) ?> <strong><?= htmlspecialchars($admin_nav_actor_label) ?></strong></div>
                                    </div>
                                </div>
                            </div>
        ...
        <div class="admin-scroll-frame">
            <div class="admin-scroll-row admin-scroll-row--top" aria-hidden="true">
                <div class="admin-scroll-cap admin-scroll-cap--l"></div>
                <div class="admin-scroll-center"></div>
                <div class="admin-scroll-cap admin-scroll-cap--r"></div>
            </div>
            <div class="admin-scroll-row admin-scroll-row--mid">
                <div class="admin-scroll-cap admin-scroll-cap--l"></div>
                <div class="admin-scroll-center">
    <div class="admin-body">
        ...
        <main class="admin-main" id="admin-main">
            ...
            <div class="admin-scroll-row admin-scroll-row--bottom" aria-hidden="true">
                <div class="admin-scroll-cap admin-scroll-cap--l"></div>
                <div class="admin-scroll-center"></div>
                <div class="admin-scroll-cap admin-scroll-cap--r"></div>
            </div>
    ...
    <footer class="basic-footer">
        ...
        <form method="get" action="" class="admin-lang-form" style="display: inline-block; margin-left: 12px;">
    ...
    <div id="admin-intro-overlay" aria-hidden="true">
        <?php foreach ($admin_intro_nav_links as $navItem): ?>
            ...
            ?>
        <a class="admin-intro-sq<?= $ph ? ' is-placeholder' : '' ?>" href="<?= $oh ?>"<?= $ph ? ' onclick="return false;" tabindex="-1"' : '' ?>><span><?= $ol ?></span></a>
        <?php endforeach; ?>
    </div>

    <script src="<?= LUPOPEDIA_PUBLIC_PATH ?>/includes/js/admin-intro-scroll.js"></script>
