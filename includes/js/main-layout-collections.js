...
function get_current_page_context() {
    ...
    var hid = document.getElementById('current-content-id');
    if (hid && hid.value) {
...
function lupoMasterCollectionsHydrateIfEmpty(dropdown, panel) {
    ...
    if (panel.getAttribute('data-master-list-hydrated') === '1') {
        return;
    }
    if (panel.getAttribute('data-master-nav-server') === '1') {
        panel.setAttribute('data-master-list-hydrated', '1');
        return;
    }
    ...
    var url = lupoPublicPath() + '/api/list_nav_collections_grouped.php';
    fetch(url)
...
function lupoCloseMasterCollectionsDropdown() {
    var w = document.querySelector('.master-collections-wrap');
    if (!w) {
        return;
    }
...
function lupoBuildShortcutTabsListHtml(tabsData) {
    ...
    for (var mainTab in tabsData) {
        ...
        html += '<a href="javascript:void(0)" class="main-tab shortcut-pin" role="button" data-collection-tab-id="' + rootId + '" onclick="lupoAddCurrentPageToCollectionTab(this); return false;">| ' + htmlEscape(mainTab) + '</a>';
        if (subTabs && subTabs._children && subTabs._children.length) {
            for (var ci = 0; ci < subTabs._children.length; ci++) {
                ...
                html += '<a href="javascript:void(0)" class="sub-tab shortcut-pin" role="button" data-collection-tab-id="' + ctid + '" onclick="lupoAddCurrentPageToCollectionTab(this); return false;">|— ' + htmlEscape(cname) + '</a>';
            }
            ...
            for (var key in subTabs) {
                ...
                html += '<a href="javascript:void(0)" class="sub-tab shortcut-pin" role="button" data-collection-tab-id="0" onclick="lupoAddCurrentPageToCollectionTab(this); return false;">|— ' + htmlEscape(val) + '</a>';
            }
        }
...
function lupoRefreshShortcutDropdown(done) {
    ...
    var url = lupoPublicPath() + '/api/load_collection_tabs.php?collection_id=' + collectionId;
    fetch(url)
        .then(function (response) { return response.json(); })
        .then(function (data) {
...
function saveCollection() {
    ...
    fetch(lupoPublicPath() + '/api/save_collection.php', {
        method: 'POST',
        headers: {
...
function loadCollectionsList() {
    ...
    fetch(lupoPublicPath() + '/api/list_collections.php')
    .then(function(response) { return response.json(); })
    .then(function(data) {
...
function loadCollectionById(collectionId, collectionName) {
    ...
    fetch(lupoPublicPath() + '/api/load_collection.php', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
function deleteCollection(collectionId) {
    ...
    fetch(lupoPublicPath() + '/api/delete_collection.php', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
function lupoAddCurrentPageToCollectionTab(el) {
    ...
    var ml = window.LUPO_MAIN_LAYOUT || {};
    var cidEl = document.getElementById('current-content-id');
    ...
    fetch(lupoPublicPath() + '/api/add_to_collection.php', {
        method: 'POST',
...
window.loadCollectionTabs = function(collectionId, collectionName) {
    ...
    fetch(lupoPublicPath() + '/api/load_collection_tabs.php?collection_id=' + collectionId)
        .then(function(response) { return response.json(); })
        .then(function(data) {
            if (data && data.success) {
                var td = (data.tabs_data && Object.keys(data.tabs_data).length > 0) ? data.tabs_data : {};
            ...
            if (data.success && data.tabs_data && Object.keys(data.tabs_data).length > 0) {
                ...
                if (container) {
                    ...
                    for (var mainTab in data.tabs_data) {
                        ...
                        html += '<div class="dropdown" data-qa-type="' + htmlEscape(tabType) + '">';
