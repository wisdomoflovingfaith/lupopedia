 ...
 * Tracking: ajax.php?action=csrf_token then ?action=track (JSON + CSRF) — not FormData api/track.php.
...
(function (window, document) {
    ...
    function getSemanticEdgeFocus() {
        var el = document.querySelector('.resources-middle-center[data-edge-focus]');
        if (el && el.getAttribute) {
            var v = el.getAttribute('data-edge-focus');
            if (v) {
                return v;
            }
        }
        var el2 = document.querySelector('.resources-middle-center[data-artifact-type]');
        if (el2 && el2.getAttribute) {
            var at = el2.getAttribute('data-artifact-type') || '';
            if (at === 'help_guide') {
                return 'tutorial';
            }
            if (at === 'text/markdown') {
    ...
    function injectChromeCss(colors) {
        if (document.getElementById('monitor-chrome-style')) {
            return;
        }
        var c = colors || {};
        ...
        style.appendChild(document.createTextNode(
            ...
            '#lupoVisitorCommandBarDiv .chrome-inner{pointer-events:auto;display:flex;flex-direction:row;align-items:center;' +
            ...
            '#lupoVisitorCommandBarDiv .chrome-btn{min-width:40px;min-height:36px;padding:6px 10px;border-radius:6px;' +
            'border:1px solid ' + bd + ';background:' + hi + ';color:' + tx + ';cursor:pointer;font-size:16px;line-height:1.2;}' +
            '#lupoVisitorCommandBarDiv .chrome-btn:hover{background:' + bd + ';}' +
            '#lupoVisitorCommandBarDiv .chrome-btn:focus{outline:2px solid ' + tx + ';outline-offset:2px;}'
        ));
    ...
    function buildCommandBar() {
        ...
        wrap.innerHTML =
            ...
            '<button type="button" class="chrome-btn" data-action="paths" title="Paths" aria-label="Paths">&#8592;</button>' +
            '<button type="button" class="chrome-btn" data-action="tags" title="Tags" aria-label="Tags">#</button>' +
            '<button type="button" class="chrome-btn" data-action="comments" title="Comments" aria-label="Comments">&#128172;</button>' +
            '<button type="button" class="chrome-btn" data-action="livehelp" title="Live help" aria-label="Live help">&#128065;</button>' +
            '</div>';
    ...
    function buildTrackBody(extra) {
        ...
        if (elAt && elAt.getAttribute) {
            var at2 = elAt.getAttribute('data-artifact-type');
            if (at2) {
