/*
 * LupoHeritageEyes — self-contained WOLFIE sprite eyes (no DynLayer / layers).
 * Expects window.LUPO_HERITAGE_EYES = { imgBase, livehelpUrl, delayMs } set before this file loads.
 * DOM: optional #heritage-master-stage (position:fixed, single z-index, pointer-events:none).
 * Layer ids: closedblockDiv, backblockDiv, lefteyeblockDiv, righteyeblockDiv, lidsblockDiv,
 *            livehelpblockDiv, eye-close-btn; img ids: heritage-* (see heritage-eyes.php).
...
(function (window, document) {
    ...
    function closeeyes() {
        ...
        hideVis(els.left);
        showVis(els.closed);
        var one = $('heritage-eyeone');
        var two = $('heritage-eyetwo');
    ...
    function movetheeyes(cfg) {
        ...
        var elLids = $('heritage-lids');
        var elClosed = $('heritage-closed');
        var elWhite = $('heritage-whites');
        if (elLids) {
