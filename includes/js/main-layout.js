function toggleSavedCollectionsDropdown(button, evt) {
    ...
    if (newState && button.closest('.master-collections-wrap')) {
        var d = dropdown;
        var p = panel;
...
document.addEventListener('click', function (event) {
    ...
    var isMenu = !!(
        ...
        el.closest('.debug-try2-nav .dropdown') ||
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var try2Drops = container.querySelectorAll('.dropdown');
...
/** Toggle footer fixed semantic bar (#footer-semantic-nav in footer.php); body class drives title-row hide icon + .main-footer padding. */
function lupoSemanticBottomNavSetVisible(visible) {
    var bar = document.getElementById('footer-semantic-nav');
    if (bar) {
        if (visible) {
    ...
    if (visible) {
        document.body.classList.add('semantic-nav-visible');
    } else {
        document.body.classList.remove('semantic-nav-visible');
    }
}
...
document.addEventListener('DOMContentLoaded', function () {
    document.addEventListener('click', function (e) {
        ...
        if (t.closest('.semantic-show-nav-trigger')) {
            e.preventDefault();
            lupoSemanticBottomNavSetVisible(true);
            return;
        }
        if (t.closest('.semantic-hide-nav-trigger')) {
            e.preventDefault();
    ...
    document.addEventListener('keydown', function (e) {
        ...
        if (el.classList.contains('semantic-show-nav-trigger')) {
            e.preventDefault();
            lupoSemanticBottomNavSetVisible(true);
            return;
        }
        if (el.classList.contains('semantic-hide-nav-trigger')) {
            e.preventDefault();
            lupoSemanticBottomNavSetVisible(false);
        }
    });
