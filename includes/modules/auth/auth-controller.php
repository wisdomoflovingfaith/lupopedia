...
lupopedia.headers:
  ...
  file_path_from_root: "includes/modules/auth/auth-controller.php"
  web_path: "http://www.lupopedia.com/lupopedia/includes/modules/auth/auth-controller.php"
  questions_toon: null
...
$session_compat_path = (defined('LUPOPEDIA_ABSPATH') ? rtrim(LUPOPEDIA_ABSPATH, DIRECTORY_SEPARATOR) : dirname(dirname(dirname(__DIR__)))) . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'functions' . DIRECTORY_SEPARATOR . 'session-compat-5.6.php';
if (is_file($session_compat_path)) {
    require_once $session_compat_path;
}

require_once(LUPOPEDIA_ABSPATH . 'includes/security/password-hash.php');
require_once(LUPOPEDIA_ABSPATH . 'includes/functions/auth-helpers.php');
require_once(LUPOPEDIA_ABSPATH . 'includes/functions/redirect-helpers.php');
...
function lupo_auth_controller_get_db()
{
    if (!class_exists('DatabaseFactory', false) && defined('LUPOPEDIA_ABSPATH')) {
        $df = LUPOPEDIA_ABSPATH . 'includes/classes/DatabaseFactory.php';
        if (is_file($df)) {
            require_once $df;
        }
    }
