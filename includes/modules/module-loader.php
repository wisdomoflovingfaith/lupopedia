...
#   file_path_from_root: "includes/modules/module-loader.php"
#   web_path: "https://www.lupopedia.com/lupopedia/includes/modules/module-loader.php"
#   status: "complete"
#   when_updated: "20260418035748"
#   trust_tier: "canonical"
#   questions_toon: null
#   memory_toon: "memory/development/canonical/1026/04/module-loader-php.toon"
#   atoms_toon: null
...
if (!defined('LUPOPEDIA_PUBLIC_PATH')) {
    ...
    $patterns = array('/includes/', '/tests/', '/admin/', '/database/', '/docs/', '/scripts/', '/tools/', '/legacy/', '/channels/', '/uploads/');
...
function lupo_module_loader_get_db()
{
    if (!class_exists('DatabaseFactory', false)) {
        ...
        $p = ($base !== '') ? rtrim(str_replace('\\', '/', $base), '/') . '/includes/classes/DatabaseFactory.php' : '';
        if ($p !== '' && is_file($p)) {
            require_once $p;
        }
    }
 ...
 */
$auth_module = LUPOPEDIA_ABSPATH . 'includes/modules/auth/auth-controller.php';
 ...
 */
$oauth_module = LUPOPEDIA_ABSPATH . 'includes/modules/auth/oauth-controller.php';
 ...
 */
$truth_module = LUPOPEDIA_ABSPATH . '/includes/modules/truth/truth-controller.php';
 ...
 */
$crafty_syntax_module = LUPOPEDIA_ABSPATH . '/includes/modules/crafty_syntax/crafty_syntax-controller.php';
 ...
 */
$help_module = LUPOPEDIA_ABSPATH . '/includes/modules/help/help-controller.php';
 ...
 */
$list_module = LUPOPEDIA_ABSPATH . '/includes/modules/list/list-controller.php';
 ...
 */
$content_module = LUPOPEDIA_ABSPATH . '/includes/modules/content/content-controller.php';
...
function lupo_route_slug($slug)
{
    ...
    if ((preg_match('#^(doctrine|qa|docs|flp)/#i', $slug) || $slug === 'flare_apply') && function_exists('lupo_resolve_web_path')) {
        ...
        if ($resolved && is_array($resolved)) {
            ...
            if ($content_id > 0) {
                $content_controller = LUPOPEDIA_ABSPATH . '/includes/modules/content/content-controller.php';
    ...
    if (preg_match('#^agent/([^/]+)(?:/(.*))?$#', $normalized_slug, $matches)) {
        ...
        $controller_path = rtrim($app_root, '/\\') . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'modules' . DIRECTORY_SEPARATOR . 'actors' . DIRECTORY_SEPARATOR . 'agent-www-controller.php';
    ...
    if ($normalized_slug === 'my-profile' && $request_method !== 'POST') {
        $app_root = defined('LUPOPEDIA_PATH') ? LUPOPEDIA_PATH : LUPOPEDIA_ABSPATH;
        $actors_controller_path = rtrim($app_root, '/\\') . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'modules' . DIRECTORY_SEPARATOR . 'actors' . DIRECTORY_SEPARATOR . 'actors-controller.php';
    ...
    if ($normalized_slug === 'nav/semantic-navbar' || $normalized_slug === 'nav/semantic-navbar-js' || $normalized_slug === 'nav/semantic_navbar') {
        $app_root = defined('LUPOPEDIA_PATH') ? LUPOPEDIA_PATH : LUPOPEDIA_ABSPATH;
        $nav_path = rtrim($app_root, '/\\') . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'modules' . DIRECTORY_SEPARATOR . 'nav' . DIRECTORY_SEPARATOR . 'semantic_navbar.php';
        if (file_exists($nav_path)) {
            require $nav_path;
            exit;
        }
        $js_path = rtrim($app_root, '/\\') . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'modules' . DIRECTORY_SEPARATOR . 'nav' . DIRECTORY_SEPARATOR . 'semantic-navbar-js.php';
    ...
    if ($normalized_slug === 'livehelp_js' || $normalized_slug === 'livehelp-js') {
        $app_root = defined('LUPOPEDIA_PATH') ? LUPOPEDIA_PATH : LUPOPEDIA_ABSPATH;
        $livehelp_js_path = rtrim($app_root, '/\\') . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'modules' . DIRECTORY_SEPARATOR . 'crafty_syntax' . DIRECTORY_SEPARATOR . 'livehelp-js.php';
    ...
    if ($normalized_slug === 'image' && ($visitor_image_what === '' || $visitor_image_what === null || in_array((string) $visitor_image_what, array('getstate', 'getcredit', 'userstat'), true))) {
        $app_root = defined('LUPOPEDIA_PATH') ? LUPOPEDIA_PATH : LUPOPEDIA_ABSPATH;
        $visitor_image_path = rtrim($app_root, '/\\') . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'modules' . DIRECTORY_SEPARATOR . 'crafty_syntax' . DIRECTORY_SEPARATOR . 'visitor-image.php';
    ...
    if ($normalized_slug === 'livehelp' || $normalized_slug === 'chat') {
        $app_root = defined('LUPOPEDIA_PATH') ? LUPOPEDIA_PATH : LUPOPEDIA_ABSPATH;
        $livehelp_path = rtrim($app_root, '/\\') . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'modules' . DIRECTORY_SEPARATOR . 'crafty_syntax' . DIRECTORY_SEPARATOR . 'livehelp.php';
    ...
    if ($normalized_slug === 'choosedepartment') {
        $app_root = defined('LUPOPEDIA_PATH') ? LUPOPEDIA_PATH : LUPOPEDIA_ABSPATH;
        $choosedepartment_path = rtrim($app_root, '/\\') . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'modules' . DIRECTORY_SEPARATOR . 'crafty_syntax' . DIRECTORY_SEPARATOR . 'choosedepartment.php';
    ...
    if ($normalized_slug === 'visitor-chat-stream') {
        $app_root = defined('LUPOPEDIA_PATH') ? LUPOPEDIA_PATH : LUPOPEDIA_ABSPATH;
        $stream_path = rtrim($app_root, '/\\') . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'modules' . DIRECTORY_SEPARATOR . 'crafty_syntax' . DIRECTORY_SEPARATOR . 'visitor-chat-stream.php';
    ...
    if ($slug === 'operator/signon') {
        ...
        $content_renderer = LUPOPEDIA_ABSPATH . '/includes/modules/content/renderers/content-renderer.php';
        ...
        if ($op_req_method === 'POST' && isset($UNTRUSTED['post']['channel_id'])) {
            ...
            if ($db && $channel_id > 0) {
                if (!function_exists('current_user')) {
                    require_once LUPOPEDIA_ABSPATH . '/includes/functions/auth-helpers.php';
                }
        ...
        $operator_signon_view = LUPOPEDIA_ABSPATH . '/includes/modules/operator/views/signon.php';
    ...
    if ($slug === 'channels') {
        $app_root = defined('LUPOPEDIA_PATH') ? LUPOPEDIA_PATH : LUPOPEDIA_ABSPATH;
        $view_path = rtrim($app_root, '/\\') . DIRECTORY_SEPARATOR . 'views' . DIRECTORY_SEPARATOR . 'dialog' . DIRECTORY_SEPARATOR . 'channels.php';
    ...
    if ($slug === 'threads') {
        $app_root = defined('LUPOPEDIA_PATH') ? LUPOPEDIA_PATH : LUPOPEDIA_ABSPATH;
        $view_path = rtrim($app_root, '/\\') . DIRECTORY_SEPARATOR . 'views' . DIRECTORY_SEPARATOR . 'dialog' . DIRECTORY_SEPARATOR . 'threads.php';
        if (file_exists($view_path)) {
            require_once $view_path;
            exit;
        }
    ...
    if ($slug === 'messages') {
        $app_root = defined('LUPOPEDIA_PATH') ? LUPOPEDIA_PATH : LUPOPEDIA_ABSPATH;
        $view_path = rtrim($app_root, '/\\') . DIRECTORY_SEPARATOR . 'views' . DIRECTORY_SEPARATOR . 'dialog' . DIRECTORY_SEPARATOR . 'messages.php';
        if (file_exists($view_path)) {
            require_once $view_path;
            exit;
        }
    ...
    if ($slug === 'thread') {
        $app_root = defined('LUPOPEDIA_PATH') ? LUPOPEDIA_PATH : LUPOPEDIA_ABSPATH;
        $api_path = rtrim($app_root, '/\\') . DIRECTORY_SEPARATOR . 'api' . DIRECTORY_SEPARATOR . 'dialog' . DIRECTORY_SEPARATOR . 'create-thread.php';
    ...
    if ($slug === 'message') {
        $app_root = defined('LUPOPEDIA_PATH') ? LUPOPEDIA_PATH : LUPOPEDIA_ABSPATH;
        $api_path = rtrim($app_root, '/\\') . DIRECTORY_SEPARATOR . 'api' . DIRECTORY_SEPARATOR . 'dialog' . DIRECTORY_SEPARATOR . 'post-message.php';
        if (file_exists($api_path)) {
            require_once $api_path;
            exit;
        }
    ...
    if (preg_match('#^message/(\d+)/actor/?$#', $slug, $api_m)) {
        ...
        $app_root = defined('LUPOPEDIA_PATH') ? LUPOPEDIA_PATH : LUPOPEDIA_ABSPATH;
        $api_path = rtrim($app_root, '/\\') . DIRECTORY_SEPARATOR . 'api' . DIRECTORY_SEPARATOR . 'dialog' . DIRECTORY_SEPARATOR . 'assign-actor.php';
        if (file_exists($api_path)) {
            require_once $api_path;
            exit;
        }
    ...
    if (preg_match('#^message/(\d+)/route/?$#', $slug, $api_m)) {
        ...
        $app_root = defined('LUPOPEDIA_PATH') ? LUPOPEDIA_PATH : LUPOPEDIA_ABSPATH;
        $api_path = rtrim($app_root, '/\\') . DIRECTORY_SEPARATOR . 'api' . DIRECTORY_SEPARATOR . 'dialog' . DIRECTORY_SEPARATOR . 'trigger-routing.php';
        if (file_exists($api_path)) {
            require_once $api_path;
            exit;
        }
    ...
    if (preg_match('#^api/registry/actors/(lookup|register)$#', $slug, $api_m)) {
        ...
        $app_root = defined('LUPOPEDIA_PATH') ? LUPOPEDIA_PATH : LUPOPEDIA_ABSPATH;
        $api_path = rtrim($app_root, '/\\') . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'modules' . DIRECTORY_SEPARATOR . 'api' . DIRECTORY_SEPARATOR . 'registry-api.php';
        if (file_exists($api_path)) {
            require_once $api_path;
            exit;
        }
    ...
    if (preg_match('#^api/channels/admin/(operators|departments|settings)(?:/(\d+))?$#', $slug, $api_m)) {
        ...
        $app_root = defined('LUPOPEDIA_PATH') ? LUPOPEDIA_PATH : LUPOPEDIA_ABSPATH;
        $api_path = rtrim($app_root, '/\\') . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'modules' . DIRECTORY_SEPARATOR . 'api' . DIRECTORY_SEPARATOR . 'channels-admin-api.php';
        if (file_exists($api_path)) {
            require_once $api_path;
            exit;
        }
    ...
    if (preg_match('#^api/channels/(\d+)/messages/?$#', $slug, $api_m)) {
        ...
        $app_root = defined('LUPOPEDIA_PATH') ? LUPOPEDIA_PATH : LUPOPEDIA_ABSPATH;
        $api_path = rtrim($app_root, '/\\') . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'modules' . DIRECTORY_SEPARATOR . 'api' . DIRECTORY_SEPARATOR . 'channels-api.php';
        if (file_exists($api_path)) {
            require_once $api_path;
            exit;
        }
    ...
    if (preg_match('#^api/kairos/tick/?$#', $slug)) {
        $app_root = defined('LUPOPEDIA_PATH') ? LUPOPEDIA_PATH : LUPOPEDIA_ABSPATH;
        $kairos_api_path = rtrim($app_root, '/\\') . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'modules' . DIRECTORY_SEPARATOR . 'api' . DIRECTORY_SEPARATOR . 'kairos-api.php';
    ...
    if (preg_match('#^api/context-graph/(context|edges|edge|channel-map)$#', $slug, $api_m)) {
        ...
        $app_root = defined('LUPOPEDIA_PATH') ? LUPOPEDIA_PATH : LUPOPEDIA_ABSPATH;
        $api_path = rtrim($app_root, '/\\') . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'modules' . DIRECTORY_SEPARATOR . 'api' . DIRECTORY_SEPARATOR . 'context-graph-api.php';
        if (file_exists($api_path)) {
            require_once $api_path;
            exit;
        }
    ...
    if (preg_match('#^api/channel/typing$#', $slug)) {
        $app_root = defined('LUPOPEDIA_PATH') ? LUPOPEDIA_PATH : LUPOPEDIA_ABSPATH;
        $typing_api_path = rtrim($app_root, '/\\') . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'modules' . DIRECTORY_SEPARATOR . 'channels' . DIRECTORY_SEPARATOR . 'channel-typing-api.php';
    ...
    if (preg_match('#^api/transcript/append/?$#', $slug)) {
        $app_root = defined('LUPOPEDIA_PATH') ? LUPOPEDIA_PATH : LUPOPEDIA_ABSPATH;
        $transcript_api_path = rtrim($app_root, '/\\') . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'modules' . DIRECTORY_SEPARATOR . 'api' . DIRECTORY_SEPARATOR . 'transcript-api.php';
    ...
    if (preg_match('#^api/channel/send$#', $slug)) {
        $app_root = defined('LUPOPEDIA_PATH') ? LUPOPEDIA_PATH : LUPOPEDIA_ABSPATH;
        $send_api_path = rtrim($app_root, '/\\') . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'modules' . DIRECTORY_SEPARATOR . 'channels' . DIRECTORY_SEPARATOR . 'channel-send-api.php';
    ...
    if (preg_match('#^api/channel/messages$#', $slug)) {
        $app_root = defined('LUPOPEDIA_PATH') ? LUPOPEDIA_PATH : LUPOPEDIA_ABSPATH;
        $messages_api_path = rtrim($app_root, '/\\') . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'modules' . DIRECTORY_SEPARATOR . 'channels' . DIRECTORY_SEPARATOR . 'channel-messages-api.php';
    ...
    if (preg_match('#^api/channel/check$#', $slug)) {
        $app_root = defined('LUPOPEDIA_PATH') ? LUPOPEDIA_PATH : LUPOPEDIA_ABSPATH;
        $check_api_path = rtrim($app_root, '/\\') . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'modules' . DIRECTORY_SEPARATOR . 'channels' . DIRECTORY_SEPARATOR . 'channel-check-api.php';
    ...
    if (preg_match('#^api/prompts/(save|list|get|dispatch)/?$#', $slug, $api_m)) {
        ...
        $app_root = defined('LUPOPEDIA_PATH') ? LUPOPEDIA_PATH : LUPOPEDIA_ABSPATH;
        $prompts_api_path = rtrim($app_root, '/\\') . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'modules' . DIRECTORY_SEPARATOR . 'api' . DIRECTORY_SEPARATOR . 'prompts-api.php';
    ...
    if (preg_match('#^api/operator/pending-visitors$#', $slug)) {
        $app_root = defined('LUPOPEDIA_PATH') ? LUPOPEDIA_PATH : LUPOPEDIA_ABSPATH;
        $api_path = rtrim($app_root, '/\\') . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'modules' . DIRECTORY_SEPARATOR . 'channels' . DIRECTORY_SEPARATOR . 'operator-pending-visitors-api.php';
        if (file_exists($api_path)) {
            require_once $api_path;
            exit;
        }
    ...
    if (preg_match('#^api/operator/accept-visitor$#', $slug)) {
        $app_root = defined('LUPOPEDIA_PATH') ? LUPOPEDIA_PATH : LUPOPEDIA_ABSPATH;
        $api_path = rtrim($app_root, '/\\') . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'modules' . DIRECTORY_SEPARATOR . 'channels' . DIRECTORY_SEPARATOR . 'operator-accept-visitor-api.php';
        if (file_exists($api_path)) {
            require_once $api_path;
            exit;
        }
    ...
    if (preg_match('#^(edges|contexts|hashtags|folders|qa|references|namespaces|next|previous)/(.+)$#', $slug, $api_m)) {
        ...
        $app_root = defined('LUPOPEDIA_PATH') ? LUPOPEDIA_PATH : LUPOPEDIA_ABSPATH;
        $api_path = rtrim($app_root, '/\\') . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'modules' . DIRECTORY_SEPARATOR . 'api' . DIRECTORY_SEPARATOR . 'semantic-navbar-api.php';
        if (file_exists($api_path)) {
            require_once $api_path;
            exit;
        }
    ...
    if ($slug === 'channels/my-channels') {
        $app_root = defined('LUPOPEDIA_PATH') ? LUPOPEDIA_PATH : LUPOPEDIA_ABSPATH;
        $channels_controller_path = rtrim($app_root, '/\\') . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'modules' . DIRECTORY_SEPARATOR . 'channels' . DIRECTORY_SEPARATOR . 'channels-controller.php';
        ...
        $content_renderer = $app_root . '/includes/modules/content/renderers/content-renderer.php';
        if (file_exists($content_renderer)) {
            require_once $content_renderer;
        }
    ...
    if ($slug === 'qa' || strpos($slug, 'qa/') === 0) {
        ...
        // Root Q/A page: /qa/
        if (empty($qa_slug)) {
            $qa_index_view = LUPOPEDIA_ABSPATH . '/includes/modules/qa/views/index.php';
        ...
        // Q/A question page: /qa/<slug>
            ...
            $qa_question_view = LUPOPEDIA_ABSPATH . '/includes/modules/qa/views/question.php';
    ...
    if (preg_match('#^edge/(.+)$#', $slug, $matches)) {
        ...
        $edge_controller = LUPOPEDIA_ABSPATH . '/includes/modules/content/edge-controller.php';
    ...
    if (strpos($slug, 'visibility/human-inbox') === 0) {
        $human_requests_controller = LUPOPEDIA_ABSPATH . '/routes/human_requests.php';
    ...
    if (strpos($slug, 'crafty_syntax') !== false) {
        ...
        $crafty_syntax_controller = LUPOPEDIA_ABSPATH . '/includes/modules/crafty_syntax/crafty_syntax-controller.php';
