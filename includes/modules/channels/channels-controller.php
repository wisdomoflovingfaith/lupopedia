...
function channels_handle_show($channel_id, $initial_thread_id = null) {
    ...
    if (!$db) {
        if (!function_exists('render_main_layout')) {
            $renderer = $app_root . '/includes/modules/content/renderers/content-renderer.php';
    ...
    $view_path = $app_root . '/includes/modules/channels/views/show.php';
...
function channels_handle_stream($channel_id) {
    ...
    $public_path = defined('LUPOPEDIA_PUBLIC_PATH') ? LUPOPEDIA_PUBLIC_PATH : '';
    $view_path = $app_root . '/includes/modules/channels/views/stream.php';
    if (!file_exists($view_path)) {
        header('Content-Type: text/html; charset=utf-8');
...
function channels_handle_log_show($channel_id) {
    ...
    $public_path = defined('LUPOPEDIA_PUBLIC_PATH') ? LUPOPEDIA_PUBLIC_PATH : '';
    $view_path = $app_root . '/includes/modules/channels/views/channel-log.php';
    if (!file_exists($view_path)) {
        return render_main_layout(array(
...
function channels_handle_my_channels() {
    ...
    $public_path = defined('LUPOPEDIA_PUBLIC_PATH') ? LUPOPEDIA_PUBLIC_PATH : '';
    $view_path = $app_root . '/includes/modules/channels/views/my-channels.php';
    if (!file_exists($view_path)) {
        return render_main_layout(array(
...
function channels_handle_edit_channel($channel_id) {
    ...
    $view_path = $app_root . '/includes/modules/channels/views/edit-channel.php';
    if (!file_exists($view_path)) {
        return render_main_layout(array(
...
function channels_list_artifacts($channel_id, $artifact_type = null) {
    ...
    $channel_dir = rtrim($base_path, $sep) . $sep . 'channels' . $sep . $channel_id;
    $artifacts = array();
