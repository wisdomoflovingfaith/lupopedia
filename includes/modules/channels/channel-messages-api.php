...
$visitor_helper_path = rtrim($app_root, '/\\') . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'modules' . DIRECTORY_SEPARATOR . 'crafty_syntax' . DIRECTORY_SEPARATOR . 'visitor-session-helper.php';
...
if ($visitor_mode) {
    ...
    $stmt = $db->prepare("SELECT m.dialog_message_id, m.dialog_thread_id, m.channel_id, m.from_actor_id, m.to_actor_id, m.message_text, m.message_type, m.created_ymdhis, m.forwarded_by_actor_id, m.original_sender_actor_id FROM {$table_prefix}dialog_messages m WHERE m.channel_id = :channel_id AND m.is_deleted = 0 AND m.created_ymdhis > :after AND (m.to_actor_id IS NULL OR m.to_actor_id = :cur_to OR m.from_actor_id = :cur_from) ORDER BY m.created_ymdhis ASC LIMIT 200");
    $stmt->execute(array(':channel_id' => $channel_id, ':after' => $after_ymdhis, ':cur_to' => $actor_id, ':cur_from' => $actor_id));
}
$messages = [];
$thread_ids = [];
$actor_ids = [];
