...
if (!class_exists('TraitEnforcer')) {
    ...
    $trait_enforcer_path = rtrim($trait_enforcer_path, '/\\') . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'classes' . DIRECTORY_SEPARATOR . 'TraitEnforcer.php';
...
// TASK INTEGRATION: Detect [task] command (Lupopedia 4.0.99)
if (strpos($message_text, '[task]') !== false) {
    if (!class_exists('TaskService')) {
        require_once LUPOPEDIA_PATH . '/includes/classes/TaskService.php';
    }
    if (!class_exists('ActorLookup')) {
        require_once LUPOPEDIA_PATH . '/includes/classes/ActorLookup.php';
    }
    $taskService = new TaskService($db);
    $taskData = $taskService->parseTaskCommand($message_text);
    if ($taskData) {
        $assignedToId = null;
        if (!empty($taskData['assigned_to'])) {
            $assignedToId = ActorLookup::findIdByName($taskData['assigned_to']);
        }
...
if ($channel_id > 0) {
    $row_ck = $db->fetchRow(
        "SELECT channel_key FROM {$table_prefix}channels WHERE channel_id = :cid AND is_deleted = 0 LIMIT 1",
        array(':cid' => $channel_id)
    );
    if ($row_ck && isset($row_ck['channel_key'])) {
        $channel_key_ins = (string) $row_ck['channel_key'];
    }
}

$stmt_ins = $db->prepare("INSERT INTO {$table_prefix}dialog_messages (dialog_thread_id, channel_id, channel_key, from_actor_id, source_faucet_slug, source_faucet_instance_id, to_actor_id, message_text, message_type, created_ymdhis, updated_ymdhis, is_deleted, forwarded_by_actor_id, original_sender_actor_id) VALUES (:dialog_thread_id, :channel_id, :channel_key, :from_actor_id, :source_faucet_slug, :source_faucet_instance_id, :to_actor_id, :message_text, 'text', :created_ymdhis, :updated_ymdhis, 0, :forwarded_by_actor_id, :original_sender_actor_id)");
$stmt_ins->execute(array(
    ':dialog_thread_id' => $dialog_thread_id,
    ':channel_id'      => $ins_channel_id,
    ':channel_key'     => $channel_key_ins,
...
if ($channel_id > 0) {
    ...
    if (!class_exists('timestamp_ymdhis', false) && $app_root !== '') {
        require_once rtrim($app_root, '/\\') . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'classes' . DIRECTORY_SEPARATOR . 'TimestampYmdhis.php';
    }
