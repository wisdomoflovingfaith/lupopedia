...
#   file_path_from_root: "includes/bootstrap.php"
#   web_path: "https://www.lupopedia.com/lupopedia/includes/bootstrap.php"
#   status: "active"
#   when_updated: "20260417073722"
#   trust_tier: "canonical"
#   questions_toon: null
#   memory_toon: "memory/development/canonical/1026/04/bootstrap-php.toon"
#   atoms_toon: null
...
All Lupopedia files (memory/, channels/, app/, includes/, etc.) live ABOVE the web root.
...
if (defined('LUPOPEDIA_ABSPATH') && LUPOPEDIA_ABSPATH !== '') {
    $lupo_runtime_dirs = array('cache', 'logs', 'uploads', 'tmp');
...
if (!defined('LUPO_APP_DIR')) {
    ...
    if (is_dir($lupo_modern_app_dir)) {
        ...
        define('LUPO_APP_DIR', 'database' . DIRECTORY_SEPARATOR . 'lupopedia' . DIRECTORY_SEPARATOR . 'content' . DIRECTORY_SEPARATOR . 'app');
    }
}
...
$lupo_auth_legacy_dir = LUPOPEDIA_ABSPATH . 'database' . DIRECTORY_SEPARATOR . 'lupopedia' . DIRECTORY_SEPARATOR . 'content' . DIRECTORY_SEPARATOR . 'app' . DIRECTORY_SEPARATOR . 'auth';
$lupo_auth_class_files = array('AuthRoleResolver.php', 'AuthService.php');
foreach ($lupo_auth_class_files as $lupo_auth_class_file) {
    $lupo_auth_primary = $app_auth . DIRECTORY_SEPARATOR . $lupo_auth_class_file;
    if (file_exists($lupo_auth_primary)) {
        require_once $lupo_auth_primary;
    } elseif (file_exists($lupo_auth_legacy_dir . DIRECTORY_SEPARATOR . $lupo_auth_class_file)) {
        require_once $lupo_auth_legacy_dir . DIRECTORY_SEPARATOR . $lupo_auth_class_file;
    }
}
...
} elseif (class_exists('App\Auth\AuthRoleResolver') && class_exists('App\Auth\AuthService') && isset($GLOBALS['mydatabase'])) {
    // Fallback: Use auth services without Session class
    $lupo_auth_role_resolver = new \App\Auth\AuthRoleResolver($GLOBALS['mydatabase']);
    $GLOBALS['lupo_auth_service'] = new \App\Auth\AuthService(null, $GLOBALS['mydatabase'], $lupo_auth_role_resolver);
}
    ...
    try {
        ...
        if ($actor_id === false) {
            ...
            if ($created && isset($created->session_id)) {
                // Session::create() already handled session_start and binding.
                // Just re-validate to update the $lupo_session instance state.
                $actor_id = $lupo_session->validateSession();
                if (defined('LUPOPEDIA_DEBUG') && LUPOPEDIA_DEBUG) {
