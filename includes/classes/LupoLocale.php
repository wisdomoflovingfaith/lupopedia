...
/**
 * UI locale: session-backed, PHP array catalogs under includes/lang/{locale}.php
...
class LupoLocale
{
     ...
     *
     * @param string $appRoot Lupopedia install root (directory containing includes)
     */
    ...
    public static function load($appRoot, $locale)
    {
        ...
        $path = $root . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'lang' . DIRECTORY_SEPARATOR . '' . $locale . '.php';
        self::$strings = array();
        if (is_file($path)) {
            $loaded = include $path;
            if (is_array($loaded)) {
