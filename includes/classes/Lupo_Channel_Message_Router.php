<?php
/**
 * Channel message router: broadcast, direct, thread — DB insert + channels artifact.
 *
 * @package Lupopedia
 * @since   4.0.80
 */
...
class Lupo_Channel_Message_Router
{
    ...
    public function __construct($db, $tablePrefix)
    {
        ...
        $this->channelsBase = $root . DIRECTORY_SEPARATOR . 'channels';
    }
    ...
    private function insertAndArtifact($channel_id, $from_actor_id, $message_text, $message_type, $routing, $to_actor_id, $thread_id, $metadata_json)
    {
        ...
        $rel = 'channels/' . $channel_id . '/' . str_replace(DIRECTORY_SEPARATOR, '/', $subdir) . '/' . $filename;
    ...
    public function generateChannelArtifact($channel_id, $subdirectory, $filename, $content, $metadata)
    {
        ...
        $yaml .= "file_path_from_root: " . json_encode(str_replace('\\', '/', 'channels/' . (int) $channel_id . '/' . str_replace(DIRECTORY_SEPARATOR, '/', $subdirectory) . '/' . $filename)) . "\n";
        $yaml .= "---\n\n";
