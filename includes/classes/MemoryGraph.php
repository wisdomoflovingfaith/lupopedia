 ...
 * no union types — per PRD 00 §4 / rules/root/php-7-4-compatibility.md).
 */
...
class MemoryGraph {
     ...
     *   2. Most-recent .toon in memory/YYYY/MM/ (current month)
     *   3. getChannelContext() — never returns empty array
     *
     * @param string|null $startRef  Path like "memory/2026/04/M-foo.toon",
    ...
    public function setActiveMemoryRef($memRef) {
        ...
        if ($ck && $sl) {
            $configPath = $this->projectRoot . '/channels/0/' . $ck . '/' . $sl . '/config.json';
        ...
        // Update session.json
        $sessionPath = $this->projectRoot . '/config/session.json';
    ...
    /**
     * Scan memory/ recursively for a .toon whose filename matches $id.
     */
    private function findToonById($id) {
        $memDir = $this->projectRoot . '/memory';
        if (!is_dir($memDir)) {
            return null;
        }
    ...
    /**
     * Find the most-recently modified .toon in memory/YYYY/MM/ (current month first,
     * then any month in current year, then any month).
     *
     * @return string|null  Project-relative path like "memory/2026/04/M-foo.toon"
     */
    private function findLatestToon() {
        $memoryDir = $this->projectRoot . '/memory';
        if (!is_dir($memoryDir)) {
            return null;
        }
    ...
    private function getChannelContext() {
        ...
        return array(
            ...
            'instructions'   => array(
                'Generate memory companions: python scripts/generate_json_headers.py --all',
                'Migrate transcripts:        python scripts/migrate_transcript_to_memory.py --all',
                'Set active ref via PHP:     MemoryGraph::setActiveMemoryRef("memory/YYYY/MM/M-foo.toon")',
                'Set active ref via Python:  python bin/tick.py --channel_key dev --slug my/slug',
            ),
        );
    }
    ...
    private function getSession() {
        ...
        $path = $this->projectRoot . '/config/session.json';
        if (file_exists($path)) {
    ...
    private function channelConfigPath($session) {
        ...
        return $this->projectRoot . '/channels/0/' . $ck . '/' . $sl . '/config.json';
    }
