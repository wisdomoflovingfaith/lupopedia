...
lupopedia.headers:
  ...
  file_path_from_root: "includes/classes/ToonSchemaCache.php"
  web_path: "http://www.lupopedia.com/lupopedia/includes/classes/ToonSchemaCache.php"
  questions_toon: null
  ...
  artifact_type: "class"
  artifact_kind: "code"
  purpose: "Schema reference JSON cache for bounded authority (canonical database/lupopedia/json); legacy class name retained for callers"
 ...
 *
 * @deprecated New work MUST read canonical files under database/lupopedia/json/
 *             (e.g. lupo_metadata.json). Legacy `.toon` / `.toon.json` trees are deprecated
 *             for new work per PRD 00 section 6. Table docs: docs/database/lupopedia/tables/active/.
...
class ToonSchemaCache
{
    ...
    public function __construct($preferredSchemaDir = null)
    {
        if (defined('LUPOPEDIA_DEBUG') && LUPOPEDIA_DEBUG) {
            error_log('DEPRECATED: ToonSchemaCache used – migrate to JSON schema files under database/lupopedia/json/. This class will be removed in 4.1.0.');
        }
    ...
    /**
     * Repository root: parent of includes.
     *
     * @return string
     */
    ...
    private function resolveJsonPath($passedDir, $tableName)
    {
        ...
        $candidates[] = self::repoRoot() . DIRECTORY_SEPARATOR . 'database' . DIRECTORY_SEPARATOR
