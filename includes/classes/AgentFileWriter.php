 ...
 *
 * Policy: see docs/versions/4.0.89/TODO.md H9 and docs/ORGANIZATION.md §2.2.
...
class AgentFileWriter
{
    ...
    public static function assertPathAllowed($path, $repoRoot = null)
    {
        ...
        if (!self::isAllowedDirectoryPrefix($rel)) {
            throw new LupoAgentFileWriterException('Path is not under an allowed directory prefix (rules, docs, channels, content): ' . $rel);
        }
    ...
    private static function auditLog($relPath, $byteLen, $context, $actorId)
    {
        ...
        $logDir = rtrim($base, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . 'logs';
