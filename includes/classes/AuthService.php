...
lupopedia.headers:
  ...
  file_path_from_root: "includes/classes/AuthService.php"
  web_path: "http://www.lupopedia.com/lupopedia/includes/classes/AuthService.php"
  questions_toon: null
...
class AuthService
{
    ...
    public function handleLogin($username, $password, $redirect = null)
    {
        ...
        error_log("SESSION_DEBUG: handleLogin after authenticate; auth_user=" . json_encode($auth_user));

        if (!$auth_user) {
            error_log("SESSION_DEBUG: handleLogin - Invalid credentials for username=$username");
        ...
        error_log("SESSION_DEBUG: handleLogin - Before getActorForAuthUser for auth_user_id=" . $auth_user['auth_user_id']);
        $existing_actor = $sessionManager->getActorForAuthUser($auth_user['auth_user_id']);
        error_log("SESSION_DEBUG: handleLogin - After getActorForAuthUser; existing_actor=" . json_encode($existing_actor));
        if ($existing_actor) {
            ...
            $this->ensureAdminRolesAfterLogin((int) $existing_actor['actor_id'], (int) $auth_user['auth_user_id']);
            ...
            if ($needs_password_change) {
                ...
                error_log("SESSION_DEBUG: handleLogin - Returning needs_password_change for actor_id=" . $existing_actor['actor_id']);
            ...
            error_log("SESSION_DEBUG: handleLogin - Returning success for actor_id=" . $existing_actor['actor_id']);
        ...
        if (!$pendingSession) {
            error_log("SESSION_DEBUG: handleLogin - Could not start pending session for auth_user_id=" . $auth_user['auth_user_id']);
            return array('error' => 'Could not start session');
        }
        ...
        error_log("SESSION_DEBUG: handleLogin - Returning needs_agent_selection for auth_user_id=" . $auth_user['auth_user_id']);
    ...
    /**
     * Grant bootstrap admin roles when the install has no admins yet (first operator login).
     *
     * @param int $actor_id
     * @return void
     */
    private function ensureAdminRolesAfterLogin($actor_id, $auth_user_id)
    {
        ...
        $legacy_resolver = defined('LUPOPEDIA_PATH')
            ? LUPOPEDIA_PATH . '/database/lupopedia/content/app/auth/AuthRoleResolver.php'
            : '';
        if (!class_exists('App\\Auth\\AuthRoleResolver', false) && $legacy_resolver !== '' && is_file($legacy_resolver)) {
            require_once $legacy_resolver;
        }
        if (!class_exists('App\\Auth\\AuthRoleResolver', false)) {
            return;
        }
        $resolver = new \App\Auth\AuthRoleResolver($this->db);
        if ($auth_user_id === 10000) {
            $resolver->repairMainAdminAuthUserRouting($auth_user_id);
        }
        $resolver->ensureBootstrapAdminForActor($actor_id);
        if ($auth_user_id === 10000) {
            $resolver->ensureReservedMainAdminCapabilities($actor_id);
        }
    }
    ...
    public function getCurrentUser()
    {
        ...
        // 1. Try to resolve via the modern mapping table (supports hybrid/system actors like WOLFIE)
        $sql = "SELECT a.actor_id, a.actor_name, aau.auth_user_id, 
                ...
                INNER JOIN {$this->table_prefix}auth_users au ON aau.auth_user_id = au.auth_user_id
        ...
        if (!$user) {
            // 2. Fallback for older actors without a mapping but with direct source link
