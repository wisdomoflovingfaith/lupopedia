...
#   file_path_from_root: "includes/classes/TimestampYmdhis.php"
#   web_path: "https://www.lupopedia.com/lupopedia/includes/classes/TimestampYmdhis.php"
#   status: "active"
#   when_updated: "20260420080000"
#   trust_tier: "canonical"
#   questions_toon: null
#   memory_toon: "memory/development/canonical/1026/04/timestamp-ymdhis.toon"
...
#     interval(int $start, int $end)        Build interval array.
#     overlaps(array $a, array $b)          Ranges overlap.
#     intersection(array $a, array $b)      Overlap range or null.
#     shift(array $interval, int $seconds)  Move both ends by seconds.
#     expand(array $interval, int $seconds) Grow symmetrically (start earlier, end later).
#
#   Internal
#     dateTimeFromPacked(int $ts)           private — UTC DateTime from packed, or false.
#
# ---------------------------------------------------------------------
...
class timestamp_ymdhis
{
    ...
    public static function diffInMinutes(int $a, int $b): int
    {
        $da = self::dateTimeFromPacked($a);
        $db = self::dateTimeFromPacked($b);
        if ($da === false || $db === false) {
        ...
        $da->setTime((int)$da->format('H'), (int)$da->format('i'), 0);
        $db->setTime((int)$db->format('H'), (int)$db->format('i'), 0);

        $iv = $db->diff($da);
        $days = $iv->days;
        if ($days === false) {
            return (int) floor( ($da->getTimestamp() - $db->getTimestamp()) / 60 );
        }

        $min = ($days * 1440) + ($iv->h * 60) + $iv->i;
        return $iv->invert ? -$min : $min;
    }
