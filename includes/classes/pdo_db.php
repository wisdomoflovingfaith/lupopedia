...
class PDO_DB
{
    ...
    public function update($table, $data, $where, $whereParams = array())
    {
        ...
        // SET clauses use positional ?. If WHERE also uses named params (:name),
        // PDO rejects the mixed statement. Convert any named WHERE params to
        // positional ? in the order they appear so everything is consistent.
        $whereValues = array();
        // Named WHERE placeholders must become positional ? to match SET clauses (? only).
        // Do not require non-empty whereParams: an omitted fourth arg would skip conversion and
        // trigger SQLSTATE[HY093] (mixed named and positional) on emulated/native PDO.
        if (preg_match('/:([a-zA-Z_][a-zA-Z0-9_]*)/', $where)) {
            $where = preg_replace_callback(
                '/:([a-zA-Z_][a-zA-Z0-9_]*)/',
                function ($m) use ($whereParams, &$whereValues) {
                    $key = $m[1];
                    $bare = ltrim($key, ':');
                    if (array_key_exists($bare, $whereParams)) {
                        $whereValues[] = $whereParams[$bare];
                    } elseif (array_key_exists(':' . $bare, $whereParams)) {
                        $whereValues[] = $whereParams[':' . $bare];
                    } else {
                        $whereValues[] = null;
                    }
