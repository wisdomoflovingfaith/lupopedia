 ...
 *
 * Whitelist segments, require file under emoji/, realpath containment (blocks traversal/symlink escape).
...
class LupoEmojiPath
{
    ...
    /**
     * Resolve an existing file under LUPOPEDIA_PATH/emoji/{folder}/{file}.
     *
     ...
     * Re-resolve paths on read so tampered DB rows cannot point outside emoji/.
     *
        ...
        while ($offset < $len) {
            ...
            if (count($parts) === 2
                ...
                if ($abs !== null && defined('LUPOPEDIA_PUBLIC_PATH')) {
                    ...
                    $url = ($pub === '' ? '' : $pub) . '/emoji/' . $folder . '/' . $file;
                    $safeUrl = htmlspecialchars($url, ENT_QUOTES, 'UTF-8');
                    $alt = htmlspecialchars($file, ENT_QUOTES, 'UTF-8');
                    $out .= '<img src="' . $safeUrl . '" alt="' . $alt . '" class="emoji-inline" />';
                } else {
    ...
    /**
     * @param string $baseReal realpath of emoji directory
