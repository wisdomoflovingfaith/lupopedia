...
def discover_sidecar(repo, stem):
    candidates = sorted((repo / "memory" / "headers" / "prd").glob(f"**/{stem}.metadata.json"))
    if not candidates:
        return None
    return candidates[-1]
...
def resolve_toon_path(repo, prd_file):
    ...
    if prd_file.name == "00_root_constitutional_system_requirements.md":
        return repo / "memory/constitutional/seed/prd-00-constitutional.toon"
    ...
    if prd_file.name == "41_captain_wolfie_identity.md":
        matches = sorted((repo / "memory/development/seed").glob("**/41-captain-wolfie-identity.toon"))
        if matches:
            return matches[-1]
        return repo / "memory/development/seed/2026/04/41-captain-wolfie-identity.toon"

    # All other PRDs follow canonical NN-slug paths.
    matches = sorted((repo / "memory/development/canonical").glob(f"**/{slug}.toon"))
    if matches:
        return matches[-1]
    # Fallback for historical slug variants (e.g. shortened slugs).
    prefix_matches = sorted((repo / "memory/development/canonical").glob(f"**/{prd_num}-*.toon"))
    if len(prefix_matches) == 1:
        return prefix_matches[0]
    return repo / f"memory/development/canonical/2026/04/{slug}.toon"
...
def main():
    ...
    prd_files = sorted((repo / "docs" / "prd").glob("[0-9][0-9]_*.md"))
