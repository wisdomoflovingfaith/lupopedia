...
#   file_path_from_root: "scripts/generate_headers_from_db.py"
#   questions_toon: null
#   channel_id: 42
#   actor_id: 102
#   actor_name: "cursor"
...
- `questions_toon`: Path to .questions.toon Q&A file, or null (PRD 16 v4.0.99 §4.2 field 6).
  NOTE: `last_modified_utc` was renamed to `questions_toon` in PRD 16 v4.0.99 — it is no longer a timestamp.
...
Usage:
    ...
    python scripts/generate_headers_from_db.py --dry-run --file-path path/from/root.md
    python scripts/generate_headers_from_db.py --use-mock-db --file-path path/from/root.md   # offline stub
...
def validate_timestamp_semantic_range(headers: Dict[str, Any]) -> List[str]:
    ...
    The old ordering rule (when_updated <= last_modified_utc) no longer applies.
    If a legacy header still carries `last_modified_utc`, it is validated as a timestamp
    but its value is not compared against `when_updated`.
    """
    issues: List[str] = []
    FLOOR = "20000101000000"
    now   = datetime.utcnow().strftime('%Y%m%d%H%M%S')

    # questions_toon is NOT a timestamp — exclude from timestamp checks.
    # last_modified_utc is kept here only for backward-compat with legacy headers.
    ...
    # Ordering rule removed: when_updated vs last_modified_utc comparison no longer applies
    # (last_modified_utc was renamed to questions_toon which is not a timestamp).
...
def enforce_timestamp_semantics(
    ...
    2. questions_toon: Replaces last_modified_utc (PRD 16 v4.0.99). Set to null if absent; migrate from old field if present.
    ...
    # Rule 2 (v4.0.99+): questions_toon replaces last_modified_utc (PRD 16 §4.2 field 6).
    # Set questions_toon to null if not present; do not set a timestamp here.
    if 'last_modified_utc' in headers_new:
        # Migrate legacy field on the fly
        headers_new['questions_toon'] = None
        del headers_new['last_modified_utc']
        print(
            "NOTICE: last_modified_utc renamed to questions_toon (PRD 16 v4.0.99 §19.5)",
            file=sys.stderr
        )
    elif 'questions_toon' not in headers_new:
        headers_new['questions_toon'] = None
...
class MockDBConnection:
    ...
    def __init__(self):
        ...
        self.contents_dir = Path("database/lupopedia/content")
        self.metadata_dir = Path("database/lupopedia/metadata")
    ...
    def fetch_metadata_rows(self, entity_type: str, entity_id: str = None) -> List[Dict]:
        ...
        return [
            ...
            },
            {
                ...
                'entity_type': entity_type,
                'entity_id': entity_id or 'default',
                'property_key': 'questions_toon',  # was last_modified_utc (renamed PRD 16 v4.0.99)
                'property_value': None,
                'class_name': 'lupopedia_header',
                'created_ymdhis': current_timestamp
            }
...
def build_headers_block(blocks: Dict[str, Dict], content: Dict[str, Any]) -> Dict[str, Any]:
    ...
    when_updated = (
        ...
        # backward compat: legacy last_modified_utc as when_updated fallback (remove after Phase 3)
    ...
    # questions_toon replaces last_modified_utc (PRD 16 v4.0.99 §4.2 field 6)
    headers['questions_toon'] = header_source.get('questions_toon', None)
