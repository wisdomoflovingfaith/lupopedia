...
#   file_path_from_root: "scripts/generate_memory_from_header.py"
#   web_path: "https://www.lupopedia.com/lupopedia/scripts/generate_memory_from_header.py"
#   status: "complete"
#   when_updated: "20260415080730"
#   trust_tier: "canonical"
#   questions_toon: null
#   memory_toon: "memory/development/canonical/1026/04/generate-memory-from-header.toon"
#   atoms_toon: null
...
#   title: "Generate memory sidecar from embedded LUPOPEDIA HEADERS"
#   summary: "Parse memory_toon via universal validator; write header_metadata .json/.toon; --check --batch --dry-run."
# ---------------------------------------------------------------------
"""
Create or verify **memory** sidecars (.json master + .toon mirror) for a source file's
...
Usage:
  python scripts/generate_memory_from_header.py docs/prd/16_lupopedia_headers.md
  python scripts/generate_memory_from_header.py scripts/foo.py --force
  python scripts/generate_memory_from_header.py path/to/file.md --check
  python scripts/generate_memory_from_header.py --batch --dry-run
  python scripts/generate_memory_from_header.py --batch --quiet
"""
...
def _build_sidecar_doc(hdr: dict, source_rel: str) -> dict:
    ...
    if content_id_val is not None:
        outbound.append(
            {
                "to": "contents/content_id/%d" % content_id_val,
                "type": "references",
                "weight": 0.95,
    ...
    if content_parent_id_val is not None:
        pat = os.path.join(
            _REPO_ROOT, "docs", "prd", "%s_*.md" % str(content_parent_id_val).zfill(2)
        )
...
def ensure_memory_files(
    ...
    """
    Create or verify memory .json + .toon for *file_path*'s embedded header.
    ...
    if not quiet:
        print(
            "     Validate: python scripts/validate_lupopedia_headers_universal.py %s --strict-memory-pair"
            % rel_source
        )
    return 0
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate or verify memory .json + .toon for a file's memory_toon."
    )
    parser.add_argument(
        "path",
        nargs="?",
