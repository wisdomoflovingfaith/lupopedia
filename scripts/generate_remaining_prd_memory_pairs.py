...
def build_prd_path_map(prd_dir):
    ...
    for p in sorted(prd_dir.glob("[0-9][0-9]_*.md")):
        ...
        if key not in path_map:
            path_map[key] = rel
    path_map[0] = "docs/prd/00_root_constitutional_system_requirements.md"
    path_map[1] = "docs/prd/01_core_identity.md"
    path_map[38] = "docs/prd/38_memory_unification.md"
    return path_map
...
def build_edges(prd_id, ref_ids, path_map):
    ...
    for i, rid in enumerate(ranked, start=1):
        ...
        target = path_map.get(rid, "docs/prd/{:02d}_unknown.md".format(rid))
...
def main():
    ...
    canonical_dir = repo / "memory" / "development" / "canonical" / year / month
    seed_dir = repo / "memory" / "development" / "seed" / year / month
    ...
    for prd_path in targets:
        ...
        base_no_ext = "memory/development/{}/{}/{}/{}".format(
        ...
        if args.write:
            ...
            if ok_toon:
                ok_validate = run_cmd(
                    [
                        "python",
                        "scripts/validate_memory_json_toon_pair.py",
                        "--base",
                        base_no_ext,
                    ],
                    repo,
