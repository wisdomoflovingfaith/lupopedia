...
# Run from repo root: sh scripts/run_tests.sh [REPO_ROOT]
...
echo "=== Unit tests ==="
if [ -d "scripts" ] && [ -f "scripts/run_unit_tests.sh" ]; then
    if sh scripts/run_unit_tests.sh "$REPO_ROOT"; then
        echo "Unit: PASS"
    else
        echo "Unit: FAIL"
        OVERALL=1
...
if [ -f "scripts/run_regression_tests.sh" ]; then
    if sh scripts/run_regression_tests.sh "$REPO_ROOT"; then
...
if [ -d "tests/integration" ]; then
    for s in tests/integration/*.sh; do
        [ -f "$s" ] || continue
        name=$(basename "$s")
    ...
    echo "Integration: skip (no tests/integration)"
fi

echo ""
echo "=== Adversarial tests (T4) ==="
if [ -d "tests/adversarial" ] && [ -n "$(ls tests/adversarial/*.php 2>/dev/null)" ]; then
    if [ -f "scripts/run_adversarial_tests.sh" ]; then
        sh scripts/run_adversarial_tests.sh "$REPO_ROOT" || OVERALL=1
    else
...
if [ -f "scripts/validate_pseudocode_discipline.py" ] && [ -f "scripts/validate_thread_structure.py" ] && [ -f "scripts/validate_edge_linking.py" ]; then
    PRD17_SCOPE="$REPO_ROOT/docs/versions/4.0.96"
    if python scripts/validate_pseudocode_discipline.py "$PRD17_SCOPE" && \
       python scripts/validate_thread_structure.py "$PRD17_SCOPE" && \
       python scripts/validate_edge_linking.py "$PRD17_SCOPE"; then
...
if [ -f "scripts/validate_trust_ladder_paths.py" ]; then
    python scripts/validate_trust_ladder_paths.py || true
