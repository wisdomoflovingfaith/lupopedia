...
- seed: memory/{channel}/seed/{slug}.toon
- canonical: memory/{channel}/canonical/{actual_year-1000}/{month}/{slug}.toon
- staging: memory/{channel}/staging/{actual_year}/{month}/{slug}.toon
- archive: memory/{channel}/archive/{actual_year}/{month}/{slug}.toon
"""
...
CHANNEL_REGISTRY = REPO_ROOT / "channels" / "registry.json"
...
def compute_memory_key(channel_key: str, trust_tier: str, ts: str, slug: str) -> tuple[str, int, int, int]:
    ...
    if trust_tier == "seed":
        return (
            "memory/{0}/seed/{1}.toon".format(channel_key, slug),
            0,
            actual_year,
            month,
        )
    ...
    return (
        "memory/{0}/{1}/{2}/{3:02d}/{4}.toon".format(
