...
Canonical rules: docs/prd/17_decisions_format.md
...
- Enforces required headers for every *.pseudo.md, *.pseudo.php, *.pseudo.txt
  ...
  - file_path_from_root
  - when_updated
  - questions_toon (replaces last_modified_utc, per PRD 16 v4.0.99 §4.2 field 6)
...
def classify(path):
    ...
    if name.endswith("_constitution.pseudo.md"):
        return "purpose1"
    if "/docs/decisions/pseudocode/" in s or s.endswith("docs/decisions/pseudocode/" + name):
        return "purpose1"
...
def _required_header_violations(path, text):
    ...
    # Accept questions_toon (v4.0.99+) or last_modified_utc (legacy pre-v4.0.99)
    if not re.search(r"(?m)^\s*questions_toon\s*:", text) and \
       not re.search(r"(?m)^\s*last_modified_utc:\s*['\"]?[0-9]{8,14}", text):
        issues.append("missing questions_toon (or legacy last_modified_utc) in lupopedia.headers")
    if issues:
