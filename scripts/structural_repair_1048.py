...
#   file_path_from_root: "scripts/structural_repair_1048.py"
#   questions_toon: null
#   channel_id: 42
#   actor_id: 102
#   actor_name: "cursor"
...
MD_GLOB_EXCLUDES = {
    ".git", "node_modules", "vendor", "legacy"
}
...
def build_thread_coverage_lines(threads):
    ...
    lines = []
    lines.append("- Source: live filesystem scan under channels/*/threads/[0-9]+")
...
def fix_headers():
    ...
    for f in iter_markdown_files(ROOT):
        ...
        if "docs/versions/4.0.85/" in rel and VW_RE.search(text):
            def _vw_repl(m):
...
def rebuild_task_registry(threads):
    reg_path = ROOT / "docs" / "versions" / VERSION / "TASK_REGISTRY.md"
    ...
    lines.append('  file_path_from_root: "docs/versions/4.0.85/TASK_REGISTRY.md"')
    ...
    lines.append('    - { to: "docs/versions/4.0.85/TODO.md", type: "depends_on" }')
    lines.append('    - { to: "docs/versions/4.0.85/PLAN.md", type: "depends_on" }')
...
def create_completion_artifact(threads_detected, registry_entries, broken_before, broken_after, header_before, header_after):
    out = ROOT / "channels" / "42" / "threads" / "1048" / f"{UTC_NOW}_thoth_structural_repair_completion.md"
