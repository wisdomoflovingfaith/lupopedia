...
#   file_path_from_root: "scripts/import_filesystem_actors_agents_to_db.py"
#   questions_toon: null
#   channel_id: 42
#   actor_id: 102
#   actor_name: "cursor"
...
FILESYSTEM SOURCES (scanned in priority order)
  1. database/lupopedia/actors/actor_id/registry.json
     Canonical actor_id → { id, type, slug, dir } mapping.
  2. agents/{id}/agent.json         — agent code/name/layer/role/description
  3. agents/{id}/properties.json    — actor_id, slug, type, role, extra props
  4. agents/{id}/capabilities.json  — list of capability strings
  5. agents/{id}/system_prompt.txt  — system prompt text
  6. actors/{id}/agent.json         — supplementary actor definition
  7. actors/{id}/properties.json    — supplementary properties
  8. actors/{id}/capabilities.json  — supplementary capabilities
...
USAGE
  python scripts/import_filesystem_actors_agents_to_db.py --repo-root .
  python scripts/import_filesystem_actors_agents_to_db.py --repo-root . --dry-run
  python scripts/import_filesystem_actors_agents_to_db.py --repo-root . --strict
  python scripts/import_filesystem_actors_agents_to_db.py --repo-root . --verbose
  python scripts/import_filesystem_actors_agents_to_db.py --repo-root . --actor-id 1
  python scripts/import_filesystem_actors_agents_to_db.py --repo-root . --actor-type agent
...
def load_actor_from_agent_dir(
    ...
    """
    Load actor definition from agents/{id}/ directory.
    Returns an ActorDef with all available fields populated.
    """
    context = f"agents/{agent_dir.name}"
...
def supplement_from_actors_dir(
    ...
    """
    Supplement actor_def with data from actors/{id}/ or actors/{slug}/.
    ...
    for candidate in candidates:
        if not candidate.is_dir():
            continue
        context = f"actors/{candidate.name}"
...
def load_registry(
    ...
    """
    registry_path = repo_root / "database" / "lupopedia" / "actors" / "actor_id" / "registry.json"
...
def discover_actors(
    ...
    Priority: registry.json → agents/{id}/ → actors/{id|slug}/
    """
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    db.add_argument("--host", default=None, help="Override DB host")
    db.add_argument("--port", default=None, type=int, help="Override DB port")
    db.add_argument("--user", default=None, help="Override DB user")
    db.add_argument("--password", default=None, help="Override DB password")
    db.add_argument("--database", default=None, help="Override DB name")
    db.add_argument("--table-prefix", default=None, help="Override table prefix")
    return p
...
def main() -> int:
    ...
    if conn is not None and args.verbose:
        ...
        m = merge_connection_params_with_args(args)
        print("DB: %s@%s:%s/%s" % (m["user"], m["host"], m["port"], m["database"]))
