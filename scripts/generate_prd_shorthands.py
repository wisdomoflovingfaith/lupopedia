...
Usage:
    python scripts/generate_prd_shorthands.py --prd 38
    python scripts/generate_prd_shorthands.py --stem 04_tags_metadata
    python scripts/generate_prd_shorthands.py --all
    python scripts/generate_prd_shorthands.py --check
    python scripts/generate_prd_shorthands.py --all --force
...
try:
    ...
    sys.stderr.write("PyYAML required: pip install -r scripts/requirements.txt\n")
    sys.exit(1)
...
lupopedia.headers:
  ...
  file_path_from_root: "docs/prd/decisions/pseudocode/%s"
  web_path: "http://www.lupopedia.com/lupopedia/docs/prd/decisions/pseudocode/%s"
  when_updated: "%s"
  questions_toon: null
...
|-----|------------------------------|-------------|
| **Runtime / staging** | **2000–2099** | `memory/{YYYY}/MM/` |
| **Living canonical / archive** | **1000–1999** | `memory/{YYYY}/MM/` |
...
def is_stale(prd_path: Path, shorthand_path: Path, prd_header: Dict) -> bool:
    ...
    if not shorthand_path.exists():
        return True
    # questions_toon replaced last_modified_utc in PRD 16 v4.0.99; fallback kept for legacy headers
...
def update_thread_index(rows: List[PrdRow]) -> None:
    ...
    lines = [
        ...
        "lupopedia.headers:",
        "  lupopedia.schema: documentation",
        "  file_path_from_root: docs/prd/decisions/pseudocode/THREAD_INDEX.md",
        '  when_updated: "%s"' % utc,
...
def main() -> None:
    ...
    parser.add_argument(
        "--stem",
        metavar="STEM",
        help="Only `docs/prd/{STEM}.md` (e.g. 04_tags_metadata when two PRDs share NN)",
    )
