#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# DRAFT — scripts/migrate_transcript_to_memory.py
# Read existing transcript.jsonl files in channels/
# For each entry, create a .toon memory file
# Optionally rewrite transcript entries as compacted stubs (--compact)
#
# Usage:
#   python scripts/migrate_transcript_to_memory.py --scan
#   python scripts/migrate_transcript_to_memory.py --file channels/0/development/prd_files/44_prd_discussion/transcript.jsonl
#   python scripts/migrate_transcript_to_memory.py --all
#   python scripts/migrate_transcript_to_memory.py --all --compact --dry-run
#   python scripts/migrate_transcript_to_memory.py --all --compact  (auto-backs up first)
...
def parse_channel_parts(channel_path: str) -> tuple:
    ...
    # Expected: channels / 0 / {channel_key} / {slug...}
    try:
        idx = list(parts).index("channels")
...
def export_memory_node(node_id: int) -> bool:
    ...
    cmd = ["php", "bin/export.php", "--node-id", str(node_id)]
...
def main() -> None:
    ...
    parser.add_argument("--all", action="store_true", help="Process all transcript.jsonl files in channels/")
