...
#   file_path_from_root: "scripts/export_db_actors_agents_to_filesystem.py"
#   questions_toon: null
#   channel_id: 42
#   actor_id: 102
#   actor_name: "cursor"
...
FILESYSTEM TARGETS WRITTEN
  agents/{actor_id}/agent.json
  agents/{actor_id}/properties.json
  agents/{actor_id}/capabilities.json
  agents/{actor_id}/system_prompt.txt   (only if non-empty in DB)
  database/lupopedia/actors/actor_id/registry.json   (canonical registry)

DOCTRINE CONSTRAINTS
  - No foreign keys / triggers / procedures at DB level
  - Timestamps are BIGINT YYYYMMDDHHIISS (UTC)
  - Does NOT write to actors/ tree (that tree is actor-specific content,
    not auto-generated from DB data)
  - Does NOT delete any filesystem file — only creates or overwrites

USAGE
  python scripts/export_db_actors_agents_to_filesystem.py --repo-root .
  python scripts/export_db_actors_agents_to_filesystem.py --repo-root . --dry-run
  python scripts/export_db_actors_agents_to_filesystem.py --repo-root . --actor-id 1
  python scripts/export_db_actors_agents_to_filesystem.py --repo-root . --verbose
  python scripts/export_db_actors_agents_to_filesystem.py --repo-root . --strict
...
def export_actor(
    ...
    """Export one actor to agents/{actor_id}/."""
...
def export_registry(
    ...
    """
    Regenerate database/lupopedia/actors/actor_id/registry.json from DB actors.
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    db = p.add_argument_group(
        "Database connection (defaults from lupopedia-config.php; override with flags)"
    )
    ...
    db.add_argument(
        "--table-prefix",
        default=None,
        help="Table prefix (default: from lupopedia-config.php)",
    )
    return p
