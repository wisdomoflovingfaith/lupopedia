...
def build_yaml_data_from_db(
    ...
    # PRD 16 v4.0.99: last_modified_utc renamed to questions_toon (field 6).
    # questions_toon is a path string or null — not a timestamp. Do not populate from DB timestamp.
    # Migrate legacy field on the fly if still present in the header.
    if "last_modified_utc" in headers:
        headers["questions_toon"] = headers.get("questions_toon", None)
        del headers["last_modified_utc"]
    elif "questions_toon" not in headers:
        headers["questions_toon"] = None
