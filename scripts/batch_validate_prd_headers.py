...
#   file_path_from_root: "scripts/batch_validate_prd_headers.py"
#   web_path: "https://www.lupopedia.com/lupopedia/scripts/batch_validate_prd_headers.py"
#   status: "complete"
#   when_updated: "20260411043037"
#   trust_tier: "canonical"
#   questions_toon: null
#   memory_toon: "memory/development/canonical/1026/04/batch-validate-prd-headers.toon"
#   atoms_toon: null
...
(docs/prd/[0-9][0-9]_*.md), on all docs/**/*.md (--all-md), and optionally
on scripts/**/*.py (--include-py).
...
Usage (from repo root):
  ...
  python scripts/batch_validate_prd_headers.py --check-links
  python scripts/batch_validate_prd_headers.py --check-db
  python scripts/batch_validate_prd_headers.py --all-md --dry-run-list
  python scripts/batch_validate_prd_headers.py --all-md --verbose --timeout 120
  python scripts/batch_validate_prd_headers.py --development
  python scripts/batch_validate_prd_headers.py --strict-memory-pair
  python scripts/batch_validate_prd_headers.py --strict
  python scripts/batch_validate_prd_headers.py --include-py --jobs 4
  python scripts/batch_validate_prd_headers.py --extensions .md,.pseudo.md --all-md
  python scripts/batch_validate_prd_headers.py --report logs/header-batch-report.json
  python scripts/batch_validate_prd_headers.py --all-md --migrate-legacy
  python scripts/batch_validate_prd_headers.py --all-md --strict-header
  python scripts/batch_validate_prd_headers.py path/to/file.php --format php
"""
...
def _collect_files(
    ...
    for ext in extensions:
        if all_md:
            pattern = str(root / "docs" / "**" / ("*" + ext))
            add_paths(glob.glob(pattern, recursive=True))
        else:
            pattern = str(root / "docs" / "prd" / ("[0-9][0-9]_*" + ext))
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "(default), or on all docs/**/* with matching extensions (--all-md), "
            "and optionally scripts/**/*.py (--include-py)."
        )
    )
    parser.add_argument(
        "--check-links",
    ...
    parser.add_argument(
        "--all-md",
        action="store_true",
        help="Validate under docs/ recursively (extensions from --extensions), not only numbered PRDs",
    )
    parser.add_argument(
        "--include-py",
        action="store_true",
        help="Also validate every *.py under scripts/ (recursive)",
    )
    parser.add_argument(
        "--format",
        dest="validator_type",
    ...
    validator = root / "scripts" / "validate_lupopedia_headers_universal.py"
    ...
    if args.all_md:
        scope_parts.append("all docs *%s" % ",".join(extensions))
    else:
        scope_parts.append("numbered PRDs *%s" % ",".join(extensions))
    if args.include_py:
        scope_parts.append("scripts *.py")
