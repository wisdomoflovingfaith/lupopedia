...
LOG_FILE="$PROJECT_ROOT/logs/admin/channel66_deployment_extended_$(date +%Y%m%d_%H%M%S).log"
CONFIG_FILE="$PROJECT_ROOT/config/channel66_production_extended_$ENVIRONMENT.ini"
BACKUP_DIR="$PROJECT_ROOT/backups/channel66_production_extended/$(date +%Y%m%d)"
...
check_prerequisites() {
    ...
    local required_files=(
        ...
        "$PROJECT_ROOT/includes/classes/Channel66HeaderProjection.php"
        "$PROJECT_ROOT/tests/integration/channel66_production_test.php"
        "$PROJECT_ROOT/tests/integration/channel66_production_extended_test.php"
    )
...
validate_environment() {
    ...
    case $ENVIRONMENT in
        dev)
            if [[ ! -d "$PROJECT_ROOT/tests/temp/channel66_dev" ]]; then
                error_exit "Development environment directory not found: $PROJECT_ROOT/tests/temp/channel66_dev"
            fi
            ;;
        staging)
            if [[ ! -d "$PROJECT_ROOT/tests/temp/channel66_staging" ]]; then
                error_exit "Staging environment directory not found: $PROJECT_ROOT/tests/temp/channel66_staging"
            fi
            ;;
        production)
            if [[ ! -d "$PROJECT_ROOT/channels/66" ]]; then
                error_exit "Production Channel 66 directory not found: $PROJECT_ROOT/channels/66"
            fi
            ;;
        *)
...
backup_deployment() {
    ...
    if [[ -f "$PROJECT_ROOT/config/channel66_production_extended_$ENVIRONMENT.ini" ]]; then
        cp "$PROJECT_ROOT/config/channel66_production_extended_$ENVIRONMENT.ini" "$BACKUP_DIR/"
        info "Configuration backed up"
    fi
    
    # Backup logs (last 7 days)
    find "$PROJECT_ROOT/logs/admin" -name "channel66_production_extended_*.jsonl" -mtime -7 -exec cp {} "$BACKUP_DIR/" \;
    local log_count=$(find "$PROJECT_ROOT/logs/admin" -name "channel66_production_extended_*.jsonl" -mtime -7 | wc -l)
    ...
    if [[ -d "$PROJECT_ROOT/database/lupopedia/toon" ]]; then
        cp -r "$PROJECT_ROOT/database/lupopedia/toon" "$BACKUP_DIR/toon_backup"
...
run_extended_tests() {
    ...
    local test_dir="$PROJECT_ROOT/tests/temp/channel66_extended_tests"
    mkdir -p "$test_dir"
    
    # Run the extended test suite
    php "$PROJECT_ROOT/tests/integration/channel66_production_extended_test.php" \
        --test-dir="$test_dir" \
...
deploy_production() {
    if [[ "$DRY_RUN" == "true" ]]; then
        ...
        info "Scope root: $PROJECT_ROOT/channels/66"
        return 0
    fi
    ...
    local test_results_dir="$PROJECT_ROOT/tests/temp/channel66_validation_test"
...
EOF
    ...
    if php "$PROJECT_ROOT/scripts/ingest_channel66_production.php" \
        --config="$CONFIG_FILE" \
        --thread-id=null; then
...
health_check() {
    ...
    local recent_logs=$(find "$PROJECT_ROOT/logs/admin" -name "channel66_production_extended_*.jsonl" -mtime -1 | wc -l)
...
cleanup() {
    ...
    rm -rf "$PROJECT_ROOT/tests/temp/channel66_extended_tests"
    rm -rf "$PROJECT_ROOT/tests/temp/channel66_validation_test"
...
rollback() {
    ...
    if [[ -f "$BACKUP_DIR/channel66_production_extended_$ENVIRONMENT.ini" ]]; then
        cp "$BACKUP_DIR/channel66_production_extended_$ENVIRONMENT.ini" "$PROJECT_ROOT/config/channel66_production_extended.ini"
