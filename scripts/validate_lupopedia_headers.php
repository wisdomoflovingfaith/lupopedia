...
function validate_lupopedia_headers($path)
{
    ...
    if (!class_exists('HeaderDbSync', false)) {
        ...
        $classFile = $root . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'classes' . DIRECTORY_SEPARATOR . 'HeaderDbSync.php';
...
if (php_sapi_name() === 'cli' && isset($argv) && isset($argv[1]) && $argv[1] !== '--check-db') {
    ...
    $hdrPath = $base . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'classes' . DIRECTORY_SEPARATOR . 'HeaderDbSync.php';
    if (!is_file($hdrPath)) {
