...
#   file_path_from_root: "scripts/validate_vproject.py"
#   questions_toon: null
#   channel_id: 42
#   actor_id: 102
#   actor_name: "cursor"
...
Spec: channels/42/threads/1019/20260318_163836_athena_spec_task_val_004_project-validation.md

Output: ERROR lines -> stdout, WARN -> stderr, INFO -> stdout.
Summary: validate_vproject: X error(s), Y warning(s)

Usage:
  python scripts/validate_vproject.py --repo-root .
  python scripts/validate_vproject.py --strict
  python scripts/validate_vproject.py --allowlist path/to/extra_allowlist.txt
  python scripts/validate_vproject.py --warnings-as-errors
  python scripts/validate_vproject.py --ignore-upstream-fail --report-staging
"""
...
def list_numeric_thread_dirs(repo: Path) -> list[str]:
    base = repo / "channels" / str(CHANNEL) / "threads"
    if not base.is_dir():
        return []
    names = []
...
def run_upstream_todo_plan(repo: Path, ignore_fail: bool) -> int:
    script = repo / "scripts" / "validate_todo_plan.py"
...
def run_vthread(repo: Path, thread_ids: list[str]) -> int:
    ...
    script = repo / "scripts" / "validate_threads.py"
    if not script.is_file():
...
def main() -> int:
    ...
    threads_root = repo / "channels" / str(CHANNEL) / "threads"
