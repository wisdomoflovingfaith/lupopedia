...
Scans Markdown fragments under ``docs/versions/<VERSION>/buffer/``, sorts by a
``## [YYYY-MM-DD HH:MM UTC]`` anchor, appends to that version's CHANGELOG.md, and archives.

**JSON buffer (mandatory for IDE agents):** use ``changelog-pending/*.json`` per
CHANGELOG_BUFFER_ARCHITECTURE.md and run::

  python scripts/consolidate_lupo_changelog_pending.py --commit

That script appends to ``docs/versions/4.1.3/changelog.md`` by default and moves merged
JSON into ``changelog-archive/``. This file remains for the older per-version ``buffer/``
...
VERSION = "4.1.2"
CHANGELOG_PATH = f"docs/versions/{VERSION}/CHANGELOG.md"
BUFFER_DIR = f"docs/versions/{VERSION}/buffer"
