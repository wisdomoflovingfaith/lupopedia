#!/usr/bin/env python3
"""
PRD 16 §15 — E2E gate helpers: JSONL deltas under channels/ and offline queue.

Usage:
  python scripts/e2e_prd16_gates.py snapshot --out baseline.json
  # ... run web / IDE test window ...
  python scripts/e2e_prd16_gates.py assert --baseline baseline.json
...
CHANNELS_GLOB_ROOT = REPO_ROOT / "channels"
OFFLINE_QUEUE = REPO_ROOT / "config" / "offline_transcript_queue.jsonl"
...
def _assert_no_delta(before: Dict[str, Any], after: Dict[str, Any]) -> List[str]:
    ...
    if new_files:
        errs.append("§15.9 FAIL: new .jsonl under channels: %s" % ", ".join(new_files))
...
def main() -> None:
    ...
    p_hint = sub.add_parser("sidecar-tmp-scan", help="List memory/**/*.metadata.json.tmp")
    ...
    if args.cmd == "snapshot":
        ...
        if tmp:
            ...
            for t in tmp:
                print("  ", t)
            sys.exit(2)
        print("[OK] no *.metadata.json.tmp under memory/")
