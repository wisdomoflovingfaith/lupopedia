...
class UserMappingMigrator
{
    ...
    {
        $stmt = $this->db->query("SELECT auth_user_id AS id, email, display_name AS name, username FROM lupo_auth_users WHERE is_active = 1 AND (is_deleted = 0 OR is_deleted IS NULL) ORDER BY email");
