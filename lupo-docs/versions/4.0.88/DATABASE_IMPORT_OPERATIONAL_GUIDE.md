...
lupopedia.headers:
  ...
  file_path_from_root: "lupo-docs/versions/4.0.88/DATABASE_IMPORT_OPERATIONAL_GUIDE.md"
  ...
  purpose: "Operational guide for hardened, journal-driven database import system (ATHENA DIRECTIVE 4.0.88)"
  mood_rgb: "4B0082"
  traits: ["operational", "athena", "database_import", "event_sourcing", "4.0.88"]
  tags: ["database_import", "journal", "event_sourcing", "operational", "v4.0.88"]

lupopedia.edges:
  outbound_edges:
    - { to: "lupo-scripts/generate_content_id.py", type: "references", weight: 1.0 }
    - { to: "lupo-scripts/event_journal_writer.py", type: "references", weight: 1.0 }
    - { to: "lupo-scripts/event_journal_consumer.py", type: "references", weight: 1.0 }
    - { to: "lupo-scripts/sync_channel_artifacts.py", type: "references", weight: 1.0 }
    - { to: "lupo-database/journal/", type: "references", weight: 1.0 }
    - { to: "lupo-database/divergences/", type: "references", weight: 1.0 }
...
python lupo-scripts/generate_content_id.py

# Allocate for specific artifact
python lupo-scripts/generate_content_id.py \
  lupo-channels/42/threads/1003/file.md 1 lupo_dialog_messages
```

**Output**:
```
content_id: 20260326190001000001
file_path: lupo-channels/42/threads/1003/file.md
...
python lupo-scripts/event_journal_writer.py write \
  lupo-channels/42/threads/1003/file.md created

# Write journal entry for updated file
python lupo-scripts/event_journal_writer.py write \
  lupo-channels/42/broadcasts/file.md updated

# Check journal status
python lupo-scripts/event_journal_writer.py status

# Load recent journal entries
python lupo-scripts/event_journal_writer.py load 10
```

**Output Location**: `lupo-database/journal/{event_id}.json`
...
```json
{
  ...
  "file_path": "lupo-channels/66/threads/2012/file.md",
  "file_hash": "sha256_hash",
  "actor_id": 0,
...
python lupo-scripts/event_journal_consumer.py status

# Dry-run: test processing single event
python lupo-scripts/event_journal_consumer.py dryrun 20260326190001000001

# Apply: import event to database
python lupo-scripts/event_journal_consumer.py apply 20260326190001000001
```
...
**Response**: Write divergence artifact to `lupo-database/divergences/`, do NOT continue
...
- 🛑 STOP
- Review divergence artifacts in `lupo-database/divergences/`
...
python lupo-scripts/sync_channel_artifacts.py --repo-root . --channel 42 --dry-run
```
...
- Print error: `FILE_NOT_FOUND: lupo-channels/42/threads/1003/file.md`
...
**Trigger**: `lupo-database/divergences/` contains unresolved issues
...
### Divergence Report
```bash
ls -la lupo-database/divergences/
```
