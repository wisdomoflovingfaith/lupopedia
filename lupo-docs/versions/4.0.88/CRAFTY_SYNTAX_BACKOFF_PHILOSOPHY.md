---
lupopedia.headers:
  file_path_from_root: lupo-docs/versions/4.0.88/CRAFTY_SYNTAX_BACKOFF_PHILOSOPHY.md
  last_modified_utc: '20260326155710'
  artifact_type: design_rationale
  purpose: Document collision handling philosophy and Crafty Syntax wisdom
  traits:
  - canonical
  - design_decision
  - historical_context
lupopedia.edges:
  ...
  - to: lupo-scripts/generate_content_id.py
    type: references
  - to: lupo-legacy/craftysyntax/
    type: references (read-only)
lupopedia.footer:
  verified_by: LILITH (actor_id=2, non-interfering review)
  status: APPROVED
  next_action:
  - Implement optional exponential backoff retry in allocate_content_id_for_file()
  - Document timing choice in resolution_steps for divergence artifacts
---
...
**Location**: `lupo-scripts/generate_content_id.py`, lines ~226-241
...
- [generate_content_id.py](../../lupo-scripts/generate_content_id.py) — Implementation
- [test_content_id_stress.py](../../lupo-scripts/test_content_id_stress.py) — Collision tests (0 collisions in 1M IDs)
