---
lupopedia.headers:
  file_path_from_root: lupo-docs/versions/4.0.88/DIVERGENCE_ARTIFACT_SCHEMA.md
  last_modified_utc: '20260326155650'
  artifact_type: specification
  purpose: Define the JSON schema for divergence artifacts produced by event_journal_consumer.py
  traits:
  - canonical
  - ATHENA_DIRECTIVE_4.0.88
  - operational_specification
lupopedia.edges:
  outbound_edges:
  - to: lupo-docs/versions/4.0.88/DATABASE_IMPORT_OPERATIONAL_GUIDE.md
    type: references
lupopedia.footer:
  verified_by: ATHENA
  next_action:
  - Use this schema for all divergence artifact generation
  - Event journal consumer MUST emit this format
  - Operators MUST follow resolution steps before retry
---
...
**Example**:
```json
"file_path": "lupo-channels/42/threads/1003/20260326_message.md"
```
...
"resolution_steps": [
  ...
  "4. Monitor lupo-database/divergences/ for new divergences",
...
"resolution_steps": [
  ...
  "      Command: cp /backup/lupo-channels/42/threads/1003/file.md lupo-channels/42/threads/1003/file.md",
  "   b) Regenerate journal entry with current file state",
  "      Command: python generate_content_id.py allocate lupo-channels/42/threads/1003/file.md",
...
```json
"_details": {
  "exception": "FileNotFoundError: [Errno 2] No such file or directory: 'lupo-channels/42/threads/1003/file.md'",
  "python_traceback": "...",
...
```json
{
  ...
  "resolution_steps": [
    "1. Verify lupo-channels/42/threads/1003/20260326_message.md exists on filesystem",
    "2. If file was deleted intentionally, update the channel manifest",
    "3. If file was deleted by accident, restore from backup:",
    "   docker cp backup-20260325:/backups/channels/42/1003/20260326_message.md lupo-channels/42/threads/1003/20260326_message.md",
    "4. Verify file hash matches expected (abc123...abc1):",
    "   sha256sum lupo-channels/42/threads/1003/20260326_message.md",
    "5. Re-run sync to resume import:",
    "   python lupo-scripts/sync_channel_artifacts.py --sync",
...
```json
{
  ...
  "divergence_type": "HASH_MISMATCH",
  "severity": "CRITICAL",
  "file_path": "lupo-channels/42/threads/1003/edited_message.md",
  ...
  "resolution_steps": [
    ...
    "      git checkout HEAD lupo-channels/42/threads/1003/edited_message.md",
    "   b) Regenerate as new journal entry (treats current file as new artifact):",
    "      python lupo-scripts/generate_content_id.py allocate lupo-channels/42/threads/1003/edited_message.md lupo_dialog_messages 102",
    "6. CHOOSE (a) or (b) above. Do not proceed without choosing one.",
    "7. After choice, re-run: python lupo-scripts/sync_channel_artifacts.py --sync"
  ],
  "manual_review_required": true,
  "escalation_contact": "LILITH (actor_id=2) — LIL001 non-interference protocol applies",
  "_details": {
...
```json
{
  ...
  "severity": "CRITICAL",
  "file_path": "lupo-channels/42/threads/1005/new_message.md",
  ...
  "resolution_steps": [
    ...
    "     python lupo-scripts/generate_content_id.py allocate lupo-channels/42/threads/1005/new_message.md",
    "5. Verify system clock is correct: date -u",
    "6. After resolution, re-run: python lupo-scripts/sync_channel_artifacts.py --sync"
  ],
  "manual_review_required": true,
  "escalation_contact": "LILITH (actor_id=2) — LIL001 non-interference protocol applies",
  "_details": {
...
### 2. Storage Phase
```
Divergence artifacts written to: lupo-database/divergences/{timestamp}_{type}.json
...
```
Operator:
  1. ls lupo-database/divergences/
  2. cat lupo-database/divergences/{filename}.json | jq '.resolution_steps[]'
...
```bash
find lupo-database/divergences -name "202603*" -type f
```

### Count by type
```bash
ls -1 lupo-database/divergences/ | cut -d_ -f2 | cut -d. -f1 | sort | uniq -c
```

### Find CRITICAL severity
```bash
for f in lupo-database/divergences/*.json; do
...
```bash
jq '.resolution_steps[]' lupo-database/divergences/20260326155651_HASH_MISMATCH.json
```
...
- [event_journal_consumer.py](../../lupo-scripts/event_journal_consumer.py) — Code that generates these artifacts
