---
lupopedia.headers:
  header_format_version: 2
  lupopedia.schema: documentation
  version_when_written: "4.0.94"
  when_updated: "20260405104405"
  file_path_from_root: "lupo-docs/versions/4.0.94/comments/THREAD_INDEX.md"
  web_path: "http://www.lupopedia.com/lupopedia/lupo-docs/versions/4.0.94/comments/THREAD_INDEX.md"
  last_modified_utc: "20260405104405"
  federation_node_id: 0
  channel_id: 42
  thread_id: "version-4.0.94-comments"
  author:
    type: "actor"
    id: 102
    name: "CURSOR"
  delegation_chain: "cursor:root"
  artifact_type: "documentation"
  artifact_kind: "thread_index"
  purpose: "Index of version-scoped comments for 4.0.94"
  tags:
  - "comments"
  - "4.0.94"
lupopedia.footer:
  last_verified: "20260405104405"
  verified_by:
    identity_type: "actor"
    actor_id: 102
  orchestrator: "cursor:root"
---

# file: lupo-docs/versions/4.0.94/comments/THREAD_INDEX.md — delegation: cursor:root

# Version 4.0.94 — Comments

| Posted (UTC) | File | Summary |
|--------------|------|---------|
| 20260405_104405 | [20260405104405_COMMENT_cursor_session_end_semantic_navbar_crafty_handoff.md](20260405104405_COMMENT_cursor_session_end_semantic_navbar_crafty_handoff.md) | Cursor: session end — semantic navbar / PRD 21 / Admin embedder UI version sync; `PLAN` M / `TODO` / `edges` / `CHANGELOG`; human handoff — **Crafty Syntax feature list**, start **easy → hard** when rested; decision `104405`. |
| 20260405_001004 | [20260405001004_COMMENT_cursor_session_end_admin_nav_logout_handoff.md](20260405001004_COMMENT_cursor_session_end_admin_nav_logout_handoff.md) | Cursor: session end — `logout.php` clears intro `sessionStorage`; admin nav logo + actor truncation; `PLAN` L / `TODO` / `edges` / `CHANGELOG`; Crafty parity checklist handoff for next session; decision `501004`. |
| 20260404_175216 | [20260404175216_COMMENT_cursor_session_end_agape_kairos_routing_version_sync.md](20260404175216_COMMENT_cursor_session_end_agape_kairos_routing_version_sync.md) | Cursor: session end — AGAPE §14.6 + PRD 37 temporal/`add-status` + multi-actor `to_actor_id` PRD cluster; version-folder `CHANGELOG`/`PLAN` K/`TODO`/`edges`/`WHAT_TO_WORK_ON_NEXT_SESSION`; decision `175216`. |
| 20260404_161001 | [20260404_161001_COMMENT_cursor_service_agents_softaculous_version_doc_sync.md](20260404_161001_COMMENT_cursor_service_agents_softaculous_version_doc_sync.md) | Cursor: `4.0.94` tree sync — service agents + PRD 00 §5 + Softaculous auto-installer + THOTH grounding; `CHANGELOG` / `PLAN` J / `TODO` / `edges`; decision `161001`. |
| 20260404_074421 | [20260404_074421_COMMENT_cursor_session_end_softaculous_wordpress_semantic_chat.md](20260404_074421_COMMENT_cursor_session_end_softaculous_wordpress_semantic_chat.md) | Cursor: session end — PRD 00 §15, WordPress + semantic + chat JS doctrines, installer htaccess + packager script; next = visitor UI + monitoring embed + admin chat (non-IIFE default). |
| 20260403_140555 | [20260403_140555_COMMENT_cursor_doctrine_audit_version_sync.md](20260403_140555_COMMENT_cursor_doctrine_audit_version_sync.md) | Cursor: 4.0.94 tree sync for doctrine audit + mobile/workflow thread; no unverified batch counts in version docs. |
| 20260403_025158 | [20260403_025158_COMMENT_cursor_session_end_prd31_next_session.md](20260403_025158_COMMENT_cursor_session_end_prd31_next_session.md) | Cursor: session end — PRD 31 sync receipt; observations (scaffold, install, Eye, schema WARN); `WHAT_TO_WORK_ON_NEXT_SESSION.md`. |
| 20260403_022546 | [20260403_022546_COMMENT_cursor_prd33_version_doc_sync.md](20260403_022546_COMMENT_cursor_prd33_version_doc_sync.md) | Cursor: PRD 33 `approved` + 4.0.94 CHANGELOG/PLAN/TODO/edges + thread artifacts `022543`–`022546`. |
| 20260402_235141 | [20260402_235141_COMMENT_lilith_lineage_audit_question_234552.md](20260402_235141_COMMENT_lilith_lineage_audit_question_234552.md) | LILITH audit: missing QUESTION `has_answer` edge fixed; ANSWER paths normalized; no separate `edges/` dir per repo pattern. |
| 20260402_234554 | [20260402_234554_COMMENT_cursor_ide_facet_documentation_pass.md](20260402_234554_COMMENT_cursor_ide_facet_documentation_pass.md) | Cursor: 4.0.94 tree updated for IDE facet thread; no speculative template claims. |
| 20260404_200100 | [20260404_200100_COMMENT_lilith_directive_version_documentation_update.md](20260404_200100_COMMENT_lilith_directive_version_documentation_update.md) | LILITH directive: 5W1H version-doc refresh; scope limited to verified thread outcomes. |
| 20260402_232719 | [20260402_232719_COMMENT_lilith_audit_ide_actor_agent_model_clarification.md](20260402_232719_COMMENT_lilith_audit_ide_actor_agent_model_clarification.md) | LILITH audit: IDE actor/agent/facet model APPROVED; shared `_shared/` prompt + thin wrappers; registry path correction. |
| 20260402_225226 | [20260402_225226_COMMENT_cursor_thread_version_doc_sync.md](20260402_225226_COMMENT_cursor_thread_version_doc_sync.md) | Cursor: PLAN/TODO/edges/CHANGELOG + THREAD_INDEX sync receipt (identity/temporal thread; no speculative template claims). |
