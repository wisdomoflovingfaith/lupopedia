...
lupopedia.init:
  required_reading:
    - path: "CHANGELOG.md"
      reason: "Canonical description of 4.0.77 work and cross-agent changes"
    - path: "lupo-docs/version.md"
      reason: "High-level version summary for 4.0.77"
    - path: "lupo-docs/doctrine/LUPOPEDIA_HEADERS/README.md"
      reason: "Header format, block order, and storage in lupo_metadata"
    - path: "lupo-docs/doctrine/LUPOPEDIA_HEADERS/LUPOPEDIA_HEADERS_FORMAT.md"
      reason: "Required header fields and session/edges/engagement semantics"
    - path: "lupo-docs/doctrine/LUPOPEDIA_HEADERS/LUPOPEDIA_HEADERS_PLAN.md"
      reason: "Canonical header plan, optional blocks, and channel support"
    - path: "lupo-docs/doctrine/BAYESIAN_DECISION_DOCTRINE.md"
      reason: "Doctrine for optional Bayesian Decision Tracking foundation tables"

lupopedia.headers:
  lupopedia.version: "4.0.77"
  lupopedia.schema: "documentation"
  system_version: "4.0.77"
  file_path_from_root: "lupo-docs/versions/4.0.77/PLAN.md"
  ...
  purpose: "Dependency-ordered implementation plan for remaining 4.0.77 work (headers, tooling, Bayesian foundation integration, upgrade validation)"
  tags: ["plan", "4.0.77", "headers", "bayesian_decisions", "tooling"]
...
lupopedia.footer:
  ...
  orchestrator: "cursor"
  next_action:
    - "Use this plan to coordinate remaining 4.0.77 work across IDE agents"
    - "Update PLAN.md and TODO.md as tasks are completed or re-scoped"
---
...
- **0.1 Coordination protocol and location**
  - Coordination artifacts for 4.0.77 live under: `lupo-database/coordination/4.0.77/`
...
- **2.0 File-based export/import (done 4.0.77)**  
  - `headers export` and `headers import` in lupo.php; `export_lupopedia_headers.php`, `import_lupopedia_headers.php`. Operate on files only; round-trip documented in `lupo-tests/fixtures/headers/README.md`. **lupo_metadata** (DB ↔ YAML) sync remains below.

- **2.1 Header export (DB → YAML)** *(deferred)*
  - Design and implement a PHP or Python tool (e.g. `lupo-scripts/export_headers_from_db.php` or `export_headers_from_db.py`) that:
...
- **2.2 Header import (YAML → DB)** *(deferred)*
  - Implement the inverse tool (e.g. `lupo-scripts/sync_metadata_from_headers.php` referenced in `plan.md`):
...
- **2.3 Validator and CI wiring**
  ...
  - Add a simple CLI entry (e.g. `php lupo-bin/lupo.php headers:validate`) for local runs and CI.
...
- **5.0 Zencoder integration (done 4.0.77)**  
  - Zencoder (actor_id 106) workspace under `lupo-actors/zencoder/` and four development table docs (lupo_analytics_campaign_vars, lupo_world_registry, lupo_auth_audit_log, lupo_channel_boot_detail) plus seed_actor_zencoder_4.0.77.sql committed and pushed by Cursor on Zencoder's behalf after git failure. Canonical directories use the **lupo-** prefix (lupo-actors, lupo-docs, lupo-database, etc.).
...
- **5.2 Handoff and reporting**
  - For each completed sub-phase, add or update a concise status artifact under `lupo-docs/status/` and reference it from CHANGELOG.md in a follow-up 4.0.77 entry if needed.

- **5.3 Table documentation initiative — 4.0.77 stop line (done)**
  - Cursor continued the Zencoder → Windsurf table-doc workstream: improved lupo_sessions and lupo_contents with 4.0.77 headers and "Where This Table Is Used"; created `lupo-docs/status/TABLE_DOCUMENTATION_4_0_77_STOP_LINE.md` defining 4.0.77 accomplishment and 4.0.78 handoff. Remaining table-doc modernization (e.g. lupo_channels, lupo_actors, Priority 2/3 tables, mass header cleanup) deferred to 4.0.78. Pattern and priorities documented in stop-line artifact.
