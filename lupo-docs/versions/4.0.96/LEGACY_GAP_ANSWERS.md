...
lupopedia.headers:
  ...
  file_path_from_root: "lupo-docs/versions/4.0.96/LEGACY_GAP_ANSWERS.md"
  web_path: "http://www.lupopedia.com/lupopedia/lupo-docs/versions/4.0.96/LEGACY_GAP_ANSWERS.md"
  ...
  purpose: "Answers to the legacy-gap questions raised during TrustLadderRegistry implementation (4.0.96)"
  tags:
    - note
    - legacy_gap
    - trust_ladder
...
# file: lupo-docs/versions/4.0.96/LEGACY_GAP_ANSWERS.md — delegation: claude_code:root
...
- Add a `php lupo-bin/lupo.php doctor` check that reports 32-bit PHP as a `WARNING`.
...
- For CLI tools (`lupo-bin/lupo.php`), the process is short-lived — no TTL needed.
...
- `php lupo-bin/lupo.php doctor` checks the max `last_validated_ymdhis` and
