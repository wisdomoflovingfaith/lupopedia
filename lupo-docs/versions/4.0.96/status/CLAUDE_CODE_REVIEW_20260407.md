...
lupopedia.headers:
  ...
  file_path_from_root: lupo-docs/versions/4.0.96/status/CLAUDE_CODE_REVIEW_20260407.md
  web_path: http://www.lupopedia.com/lupopedia/lupo-docs/versions/4.0.96/status/CLAUDE_CODE_REVIEW_20260407.md
  last_modified_utc: "20260407000000"
  channel_id: 42
  actor_id: 116
  ...
  purpose: "Claude Code review of 4.0.96 trust-ladder work: IdGenerator.php, KairosConsolidationService.php, validate_trust_ladder_registry.py, doctrine docs"
  tags:
    - "4.0.96"
    - "trust_ladder"
    - "code_review"
...
lupopedia.edges:
  outbound_edges:
    - to: lupo-docs/versions/4.0.96/status/FOR_CLAUDE_CODE_ON_PK_IDS.md
      type: references
      weight: 1.0
      reason: Handoff doc reviewed
    - to: lupo-includes/classes/IdGenerator.php
      type: references
      weight: 1.0
      reason: Primary code subject of review
    - to: app/Services/Kairos/KairosConsolidationService.php
      type: references
      weight: 1.0
      reason: Primary code subject of review
    - to: lupo-scripts/validate_trust_ladder_registry.py
      type: references
      weight: 1.0
      reason: Tooling subject of review
    - to: lupo-docs/doctrine/CHRONOLOGICAL_TRUST_LADDER.md
      type: references
      weight: 1.0
      reason: Normative spec verified against
lupopedia.footer:
  last_verified: "20260407000000"
  verified_by:
    identity_type: actor
    actor_id: 116
...
| P1 | `validateTrustLadderPk()` seed band accepts **any** short numeric < 2026 without the registry check mandated by CHRONOLOGICAL_TRUST_LADDER.md §2.2.2 Rule 1 | `lupo-includes/classes/IdGenerator.php:260` |
| P2 | `TRUST_LADDER_REGISTRY.md`: `lupo_actor_memory` KAIROS entry is stale — KAIROS migrated to `lupo_memory_nodes` in this very batch | `lupo-docs/doctrine/TRUST_LADDER_REGISTRY.md:76` |
...
| P2 | `@version` docblock in `IdGenerator.php` is `4.0.94`, not updated to `4.0.96` | `lupo-includes/classes/IdGenerator.php:13` |
| P3 | `toCanonicalIdSafe()` does not validate that the incoming `$stagingId` is actually staging-shaped (year 2000–2099); a canonical id passed by mistake would double-subtract | `lupo-includes/classes/IdGenerator.php:128` |
...
**File:** `lupo-includes/classes/IdGenerator.php:254–315`
...
**File:** `lupo-docs/doctrine/TRUST_LADDER_REGISTRY.md:76`
