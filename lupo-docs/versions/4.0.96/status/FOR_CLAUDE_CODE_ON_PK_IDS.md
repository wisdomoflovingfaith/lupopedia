...
lupopedia.headers:
  ...
  file_path_from_root: lupo-docs/versions/4.0.96/status/FOR_CLAUDE_CODE_ON_PK_IDS.md
  web_path: http://www.lupopedia.com/lupopedia/lupo-docs/versions/4.0.96/status/FOR_CLAUDE_CODE_ON_PK_IDS.md
  ...
  purpose: "Handoff for Claude Code (116): Chronological Trust Ladder, PK/seed semantics, install/runtime, future web UI for install records"
  tags:
    - "4.0.96"
    - "trust_ladder"
    - "pk"
...
# file: FOR_CLAUDE_CODE_ON_PK_IDS — delegation: cursor:root — web_path: [http://www.lupopedia.com/lupopedia/lupo-docs/versions/4.0.96/status/FOR_CLAUDE_CODE_ON_PK_IDS.md](http://www.lupopedia.com/lupopedia/lupo-docs/versions/4.0.96/status/FOR_CLAUDE_CODE_ON_PK_IDS.md)
...
**Temporal anchor:** `20260408031925` UTC (`python lupo-bin/tick.py` batch for this document).
...
| **WHERE** | Canonical doctrine: `lupo-docs/doctrine/CHRONOLOGICAL_TRUST_LADDER.md`, `TRUST_LADDER_REGISTRY.md`, `RETENTION_POLICY.md`. Runtime: `lupo-includes/classes/IdGenerator.php`, `app/Services/Kairos/KairosConsolidationService.php`. Validation: `lupo-scripts/validate_trust_ladder_registry.py`. Version bundle: `lupo-docs/versions/4.0.96/`. |
| **WHEN** | Handoff timestamp **20260408031925** UTC; see `lupo-docs/versions/4.0.96/CHANGELOG.md` for session rollups. |
...
- `lupo-docs/doctrine/CHRONOLOGICAL_TRUST_LADDER.md` — full normative text, **`validateTrustLadderPk`**, **`toCanonicalIdSafe`**, migration notes, tests.
- `lupo-docs/doctrine/TRUST_LADDER_REGISTRY.md` — which tables participate and how.
- `lupo-content/federation_node/0/captains_log/20260408_CHRONOLOGICAL_TRUST_LADDER.md` — narrative summary and links.
...
- **Captain's Log** — `lupo-content/federation_node/0/captains_log/20260408_CHRONOLOGICAL_TRUST_LADDER.md`.
...
1. **Actors** — Canonical registry in **`lupo-database/lupopedia/actors/registry.json`**. Seeds in SQL stay **short**; canonical **18-digit** strings for ladder contexts come from **`IdGenerator::seedActorToCanonicalId()`** when needed.
...
5. **Install/seed** — Follow **`lupo-docs/prd/41_install_seed_doctrine.md`**: seed rows are not reinterpreted as “already canonical ladder” unless doctrine says so.
...
1. **Reads** canonical install/seed **sources** (SQL files under `lupo-database/lupopedia/mysql/install/` and seed paths per project layout) and **displays** rows that participate in the trust ladder or that operators must edit (e.g. registry-aligned seeds).
...
- `lupo-docs/versions/4.0.96/CHANGELOG.md` — session changelog entries.
- `lupo-docs/versions/4.0.96/SUMMARY.md` — 5W1H rollup.
- `lupo-docs/versions/4.0.96/status/THREAD_INDEX.md` — status thread index.
