...
lupopedia.headers:
  ...
  file_path_from_root: lupo-docs/versions/4.0.96/status/INSTALLER_AUDIT_20260407.md
  web_path: http://www.lupopedia.com/lupopedia/lupo-docs/versions/4.0.96/status/INSTALLER_AUDIT_20260407.md
  ...
  purpose: "Claude Code installer audit — fresh install path after dropping all tables and deleting lupopedia-config.php"
  tags:
    - "installer"
    - "audit"
    - "trust_ladder"
...
lupopedia.edges:
  outbound_edges:
    - to: install.php
      type: references
      weight: 1.0
      reason: Primary audit subject
    - to: install_wizard_classes.php
      type: references
      weight: 1.0
      reason: Primary audit subject
    - to: lupo-includes/classes/InstallWizardMysqliLink.php
      type: references
      weight: 1.0
      reason: DB layer audited
    - to: lupo-database/lupopedia/mysql/seed/seed_4.1.0.sql
      type: references
      weight: 1.0
      reason: Seed file audited
    - to: install/seed_lupopedia_4_1_0.sql
      type: references
      weight: 1.0
      reason: Consolidated seed audited
lupopedia.footer:
...
**Scope:** `install.php`, `install_wizard_classes.php`, `InstallWizardMysqliLink.php`, `install_new_lupopedia.sql` (spot-checked), `install/seed_lupopedia_4_1_0.sql`, `lupo-database/lupopedia/mysql/seed/seed_4.1.0.sql`
...
2. **Fallback: `lupo-database/lupopedia/mysql/seed/seed_4.1.0.sql`** — uses hardcoded `lupo_` names.
...
**Mitigation already present:** If the consolidated file exists (produced by `lupo-scripts/build_consolidated_seed_4_1_0.py`), the fallback is never reached. The fallback is only a risk on dev setups that haven't run the build script.
...
### lupo-database/lupopedia/mysql/seed/seed_4.1.0.sql (fallback)
...
| P2 | Fallback seed (`seed_4.1.0.sql`) uses hardcoded `lupo_` — only safe with default prefix | `lupo-database/…/seed_4.1.0.sql` | all |
