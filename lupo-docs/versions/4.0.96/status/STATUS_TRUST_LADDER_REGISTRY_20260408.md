...
lupopedia.headers:
  ...
  file_path_from_root: "lupo-docs/versions/4.0.96/status/STATUS_TRUST_LADDER_REGISTRY_20260408.md"
  web_path: "http://www.lupopedia.com/lupopedia/lupo-docs/versions/4.0.96/status/STATUS_TRUST_LADDER_REGISTRY_20260408.md"
  ...
  purpose: "Post-implementation status and observations for the TrustLadderRegistry sprint (2026-04-08 13:32 UTC)"
  tags:
    - status
    - trust_ladder
    - registry
...
|-----------|------|--------------------|--------|
| `TrustLadderRegistry.php` | `lupo-includes/classes/TrustLadderRegistry.php` | ~310 lines | ✅ Done |
| `sync_trust_ladder_registry_to_db.py` | `lupo-scripts/sync_trust_ladder_registry_to_db.py` | ~340 lines | ✅ Done |
| Unit tests | `lupo-tests/unit/trust_ladder_registry_test.php` | 47/47 pass | ✅ Done |
| SQL DDL + seed rows | `install_new_lupopedia.sql` (appended) | 13 seed rows | ✅ Done |
| `IdGenerator::toCanonicalIdSafe()` registry check | `lupo-includes/classes/IdGenerator.php` | +25 lines | ✅ Done |
| Legacy gap answers | `lupo-docs/versions/4.0.96/LEGACY_GAP_ANSWERS.md` | 11 answers | ✅ Done |
...
```
php lupo-tests/unit/trust_ladder_registry_test.php
...
5. **`php lupo-bin/lupo.php doctor` registry check** — validate that all tables
...
**Forward action:** Run `php lupo-bin/lupo.php doctor` against a seeded DB to confirm
registry integration. Run `python lupo-scripts/sync_trust_ladder_registry_to_db.py --dry-run`
