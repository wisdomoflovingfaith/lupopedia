...
lupopedia.headers:
  ...
  file_path_from_root: "lupo-docs/versions/4.0.96/status/STATUS_SESSION_IDENTITY_AND_GC_20260408161022.md"
  web_path: "http://www.lupopedia.com/lupopedia/lupo-docs/versions/4.0.96/status/STATUS_SESSION_IDENTITY_AND_GC_20260408161022.md"
  ...
  purpose: "End-of-session handoff: session identity, GC, docs; observations and follow-ups for Claude Code / next session"
  tags:
    - session
    - gc
    - "4.0.96"
...
- **Runtime:** `app/auth/Session.php`, `lupo-includes/classes/SessionManager.php`, `lupo-includes/bootstrap.php`, `lupopedia-config.php`.
- **Schema:** `lupo-database/lupopedia/mysql/install/install_new_lupopedia.sql` (`session_identity_hash`, indexes).
- **Tooling:** `lupo-scripts/generate_session_salt.php`.
- **Docs:** `lupo-docs/versions/4.0.96/CHANGELOG.md`, `PLAN.md`, `TODO.md`, this file.

## WHEN

**`20260408161022` UTC** — **2026-04-08, 16:10 UTC** (`python lupo-bin/tick.py` anchor for this batch).
...
3. **TOON/JSON** exports under `lupo-docs/toons/` or `lupo-database/lupopedia/json/` were **not** regenerated in this session; run project **`generate_toon_from_sql.py`** / pipeline when schema export must match install.
...
1. **Optional `ClientIpResolver` class** in `lupo-includes/classes/` — single implementation shared by Session, visits, and analytics (avoid drift from Crafty `get_ipaddress()` reference).
