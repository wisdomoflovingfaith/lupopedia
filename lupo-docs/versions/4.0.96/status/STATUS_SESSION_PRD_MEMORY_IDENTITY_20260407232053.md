...
lupopedia.headers:
  ...
  file_path_from_root: lupo-docs/versions/4.0.96/status/STATUS_SESSION_PRD_MEMORY_IDENTITY_20260407232053.md
  web_path: http://www.lupopedia.com/lupopedia/lupo-docs/versions/4.0.96/status/STATUS_SESSION_PRD_MEMORY_IDENTITY_20260407232053.md
  last_modified_utc: "20260407232053"
  when_updated: "20260407232053"
  federation_node_id: 0
  channel_id: 42
  artifact_type: documentation
  artifact_kind: status_report
  purpose: "What was completed in-session (PRD 38, memory DDL, export service, PRD 01 sessions, changelogs) + forward prompt for constitutional PRD fixes"
  tags:
    - "4.0.96"
    - status
    - prd
...
| **Runtime** | New `lupo-includes/classes/MemoryExportService.php` (PDO_DB, `generateSlug`, export/remove/full/incremental). |
...
| **Changelog** | `lupo-docs/versions/4.0.96/CHANGELOG.md` entries for PRD 38, PRD 38 edges/phases, PRD 01 session section, and this batch (header ticks via `lupo-bin/tick.py`). |
...
- Regenerate **TOON/JSON** from install after DDL stabilizes (`lupo-scripts/generate_toon_from_sql.py` or project standard).
...
Change `file_path_from_root: "/lupo-docs/prd/27_installer_requirements.md"` → remove leading `/`
...
Change `lupo-database/lupopedia/toon/` → `lupo-database/lupopedia/json/`
...
| **CRITICAL 2 — `lupo_actor_memory` PK** | **Done in code + install:** `memory_id` → **`actor_memory_id`** in `install_new_lupopedia.sql`, **`KairosConsolidationService.php`**, **`lupo-docs/prd/01_core_identity.md`** (table + index names + training cross-ref), **`lupo-docs/prd/09_federation_sync.md`**. **`lupo-database/lupopedia/json/lupo_actor_memory.json`** updated to match; **`.toon.json`** regenerated via script. |
...
| **`python lupo-scripts/generate_toon_from_sql.py`** | **Was run:** regenerated **177** TOONs and **removed 48** prior `json`/`toon` exports not present in current install DDL. **Verify with `git status`** — restore any removed files if your workflow still expected them. |
