...
lupopedia.headers:
  ...
  file_path_from_root: "lupo-docs/versions/4.0.89/TODO.md"
  content_id: 8714183816754237706
  web_path: "http://www.lupopedia.com/lupopedia/lupo-docs/versions/4.0.89/TODO.md"
  ...
  artifact_type: todo
  artifact_kind: task
  purpose: 4.0.89 release tasks — LUPOPEDIA HEADERS validation, import (Python IDE agents + PHP API agents), PHP agent content-only filesystem policy (H9), lupo-* repo literacy, IDE rules, DB parity, release-gate tests
  tags:
    - "4.0.89"
    - headers
    - todo
...
lupopedia.edges:
  outbound_edges:
    ...
    - to: "lupo-docs/versions/4.0.90/TODO.md"
      type: references
      weight: 0.9
      reason: Deferred non-header backlog
    - to: "lupo-docs/versions/4.0.89/WHAT_TO_DO_NEXT_SESSION.md"
      type: references
      weight: 1.0
      reason: IDE session handoff (headers line)
lupopedia.footer:
  ...
  next_action:
    - "Read WHAT_TO_DO_NEXT_SESSION.md in this folder (version root) — Cursor handoff; not legacy_research/WHAT_TO_DO_NEXT_SESSION.md"
    - "H5/H7/H8 verification recorded in CHANGELOG.md — tag 4.0.89 per WOLFIE; slug/content_id conflicts: python lupo-scripts/import_content.py --write-back <path.md> (see CHANGELOG errata)"
    - "Close PHP/admin parity items (H2.1, H2.3)"
    - "H4.4 org literacy + H4.2 rule packs (process); H9.4 extended audit optional"
---
...
**See also:** **[README.md](README.md)** (full narrative overview), **[CHANGELOG.md](CHANGELOG.md)** (version overview + achievements table), **[`ORGANIZATION.md`](../../../ORGANIZATION.md)** (root **`lupo-*` map**), **[`lupo-docs/ORGANIZATION.md`](../../ORGANIZATION.md)** (doc authority + **§2.2** PHP boundaries).
...
| H2.3 | **Admin or operator UI** — page or action that runs header validation on a chosen `.md` (or batch) and shows errors | **PENDING** (gap: no dedicated module found under `lupo-includes/modules` yet) |
...
| ID | Task | Status |
|----|------|--------|
| H3.1 | Confirm `install_new_lupopedia.sql` + `lupo-database/lupopedia/json/*.json` match `header_db_sync` expectations | VERIFY |
...
## H4 — `lupo-*` trees + IDE rules

| ID | Task | Status |
|----|------|--------|
| H4.1 | `lupo-rules/root/RULE_FILES_HEADER_REQUIREMENT.md` + header doctrines referenced from AGENTS / onboarding | VERIFY |
...
| H4.3 | Channel / filesystem fallback paths (`lupo-channels/`, offline doctrine) documented for header-bearing artifacts | VERIFY |
| H4.4 | **Repository literacy (4.0.89 exit)** — Everyone operating this release (including **PHP agent** operators) understands what each major **`lupo-*`** tree is for: read **[`ORGANIZATION.md`](../../../ORGANIZATION.md)** (root) **§4** (directory map) and **[`lupo-docs/ORGANIZATION.md`](../../ORGANIZATION.md)** (**§2** documentation authority + **§2.2** PHP agent boundaries). Cross-check with **`AGENTS.md`** *Key Directories* / architecture overview. | VERIFY |

---

## H5 — Release-gate testing (mandatory before tag)

Run **at least three** distinct markdown artifacts (e.g. root doctrine, one `lupo-docs/doctrine/` file, one `lupo-channels/` artifact) through:

1. **Import:** `python lupo-scripts/import_content.py <path> [--write-back]` (IDE path) **or** `php lupo-scripts/import_content.php <path> [--write-back]` (PHP path) — success; for validators/`--check-db`, **`content_id`** should appear in the file (use **`--write-back`** on either toolchain, or **`generate_headers_from_db`** from DB).
2. **Read-back:** Query `lupo_contents` (+ edges if applicable) — matches file intent.
3. **Regenerate:** `python lupo-scripts/generate_headers_from_db.py --file-path <path>` **or** `php lupo-scripts/generate_headers_from_db.php --file-path <path>` — YAML sane; no unexpected loss of `outbound_edges` / `lupopedia.history` vs DB policy. **At least one** artifact in the matrix must carry a non-empty `lupopedia.history` so **H7** is exercised (see below).
...
**Binding text:** [`lupo-rules/root/LUPOPEDIA_HEADERS_DOCTRINE.md`](../../../lupo-rules/root/LUPOPEDIA_HEADERS_DOCTRINE.md) — *Dual running log — file and database*.
...
|-------------|--------|----------|------------|
| **IDE / dev** (Python available) | `python lupo-scripts/import_content.py` | `validate_lupopedia_headers.py` | `generate_headers_from_db.py` |
| **Shared hosting / PHP agents** | `php lupo-scripts/import_content.php` | `validate_lupopedia_headers.php` | `generate_headers_from_db.php` |
...
### H8.1 — `lupo-scripts/import_content.php`
...
### H8.2 — `lupo-scripts/validate_lupopedia_headers.php`
...
### H8.3 — `lupo-scripts/generate_headers_from_db.php`
...
### H8.4 — `lupo-includes/classes/HeaderDbSync.php`
...
```bash
python lupo-scripts/import_content.py path/to/file.md --dry-run
php lupo-scripts/import_content.php path/to/file.md --dry-run
```
...
- [x] **`lupo-docs/ORGANIZATION.md`** — agent toolchain table + **§2.2** PHP agent content-only policy
...
- [ ] Optional: dedicated faucet / operator **prompt snippet** beyond **`lupo-docs/ORGANIZATION.md` §2.2** (nice-to-have)
...
|--------------|------------|------------|
| Code directories (`lupo-includes/`, `app/`, `lupo-scripts/`, `lupo-bin/`, `lupo-api/`, …) | **Allowed** | **Forbidden** |
| Root PHP entry points | **Allowed** | **Forbidden** |
| **`.php`**, **`.py`**, **`.sh`**, **`.js`**, **`.html`**, **`.htm`**, … | **Allowed** | **Forbidden** |
| **`lupo-rules/`**, **`lupo-docs/`**, **`lupo-channels/`**, **`lupo-content/`** | **Allowed** | **Allowed** |
...
|----------|-----------|---------|-----------|
| **Directories (write)** | **`lupo-includes/`**, **`app/`**, **`lupo-bin/`**, **`lupo-api/`**, **`lupo-routes/`**, **`lupo-views/`**, **`lupo-admin/`**, **`lupo-install/`**, **web root** / entrypoint PHP areas | **`lupo-rules/`**, **`lupo-docs/`**, **`lupo-channels/`**, **`lupo-content/`** | No runtime or tooling trees |
...
| ID | Task | Status |
|----|------|--------|
| H9.1 | Policy documented (**this section**, **`lupo-docs/ORGANIZATION.md` §2.2**, root **`ORGANIZATION.md` §4.1**, **`README.md` #12**, **`CHANGELOG.md`**) | **DONE** (verify on tag review) |
...
| H9.4 | Audit log **`lupo-logs/php_agent_filesystem_writes.log`**; optional antivirus / **`ContentSafetyService`** | **PARTIAL** (file log); extended service **PENDING** |
