...
lupopedia.headers:
  ...
  file_path_from_root: "lupo-docs/versions/REQUIRED_TABLES_4.0.6.md"
...
*(List derived from `lupo-database/lupopedia/mysql/install/install_new_lupopedia.sql`. Table count = number of CREATE TABLE statements in that file. lupo_channel_roles removed in 4.0.6; lupo_system_health_snapshots removed from install, DDL in future_features.)*
...
These tables are **not** created by `install_new_lupopedia.sql`. Their definitions live in `lupo-database/lupopedia/mysql/install/future_features_lupopedia.sql` only; run that file separately if needed.
