...
| `app/auth/SessionHandler.php` | `LUPO_TABLE_PREFIX . 'sessions'` (method `table()`) |
...
**{prefix}sessions (the TABLE):**

- **No PHP code reads or writes the `sessions` table.**  
- All references to “unified” in app code are:
  - **SessionHandler** → uses `table() = LUPO_TABLE_PREFIX . 'sessions'` (i.e. **sessions**, not sessions).
...
| **App\Auth\SessionHandler** | {prefix}sessions | createUnifiedSession, getUnifiedSession, cookie, cleanup — all use `table()` = sessions |
| **App\Auth\AuthGuard** | {prefix}sessions | updateUserActivity (last_seen) |
| **App\Auth\AuthManager** | Via SessionHandler only (sessions) | checkUnifiedAuth, getUnifiedUser (cookie + sessions) |
...
None of these use the **sessions** table in code.

---

## 2. Why two tables exist

### 2.1 Original purpose of {prefix}sessions

- **Schema (install_new_lupopedia.sql):**  
  `session_id` (PK), `session_id`, `user_id`, `system_context`, `session_data` (json), `expires_at`, `created_at`, `updated_at` (bigint — Unix timestamps).
...
- **sessions** was not tied to legacy operator tables in the schema (no FK or column to operator id).  
...
### 2.4 Is {prefix}sessions obsolete?

- **In code: yes.** No PHP reads or writes the `sessions` table.  
- **SessionHandler** is named “unified” but uses only **{prefix}sessions** (and a cookie named `{prefix}session`).  
- **Doctrine (one_time_sessions_to_sessions.sql):**  
  “Doctrine: single sessions table (prefix + 'sessions'); no separate sessions.”  
  That migration copies data from `lupo_sessions` into `lupo_sessions` and then **DROP TABLE lupo_sessions**.  
- So after that migration, **sessions** is intended to be gone. If the migration has not been run in an environment, the table may still exist but is unused by application code.
...
**Conclusion:** TOONs are not present for these two tables. Schema below is taken from **install_new_lupopedia.sql** and migrations (add_system_context, add_name_key_is_named, one_time_sessions_to_sessions).
...
### 3.2 lupo_sessions (from install)

- **Columns:**  
  session_id (PK), session_id (unique), user_id, system_context, session_data (json), expires_at, created_at, updated_at (bigint, Unix seconds).  
...
- **SessionHandler** today:
  - Writes/reads **{prefix}sessions** (not sessions).
...
- So “unification” here means: **one table ({prefix}sessions) and one place for session logic.**  
  ...
  - SessionHandler can either be refactored to delegate to Session or be reduced to cookie + system_context and thin wrappers that use Session.

### 4.2 Can {prefix}sessions be deprecated or merged?

- **Yes.**  
  - It is **already** merged in doctrine: `one_time_sessions_to_sessions.sql` migrates rows into lupo_sessions and drops lupo_sessions.  
  - No application code uses the sessions table.  
...
### 4.3 Remaining code paths that depend on {prefix}sessions

- **None.** No PHP reads or writes the sessions table.  
- The only “session” references are the cookie name and the AuthManager `'source' => 'session'` label.
...
- All session logic through **Session** class (and optionally a thin SessionHandler that uses Session).  
- No dependency on **{prefix}sessions**; table dropped where it still exists (after one-time migration).
...
| **SessionHandler** | Document whether it delegates to Session or is a thin wrapper (see 5.1a C). Optionally refactor to delegate; or leave as thin wrapper using same table. |
| **Docs** | Update REQUIRED_TABLES_4.1.0.md and any other docs that list `lupo_sessions` to state it is deprecated/removed after migration (plan D). |

### 5.3 Helper functions to remove

- **None** that reference sessions (no such helpers exist).  
...
- No queries currently target sessions, so no “replace sessions query with sessions” steps.
...
- **SessionHandler:** Either (a) make it call Session for create/read/update/destroy and only handle cookie + system_context, or (b) leave it as a second writer to the same table and document that both write to {prefix}sessions.  
- **AuthManager / AuthGuard:** Already use SessionHandler (which uses sessions table); no change required unless we refactor SessionHandler to delegate to Session.

### 5.6 Eliminating {prefix}sessions safely

1. **Confirm one-time migration:** Ensure `one_time_sessions_to_sessions.sql` has been run in every environment (so any data from sessions is in sessions and sessions can be dropped).  
2. **Drop table (per env):** Run `DROP TABLE IF EXISTS {prefix}sessions` (using configured prefix).  
3. **Schema/docs:** Remove creation of sessions from new installs (or leave CREATE in install for historical reference and document “do not use; run one_time migration and drop”).  
4. **Docs:** State that the single session table is {prefix}sessions and that sessions is deprecated/removed.

### 5.7 Summary

- **Two tables in schema:** {prefix}sessions (main) and {prefix}sessions (legacy).  
- **One table in use:** Only {prefix}sessions is used by PHP.  
- **Unification:** Already done in code; only cleanup is (1) run the existing one-time migration and drop sessions where it exists, (2) fix Legacy* hardcoding to use prefix, (3) optionally fold SessionHandler into Session and document.
