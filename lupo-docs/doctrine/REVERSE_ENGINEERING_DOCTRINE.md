...
lupopedia.headers:
  ...
  purpose: "Codify learning from external packages without adopting them as runtime dependencies; trust nothing you have not understood"
  status: active
  tags:
    - security
    - dependencies
...
lupopedia.edges:
  outbound_edges:
    - to: "lupo-docs/prd/00_root_constitutional_system_requirements.md"
      type: implements
      weight: 1.0
      reason: "Aligns with no-runtime-dependency posture"
    - to: "lupo-docs/doctrine/TWO_LAYER_SECURITY_DOCTRINE.md"
      type: references
      weight: 1.0
      reason: "Layer 1 analysis vs adoption; testing deps vs shipped runtime"
    - to: "AGENTS.md"
      type: references
      weight: 1.0
      reason: "PHP constraints and IDE agent workflow"
    - to: "lupo-docs/doctrine/ADVERSARIAL_TEST_IDENTITY_DOCTRINE.md"
      type: references
      weight: 0.85
      reason: "Controlled test naming when simulating hostile supply-chain scenarios"
    - to: "lupo-docs/prd/29_project_structure.md"
      type: references
      weight: 1.0
      reason: "lupo-research/federation_nodes/ layout; external research boundaries"
    - to: "lupo-research/federation_nodes/_templates/MANIFEST_TEMPLATE.md"
      type: references
      weight: 0.95
      reason: "MANIFEST for ingested external sources"
    - to: "lupo-docs/prd/34_federation_node_semantic_network.md"
      type: references
      weight: 0.95
      reason: "Draft PRD — semantic network peers, trust, discovery (post-4.0.x)"
...
lupopedia.footer:
  ...
  verified_by:
    ...
    name: "lilith"
  next_action:
    - "When implementing a pattern from external source, document inspired_by edges on the Lupopedia artifact"
    - "Keep federation_node >= 2 research trees read-only reference; never wire them into lupo-includes bootstrap"
    - "Defer federation semantic-network PRD until post-4.0.x stabilization; see dual-purpose section below"
---

# file: REVERSE_ENGINEERING_DOCTRINE — delegation: cursor:root — web_path: http://www.lupopedia.com/lupopedia/lupo-docs/doctrine/REVERSE_ENGINEERING_DOCTRINE.md
...
`lupo-research/federation_nodes/{federation_node_id}/<package_key>/`

Use **`federation_node_id >= 2`** for **upstream clones, specs, and read-only reference trees** (existing examples in-repo include nodes **2** and **3**). Add a **`MANIFEST.md`** from **[lupo-research/federation_nodes/_templates/MANIFEST_TEMPLATE.md](../../lupo-research/federation_nodes/_templates/MANIFEST_TEMPLATE.md)** so provenance, purpose, and license context stay attached to the tree.

This directory is **not** a Composer/npm install target and **must not** be `require`d by `index.php` or `lupo-includes/` bootstrap. It is a **textbook shelf**, not a runtime library.

### Boundary

| Location | Role |
|----------|------|
| **`app/`**, **`lupo-includes/`** (and normal shipped PHP/JS) | **Your implementation** — ships with the product; own the code. |
| **`lupo-research/federation_nodes/{id}/`** with **`id >= 2`** | **External material** — study, diff, cite; do **not** treat as a dependency. |
| **Docs with `federation_node_id: 0` in headers** | Default **repository / documentation** scope for many doctrines and guides — **not** interchangeable with “sandbox clone path”; sandboxes are identified by **folder** under `lupo-research/federation_nodes/`. |

### Workflow (dependency order)

1. Create **`lupo-research/federation_nodes/{n}/<package_key>/`** (choose **`n >= 2`** consistent with `lupo_federation_nodes` and project practice).
...
4. **Implement** behavior under **`lupo-includes/classes/`** or **`app/`** as native Lupopedia code.
...
| Clear boundary | **Ship** from `app/` / `lupo-includes/`; **archive** upstream under **`lupo-research/federation_nodes/2+`** |
...
- **Content:** Cloned upstream packages, specs, read-only reference trees under **`lupo-research/federation_nodes/{id}/`**.
...
Existing trees (e.g. nodes **2** and **3** under **`lupo-research/federation_nodes/`**) remain valid **research ingests** regardless of this sketch.
