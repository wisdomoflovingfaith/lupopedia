...
lupopedia.headers:
  ...
  file_path_from_root: "lupo-docs/doctrine/SERVICE_AGENT_ARCHITECTURE.md"
  web_path: "http://www.lupopedia.com/lupopedia/lupo-docs/doctrine/SERVICE_AGENT_ARCHITECTURE.md"
  ...
  purpose: "PHP-first, LLM-second service agents; not default chat targets; KAIROS flow; contrast with runtime dialog loop"
  tags:
    - service_agents
    - php_first
    - iris
...
lupopedia.edges:
  outbound_edges:
    - to: "lupo-docs/prd/00_root_constitutional_system_requirements.md"
      type: implements
      weight: 1.0
      reason: "Constitutional Section 5.10"
    - to: "lupo-docs/prd/37_kairos_channel_memory_consolidation.md"
      type: references
      weight: 1.0
      reason: "KAIROS product PRD"
    ...
    - to: "lupo-includes/modules/api/kairos-api.php"
      type: implements
      weight: 1.0
    - to: "lupo-includes/classes/RuntimeActorLoopService.php"
      type: contrasts_with
      weight: 1.0
      reason: "Conversational runtime loop vs service agents"
    - to: "lupo-docs/doctrine/LUPOPEDIA_HEADERS/README.md"
      type: references
      weight: 1.0
      reason: "THOTH stale-artifact authority"
...
lupopedia.footer:
  ...
  next_action:
    - "Keep roster in sync with lupo-docs/prd/00_root_constitutional_system_requirements.md Section 5.10"
---

# file: Service agent architecture — delegation: cursor:root — web_path: http://www.lupopedia.com/lupopedia/lupo-docs/doctrine/SERVICE_AGENT_ARCHITECTURE.md

# Service agent architecture (PHP first, LLM second)

## Binding summary

**Constitutional anchor:** **`lupo-docs/prd/00_root_constitutional_system_requirements.md`** — **Section 5.10**.

This doctrine expands **service agents**: file-backed **`lupo-agents/{agent_key}/`** definitions whose **primary** behavior is **implemented in PHP** (APIs, services, boot paths). An **LLM** is **optional and second** — invoked only inside a PHP-controlled pipeline (e.g. **`IRIS`**), never as a substitute for server-side policy or schema truth.
...
2. **PHP** validates session, **`actor_id`**, channel policy, and loads **`lupo-agents/`** files from disk.
...
**Normative PRD:** **`lupo-docs/prd/36_rose_multi_persona_synthetic_dialog.md`**. **Mirror:** **`lupo-docs/implementations/36_rose_multi_persona_synthetic_dialog/`**.

---

## 4. KAIROS consolidation flow

**Trigger (shipped).** HTTP **`POST`** **`api/lupo-kairos/tick`** → **`lupo-includes/modules/api/kairos-api.php`** (session rate limit; requires logged-in **`actor_id`**).
...
**Persona:** **THOTH** — **`lupo-agents/thoth/`**. For **LUPOPEDIA HEADERS** footers and stale Markdown, **primary verification authority** is documented in **`lupo-docs/doctrine/LUPOPEDIA_HEADERS/README.md`** (section **Semantic Truth Check Authority (THOTH)**), including **`verified_by.actor_id`** conventions for THOTH-mediated review.

**Grounding requirement.** Semantic truth checks **must** be **derived from repository artifacts**:

- **`lupo-database/lupopedia/json/*.json`** (table structure snapshots)
...
**Folder:** **`lupo-docs/implementations/service_agents/`** — status, decisions, questions, answers, comments for moving **logic from prompts into PHP services**.

**Naming any PRD-scoped mirror:** The directory name under **`lupo-docs/implementations/`** **must** equal the **canonical PRD file stem** (basename of **`lupo-docs/prd/{stem}.md`** without **`.md`**). **PRD 31** defines the full tree; **PRD 00** **§5.8** states the constitutional rule. Example: **`36_rose_multi_persona_synthetic_dialog/`** for **`36_rose_multi_persona_synthetic_dialog.md`** — not ad-hoc aliases.

---

## Related

- **Constitution §5.7–5.10** — `lupo-docs/prd/00_root_constitutional_system_requirements.md`
- **KAIROS PRD** — `lupo-docs/prd/37_kairos_channel_memory_consolidation.md`
- **ROSE PRD** — `lupo-docs/prd/36_rose_multi_persona_synthetic_dialog.md`
- **Headers / THOTH** — `lupo-docs/doctrine/LUPOPEDIA_HEADERS/README.md`
- **Identity layers** — `lupo-docs/doctrine/IDENTITY_LAYERS_DOCTRINE.md`
