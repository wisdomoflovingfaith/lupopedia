...
lupopedia.headers:
  ...
  file_path_from_root: "lupo-docs/doctrine/HEADER_DB_REVERSIBILITY_DOCTRINE.md"
  web_path: "http://www.lupopedia.com/lupo-docs/doctrine/HEADER_DB_REVERSIBILITY_DOCTRINE.md"
  ...
  purpose: "Canonical definition of deterministic round-trip semantics between LUPOPEDIA HEADERS (filesystem mode) and database state (structural mode). Binding constraint for dual-mode system."
  traits: ["canonical", "doctrine", "reversibility", "dual_mode", "headers", "deterministic", "4.0.84"]
  tags: ["headers", "database", "reversibility", "deterministic", "dual_mode", "semantic_bridge", "structural_integrity"]
lupopedia.edges:
  outbound_edges:
    - { to: "lupo-docs/doctrine/LUPOPEDIA_HEADERS/README.md", type: "extends", weight: 1.0, reason: "Header format doctrine is prerequisite to reversibility rules" }
...
lupopedia.footer:
  ...
  enforcement: "binding"
  next_action:
    - "Complete generate_headers_from_db.py implementation per this doctrine's determinism rules"
    - "Implement HEADERS → DB ingestion per this doctrine's preservation rules"
    - "Add reversibility test suite (DB → HEADERS → DB produces identical state)"
---
...
Primary tool: `lupo-scripts/generate_headers_from_db.py`
