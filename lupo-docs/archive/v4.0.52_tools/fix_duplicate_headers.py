...
# =========================
# FLARE DUPLICATE HEADER FIX
# =========================

ROOT = os.getcwd()
TOOLS_DIR = "tools"
LOG = os.path.join(TOOLS_DIR, "duplicate_header_fix_log.txt")
...
log("Starting FLARE duplicate header fix")
...
def fix_duplicate_headers(content, path):
    ...
    for i, line in enumerate(lines):
        if "FLARE Header" in line:
            ...
            for j in range(i, min(i + 120, len(lines))):
                if lines[j].strip() == "---" and yaml_start == -1:
                    yaml_start = j
                elif lines[j].strip() == "---" and yaml_start != -1:
                    yaml_end = j
                    break
            
            if yaml_start != -1 and yaml_end != -1:
                flare_blocks.append((i, yaml_start, yaml_end))
                flare_starts.append(i)
    ...
    # Keep everything before the first FLARE header
    new_lines.extend(lines[:keep_start])
    
    # Keep the first complete FLARE block
...
try:
    ...
    if result.returncode == 0:
        ...
        subprocess.run(["git", "commit", "-m", "FLARE: Fixed duplicate headers in markdown files"], 
