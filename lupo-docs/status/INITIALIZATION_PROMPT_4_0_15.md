...
When instructed to bump the version to 4.0.15, you MUST update the version string in **all** of these locations (see `lupo-docs/doctrine/VERSIONING_DOCTRINE.md` §8):
...
| **lupo-database/migrations/seed_lupopedia.sql** | `@lupo_version` to `'4.0.15'`, `@lupo_version_code` to 40015. |
...
- **RELOAD** the Crafty Syntax 3.7.5 schema (e.g. from `lupo-database/migrations/old_crafty_syntax_3_7_5_start.sql` or equivalent baseline).
...
- **Versioning doctrine** — Patch-only bumps (4.0.14 → 4.0.15); single canonical file `lupo-docs/doctrine/VERSIONING_DOCTRINE.md`.
...
- **ANUBIS doctrine** — Custodial intelligence for dialogs, lineage, orphans, redirects. See lupo-docs/doctrine/ANUBIS/ANUBIS_OVERVIEW.md, ANUBIS_ORPHAN_RULES.md, ANUBIS_PROGRAM_SPEC.md.

**Canonical mapping directories:**

- **lupo-docs/doctrine/database/** — Per-table doctrine.
- **lupo-docs/doctrine/migrations/** — Legacy → Lupopedia mapping and migration pathways.
...
- **lupo-docs/doctrine/FLIP/FLIP_DOCTRINE.md** — Canonical FLIP doctrine.
- **lupo-docs/doctrine/FLIP/FLIPPING_FILE_LEXA_LILITH.md** — LEXA/LILITH roles, Part 1.4 Universal Agent Flipping, Parts 6.1–6.3 API spec/security/future auth.
...
- **lupo-scripts/import_os.py** — FLIP header extraction; path validation; optional dialog block → dialog_notes; optional fields (mood_rgb, tags, atoms) → lupo_contents.metadata_json via json.dumps; parameterized SQL only.
- **lupo-tools/generate_flip_header.py** — Reconstructs FLIP header from DB; `--web` flag for JSON output (API-compatible); parameterized SQL only.
...
- **lupo-api/flip-header.php** — GET endpoint. Params: `path`, `url`, or `content_id` (precedence: path > url > content_id). Output: JSON `{header, resolved, channel_id}`; `?format=yaml` for raw YAML. HTTP 400/404/500. LEXA security: parameterized SQL, path validation. CORS enabled.
- **lupo-docs/api/FLIP_API.md** — Full API documentation.
...
- **lupo_dialog_channels:** message_count = 31; file_source = 'lupo-docs/doctrine/FLIP/FLIPPING_FILE_LEXA_LILITH.md'.
...
- Doctrine: lupo-docs/doctrine/ANUBIS/ANUBIS_OVERVIEW.md (role: custodial for dialogs/orphans/redirects); ANUBIS_ORPHAN_RULES.md (orphan def, resolution order, adoption to channel 42/thread 1/actor 3); ANUBIS_PROGRAM_SPEC.md (Python/PHP specs, tables used, no schema changes).
- Programs: lupo-tools/anubis_orphan_scanner.py (scanner/resolver/adoption planner, no-DB mode, parameterized); lupo-includes/classes/ANUBIS_Resolver.php (PHP 5.3: classifyOrphan, resolveParent, adoptIntoSeed via PDO_DB).
...
- **Use lupo-legacy\craftysyntax only as reference** — read for behavior; never modify.
...
- **Modifying lupo-legacy\craftysyntax** — Reference only.
