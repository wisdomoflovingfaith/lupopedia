# LUPOPEDIA HEADERS (replaces FLARE) — see http://www.lupopedia.com/status/CURSOR_FLARE_APPLY_LINK_CHECK_4.0.57
...
- **flare_see.py:** Builds `artifacts/index/flame_see_index.json` from `lupopedia.see` mappings in markdown files. No markdown file currently declares a `lupopedia.see` mapping for `http://www.lupopedia.com/flare_apply`.
...
| `lupo see http://www.lupopedia.com/flare_apply` | **No** (no lupopedia.see mapping) |

**Action:** Create mapping end-to-end: markdown doc, FLARE header with lupopedia.see, route for slug `flare_apply`, and seed row in `lupo_contents` (and optionally `lupo_channel_content` if needed). Ensure `content_resolve_body_from_file` can serve the doc via `file_path_from_root`.
