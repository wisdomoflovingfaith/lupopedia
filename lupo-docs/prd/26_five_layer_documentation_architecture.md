...
lupopedia.headers:
  ...
  last_modified_utc: "20260403024041"
...
lupopedia.footer:
  ...
  next_action:
    - "Keep validate_implementation.py aligned with doc_arch_version and filesystem-only checks"
    - "Add doc_arch_version to implementations that lack it"
    - "Migrate legacy implementations within 90-day window where applicable"
---
...
prd_slug: five_layer_documentation_architecture
title: "Five-Layer Documentation Architecture"
status: "draft|review|approved|implemented|active|deprecated"
...
- **`content_id`** is the **`lupo_contents.content_id`** BIGINT — **application-layer** assignment on import, **not** hand-authored to match **`prd_id`** or other IDs (see [PRD 16](16_lupopedia_headers.md)).
...
### 6.1 `doc_arch_version` and Release & Migration Doctrine (4.0.x)

Until **4.1.0**, there is **no** Lupopedia→Lupopedia upgrade path and **no** in-place migration of documentation **folder trees** driven by a SQL migration. When **`doc_arch_version`** increments:
...
Do not interpret **`doc_arch_version`** as requiring a database migration script before 4.1.0.
