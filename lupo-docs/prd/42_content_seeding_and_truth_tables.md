...
lupopedia.headers:
  ...
  file_path_from_root: "lupo-docs/prd/42_content_seeding_and_truth_tables.md"
  web_path: "http://www.lupopedia.com/lupopedia/lupo-docs/prd/42_content_seeding_and_truth_tables.md"
  last_modified_utc: "20260408012727"
  federation_node_id: 0
  channel_id: 42
  thread_id: "prd-42-content-truth-seeding"
  prd_id: 42
  prd_slug: content_seeding_and_truth_tables
  author:
    type: "actor"
    id: 102
    name: "CURSOR"
  ...
  artifact_type: prd
  artifact_kind: product_requirements
  purpose: "Define online help, content pages, truth Q&A tables, seeding, edges, and promotion/consolidation lifecycle using only install+seed facts"
  status: "draft"
  tags:
    - prd
    - content
...
lupopedia.edges:
  outbound_edges:
    - to: "lupo-docs/prd/00_root_constitutional_system_requirements.md"
      type: references
      weight: 1.0
      reason: "PK generation, soft delete, no FKs — §3.2.1, §3.7 Chronological Trust Ladder where IdGenerator applies"
    - to: "lupo-docs/prd/01_core_identity.md"
      type: references
      weight: 0.9
      reason: "Actor and auth context for asked_by / answered_by columns"
    - to: "lupo-docs/prd/03_truth_knowledge.md"
      type: references
      weight: 1.0
      reason: "Truth and knowledge PRD — companion domain"
    - to: "lupo-docs/prd/15_actors.md"
      type: references
      weight: 0.9
      reason: "Actor-scoped content and help"
    - to: "lupo-docs/prd/24_cli_interface_prd.md"
      type: references
      weight: 0.85
      reason: "Future CLI for content/truth maintenance may mirror memory/archive patterns"
    - to: "lupo-docs/prd/37_kairos_channel_memory_consolidation.md"
      type: references
      weight: 0.8
      reason: "Distinction: actor memory graph vs public content/truth surfaces"
    - to: "lupo-docs/prd/38_memory_unification.md"
      type: references
      weight: 0.8
      reason: "Memory nodes vs contents — separate tables; shared edge discipline"
    - to: "lupo-docs/prd/41_install_seed_doctrine.md"
      type: references
      weight: 1.0
      reason: "Install seed tier vs living canonical vs staging — content/truth must follow same doctrine at allocation time"
    - to: "lupo-database/lupopedia/mysql/install/install_new_lupopedia.sql"
      type: references
      weight: 1.0
      reason: "Canonical DDL for all table and column names in this PRD"
    - to: "lupo-database/lupopedia/mysql/seed/seed_online_help_and_content.sql"
      type: references
      weight: 1.0
      reason: "Current help/content seed script — behavior and IDs documented in §5–§6"
lupopedia.footer:
  ...
  next_action:
    - "Reconcile seed_online_help_and_content.sql with install schema (truth_* vs legacy question/answer table names)"
    - "Align seed content_id bands with PRD 41 tier-0 policy or document explicit exception"
    - "Specify storage_type vs file_path_from_root vs content column invariants for file_backed help rows"
---

# file: PRD 42 — Content seeding & truth tables — web_path: [http://www.lupopedia.com/lupopedia/lupo-docs/prd/42_content_seeding_and_truth_tables.md](http://www.lupopedia.com/lupopedia/lupo-docs/prd/42_content_seeding_and_truth_tables.md)
...
- **Seeding at install** via `seed_online_help_and_content.sql` (and any consolidated bundle that embeds it, e.g. `install/seed_lupopedia_4_1_0.sql`).
...
- **`lupo-database/lupopedia/mysql/install/install_new_lupopedia.sql`** (with `{{prefix}}` → `lupo_` in a default install).
...
### 3.4 `lupo_documentation_frameworks`

**Purpose:** Registry of **documentation frameworks** (namespace, channel, collection keys, optional `database_table`, PHP floor, etc.).

**Primary key:** `documentation_framework_id`.

**Key columns:** `framework_key`, `framework_name`, `class_type`, `namespace_key`, `channel_id`, `collection_key`, `orchestrator_actor_id`, `facet_slug`, `agent_key`, `role_key`, `task_scope`, `database_table`, `runtime_min_php`, `created_ymdhis`, `updated_ymdhis`, `is_deleted`, `deleted_ymdhis`, `properties_json`.

### 3.5 Truth stack

#### `lupo_truth_questions`

**Purpose:** **Questions** anchored to a target entity via `target_object_type` + `target_object_id` (e.g. `content` + `content_id` — see runtime joins in `lupo-includes/modules/truth/truth-model.php`).
...
### 3.7 Supporting / adjacent content tables (install)
...
- Inserts rows with explicit **`content_id`** (`1000001` …), **`federation_node_id`**, **`channel_id`**, **`actor_id`**, **`title`**, **`slug`**, **`description`**, **`body`**, **`content_type`** (`help_guide`), **`format`** (`markdown`), **`status`**, **`visibility`**, **`created_ymdhis`**, **`updated_ymdhis`**, **`utc_cycle`**, **`tags`**, **`hashtags`**, **`file_path_from_root`** pointing under `lupo-content/0/help_documentation/`.
...
**Runtime note:** `lupo-includes/modules/content/content-controller.php` and `edge-controller.php` join **`lupo_edges`** to **`lupo_contents`** when object types are **`content`**.
