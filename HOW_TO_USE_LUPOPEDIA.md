...
graph TD
    ...
    PROMPTS --> KIRO_P[kiro/]
    PROMPTS --> ANTI_P[antigravity/]
    
    LUPO --> LOCKS[locks/]
    LUPO --> STATE[state/]
    
    style DOCTRINE fill:#ff6b6b
    style VSX fill:#4ecdc4
    style CH42 fill:#ffe66d
    style LUPO fill:#95e1d3
```
...
```mermaid
graph LR
    QUICK[QUICKSTART.md] -->|references| SEC[SECURITY_DOCTRINE.md]
    AUDIT[security_audit_4_1_0.md] -->|validates| SEC
    BROADCAST[20260224_security_review.md] -->|announces| SEC
    
    SEC -->|implements| AUTH[AuthService.php]
    SEC -->|implements| SESS[Session.php]
    SEC -->|implements| ROLE[AuthRoleResolver.php]
    
    SEC -.->|supersedes| OLD[OLD_SECURITY_DOCTRINE.md]
    
    LILITH[LILITH 2038] -.->|reviewed| SEC
    LEXA[LEXA 24] -.->|reviewed| SEC
    
    style SEC fill:#ff6b6b,stroke:#333,stroke-width:4px
    style AUTH fill:#4ecdc4
    style SESS fill:#4ecdc4
    style ROLE fill:#4ecdc4
    style OLD fill:#95a5a6,stroke-dasharray: 5 5
```
