...
#   file_path_from_root: "database/lupopedia/content/app/Services/ActorService.php"
...
class ActorService
{
     ...
     * 18-digit deterministic IDs use actors/YYYY/MM/actor_id.
     * Legacy IDs use actors/actor_id.
     *
    ...
    /**
     * Path to actor registry (actor_name primary). Prefer database/lupopedia/actors/registry.json.
     *
     * @return string
     */
    protected function getRegistryPath()
    {
        ...
        $reg = $base . DIRECTORY_SEPARATOR . 'database' . DIRECTORY_SEPARATOR . 'lupopedia' . DIRECTORY_SEPARATOR . 'actors' . DIRECTORY_SEPARATOR . 'registry.json';
     ...
     * Returns registry dir (e.g. actors/0, actors/111) or fallback actors/{actor_name}.
     *
     * @param string $actor_name
     * @return string
     */
    ...
    public function getActorsUserCanActAs($authUserId, $isAdmin = false)
    {
        ...
        $path = rtrim(LUPOPEDIA_PATH, '/\\') . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'classes' . DIRECTORY_SEPARATOR . 'AuthSessionManager.php';
        if (!is_file($path)) {
            return array();
        }
        require_once $path;
    ...
    public function createActorForAuthUser($authUserId, $email, $displayName)
    {
        ...
        try {
            ...
            if ($ok !== false) {
                // Provision filesystem workspace under actors/YYYY/MM/actor_id.
                $app_root = defined('LUPOPEDIA_PATH') ? LUPOPEDIA_PATH : (defined('ABSPATH') ? ABSPATH : '');
    ...
    /**
     * Read agent role from blueprint, defaulting to watcher.
     *
    ...
    {
        $reader = new AgentBlueprintReader();
        return $reader->getAgentRole($agentKey);
    }

    /**
     * Create actor from blueprint with schema-backed role fields.
     *
     * @param string $agentKey
     * @param string $actorName
     * @return int|false
     */
    public function createActorFromBlueprint($agentKey, $actorName)
    {
        if (!is_string($agentKey) || trim($agentKey) === '' || !is_string($actorName) || trim($actorName) === '') {
            return false;
        }
        ...
        $reader = new AgentBlueprintReader();
        $role = $reader->getAgentRole($agentKey);
        $blueprintPath = $reader->getBlueprintPath($agentKey);
        if ($blueprintPath === null) {
            $blueprintPath = 'lupo-agents/' . $agentKey . '/';
        }

        $now = class_exists('\timestamp_ymdhis') ? \timestamp_ymdhis::now() : (int) gmdate('YmdHis');
        $slug = strtolower($actorName);
        $slug = preg_replace('/[^a-z0-9]+/', '-', $slug);
        $slug = trim($slug, '-');
        if ($slug === '') {
            $slug = strtolower($agentKey);
        }

        $table = $this->db->quoteIdentifier($this->prefix . 'actors');
        for ($attempt = 0; $attempt < 10; $attempt++) {
            $actorId = class_exists('\IdGenerator') ? \IdGenerator::generate() : (gmdate('YmdHis') . str_pad((string) mt_rand(0, 9999), 4, '0', STR_PAD_LEFT));
            $exists = $this->db->fetchRow(
            ...
            $ok = $this->db->insert($this->prefix . 'actors', array(
                ...
                'agent_key' => $agentKey,
                'agent_role' => $role,
                'agent_blueprint_path' => $blueprintPath,
                'created_ymdhis' => $now,
                'updated_ymdhis' => $now,
                'is_active' => 1,
                'is_deleted' => 0
