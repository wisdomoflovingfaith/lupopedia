...
class SavedCollectionsService
{
    ...
    public function renderSavedCollections(int $userId): array
    {
        ...
        if ($userId === 0) {
            ...
            if (empty($collections)) {
                ...
                if ($actorId > 0) {
                    $collections = $this->getCollectionsForActorLinks($actorId, $collT);
                }
            }
            if (empty($collections)) {
                $collections = $this->db->fetchAll(
    ...
    public function loadTabChildren(int $tabId): array
    {
        ...
        foreach ($rows as $mapping) {
            ...
            if ($itemType === 'tab') {
                ...
                $content = $this->db->fetchRow("SELECT content_id, title, slug FROM {$contT} WHERE content_id = :content_id LIMIT 1", ['content_id' => $itemId]);
                if ($content) {
     ...
     */
    /**
     * Collections linked to an actor via lupo_actor_collections (read access).
     *
     * @param int $actorId
     * @param string $collT Quoted collections table name
     * @return array Rows collection_id, name, slug
     */
    private function getCollectionsForActorLinks($actorId, $collT)
    {
        if ($actorId <= 0) {
            return [];
        }
        $acT = $this->db->quoteIdentifier($this->prefix . 'actor_collections');
        return $this->db->fetchAll(
            "SELECT DISTINCT c.collection_id, c.name, c.slug FROM {$collT} c INNER JOIN {$acT} ac ON c.collection_id = ac.collection_id AND ac.actor_id = :actor_id WHERE (c.is_deleted = 0 OR c.is_deleted IS NULL) AND (ac.is_deleted = 0 OR ac.is_deleted IS NULL) ORDER BY c.slug, c.name",
