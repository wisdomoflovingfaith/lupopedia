...
class AuthRoleResolver
{
    ...
    /**
     * Reserved main admin (auth_user_id / actor_id 10000): idempotent admin capability rows.
     *
     * @param int $actorId Must be 10000
     * @return void
     */
    public function ensureReservedMainAdminCapabilities($actorId)
    {
        $actorId = (int) $actorId;
        if ($actorId !== 10000) {
            return;
        }
        foreach (array(0, 1, 42, 51) as $channelId) {
            $this->grantChannelRoleIfMissing($actorId, (int) $channelId, 'captain');
        }
        $this->grantSystemAdministratorIfMissing($actorId);
    }

    /**
     * Fix broken installs: WOLFIE (actor 1) must not win login routing over captain (actor 10000).
     *
     * @param int $authUserId
     * @return void
     */
    public function repairMainAdminAuthUserRouting($authUserId)
    {
        ...
        $prefix = defined('LUPO_TABLE_PREFIX') ? LUPO_TABLE_PREFIX : 'lupo_';
        $aau = $this->db->quoteIdentifier($prefix . 'actor_auth_users');
        $now = (int) gmdate('YmdHis');
        $this->db->execute(
            "UPDATE {$aau} SET relationship_role = 'primary_owner', is_primary = 1, routing_priority = 0, status = 'active', updated_ymdhis = :now
             WHERE auth_user_id = :aid AND actor_id = 10000 AND (is_deleted = 0 OR is_deleted IS NULL)",
            array('now' => $now, 'aid' => $authUserId)
        );
        $this->db->execute(
            "UPDATE {$aau} SET relationship_role = 'delegate', is_primary = 0, routing_priority = 200, status = 'active', updated_ymdhis = :now
             WHERE auth_user_id = :aid AND actor_id = 1 AND (is_deleted = 0 OR is_deleted IS NULL)",
            array('now' => $now, 'aid' => $authUserId)
        );
    }
