...
lupopedia.headers:
  ...
  path_from_lupopedia_root: database/lupopedia/pending_tasks_dialog_fallback_4.0.69.md
  web_path: https://www.lupopedia.com/lupopedia/database/lupopedia/pending_tasks_dialog_fallback_4.0.69.md
  status: null
...
1. **TraitEnforcer class** — `includes/classes/TraitEnforcer.php`: `actorHasTrait($actor_id, $trait_key, $federation_node_id)`, `isActionAuthorized($actor_id, $action_key, $channel_id)`; PDO_DB only; PHP 5.3 compatible.
...
5. **SessionCustodian** — Optional tool for Antigravity: audit/correct `database/sessions/*.md` (e.g. paired_actor_id drift); report only or auto-correct with dry_run.
