...
lupopedia.conditional:
  ...
  brief:
    ...
    where:
      repo_paths: ["database\lupopedia\channels\channels\42\plans\plan-4.0.55.md"]
      runtime_scope: "cli"
      channels:
...
lupopedia.headers:
  ...
  file_path_from_root: "database/lupopedia/channels/channel_id/42/plans/plan-4.0.55.md"
  ...
  tags: ["database", "lupopedia", "channels", "channels", "42", "plans"]
...
lupopedia.see:
  mappings:
    - ["database\lupopedia\channels\channels\42\plans\plan-4.0.55.md", "http://www.lupopedia.com/PLAN-4.0.55"]
...
---
wolfie.headers: {
  file_path_from_root: "channels/42/plans/plan-4.0.55.md",
...
flip.footer: {
  outbound_edges: [
    { to: "channels/42/tasks/active/", type: "references" },
    { to: "channels/42/threads/DEVELOPMENT_CYCLE_4_0_55/", type: "references" },
    { to: "channels/42/collections/", type: "references" }
  ],
...
- **Database Fallback System**: Introduce the `database/` directory as a file-based secondary.
- **Recursive Channel Migration**: Fully relocate `channels/` and its entire subfolder tree (`tasks/`, `plans/`, `threads/`, etc.) into the new structure.
...
A new top-level `database/` directory will serve as the root for all file-based "Root Truth" data.
- **Migration Target**: `database/lupopedia/channels/`
- **Migration Source**: `wholesale move of channels/` (Every file/folder).
...
- **Mitigation**: Create a full backup of the `channels/` tree before execution and verify file counts post-move.
