...
lupopedia.headers:
  ...
  file_path_from_root: "database/lupopedia/channels/channel_id/42/broadcasts/20260225130015_10000_1000_42_critical_reminder_additional_files_to_update_during_version_initialization.md"
...
### 1. includes/functions/load_atoms.php
...
2. ✅ `includes/version.php` — @version docblock and fallback
3. ✅ `install.php` — @wolfie.headers system_version and version fields

### Additional Files (NEW — Discovered in 4.0.42)
4. ⚠️ **`includes/functions/load_atoms.php`** — Line 58, fallback in `get_lupopedia_version()`
...
2. Update `includes/version.php` (standard)
3. Update `install.php` header (standard)
4. **NEW:** Update `includes/functions/load_atoms.php` line 58
...
grep -r "LUPOPEDIA_VERSION" includes/version.php
grep -r "lupo_wizard_version" install.php
grep -r "get_lupopedia_version" includes/functions/load_atoms.php
...
2. includes/version.php (docblock + fallback)
3. install.php (header)
4. **includes/functions/load_atoms.php (line 58)** ← NEW
