...
CREATE TABLE {{prefix}}actors (
  ...
  agent_role       varchar(32)   NOT NULL DEFAULT 'watcher',
  agent_blueprint_path varchar(512) DEFAULT NULL,
  current_channel_key varchar(255) DEFAULT NULL,
  current_thread_id varchar(255) DEFAULT NULL,
  ...
  -- PRD 02_C-i_CHANNELS_DISCUSSIONS CCV (Channel Coherence Vector): optional actor-level semantic / mood centroid cache
  ccv_actor_centroid_embedding_json json DEFAULT NULL COMMENT 'PRD 02_C CCV opaque float array JSON; m_obj / actor centroid family',
...
CREATE TABLE {{prefix}}actor_pairing (
  ...
  pairing_role varchar(32) NOT NULL DEFAULT 'peer',
  pairing_type varchar(32) NOT NULL DEFAULT 'operational',
...
-- [EJECTED 4.1.2] actor_relationships removed
...
-- [EJECTED 4.1.2] actor_apps removed
...
-- See docs/doctrine/DECISION_MODEL.md for the canonical model.
...
-- [EJECTED 4.1.2] agent_performance_stats removed
-- Orphaned indexes also removed
...
-- [EJECTED 4.1.2] agent_tools removed

-- ============================================================================
-- SECTION 8: AGENT BOUNDARIES (domain boundary definitions per agent)
-- ============================================================================

-- [EJECTED 4.1.2] agent_boundaries removed
...
-- [EJECTED 4.1.2] actor_versions removed

-- ============================================================================
-- SECTION 13: AGENT DEFINITION VERSIONS
-- ============================================================================

-- [EJECTED 4.1.2] agent_definition_versions removed
-- Orphaned indexes also removed
...
-- KAIROS canonical storage: {{prefix}}memory_nodes + {{prefix}}memory_edges only (no legacy kairos_* tables in 4.0.x install).
...
-- [EJECTED 4.1.2] actor_runtime_state removed

-- Runtime lifecycle events (activate, deactivate, handoff, error, recover)
-- [EJECTED 4.1.2] actor_runtime_events removed
...
-- [EJECTED 4.1.2] faucet_rules removed
...
-- [EJECTED 4.1.2] pairing_rules removed
...
-- [EJECTED 4.1.2] department_capabilities removed
-- Orphaned indexes also removed
...
-- [EJECTED 4.1.2] identity_context removed
...
-- [EJECTED 4.1.2] agent_memory_config removed
...
-- PATHS/VISITS doctrine (4.0.86+): layered raw vs aggregate (Crafty Syntax pattern).
--   {{prefix}}visits - per-hit page events (path_url, session_id, is_processed).
--   {{prefix}}paths_raw - optional URL-pair transitions (from_url -> to_url), append-only; is_aggregated for rollup to paths_daily.
--   {{prefix}}paths_daily / {{prefix}}paths_monthly - URL-pair rollups with optional from_content_id / to_content_id (Semantic Widget).
--   {{prefix}}paths - content-id aggregates (entercontentid/exitcontentid); Crafty import + gc from visits.
--   {{prefix}}referers_raw - optional per-hit referrer events; roll up into {{prefix}}referers / {{prefix}}referers_daily.
-- All PKs application-assigned (IdGenerator); no AUTO_INCREMENT. Timestamps BIGINT UTC YmdHis only.
...
-- URL-pair transition stream (high volume). Purge after aggregation is a product policy (maintenance-tier);
-- prefer soft-delete columns for audit unless operator enables physical delete. See PRD 11 / PRD 51.
CREATE TABLE {{prefix}}paths_raw (
  path_raw_id bigint NOT NULL,
  from_url varchar(2000) DEFAULT NULL,
  to_url varchar(2000) DEFAULT NULL,
  session_id bigint DEFAULT NULL,
  actor_id bigint DEFAULT NULL,
...
CREATE INDEX {{prefix}}paths_raw_idx_created_agg ON {{prefix}}paths_raw (created_ymdhis, is_aggregated);
CREATE INDEX {{prefix}}paths_raw_idx_session ON {{prefix}}paths_raw (session_id);
CREATE INDEX {{prefix}}paths_raw_idx_is_deleted ON {{prefix}}paths_raw (is_deleted);
...
-- Per-hit referrer stream (optional). Rolls up into {{prefix}}referers / {{prefix}}referers_daily in application GC.
...
CREATE INDEX {{prefix}}referers_raw_idx_created_agg ON {{prefix}}referers_raw (created_ymdhis, is_aggregated);
CREATE INDEX {{prefix}}referers_raw_idx_domain ON {{prefix}}referers_raw (referer_domain);
CREATE INDEX {{prefix}}referers_raw_idx_session ON {{prefix}}referers_raw (session_id);
CREATE INDEX {{prefix}}referers_raw_idx_is_deleted ON {{prefix}}referers_raw (is_deleted);

-- [EJECTED 4.1.2] anubis_log removed
...
CREATE TABLE {{prefix}}auth_users (
  ...
  -- Two
  -- [EJECTED 4.1.2] two_factor_audit removed

  -- Magic link authentication
  -- [EJECTED 4.1.2] magic_link_tokens removed

  -- Rate limiting for authentication
  -- [EJECTED 4.1.2] auth_rate_limits removed
...
-- PRD 02_C-i_CHANNELS_DISCUSSIONS CCV -- per-channel centroid aggregate (alternative to repeating centroid_snapshot on every row)
-- channel_centroid_id NOT AUTO_INCREMENT; application supplies explicit IDs (reserved ID doctrine).
...
-- [EJECTED 4.1.2] channel_escalations removed

-- [EJECTED 4.1.2] channel_escalation_rules removed
...
-- [EJECTED 4.1.2] channel_state removed
...
CREATE TABLE {{prefix}}contents (
  ...
  -- PRD 02_C-i_CHANNELS_DISCUSSIONS CCV (channel-scoped content when channel_id set)
  coherence_magnitude decimal(5,4) DEFAULT NULL COMMENT 'PRD 02_C CCV cosine magnitude m in [-1,1]',
  pono_kapu_state varchar(16) DEFAULT NULL COMMENT 'PRD 02_C: pono|kapu|neutral relative to channel centroid',
  centroid_snapshot json DEFAULT NULL COMMENT 'PRD 02_C: frozen channel centroid c at ccv_computed_ymdhis',
  ccv_embedding_vector json DEFAULT NULL COMMENT 'PRD 02_C: object embedding o as opaque float array JSON',
  embedding_model_id varchar(64) DEFAULT NULL COMMENT 'PRD 02_C CCV embedding model id',
  ccv_computed_ymdhis bigint DEFAULT NULL COMMENT 'PRD 02_C CCV BIGINT UTC YYYYMMDDHHIISS',
  deviation_vector json DEFAULT NULL COMMENT 'PRD 02_C CCV optional (o - c) deviation signature',
...
-- [EJECTED 4.1.2] legacy_content_mapping removed
...
-- MUST be < 10000. IDs 0 (system root), 10000 (main admin), and 10001 (red team) are
-- system-reserved and must NEVER appear in this table.
-- No CHECK constraint per DB=Storage Only doctrine.
-- PHP must validate before every INSERT and UPDATE. See PRD 85.
...
CREATE INDEX {{prefix}}crafty_user_mapping_idx_created ON {{prefix}}crafty_user_mapping (created_ymdhis);
...
-- [EJECTED 4.1.2] channel_departments removed

-- PRD 02_A-i_CHANNELS_DB_DESIGN section "Agent-Specific Color Override (Optional)" -- Agent Colors Table Schema
-- Per-actor UI color overrides (hex). actor_id is NOT AUTO_INCREMENT; application supplies explicit IDs (reserved ID doctrine).
...
CREATE TABLE {{prefix}}dialog_messages (
  ...
  channel_id bigint DEFAULT NULL,
  channel_key varchar(255) NOT NULL DEFAULT '',
  from_actor_id bigint DEFAULT NULL,
  from_session_id varchar(128) DEFAULT NULL,
  ...
  to_session_id varchar(128) DEFAULT NULL,
  ...
  metadata_json json DEFAULT NULL,
  mood_vector char(6) DEFAULT NULL,
  mood_framework varchar(32) NOT NULL DEFAULT 'western_analytical',
  -- PRD 02_C-i_CHANNELS_DISCUSSIONS CCV (dialog_message channel-scoped rows)
  coherence_magnitude decimal(5,4) DEFAULT NULL COMMENT 'PRD 02_C CCV cosine magnitude m in [-1,1]',
  pono_kapu_state varchar(16) DEFAULT NULL COMMENT 'PRD 02_C: pono|kapu|neutral relative to channel centroid',
  centroid_snapshot json DEFAULT NULL COMMENT 'PRD 02_C: frozen channel centroid c at ccv_computed_ymdhis',
  ccv_embedding_vector json DEFAULT NULL COMMENT 'PRD 02_C: object embedding o as opaque float array JSON',
...
CREATE INDEX {{prefix}}dialog_messages_idx_channel_key ON {{prefix}}dialog_messages (channel_key);
...
-- No dedicated from_session_id / to_session_id indexes here: add only at measured scale (PRD 02_channels_db_design.md).

-- PRD 02_A-i_CHANNELS_DB_DESIGN section "Recent Files Browser/Table" -- lupo_dialog_recent_files
-- recent_file_id is NOT AUTO_INCREMENT; application supplies explicit IDs (reserved ID doctrine).
...
-- lupo_dialog_pending_tasks: captures agent tasks extracted from the chat stream for orchestration tracking
CREATE TABLE {{prefix}}dialog_pending_tasks (
  ...
  PRIMARY KEY (task_id),
  KEY dialog_pending_tasks_idx_assignee (assignee_actor_id, status),
  KEY dialog_pending_tasks_idx_channel (channel_id, status)
);

-- lupo_prompts: saved prompt artifacts (library, refine, dispatch) — not ephemeral chat lines
CREATE TABLE {{prefix}}prompts (
  ...
  created_by_actor_id BIGINT NOT NULL,
  created_ymdhis BIGINT NOT NULL,
  last_updated_ymdhis BIGINT NOT NULL,
  updated_ymdhis BIGINT NOT NULL,
  channel_key VARCHAR(64) NOT NULL DEFAULT '',
  thread_id VARCHAR(64) NOT NULL DEFAULT '',
  is_deleted TINYINT NOT NULL DEFAULT 0,
  deleted_ymdhis BIGINT DEFAULT NULL,
  -- PRD 02_C-i_CHANNELS_DISCUSSIONS CCV (channel_key-scoped prompts)
  coherence_magnitude DECIMAL(5,4) DEFAULT NULL COMMENT 'PRD 02_C CCV cosine magnitude m',
  pono_kapu_state VARCHAR(16) DEFAULT NULL COMMENT 'PRD 02_C: pono|kapu|neutral',
  centroid_snapshot JSON DEFAULT NULL COMMENT 'PRD 02_C: frozen centroid snapshot JSON',
  ccv_embedding_vector JSON DEFAULT NULL COMMENT 'PRD 02_C: object embedding o JSON',
  embedding_model_id VARCHAR(64) DEFAULT NULL COMMENT 'PRD 02_C CCV embedding model id',
  ccv_computed_ymdhis BIGINT DEFAULT NULL COMMENT 'PRD 02_C CCV BIGINT UTC YYYYMMDDHHIISS',
  deviation_vector JSON DEFAULT NULL COMMENT 'PRD 02_C CCV optional deviation signature',
  PRIMARY KEY (prompt_id),
  KEY prompts_idx_channel_updated (channel_key, last_updated_ymdhis),
  KEY prompts_idx_channel_status (channel_key, status, is_deleted),
  KEY prompts_idx_creator (created_by_actor_id, is_deleted)
);
...
CREATE TABLE {{prefix}}dialog_threads (
  ...
  -- PRD 02_C-i_CHANNELS_DISCUSSIONS CCV (thread-level cached aggregates / thread centroid family)
  coherence_magnitude decimal(5,4) DEFAULT NULL COMMENT 'PRD 02_C CCV thread-level cosine magnitude m',
  pono_kapu_state varchar(16) DEFAULT NULL COMMENT 'PRD 02_C: pono|kapu|neutral relative to channel centroid',
  centroid_snapshot json DEFAULT NULL COMMENT 'PRD 02_C: thread partial centroid or snapshot JSON',
  ccv_embedding_vector json DEFAULT NULL COMMENT 'PRD 02_C: thread aggregate embedding cache JSON',
  embedding_model_id varchar(64) DEFAULT NULL COMMENT 'PRD 02_C CCV embedding model id',
  ccv_computed_ymdhis bigint DEFAULT NULL COMMENT 'PRD 02_C CCV BIGINT UTC YYYYMMDDHHIISS',
  deviation_vector json DEFAULT NULL COMMENT 'PRD 02_C CCV optional deviation signature',
...
CREATE UNIQUE INDEX {{prefix}}dialog_threads_uq_node_channel_thread_key ON {{prefix}}dialog_threads (federation_node_id, channel_id, thread_key);

-- lupo_dialog_read_log: tracks per-actor read position within a channel thread
CREATE TABLE {{prefix}}dialog_read_log (
  ...
  last_read_created_ymdhis   bigint   NOT NULL,
  created_ymdhis             bigint   NOT NULL,
  updated_ymdhis             bigint   NOT NULL,
  is_deleted                 tinyint  NOT NULL DEFAULT 0,
  deleted_ymdhis             bigint   DEFAULT NULL,
...
CREATE UNIQUE INDEX {{prefix}}dialog_read_log_uq_actor_thread ON {{prefix}}dialog_read_log (actor_id, dialog_thread_id);
CREATE INDEX {{prefix}}dialog_read_log_idx_actor ON {{prefix}}dialog_read_log (actor_id);
CREATE INDEX {{prefix}}dialog_read_log_idx_channel ON {{prefix}}dialog_read_log (channel_id);
CREATE INDEX {{prefix}}dialog_read_log_idx_thread ON {{prefix}}dialog_read_log (dialog_thread_id);
CREATE INDEX {{prefix}}dialog_read_log_idx_updated ON {{prefix}}dialog_read_log (updated_ymdhis);
CREATE INDEX {{prefix}}dialog_read_log_idx_deleted ON {{prefix}}dialog_read_log (is_deleted);
...
-- [EJECTED 4.1.2] emotional_frameworks removed
...
CREATE INDEX {{prefix}}federation_nodes_idx_node_base_url ON {{prefix}}federation_nodes (base_url);
...
-- [EJECTED 4.1.2] help_tree removed
...
-- Export path: memory/YYYY/MM/{slug}.json - slug from PHP (see MemoryExportService).
...
CREATE TABLE {{prefix}}memory_nodes (
  ...
  memory_toon      varchar(255)  NOT NULL,
  ...
  context_json     json          DEFAULT NULL,
  channel_key      varchar(128)  DEFAULT NULL,
  updated_ymdhis   bigint        NOT NULL DEFAULT 0,
...
CREATE INDEX {{prefix}}memory_nodes_idx_key ON {{prefix}}memory_nodes (memory_toon, owner_actor_id);
...
CREATE INDEX {{prefix}}idx_memory_nodes_channel_key ON {{prefix}}memory_nodes (channel_key, is_deleted);
...
CREATE INDEX {{prefix}}idx_memory_edges_channel_key ON {{prefix}}memory_edges (channel_key, is_deleted);
...
-- [EJECTED 4.1.2] semantic_index removed
...
CREATE TABLE {{prefix}}sessions (
  ...
  -- Visitor / Live Help columns (Crafty compatibility + mixed participant support)
  visitor_status ENUM('browsing','invited','chatting','stopped') NOT NULL DEFAULT 'browsing',
  livehelp_department bigint NOT NULL DEFAULT 0,
  onchannel bigint NOT NULL DEFAULT 0,
  user_alert tinyint NOT NULL DEFAULT 0,
  auto_invite ENUM('Y','N') NOT NULL DEFAULT 'N',
  istyping int NOT NULL DEFAULT 0,
  livehelp_user_id bigint DEFAULT NULL,
  livehelp_sessionid varchar(128) DEFAULT NULL,
...
-- No extra visitor-list indexes here: add only at measured scale and for proven queries (PRD 02_channels_db_design.md).
...
-- [EJECTED 4.1.2] truth_context_map removed

-- TABLE 5: Truth Question Followers
-- [EJECTED 4.1.2] truth_followers removed

-- URL-pair daily rollup (from {{prefix}}paths_raw and/or derived from {{prefix}}visits). Query-optimized; content_id columns for widget.
CREATE TABLE {{prefix}}paths_daily (
  path_daily_id bigint NOT NULL,
  from_url varchar(2000) DEFAULT NULL,
  to_url varchar(2000) DEFAULT NULL,
  from_content_id bigint DEFAULT NULL,
  to_content_id bigint DEFAULT NULL,
...
CREATE INDEX {{prefix}}paths_daily_idx_date ON {{prefix}}paths_daily (date_ymd);
CREATE INDEX {{prefix}}paths_daily_idx_from_content ON {{prefix}}paths_daily (from_content_id, date_ymd);
CREATE INDEX {{prefix}}paths_daily_idx_to_content ON {{prefix}}paths_daily (to_content_id, date_ymd);
CREATE INDEX {{prefix}}paths_daily_idx_is_deleted ON {{prefix}}paths_daily (is_deleted);

-- URL-pair monthly rollup (long-horizon trends). date_ym = YYYYMM int.
...
CREATE INDEX {{prefix}}paths_monthly_idx_date_ym ON {{prefix}}paths_monthly (date_ym);
CREATE INDEX {{prefix}}paths_monthly_idx_from_content ON {{prefix}}paths_monthly (from_content_id, date_ym);
CREATE INDEX {{prefix}}paths_monthly_idx_to_content ON {{prefix}}paths_monthly (to_content_id, date_ym);
CREATE INDEX {{prefix}}paths_monthly_idx_is_deleted ON {{prefix}}paths_monthly (is_deleted);

-- {{prefix}}paths: content-id aggregated navigation flows (low-volume). Populated by gc.php from {{prefi
...
-- [EJECTED 4.1.2] paths_summary removed

-- Reference Map (Explicit mapping table, 4.0.86)
-- [EJECTED 4.1.2] reference_map removed

-- Collection Links (Explicit link objects within collections, 4.0.86)
-- [EJECTED 4.1.2] collection_links removed
...
-- See: includes/classes/IdGenerator.php
...
-- [EJECTED 4.1.2] world_registry removed
...
-- [EJECTED 4.1.2] actor_skills removed

-- Table: {{prefix}}actor_tools
-- [EJECTED 4.1.2] actor_tools removed

-- Table: {{prefix}}actor_prompts
-- [EJECTED 4.1.2] actor_prompts removed

-- Table: {{prefix}}actor_training
-- [EJECTED 4.1.2] actor_training removed
...
-- =============================================================================
-- {{prefix}}orchestrator_rules (optional; v4.0.86 --- orchestrator rule storage)
-- =============================================================================
...
-- registry_id values: 2-12 (seed range 0-999,999, per PRD 41 -2.1).
-- Source of truth for archetype/participates: TRUST_LADDER_REGISTRY.md.
-- After any doctrine change, re-run: python scripts/sync_trust_ladder_registry_to_db.py
-- ----------------------------------------------------------------------------
...
INSERT INTO {{prefix}}trust_ladder_registry
  (registry_id, table_name, archetype, participates, seed_required,
   ...
   canonical_id, reference_target, notes)
VALUES
  (6, 'lupo_actors', 'parent', 'seed_only', 1,
   NULL, NULL, 1,
   @now, @now, 0, 0,
   NULL, 'seed',
...
INSERT INTO {{prefix}}trust_ladder_registry
  (registry_id, table_name, archetype, participates, seed_required,
   ...
   canonical_id, reference_target, notes)
VALUES
  (7, 'lupo_agent_definitions', 'system', 'seed_only', 1,
   NULL, NULL, 1,
   @now, @now, 0, 0,
   NULL, 'seed',
...
INSERT INTO {{prefix}}trust_ladder_registry
  (registry_id, table_name, archetype, participates, seed_required,
   ...
   canonical_id, reference_target, notes)
VALUES
  (8, 'lupo_departments', 'parent', 'seed_only', 1,
   NULL, NULL, 1,
   @now, @now, 0, 0,
   NULL, 'seed',
...
INSERT INTO {{prefix}}trust_ladder_registry
  (registry_id, table_name, archetype, participates, seed_required,
   ...
   canonical_id, reference_target, notes)
VALUES
  (9, 'lupo_channels', 'parent', 'seed_only', 1,
   NULL, NULL, 1,
   @now, @now, 0, 0,
   NULL, 'seed',
...
INSERT INTO {{prefix}}trust_ladder_registry
  (registry_id, table_name, archetype, participates, seed_required,
   ...
   canonical_id, reference_target, notes)
VALUES
  (10, 'lupo_auth_users', 'parent', 'seed_only', 1,
   NULL, NULL, 1,
   @now, @now, 0, 0,
   NULL, 'seed',
...
INSERT INTO {{prefix}}trust_ladder_registry
  (registry_id, table_name, archetype, participates, seed_required,
   ...
   canonical_id, reference_target, notes)
VALUES
  (11, 'lupo_permissions', 'system', 'seed_only', 1,
   NULL, NULL, 1,
   @now, @now, 0, 0,
   NULL, 'seed',
...
INSERT INTO {{prefix}}trust_ladder_registry
  (registry_id, table_name, archetype, participates, seed_required,
   ...
   canonical_id, reference_target, notes)
VALUES
  (12, 'lupo_rules', 'system', 'seed_only', 1,
   NULL, NULL, 1,
   @now, @now, 0, 0,
   NULL, 'seed',
...
-- ----------------------------------------------------------------------------
-- SECTION 6: MISSING TABLES FROM JSON SCHEMA
-- Added to match JSON schema mirrors
-- ----------------------------------------------------------------------------

-- Transcripts for conversation tracking
CREATE TABLE {{prefix}}transcripts (
...
-- Transcript messages
CREATE TABLE {{prefix}}transcript_messages (
  transcript_message_id BIGINT PRIMARY KEY,
...
-- Dialog recent files
CREATE TABLE {{prefix}}dialog_recent_files (
  dialog_recent_file_id BIGINT PRIMARY KEY,
  actor_id BIGINT,
  file_path TEXT,
  opened_ymdhis BIGINT
);
...
-- Actor logs
CREATE TABLE {{prefix}}actor_logs (
  actor_log_id BIGINT PRIMARY KEY,
  actor_id BIGINT,
...
-- Runtime errors
CREATE TABLE {{prefix}}runtime_errors (
  runtime_error_id BIGINT PRIMARY KEY,
...
-- Runtime warnings
CREATE TABLE {{prefix}}runtime_warnings (
  runtime_warning_id BIGINT PRIMARY KEY,
...
-- Runtime validations
CREATE TABLE {{prefix}}runtime_validations (
  runtime_validation_id BIGINT PRIMARY KEY,
...
CREATE TABLE {{prefix}}collection_items (
  collection_item_id BIGINT PRIMARY KEY,
...
-- Dialog pending tasks
CREATE TABLE {{prefix}}dialog_pending_tasks (
  dialog_pending_task_id BIGINT PRIMARY KEY,
  task_type VARCHAR(64),
  payload LONGTEXT,
  created_ymdhis BIGINT
);
...
-- Dialog read log
CREATE TABLE {{prefix}}dialog_read_log (
  dialog_read_log_id BIGINT PRIMARY KEY,
  file_path TEXT,
  read_ymdhis BIGINT
);

-- Memory indexing tables
CREATE TABLE {{prefix}}memory_keywords (
  memory_keyword_id BIGINT PRIMARY KEY,
  memory_node_id BIGINT NOT NULL,
  keyword VARCHAR(255) NOT NULL,
  weight DECIMAL(10,4) DEFAULT 1.0,
  created_ymdhis BIGINT NOT NULL,
  INDEX idx_memory_node (memory_node_id),
  INDEX idx_keyword (keyword)
);

CREATE TABLE {{prefix}}memory_embeddings (
  memory_embedding_id BIGINT PRIMARY KEY,
  memory_node_id BIGINT NOT NULL,
...
CREATE TABLE {{prefix}}memory_search_index (
  memory_search_index_id BIGINT PRIMARY KEY,
  memory_node_id BIGINT NOT NULL,
  title VARCHAR(255),
  summary TEXT,
  keywords TEXT,
...
CREATE TABLE {{prefix}}memory_tags (
  memory_tag_id BIGINT PRIMARY KEY,
  memory_node_id BIGINT NOT NULL,
...
CREATE TABLE {{prefix}}memory_hash_index (
  memory_hash_index_id BIGINT PRIMARY KEY,
  memory_node_id BIGINT NOT NULL,
  content_hash VARCHAR(64) NOT NULL,
  created_ymdhis BIGINT NOT NULL,
  INDEX idx_memory_node (memory_node_id),
  INDEX idx_content_hash (content_hash),
  UNIQUE INDEX idx_node_hash (memory_node_id, content_hash)
);

-- Content indexing tables
CREATE TABLE {{prefix}}content_keywords (
  content_keyword_id BIGINT PRIMARY KEY,
  content_id BIGINT NOT NULL,
  keyword VARCHAR(255) NOT NULL,
  weight DECIMAL(10,4) DEFAULT 1.0,
  created_ymdhis BIGINT NOT NULL,
...
CREATE TABLE {{prefix}}content_search_index (
  content_search_index_id BIGINT PRIMARY KEY,
  content_id BIGINT NOT NULL,
  title VARCHAR(255),
  summary TEXT,
  keywords TEXT,
...
CREATE TABLE {{prefix}}content_tags (
  content_tag_id BIGINT PRIMARY KEY,
  content_id BIGINT NOT NULL,
  tag VARCHAR(128) NOT NULL,
  created_ymdhis BIGINT NOT NULL,
  INDEX idx_content (content_id),
...
CREATE TABLE {{prefix}}content_hash_index (
  content_hash_index_id BIGINT PRIMARY KEY,
  content_id BIGINT NOT NULL,
  content_hash VARCHAR(64) NOT NULL,
  created_ymdhis BIGINT NOT NULL,
  INDEX idx_content (content_id),
  INDEX idx_content_hash (content_hash),
  UNIQUE INDEX idx_content_hash_unique (content_id, content_hash)
);

-- =============================================================================
-- SECTION 7: AUTH USERS SEED DATA
-- =============================================================================
...
--   - Crafty livehelp_users.user_id values mapped into crafty_user_mapping
--     MUST be < 10000.
--   - System-reserved IDs 0, 10000, and 10001 must NEVER appear in crafty_user_mapping.
--   - All new users after install use IdGenerator (timestamp-based PK).
-- =============================================================================

-- System root user (ID 10000)
...
-- Administrator / red-team baseline user (ID 10001) - default password: admin123
...
-- Auth user departments for the new users
INSERT INTO {{prefix}}auth_user_departments (
    auth_user_department_id,
    auth_user_id,
    department_id,
    is_primary,
    role_key,
...
-- System user in Root department
(1, 10000, 0, 1, 'system', 'System User', 20260420000000, 20260420000000, 0),
-- Admin user in Root department
(2, 10001, 0, 1, 'administrator', 'Administrator', 20260420000000, 20260420000000, 0);

-- ----------------------------------------------------------------------------
-- End of trust_ladder_registry seed rows.
-- After any doctrine change: python scripts/sync_trust_ladder_registry_to_db.py --force
-- ----------------------------------------------------------------------------

-- ============================================================================
-- POST-INSTALL CONFIGURATION (Action Required)
-- ============================================================================
--
-- LLM API KEYS
-- ------------
-- Agent LLM configs are seeded with api_key_id = NULL. No LLM-invoking agent
-- will function until keys are configured. After install:
--
--   1. Log in as admin (auth_user_id 10001, default password: admin123).
--      CHANGE THE PASSWORD immediately.
--
--   2. Open Admin > Agent Config > LLM Providers and enter your API key(s):
--        Anthropic (Claude): https://console.anthropic.com/keys
--        OpenAI (GPT-4):     https://platform.openai.com/api-keys
--
--   3. Keys are stored server-side (never in the DB in plaintext). The admin
--      UI writes only a hash and display preview to {{prefix}}api_keys (to be
--      created in a follow-on schema patch for 4.1.4). For now, place raw keys
--      in config/llm_keys.php (excluded from git via .gitignore) and set
--      api_key_id on each {{prefix}}agent_llm_configs row to a placeholder ID.
--
--   4. A formal {{prefix}}api_keys table and admin UI are targeted post-May-1.
--      Tracked as open question in PRD 49 / PRD 07 (Agents Faucets).
--
-- ============================================================================
...
CREATE TABLE {{prefix}}agent_status (
  agent_status_id BIGINT PRIMARY KEY COMMENT 'PK following RULE 93.PK_NAMING',
...
-- Operator Scratchpad
CREATE TABLE {{prefix}}operator_scratchpad (
  scratchpad_id BIGINT PRIMARY KEY COMMENT 'PK following RULE 93.PK_NAMING',
...
-- Routing Events (HERMES message routing)
CREATE TABLE {{prefix}}routing_events (
  routing_event_id BIGINT PRIMARY KEY COMMENT 'PK following RULE 93.PK_NAMING',
...
CREATE TABLE {{prefix}}sticky_notes (
  sticky_note_id BIGINT PRIMARY KEY COMMENT 'PK following RULE 93.PK_NAMING',
  channel_id BIGINT NOT NULL,
  actor_id BIGINT NOT NULL,
