...
//         Copyright (C) 2003-2007 Eric Gerdes   (https://lupopedia.com )
// ----------------------------------------------------------------------------
    ...
    if(typeof chatelement!='undefined') { 
  	...
  	if(res_typeof=="HTML"){  
	    if( res_timeof>HTMLtimeof ){    
          ...
          up();     
            try {     
           if( (window.parent.rooms.document.mine.typing_alert_new.checked) && (<?php echo $jsrn; ?> != res_jsrn)){
           	  window.parent.bottomof.shoulditype();  
...
function update_xmlhttp() { // account for cache.. 
	...
	sURL = 'xmlhttp.php'; 
	extra = ""; 
  if( (window.parent.bottomof.loaded) && (window.parent.bottomof.channelset) ){ 
        if(window.parent.bottomof.document.chatter.previewsetting.value<3){
        	 extra = '&includeself=1';
        }
  }      	 
...
<body bgcolor="#<?php echo $color_background;?>" background="images/leftbk2.png">
