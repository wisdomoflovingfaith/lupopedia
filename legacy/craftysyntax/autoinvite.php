...
if($UNTRUSTED['whattodo'] == $lang['SAVE']){
 ...
 if(empty($UNTRUSTED['editidnum'])){
  $query = "INSERT INTO livehelp_autoinvite (isactive,offline,department,message,page,visits,referer,typeof,seconds,user_id,socialpane,excludemobile,onlymobile) VALUES ('Y',0,".intval($UNTRUSTED['department']).",'".filter_sql($UNTRUSTED['comment'])."','".filter_sql($UNTRUSTED['page'])."',". intval($UNTRUSTED['visits']).",'".filter_sql($UNTRUSTED['referer'])."','".filter_sql($UNTRUSTED['typeof'])."','$seconds','".intval($UNTRUSTED['user_id'])."','".filter_sql($UNTRUSTED['socialpane'])."','".filter_sql($UNTRUSTED['excludemobile'])."','".filter_sql($UNTRUSTED['onlymobile'])."')";
...
if($UNTRUSTED['whattodo'] == $lang['UPDATE']){  
  $query = "SELECT * FROM livehelp_autoinvite WHERE offline=0";
...
function refreshthis(what){
  if (what == "popup")
       	window.location.replace("autoinvite.php?createnew=<?php echo rawurlencode($UNTRUSTED['createnew']); ?>&whattodo=<?php echo rawurlencode($UNTRUSTED['whattodo']); ?>&which=<?php echo rawurlencode($UNTRUSTED['which']); ?>&force=popup");  
  else
       	window.location.replace("autoinvite.php?createnew=<?php echo rawurlencode($UNTRUSTED['createnew']); ?>&whattodo=<?php echo rawurlencode($UNTRUSTED['whattodo']); ?>&which=<?php echo rawurlencode($UNTRUSTED['which']); ?>&force=layer");  
...
<table>
<tr><td><b>Show only if the Operator:</b>
	<select name=user_id>
 ...
 while($row = $data->fetchRow(DB_FETCHMODE_ASSOC)){  
   ...
   if($row['user_id']==$item['user_id']){ print " SELECTED "; }
