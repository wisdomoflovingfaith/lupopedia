...
if(empty($UNTRUSTED['perpage'])){ $UNTRUSTED['perpage'] = 25; } if(empty($UNTRUSTED['whichdepartment'])){ $UNTRUSTED['whichdepartment'] = 1; } 

$whichdepartment = intval($UNTRUSTED['whichdepartment']);
...
<b><?php echo $lang['txt43']; ?>:</b> <select name="whichdepartment" onchange="document.dataformvisit.submit();">
...
if($UNTRUSTED['typeofview']== "levelvisits"){
  ...
  showchildrenof(0,10,$whatYm,$expand_array,'livehelp_visits_monthly','visit','','',$whichdepartment);
  print "</table>";
} else {
  $query = "SELECT count(*) as totalrows FROM livehelp_visits_monthly WHERE dateof=". intval($whatYm) ." AND directvisits!=0 AND department='" . intval($UNTRUSTED['whichdepartment']) . "' ORDER by directvisits DESC";
  ...
  $row = $sth->fetchRow(DB_FETCHMODE_ASSOC);
	...
	$query = "SELECT * FROM livehelp_visits_monthly WHERE dateof=". intval($whatYm) ." AND directvisits!=0 AND department='" . intval($UNTRUSTED['whichdepartment']) . "'  ORDER by directvisits DESC LIMIT " . intval($UNTRUSTED['offset']) . "," . intval($UNTRUSTED['perpage']);
  $visits_a = $mydatabase->query($query);
  $pageUrl = "data.php";
  $varstring = "&show=visit&tab=".$UNTRUSTED['tab'] . "&month=".$UNTRUSTED['month'] . "&year=" . $UNTRUSTED['year'] . "&whichdepartment=" . $UNTRUSTED['whichdepartment'];
  ...
  $bgcolor="F0F1E1"; 
   while($visits = $visits_a->fetchRow(DB_FETCHMODE_ASSOC)){ 
    ...
    print "<tr bgcolor=".$bgcolor."><td><a href=" . $visits['pageurl'] . " target=_blank>" . $visits['pageurl'] . "</a></td><td>" . $visits['directvisits'] . "</td><td> <a href=data.php?tab=4&parent=" . $visits['recno'] . "&month=".$UNTRUSTED['month'] . "&year=" . $UNTRUSTED['year'] . "> View Path</a></td><td> <a href=graph.php?item=" . $visits['recno'] . "&type=visit&typeof=directvisits&whichdepartment=".$whichdepartment." target=_blank>" . $lang['graph'] . "</a></td></tr>";
   }
  print "</table>";
}  
?>
