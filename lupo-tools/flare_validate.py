#!/usr/bin/env python3
"""
FLARE header validator (v4.0.56+). Enforces canonical order, mandated comment, typed actions, guards/brief.
Writes issues to flare_validate_issues.json when run (errors/warnings per file).
"""
import os
import sys
import json
...
# Issues output: mandated comments/order and other validation results
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
ISSUES_JSON = os.path.join(SCRIPT_DIR, "flare_validate_issues.json")
...
def validate_flare_file(path, **kwargs):
    ...
    try:
        ...
        # Resolve path for flame.see and path checks (before order/see blocks)
        ...
        # 0. MANDATED COMMENT (FLARE doctrine: first comment links to authoritative resolution)
        first_non_empty = None
        for line in lines[:max(flare_start, 5)]:
            s = line.strip()
            if s and not s.startswith("---"):
                first_non_empty = s
                break
        if first_non_empty and "FLARE" not in first_non_empty and "lupopedia.com/FLARE" not in first_non_empty and first_non_empty.startswith("#"):
            warnings.append("Missing mandated FLARE comment: first comment line should reference FLARE (e.g. '# FLARE Header ...' or 'see http://www.lupopedia.com/FLARE')")
...
def main():
    ...
    # Use flare_md_index.txt generated by flare_apply.py; fallback to repo root
    index_file = os.path.join(SCRIPT_DIR, "..", "tools", "flare_md_index.txt")
    ...
    for path in paths:
        ...
        if errors or warnings:
            issues_per_file.append({"path": path, "errors": errors, "warnings": warnings})
        if errors:
    ...
    for url, owners in url_tracker.items():
        if len(owners) > 1:
            ...
            issues_per_file.append({
                "path": "(url_collision)",
                "errors": [f"URL '{url}' claimed by: " + ", ".join(owners)],
                "warnings": []
            })

    # Write issues to issues.json (mandated comment/order and other findings)
    try:
        with open(ISSUES_JSON, "w", encoding="utf-8") as f:
            json.dump({
                ...
                "files": issues_per_file
            }, f, indent=2)
    except Exception as e:
        print(f"Warning: could not write {ISSUES_JSON}: {e}")

    print(f"\nValidation complete: {total_errors} errors, {total_warnings} warnings")
    if issues_per_file:
        print(f"Issues written to {ISSUES_JSON}")

    if ci_mode and total_errors > 0:
        sys.exit(1)


def test_order_and_guards():
    """Unit tests: canonical order and flare.conditional guards/brief."""
    # Order: present keys must follow canonical order
    expected_order = ["flame.init", "flare.conditional", "flare.headers", "flare.edges", "flare.footer", "flame.see", "flame.close"]
    keys_ok = ["flare.headers", "flare.edges", "flare.footer"]
    present_expected = [k for k in expected_order if k in keys_ok]
    actual_order = [k for k in keys_ok if k in expected_order]
    assert present_expected == actual_order, "Order check: subset should match"

    # Order: wrong order should fail
    keys_bad = ["flare.footer", "flare.headers"]
    present_bad = [k for k in expected_order if k in keys_bad]
    actual_bad = [k for k in keys_bad if k in expected_order]
    assert present_bad != actual_bad, "Order check: reversed subset should mismatch"

    # Guards: flare.conditional must have guards and brief
    cond_ok = {"guards": {"allow": {"actor_ids": [0]}}, "brief": {"who": {"owner_actor_id": 0}}}
    assert isinstance(cond_ok.get("guards"), dict), "guards must be object"
    assert isinstance(cond_ok.get("brief"), dict), "brief must be object"
    print("test_order_and_guards passed")


if __name__ == "__main__":
    if "--test" in sys.argv:
        test_order_and_guards()
        sys.exit(0)
    main()
