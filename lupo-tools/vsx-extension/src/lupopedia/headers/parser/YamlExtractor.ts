// src/lupopedia/headers/parser/YamlExtractor.ts
...
export class YamlExtractor {
    ...
    public extractHeaderBlock(text: string): string | null {
        ...
        // 2. Try @lupopedia.headers { ... } annotation (common in code comments)
        const annotatedMatch = head.match(/@lupopedia\.headers\s*({[\s\S]*?})\s*(?:\*\/|\n)/);
    ...
    public extractFooterBlock(text: string): string | null {
        ...
        // 2. Try @lupopedia.footer { ... } annotation
        const annotatedMatch = tail.match(/@lupopedia\.footer\s*({[\s\S]*?})\s*(?:\*\/|\n)/);
        if (annotatedMatch) return annotatedMatch[1].trim();
    ...
    private parseRecursive(lines: string[], indent: number): { result: any, nextIdx: number } {
        ...
        while (i < lines.length) {
            ...
            if (trimmed.startsWith('-')) {
                ...
                // For LUPOPEDIA HEADER, let's assume lists are simple strings
                i++;
                continue;
            }
