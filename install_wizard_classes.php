 ...
 *   file_path_from_root: "install_wizard_classes.php"
 *   web_path: "https://www.lupopedia.com/lupopedia/install_wizard_classes.php"
 *   status: "active"
 *   when_updated: "20260422100000"
 *   trust_tier: "canonical"
 *   questions_toon: null
 *   memory_toon: "lupo-memory/development/canonical/1026/04/install-wizard-classes-php.toon"
 *   atoms_toon: null
 *   transcript_jsonl: "0/development/install-wizard-classes-php"
 ...
 *   title: "install_wizard_classes.php -- installer wizard class helpers"
 *   summary: "Helper classes for install wizard step flow, config write/protection, bootstrap/import orchestration, and Crafty Syntax upgrade path."
 */
/**
 * ============================================================
 * INSTALL WIZARD CLASS FUNCTION MAP
 * ============================================================
 *
 * Class: InstallWizardSteps
 * - getWizardSteps()         : Returns array of wizard step definitions for new install or upgrade flow.
 * - getCurrentStepIndex()    : Gets the 1-based index of current step from step ID.
 * - getTotalSteps()          : Returns total count of wizard steps.
 *
 * Class: InstallWizardSecurity
 * - getCsrfToken()           : Generates or retrieves CSRF token for form protection.
 * - validateCsrf()           : Validates CSRF token for POST requests.
 *
 * Class: InstallWizardLogger
 * - logEntry()               : Logs installer events with timestamp to session audit log.
 * - safeErrorMessage()        : Returns generic error message for database operation failures.
 *
 * Class: InstallWizardCredentials
 * - craftyConfigExists()     : Detects if Crafty Syntax config.php exists for upgrade detection.
 * - crafty3775VersionDetect() : Detects Crafty Syntax version from config file content.
 * - validateCraftyPreMigration() : Validates required Crafty tables exist before migration.
 * - getDbCredentials()       : Extracts database credentials from Crafty config for upgrade.
 *
 * Class: InstallWizardDb
 * - connectPdo()             : Creates mysqli-style database connection for installer.
 * - connectPdoBuffered()     : Creates real PDO connection with buffered queries.
 * - escapeMysqlLikePattern() : Escapes special characters for MySQL LIKE queries.
 * - tableExists()            : Checks if database table exists using information_schema.
 * - detectLivehelpTables()   : Detects presence of Crafty Livehelp tables for upgrade.
 *
 * Class: InstallWizardRegistryValidator
 * - extractRegistryIdsFromSql() : Extracts registry ID patterns from SQL content.
 * - checkRegistryIdConflict()   : Checks for registry ID conflicts in database tables.
 *
 * Class: InstallWizardSqlRunner
 * - applyTablePrefixToSql()  : Applies table prefix to SQL statements safely.
 * - splitSqlStatements()     : Splits large SQL file into individual statements.
 * - runSqlFile()             : Executes SQL file with error handling and logging.
 * - formatLastSqlFailureForException() : Formats detailed SQL failure message for exceptions.
 * - dropLivehelpTables()     : Drops legacy Crafty tables after successful migration.
 *
 * Class: InstallWizardConfigWriter
 * - writeConfig()            : Generates and writes lupopedia-config.php with DB config, API keys, and federation secret.
 * - buildApiProviderRuntimeExport() : Builds runtime API provider configuration array.
 * - ensureConfigDenyRule()   : Ensures .htaccess deny rule exists for config protection.
 * - buildApiProviderConfigFromOptions() : Builds API provider config from install options.
 * - ensureActorZeroDirs()    : Creates required actor zero directories.
 * - ensureChannelsDir()      : Creates and secures channels directory structure.
 * - enqueueBackgroundCommand() : Enqueues background command for channel/artifact import.
 *
 * Class: InstallWizardMainAdmin
 * - hashPassword()           : Hashes admin password using secure hashing algorithm.
 * - createMainAdmin()        : Creates main admin user account in database.
 *
 * Class: InstallWizardNormalize
 * - usernameToSlug()         : Converts username to URL-friendly slug format.
 * - isValidEmail()           : Validates email address format.
 * - isValidLupopediaSlug()   : Validates slug meets Lupopedia requirements.
 * - isAcceptableResolvedEmail() : Checks if resolved email is acceptable for normalization.
 * - loadCraftyUsers()        : Loads user data from Crafty tables for normalization.
 * - computeProposedIdentities() : Computes proposed identity mappings for users.
 * - findDuplicateEmailGroups() : Identifies groups with duplicate email addresses.
 * - collectNormalizeWarnings() : Collects warnings from identity normalization process.
 * - validateResolvedEmails() : Validates resolved email addresses are acceptable.
 * - applyNormalizationToLivehelp() : Applies normalization changes to Crafty user data.
 *
 * Class: InstallWizardDepartments
 * - ensureSystemDepartment() : Ensures system department exists in database.
 * - ensureDefaultDepartment() : Ensures default department exists for operators.
 * - isSystemDepartment()     : Checks if department ID is system department.
 *
 * Class: InstallWizardChannels
 * - createReservedSystemChannels() : Creates reserved system channels (broadcasts, etc.).
 * - createOperatorChannels() : Creates operator channels for user accounts.
 * - ensureReservedChannels() : Ensures all reserved system channels exist.
 * - ensureOperatorChannels() : Ensures operator channels exist for all users.
 *
 * Class: InstallWizardLegacyImport
 * - checkLegacyTables()      : Checks for legacy tables that need importing.
 * - executeImport()          : Executes legacy data import from old tables.
 *
 * Class: InstallWizardUnregistry
 * - seedUnregistryFromGaps() : Seeds unregistry table with ID gaps for sequence.
 *
 * Class: InstallWizardBannedIdentities
 * - ensureStonedWolfieBannedIdentities() : Ensures banned identities are properly configured.
 *
 * ============================================================
 */
/*
 * Filesystem Rule (canonical per AGENTS.md):
 *
 * lupopedia-config.php MUST be written INSIDE the web-accessible directory and protected (0600 + .htaccess deny).
 *
 * All other files (lupo-memory/, lupo-channels/, app/, etc.) live ABOVE the web root.
 *
...
require_once LUPOPEDIA_PATH . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'classes' . DIRECTORY_SEPARATOR . 'InstallWizardMysqliLink.php';
    ...
    public static function getWizardSteps()
    {
        ...
        if ($isUpgrade) {
            return array(
                ...
                array('id' => 'api_keys', 'label' => 'API Keys'),
        ...
        return array(
            ...
            array('id' => 'config', 'label' => 'Config'),
            array('id' => 'api_keys', 'label' => 'API Keys'),
            array('id' => 'legacy_import', 'label' => 'Legacy Import'),
            array('id' => 'complete', 'label' => 'Complete'),
        );
    }
...
class InstallWizardCredentials
{
    ...
    {
        // DISABLED: Legacy config detection is causing runtime issues.
        // Users upgrading must manually enter DB credentials and select Upgrade mode.
        return false;
    }
    ...
    public static function getDbCredentials()
    {
        ...
        // DISABLED: Legacy config.php extraction is causing runtime issues.
        // Users must enter their database credentials manually in the installer UI.
        return null;
    }
}
...
class InstallWizardConfigWriter
{
    ...
    public static function writeConfig($db_vars, &$log, $options = array())
    {
        ...
        // Generate session salt (64 characters hex)
        $sessionSalt = '';
        if (function_exists('random_bytes')) {
            ...
            for ($i = 0; $i < 64; $i++) {
                $sessionSalt .= $hexChars[lupo_random_int(0, 15)];
            }
        }
        ...
        $preferredConfigDir = isset($options['config_target_dir']) ? (string) $options['config_target_dir'] : '';
        $writeTargets = LupopediaConfigResolver::defaultWriteTargets(LUPOPEDIA_PATH, $preferredConfigDir);
        ...
        $apiProviderConfigPhp = var_export($apiProviderConfig, true);
        $apiProviderRuntimeExport = self::buildApiProviderRuntimeExport($apiProviderConfig, $installRoot);
        $apiProviderRuntimeExportPhp = var_export($apiProviderRuntimeExport, true);

$content = '<?php
/**
 * Lupopedia Configuration — generated by install wizard
 * WARNING: This file contains sensitive API keys and is intentionally written under web root.
 * Ensure direct HTTP access to this file is denied by web server rules.
 * @package Lupopedia
 */
$lupo_prefix = \'\';
...
define(\'LUPO_SESSION_SALT\', \'' . $sessionSalt . '\');
...
if (!defined(\'LUPO_DATABASE_DIR\')) { define(\'LUPO_DATABASE_DIR\', \'database\'); }
...
' . "\n// LUPO_API_PROVIDER_CONFIG_START\n" . '$GLOBALS[\'LUPO_API_PROVIDER_CONFIG\'] = ' . $apiProviderConfigPhp . ';' . "\n" . '$GLOBALS[\'LUPO_API_PROVIDER_CONFIG\'][\'memory_path\'] = \'' . str_replace('\\', '/', rtrim($installRoot, DIRECTORY_SEPARATOR)) . '/memory/\';' . "\n" . '$GLOBALS[\'LUPO_API_PROVIDER_CONFIG\'][\'channels_path\'] = \'' . str_replace('\\', '/', rtrim($installRoot, DIRECTORY_SEPARATOR)) . '/channels/\';' . "\n" . '$GLOBALS[\'LUPO_API_PROVIDER_CONFIG_SOURCE\'] = \'install_wizard\';' . "\n// LUPO_API_PROVIDER_CONFIG_END\n" . '
...
$lupopedia_runtime_api_config = ' . $apiProviderRuntimeExportPhp . ';
return $lupopedia_runtime_api_config;
';
        ...
        LupopediaConfigResolver::protectConfigFile($configPath);
        if (is_file($configPath) && is_readable($configPath) && substr(sprintf('%o', @fileperms($configPath)), -4) !== '0600') {
            @chmod($configPath, 0600);
        }
        self::ensureConfigDenyRule($configDir, $log);
    ...
    private static function buildApiProviderRuntimeExport($providerConfig, $installRoot)
    {
        $providers = array();
        $providerRows = isset($providerConfig['providers']) && is_array($providerConfig['providers'])
            ? $providerConfig['providers']
            : array();

        foreach ($providerRows as $providerKey => $providerValues) {
            if (!is_array($providerValues)) {
                continue;
            }
            $apiKey = '';
            if (isset($providerValues['key']) && trim((string) $providerValues['key']) !== '') {
                $apiKey = trim((string) $providerValues['key']);
            } elseif (isset($providerValues['api_key'])) {
                $apiKey = trim((string) $providerValues['api_key']);
            }
            $providerName = isset($providerValues['display_name']) && trim((string) $providerValues['display_name']) !== ''
                ? trim((string) $providerValues['display_name'])
                : ucwords(str_replace('_', ' ', (string) $providerKey));
            $providers[(string) $providerKey] = array(
                'key' => $apiKey,
                'budget' => isset($providerValues['budget']) ? (float) $providerValues['budget'] : 15.0,
                'enabled' => isset($providerValues['enabled']) ? (bool) $providerValues['enabled'] : ($apiKey !== ''),
                'name' => $providerName,
            );
        }
        ...
        return array(
            'providers' => $providers,
            'fallback_order' => isset($providerConfig['fallback_order']) && is_array($providerConfig['fallback_order'])
                ? $providerConfig['fallback_order']
                : (isset($providerConfig['provider_order']) && is_array($providerConfig['provider_order']) ? $providerConfig['provider_order'] : array('gemini', 'deepseek', 'groq')),
            'memory_path' => str_replace('\\', '/', rtrim($installRoot, DIRECTORY_SEPARATOR)) . '/memory/',
            'channels_path' => str_replace('\\', '/', rtrim($installRoot, DIRECTORY_SEPARATOR)) . '/channels/',
            'config_version' => isset($providerConfig['config_version']) ? (string) $providerConfig['config_version'] : '2026.04',
        );
    }

    private static function ensureConfigDenyRule($configDir, &$log)
    {
        $dir = is_string($configDir) ? rtrim(str_replace('\\', DIRECTORY_SEPARATOR, $configDir), DIRECTORY_SEPARATOR) : '';
        ...
        if ($existing !== false && strpos($existing, 'lupopedia-config.php') !== false) {
            return;
        }
        $prefix = ($existing === false || $existing === '') ? '' : rtrim($existing) . "\n\n";
        if (@file_put_contents($htaccessPath, $prefix . $rule) !== false) {
            $log[] = InstallWizardLogger::logEntry('ok', 'Updated .htaccess to block direct access to lupopedia-config.php.');
        } else {
            $log[] = InstallWizardLogger::logEntry('skip', 'Could not update .htaccess with config deny rule (check permissions).');
        }
    }

    private static function buildApiProviderConfigFromOptions($options)
    {
        $providerConfig = array(
            ...
            'providers' => array(
                'anthropic' => array('enabled' => false, 'api_key' => '', 'key' => '', 'budget' => 15.0, 'display_name' => 'Anthropic'),
                'gemini' => array('enabled' => true, 'api_key' => '', 'key' => '', 'budget' => 15.0, 'display_name' => 'Google Gemini'),
                'grok' => array('enabled' => false, 'api_key' => '', 'key' => '', 'budget' => 15.0, 'display_name' => 'Grok xAI'),
                'deepseek' => array('enabled' => true, 'api_key' => '', 'key' => '', 'budget' => 15.0, 'display_name' => 'DeepSeek'),
                'groq' => array('enabled' => true, 'api_key' => '', 'key' => '', 'budget' => 15.0, 'display_name' => 'Groq'),
        ...
        if (isset($options['api_provider_config']) && is_array($options['api_provider_config'])) {
            $custom = $options['api_provider_config'];

            if (isset($custom['provider_order']) && is_array($custom['provider_order']) && !empty($custom['provider_order'])) {
                $providerConfig['provider_order'] = $custom['provider_order'];
                $providerConfig['request_class_order']['default'] = $custom['provider_order'];
                $providerConfig['fallback_order'] = $custom['provider_order'];
            }

            if (isset($custom['request_class_order']) && is_array($custom['request_class_order'])) {
                $providerConfig['request_class_order'] = array_merge($providerConfig['request_class_order'], $custom['request_class_order']);
            }

            if (isset($custom['monthly_budget_cap_usd'])) {
                $providerConfig['monthly_budget_cap_usd'] = (float) $custom['monthly_budget_cap_usd'];
            }
            if (isset($custom['premium_provider_block_threshold_usd'])) {
                $providerConfig['premium_provider_block_threshold_usd'] = (float) $custom['premium_provider_block_threshold_usd'];
            }
            if (isset($custom['premium_providers']) && is_array($custom['premium_providers'])) {
                $providerConfig['premium_providers'] = $custom['premium_providers'];
            }
            if (isset($custom['fallback_order']) && is_array($custom['fallback_order'])) {
                $providerConfig['fallback_order'] = $custom['fallback_order'];
            }
            if (isset($custom['config_version'])) {
                $providerConfig['config_version'] = trim((string) $custom['config_version']);
            }
            if (isset($custom['memory_path'])) {
                $providerConfig['memory_path'] = trim((string) $custom['memory_path']);
            }
            ...
            if (isset($custom['providers']) && is_array($custom['providers'])) {
                foreach ($custom['providers'] as $providerKey => $providerValues) {
                    if (!isset($providerConfig['providers'][$providerKey])) {
                        $providerConfig['providers'][$providerKey] = array(
                            'enabled' => false,
                            'api_key' => '',
                            'key' => '',
                            'budget' => 15.0,
                            'display_name' => ucwords(str_replace('_', ' ', (string) $providerKey)),
                        );
                    }
                    if (isset($providerValues['enabled'])) {
                        $providerConfig['providers'][$providerKey]['enabled'] = (bool) $providerValues['enabled'];
                    }
                    if (isset($providerValues['api_key'])) {
                        $providerConfig['providers'][$providerKey]['api_key'] = trim((string) $providerValues['api_key']);
                    }
                    if (isset($providerValues['key'])) {
                        $providerConfig['providers'][$providerKey]['key'] = trim((string) $providerValues['key']);
                    } else {
                        $providerConfig['providers'][$providerKey]['key'] = isset($providerConfig['providers'][$providerKey]['api_key']) ? $providerConfig['providers'][$providerKey]['api_key'] : '';
                    }
                    if (isset($providerValues['budget'])) {
                        $providerConfig['providers'][$providerKey]['budget'] = (float) $providerValues['budget'];
                    }
                    if (isset($providerValues['display_name'])) {
                        $providerConfig['providers'][$providerKey]['display_name'] = trim((string) $providerValues['display_name']);
                    }
                    if (isset($providerValues['name']) && !isset($providerValues['display_name'])) {
                        $providerConfig['providers'][$providerKey]['display_name'] = trim((string) $providerValues['name']);
                    }
                }
            }
        }
...
class InstallWizardMainAdmin
{
    ...
    const RED_TEAM_AUTH_USER_ID = 10001;
    const WOLFIE_ACTOR_ID = 1;
    ...
    public static function createMainAdmin($pdo, $table_prefix, $email, $password, &$log)
    {
        ...
        try {
            ...
            self::ensureMainAdminActorAuthUserLink($pdo, $aau_t, $actor_id, $auth_id, $now, $log);

            // WOLFIE pairing for act-as / orchestration only — must NOT win login routing over captain (actor 10000).
            $wolfieActorId = self::WOLFIE_ACTOR_ID;
            $checkWolfiePairing = $pdo->prepare('SELECT 1 FROM `' . str_replace('`', '``', $aau_t) . '` WHERE actor_id = ? AND auth_user_id = ? AND (is_deleted = 0 OR is_deleted IS NULL) LIMIT 1');
            $checkWolfiePairing->execute(array($wolfieActorId, $auth_id));
            if (!$checkWolfiePairing->fetchColumn()) {
                $nextAauId = (int) $pdo->query('SELECT COALESCE(MAX(actor_auth_user_id), 0) + 1 FROM `' . str_replace('`', '``', $aau_t) . '`')->fetchColumn();
                $insWolfiePairing = $pdo->prepare('INSERT INTO `' . str_replace('`', '``', $aau_t) . '` (actor_auth_user_id, actor_id, auth_user_id, relationship_role, is_primary, routing_priority, status, created_ymdhis, updated_ymdhis, is_deleted) VALUES (?, ?, ?, ?, 0, 200, ?, ?, ?, 0)');
                $insWolfiePairing->execute(array($nextAauId, $wolfieActorId, $auth_id, 'delegate', 'active', $now, $now));
                $log[] = InstallWizardLogger::logEntry('ok', 'Paired auth_user ' . $auth_id . ' with WOLFIE (actor_id 1) for act-as (non-login routing).');
            } else {
                $upWolfiePair = $pdo->prepare('UPDATE `' . str_replace('`', '``', $aau_t) . '` SET relationship_role = ?, is_primary = 0, routing_priority = 200, status = ?, updated_ymdhis = ? WHERE actor_id = ? AND auth_user_id = ? AND (is_deleted = 0 OR is_deleted IS NULL)');
                $upWolfiePair->execute(array('delegate', 'active', $now, $wolfieActorId, $auth_id));
            }

            if (!self::ensureMainAdminPrivileges($pdo, $prefix, $actor_id, $auth_id, $now, $log)) {
                return false;
            }
    ...
    /**
     * Link auth_user 10000 to captain actor 10000 as primary_owner (login routing).
     *
     * @param PDO $pdo
     * @param string $aau_t
     * @param int $actor_id
     ...
     * @return void
     */
    private static function ensureMainAdminActorAuthUserLink($pdo, $aau_t, $actor_id, $auth_id, $now, &$log)
    {
        $checkAau = $pdo->prepare('SELECT 1 FROM `' . str_replace('`', '``', $aau_t) . '` WHERE actor_id = ? AND auth_user_id = ? AND (is_deleted = 0 OR is_deleted IS NULL) LIMIT 1');
        $checkAau->execute(array($actor_id, $auth_id));
        if (!$checkAau->fetchColumn()) {
            $nextAauId = (int) $pdo->query('SELECT COALESCE(MAX(actor_auth_user_id), 0) + 1 FROM `' . str_replace('`', '``', $aau_t) . '`')->fetchColumn();
            $insAau = $pdo->prepare('INSERT INTO `' . str_replace('`', '``', $aau_t) . '` (actor_auth_user_id, actor_id, auth_user_id, relationship_role, is_primary, routing_priority, status, created_ymdhis, updated_ymdhis, is_deleted) VALUES (?, ?, ?, ?, 1, 0, ?, ?, ?, 0)');
            $insAau->execute(array($nextAauId, $actor_id, $auth_id, 'primary_owner', 'active', $now, $now));
            $log[] = InstallWizardLogger::logEntry('ok', 'Linked main admin actor to auth user (primary_owner, routing 0).');
        } else {
            $upAau = $pdo->prepare('UPDATE `' . str_replace('`', '``', $aau_t) . '` SET relationship_role = ?, is_primary = 1, routing_priority = 0, status = ?, updated_ymdhis = ? WHERE actor_id = ? AND auth_user_id = ? AND (is_deleted = 0 OR is_deleted IS NULL)');
            $upAau->execute(array('primary_owner', 'active', $now, $actor_id, $auth_id));
            $log[] = InstallWizardLogger::logEntry('ok', 'Updated main admin actor_auth_users link (primary_owner, routing 0).');
        }
    }

    /**
     * Idempotent admin capability rows for main admin actor (channel roles, department roles, module owner).
     *
     * @param PDO $pdo
     ...
     * @return bool
     */
    public static function ensureMainAdminPrivileges($pdo, $table_prefix, $actor_id, $auth_id, $now, &$log)
    {
        $prefix = ($table_prefix !== null && $table_prefix !== '') ? $table_prefix : 'lupo_';
        $acr_t = $prefix . 'actor_channel_roles';
        $ac_t = $prefix . 'actor_channels';
        ...
        foreach ($channels as $chId) {
            $checkAcr = $pdo->prepare('SELECT 1 FROM `' . str_replace('`', '``', $acr_t) . '` WHERE actor_id = ? AND channel_id = ? AND role_key = ? AND (is_deleted = 0 OR is_deleted IS NULL) LIMIT 1');
            $checkAcr->execute(array($actor_id, $chId, 'captain'));
    ...
    /**
     * Repair installs where login routed to WOLFIE (actor 1) without admin roles. Safe to re-run.
     *
     * @param PDO $pdo
     * @param string $table_prefix
     * @param array $log
     * @return bool
     */
    public static function repairMainAdminInstall($pdo, $table_prefix, &$log)
    {
        $now = (int) gmdate('YmdHis');
        $prefix = ($table_prefix !== null && $table_prefix !== '') ? $table_prefix : 'lupo_';
        $aau_t = $prefix . 'actor_auth_users';
        ...
        $upWolfiePair = $pdo->prepare('UPDATE `' . str_replace('`', '``', $aau_t) . '` SET relationship_role = ?, is_primary = 0, routing_priority = 200, status = ?, updated_ymdhis = ? WHERE actor_id = ? AND auth_user_id = ? AND (is_deleted = 0 OR is_deleted IS NULL)');
        $upWolfiePair->execute(array('delegate', 'active', $now, $wolfieActorId, $auth_id));
        $log[] = InstallWizardLogger::logEntry('ok', 'Demoted WOLFIE act-as link for auth_user ' . $auth_id . ' (non-login routing).');

        return self::ensureMainAdminPrivileges($pdo, $prefix, $actor_id, $auth_id, $now, $log);
    }

    /**
     * Create red team user (auth_user_id 10001) and pair with WOLFIE (actor_id 1).
     * This user is for adversarial testing and security validation.
     *
     * @param PDO $pdo
     * @param string $table_prefix e.g. lupo_
     * @param array $log
     * @return bool
     */
    public static function createRedTeamUser($pdo, $table_prefix, &$log)
    {
        $prefix = ($table_prefix !== null && $table_prefix !== '') ? $table_prefix : 'lupo_';
        $auth_t = $prefix . 'auth_users';
        $actors_t = $prefix . 'actors';
        ...
        try {
            $auth_id = self::RED_TEAM_AUTH_USER_ID;
            $wolfieActorId = self::WOLFIE_ACTOR_ID;

            // Check if WOLFIE actor exists
            $checkWolfie = $pdo->prepare('SELECT 1 FROM `' . str_replace('`', '``', $actors_t) . '` WHERE actor_id = ? LIMIT 1');
            $checkWolfie->execute(array($wolfieActorId));
            if (!$checkWolfie->fetchColumn()) {
                $log[] = InstallWizardLogger::logEntry('error', 'WOLFIE actor (actor_id 1) not found. Cannot pair red team user.');
                return false;
            }

            // Create red team auth_user if not exists
            $checkAuth = $pdo->prepare('SELECT 1 FROM `' . str_replace('`', '``', $auth_t) . '` WHERE auth_user_id = ? LIMIT 1');
            $checkAuth->execute(array($auth_id));
            $authExists = $checkAuth->fetchColumn();
            if (!$authExists) {
                ...
                $email = 'redteam@lupopedia.local';
                $password = 'redteam123'; // Default password, should be changed
                $hash = self::hashPassword($password);
                if ($hash === false) {
                    $log[] = InstallWizardLogger::logEntry('error', 'Could not hash red team password.');
                    return false;
                }

                $ins = $pdo->prepare('INSERT INTO `' . str_replace('`', '``', $auth_t) . '` (auth_user_id, username, display_name, email, password_hash, auth_provider, provider_id, created_ymdhis, updated_ymdhis, is_active, is_deleted, deleted_ymdhis) VALUES (?, ?, ?, ?, ?, ?, NULL, ?, ?, 1, 0, NULL)');
                $ins->execute(array($auth_id, $username, $display_name, $email, $hash, 'local', $now, $now));
                $log[] = InstallWizardLogger::logEntry('ok', 'Created red team user (auth_user_id ' . $auth_id . ').');
            } else {
                $log[] = InstallWizardLogger::logEntry('ok', 'Red team user (auth_user_id ' . $auth_id . ') already exists.');
            }

            // Link to System department (0)
            $checkAud = $pdo->prepare('SELECT 1 FROM `' . str_replace('`', '``', $aud_t) . '` WHERE auth_user_id = ? AND department_id = 0 AND (is_deleted = 0 OR is_deleted IS NULL) LIMIT 1');
            $checkAud->execute(array($auth_id));
            if (!$checkAud->fetchColumn()) {
                $nextAudId = (int) $pdo->query('SELECT COALESCE(MAX(auth_user_department_id), 0) + 1 FROM `' . str_replace('`', '``', $aud_t) . '`')->fetchColumn();
                $insAud = $pdo->prepare('INSERT INTO `' . str_replace('`', '``', $aud_t) . '` (auth_user_department_id, auth_user_id, department_id, is_primary, role_key, title, created_ymdhis, updated_ymdhis, is_deleted) VALUES (?, ?, 0, 1, ?, ?, ?, ?, 0)');
                $insAud->execute(array($nextAudId, $auth_id, 'red_team', 'Red Team Testing', $now, $now));
                $log[] = InstallWizardLogger::logEntry('ok', 'Linked red team auth user to System department (0).');
            }

            // Pair with WOLFIE as primary actor
            $checkWolfiePairing = $pdo->prepare('SELECT 1 FROM `' . str_replace('`', '``', $aau_t) . '` WHERE actor_id = ? AND auth_user_id = ? AND (is_deleted = 0 OR is_deleted IS NULL) LIMIT 1');
            $checkWolfiePairing->execute(array($wolfieActorId, $auth_id));
            if (!$checkWolfiePairing->fetchColumn()) {
                $nextAauId = (int) $pdo->query('SELECT COALESCE(MAX(actor_auth_user_id), 0) + 1 FROM `' . str_replace('`', '``', $aau_t) . '`')->fetchColumn();
                $insWolfiePairing = $pdo->prepare('INSERT INTO `' . str_replace('`', '``', $aau_t) . '` (actor_auth_user_id, actor_id, auth_user_id, relationship_role, is_primary, routing_priority, status, created_ymdhis, updated_ymdhis, is_deleted) VALUES (?, ?, ?, ?, 1, 1, ?, ?, ?, 0)');
                $insWolfiePairing->execute(array($nextAauId, $wolfieActorId, $auth_id, 'supporting_human', 'active', $now, $now));
                $log[] = InstallWizardLogger::logEntry('ok', 'Paired red team auth_user 10001 with WOLFIE (actor_id 1) as primary default.');
            }

            return true;
        } catch (Exception $e) {
            $log[] = InstallWizardLogger::logEntry('error', 'Red team user creation failed: ' . $e->getMessage());
            error_log('Lupopedia InstallWizardMainAdmin: ' . $e->getMessage());
            return false;
        }
    }
...
/**
 * Legacy Crafty Syntax Import Handler
 * Handles the optional import step after base install is complete.
 */
class InstallWizardLegacyImport
{
    
    /**
     * Check if the required 5 legacy tables exist for import
     */
    public static function checkLegacyTables($pdo, &$log)
    {
        ...
        if (!empty($missingTables)) {
            $log[] = InstallWizardLogger::logEntry('error', 'Missing required legacy tables: ' . implode(', ', $missingTables));
            return false;
        }
        
        $log[] = InstallWizardLogger::logEntry('ok', 'All required legacy tables found: ' . implode(', ', $requiredTables));
        return true;
    }
    
    /**
     * Execute legacy import using the PHP wrapper
     */
    public static function executeImport($pdo, $tablePrefix, &$log)
    {
        require_once LUPOPEDIA_PATH . DIRECTORY_SEPARATOR . 'install' . DIRECTORY_SEPARATOR . 'ImportLegacyCraftySyntax.php';
        
        try {
            $importer = new ImportLegacyCraftySyntax($pdo, $tablePrefix);
            $result = $importer->executeImport();
            
            // Log results
            foreach ($result['log'] as $logEntry) {
                $log[] = InstallWizardLogger::logEntry($logEntry['level'], $logEntry['message']);
            }
            
            // Log summary
            if (isset($result['mapping_summary'])) {
                $log[] = InstallWizardLogger::logEntry('ok', 
                    sprintf('Import Summary: %d users mapped (IDs %d-%d → %d-%d)', 
                        $result['mapping_summary']['total_mapped'],
                        $result['mapping_summary']['min_legacy_id'],
                        $result['mapping_summary']['max_legacy_id'],
                        $result['mapping_summary']['min_new_id'],
                        $result['mapping_summary']['max_new_id']
                    )
                );
            }
            
            $log[] = InstallWizardLogger::logEntry('ok', 
                sprintf('Imported %d users, skipped %d users', 
            ...
            // Log updated tables
            if (isset($result['updated_tables']) && !empty($result['updated_tables'])) {
                foreach ($result['updated_tables'] as $table) {
                    $log[] = InstallWizardLogger::logEntry('ok', 
                        sprintf('Updated %d rows in %s', $table['rows_updated'], $table['table'])
                    );
                }
            }
            ...
            $log[] = InstallWizardLogger::logEntry('error', 'Import failed: ' . $e->getMessage());
            return false;
        }
    }
}
