...
if($UNTRUSTED['action'] == "addnew"){ // just a little error checking..
 ...
 if(!(empty($UNTRUSTED['useimage']))){    	
  if($UNTRUSTED['useimage']=="Y"){ $useimage_Y = "Y"; } else { $useimage_Y = "N";  }
 }
 $query = "INSERT INTO livehelp_users (username,password,displayname,isoperator,isadmin,isnamed,email,user_alert,show_arrival,identity,greeting,showtype,useimage,alertchat,alerttyping,alertinsite)
           VALUES ('".filter_sql($UNTRUSTED['newuser'])."','".filter_sql(md5($UNTRUSTED['newpass']))."','".filter_sql($UNTRUSTED['newdisplayname'])."','Y','".filter_sql($UNTRUSTED['isadmin'])."','Y','".filter_sql($UNTRUSTED['email'])."','".filter_sql($UNTRUSTED['user_alert'])."','".filter_sql($UNTRUSTED['show_arrival'])."','$md5string','".filter_sql($UNTRUSTED['greeting'])."','".filter_sql($UNTRUSTED['showtyping'])."','".$useimage_Y."','".filter_sql($UNTRUSTED['alertchat'])."','".filter_sql($UNTRUSTED['alerttyping'])."','".filter_sql($UNTRUSTED['alertinsite'])."')";
...
if($UNTRUSTED['action'] == "updateuser"){
	  if($isadminsetting == "Y") 
	   $isadminsql = " isadmin='".filter_sql($UNTRUSTED['isadmin'])."',";
  	  ...
  	  $isadminsql = "";
    $passsql = " ";
    if(!(empty($UNTRUSTED['newpass']))){         $passsql = "  password='".filter_sql(md5($UNTRUSTED['newpass']))."', "; }
...
if($isadminsetting == "Y") {
  ...
  if(!($serversession))
    ...
    sleep(1);
    if($isadminsetting == "Y"){
      header("Location: operators.php",TRUE,307);
      exit;  
    } else {
...
function uncheck2(whichone){
		if (whichone==1){ 
		   document.operatorform.usetemplateimage.checked = false;	 
	  } 
		if (whichone==2){
		   document.operatorform.useimage.checked = false;	   
	  } 
...
<table border=0>
<tr bgcolor=FFFFFF><td><b> displayname </b></td><td><b><?php echo $lang['txt184']; ?></b></td><td><b><?php echo $lang['dept']; ?></b></td><td><b>Rights</b></td><td><b>e-mail</b></td><td><b><?php echo $lang['options']; ?></b></td></tr>
<?php 
$query = "SELECT * FROM livehelp_users WHERE isoperator='Y' ";
$data = $mydatabase->query($query);
while($row = $data->fetchRow(DB_FETCHMODE_ASSOC)){
   if($bgcolor=="$color_alt2"){ $bgcolor="$color_alt1"; } else { $bgcolor="$color_alt2"; }
 print "<tr bgcolor=$bgcolor><td>" . $row['displayname'] . "</td><td>" . $row['username'] . "</td><td>";
 $query = "SELECT * 
...
if($userinfo['useimage']!="N"){ $useimage_check = " CHECKED "; } else { $useimage_check = "  "; }
if($CSLH_Config['showoperator']!="N"){ $usetemplateimage_check = " CHECKED "; } else { $usetemplateimage_check = "  "; }

 ?>
 
<tr><td>Display Name:</td><td><input type=text name=newdisplayname size=42 value="<?php echo $userinfo['displayname']; ?>"></td></tr>

<tr><td>username:</td><td><input type=text name=newuser size=30 value="<?php echo $userinfo['username']; ?>"></td></tr>
<tr><td>password:<br><i>Enter only to change current password</i></td><td><input type=password name=newpass size=30 value=""></td></tr>
...
if ($userinfo['user_alert'] == "Y"){ $user_alert_y = " CHECKED "; } else { $user_alert_y ="";  }
if ($userinfo['user_alert'] == "N"){ $user_alert_n = " CHECKED "; } else { $user_alert_n =""; }
if ($userinfo['user_alert'] == "E"){ $user_alert_e = " CHECKED "; } else { $user_alert_e = ""; }
if ($userinfo['user_alert'] == "A"){ $user_alert_a = " CHECKED "; } else { $user_alert_a = ""; }  
...
<tr><td><?php echo $lang['txt136']?>:</td><td><input type=radio name=user_alert value=A <?php echo $user_alert_a; ?> > HTML5 audio <input type=radio name=user_alert value=Y <?php echo $user_alert_y; ?> > JS alert <input type=radio name=user_alert value=N <?php echo $user_alert_n; ?> > EMBED wav sound</td></tr>
...
<tr><td colspan=2>
	Below are the sounds that are played for various events in the Live
	Help program. These options are customizable by operator. If you
	would like to ADD sounds to these options simply upload additional 
	sound wav files to the folder named "<b>sounds/</b> in your live help
	application .  If by clicking PLAY no sound is played change the <b>sound alert type</b>
	above from embed to audio to use HTML5 tags and click save to use that feature.

  <a href=https://lupopedia.com/sounds.php?d=<?php echo $CSLH_Config['directoryid'] ?> target=_blank>DOWNLOAD MORE SOUND FILES</a><br>
   uploaded images in "<b>sounds/</b>"to choose from: (simply upload more images in the directory to add to this list)<br>
   
<div id="sound_element"></div>
</td></tr>
<tr><td> Sound for new Chat:</td><td>
 <select name="alertchat" id="alertchat"  > 
<?php
// get list of installed sounds..
$dir = "sounds/";
$handle=opendir($dir);
	while($file=readdir($handle)){
		...
		if(is_file("$dir/$file"))
			{ 
			?><option value=<?php echo $file; ?> <?php if ($userinfo['alertchat'] == $file){ print " SELECTED "; } ?> ><?php echo $file; ?> </option><?php
			}
	}
?>
</select> <a href="javascript:playalert(1)">PLAY</a>
</td></tr>
<tr><td> Sound for new visitor:</td>
	<td>
 <select name="alertinsite" id="alertinsite" > 
<?php
// get list of installed sounds..
$dir = "sounds/";
$handle=opendir($dir);
	while($file=readdir($handle)){
		...
		if(is_file("$dir/$file"))
			{ 
			?><option value=<?php echo $file; ?> <?php if ($userinfo['alertinsite'] == $file){ print " SELECTED "; } ?> ><?php echo $file; ?> </option><?php
			}
	}
?>
</select> <a href="javascript:playalert(2)">PLAY</a>
</td></tr>

 
<tr><td> Sound for is typing:</td>
	<td>
 <select name=alerttyping id="alerttyping" > 
<?php
// get list of installed sounds..
$dir = "sounds/";
$handle=opendir($dir);
	while($file=readdir($handle)){
		...
		if(is_file("$dir/$file"))
			{ 
			?><option value=<?php echo $file; ?> <?php if ($userinfo['alerttyping'] == $file){ print " SELECTED "; } ?> ><?php echo $file; ?> </option><?php
			}
	}
?>
</select> <a href="javascript:playalert(3)">PLAY</a>
</td></tr>
...
<?php
$query = "SELECT * FROM livehelp_departments ORDER by website";
$data = $mydatabase->query($query);
$currentwebsite = 0;
while($dept = $data->fetchRow(DB_FETCHMODE_ASSOC)){
	if($currentwebsite != $dept['website']){
		 $currentwebsite = $dept['website'];
		 $query = "SELECT site_name,site_url 
            FROM livehelp_websites
            WHERE id=" . intval($dept['website']);
     $check = $mydatabase->query($query);
     $temprow = $check->fetchRow(DB_FETCHMODE_ORDERED);	
     print "<table width=300><tr><td bgcolor=#FFFFFF><b>" . $temprow[0] . " " . 	$temprow[1] . "</b></td></tr></table><br>";
	}
  $query = "SELECT * 
...
<input type=checkbox name=useimage <?php echo $useimage_check ?> value="Y" onclick="uncheck2(1)">Show Image in chat upon answering chat <br>
(Check this box only if operators image is not used in the template for the chat window.
<br>
<input type=checkbox name=usetemplateimage <?php echo $usetemplateimage_check ?> value="Y"  onclick="uncheck2(2)"  >Show Operator image in template of chat window<br>
(Check this box only if operators image is not used upon answering chat.
</td></tr>
</table>
...
function playalert(whichselection){
 
if(whichselection == 1){ 
 var x=document.getElementById("alertchat").selectedIndex; 
}
if(whichselection == 2){ 
 var x=document.getElementById("alertinsite").selectedIndex;
}
if(whichselection == 3){ 
 var x=document.getElementById("alerttyping").selectedIndex; 
}
	...
	while($file=readdir($handle)){
		...
		if(is_file("$dir/$file"))
			{ 
			 ...
			 if( (x + 1) == "<?php echo $count; ?>"){
    	  <?php if ($userinfo['user_alert'] == "A"){ ?>
    	  document.getElementById("sound_element").innerHTML= "<audio autoplay><source src='sounds/<?php echo $file; ?>' type='audio/mpeg'></audio>";
    	  <?php } else { ?> 	
    	   document.getElementById("sound_element").innerHTML= "<embed src='sounds/<?php echo $file; ?>' hidden=false width=5 height=5 autostart=true loop=false>";
       <?php } ?>
       }
		  <?php
		  }
