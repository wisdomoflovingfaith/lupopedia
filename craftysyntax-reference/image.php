...
if(!(isset($UNTRUSTED['hide']))){ $UNTRUSTED['hide'] = "N"; }
...
if($UNTRUSTED['what'] == "getlayerinvite"){
   ...
   if($data->numrows() !=0){
    ...
    // ========================================================================
    // BRILLIANT HACK: Using IMAGE SIZE/FILENAME to Send Data via HTTP
    // ========================================================================
    // This is a workaround for cross-domain communication in 2003 era browsers
    // that didn't support modern APIs like CORS, JSONP, or XMLHttpRequest.
    // 
    // THE PROBLEM:
    // - Need to send data from server to embedded visitor tracking script
    // - Can't use standard HTTP methods (pre-CORS era)
    // - Need to "push" invite layer ID to the client
    // - Can't use polling because that's expensive
    //
    // THE SOLUTION:
    // - Store layer invite ID in session: sessiondata = "invite=123"
    // - Convert ID to individual digits (123 = hundreds, tens, ones)
    // - Return a DIFFERENT IMAGE FILE for each digit position
    // - Client reads the image filename to get the digit
    // - Client knows which position (ones/tens/hundreds) to request next
    //
    // HOW IT WORKS:
    // 1. Client calls: image.php?what=getlayerinvite&whatplace=ones
    // 2. Server reads sessiondata: "invite=145"
    // 3. Server breaks 145 into: hundreds=1, tens=4, ones=5
    // 4. Server returns digit5.gif (for the "ones" place)
    // 5. Client reads the image filename, knows digit is 5
    // 6. Client calls again: image.php?what=getlayerinvite&whatplace=tens
    // 7. Server returns digit4.gif
    // 8. Client calls: image.php?what=getlayerinvite&whatplace=hundreds
    // 9. Server returns digit1.gif
    // 10. Client reconstructs: 145 and displays appropriate invite layer
    //
    // WHY THIS WORKS:
    // - Images can load from ANY domain without CORS restrictions
    // - Browser tells you which image loaded (or failed to load)
    // - Image filename encodes the data (digitX.gif = digit X)
    // - Multiple requests needed, but that's ok for small data
    //
    // MODERN EQUIVALENT:
    // Today you'd use: fetch() with JSON, WebSockets, or SSE (Server-Sent Events)
    // But in 2003, this was incredibly clever for its time!
    // ========================================================================
    
     // Parse URL-encoded session data (key=value&key2=value2 format)
...
if($UNTRUSTED['what'] == "userstat"){    
   ...
   if($data->numrows() !=0){
    ...
    if($UNTRUSTED['page']!=""){ 
      ...
      if($data_tmp2->numrows() ==0){
       if( ($UNTRUSTED['page']!="") &&  ($UNTRUSTED['page']!="http://") ){
   ...
   if(!($serversession))
    ...
    if($status == "request"){
     ...
     showimage($filepath,"image/gif");
     exit; 
    } elseif (($status == "DHTML") || ($status == "invited") ) {
       $filepath = "images/requestDHTML.gif"; 
       showimage($filepath,"image/gif"); 
     exit;     	
    } else {	
...
if(!(empty($_GET['xy']))){ $xyz = $_GET['xy']; }
if($xyz == "N"){
    $sqlquery = "SELECT creditline FROM livehelp_departments WHERE recno='".intval($department)."'";
    $data2 = $mydatabase->query($sqlquery);	
    $orderedarray = $data2->fetchRow(DB_FETCHMODE_ORDERED);
    $xyz = $orderedarray[0];
 }
...
if($UNTRUSTED['what'] == "getstate"){
   ...
   $sqlquery .= " ORDER by lastaction LIMIT 1";
    ...
    $query = "SELECT user_id FROM livehelp_users WHERE isoperator='Y' AND authenticated='Y' AND lastaction<'$oldtime'  AND ismobile='N' ";
    ...
    $oldtime = date("YmdHis",$prev);
    $query = "SELECT user_id FROM livehelp_users WHERE isoperator='Y' AND authenticated='Y' AND lastaction<'$oldtime' AND (ismobile='Y' OR ismobile='P') ";
    $data2 = $mydatabase->query($query); 
    while($row = $data2->fetchRow(DB_FETCHMODE_ASSOC)){
    	 $opid = $row['user_id'];
