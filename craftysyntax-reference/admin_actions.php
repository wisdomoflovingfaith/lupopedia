...
// insite Invite
if($UNTRUSTED['whattodo'] == "insite"){
   Header("Location: invite.php?selectedwho=$listedids&insite=1");
   exit;
}
...
if($UNTRUSTED['whattodo'] == "transfer"){
 ...
 ?>
  ...
  <br>
  <input type=checkbox name=keeptalking value=YES> - Keeep me connected to this chat session. (Only check this if a operator has been selected from above)
  </form>
<?php 
  exit;
}
...
if($UNTRUSTED['whattodo'] == "redirect"){
  $pairs = explode(",",$UNTRUSTED['redirectids']);
  for($i=0;$i<count($pairs);$i++){
    $thisid = $pairs[$i];	
    ...
    $query = "SELECT * FROM livehelp_departments WHERE recno=".intval($UNTRUSTED['todepartment']);	
    $rs = $mydatabase->query($query);  
    $person = $rs->fetchRow(DB_FETCHMODE_ASSOC);  
    $nameof = $person['nameof'];
 	  $message = "transfering chat to : $nameof ...";
    ...
    if( intval($UNTRUSTED['tooperator']) > 0 ){
     ...
     $query = "SELECT * FROM livehelp_users WHERE user_id=".intval($UNTRUSTED['tooperator']);	
     $rs = $mydatabase->query($query);  
     $person = $rs->fetchRow(DB_FETCHMODE_ASSOC);  
     $username = $person['username'];
 	   $message = "chat connected to : $username ";
     $query = "INSERT INTO livehelp_messages (message,channel,timeof,saidfrom,saidto) VALUES ('$message',".intval($onchannel).",'$rightnow',".intval($myid).",'".intval($thisid)."')";  
     $mydatabase->query($query);
