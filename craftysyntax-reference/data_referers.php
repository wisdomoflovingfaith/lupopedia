...
<input type=hidden name=tab value=<?php echo intval($UNTRUSTED['tab']); ?>>
...
<b><?php echo $lang['txt43']; ?>:</b> <select name="whichdepartment" onchange="document.dataformrefer.submit();">
...
if($UNTRUSTED['typeofview']== "levelvisits"){
  ...
  showchildrenof(0,10,$whatYm,$expand_array,'livehelp_referers_monthly',"refer","","",$UNTRUSTED['whichdepartment']);
  print "</table>";
} else {
  $query = "SELECT count(*) as totalrows FROM livehelp_referers_monthly WHERE dateof=". intval($whatYm) ." AND directvisits!=0 AND department='".intval($UNTRUSTED['whichdepartment'])."' ORDER by directvisits DESC";
  ...
  $row = $sth->fetchRow(DB_FETCHMODE_ASSOC);
	...
	$query = "SELECT * FROM livehelp_referers_monthly WHERE dateof=". intval($whatYm) ." AND directvisits!=0  AND department='".intval($UNTRUSTED['whichdepartment'])."'  ORDER by directvisits DESC LIMIT " . intval($UNTRUSTED['offset']) . "," . intval($UNTRUSTED['perpage']);
  $visits_a = $mydatabase->query($query);
  $pageUrl = "data.php";
  $varstring = "&tab=".$UNTRUSTED['tab']."&month=".$UNTRUSTED['month'] . "&show=referer&month=".$UNTRUSTED['month'] . "&year=" . $UNTRUSTED['year'] . "&typeofview=" . $UNTRUSTED['typeofview'] . "&whichdepartment=" . intval($UNTRUSTED['whichdepartment']);
  ...
  $bgcolor="F0F1E1"; 
   while($visits = $visits_a->fetchRow(DB_FETCHMODE_ASSOC)){ 
     if($bgcolor==$color_alt2){$bgcolor=$color_alt1; } else { $bgcolor=$color_alt2; }
    print "<tr bgcolor=".$bgcolor."><td><a href=" . $visits['pageurl'] . " target=_blank>" . $visits['pageurl'] . "</a></td><td>" . $visits['directvisits'] . "</td><td> <a href=graph.php?item=" . $visits['recno'] . "&type=refer&typeof=directvisits&whichdepartment=".intval($UNTRUSTED['whichdepartment'])." target=_blank>" . $lang['graph'] . "</a></td></tr>";
   }
  print "</table>";
}
?>
