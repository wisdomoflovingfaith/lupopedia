...
function countDown(){ countDownTime--; if (countDownTime <=0){ countDownTime=countDownInterval; clearTimeout(counter) window.location.reload() return } if (document.all)   document.all.countDownText.innerText = countDownTime+" "; 
...
function showip(ip){
	  window.open('http://whatismyipaddress.com/ip/' + ip + '#Geolocation-Information', 'ip'+ip, 'width=500,height=400,menubar=no,scrollbars=1,resizable=1');
}
function startit(){ 
...
print "<b>User Agent :</b>" . $b->getUserAgent()  ."<br>"; 
print "<b>Browser :</b>" . $b->getBrowser()  ."<br>"; 
print "<b>Browser_Version :</b>". $b->getVersion()   ."<br>"; 
print "<b>Ip :</b><a href=javascript:showip('".$user_info['ipaddress']."')>" . $user_info['ipaddress'] . "</a> (<a href=javascript:showip('".$user_info['ipaddress']."')>Look up locaton</a>)<br>";
