...
function refreshnow(){
 window.location.replace("user_chat_flush.php?offset=<?php echo intval($UNTRUSTED['offset']); ?>&department=<?php echo $department; ?><?php echo $querystringadd; ?>");	
}
function printit(){
  ...
  if( ($buffer_html != "") || ($buffer_layer!="") ){
	...
	$query = "SELECT * FROM livehelp_users WHERE user_id=".intval($myid)." AND status='chat'";
        ...
        if($alive->numrows() == 0){                        	        	
        	...
        	<SCRIPT type="text/javascript">
        	function redirectme(){
                  window.parent.location.replace("wentaway.php?department=<?php echo intval($department); ?>");
                }
                redirectme();
        	</SCRIPT>
        	<?php
