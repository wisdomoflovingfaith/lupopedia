...
if($askquestions=="N"){
 ?>
    <SCRIPT type="text/javascript">
    window.location.replace("user_connect.php?try=0&tab=1&doubleframe=yes&pageurl=offline.php&department=<?php echo intval($UNTRUSTED['department']); ?>");       
    </SCRIPT>
 <?php
if(!($serversession))
  $mydatabase->close_connect();
  ...
  while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
     ...
     if( ($row['fieldtype'] == "username") || ($row['fieldtype'] == "textfield") || ($row['fieldtype'] == "email") ){	     	   
        ...
        if($indent){
        	  ...
        	  if(!(empty($UNTRUSTED[$fieldname])))
        	     print " value=\"".htmlspecialchars($UNTRUSTED[$fieldname], ENT_QUOTES, 'UTF-8')."\"";
      ...
      if($row['fieldtype'] == "textarea"){
         ...
         if(!(empty($UNTRUSTED[$fieldname]))) 
            print htmlspecialchars($UNTRUSTED[$fieldname], ENT_QUOTES, 'UTF-8');
