...
<?php 
   if(!(empty($UNTRUSTED['auto_focus']))){
	   if($UNTRUSTED['auto_focus']=="FIRST"){ print "\n setTimeout('shouldifocus()',15000);\n"; }
	 }
	?>
...
function refreshit(){
 if(document.chatter.typing.value=="no"){
  window.parent.connection.location="admin_connect.php?starttimeof=<?php echo intval($UNTRUSTED['starttimeof']); ?>";
...
if(empty($myuser)){
	?>
	<h2>User has closed the connection...</h2>
	this window will close in a few seconds....<a href=javascript:window.parent.closewindow()>click here to close now</a><br>
	<hr>
  <SCRIPT type="text/javascript">
		setTimeout('window.parent.closewindow()',1900);
		</script>
	<?php	
	exit;
}  
