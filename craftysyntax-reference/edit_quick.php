...
if(empty($UNTRUSTED['what'])){ $UNTRUSTED['what'] = ""; }if(empty($UNTRUSTED['ishtml'])){ $UNTRUSTED['ishtml'] = ""; } 
...
<a href=edit_quick.php?action=edit&typeof=<?php echo htmlspecialchars($UNTRUSTED['typeof'], ENT_QUOTES, 'UTF-8'); ?> ><b>+<?php echo $lang['CREATE']; ?></b></a>
<SCRIPT type="text/javascript">
window.focus();
</SCRIPT>
<?php
} else {
  if (empty($UNTRUSTED['id'])){     $UNTRUSTED['id'] = 0; 	$result = array();	$result['id'] = 0;	$result['name'] = "";	$result['visiblity'] = "";	$result['department'] = "";	$result['message'] = "";	$result['ishtml'] = "";  } else {
...
<input type=hidden name=typeof value="<?php echo htmlspecialchars($UNTRUSTED['typeof'], ENT_QUOTES, 'UTF-8'); ?>">
...
$query = "SELECT * FROM livehelp_departments ";
$data = $mydatabase->query($query);
$haystack = explode(",",$result['department']);
while( $row =  $data->fetchRow(DB_FETCHMODE_ASSOC) ){
   $needle = $row['recno'];	 
