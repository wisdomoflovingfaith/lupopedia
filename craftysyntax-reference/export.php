...
if($_GET['mark']==1){
	  $sql = "update livehelp_leads set Status='exported' where status='NEW'";
...
if($_GET['csv']==1){
 ...
 $data_questions = $mydatabase->query($sql);
  print "email,source,status,firstname,lastname,data\n<br>";
 
 while($row_questions = $data_questions->fetchRow(DB_FETCHMODE_ASSOC)){
 
  print $row_questions['email'].",".$row_questions['source'].",".$row_questions['status'].",".$row_questions['firstname'].",".$row_questions['lastname'].",'".substr($row_questions['data'],0,25)."'\n<br>";
   
 }
   exit;
} else {
	print " <a href=export.php?csv=1 target=_blank>Click here to download CSV</a> after downloading <a href=export.php?mark=1 target=_blank>click here to mark all new as exported</a> | <a href=leads.php> click here to return to leads.php</a>";
}
