...
<td>&nbsp;</td>
<td><b>Search:</b></td>
	<td><input type=text name=keywords size=15 value="<?php if(!(empty($UNTRUSTED['keywords']))) echo htmlspecialchars($UNTRUSTED['keywords'], ENT_QUOTES, 'UTF-8'); ?>" ><a href=javascript:document.dataformrefer.submit()><img src=images/go.gif width=20 height=20 border=0></a></td>
</tr>
...
<?php
 ...
 $query = "SELECT * FROM livehelp_keywords_monthly WHERE dateof=$whatYm $keywordssql AND department='". intval($UNTRUSTED['whichdepartment']) ."'  order by levelvisits DESC  LIMIT ". intval($UNTRUSTED['offset']) ."," . intval($perpage);
$data = $mydatabase->query($query);
