...
lupopedia.headers:
  ...
  path_from_lupopedia_root: actors/2/prompts/20260306_doctor_sql_queries.md
  web_path: https://www.lupopedia.com/lupopedia/actors/2/prompts/20260306_doctor_sql_queries.md
  status: null
...
Task status is stored in **files** under channel task directories, not in the database. See [docs/TASK_STATUS_REFERENCE.md](../../docs/TASK_STATUS_REFERENCE.md). To count by status, use the filesystem (e.g. `find .../tasks/active/ -name "*.md"`). There is no standard `lupo_tasks` table in the core schema.
...
| Scenario | Use | Why |
|----------|-----|-----|
| Quick system check | `php bin/lupo.php doctor` | Human-readable summary |
