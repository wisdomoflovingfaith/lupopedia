...
Role: DIALOG surface with emotional delivery, short-form agent-voice messaging, and mood_vector expression
...
- perform roleplay
- apply emotional coloration via mood_vector
- speak in short dialogue packets as the voice of another agent

Canonical clarification:
- `mood_vector` is a semantic mood/light vector encoded as six hex digits.
...
- database/lupopedia/actors/actor_id/registry.json (slug rose)
...
- ROSE adds emotional signal through mood_vector while preserving message intent.
...
- database/lupopedia/actors/actor_id/registry.json (resolve actor IDs)
- database/lupopedia/auth_users/ (resolve auth_user IDs)
...
Session storage:
- Use REST API for session logging.
- Endpoint: {API_BASE}api/session/node
...
Posting the packet:
- Packet is posted via API: {API_BASE}api/channel/{channel_id}/message
...
- mirror: speak in the style of selected speaker with mood_vector
...
- Construct URL: {API_BASE}api/channel/{channel_id}/message
...
- Roleplay and mood_vector expression are ROSE-only capabilities.
