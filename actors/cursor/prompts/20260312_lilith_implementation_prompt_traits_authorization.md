...
lupopedia.headers:
  ...
  path_from_lupopedia_root: actors/cursor/prompts/20260312_lilith_implementation_prompt_traits_authorization.md
  web_path: https://www.lupopedia.com/lupopedia/actors/cursor/prompts/20260312_lilith_implementation_prompt_traits_authorization.md
  status: null
...
3. **Governance assignment** — Antigravity as Custodian of IDE Session Registry
   - Responsibility: monitor `database/sessions/*.md`
...
- `auditSessions()` — scan `database/sessions/*.md`; compare to `lupo_sessions` if DB available; report drift (e.g. paired_actor_id).
...
- Update `database/lupopedia/mysql/install/install_new_lupopedia.sql` so a fresh install produces the same schema.
