...
lupopedia.headers:
  ...
  path_from_lupopedia_root: actors/cursor/prompts/20260310_cursor_thread_onboarding_4.0.69.md
  web_path: https://www.lupopedia.com/lupopedia/actors/cursor/prompts/20260310_cursor_thread_onboarding_4.0.69.md
  status: null
...
| 3 | **.cursor/rules/** | Root rules applied in Cursor (synced from `rules/root/*.md`). All 16 .mdc files; run `php scripts/propagate_agent_rules.php` after editing root rules. |
| 4 | **docs/doctrine/LUPOPEDIA_HEADERS/README.md** | Header format: `---` first, YAML blocks (`lupopedia.*`), then `---`, then identity line `# file: ...`, then body. Session block in `lupopedia.session`; session files at `database/sessions/{session_id}.md`. |
| 5 | **docs/doctrine/COMMUNICATION_DOCTRINE.md** | All communication uses **lupo_dialog_*** only (`lupo_dialog_channels`, `lupo_dialog_threads`, `lupo_dialog_messages`). No `lupo_threads` or `lupo_messages`. |
| 6 | **docs/doctrine/ActorFaucetOntology.md** | Actor = identity, rules, skills (e.g. Wolfie). Faucet = execution surface (Cursor, Kiro, OpenAI API). **IDE agents are faucets, not actors.** Fallback routes between faucets. `lupo_agent_faucets.faucet_class` = `ide` or `llm`. |
| 7 | **docs/doctrine/FallbackDoctrine.md** | Mandatory fallback invariant; rule_id 1003; seed `seed_fallback_rule_4.0.69.sql`. |
| 8 | **docs/doctrine/HumanActorIdDoctrine.md** | Human actors must have `actor_id >= 1000`. No AUTO_INCREMENT on `lupo_actors`; use `lupo_registry_open`. |
| 9 | **docs/HELP.md** | Communication system, CLI (whoami, doctor, threads, messages, send), rules/skills commands. |
...
- **Code:** `includes/Dialog/Database/DialogDatabase.php` and channel/CLI code use `lupo_dialog_messages` (not `dialog_doctrine`).

---

## 5. Root rules and Cursor rules

- **Root rules** (all IDE/code-writing agents): `rules/root/*.md` (17 files including README). Attached to actor_id 1 via `seed_actor_1_cursor_rules_4.0.68.sql` (meta_type `root_rule`, paths `rules/root/*.md`).
- **Cursor IDE rules:** `.cursor/rules/*.mdc` are synced **from** `rules/root/*.md`. After editing any file in `rules/root/`, run:  
...
- **Session files:** Stored at `database/sessions/{session_id}.md` (e.g. `L-LUPO-ROOT-CURSOR.md`).
...
| **Seeds** | `database/lupopedia/mysql/seed/` (registry, actors/agents, rules, skills, root rules, fallback rule, etc.) |
...
- `database/migrations/` — one-time SQL for existing DBs (e.g. drop duplicate tables, add `faucet_class`, record Human Actor ID doctrine). Consolidate into install SQL for clean installs; run migrations only when upgrading an existing DB.
...
- **Root rules index:** `rules/root/README.md`
- **Sync root → Cursor:** `php scripts/propagate_agent_rules.php`
- **CLI:** `php bin/lupo.php` (whoami, doctor, help, rules, skills, messages, send)
- **Table count:** From TOON files after `python scripts/generate_toon_files.py`; doc ceiling 199 tables, optimization trigger 200+.
