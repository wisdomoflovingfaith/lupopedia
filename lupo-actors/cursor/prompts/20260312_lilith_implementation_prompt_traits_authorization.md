...
lupopedia.headers:
  ...
  file_path_from_root: "lupo-prompts/cursor/20260312_lilith_implementation_prompt_traits_authorization.md"
...
lupopedia.edges:
  outbound_edges:
    - { to: "lupo-docs/status/ideas_on_actor_rolls_on_channels.md", type: "references", weight: 1.0 }
    - { to: "lupo-docs/status/lilith_suggestions_on_database_channels_semantic_organisation.md", type: "references", weight: 1.0 }
    - { to: "lupo-docs/status/DESIGN_NOTE_LUPO_ACTOR_TRAITS_4.0.69.md", type: "references", weight: 1.0 }
...
| **Single-install doctrine** | All schema for 4.0.x goes into `install_new_lupopedia.sql`; one-time migrations in `lupo-database/migrations/` and content consolidated into install. | New tables: add to install SQL. New columns: add via migration and add same columns to install SQL so fresh install matches. |
...
- Align with `lupo-scripts/validate_session_consistency.php` and SESSION_RECONCILIATION_DOCTRINE (no silent auto-correction by default).
...
- One-time migration file: `lupo-database/migrations/20260312_add_traits_edge_types_authorization.sql` (or similar). Idempotent where possible.
