...
**file_path_from_root:** rules/root/NO_COMPOSER_DOCTRINE.md  
**web_path:** http://www.lupopedia.com/rules/root/NO_COMPOSER_DOCTRINE.md  
...
Lupopedia MUST NOT use Composer, Packagist, or any external PHP package manager for dependency management. However, self-contained external libraries that are manually bundled in `includes/` are PERMITTED.
...
- External libraries placed in `includes/{library-name}/`
...
**Example:** PHPMailer is allowed because it's self-contained in `includes/PHPMailer/` and follows the rules in EXTERNAL_LIBRARIES_DOCTRINE.md.
...
|---------|---------------|-------------|
| `vendor/` directory | Composer dependency folder | Use `includes/` for all dependencies |
