---
lupopedia.rules:
  comment: "Offline Database Fallback and Filesystem Sync Doctrine"
  declares:
    - rule_id: "DB008"
...
1. **Filesystem as a Surrogate Database**
   ...
   - Files are stored categorically. Example: Dialog Messages for a channel mapped to `channels/0/threads/4.0.x/`.
   - Structural records like Actors or Agents fallback to flat CSV tables under `database/lupopedia/csv/`.
...
2. **Rehydration / Bi-Directional Sync**
   ...
   - Operations performed by agents directly inside the `channels/` or `database/lupopedia/csv/` directories must maintain strict formatting matching the database TOON schemas natively tracking that logic.

## Non-Negotiable Violations

- **Unstructured Fallback**: Dropping unstructured or malformed notes in a random temp directory instead of strictly mapped `channels/` or `database/` structures.
