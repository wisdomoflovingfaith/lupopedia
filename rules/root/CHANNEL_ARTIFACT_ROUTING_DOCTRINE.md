...
lupopedia.headers:
  ...
  path_from_lupopedia_root: rules/root/CHANNEL_ARTIFACT_ROUTING_DOCTRINE.md
  web_path: https://www.lupopedia.com/lupopedia/rules/root/CHANNEL_ARTIFACT_ROUTING_DOCTRINE.md
  status: null
...
**Numeric channel trees** (e.g. channel **42**) may exist under the **active** `channels/` tree when provisioned by the router/seed, or **only** under **`channels_before_4_0_93/{channel_id}/`** for pre–4.0.93 snapshots. Do not assume `channels/42/` exists in the active tree without checking; **§6–8 example links** point at **archive** paths where those files are preserved.

`docs/status/` is **not** the default sink for active channel work (archival / redirect only).

### 1.1 Human-readable thread tree (documentation coordination — 4.0.9x)

**In addition** to §1–3, **active** multi-agent **documentation** threads MAY use:

`channels/{federation_node_id}/{channel_key}/{thread_key}/`

Example: `channels/0/organization/prd_29_project_organization/` with `decisions/`, `questions/`, `answers/`, `comments/` per **`docs/prd/17_decisions_format.md`**. Register channels in **`channels/channel_index.md`**. Pre–4.0.93 on-disk trees are archived under **`channels_before_4_0_93/`** (read-only; see **`docs/prd/29_project_structure.md`**).

This layout does **not** replace §2–3 for **API-bound** thread posts that require a numeric `dialog_thread_id` folder under `threads/`; it is the preferred shape for **markdown-first** coordination that follows PRD 17 threading without mirroring `threads/{id}/` naming.
...
Reference implementation: `Lupo_Channel_Artifact_Validator` in `includes/classes/Lupo_Channel_Artifact_Validator.php`.

## 5. Enforcement

- API and router validate thread IDs before path generation.  
- Validators / `scripts/validate_channel_artifacts.py` flag malformed trees.  
...
- **Strategy reference:** [ATHENA thread-creation policy](../../channels_before_4_0_93/42/threads/1002/20260317_223020_athena_thread-creation-policy.md) (thread 1002; **archive**). **WOLFIE directive:** `20260317_224500_wolfie_thread-provisioning-option-a.md` (same thread).
- **Seed:** Coordination threads for channel 42 (e.g. 1001, 1002, 1004) SHOULD be present after seed — see `database/lupopedia/mysql/seed/seed_channel_42_dialog_threads_4.0.80.sql`.
...
- **Filesystem / CI:** `python scripts/validate_channel_artifacts.py --mode enforce` (strict filenames + review + help_response bodies). Path-level PHP: `ChannelArtifactValidator::validateThreadArtifact($path)`.
- **Reference review:** [LILITH channel system review, thread 1001](../../channels_before_4_0_93/42/threads/1001/20260317_223420_lilith_channel-system-review.md) (**archive**).
...
- **Naming:** `YYYYMMDD_HHIISS_hermes_prompt_{target_slug}_{purpose}.md` — see [prompts README](../../channels_before_4_0_93/42/prompts/README.md) (**archive**).
...
- **Human directive:** [20260317_230500 wisdomoflovingfaith channel-system directive](../../channels_before_4_0_93/42/threads/1001/20260317_230500_wisdomoflovingfaith_channel-system-docs-and-routing-directive.md) (**archive**).
