...
lupopedia.headers:
  ...
  path_from_lupopedia_root: rules/root/LUPOPEDIA_HEADERS_DOCTRINE.md
  web_path: https://www.lupopedia.com/lupopedia/rules/root/LUPOPEDIA_HEADERS_DOCTRINE.md
  status: null
  ...
  thread_key: headers-doctrine
  lupopedia.schema: doctrine
  prd_cluster: null
  title: Lupo rules root lupopediaheadersdoctrine
  summary: null
---
# file: Lupo rules root lupopediaheadersdoctrine — delegation: wolfie:root — web_path: http://www.lupopedia.com/lupopedia/rules/root/LUPOPEDIA_HEADERS_DOCTRINE.md
...
**Single source of truth:** This file (`rules/root/LUPOPEDIA_HEADERS_DOCTRINE.md`) is the **only** binding LUPOPEDIA HEADERS doctrine. The path `docs/doctrine/LUPOPEDIA_HEADERS/LUPOPEDIA_HEADERS_DOCTRINE.md` is a **stable alias** (redirect stub); it must not copy or diverge from this document.
...
| **Format + reversibility** | Block order, validators, export/import behavior: [`docs/doctrine/LUPOPEDIA_HEADERS/README.md`](../../docs/doctrine/LUPOPEDIA_HEADERS/README.md), [`HEADER_DB_REVERSIBILITY_DOCTRINE.md`](../../docs/doctrine/HEADER_DB_REVERSIBILITY_DOCTRINE.md). (Stable alias to this file: [`docs/.../LUPOPEDIA_HEADERS_DOCTRINE.md`](../../docs/doctrine/LUPOPEDIA_HEADERS/LUPOPEDIA_HEADERS_DOCTRINE.md) — pointer only.) |
| **Edge semantics** | Single edge table and types: [`EDGE_MODEL_DOCTRINE.md`](../../docs/doctrine/EDGE_MODEL_DOCTRINE.md). |
...
Edges may reference:
1. **File paths**: `"docs/prd/01_core_identity.md"`
...
| `file_path_from_root` | ✅ | relative path | `"rules/root/FILE.md"` |
...
| `alias` | Stable pointer / redirect stub (no duplicate binding text) | `docs/.../LUPOPEDIA_HEADERS_DOCTRINE.md` |
...
**4.0.89 release:** Prove round-trip on at least one representative `.md` (see `docs/versions/4.0.89/TODO.md` H7) — validators must accept the block; **`--check-db`** should not warn spuriously when file and DB match after import.
...
lupopedia.history:
  ...
  - event_id: 3
    ...
    affected_files: ["includes/classes/IdGenerator.php"]
...
| `rules/root/` | 0 |
| `channels/42/` | 1 |
| `content/federation_node_id/{id}/` | 2+ |
...
lupopedia.headers:
  ...
  file_path_from_root: "content/federation_node_id/4/doomemacs/README.md"
```
...
- `file_path_from_root` MUST be under `content/federation_node_id/{id}/` 
...
- Do NOT edit stale files directly without a **regeneration or semantic review** pass (see footer / THOTH doctrine in `docs/doctrine/LUPOPEDIA_HEADERS/VALIDATORS_AND_TOOLING.md`).
...
1. Ensure row exists: `python scripts/import_content.py <file.md>` or `python scripts/ensure_imported.py <file.md>`
2. Regenerate: `python scripts/generate_headers_from_db.py --file-path <repo-relative-path.md>`  
   (Use `--use-mock-db` only for offline stub behavior.)

### Batch / legacy tooling

- `scripts/regenerate_headers_for_stale_files.py` — batch helper for stale file refresh (may be file-centric; use DB-first flow above when the artifact is imported).
...
### Why
- Ensures all **in-scope** authored files comply with current doctrine (see [PRD 16 — Header applicability and scope](../../docs/prd/16_lupopedia_headers.md#header-applicability-and-scope); excludes binaries, generated exports, vendor trees)
...
```yaml
outbound_edges:
  - to: "rules/root/DATABASE_DOCTRINE.md"  # ✅ MUST exist
```
...
1. **DDL:** `database/lupopedia/mysql/install/install_new_lupopedia.sql` — full `CREATE TABLE` definitions (binding per [DATABASE_DOCTRINE.md](DATABASE_DOCTRINE.md)).
2. **Column oracle (generated):** `database/lupopedia/json/<table_name>.json` — one JSON file per table (e.g. `lupo_contents.json`, `lupo_metadata.json`, `lupo_edges.json`). These mirror install columns and are what tooling such as `import_content.py` uses for explicit insert column lists.  
...
Custom tooling must **not** invent `weight` / `reason` columns on `lupo_edges`. The canonical **read** path for regeneration is **`build_yaml_data_from_db`** in `scripts/lib/header_db_sync.py` (uses `fetch_header_edges` + `outbound_edges_from_db_rows`), which maps those columns back to YAML `weight` / `reason`. The canonical **write** path is **`sync_header_artifact_to_db`** in the same module (invoked from `import_content.py` after `lupo_contents` upsert).
...
**`content_id`** is required in the DB workflow; validators **warn** when it is missing on disk. Optional: `python scripts/validate_lupopedia_headers.py <file> --check-db` warns when the file declares edges or history but the database has no matching rows for that `content_id`.
...
### **Validation**
- `scripts/validate_lupopedia_headers_universal.py` — broad / strict checks where configured
- `scripts/validate_lupopedia_headers.py` — focused LUPOPEDIA HEADERS pass (includes **`content_id`** warning when missing)
- Shared logic: `scripts/lib/header_validation.py`

### **Import, ensure-imported, DB → file regeneration**
- `scripts/import_content.py` — upsert `lupo_contents`, write `content_id` into file, sync metadata + edges + optional `revision_history`
- `scripts/ensure_imported.py` — runs import when `content_id` absent
- `scripts/generate_headers_from_db.py` — regenerate YAML from DB (default live MySQL; `--use-mock-db` for stub)
- `scripts/lib/header_db_sync.py` — shared DB round-trip helpers (`sync_header_artifact_to_db`, `build_yaml_data_from_db`)

### **Batch / stale files**
- `scripts/regenerate_headers_for_stale_files.py` — batch refresh; prefer DB-first flow when artifacts are imported
...
|----------------|------|-------------|-------------------|
| < 2026 | Core Actor | `actors/<actor_id>/` | Department 0 auth_users only |
| ≥ 20260101010101 | Runtime Actor | `actors/YYYY/MM/<actor_id>/` | Department-scoped |

### Rules for Headers

1. **Core Actors** (actor_id < 2026):
   - Must have `learning_boundary: "Department 0 auth_users only"` in authority PRDs
   - Folder path is always `actors/<actor_id>/`
...
2. **Runtime Actors** (actor_id ≥ 20260101010101):
   ...
   - Folder path: `actors/YYYY/MM/<actor_id>/` (extracted from timestamp)
...
- Slug-only actor folder names under `actors/` (e.g. `wolfie/`, `lilith/`) are deprecated; registry `dir` uses `actors/{actor_id}/`
...
lupopedia.headers:
  lupopedia.schema: doctrine
  file_path_from_root: "rules/root/DATABASE_DOCTRINE.md"
  web_path: "http://www.lupopedia.com/lupopedia/rules/root/DATABASE_DOCTRINE.md"
  federation_node_id: 0
  # ... rest of fields
  artifact_type: doctrine
...
lupopedia.headers:
  ...
  file_path_from_root: "rules/root/INDEPENDENT_CODERS_MANIFESTO.md"
  web_path: "http://www.lupopedia.com/lupopedia/rules/root/INDEPENDENT_CODERS_MANIFESTO.md"
  federation_node_id: 0
  # ... rest of fields
...
lupopedia.headers:
  ...
  file_path_from_root: "includes/classes/IdGenerator.php"
  web_path: "http://www.lupopedia.com/lupopedia/includes/classes/IdGenerator.php"
  federation_node_id: 0
  # ... rest of fields
  artifact_type: class
  artifact_kind: code
...
- Core actors: Must be in `actors/<actor_id>/`
- Runtime actors: Must be in `actors/YYYY/MM/<actor_id>/`
...
- **[HEADER_DB_REVERSIBILITY_DOCTRINE.md](../../docs/doctrine/HEADER_DB_REVERSIBILITY_DOCTRINE.md)** — DB ↔ headers round-trip expectations
- **[EDGE_MODEL_DOCTRINE.md](../../docs/doctrine/EDGE_MODEL_DOCTRINE.md)** — `lupo_edges` authority
- **[docs/doctrine/LUPOPEDIA_HEADERS/README.md](../../docs/doctrine/LUPOPEDIA_HEADERS/README.md)** — Format overview and tooling index (same folder as the alias below)
- **[docs/doctrine/LUPOPEDIA_HEADERS/LUPOPEDIA_HEADERS_DOCTRINE.md](../../docs/doctrine/LUPOPEDIA_HEADERS/LUPOPEDIA_HEADERS_DOCTRINE.md)** — **Canonical pointer only** (duplicate path for bookmarks; content lives in **this** root file)
- **[validate_lupopedia_headers_universal.py](../../scripts/validate_lupopedia_headers_universal.py)** — Universal validator implementation
